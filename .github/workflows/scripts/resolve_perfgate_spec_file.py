...
def detect_chip_model_from_text(text: str) -> str:
    ...
    match = re.search(r"\bASCEND\s*910B([23])\b", text, flags=re.IGNORECASE)
    if match:
        return f"910B{match.group(1)}"
...
def resolve_spec_file(
    *,
    ...
    if explicit_spec_file:
        chip_model = explicit_chip_model or detect_chip_model_from_text(explicit_spec_file)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Resolve the PR perfgate same-spec file for the current Ascend runner."
    )
    parser.add_argument(
        "--explicit-spec-file",
    ...
    if args.benchmark_repo:
        ...
        if not spec_path.is_file():
            print(f"resolved perfgate spec file not found: {spec_path}", file=sys.stderr)
            return 2
