...
initialize_native_environment() {
  ...
  export MODELSCOPE_CACHE VLLM_ASSETS_CACHE VLLM_TEST_CACHE_ONLY
...
if is_native_runtime; then
  ...
  clear_ci_orchestration_env
  export VLLM_TEST_CACHE="${amd_native_test_cache}"
...
mkdir -p "${HF_CACHE}" "${MODELSCOPE_CACHE}" "${VLLM_TEST_CACHE_HOST}" "${VLLM_ASSETS_CACHE}"
