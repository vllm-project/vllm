...
if [[ $commands == *"--shard-id="* ]]; then
  ...
  if [[ "$commands" =~ ^(.*)"["(.*)"] && ["(.*)"]"$ ]]; then
      ...
      for i in "${!node0[@]}";do 
        ...
        #${prefix} && ../
        export commands="curl -fsSL https://get.docker.com -o get-docker.sh && sh get-docker.sh --version ${DCKR_VER} && rm get-docker.sh && nohup bash -c 'dockerd &' && sleep 5 && .buildkite/scripts/run-multi-node-test.sh ./tests 2 2 ${image_name} '${command_node_0}' '${command_node_1}'"
        echo "COMMANDS: ${commands}"
        docker run \
          --privileged \
          --device /dev/kfd \
