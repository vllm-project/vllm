...
input_path = './triton_topk_topp_test_20250927_222024.csv'
...
def get_filtered_data(df, vocab_size, p_val, k_val):
    ...
    filtered_df = df[(df['vocab_size'] == vocab_size) & p_condition
                     & k_condition].copy()
...
def create_speedup_plots():
    ...
    # Define the parameter combinations for columns
    column_configs = [{
        'p': None,
        'k': 200,
        'title': 'P=None, K=200'
    ...
    # Define vocab sizes for rows
    vocab_sizes = [4096, 65536, 102400, 128256]
    ...
    for row, vocab_size in enumerate(vocab_sizes):
        for col, config in enumerate(column_configs):
            ...
            if not data.empty:
                ...
                ax.set_ylim(
                    0.0, local_y_max)  # Set y-axis range from 0 to local max
                ...
                if len(data) > 0:
                    ...
                    ax.set_xlim(batch_sizes.min() * 0.8,
                                batch_sizes.max() * 1.2)
                ...
                ax.set_xlim(1, 2048)
                ax.set_ylim(
                    0.0, default_y_max)  # Set y-axis range from 0 to default
                ax.set_xscale('log', base=2)
                ax.grid(True, alpha=0.3)
...
def print_data_summary():
    ...
    print(
        f"Unique p values: {sorted([p for p in df['p'].unique() if p != 'nan'])}"
    )
    print(
        f"Unique k values: {sorted([k for k in df['k'].unique() if k != 'nan'])}"
    )
    print()

    # Check data availability for each configuration
    column_configs = [{
        'p': None,
        'k': 200,
        'title': 'P=None, K=200'
    ...
    vocab_sizes = [4096, 32768, 102400, 128256]
