...
| bs1 latency, fused | `recipe/dsv41/sra_vigil_tp4_8k1k_32k1k_bs1_fused.yaml` (TP4, `--max-model-len 40960`, 20 random prompts, concurrency 1, no speculative decoding, job 27872 on gb200-rack1-13, before the sentinel fix, which only adds a tail store to the prefill index kernel). TPOT mean/p50/p99: 8k/1k 6.46 / 6.43 / 6.54 ms, 32k/1k 6.48 / 6.46 / 6.59 ms; TTFT p50 229 ms and 540 ms. The 2026-09-10 unfused reference (sra-tracking `8490f6546`, job 27671) was 6.65 / 6.62 / 6.79 and 6.73 / 6.66 / 7.79 ms with TTFT 245 and 688 ms, so decode is about 3-4 % faster per token and prefill 6-22 % faster; a same-branch unfused control run is pending for a clean A/B. |
