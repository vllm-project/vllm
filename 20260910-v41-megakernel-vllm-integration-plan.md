...
| End-to-end (first pass) | The fused TP4 run loaded, captured graphs and generated, but produced garbage: under the `DeepseekV41ForConditionalGeneration` wrapper `AutoWeightsLoader` reports child-relative names (`model.layers.N…`) while the attention prefix is `language_model.model.layers.N.attn`, so the load-time permutation never ran. Fixed by matching on the `layers.<id>.attn` tail and by refusing to run a fused layer whose weights were not permuted (commit `5bcae3291`). Two environment fixes were also needed for the `LLM()` path: `VLLM_USE_BREAKABLE_CUDAGRAPH` auto-enable for the wrapper architecture (commit `f35a1cec9`) and `.venv/bin` on PATH for the `ninja` JIT step. |
| End-to-end (after the fix) | TP4, `fp8_ds_mla`, greedy, 48 tokens, four prompts. Unfused: "Paris.", a Rayleigh-scattering explanation, a Fibonacci definition, "391". Fused (commit `5bcae3291`): "Paris." (identical), a Rayleigh-scattering explanation, a Fibonacci spec, "391". Continuations diverge after the first tokens (different fp8 rounding / accumulation order under greedy decoding); both are coherent and factually right. Engine init with the fused layer: profiling, 75 s graph capture and FlashInfer autotune all pass. |
| End-to-end, `nvfp4_ds_mla` | Same TP4 smoke with the fused layer and `--kv-cache-dtype nvfp4_ds_mla` (SWA V4.1 fp8 528 B, compressed V4.1 fp4 288 B): engine init, graph capture and generation pass; outputs are coherent and right on all four prompts. KV budget at 8k context grows from 2.93-2.95 M to 4.04-4.08 M tokens (1.38x) versus `fp8_ds_mla`. Local 128-problem gsm8k after the sentinel fix (byte-identical prompts to the vigil eval): 0.883, all 128 stopping normally (fused fp8 0.938, unfused 0.852 on the same problems); a full 1319-problem nvfp4 run is in `recipe/dsv41/sra_vigil_tp4_gsm8k_fused_nvfp4.yaml`. Long-context accuracy of the fp4 compressed path is still unmeasured. |
| gsm8k, first fused run | TP4 vigil runs on the same branch and shim (`recipe/dsv41/sra_vigil_tp4_gsm8k_{fused,worktree_unfused}.yaml`, 1319 problems, 5-shot, greedy): unfused 0.889 flexible / 0.889 strict, eval 67 s; fused 0.229 / 0.228, eval 2088 s. 949 fused generations were 4096 tokens of empty text (argmax over NaN logits lands on the BOS token), 15 were byte garbage, 302 correct, 53 coherent but wrong. Root cause: `combine_topk_swa_indices` only writes the valid prefix of each row and relied on `torch.full(-1)` for the tail, but both prefill paths pass a reused workspace through `out=`, so slots past `combined_lens` held stale indices. `flash_mla_sparse_fwd` caps reads at `topk_length`; the upstream fused prefill documents that a stale valid index naming a NaN row still contaminates the output, and the gathered bf16 workspace is uninitialized past each request's `seq_len`. Fixed in `c772ee695` (kernel writes -1 sentinels to the tail; regression test `tests/kernels/test_dsv41_combine_indices.py`). |
| gsm8k, after the fix | Same fused recipe, job 27878 on gb200-rack1-08, commit `c772ee695`: 0.900 flexible / 0.899 strict on 1319 problems, 0 request errors, eval 73 s (unfused same branch: 0.889 / 0.889, 67 s; the main-branch reference at `984646a24` earlier today: 0.896 / 0.895). Local 128-problem reruns with byte-identical prompts: pre-fix fused 0.086 with 117 length-capped generations, post-fix fused 0.938 with all 128 stopping normally (unfused on those 128 in the cluster run: 0.852). |
| bs1 latency | `recipe/dsv41/sra_vigil_tp4_8k1k_32k1k_bs1_{fused,worktree_unfused}.yaml` (TP4, `--max-model-len 40960`, 20 random prompts, concurrency 1, no speculative decoding). TPOT mean/p50/p99: fused (job 27872, gb200-rack1-13) 8k/1k 6.46 / 6.43 / 6.54 ms, 32k/1k 6.48 / 6.46 / 6.59 ms, TTFT p50 229 / 540 ms; unfused same branch (job 27879, gb200-rack1-18) 8k/1k 6.20 / 6.14 / 6.60 ms, 32k/1k 6.24 / 6.17 / 6.58 ms, TTFT p50 230 / 591 ms. So at bs1 the fused layer is ~4 % slower per token on a different node (the 2026-09-10 morning reference of 6.65 / 6.73 ms at `8490f6546` predates other branch speedups and is not the right comparison). Kernel-only (`benchmarks/kernels/benchmark_dsv41_fused_attention.py`, eager): fused decode 25 / 28 / 50 / 155 µs at s_q 1 / 64 / 256 / 1024 vs split-KV attention 40 / 36 / 48 / 161 µs; fused prefill 30 / 163 µs at s_q 184 / 2123 vs 375 / 790 µs for the sparse_fwd pipeline. The kernel wins, so the bs1 regression is launch overhead: the fused eager region now holds `dsv41_q_layout`, the pybind op (three fresh allocations) and the DeepGEMM einsum in Python, whereas the base layer runs only `flash_mla_with_kvcache` eagerly and its whole o-projection inside the captured graph. Follow-up: give the eager region persistent `out_fp8` / `out_sf` buffers per layer and move the einsum into `_finish_o_proj`, or add `out=` to the upstream op; until then `dsv4_fused_attention` should stay opt-in for bs1-sensitive serving. |
| Pending | gsm8k parity (fused vs unfused, same branch) and bs1 latency via the `recipe/dsv41/*_fused.yaml` recipes, which use the config-shim checkpoint `/mnt/lustre/sra-weights/ckpt20260903-v41cfg` (origin/sra-tracking #64 renamed the hf_config fields; the shim must live on Lustre because `/mnt/data` is node-local); gpqa and a 32k-context task with `nvfp4_ds_mla`. |
