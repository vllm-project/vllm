...
| Pending | gsm8k/gpqa parity and bs1 latency via the `recipe/dsv41/*_fused.yaml` recipes (which use the config-shim checkpoint `/mnt/data/yongye/cache/ckpt20260903-v41cfg` because origin/sra-tracking #64 renamed the hf_config fields); `nvfp4_ds_mla` end-to-end smoke and its long-context accuracy. |
