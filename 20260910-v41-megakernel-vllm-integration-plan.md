...
| End-to-end, `nvfp4_ds_mla` | Same TP4 smoke with the fused layer and `--kv-cache-dtype nvfp4_ds_mla` (SWA V4.1 fp8 528 B, compressed V4.1 fp4 288 B): engine init, graph capture and generation pass; outputs are coherent and right on all four prompts ("Paris.", Rayleigh scattering, a `fib` spec, "391"). KV budget at 8k context grows from 2.95 M to 4.08 M tokens (1.38x) versus `fp8_ds_mla`. Long-context accuracy of the fp4 compressed path is still unmeasured. |
