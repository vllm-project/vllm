...
| Pending | TP4 end-to-end parity after the fix (fused vs unfused, `fp8_ds_mla` and `nvfp4_ds_mla`), gsm8k/gpqa and bs1 latency via the `recipe/dsv41/*_fused.yaml` recipes (which use the config-shim checkpoint `/mnt/data/yongye/cache/ckpt20260903-v41cfg` because origin/sra-tracking #64 renamed the hf_config fields). |
