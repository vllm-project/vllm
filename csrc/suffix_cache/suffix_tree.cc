  ...
  for (size_t i = 0; i < _active_nodes[seq_id].size(); ++i) {
    ...
    if (child == nullptr) {
      ...
      if (node->count == 1 && node != _root.get()) {
        // The active node has count = 1, which means the only suffix that ends
        // here is the one that's being extended right now. Then this node
        // should be a leaf node, and we can simply extend the length of this
        // node.
        ...
        // Either this is the root node, or the current suffix is not the only
        // one that ends here. Either case, we need to extend the current suffix
        // into a new child.
      ...
      // The active node has a child for the new token, and the child's count is
      // exactly one fewer than the active node's count. Since the suffix for
      // the active node ends here, that means all other suffixes that pass
      // through this node must go to that child.
      assert(node->children.size() ==
             1);  // The active node should have only one child.
      assert(node->endpoints.size() ==
             1);  // Only the current suffix should end here.
      if (child->length == 1) {
        // The child only has length 1. If we append the new token to the
        // current suffix, then it will perfectly overlap with the child. In
        // this case, we should just fuse the current suffix into the child and
        ...
        child->parent = parent;
        // Give ownership of child pointer to parent and should also free the
        // current node.
        ...
        // The child has length > 1. If we append the new token to the current
        // suffix, then it still does not reach the child node. In this case, we
        // keep both nodes but extend the length of the current node by 1 into
        // the child node.
        node->length += 1;
        node->endpoints[seq_id] += 1;
        node->ref_seq = seq_id;
      ...
      // There is a child for the new token, and should move the active node
      // into that child.
      if (child->length == 1) {
        // The child node has length 1, just update the active node pointer to
        // it.
        node->endpoints.erase(seq_id);
        child->count += 1;
        child->endpoints[seq_id] = static_cast<int>(_seqs[seq_id].size());
        ...
        // The child node has length > 1. If we extend the current suffix into
        // it, then it must be split into a segment of length 1 and another
        // segment with the remainder.
        ...
        new_node->ref_idx =
            static_cast<int>(_seqs[seq_id].size()) - new_node->length;
        // The child node's first token should be updated to its second token.
...
Candidate SuffixTree::speculate(const std::vector<int>& pattern,
                                int max_spec_tokens, float max_spec_factor,
                                float max_spec_offset, float min_token_prob,
                                bool use_tree_spec) {
  Candidate result;
...
std::string SuffixTree::_check_node_integrity(Node* node) {
  ...
  // Are all my children's counts less than mine? If equal, then we should have
  // been merged.
  for (const auto& [token, child] : node->children) {
    CHECK_OR_RETURN(child->count < node->count,
                    "internal node count is not greater than child count");
  }
  // Check my reference sequence and index.
  CHECK_OR_RETURN(_seqs.count(node->ref_seq),
                  "internal node has invalid ref_seq");
  ...
  for (auto [seq_id, end_idx] : node->endpoints) {
    CHECK_OR_RETURN(_seqs.count(seq_id),
                    "node endpoint refers to nonexistent sequence");
...
Candidate SuffixTree::_speculate_path(Node* node, int idx, int max_spec_tokens,
...
Candidate SuffixTree::_speculate_tree(Node* node, int idx, int max_spec_tokens,
                                      float min_token_prob) {
  Candidate ret;
  ...
  while (ret.token_ids.size() < max_spec_tokens && !queue.empty()) {
    ...
    if (item.idx < item.node->length) {
      ...
      for (const auto& kv : item.node->children) {
        ...
        float prob =
            item.prob * child->count / static_cast<float>(item.node->count);
