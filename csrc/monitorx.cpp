...
static PyObject *method_monitorx(PyObject *self, PyObject *args, PyObject *kwargs)
{
    Py_buffer buffer;
    PyObject *callback;
    double timeout = 0.;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "y*O|d", keywords, &buffer, &callback, &timeout))
    {
        return NULL;
    }
    ...
    while (1)
    {
        ...
        // Check timeout at most every 16 iterations to avoid clock_gettime and comparison cost
        if (have_timeout && (iteration & 15u) == 0)
        {
            ...
            const double elapsed = (double)(t_now.tv_sec - t_start.tv_sec) + (t_now.tv_nsec - t_start.tv_nsec) * 1e-9;
            if (elapsed >= timeout)
            {
                result = false;
                break;
        ...
        if (buffer_qualifies && state->cpu_support == CPU_SUPPORT_MONITORX)
        {
            ...
            // Check once more in case the buffer has been modified while we were arming the monitor hardware
...
{
    {"monitorx", (PyCFunction)method_monitorx, METH_VARARGS | METH_KEYWORDS, "Wait for store with callback"},
    {NULL, NULL, 0, NULL}
};
