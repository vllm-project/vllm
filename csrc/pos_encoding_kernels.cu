...
void rotary_embedding(
    ...
    torch::stable::Tensor& query,      // [batch_size, seq_len, num_heads *
                                       // head_size] or
  ...
  if (positions_ndim == 1) {
    STD_TORCH_CHECK(query.size(0) == positions.size(0) &&
                        (!key.has_value() || key->size(0) == positions.size(0)),
                    "query, key and positions must have the same number of "
                    "tokens");
  }
  if (positions_ndim == 2) {
    STD_TORCH_CHECK(
        ...
        if (is_neox) {
          vllm::rotary_embedding_kernel<scalar_t, true>
              <<<grid, block, 0, stream>>>(
                  ...
                  cos_sin_cache.const_data_ptr<scalar_t>(), rot_dim,
