...
__global__ void deepseekV4QNormRopeKernel(
    ...
    int64_t const* __restrict__ position_ids, // [N] i64
    float const* __restrict__ cos_sin_cache,  // [max_pos, 64] fp32
    float const eps, int const num_tokens, int const num_heads_q) {
#if (!defined(__CUDA_ARCH__) || __CUDA_ARCH__ < 800) && !defined(USE_ROCM)
  if constexpr (std::is_same_v<scalar_t_in, c10::BFloat16>) {
    return;
  } else {
    ...
    int const tokenIdx = globalWarpIdx / num_heads_q;
    int const headIdx = globalWarpIdx % num_heads_q;
    if (tokenIdx >= num_tokens) return;

    int const dim_base = laneId * kElemsPerLane;
    scalar_t_in const* src_ptr =
        q_inout +
        (static_cast<int64_t>(tokenIdx) * num_heads_q + headIdx) * kHeadDim +
        dim_base;
...
template <typename scalar_t_in>
__global__ void deepseekV4KVRopeQuantInsertKernel(
    scalar_t_in const* __restrict__ kv_in,     // [N, 512] bf16
    uint8_t* __restrict__ k_cache,             // [num_blocks, block_stride]
    int64_t const* __restrict__ slot_mapping,  // [num_tokens_insert] i64
    int64_t const* __restrict__ position_ids,  // [N] i64
...
void launchDeepseekV4QNormRopeTemplated(
  ...
  constexpr int kWarpsPerBlock = kBlockSize / 32;
  int64_t const total_warps =
      static_cast<int64_t>(num_tokens) * num_heads_q;
  int const grid =
      static_cast<int>((total_warps + kWarpsPerBlock - 1) / kWarpsPerBlock);
#ifndef USE_ROCM
  static int const sm_version = getSMVersion();
  TORCH_CHECK(sm_version >= 80,
              "deepseek_v4_qnorm_rope requires sm_80+ (Ampere or newer); got "
              "sm_",
              sm_version);
#endif
...
template <typename scalar_t_in>
void launchDeepseekV4KVRopeQuantInsert(
    scalar_t_in const* kv_in, uint8_t* k_cache,
    int64_t const* slot_mapping, int64_t const* position_ids,
    float const* cos_sin_cache, int const num_tokens_full,
    int const num_tokens_insert, int const cache_block_size,
    int const kv_block_stride, cudaStream_t stream) {
  constexpr int kBlockSize = 256;
  constexpr int kWarpsPerBlock = kBlockSize / 32;
  int const grid =
      static_cast<int>((num_tokens_insert + kWarpsPerBlock - 1) / kWarpsPerBlock);
#ifndef USE_ROCM
  static int const sm_version = getSMVersion();
  TORCH_CHECK(
      sm_version >= 80,
      "deepseek_v4_kv_rope_quant_insert requires sm_80+ (Ampere or newer); got "
      "sm_",
      sm_version);
#endif
  deepseekV4KVRopeQuantInsertKernel<scalar_t_in>
      <<<grid, kBlockSize, 0, stream>>>(
          kv_in, k_cache, slot_mapping, position_ids, cos_sin_cache,
          num_tokens_full, num_tokens_insert, cache_block_size, kv_block_stride);
}
...
void deepseek_v4_qnorm_rope(torch::Tensor& q,
  ...
  TORCH_CHECK(static_cast<int>(position_ids.size(0)) == q.size(0),
              "q/position_ids row counts must match");
  TORCH_CHECK(cos_sin_cache.dim() == 2 && cos_sin_cache.size(1) == 64,
              "cos_sin_cache shape [max_pos, 64]");
  TORCH_CHECK(cos_sin_cache.dtype() == torch::kFloat32,
              "cos_sin_cache must be float32");

  int const num_tokens = static_cast<int>(q.size(0));
  int const num_heads_q = static_cast<int>(q.size(1));
  if (num_tokens == 0 || num_heads_q == 0) return;

  at::cuda::OptionalCUDAGuard device_guard(device_of(q));
  auto stream = at::cuda::getCurrentCUDAStream();

  VLLM_DISPATCH_HALF_TYPES(q.scalar_type(), "deepseek_v4_qnorm_rope", [&] {
    using qkv_scalar_t = scalar_t;
    vllm::deepseek_v4_fused_ops::launchDeepseekV4QNormRope<qkv_scalar_t>(
        reinterpret_cast<qkv_scalar_t*>(q.data_ptr()),
        reinterpret_cast<int64_t const*>(position_ids.data_ptr()),
        cos_sin_cache.data_ptr<float>(), static_cast<float>(eps), num_tokens,
        num_heads_q, stream);
  });
}
...
void deepseek_v4_kv_rope_quant_insert(
  ...
  TORCH_CHECK(kv.dim() == 2 && kv.size(1) == 512, "kv shape [N, 512]");
  TORCH_CHECK(k_cache.dtype() == torch::kUInt8, "k_cache must be uint8");
  TORCH_CHECK(static_cast<int>(position_ids.size(0)) == kv.size(0),
              "kv/position_ids row counts must match");
  TORCH_CHECK(slot_mapping.size(0) <= kv.size(0),
  ...
  int const num_tokens_insert = static_cast<int>(slot_mapping.size(0));
  if (num_tokens_full == 0 || num_tokens_insert == 0) return;
  int const cache_block_size_i = static_cast<int>(cache_block_size);
  int const kv_block_stride = static_cast<int>(k_cache.stride(0));

  at::cuda::OptionalCUDAGuard device_guard(device_of(kv));
  auto stream = at::cuda::getCurrentCUDAStream();

  VLLM_DISPATCH_HALF_TYPES(
      kv.scalar_type(), "deepseek_v4_kv_rope_quant_insert", [&] {
        using qkv_scalar_t = scalar_t;
        vllm::deepseek_v4_fused_ops::
            launchDeepseekV4KVRopeQuantInsert<qkv_scalar_t>(
                reinterpret_cast<qkv_scalar_t const*>(kv.data_ptr()),
                reinterpret_cast<uint8_t*>(k_cache.data_ptr()),
                reinterpret_cast<int64_t const*>(slot_mapping.data_ptr()),
                reinterpret_cast<int64_t const*>(position_ids.data_ptr()),
                cos_sin_cache.data_ptr<float>(), num_tokens_full,
                num_tokens_insert, cache_block_size_i, kv_block_stride, stream);
      });
}
