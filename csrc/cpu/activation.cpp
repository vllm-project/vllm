...
void silu_and_mul(torch::Tensor& out, torch::Tensor& input) {
  ...
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "silu_and_mul_impl", [&] {
    ...
    activation_kernel<scalar_t, silu_act, true>(num_tokens, d, input.data_ptr<scalar_t>(),
                  ...
                  torch::Tensor& input)  // [..., 2 * d]
{
  ...
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "gelu_and_mul_impl", [&] {
    ...
    activation_kernel<scalar_t, gelu_act, true>(num_tokens, d, input.data_ptr<scalar_t>(),
                                                out.data_ptr<scalar_t>());
                       ...
                       torch::Tensor& input)  // [..., 2 * d]
{
  ...
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "gelu_tanh_and_mul_impl", [&] {
    ...
    activation_kernel<scalar_t, gelu_tanh_act, true>(num_tokens, d, input.data_ptr<scalar_t>(),
                                                     out.data_ptr<scalar_t>());
...
void gelu_new(torch::Tensor& out, torch::Tensor& input) {
  ...
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "gelu_new_impl", [&] {
    ...
    activation_kernel<scalar_t, gelu_new_act, false>(num_tokens, d, input.data_ptr<scalar_t>(),
                                                     out.data_ptr<scalar_t>());
...
void gelu_fast(torch::Tensor& out, torch::Tensor& input) {
  ...
  VLLM_DISPATCH_FLOATING_TYPES(input.scalar_type(), "gelu_fast_impl", [&] {
    ...
    activation_kernel<scalar_t, gelu_fast_act, false>(num_tokens, d, input.data_ptr<scalar_t>(),
                                                      out.data_ptr<scalar_t>());
