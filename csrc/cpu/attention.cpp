...
reduceSoftmaxAlibi(T *data, const int size, const int capacity,
                   ...
                   const int seqlen) {
  data[0] += alibi_slope * (start_index - seqlen + 1);
  T max = data[0];
  for (int i = 1; i < size; ++i) {
    T qk = data[i] + alibi_slope * (start_index + i - seqlen + 1);
    data[i] = qk;
    max = max >= qk ? max : qk;
  }
...
struct paged_attention_v1_impl {
  ...
  call(scalar_t *__restrict__ out,           // [num_seqs, num_heads, head_size]
       ...
       const int *__restrict__ seqlens, // [num_seqs]
    ...
    int max_seqlen = max_num_blocks_per_seq * BLOCK_SIZE;
    int max_seqlen_padded = (max_seqlen + 15) & 0xFFFFFFF0;
    TORCH_CHECK((max_seqlen_padded * sizeof(float)) % 64 == 0);

    const int parallel_work_item_num = omp_get_max_threads();

    size_t logits_bytes =
        parallel_work_item_num * max_seqlen_padded * sizeof(float);
    float *logits = (float *)std::aligned_alloc(
        64, logits_bytes); // Cacheline alignment for each context token.
                           // [parallel_work_item_num, max_seqlen_padded]

#pragma omp parallel for collapse(2) schedule(dynamic, 1)
    for (int seq_idx = 0; seq_idx < num_seqs; ++seq_idx) {
      for (int head_idx = 0; head_idx < num_heads; ++head_idx) {
        int seqlen = seqlens[seq_idx];
        const int *seq_block_table =
            block_tables + max_num_blocks_per_seq * seq_idx;
        const int block_num = (seqlen + BLOCK_SIZE - 1) / BLOCK_SIZE;
        const int64_t kv_head_idx = head_idx / num_queries_per_kv;
        const scalar_t *__restrict__ q_vec_ptr =
            q + seq_idx * q_stride + head_idx * HEAD_SIZE;
        const int last_block_token_num =
            seqlen - (block_num - 1) * BLOCK_SIZE;
        float *__restrict__ thread_block_logits =
            logits + omp_get_thread_num() * max_seqlen_padded;
        ...
        // Compute softmax
        if (alibi_slopes) {
          reduceSoftmaxAlibi(thread_block_logits, seqlen,
                             block_num * BLOCK_SIZE, alibi_slopes[head_idx], 0,
                             seqlen);
        } else {
          reduceSoftmax(thread_block_logits, seqlen,
...
#define LAUNCH_V1_ATTENTION_KERNEL(T, HEAD_SIZE, BLOCK_SIZE)                   \
  paged_attention_v1_impl<T, HEAD_SIZE, BLOCK_SIZE>::call(                     \
      ...
      block_tables_ptr, seqlens_ptr, max_num_blocks_per_seq,              \
...
void paged_attention_v1_impl_launcher(
    ...
    torch::Tensor &block_tables, torch::Tensor &seqlens,
    int max_seqlen, const c10::optional<torch::Tensor> &alibi_slopes) {
  ...
  int *seqlens_ptr = seqlens.data_ptr<int>();
...
#define CALL_V1_KERNEL_LAUNCHER(T, BLOCK_SIZE)                                 \
  paged_attention_v1_impl_launcher<T, BLOCK_SIZE>(                             \
      ...
      seqlens, max_seqlen, alibi_slopes);
...
void paged_attention_v1(torch::Tensor &out, torch::Tensor &query,
                        ...
                        torch::Tensor &seqlens, int block_size,
                        int max_seqlen,
...
#pragma omp parallel for collapse(3) schedule(static, 1)
    for (int seq_idx = 0; seq_idx < num_seqs; ++seq_idx) {
      for (int partition_idx = 0; partition_idx < max_num_partitions;
           ++partition_idx) {
        for (int head_idx = 0; head_idx < num_heads; ++head_idx) {
          const int seqlen = seqlens[seq_idx];
          const int start_token_idx = partition_idx * PARTITION_SIZE;

          if (start_token_idx >= seqlen)
            continue;

          const int partition_num =
              (seqlen + PARTITION_SIZE - 1) / PARTITION_SIZE;
          const bool no_reduce = (partition_num == 1);
          const int token_num =
              (std::min(seqlen, start_token_idx + PARTITION_SIZE) -
               start_token_idx);
          const int block_num =
          ...
          if (alibi_slopes) {
            max_and_sum = reduceSoftmaxAlibi(
                ...
                alibi_slopes[head_idx], start_token_idx, seqlen);
          } else {
...
#define LAUNCH_V2_ATTENTION_KERNEL(T, HEAD_SIZE, BLOCK_SIZE)                   \
  paged_attention_v2_impl<T, HEAD_SIZE, BLOCK_SIZE, PARTITION_SIZE>::call(     \
      ...
      seqlens_ptr, max_num_blocks_per_seq, alibi_slopes_ptr, q_stride,    \
...
void paged_attention_v2_impl_launcher(
    ...
    torch::Tensor &value_cache, int num_kv_heads, float scale,
    torch::Tensor &block_tables, torch::Tensor &seqlens, int block_size,
    int max_seqlen, const c10::optional<torch::Tensor> &alibi_slopes) {
  int num_seqs = query.size(0);
  int num_heads = query.size(1);
  int head_size = query.size(2);
...
#define CALL_V2_KERNEL_LAUNCHER(T, BLOCK_SIZE)                                 \
  paged_attention_v2_impl_launcher<T, BLOCK_SIZE>(                             \
      ...
      num_kv_heads, scale, block_tables, seqlens, block_size,             \
      max_seqlen, alibi_slopes);
