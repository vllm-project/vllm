...
template <typename T>
FORCE_INLINE std::pair<T, T> reduceSoftmax(T* data, const int size, const int capacity) {
  T max = data[0];
...
template <typename T>
FORCE_INLINE std::pair<T, T> reduceSoftmaxAlibi(T* data, const int size, const int capacity,
...
template <typename T>
FORCE_INLINE void reducePartitonSoftmax(const T* max_data, T* sum_data, const int size) {
  T max = max_data[0];
  for (int i = 1; i < size; ++i) {
...
struct reduceQKBlockKernel {
  ...
  FORCE_INLINE static void call(const scalar_t* __restrict__ q,
                                const scalar_t* __restrict__ k_block, float* __restrict__ logits,
                                float scale, const int token_num) {
          ...
          [&group_accums, logits, scale, token_group_idx](int token_idx) {
            float dot_v =
                group_accums[token_group_idx]
                    .template reduce_sub_sum<qk_acc_vec_type::get_elem_num() / TOKEN_PER_GROUP>(
                        token_idx);
...
struct paged_attention_v1_impl {
  static void call(scalar_t* __restrict__ out,            // [num_seqs, num_heads, head_size]
      ...
      for (int head_idx = 0; head_idx < num_heads; ++head_idx) {
        ...
        for (int block_idx = 0; block_idx < block_num; ++block_idx) {
          ...
          const scalar_t* __restrict__ k_block_cache_ptr =
              k_cache + physical_block_idx * kv_block_stride + kv_head_idx * kv_head_stride;
          float* __restrict__ head_block_logits = thread_block_logits + block_idx * BLOCK_SIZE;
        ...
        // Compute softmax
        if (alibi_slopes) {
          reduceSoftmaxAlibi(thread_block_logits, seq_len, block_num * BLOCK_SIZE,
                             alibi_slopes[head_idx], 0, seq_len);
        } else {
        ...
        for (int head_part_idx = 0; head_part_idx < head_partition_num; ++head_part_idx) {
          ...
          for (int block_idx = 0; block_idx < block_num; ++block_idx) {
            ...
            reduceValueBlock<scalar_t, HEAD_SIZE, BLOCK_SIZE, head_elem_num_per_partition>(
                prob_vec_ptr, v_block_cache_ptr, accums);

            if (block_idx != block_num - 1) {
              const int64_t next_physical_block_idx = seq_block_table[block_idx + 1];
              const scalar_t* __restrict__ next_v_block_cache_ptr =
                  v_cache + next_physical_block_idx * kv_block_stride +
                  kv_head_idx * kv_head_stride +
                  BLOCK_SIZE * head_part_idx * head_elem_num_per_partition;
              vec_op::unroll_loop<int, head_elem_num_per_partition>([&](int head_elem_idx) {
...
#define LAUNCH_V1_ATTENTION_KERNEL(T, HEAD_SIZE, BLOCK_SIZE)                                     \
  paged_attention_v1_impl<T, HEAD_SIZE, BLOCK_SIZE>::call(                                       \
      out_ptr, query_ptr, key_cache_ptr, value_cache_ptr, num_kv_heads, scale, block_tables_ptr, \
      seq_lens_ptr, max_num_blocks_per_seq, alibi_slopes_ptr, q_stride, kv_block_stride,         \
      kv_head_stride, num_seqs, num_heads);

template <typename T, int BLOCK_SIZE>
void paged_attention_v1_impl_launcher(torch::Tensor& out, torch::Tensor& query,
...
#define CALL_V1_KERNEL_LAUNCHER(T, BLOCK_SIZE)                                                 \
  paged_attention_v1_impl_launcher<T, BLOCK_SIZE>(out, query, key_cache, value_cache,          \
      ...
      for (int partition_idx = 0; partition_idx < max_num_partitions; ++partition_idx) {
        for (int head_idx = 0; head_idx < num_heads; ++head_idx) {
          ...
          for (int block_idx = 0; block_idx < block_num; ++block_idx) {
            ...
            const scalar_t* __restrict__ k_block_cache_ptr =
                k_cache + physical_block_idx * kv_block_stride + kv_head_idx * kv_head_stride;
            float* __restrict__ head_block_logits = logits + block_idx * BLOCK_SIZE;
          ...
          if (alibi_slopes) {
            max_and_sum = reduceSoftmaxAlibi(logits, token_num, block_num * BLOCK_SIZE,
                                             alibi_slopes[head_idx], start_token_idx, seq_len);
          } else {
            max_and_sum = reduceSoftmax(logits, token_num, block_num * BLOCK_SIZE);
          }
      ...
      for (int head_idx = 0; head_idx < num_heads; ++head_idx) {
        for (int group_idx = 0; group_idx < head_group_num; ++group_idx) {
          ...
          const scalar_t* __restrict__ seq_head_tmp_out =
              ...
              head_idx * max_num_partitions * HEAD_SIZE + group_idx * head_elem_num_per_group;
          scalar_t* __restrict__ seq_head_output = out + seq_idx * num_heads * HEAD_SIZE +
                                                   head_idx * HEAD_SIZE +
...
#define LAUNCH_V2_ATTENTION_KERNEL(T, HEAD_SIZE, BLOCK_SIZE)                               \
  paged_attention_v2_impl<T, HEAD_SIZE, BLOCK_SIZE, PARTITION_SIZE>::call(                 \
      out_ptr, exp_sums_ptr, max_logits_ptr, tmp_out_ptr, query_ptr, key_cache_ptr,        \
...
void paged_attention_v2_impl_launcher(torch::Tensor& out, torch::Tensor& exp_sums,
...
#define CALL_V2_KERNEL_LAUNCHER(T, BLOCK_SIZE)                                                \
  paged_attention_v2_impl_launcher<T, BLOCK_SIZE>(                                            \
      out, exp_sums, max_logits, tmp_out, query, key_cache, value_cache, num_kv_heads, scale, \
