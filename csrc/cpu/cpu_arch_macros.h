...
// x86_64
  ...
  #define FAST_SPINNING _mm_pause();
    ...
    // Implementations adapted from Arm Optimized Routines (SVE expf):
    // https://github.com/ARM-software/optimized-routines/blob/master/math/aarch64/sve/expf.c
    // and experimental SVE expf:
    // https://github.com/ARM-software/optimized-routines/blob/v26.01/math/aarch64/experimental/sve/sv_expf_inline.h
    // The fast_exp routine:
    // - approximates exp(r) - 1 as r + 0.5 r^2
    // - invokes a special case path for |x| > 87.3365
    // - special case path clamps x to -103.9721 and 88.7228 and offsets the
    // exponent by 23
    //   to keep the intermediate result of the SVE FEXPA instruction
    //   representable.
    // - the bounded x values cause the computation to underflow to 0 and
    // overflow to inf The maximum observed error is 0.88 + 0.5 ULP.
    //
    // The experimental fast_exp_f16 is intended for cases where outputs will be
    // downcasted to FP16 / BF16 (e.g. attention softmax).
    // Accurate within 1 ULP for FP16. Accurate within 1 ULP for BF16 for inputs
    // in [-87.345314, FLT_MAX] & clamps inputs < -87.345314 to zero.
    ...
    // - avoids special case code by clamping exp(x) to 0 for x < -87.345314 and
    // inf for x > 88.717422
    #define DEFINE_FAST_EXP                                                    \
      alignas(16) static constexpr float sve_expf_data[4] = {                  \
          0x1.62e4p-1f, 0x1.7f7d1cp-20f, 0.5f, 0x1.803f8p17f};                 \
      constexpr float sve_expf_shift = 0x1.803f8p17f;                          \
      constexpr float sve_expf_inv_ln2 = 0x1.715476p+0f;                       \
      constexpr float sve_expf_special_bound = 0x1.5d589ep+6f;                 \
      constexpr float sve_expf_inf_bound = 0x1.62e42fp6f;                      \
      constexpr float sve_expf_zero_bound = -0x1.9fe368p6f;                    \
          ...
          [&](svfloat32_t x, const svbool_t pg)                                \
              __attribute__((always_inline)) {                                 \
                ...
                const svfloat32_t z = svmad_x(                                 \
                    pg, svdup_n_f32(sve_expf_inv_ln2), x, sve_expf_shift);     \
                const svfloat32_t n = svsub_x(pg, z, sve_expf_shift);          \
      ...
      auto sve_expf_special_case =                                             \
          [&](svfloat32_t x, const svbool_t pg) __attribute__((noinline)) {    \
            ...
            x = svmin_x(pg, svmax_x(pg, x, sve_expf_zero_bound),               \
                        sve_expf_inf_bound);                                   \
            const svfloat32_t lane_constants =                                 \
                svld1rq(svptrue_b32(), sve_expf_data);                         \
            svfloat32_t z =                                                    \
                svmad_x(pg, svdup_n_f32(sve_expf_inv_ln2), x, sve_expf_shift); \
            const svfloat32_t n = svsub_x(pg, z, sve_expf_shift);              \
            svfloat32_t r = svmls_lane(x, n, lane_constants, 0);               \
            r = svmls_lane(r, n, lane_constants, 1);                           \
      ...
      auto sve_expf = [&](svfloat32_t x) __attribute__((always_inline)) {      \
        ...
        const svbool_t special = svacgt(pg, x, sve_expf_special_bound);        \
      ...
      alignas(16) static constexpr float sve_fexp_data[4] = {                  \
          0x1.62e43p-1f, 0x1.715476p+0f, 0x1.803f8p17f, 0.0f};                 \
      constexpr float sve_fexp_upper_bound = 0x1.62dea4p+6f;                   \
      constexpr float sve_fexp_lower_bound = -0x1.5d619ap+6f;                  \
      auto sve_exp_f16 = [&](svfloat32_t x, const svbool_t pg) __attribute__(( \
                             always_inline)) {                                 \
        const svfloat32_t lane_constants =                                     \
            svld1rq(svptrue_b32(), sve_fexp_data);                             \
        const svfloat32_t z =                                                  \
            svmla_lane(svdup_n_f32(sve_fexp_data[2]), x, lane_constants, 1);   \
        const svfloat32_t n = svsub_x(pg, z, sve_fexp_data[2]);                \
        const svfloat32_t r = svmls_lane(x, n, lane_constants, 0);             \
        const svfloat32_t scale = svexpa(svreinterpret_u32(z));                \
        svfloat32_t y = svmla_x(pg, scale, scale, r);                          \
        y = svsel_f32(svcmplt_f32(pg, x, svdup_n_f32(sve_fexp_lower_bound)),   \
                      svdup_n_f32(0.0f), y);                                   \
        y = svsel_f32(svcmpgt_f32(pg, x, svdup_n_f32(sve_fexp_upper_bound)),   \
                      svdup_n_f32(INFINITY), y);                               \
        return y;                                                              \
      };                                                                       \
      auto fast_exp_f16 = [&](const vec_op::FP32Vec16& vec) __attribute__((    \
                              always_inline)) {                                \
        const svbool_t pg = svptrue_b32();                                     \
        vec_op::FP32Vec16 result(vec_op::uninit);                              \
        vec_op::unroll_loop<int, vec_op::FP32Vec16::VEC_REG_NUM>([&](int i) {  \
          result.reg.val[i] = vec_op::FP32Vec16::VectorizedT(                  \
              sve_exp_f16(static_cast<svfloat32_t>(vec.reg.val[i]), pg));      \
        });                                                                    \
        return result;                                                         \
      };
  #else
