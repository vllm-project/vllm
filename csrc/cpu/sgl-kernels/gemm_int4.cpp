...
inline std::array<m256i_wrapper, 2> load_zps_4vnni(const int8_t* __restrict__ zps) {
  ...
  __m256i vzps_high = _mm256_set1_epi64x(*reinterpret_cast<const int64_t*>(zps + 8));
...
#define CALL_DEQUANT_GEMM_ACCUM_SMALL_M(M) \
  _dequant_gemm_accum_small_M<sym_quant_act, M, N, ldb>(C, A, scales_a, qzeros_a, B, scales_b, qzeros_b, K, lda, ldc);
#endif
...
void _dequant_gemm_accum(
  ...
  if (use_brgemm) {
    ...
    _dequant_and_store<sym_quant_act, N, true>(
  ...
  at::parallel_for(0, num_blocks, 1, [&](int64_t begin, int64_t end) {
    ...
    for (const auto i : c10::irange(begin, end)) {
      ...
      for (int mci = mc; mci < mc_end; ++mci) {
        ...
        store_out<BLOCK_N>(C_tmp, output + mci * block_m * N + nc * BLOCK_N, m_size, N /*lda*/);
      }
    }
    if (use_brgemm) {
...
at::Tensor int4_scaled_mm_cpu_with_quant(
  ...
  RECORD_FUNCTION("vllm::int4_scaled_mm_cpu_with_quant", std::vector<c10::IValue>({input, weight}));
  ...
  using Tin = typename ActDtype<sym_quant_act>::type;
  int64_t act_buffer_size = M_a * K_a +
      M_a * sizeof(float) +
      M_a * sizeof(int32_t);
...
#define LAUNCH_DA8W4_LINEAR_WITH_QUANT_IMPL(sym_quant_act)                                         \
  AT_DISPATCH_FLOATING_TYPES_AND2(                                                                 \
      at::ScalarType::BFloat16, at::ScalarType::Half, output_dtype, "int4_scaled_mm_cpu", [&] {    \
