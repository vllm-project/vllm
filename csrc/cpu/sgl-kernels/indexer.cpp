...
void fused_indexer_q_rope_quant_impl(
    ...
    int64_t index_q_stride0,
    int64_t index_q_stride1,
    const float* __restrict__ cos_sin_cache,  // [max_pos, rot_dim]
    int64_t cos_sin_stride,
    int64_t half_rot_dim,
    at::Float8_e4m3fn* __restrict__ index_q_fp8,  // [num_tokens, num_heads, head_dim]
    int64_t index_q_fp8_stride0,
    int64_t index_q_fp8_stride1,
    int64_t head_dim,
    const float* __restrict__ index_weights,  // [num_tokens, num_heads]
    int64_t index_weights_stride,
    float softmax_scale,
    float head_scale,
    float* __restrict__ index_weights_out,  // [num_tokens, num_heads]
    int64_t index_weights_out_stride,
    int64_t num_tokens,
    int64_t num_heads) {
  ...
  at::parallel_for(0, num_tokens * num_heads, 0, [&](int64_t begin, int64_t end) {
    ...
    for (int64_t idx = begin; idx < end; ++idx) {
      ...
      const float* const q = index_q + t * index_q_stride0 + h * index_q_stride1;
...
#if defined(CPU_CAPABILITY_AVX512)
      {
        ...
        for (int64_t i = 0; i < half_rot_dim; i += 16) {
          ...
          __m512 cos_b0 = _mm512_permutexvar_ps(dup_idx, _mm512_castps256_ps512(_mm256_loadu_ps(cos + i)));
          __m512 sin_b0 = _mm512_permutexvar_ps(dup_idx, _mm512_castps256_ps512(_mm256_loadu_ps(sin + i)));
          __m512 out0 = _mm512_fmadd_ps(
              xv0, cos_b0, _mm512_mul_ps(_mm512_permute_ps(xv0, 0xB1), _mm512_xor_ps(sin_b0, sign_mask)));
          // Match reference numerics: fp32 -> bf16 -> fp32 before the absmax.
          ...
          __m512 cos_b1 =
              _mm512_permutexvar_ps(dup_idx, _mm512_castps256_ps512(_mm256_loadu_ps(cos + i + 8)));
          __m512 sin_b1 =
              _mm512_permutexvar_ps(dup_idx, _mm512_castps256_ps512(_mm256_loadu_ps(sin + i + 8)));
...
#if defined(CPU_CAPABILITY_AVX512)
      {
        ...
        for (int64_t d2 = 0; d2 < head_dim; d2 += 16) {
          ...
          v = _mm512_min_ps(_mm512_max_ps(_mm512_mul_ps(v, vinv), vmin), vmax);
          _mm512_store_ps(scaled_buf, v);
          for (int64_t j = 0; j < 16; ++j) out[d2 + j] = at::Float8_e4m3fn(scaled_buf[j]);
        }
      }
#endif
...
void fused_indexer_q_rope_quant_cpu(
    at::Tensor& positions,
    at::Tensor& index_q,
  ...
  TORCH_CHECK(positions.dim() == 1 && positions.scalar_type() == at::kLong,
              "fused_indexer_q_rope_quant_cpu: positions must be a 1D int64 tensor");
  TORCH_CHECK(positions.is_contiguous(), "fused_indexer_q_rope_quant_cpu: positions must be contiguous");
  TORCH_CHECK(index_q.dim() == 3, "fused_indexer_q_rope_quant_cpu: index_q must be 3D");
  TORCH_CHECK(index_q.scalar_type() == at::kFloat,
              "fused_indexer_q_rope_quant_cpu: index_q must be float32 (convert once in Python)");
  TORCH_CHECK(index_q.stride(2) == 1,
              "fused_indexer_q_rope_quant_cpu: index_q must be contiguous in the head_dim axis");
  TORCH_CHECK(index_q_cos_sin_cache.dim() == 2 && index_q_cos_sin_cache.stride(1) == 1,
              "fused_indexer_q_rope_quant_cpu: index_q_cos_sin_cache must be a contiguous 2D tensor");
  TORCH_CHECK(index_q_cos_sin_cache.scalar_type() == at::kFloat,
  ...
  TORCH_CHECK(index_q_fp8.sizes() == index_q.sizes() &&
                  index_q_fp8.scalar_type() == at::kFloat8_e4m3fn && index_q_fp8.stride(2) == 1,
              "fused_indexer_q_rope_quant_cpu: index_q_fp8 must match index_q's shape, be "
              "float8_e4m3fn, and contiguous in the head_dim axis");
  TORCH_CHECK(index_weights.dim() == 2 && index_weights.stride(1) == 1,
              "fused_indexer_q_rope_quant_cpu: index_weights must be a contiguous-in-head 2D tensor");
  TORCH_CHECK(index_weights.scalar_type() == at::kFloat,
              "fused_indexer_q_rope_quant_cpu: index_weights must be float32 (convert once in Python)");
  TORCH_CHECK(index_weights_out.sizes() == index_weights.sizes() &&
                  index_weights_out.scalar_type() == at::kFloat && index_weights_out.stride(1) == 1,
              "fused_indexer_q_rope_quant_cpu: index_weights_out must match index_weights' shape, "
              "be float32, and contiguous in the head axis");
  TORCH_CHECK(
      positions.device().is_cpu() && index_q.device().is_cpu() &&
          index_q_cos_sin_cache.device().is_cpu() && index_q_fp8.device().is_cpu() &&
          index_weights.device().is_cpu() && index_weights_out.device().is_cpu(),
  ...
  TORCH_CHECK(head_dim % 16 == 0 && (head_dim - 2 * half_rot_dim) % 16 == 0 && half_rot_dim % 16 == 0,
              "fused_indexer_q_rope_quant_cpu: head_dim, head_dim-rot_dim, and rot_dim/2 "
