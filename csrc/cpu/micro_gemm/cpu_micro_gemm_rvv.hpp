...
template <>
FORCE_INLINE fixed_fp32x8_t load_row8_b_as_f32<c10::Half>(
    const c10::Half* ptr) {
...
template <>
FORCE_INLINE fixed_fp32x8_t load_row8_b_as_f32<c10::BFloat16>(
  ...
  auto shifted =
      RVVI(__riscv_vsll_vx_u32, LMUL_256)(wide, 16, RVV_MGEMM_N8);
  return RVVI4(__riscv_vreinterpret_v_u32, LMUL_256, _f32, LMUL_256)(
      shifted);
#endif
}
...
FORCE_INLINE void gemm_micro_rvv_fma_mx8_ku4(
    const scalar_t* __restrict__ a_ptr, const scalar_t* __restrict__ b_ptr,
    float* __restrict__ c_ptr, const int64_t lda, const int64_t ldc,
...
#define RVV_INIT_ACC(i)                                                \
  RVV_IF_M(i) {                                                        \
    if (accum_c) {                                                     \
      acc##i = RVVI(__riscv_vle32_v_f32, LMUL_256)(                   \
          c_ptr + (i) * ldc, RVV_MGEMM_N8);                            \
    } else {                                                           \
      acc##i = RVVI(__riscv_vfmv_v_f_f32, LMUL_256)(0.0f,             \
                                                   RVV_MGEMM_N8);      \
    }                                                                  \
  }
...
#undef RVV_FMA_ROW
  ...
  for (; k_idx < k; ++k_idx) {
    fixed_fp32x8_t b = load_row8_b_as_f32<scalar_t>(
        b_ptr + k_idx * RVV_MGEMM_B_GROUP_STRIDE);
...
FORCE_INLINE void gemm_micro_rvv_mx32_ku4(DEFINE_CPU_MICRO_GEMM_PARAMS) {
  ...
  gemm_micro_rvv_fma_mx8_ku4<M>(a_ptr, curr_b_1, c_ptr + 16, lda, ldc, k,
                                accum_c);
  gemm_micro_rvv_fma_mx8_ku4<M>(a_ptr, curr_b_1 + RVV_MGEMM_N8, c_ptr + 24,
                                lda, ldc, k, accum_c);
}
