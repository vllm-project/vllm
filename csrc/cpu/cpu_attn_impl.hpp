...
struct AttentionMetadata {
  ...
  int32_t cu_workitem_num_per_thread[1025] = {0};
  char _padding2[56];
...
struct AttentionInput {
  ...
  #define DEFINE_AVX512_EXP                                                    \
    ...
    auto avx512_exp_u20 = [&](vec_op::FP32Vec16& vec) __attribute__((          \
                              always_inline)) {                                \
    ...
    void apply_softmax(logits_buffer_t* __restrict__ logits_buffer,
      ...
      for (int32_t i = 0; i < q_head_num; ++i) {
        ...
        if (need_rescale) {
          ...
          rescale_factor_vec = avx512_exp_u20(rescale_factor_vec);
          rescale_factor = _mm512_cvtss_f32(rescale_factor_vec.reg);
#else
