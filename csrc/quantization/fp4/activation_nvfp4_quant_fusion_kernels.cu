   ...
   for (int rowIdx = blockIdx.x; rowIdx < numRows; rowIdx += gridDim.x) {
     if (colIdx < num_padded_cols) {
       ...
       if constexpr (CVT_FP4_PACK16) {
         ...
         ld256_cg_or_zero(
             reinterpret_cast<u32x8_t&>(in_vec2),
             &reinterpret_cast<const uint32_t*>(in)[inOffset2 * 8], valid);
       } else {
         ld128_cg_or_zero(
             reinterpret_cast<uint4&>(in_vec),
             &reinterpret_cast<const uint32_t*>(in)[inOffset * 4], valid);
         ld128_cg_or_zero(
             reinterpret_cast<uint4&>(in_vec2),
             &reinterpret_cast<const uint32_t*>(in)[inOffset2 * 4], valid);
       }
