...
void mxfp4_run_get_group_gemm_starts(
  ...
  __CALL_MXFP4_GET_STARTS_KERNEL(cutlass::float_e2m1_t, cutlass::float_ue8m0_t,
                                 torch::kBFloat16, cutlass::bfloat16_t,
                                 LayoutSFA, LayoutSFB, ScaleConfig)
  __CALL_MXFP4_GET_STARTS_KERNEL(cutlass::float_e2m1_t, cutlass::float_ue8m0_t,
                                 torch::kFloat16, half, LayoutSFA, LayoutSFB,
                                 ScaleConfig)
  else {
...
void run_mxfp4_blockwise_scaled_group_mm_sm100(
  ...
  TORCH_CHECK(status == cutlass::Status::kSuccess, "Failed to run MXFP4 GEMM");
}
...
TORCH_LIBRARY_IMPL_EXPAND(TORCH_EXTENSION_NAME, CUDA, m) {
  m.impl("cutlass_mxfp4_group_mm", &cutlass_mxfp4_group_mm);
}
