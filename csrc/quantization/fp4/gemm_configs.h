...
enum class MainloopScheduleType {
  AUTO  // Automatically selects between pingpong and cooperative schedules on
        // Hopper. On older architectures, this defaults to the "legacy" main
        // loop schedule.
};

enum class EpilogueScheduleType {
  AUTO  // Automatically chooses an epilogue schedule compatible with the
        // selected main loop schedule for Hopper. For architectures older than
        // hopper, the epilogue is always performed by the same thread block as
        // the main loop.
};
...
struct CutlassGemmConfig {
  ...
  CutlassTileConfigSM90 tile_config_sm90 =
      CutlassTileConfigSM90::ChooseWithHeuristic;
  CutlassTileConfigSM100 tile_config_sm100 =
      CutlassTileConfigSM100::ChooseWithHeuristic;
  ...
  CutlassGemmConfig(CutlassTileConfig tile_config, SplitKStyle split_k_style,
