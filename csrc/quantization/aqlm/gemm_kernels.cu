...
#pragma unroll
    ...
    if (threadIdx.x % 32 == 0) reinterpret_cast<__half*>(C)[c_gl_wr] = __float2half(res);
  }
}
...
#pragma unroll
      for (int i = 0; i < 8; i++) {
        ...
        half2* a1 = reinterpret_cast<half2*>(&sh_code1[8 * enc[2 * i + 1] + lane]);
#pragma unroll
        for (int j = 0; j < 4; j++) reinterpret_cast<half2*>(&chunk)[j] = __hadd2(a0[j], a1[j]);
        C[a_gl_rd * 8 + i] = chunk;
      }
    }
    a_gl_rd += 32;
...
void code1x16_matvec_cuda(const void* __restrict__ A, const void* __restrict__ B,
                          void* __restrict__ C, const void* __restrict__ codebook, int prob_m,
                          int prob_k, const int4 codebook_a_sizes, const int codebook_stride) {
  int sms;
...
void code1x16_matvec(
    const torch::Tensor& A, const torch::Tensor& B, torch::Tensor& C, const torch::Tensor& codebook,
    const int4 codebook_a_sizes  // cumulative sizes of A spanning each codebook, at most 3 long.
) {
...
torch::Tensor aqlm_dequant(const torch::Tensor& codes, const torch::Tensor& codebooks,
  ...
  auto weights =
      torch::empty({out_features, in_features},
                   torch::TensorOptions().dtype(codebooks.dtype()).device(codebooks.device()));

  if (nbooks == 1 && entries == (1 << 16)) {
    vllm::aqlm::code1x16_dequant_cuda(codes.data_ptr(), weights.data_ptr(), codebooks.data_ptr(),
                                      out_features, in_features, cumulative_sizes,
                                      vllm::aqlm::codebook_stride(codebooks));

    // if you wanted to flip to scaling the weights, (though it's 30%-ish slower and not consistent
    // with gemv implementation.) weights *= scales.index({"...", 0, 0});

    return weights;
  }

  if (nbooks == 2 && entries == (1 << 8)) {
    vllm::aqlm::code2x8_dequant_cuda(codes.data_ptr(), weights.data_ptr(), codebooks.data_ptr(),
                                     out_features, in_features, cumulative_sizes,
                                     vllm::aqlm::codebook_stride(codebooks));
