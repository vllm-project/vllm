...
struct ScaledQuantEpilogue {
 private:
  ...
  using ComputeB = cutlass::epilogue::fusion::Sm90Compute<
      cutlass::multiplies, float, float,
      cutlass::FloatRoundStyle::round_to_nearest>;

  using EVTComputeB =
      cutlass::epilogue::fusion::Sm90EVT<ComputeB, ScaleB, Accum>;

  // Define epilogue to multiply the output by scale_c
  using ScaleCDescriptor =
      cutlass::epilogue::collective::detail::RowBroadcastDescriptor<
          EpilogueDescriptor, float>;

  using ScaleC = cutlass::epilogue::fusion::Sm90RowOrScalarBroadcast<
      ScaleCDescriptor::Stages, typename EpilogueDescriptor::TileShape,
      typename ScaleCDescriptor::Element, Stride<Int<0>, Int<1>, Int<0>>>;
  ...
  using EVTComputeC =
      cutlass::epilogue::fusion::Sm90EVT<ComputeC, ScaleC, EVTComputeB>;
  ...
  using ComputeA = cutlass::epilogue::fusion::Sm90Compute<
      cutlass::multiplies, ElementD, float,
      cutlass::FloatRoundStyle::round_to_nearest>;
  using EVTComputeA =
      cutlass::epilogue::fusion::Sm90EVT<ComputeA, ScaleA, EVTComputeC>;

 public:
  using EVTCompute = EVTComputeA;
  using ArgumentType = typename EVTCompute::Arguments;
...
void cutlass_scaled_mm_dq_sm90(torch::Tensor& out, torch::Tensor const& a,
  ...
  if (a.dtype() == torch::kInt8) {
    ...
    if (out.dtype() == torch::kBFloat16) {
      return cutlass_scaled_mm_dispatcher<cutlass_3x_gemm<
          cutlass::float_e4m3_t, cutlass::bfloat16_t, ScaledDequantEpilogue,
      ...
      return cutlass_scaled_mm_dispatcher<cutlass_3x_gemm<
          cutlass::float_e4m3_t, cutlass::half_t, ScaledDequantEpilogue,
          TileShape, ClusterShape, KernelSchedule, EpilogueSchedule>>(
          out, a, b, a_scales, b_scales);
    }
  }
...
void cutlass_scaled_mm_qout_sm90(torch::Tensor& out, torch::Tensor const& a,
                                 torch::Tensor const& b,
                                 torch::Tensor const& a_scales,
                                 torch::Tensor const& b_scales,
  ...
  if (a.dtype() == torch::kInt8) {
    ...
    return cutlass_scaled_mm_dispatcher<
        cutlass_3x_gemm<int8_t, int8_t, ScaledQuantEpilogue, TileShape,
                        ClusterShape, KernelSchedule, EpilogueSchedule>>(
        out, a, b, a_scales, b_scales, c_scales);
  } else {
    TORCH_CHECK(a.dtype() == torch::kFloat8_e4m3fn);
    TORCH_CHECK(b.dtype() == torch::kFloat8_e4m3fn);
    ...
    return cutlass_scaled_mm_dispatcher<cutlass_3x_gemm<
        cutlass::float_e4m3_t, cutlass::float_e4m3_t, ScaledQuantEpilogue,
        TileShape, ClusterShape, KernelSchedule, EpilogueSchedule>>(
        out, a, b, a_scales, b_scales, c_scales);
  }
}
