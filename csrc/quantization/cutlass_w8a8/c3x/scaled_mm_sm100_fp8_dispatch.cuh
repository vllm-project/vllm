...
// Trait to check if the epilogue scheduler is a pointer array type
template <class T>
struct is_ptr_array_epilogue : std::false_type {};

template <>
struct is_ptr_array_epilogue<cutlass::epilogue::PtrArrayNoSmemWarpSpecialized1Sm> : std::true_type {};

template <>
struct is_ptr_array_epilogue<cutlass::epilogue::PtrArrayNoSmemWarpSpecialized2Sm> : std::true_type {};
...
struct cutlass_3x_gemm_fp8_blockwise {
  ...
  using EpilogueSchedulerType = EpilogueScheduler; // Add this to fix EpilogueScheduler error
};
...
void cutlass_gemm_caller_blockwise(torch::Tensor& out, torch::Tensor const& a,
  ...
  c_stride = cutlass::make_cute_packed_stride(StrideC{}, swap_ab ? cute::make_shape(n, m, 1) : cute::make_shape(m, n, 1));
  ...
  ElementD* c_ptr_array[1] = {c_ptr};

  typename GemmKernel::EpilogueArguments epilogue_args{{}, nullptr, c_stride, nullptr, c_stride};

  if constexpr (is_ptr_array_epilogue<typename Gemm::EpilogueSchedulerType>::value) {
    epilogue_args.ptr_D = c_ptr_array;
  } else {
    epilogue_args.ptr_D = c_ptr;
  }
  ...
  if (!swap_ab) {
    ...
    if (tile_m == 64) {
      if (use_tma_epilogue) {
        ...
        cutlass_gemm_caller_blockwise<cutlass_3x_gemm_fp8_blockwise<
            OutType, 1, TILE_N, TILE_K, Shape<_64, Int<TILE_N>, Int<TILE_K>>,
            Shape<_1, _1, _1>, cutlass::epilogue::PtrArrayNoSmemWarpSpecialized1Sm,
            cutlass::gemm::KernelTmaWarpSpecializedBlockwise1SmSm100>>(
            out, a, b, a_scales, b_scales);
      }
    } else if (tile_m == 128) {
      ...
      if (use_tma_epilogue) {
          ...
          cutlass_gemm_caller_blockwise<cutlass_3x_gemm_fp8_blockwise<
              OutType, 1, TILE_N, TILE_K, Shape<_256, Int<TILE_N>, Int<TILE_K>>,
            Shape<_2, _1, _1>, cutlass::epilogue::PtrArrayNoSmemWarpSpecialized2Sm,
            cutlass::gemm::KernelTmaWarpSpecializedBlockwise2SmSm100>>(
            out, a, b, a_scales, b_scales);
      }
    }
