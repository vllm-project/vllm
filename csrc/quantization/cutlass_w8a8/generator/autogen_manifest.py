...
DefaultCutlass3xArgsINT8 = Cutlass3xArgs(
    "int8", 90, (128, 128, 128), (2, 1, 1),
    "cutlass::gemm::KernelTmaWarpSpecializedPingpong",
    "cutlass::epilogue::TmaWarpSpecialized",
    "cutlass::gemm::PersistentScheduler",
    "cutlass::gemm::GemmUniversalMode::kGemm", "int32_t")
...
acc_types = ["float"]
acc_types_int = ["int32_t"]

## Make Cutlass3xArgsTest

Cutlass3xArgsTestFP8 = []
Cutlass3xArgsTestINT8 = []

for ts, cs, ks, es, tile_schedule, gm, at in product(
        tile_shapes, cluster_shapes, kernel_schedules_fp8, epilogue_schedules,
        tile_schedules, gemm_modes, acc_types):

    Cutlass3xArgsTestFP8.append(
...
for ts, cs, ks, es, tile_schedule, gm, at in product(
        ...
        tile_schedules, gemm_modes, acc_types_int):

    Cutlass3xArgsTestINT8.append(
        DefaultCutlass3xArgsINT8.with_tile_shape(ts).with_cluster_shape(cs).
        with_kernel_schedule(ks).with_epilogue_schedule(es).with_tile_schedule(
            tile_schedule).with_gemm_mode(gm).with_acc_type(at))

# Cutlass3xArgsTestFP8 = Cutlass3xArgsTestFP8[:5]
# Cutlass3xArgsTestFP16.append(DefaultCutlass3xArgsFP16)
# Cutlass3xArgsTestBF16.append(DefaultCutlass3xArgsBF16)