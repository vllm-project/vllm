...
def main(args):
    ...
    kernel_compiler: KernelCompiler = KernelCompiler(
        ...
        cache_write_only=args.cache_write_only)
    ...
    for idx in tqdm(range(0, len(cutlass_args_list), batch_size)):
        ...
        # fill-out ops.h
        KernelGenerator.write_ops(generator_type, args,
                                  generator_outputs.file_paths,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dtype-str",
                        required=True,
                        type=str,
                        choices=["int8", "fp8", "fp16", "bf16"],
                        help="Data type string. Example: fp8")
    parser.add_argument("--cache-write-only",
                        action='store_true',
                        help="Don't read from cache, only write to cache")
