...
class KernelCompileCache:
    ...
    def __init__(self, test_compile=False, write_only=False):
    ...
    def is_bad_kernel(self, kernel_file_name: str):
        if self.test_compile or self.write_only:
            return False
    ...
    def add(self, success: List[str], fail: List[str]):
        ...
        # Remove good kernels from bad kernels
        self.bad_kernels = list(set(self.bad_kernels).difference(set(self.good_kernels)))
...
class KernelCompiler:
    ...
    def init_compile_cache(self):
        self.cache = KernelCompileCache(self.test_compile, self.cache_write_only)
