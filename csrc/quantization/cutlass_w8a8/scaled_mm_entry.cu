...
void cutlass_scaled_mm_qout_sm89(torch::Tensor& c, torch::Tensor const& a,
                               ...
                               torch::Tensor const& b_scales,
                               torch::Tensor const& c_scales);
void cutlass_scaled_mm_qout_sm90(torch::Tensor& c, torch::Tensor const& a,
                               torch::Tensor const& b,
                               torch::Tensor const& a_scales,
                               torch::Tensor const& b_scales,
                               torch::Tensor const& c_scales);
...
void cutlass_scaled_mm_qout(torch::Tensor& c, torch::Tensor const& a,
                            torch::Tensor const& b,
                            torch::Tensor const& a_scales,
                            torch::Tensor const& b_scales,
                            torch::Tensor const& c_scales) {
  ...
  TORCH_CHECK(a_scales.numel() == 1 || a_scales.numel() == a.size(0));
  TORCH_CHECK(b_scales.numel() == 1 || b_scales.numel() == b.size(1));
  TORCH_CHECK(c_scales.numel() == 1 || c_scales.numel() == c.size(1));
  ...
  TORCH_CHECK(c.stride(0) % 16 == 0 &&
              b.stride(1) % 16 == 0);  // 16 Byte Alignment
  TORCH_CHECK(a_scales.is_contiguous() && b_scales.is_contiguous() &&
              c_scales.is_contiguous());

  at::cuda::OptionalCUDAGuard const device_guard(device_of(a));

  if (version_num >= 90) {
    // Hopper
    cutlass_scaled_mm_qout_sm90(c, a, b, a_scales, b_scales, c_scales);
    return;
  } else if (version_num == 89) {
    // Ada Lovelace
    cutlass_scaled_mm_qout_sm89(c, a, b, a_scales, b_scales, c_scales);
  } else if (version_num >= 80) {
    // Ampere
    cutlass_scaled_mm_qout_sm80(c, a, b, a_scales, b_scales, c_scales);
  } else {
    // Turing
    TORCH_CHECK(version_num >= 75);
    cutlass_scaled_mm_qout_sm75(c, a, b, a_scales, b_scales, c_scales);
  }
}
