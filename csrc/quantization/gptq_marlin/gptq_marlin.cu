...
#define __CALL_IF(NUM_BITS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, HAS_ACT_ORDER,     \
    ...
    cudaFuncSetAttribute(Marlin<NUM_BITS, NUM_THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS,          \
                                THREAD_K_BLOCKS, pipe_stages, HAS_ACT_ORDER, GROUP_BLOCKS>,       \
                         cudaFuncAttributeMaxDynamicSharedMemorySize, max_shared_mem);            \
    Marlin<NUM_BITS, NUM_THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, pipe_stages, \
           HAS_ACT_ORDER, GROUP_BLOCKS><<<blocks, NUM_THREADS, max_shared_mem, stream>>>(         \
...
torch::Tensor gptq_marlin_gemm(torch::Tensor& a, torch::Tensor& b_q_weight, torch::Tensor& b_scales,
  ...
  gptq_marlin::marlin_mm_f16i4(
      a.data_ptr(), b_q_weight.data_ptr(), c.data_ptr(), b_scales.data_ptr(), g_idx.data_ptr(),
      perm.data_ptr(), a_tmp.data_ptr(), size_m, size_n, size_k, workspace.data_ptr(), num_bits,
