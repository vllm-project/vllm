...
#ifdef INCLUDE_GPT_MARGIN_BFLOAT16

using B16_DTYPE = nv_bfloat16;
using B16_DTYPE2 = nv_bfloat162;
#define B162FLOAT __bfloat162float
#define FLOAT2B16 __float2bfloat16
#define B162B162 __bfloat162bfloat162
#define B16S2B162 __halves2bfloat162

#else


using B16_DTYPE = half;
using B16_DTYPE2 = half2;
#define B162FLOAT __half2float
#define FLOAT2B16 __float2half
#define B162B162 __half2half2
#define B16S2B162 __halves2half2

#endif



using FragA = Vec<B16_DTYPE2, 4>;
using FragB = Vec<B16_DTYPE2, 2>;
using FragC = Vec<float, 4>;
using FragS = Vec<B16_DTYPE2, 1>; // quantization scales
...
__device__ inline FragB dequant_4bit(int q) {
  ...
  frag_b[0] = __hfma2(*reinterpret_cast<B16_DTYPE2*>(&lo),
                      *reinterpret_cast<const B16_DTYPE2*>(&MUL),
                      *reinterpret_cast<const B16_DTYPE2*>(&ADD));
  frag_b[1] = __hfma2(*reinterpret_cast<B16_DTYPE2*>(&hi),
                      *reinterpret_cast<const B16_DTYPE2*>(&MUL),
                      *reinterpret_cast<const B16_DTYPE2*>(&ADD));
  return frag_b;
}
...
__device__ inline FragB dequant_8bit(int q) {
  ...
  FragB frag_b;
  frag_b[0] = __hsub2(*reinterpret_cast<B16_DTYPE2*>(&lo),
                      *reinterpret_cast<const B16_DTYPE2*>(&I8s_TO_F16s_MAGIC_NUM));
  frag_b[1] = __hsub2(*reinterpret_cast<B16_DTYPE2*>(&hi),
                      *reinterpret_cast<const B16_DTYPE2*>(&I8s_TO_F16s_MAGIC_NUM));
  return frag_b;
}
...
__device__ inline void scale(FragB &frag_b, FragS &frag_s, int i) {
  B16_DTYPE2 s = B162B162(reinterpret_cast<B16_DTYPE *>(&frag_s)[i]);
...
__device__ inline void scale4(FragB &frag_b, FragS &frag_s_1, FragS &frag_s_2,
  ...
  B16_DTYPE2 s_val_1_2;
  s_val_1_2.x = reinterpret_cast<B16_DTYPE *>(&frag_s_1)[i];
  s_val_1_2.y = reinterpret_cast<B16_DTYPE *>(&frag_s_2)[i];

  B16_DTYPE2 s_val_3_4;
  s_val_3_4.x = reinterpret_cast<B16_DTYPE *>(&frag_s_3)[i];
  s_val_3_4.y = reinterpret_cast<B16_DTYPE *>(&frag_s_4)[i];
...
__device__ inline void scale_float(float *c, FragS &s) {
  B16_DTYPE *s_ptr = reinterpret_cast<B16_DTYPE *>(&s);
  c[0] = __fmul_rn(c[0], B162FLOAT(s_ptr[0]));
  c[1] = __fmul_rn(c[1], B162FLOAT(s_ptr[1]));
}
...
__global__ void permute_cols_kernel(int4 const *__restrict__ a_int4_ptr,
  ...
  int row_stride = size_k * sizeof(B16_DTYPE) / 16;
  ...
  auto permute_row = [&](int row) {
    ...
    B16_DTYPE const *a_row_half =
        reinterpret_cast<B16_DTYPE const *>(a_int4_ptr + offset);
    B16_DTYPE *out_half = reinterpret_cast<B16_DTYPE *>(out_int4_ptr + offset);
...
#pragma unroll
            for (int j = 0; j < 2 * 4; j++) {
              reinterpret_cast<float *>(
                  ...
                  B162FLOAT(reinterpret_cast<B16_DTYPE *>(&c_red)[j]);
            }
          }
          if (!last) {
            int4 c;
#pragma unroll
            for (int j = 0; j < 2 * 4; j++) {
              reinterpret_cast<B16_DTYPE *>(&c)[j] =
                  FLOAT2B16(reinterpret_cast<float *>(
                      &frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)]);
            }
  ...
  auto write_result = [&]() {
    ...
    auto write = [&](int idx, float c0, float c1, FragS &s) {
      B16_DTYPE2 res = B16S2B162(FLOAT2B16(c0), FLOAT2B16(c1));
