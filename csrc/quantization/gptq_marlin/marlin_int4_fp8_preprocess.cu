...
__global__ void marlin_int4_fp8_preprocess_kernel_awq(
    ...
    int32_t size_n, int32_t size_k, int32_t group_size
) {
...
torch::Tensor marlin_int4_fp8_preprocess(
    ...
    TORCH_CHECK(qweight.scalar_type() == at::ScalarType::Int, "qweight.dtype != torch.int32");
    ...
    if (!qzeros_or_none.has_value()) {
        TORCH_CHECK(qweight.size(0) * qweight.size(1) * 8 % 256 == 0, "qweight.size(0) * qweight.size(1) * 8 % 256 != 0");
        ...
        TORCH_CHECK(qzeros.scalar_type() == at::ScalarType::Int, "qweight.dtype != torch.int32");
        TORCH_CHECK(device_of(qweight) == device_of(qzeros), "qzeros is not on the same device with qweight");

        int32_t group_size = qweight.size(0) / qzeros.size(0);
        TORCH_CHECK(qweight.size(1) == qzeros.size(1), "qweight.size(1) != qzeros.size(1)");
        TORCH_CHECK(qweight.size(0) % qzeros.size(0) == 0, "qweight.size(0) % qzeros.size(0) != 0");
