...
static __device__ __forceinline__ void mul_mat_q(
    ...
    half* __restrict__ dst, const int ncols_x, const int nrows_x,
...
#pragma unroll
      for (int ids0 = 0; ids0 < mmq_x; ids0 += nwarps * QI8_1) {
        const int ids =
            (ids0 + threadIdx.y * QI8_1 + threadIdx.x / (WARP_SIZE / QI8_1)) %
            mmq_x;
...
#pragma unroll
    for (int i = 0; i < mmq_y; i += WARP_SIZE) {
      ...
      dst[col_dst * nrows_dst + row_dst] =
          __float2half(sum[i / WARP_SIZE][j / nwarps]);
    }
  }
}
