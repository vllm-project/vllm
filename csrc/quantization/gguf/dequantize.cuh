// Dequant functions
static __device__ __forceinline__ void dequantize_q4_0(const void* vx,
                                                       const int ib,
                                                       const int iqs,
                                                       dfloat2& v) {
...
static __device__ __forceinline__ void dequantize_q4_1(const void* vx,
                                                       const int ib,
                                                       const int iqs,
                                                       dfloat2& v) {
...
static __device__ __forceinline__ void dequantize_q5_0(const void* vx,
                                                       const int ib,
                                                       const int iqs,
                                                       dfloat2& v) {
...
static __device__ __forceinline__ void dequantize_q5_1(const void* vx,
                                                       const int ib,
                                                       const int iqs,
                                                       dfloat2& v) {
...
static __device__ __forceinline__ void dequantize_q8_0(const void* vx,
                                                       const int ib,
                                                       const int iqs,
                                                       dfloat2& v) {
...
static __global__ void dequantize_block_q2_K(const void* __restrict__ vx,
...
static __global__ void dequantize_block_q3_K(const void* __restrict__ vx,
  ...
  for (int l = l0; l < l0 + 4; ++l)
    y[l] = __hmul(dl, __int2half_rn((int8_t)((q[l] >> shift) & 3) -
                                    ((hm[l] & m) ? 0 : 4)));
}
...
static __global__ void dequantize_block_q5_K(const void* __restrict__ vx,
  ...
  uint8_t hm = 1 << (2 * il);
  y[0] = __hsub(
      __hmul(d1, __int2half_rn((ql[0] & 0xF) + (qh[0] & hm ? 16 : 0))), m1);
  y[1] = __hsub(
      __hmul(d1, __int2half_rn((ql[1] & 0xF) + (qh[1] & hm ? 16 : 0))), m1);
  hm <<= 1;
  y[32] = __hsub(
      __hmul(d2, __int2half_rn((ql[0] >> 4) + (qh[0] & hm ? 16 : 0))), m2);
  y[33] = __hsub(
      __hmul(d2, __int2half_rn((ql[1] >> 4) + (qh[1] & hm ? 16 : 0))), m2);
}
...
static __global__ void dequantize_block_q6_K(const void* __restrict__ vx,
  ...
  y[64] = __hmul(
      d, __int2half_rn(sc[4] *
                       ((int8_t)((ql[0] >> 4) | (((qh >> 4) & 3) << 4)) - 32)));
  y[96] = __hmul(
      d, __int2half_rn(
...
static __global__ void dequantize_block_iq2_xxs(const void* __restrict__ vx,
  ...
  for (int j = 0; j < 8; ++j)
    y[j] = __float2half(d * grid[j] * (signs & kmask_iq2xs[j] ? -1.f : 1.f));
}
...
static __global__ void dequantize_block_iq2_s(const void* __restrict__ vx,
  ...
  dst_t* y = yy + i * QK_K + 32 * ib + 8 * il;
  const uint8_t* grid =
      (const uint8_t*)(iq2s_grid + (x[i].qs[4 * ib + il] |
                                    ((x[i].qh[ib] << (8 - 2 * il)) & 0x300)));
  const float d = __half2float(x[i].d) *
                  (0.5f + ((x[i].scales[ib] >> 4 * (il / 2)) & 0xf)) * 0.25f;
...
static __global__ void dequantize_block_iq3_xxs(const void* __restrict__ vx,
  ...
  for (int j = 0; j < 4; ++j) {
    y[j + 0] =
        __float2half(d * grid1[j] * (signs & kmask_iq2xs[j + 0] ? -1.f : 1.f));
    y[j + 4] =
        __float2half(d * grid2[j] * (signs & kmask_iq2xs[j + 4] ? -1.f : 1.f));
  }
}
...
static __global__ void dequantize_block_iq3_s(const void* __restrict__ vx,
  ...
  const uint8_t* grid1 =
      (const uint8_t*)(iq3xs_grid + (qs[2 * il + 0] |
                                     ((x[i].qh[ib] << (8 - 2 * il)) & 256)));
  const uint8_t* grid2 =
      (const uint8_t*)(iq3xs_grid + (qs[2 * il + 1] |
                                     ((x[i].qh[ib] << (7 - 2 * il)) & 256)));
  const float d = __half2float(x[i].d) *
                  (0.5f + ((x[i].scales[ib / 2] >> 4 * (ib % 2)) & 0xf)) * 0.5f;
...
static __global__ void dequantize_block_iq1_s(const void* __restrict__ vx,
  ...
  const int8_t* grid =
      (const int8_t*)(iq1s_grid + (x[i].qs[i8] | ((h & 8) << 5)));
...
static __global__ void dequantize_block_iq4_xs(const void* __restrict__ vx,
  ...
  const float d =
      __half2float(x[i].d) * ((((x[i].scales_l[ib / 2] >> 4 * (ib % 2)) & 0xf) |
...
static void dequantize_block_cuda(const void* __restrict__ vx,
                                  dst_t* __restrict__ y, const int k,
                                  cudaStream_t stream) {
  const int num_blocks = (k + 2 * CUDA_DEQUANTIZE_BLOCK_SIZE - 1) /
                         (2 * CUDA_DEQUANTIZE_BLOCK_SIZE);
  dequantize_block<qk, qr, dequantize_kernel>
