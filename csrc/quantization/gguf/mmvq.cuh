...
static __device__ __forceinline__ float vec_dot_iq1_s_q8_1(
  ...
  const int* grid1 =
      (const int*)(iq1s_grid + (bq1->qs[4 * ib32 + 0] | ((h1 & 0x08) << 5)));
  const int* grid2 =
      (const int*)(iq1s_grid + (bq1->qs[4 * ib32 + 1] | ((h1 & 0x80) << 1)));
  const int* grid3 =
      (const int*)(iq1s_grid + (bq1->qs[4 * ib32 + 2] | ((h2 & 0x08) << 5)));
  const int* grid4 =
      (const int*)(iq1s_grid + (bq1->qs[4 * ib32 + 3] | ((h2 & 0x80) << 1)));
  for (int j = 0; j < 2; ++j) {
...
static __global__ void mul_mat_vec_q(const void* __restrict__ vx,
                                     ...
                                     half* __restrict__ dst, const int ncols,
                                     const int nrows) {
...
static void mul_mat_vec_iq4_nl_q8_1_cuda(const void* vx, const void* vy,
  ...
  const dim3 block_nums(block_num_y, 1, 1);
  const dim3 block_dims(WARP_SIZE, GGML_CUDA_MMV_Y, 1);
  mul_mat_vec_q<QK4_NL, QI4_NL, block_iq4_nl, VDR_Q4_0_Q8_1_MMVQ,
                vec_dot_iq4_nl_q8_1>
      <<<block_nums, block_dims, 0, stream>>>(vx, vy, dst, ncols, nrows);
}
