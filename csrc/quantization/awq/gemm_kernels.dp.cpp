                                       ...
                                       int OC, sycl::half *__restrict__ C)
{
  ...
  float C_warp[32];
  __shared__ sycl::half A_shared[16 * (32 + 8)];
  __shared__ sycl::half B_shared[32 * (128 + 8)];
  
  __shared__ sycl::half scaling_factors_shared[128];
  __shared__ sycl::half zeros_shared[128];
...
// Why * 1 in the above line?
  ...
  sycl::half* A_shared_ptr = A_shared 
                    + ((int)threadIdx.y) * row_stride_warp * (32 + 8) 
                    + (((int)threadIdx.x) / (32 / 8)) * (32 + 8)
                    + (((int)threadIdx.x) % (32 / 8) ) * 8;

  sycl::half* B_shared_ptr = B_shared
...
void gemm_forward_4bit_cuda_m16n64k32(int G, int split_k_iters,
                                      sycl::half *__restrict__ A,
                                      int *__restrict__ B,
                                      sycl::half *__restrict__ scaling_factors,
                                      int *__restrict__ zeros, int M, int IC,
    ...
    int split_k_iters)
{
    ...
    if (num_out_channels % 128 == 0)
    {
        ...
        sycl::range<3> num_blocks(
            1, 1, (num_out_feats + 16 - 1) / 16 * j_factors1 * split_k_iters);
        // threadIdx.x: 32
        // threadIdx.y: i_factors[2] * j_factors[2]
        sycl::range<3> threads_per_block(1, 2, 32);
        /*
        DPCT1049:0: The work-group size passed to the SYCL kernel may exceed the
        limit. To get the device limit, query info::device::max_work_group_size.
        Adjust the work-group size if needed.
        */
        stream->parallel_for(sycl::nd_range<3>(num_blocks * threads_per_block,
                                               threads_per_block),
                             [=](sycl::nd_item<3> item_ct1) {
                                 vllm::awq::gemm_forward_4bit_cuda_m16n128k32(
                                     group_size, split_k_iters, in_feats,
                                     kernel, scaling_factors, zeros,
                                     num_in_feats, num_in_channels,
    ...
    else if (num_out_channels % 64 == 0)
    {
        ...
        sycl::range<3> num_blocks(1, 1,
                                  1 * (num_out_feats + 16 - 1) / 16 *
