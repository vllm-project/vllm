  ...
  for (int _k_0_0 = 0; _k_0_0 < k_bound; ++_k_0_0) {
    ...
    */
    ...
    for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < N / 16; ++ax0_ax1_fused_0) {
      ...
      // 272)) +
      // ((((int)threadIdx.x) >> 4) * 136)) + ((((int)threadIdx.x) & 15) * 8)))
      // = *(uint4*)(B +
      // ((((((k_0_0 * 163840) + (ax0_ax1_fused_0 * 20480)) +
      // (((int)threadIdx.y) * 10240)) +
      // ((((int)threadIdx.x) >> 4) * 5120)) + (((int)blockIdx_y) * 128)) +
      // ((((int)threadIdx.x) & 15) * 8))); row stride in shared memory: (NWARPS
      // * 32 * 8 / cta_N)
      uint32_t B_loaded =
