...
__device__ void rms_norm_dynamic_per_token_symmetric_quant_vec(
...
// RMS norm + symmetric quant kernel
template <typename scalar_t, typename scalar_out_t, bool has_residual = false>
__global__ void rms_norm_dynamic_per_token_symmetric_quant_kernel(
    scalar_out_t* __restrict__ out,       // [..., hidden_size]
    float* __restrict__ scales,           // [num_tokens]
    scalar_t const* __restrict__ input,   // [..., hidden_size]
    scalar_t const* __restrict__ weight,  // [hidden_size]
...
// Residual add + RMS norm + symmetric dynamic per token
template <typename scalar_in_t>
void rms_norm_dynamic_per_token_symmetric_quant_dispatch(
...
void rms_norm_dynamic_per_token_asymmetric_quant_dispatch(
  ...
  if (residual.has_value()) {
    VLLM_DISPATCH_QUANT_TYPES(
        out.scalar_type(), "rms_norm_dynamic_per_token_asymmetric_quant_kernel", [&] {
          vllm::rms_norm_dynamic_per_token_asymmetric_quant_kernel<scalar_in_t, scalar_t,
                                                        true>
              <<<grid, block, 0, stream>>>(
                  out.data_ptr<scalar_t>(), scales.data_ptr<float>(), azps->data_ptr<int32_t>(),
                  input.data_ptr<scalar_in_t>(), weight.data_ptr<scalar_in_t>(),
                  scale_ub.has_value() ? scale_ub->data_ptr<float>() : nullptr,
                  var_epsilon, min_scaling_factor, hidden_size,
                  residual->data_ptr<scalar_in_t>());
    ...
    VLLM_DISPATCH_FLOATING_TYPES(
        input.scalar_type(), "rms_norm_dynamic_per_token_asymmetric_quant_dispatch", [&] {
          rms_norm_dynamic_per_token_asymmetric_quant_dispatch<scalar_t>(
              out, input, weight, scales, var_epsilon, scale_ub, residual, azps);
        });
  } else {
    VLLM_DISPATCH_FLOATING_TYPES(
