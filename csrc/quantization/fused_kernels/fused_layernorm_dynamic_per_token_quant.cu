...
__device__ void rms_norm_dynamic_per_token_quant_vec(
    ...
    weight_t const* __restrict__ weight,  // [hidden_size]
  ...
  vllm::vectorized::compute_dynamic_per_token_scales<
      scalar_t, weight_t, scalar_out_t, has_residual>(
  ...
  if constexpr (std::is_same_v<scalar_out_t, int8_t>) {
    ...
    vllm::vectorized::norm_and_quant<scalar_t, weight_t, scalar_out_t, true,
                                     has_residual>(out, input, weight, rms,
                                                   &token_scale, hidden_size,
                                                   input_stride, residual);
  } else {
    // FP8 - Do not invert token_scale for exact match with FBGemm
    vllm::vectorized::norm_and_quant<scalar_t, weight_t, scalar_out_t, false,
                                     has_residual>(out, input, weight, rms,
                                                   &token_scale, hidden_size,
                                                   input_stride, residual);
  }
...
__global__ void rms_norm_dynamic_per_token_quant_kernel(
  ...
  if (can_vectorize) {
    return rms_norm_dynamic_per_token_quant_vec<scalar_t, weight_t,
                                                scalar_out_t, has_residual>(
  ...
  vllm::compute_rms<scalar_t, has_residual>(
      &rms, input, hidden_size, input_stride, var_epsilon, residual);
  // Compute Scale
  vllm::compute_dynamic_per_token_scales<scalar_t, weight_t, scalar_out_t,
                                         has_residual>(
      &token_scale, scales, input, weight, rms, scale_ub, hidden_size,
      input_stride, residual);
  ...
  if constexpr (std::is_same_v<scalar_out_t, int8_t>) {
    token_scale = 1.0f / token_scale;
    vllm::norm_and_quant<scalar_t, weight_t, scalar_out_t, true, has_residual>(
        out, input, weight, rms, &token_scale, hidden_size, input_stride,
        residual);
  } else {
    // FP8 - Do not invert s_token_scale for exact match with FBGemm
    vllm::norm_and_quant<scalar_t, weight_t, scalar_out_t, false, has_residual>(
        out, input, weight, rms, &token_scale, hidden_size, input_stride,
        residual);
  }
}
...
__global__ void rms_norm_per_block_quant_kernel(
  ...
  vllm::vectorized::compute_dynamic_per_token_scales<
      scalar_t, weight_t, scalar_out_t, has_residual, is_scale_transposed,
  ...
  vllm::vectorized::norm_and_quant<
      scalar_t, weight_t, scalar_out_t, std::is_same_v<scalar_out_t, int8_t>,
...
template <typename scalar_in_t, typename weight_t>
  ...
  VLLM_DISPATCH_BOOL(residual.has_value(), has_residual, [&] {
    VLLM_DISPATCH_QUANT_TYPES(
        out.scalar_type(), "rms_norm_dynamic_per_token_quant_kernel", [&] {
          vllm::rms_norm_dynamic_per_token_quant_kernel<scalar_in_t, weight_t,
                                                        scalar_t, has_residual>
              <<<grid, block, 0, stream>>>(
                  out.data_ptr<scalar_t>(), scales.data_ptr<float>(),
                  input.data_ptr<scalar_in_t>(), weight.data_ptr<weight_t>(),
...
void rms_norm_dynamic_per_token_quant(
  ...
  TORCH_CHECK(weight.scalar_type() == input.scalar_type() ||
              ...
              "rms_norm_dynamic_per_token_quant expects weight dtype to match "
              "input or be fp32. got input dtype=",
              input.scalar_type(), ", weight dtype=", weight.scalar_type());
  if (weight.scalar_type() == torch::kFloat32) {
    TORCH_CHECK(input.scalar_type() == torch::kFloat16 ||
                    input.scalar_type() == torch::kBFloat16,
                "rms_norm_dynamic_per_token_quant only supports fp16/bf16 "
  ...
  if (weight.scalar_type() == torch::kFloat32) {
    VLLM_DISPATCH_FLOATING_TYPES(
        input.scalar_type(), "rms_norm_dynamic_per_token_quant_dispatch", [&] {
          rms_norm_dynamic_per_token_quant_dispatch<scalar_t, float>(
              out, input, weight, scales, var_epsilon, scale_ub, residual);
        });
  } else {
    VLLM_DISPATCH_FLOATING_TYPES(
        input.scalar_type(), "rms_norm_dynamic_per_token_quant_dispatch", [&] {
          rms_norm_dynamic_per_token_quant_dispatch<scalar_t, scalar_t>(
              out, input, weight, scales, var_epsilon, scale_ub, residual);
        });
  }
}
        ...
        VLLM_DISPATCH_QUANT_TYPES(
            out.scalar_type(), "rms_norm_per_block_quant_kernel", [&] {
              vllm::rms_norm_per_block_quant_kernel<
                  scalar_in_t, weight_t, scalar_t, has_residual,
...
void rms_norm_per_block_quant(torch::Tensor& out, torch::Tensor const& input,
  ...
  TORCH_CHECK(weight.scalar_type() == input.scalar_type() ||
                  weight.scalar_type() == torch::kFloat32,
              "rms_norm_per_block_quant expects weight dtype to match input "
              "or be fp32. got input dtype=",
              input.scalar_type(), ", weight dtype=", weight.scalar_type());
  if (weight.scalar_type() == torch::kFloat32) {
    TORCH_CHECK(input.scalar_type() == torch::kFloat16 ||
                    input.scalar_type() == torch::kBFloat16,
                "rms_norm_per_block_quant only supports fp16/bf16 input with "
  ...
  if (weight.scalar_type() == torch::kFloat32) {
    VLLM_DISPATCH_FLOATING_TYPES(
        input.scalar_type(), "rms_norm_per_block_quant_fp_dispatch", [&] {
          rms_norm_per_block_quant_dispatch<scalar_t, float>(
    ...
    VLLM_DISPATCH_FLOATING_TYPES(
        input.scalar_type(), "rms_norm_per_block_quant_fp_dispatch", [&] {
          rms_norm_per_block_quant_dispatch<scalar_t, scalar_t>(
              out, input, weight, scales, group_size, var_epsilon, scale_ub,
              residual, is_scale_transposed);
        });
  }
