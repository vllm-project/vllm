...
// warp-shuffle reduction -- no shared memory and no __syncthreads. kWarpsPerBlock
// groups are packed into one 128-thread block.
//
...
__global__ void __launch_bounds__(kWarpsPerBlock * 32)
  ...
  // Elements per thread: 2 for group_size=64, 4 for group_size=128. Each maps to
  // a single vector load/store per lane (4B/8B in, 2B/4B out).
  ...
  // num_groups is no longer gridDim.y (we pack kWarpsPerBlock groups per block),
  // so recover it from the compile-time group_size.
  ...
  // owns the EPT contiguous elements at group_start + lane_id * EPT, so the warp
  // reads the whole group as one fully-coalesced wide load for gate and for up.
  ...
  scalar_t const* token_input_gate = input + token_idx * input_stride + lane_base;
...
#pragma unroll
  ...
  // Step 2: per-group abs-max via warp-shuffle. fmaxf is order-invariant, so the
  // group max (and therefore the scale) is bit-identical to the tree reduction.
#pragma unroll
...
#pragma unroll
  for (int k = 0; k < EPT; ++k) {
    out_v.q[k] =
        vllm::ScaledQuant<scalar_out_t, false>::quant_fn(result[k], group_scale);
  }
...
void silu_and_mul_per_block_quant(torch::Tensor& out,
  ...
  // One warp per group; vllm::kWarpsPerBlock groups packed per 128-thread block.
