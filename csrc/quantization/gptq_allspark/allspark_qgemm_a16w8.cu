...
torch::Tensor allspark_a16w8_gemm(
  ...
  TORCH_CHECK_NOT_IMPLEMENTED(
      false, "allspark_a16w8_gemm(..) requires CUDA_ARCH >= 8.0");
 ...
 * FType is half or bfloat16, QType is uint8
...
struct GmemTile_A16W8_PerC_MtilexNtilex32_multistage_SM8x_SplitK {
  ...
  __device__ GmemTile_A16W8_PerC_MtilexNtilex32_multistage_SM8x_SplitK(
      const SM8x_GEMM_A16W8_Splitk_Params<FType, QType>& k_params,
  ...
  const SM8x_GEMM_A16W8_Splitk_Params<FType, QType>& params;
};
...
void ampere_hgemm_A16W8_perc_f16_f16_MtilexNtilex32_mma16816_multistage_AN_BTN32K16_CN_splitk(
  ...
  if (enable_fuse) {
    ...
    CHECK_CUDA(cudaMemsetAsync(red_count, 0, grid_x * grid_y * sizeof(uint32_t),
                               stream));
    SM8x_GEMM_A16W8_Splitk_Params<FType, QType> params{
...
size_t allspark_qgemm_a16w8_perc_n32k16_ampere_workspace_size(
...
template <typename FT, typename QT>
void a16w8_gemm_dq_cublas(const FT* in, const QT* rhs_qdata_ptr,
  ...
  restore_N32_K16_dequantize_rhs_a16w8(rhs_qdata_ptr, rhs_scales_ptr,
...
template <typename FType, typename QType>
void allspark_qgemm_a16w8_perc_ampere(
    const FType* A, const QType* B, const FType* B_scale, const FType* B_zero,
  ...
  if (M > CUBLAS_M_THRESHOLD) {
    a16w8_gemm_dq_cublas<FType, QType>(A, B, B_scale, B_zero, C, workspace, M,
...
torch::Tensor allspark_a16w8_gemm(
  ...
  if (m > CUBLAS_M_THRESHOLD) {
    ...
    ws_size = allspark::allspark_qgemm_a16w8_perc_n32k16_ampere_workspace_size(
  ...
  if (a.dtype() == at::ScalarType::Half) {
    allspark::allspark_qgemm_a16w8_perc_ampere<__half, uint8_t>(
    ...
    allspark::allspark_qgemm_a16w8_perc_ampere<__nv_bfloat16, uint8_t>(
...
TORCH_LIBRARY_IMPL_EXPAND(TORCH_EXTENSION_NAME, CUDA, m) {
  m.impl("allspark_a16w8_gemm", &allspark_a16w8_gemm);
}