...
torch::Tensor marlin_gemm(torch::Tensor& a, torch::Tensor& b_q_weight,
  ...
  int actual_size_n =
      (b_q_weight.size(1) / marlin_dense::tile_size) * marlin_dense::pack_factor_4bit;
  TORCH_CHECK(
  ...
  TORCH_CHECK(
      size_n % marlin_dense::min_thread_n == 0,
      "size_n = " + str(size_n) +
          ", is not divisible by min_thread_n = " + str(marlin_dense::min_thread_n));
