...
__global__ void Marlin_24(
  ...
  #pragma unroll
  for (int i = 0; i < m_sh_iters; i++) meta_ptr[i] = meta + m_gl_rd_delta_i * i + m_gl_rd;
  ...
  #pragma unroll
      for (int i = 0; i < m_sh_iters; i++) {
        if (m_sh_wr_pred) cp_async4(&sh_meta_stage[m_sh_wr_delta * i + m_sh_wr], meta_ptr[i]);
  ...
  auto global_reduce = [&](bool first = false, bool last = false) {
    ...
    if (threadIdx.x < active_threads) {
      ...
      int c_gl_wr =
          2 * c_gl_stride * (threadIdx.x % 4) + 8 * (threadIdx.x / 32) + (threadIdx.x % 32) / 4;
  ...
  #pragma unroll
              for (int j1 = 0; j1 < 4; j1++) {
                reinterpret_cast<__half*>(&c)[(j2 * 4 + j1)] = __float2half(
                    reinterpret_cast<float*>(&frag_c)[4 * 2 * 4 * (i / 4) + 8 * j1 + 2 * j2 +
                                                      4 * ((i % 4) / 2) + i % 2]);
              }
            }
  ...
  auto write_result = [&]() {
    ...
    auto write = [&](int idx, float c0, float c1, float c2, float c3, FragS& s0, float c4, float c5,
                     float c6, float c7, FragS& s1) {
      uint2 res[2];
  ...
  #pragma unroll
      for (int i = 0; i < thread_m_blocks; i++) {
        int wr = c_sh_wr;
        write(wr, frag_c[i][0][0][0], frag_c[i][1][0][0], frag_c[i][2][0][0], frag_c[i][3][0][0],
              frag_s[0][0], frag_c[i][0][0][2], frag_c[i][1][0][2], frag_c[i][2][0][2],
              frag_c[i][3][0][2], frag_s[0][2]);
        write(wr + c_sh_stride, frag_c[i][0][0][1], frag_c[i][1][0][1], frag_c[i][2][0][1],
              frag_c[i][3][0][1], frag_s[0][0], frag_c[i][0][0][3], frag_c[i][1][0][3],
              frag_c[i][2][0][3], frag_c[i][3][0][3], frag_s[0][2]);
        write(wr + 4 * c_sh_stride_2, frag_c[i][0][1][0], frag_c[i][1][1][0], frag_c[i][2][1][0],
              frag_c[i][3][1][0], frag_s[0][0], frag_c[i][0][1][2], frag_c[i][1][1][2],
              frag_c[i][2][1][2], frag_c[i][3][1][2], frag_s[0][2]);
        write(wr + 4 * c_sh_stride_2 + c_sh_stride, frag_c[i][0][1][1], frag_c[i][1][1][1],
              frag_c[i][2][1][1], frag_c[i][3][1][1], frag_s[0][0], frag_c[i][0][1][3],
              frag_c[i][1][1][3], frag_c[i][2][1][3], frag_c[i][3][1][3], frag_s[0][2]);
  ...
  #pragma unroll
          for (int i = 0; i < thread_m_blocks; i++) {
            scale_floats(&frag_c[i][0][0][0], &frag_c[i][1][0][0], &frag_c[i][2][0][0],
                         &frag_c[i][3][0][0], frag_s[0][0], &frag_c[i][0][0][2],
                         &frag_c[i][1][0][2], &frag_c[i][2][0][2], &frag_c[i][3][0][2],
                         frag_s[0][2]);

            scale_floats(&frag_c[i][0][0][1], &frag_c[i][1][0][1], &frag_c[i][2][0][1],
                         &frag_c[i][3][0][1], frag_s[0][0], &frag_c[i][0][0][3],
                         &frag_c[i][1][0][3], &frag_c[i][2][0][3], &frag_c[i][3][0][3],
                         frag_s[0][2]);

            scale_floats(&frag_c[i][0][1][0], &frag_c[i][1][1][0], &frag_c[i][2][1][0],
                         &frag_c[i][3][1][0], frag_s[0][0], &frag_c[i][0][1][2],
                         &frag_c[i][1][1][2], &frag_c[i][2][1][2], &frag_c[i][3][1][2],
                         frag_s[0][2]);

            scale_floats(&frag_c[i][0][1][1], &frag_c[i][1][1][1], &frag_c[i][2][1][1],
                         &frag_c[i][3][1][1], frag_s[0][0], &frag_c[i][0][1][3],
                         &frag_c[i][1][1][3], &frag_c[i][2][1][3], &frag_c[i][3][1][3],
                         frag_s[0][2]);
          }
        }
      }
  ...
  #pragma unroll
        for (int i = 0; i < m_sh_iters; i++) meta_ptr[i] += (m_sh_stride)-m_gl_rd_delta_o * k_tiles;
        if (slice_col == 0) {
  #pragma unroll
...
#define CALL_IF_2_4(NUM_BITS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS) \
    ...
    cudaFuncSetAttribute(Marlin_24<NUM_BITS, THREADS, THREAD_N_BLOCKS, THREAD_M_BLOCKS,        \
                                   THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>,                     \
                         cudaFuncAttributeMaxDynamicSharedMemorySize, max_shared_mem);         \
    Marlin_24<NUM_BITS, THREADS, THREAD_N_BLOCKS, THREAD_M_BLOCKS, THREAD_K_BLOCKS, STAGES,    \
              GROUP_BLOCKS><<<blocks, THREADS, max_shared_mem, stream>>>(                      \
        A_ptr, B_ptr, meta_ptr, C_ptr, s_ptr, prob_n, prob_m, prob_k, locks);                  \
  }

void marlin_cuda_2_4(const void* A, const void* B, const void* meta, void* C, void* s, int prob_m,
                     int prob_n, int prob_k, void* workspace, int num_bits, int groupsize = -1,
                     int dev = 0, cudaStream_t stream = 0, int thread_k = -1, int thread_m = -1,
  ...
  for (int i = 0; i < tot_n_blocks; i += 4) {
    ...
    CALL_IF_2_4(8, 16, 4, 2, 4)
      ...
      throw std::runtime_error("Unsupported shapes: MKN = [" + str(prob_m) + ", " + str(prob_k) +
                               ", " + str(prob_n) + "]" + ", groupsize = " + str(groupsize) +
...
torch::Tensor gptq_marlin_24_gemm(torch::Tensor& a, torch::Tensor& b_q_weight,
  ...
  TORCH_CHECK(size_m == a.size(0),
              "Shape mismatch: a.size(0) = " + str(a.size(0)) + ", size_m = " + str(size_m));

  // Verify K
  TORCH_CHECK(size_k == a.size(1),
              "Shape mismatch: a.size(1) = " + str(a.size(1)) + ", size_k = " + str(size_k));
  TORCH_CHECK(
      size_k % marlin_24::tile_size == 0,
  ...
  TORCH_CHECK((size_k / marlin_24::tile_size / 2) == b_q_weight.size(0),
              "Shape mismatch: b_q_weight.size(0) = " + str(b_q_weight.size(0)) +
                  ", size_k = " + str(size_k) + ", tile_size = " + str(marlin_24::tile_size));

  // Verify N
  TORCH_CHECK(b_scales.size(1) == size_n,
              "b_scales.size(1) = " + str(b_scales.size(1)) + ", size_n = " + str(size_n));
  ...
  if (b_scales.size(0) > 1) {
    TORCH_CHECK(size_k % b_scales.size(0) == 0,
                "size_k = " + str(size_k) +
                    ", is not divisible by b_scales.size(0) = " + str(b_scales.size(0)));
  ...
  TORCH_CHECK(size_n % marlin_24::min_thread_n == 0,
              "size_n = " + str(size_n) +
                  ", is not divisible by min_thread_n = " + str(marlin_24::min_thread_n));
  ...
  marlin_24::marlin_cuda_2_4(a.data_ptr(), b_q_weight.data_ptr(), b_meta.data_ptr(), c.data_ptr(),
                             ...
                             num_bits, groupsize, dev, at::cuda::getCurrentCUDAStream(dev),
