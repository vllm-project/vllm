...
// Persistent variant of BF16 → SiLU+Mul → NVFP4 TMA warp-specialized kernel.
// CTAs loop until all work is consumed via atomic counter instead of static
// blockIdx.y striding. Uses TMA tensor descriptors with SWIZZLE_128B to reduce
// smem bank conflicts from 8-way to 4-way.
...
__global__ void __launch_bounds__((N_COMPUTE + 1) * 32)
    silu_mul_nvfp4_quant_tma_ws_persistent_kernel_bf16(
      ...
      for (int t = 0; t < actual_load; t++) {
        for (int w = 0; w < validGroups; w++) {
          tma_load_3d(dst + t * ROW_BYTES + w * 1024, tensorMap, 0,
                      gateDim1 + w * 8, batchStart + t, &full_mbar[fillStage]);
          tma_load_3d(dst + t * ROW_BYTES + NC_SLICE_BYTES + w * 1024,
                      tensorMap, 0, upDim1 + w * 8, batchStart + t,
                      &full_mbar[fillStage]);
        }
      }
    ...
    while (true) {
      ...
      if (valid) {
        ...
        for (int t = 0; t < actual_bs; t++) {
          ...
          int const flat_off = laneId * 32;
          int const sw_off_lo = flat_off ^ ((flat_off >> 3) & 0x70);
          int const sw_off_hi =
              (flat_off + 16) ^ (((flat_off + 16) >> 3) & 0x70);
          char* gate_ptr = sp + t * ROW_BYTES + warpSliceOff;
          char* up_ptr = sp + t * ROW_BYTES + NC_SLICE_BYTES + warpSliceOff;
          reinterpret_cast<nvfp4::PackedU32x4&>(gate_vec.elts[0]) =
              *reinterpret_cast<nvfp4::PackedU32x4 const*>(gate_ptr +
                                                           sw_off_lo);
          reinterpret_cast<nvfp4::PackedU32x4&>(gate_vec.elts[4]) =
              *reinterpret_cast<nvfp4::PackedU32x4 const*>(gate_ptr +
                                                           sw_off_hi);
          reinterpret_cast<nvfp4::PackedU32x4&>(up_vec.elts[0]) =
              *reinterpret_cast<nvfp4::PackedU32x4 const*>(up_ptr + sw_off_lo);
          reinterpret_cast<nvfp4::PackedU32x4&>(up_vec.elts[4]) =
              *reinterpret_cast<nvfp4::PackedU32x4 const*>(up_ptr + sw_off_hi);
