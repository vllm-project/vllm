...
__global__ void moe_wna16_gemm_kernel(
    ...
    uint16_t num_experts, uint16_t group_size, uint16_t top_k,
    uint32_t size_m, uint32_t size_n, uint32_t size_k,
    uint16_t BLOCK_SIZE_M, uint16_t BLOCK_SIZE_N, uint16_t BLOCK_SIZE_K,
  ...
  for (int tmp_k = 0; tmp_k < BLOCK_SIZE_K / pack_factor; tmp_k++) {
    ...
    if (tmp_k % 4 == 0) {
      *expert_qweight_tmp_float4 = reinterpret_cast<float4*>(&expert_qweight)[
        weight_offset / pack_factor / 4];
    }
    ...
    for (int m = 0; m < num_valid_tokens; m++) {
      ...
      for (int i = 0; i < 16 / bit; i++) {
        ...
        res2 = __hfma2(
            __hmul2(__hsub2(weight_half2[i], qzero_f2), scale_f2),
...
void run_moe_wna16_gemm(const scalar_t* input, scalar_t* output,
                        ...
                        int num_token_blocks,  int top_k,
                        int size_m, int size_n, int size_k,
                        int BLOCK_SIZE_M, int BLOCK_SIZE_N, int BLOCK_SIZE_K,
                        int bit, bool has_zp, bool mul_topk_weight) {
  ...
  kernel<<<gridDim, blockDim, shared_mem_size, stream>>>(
      ...
      num_experts, group_size, top_k, size_m, size_n, size_k,
      BLOCK_SIZE_M, BLOCK_SIZE_N, BLOCK_SIZE_K,
      has_zp, mul_topk_weight);
}


torch::Tensor moe_wna16_gemm(torch::Tensor input,
                             torch::Tensor output,
  ...
  TORCH_CHECK(bit == 4 || bit == 8,
              "bit must be 4 or 8");
  TORCH_CHECK(size_k % BLOCK_SIZE_K == 0,
              "size_k must divisable by BLOCK_SIZE_K");
  TORCH_CHECK(BLOCK_SIZE_K % group_size == 0,
              "BLOCK_SIZE_K must divisable by group_size");
  TORCH_CHECK(BLOCK_SIZE_M <= 64,
              "BLOCK_SIZE_M must less or equal to 64");
  ...
  if (input.scalar_type() == at::ScalarType::Half) {
    run_moe_wna16_gemm<half>(
        ...
        num_experts, group_size, num_token_blocks,
        top_k, size_m, size_n, size_k,
        BLOCK_SIZE_M, BLOCK_SIZE_N, BLOCK_SIZE_K,
    ...
    run_moe_wna16_gemm<nv_bfloat16>(
        ...
        (const uint32_t*)b_qweight.data_ptr<uint8_t>(),
        (const nv_bfloat16*)b_scales.data_ptr<at::BFloat16>(),
        b_qzeros_ptr, topk_weights_ptr,
        sorted_token_ids.data_ptr<int32_t>(),
        expert_ids.data_ptr<int32_t>(),
        num_tokens_post_pad.data_ptr<int32_t>(),
