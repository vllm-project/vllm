...
// // corresponding index accesses must be compile-time constants, which is why
// we
...
// // dequantization as the compiler does not seem to automatically recognize it
// in
// // all cases.
// template <int lut>
...
// // B-fragment of 4 fp16 values. We mostly follow the strategy in the link
// below,
...
// __device__ inline void scale4(FragB& frag_b, FragS& frag_s_1, FragS&
// frag_s_2,
...
//     const int4* __restrict__ scales_ptr,  // fp16 quantization scales of
//     shape
//                                           // (k/groupsize)xn
...
//     int* locks,            // extra global storage for barrier
//     synchronization bool replicate_input,  // do we use the same input for
//     each expert? bool apply_weights,    // apply weights to output int
//     current_m_block    // current m block to start kernel computation from
// ) {
...
//       // groupsize; this avoids an annoying special case where a stripe
//       starts
...
//   int slice_idx;  // index of threadblock in current slice; numbered bottom
//   to
//                   // top
...
//     if (slice_iters < 0 || slice_col_par >= n_tiles * parallel) slice_iters =
//     0; if (slice_iters == 0) return; if (slice_row + slice_iters > k_tiles)
//     slice_iters = k_tiles - slice_row; slice_count = 1; slice_idx = 0; int
//     col_first = iters * ceildiv(k_tiles * slice_col_par, iters); if
//     (col_first <= k_tiles * (slice_col_par + 1)) {
...
//   constexpr int a_sh_rd_delta_o = 2 * ((threads / 32) / (thread_n_blocks /
//   4));
...
//   constexpr int b_sh_stride = ((thread_n_blocks * 16) * 16 / pack_factor) /
//   4; constexpr int b_thread_vecs = w_type.size_bits() == 4 ? 1 : 2; constexpr
//   int b_sh_stride_threads = b_sh_stride / b_thread_vecs;
...
//     } else {
//       zp_gl_rd = zp_gl_stride * ((thread_k_blocks * slice_row) /
//       group_blocks) +
...
//   main
//   // loop unrolls, all shared memory accesses are static, we simply
//   precompute
...
//   // runtime; we break dependencies between subsequent accesses with a tile
//   by
...
//           cp_async4(&sh_b_stage[b_sh_wr_delta * i + b_sh_wr + j], B_ptr[i] +
//           j);
//         }
...
//       ldsm4(frag_a[k % 2][i], &sh_a_stage[a_sh_rd_trans[k %
//       b_sh_wr_iters][i]]);
//     int4* sh_b_stage = sh_b + b_sh_stage * pipe;
...
//       any
//       // unnecessary read or write iterations, e.g., for two warps we write
//       only
...
//               if (i < red_off) {
//                 float* c_rd =
//                     reinterpret_cast<float*>(&sh[red_sh_delta * j +
//                     red_sh_rd]);
...
//           int c_idx =
//               c_gl_wr + c_gl_wr_delta_o * (i / 2) + c_gl_wr_delta_i * (i %
//               2);
...
//   // Write out the reduce final result in the correct layout. We only
//   actually
//   // reshuffle matrix fragments in this step, the reduction above is
//   performed
//   // in fragment layout.
//   auto write_result = [&]() {
//     int c_gl_stride = prob_n / 8;
...
//     c_gl_wr += (2 * thread_n_blocks) * slice_col;
//     int c_sh_wr =
//         (4 * c_sh_stride) * ((threadIdx.x % 32) / 4) + (threadIdx.x % 32) %
//         4;
...
//             for (int j = 0; j < 8; ++j) {
//               ctrg[j] = __float2half(topk_weights[row] *
//               __half2float(csrc[j]));
//             }
//           }
...
//       if (slice_count > 1) {  // only globally reduce if there is more than
//       one
//                               // block in a slice
...
//       sorted_ids_base + expert_offsets[expert_idx] + m_block_ctr * 4 *
//       max_par;
...
// // than 1 warp per schedule allows some more latency hiding. At the same
// time,
// // we want relatively few warps to have many registers per warp and small
// tiles. const int USER_THREADS =
//     256;               // Note: This is only used with user-provided
//     thread_k/n
