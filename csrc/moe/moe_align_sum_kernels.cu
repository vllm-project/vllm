...
__global__ void batched_moe_align_block_size_kernel(
  ...
  // Phase 4: Write sorted_ids and block_ids. All 1024 threads cooperate on
  // each batch, dividing the writes evenly instead of one thread doing them
  // all serially.
  for (int32_t batch = 0; batch < num_batches; ++batch) {
    ...
    int32_t const n_blocks = CEILDIV(batch_tokens, block_size);
