...
__launch_bounds__(TPB) __global__
    void moeSigmoid(const InputType* input, const bool* finished, float* output, const int num_cols, const float* correction_bias)
{
    ...
    for (int ii = threadIdx.x; ii < num_cols; ii += TPB)
    {
        ...
        float val = toFloat(input[idx]);

        val = 1.0f / (1.0f + expf(-val));

        if (correction_bias != nullptr) {
            val += correction_bias[idx];
        }
...
/*
  ...
  1) This implementation is optimized for when the number of experts is a small power of 2.
    ...
    faster than the computing sigmoid and topK separately (only tested on CUDA yet).
...
__launch_bounds__(WARPS_PER_CTA* WARP_SIZE_PARAM) __global__
    void topkGatingSigmoid(const InputType* input, const bool* finished, float* output, const int num_rows, IndType* indices,
        int* source_rows, const int k, const int start_expert, const int end_expert, const bool renormalize, const float* correction_bias)
{
...
#pragma unroll
    ...
    // Now, sigmoid_res contains the sigmoid of the row chunk. Now, I want to find the topk elements in each row, along
    // with the max index.
...
void topkGatingSigmoidLauncherHelper(const InputType* input, const bool* finished, float* output, IndType* indices,
    int* source_row, const int num_rows, const int k, const int start_expert, const int end_expert, const bool renormalize, const float* correction_bias,
    cudaStream_t stream)
{
    ...
    topkGatingSigmoid<VPT, EXPERTS, WARPS_PER_TB, BYTES_PER_LDG, WARP_SIZE_PARAM, IndType, InputType><<<num_blocks, block_dim, 0, stream>>>(
        input, finished, output, num_rows, indices, source_row, k, start_expert, end_expert, renormalize, correction_bias);
}

#ifndef USE_ROCM
#define LAUNCH_SIGMOID(NUM_EXPERTS, WARPS_PER_TB, MAX_BYTES)                          \
    static_assert(WARP_SIZE == 32,                                                    \
                  "Unsupported warp size. Only 32 is supported for CUDA");            \
    topkGatingSigmoidLauncherHelper<NUM_EXPERTS, WARPS_PER_TB, WARP_SIZE, MAX_BYTES>( \
        gating_output, nullptr, topk_weights, topk_indices, token_expert_indices,     \
        num_tokens, topk, 0, num_experts, renormalize, correction_bias, stream);
#else
#define LAUNCH_SIGMOID(NUM_EXPERTS, WARPS_PER_TB, MAX_BYTES)                             \
    if (WARP_SIZE == 64) {                                                               \
        topkGatingSigmoidLauncherHelper<NUM_EXPERTS, WARPS_PER_TB, 64, MAX_BYTES>(       \
            gating_output, nullptr, topk_weights, topk_indices, token_expert_indices,    \
            num_tokens, topk, 0, num_experts, renormalize, correction_bias, stream);                      \
    } else if (WARP_SIZE == 32) {                                                        \
        topkGatingSigmoidLauncherHelper<NUM_EXPERTS, WARPS_PER_TB, 32, MAX_BYTES>(       \
            gating_output, nullptr, topk_weights, topk_indices, token_expert_indices,    \
            num_tokens, topk, 0, num_experts, renormalize, correction_bias, stream);                      \
    } else {                                                                             \
...
void topkGatingSigmoidKernelLauncher(
    ...
    switch (num_experts) {
        case 1:
            LAUNCH_SIGMOID(1, WARPS_PER_TB, BYTES_PER_LDG_POWER_OF_2);
            break;
        case 2:
            LAUNCH_SIGMOID(2, WARPS_PER_TB, BYTES_PER_LDG_POWER_OF_2);
            break;
        case 4:
            LAUNCH_SIGMOID(4, WARPS_PER_TB, BYTES_PER_LDG_POWER_OF_2);
            break;
        case 8:
            LAUNCH_SIGMOID(8, WARPS_PER_TB, BYTES_PER_LDG_POWER_OF_2);
            break;
        case 16:
            LAUNCH_SIGMOID(16, WARPS_PER_TB, BYTES_PER_LDG_POWER_OF_2);
            break;
        case 32:
            LAUNCH_SIGMOID(32, WARPS_PER_TB, BYTES_PER_LDG_POWER_OF_2);
            break;
        case 64:
            LAUNCH_SIGMOID(64, WARPS_PER_TB, BYTES_PER_LDG_POWER_OF_2);
            break;
        case 128:
            LAUNCH_SIGMOID(128, WARPS_PER_TB, BYTES_PER_LDG_POWER_OF_2);
            break;
        case 256:
            LAUNCH_SIGMOID(256, WARPS_PER_TB, BYTES_PER_LDG_POWER_OF_2);
            break;
        case 512:
            LAUNCH_SIGMOID(512, WARPS_PER_TB, BYTES_PER_LDG_POWER_OF_2);
            break;
...
#ifndef USE_ROCM
        case 192:
            LAUNCH_SIGMOID(192, WARPS_PER_TB, BYTES_PER_LDG_MULTIPLE_64);
            break;
        case 320:
            LAUNCH_SIGMOID(320, WARPS_PER_TB, BYTES_PER_LDG_MULTIPLE_64);
            break;
        case 384:
            LAUNCH_SIGMOID(384, WARPS_PER_TB, BYTES_PER_LDG_MULTIPLE_64);
            break;
        case 448:
            LAUNCH_SIGMOID(448, WARPS_PER_TB, BYTES_PER_LDG_MULTIPLE_64);
            break;
        case 576:
            LAUNCH_SIGMOID(576, WARPS_PER_TB, BYTES_PER_LDG_MULTIPLE_64);
            break;
#endif
        default: {
            TORCH_CHECK(sigmoid_workspace != nullptr,
                "sigmoid_workspace must be provided for num_experts that are not a power of 2 or multiple of 64.");
            ...
            moeTopK<TPB><<<num_tokens, TPB, 0, stream>>>(
                sigmoid_workspace, nullptr, topk_weights, topk_indices, token_expert_indices,
                num_experts, topk, 0, num_experts, renormalize, correction_bias);
        }
    }
}
...
void dispatch_topk_sigmoid_launch(
    ...
    const c10::optional<torch::Tensor>& correction_bias, cudaStream_t stream)
{
    ...
    if (topk_indices.scalar_type() == at::ScalarType::Int) {
        vllm::moe::topkGatingSigmoidKernelLauncher<int, ComputeType>(
            reinterpret_cast<const ComputeType*>(gating_output.data_ptr()),
            topk_weights.data_ptr<float>(),
            topk_indices.data_ptr<int>(),
            token_expert_indices.data_ptr<int>(),
            sigmoid_workspace.data_ptr<float>(),
            num_tokens, num_experts, topk, renormalize, bias_ptr, stream);
    } else if (topk_indices.scalar_type() == at::ScalarType::UInt32) {
        vllm::moe::topkGatingSigmoidKernelLauncher<uint32_t, ComputeType>(
            reinterpret_cast<const ComputeType*>(gating_output.data_ptr()),
            topk_weights.data_ptr<float>(),
        ...
        vllm::moe::topkGatingSigmoidKernelLauncher<int64_t, ComputeType>(
            reinterpret_cast<const ComputeType*>(gating_output.data_ptr()),
            topk_weights.data_ptr<float>(),
    ...
    const c10::optional<torch::Tensor>& correction_bias)
{
    ...
    torch::Tensor sigmoid_workspace = torch::empty({workspace_size}, workspace_options);

    if (gating_output.scalar_type() == at::ScalarType::Float) {
        dispatch_topk_sigmoid_launch<float>(gating_output, topk_weights, topk_indices, 
            token_expert_indices, sigmoid_workspace, num_tokens, num_experts, topk, renormalize, correction_bias, stream);
    } else if (gating_output.scalar_type() == at::ScalarType::Half) {
        dispatch_topk_sigmoid_launch<__half>(gating_output, topk_weights, topk_indices, 
            token_expert_indices, sigmoid_workspace, num_tokens, num_experts, topk, renormalize, correction_bias, stream);
    } else if (gating_output.scalar_type() == at::ScalarType::BFloat16) {
        dispatch_topk_sigmoid_launch<__nv_bfloat16>(gating_output, topk_weights, topk_indices, 
            token_expert_indices, sigmoid_workspace, num_tokens, num_experts, topk, renormalize, correction_bias, stream);
    } else {
