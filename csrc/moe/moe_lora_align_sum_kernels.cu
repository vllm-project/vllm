...
__global__ void moe_lora_align_sum_kernel_shared(
...
template <typename scalar_t, typename token_cnts_t>
__global__ void moe_lora_align_sum_kernel_global(
    scalar_t* __restrict__ topk_ids, int32_t* token_lora_mapping,
    int64_t block_size, int num_experts, int max_loras, size_t numel,
    int max_num_tokens_padded, int max_num_m_blocks,
    int32_t* __restrict__ sorted_token_ids, int32_t* __restrict__ expert_ids,
    int topk_num, int32_t* total_tokens_post_pad, int32_t* adapter_enabled,
    int32_t* lora_ids, int32_t* global_cumsum,
    token_cnts_t* global_tokens_cnts) {
  const size_t tokens_per_thread = div_ceil(numel, blockDim.x);
  const size_t start_idx = threadIdx.x * tokens_per_thread;
  ...
  int32_t* cumsum = global_cumsum + lora_idx * (num_experts + 1);
  token_cnts_t* tokens_cnts =
      global_tokens_cnts + lora_idx * (blockDim.x + 1) * num_experts;
  ...
  VLLM_DISPATCH_INTEGRAL_TYPES(
      topk_ids.scalar_type(), "moe_lora_align_sum_kernel", [&] {
        ...
        if (use_global_memory) {
          ...
          auto global_cumsum =
              torch::zeros({max_loras, num_experts + 1}, options);
          auto global_tokens_cnts =
              torch::zeros({max_loras, num_thread + 1, num_experts}, options);

          auto kernel = moe_lora_align_sum_kernel_global<scalar_t, int32_t>;
          kernel<<<max_loras, blockDim, 0, stream>>>(
              ...
              global_cumsum.data_ptr<int32_t>(),
              global_tokens_cnts.data_ptr<int32_t>());
        } else {
          auto kernel = moe_lora_align_sum_kernel_shared<scalar_t, int32_t>;
