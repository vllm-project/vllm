...
#define __CALL_IF_MOE(W_TYPE, THREAD_N_BLOCKS, THREAD_K_BLOCKS, HAS_ACT_ORDER, \
    ...
    if (a_type == vllm::kHalf) {                                               \
      cudaFuncSetAttribute(                                                    \
          MarlinMoE<half, W_TYPE.id(), NUM_THREADS, THREAD_N_BLOCKS,           \
                    THREAD_K_BLOCKS, STAGES, HAS_ACT_ORDER, GROUP_BLOCKS>,     \
      ...
      MarlinMoE<half, W_TYPE.id(), NUM_THREADS, THREAD_N_BLOCKS,               \
          ...
          <<<blocks, NUM_THREADS, max_shared_mem, stream>>>(                   \
              ...
              cfg_max_m_blocks);                                               \
    } else if (a_type == vllm::kBFloat16) {                                    \
      cudaFuncSetAttribute(                                                    \
          MarlinMoE<nv_bfloat16, W_TYPE.id(), NUM_THREADS, THREAD_N_BLOCKS,    \
                    THREAD_K_BLOCKS, STAGES, HAS_ACT_ORDER, GROUP_BLOCKS>,     \
          cudaFuncAttributeMaxDynamicSharedMemorySize, max_shared_mem);        \
      MarlinMoE<nv_bfloat16, W_TYPE.id(), NUM_THREADS, THREAD_N_BLOCKS,        \
                THREAD_K_BLOCKS, STAGES, HAS_ACT_ORDER, GROUP_BLOCKS>          \
      ...
      TORCH_CHECK(false, "only float16 and bfloat16 is supported");            \
    }                                                                          \
  }
