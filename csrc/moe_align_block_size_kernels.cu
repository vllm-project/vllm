...
__global__ void moe_align_block_size_kernel(scalar_t* __restrict__ topk_ids,
  ...
  for (int i = start_idx; i < numel && i < start_idx + tokens_per_thread; ++i) {
    ...
    int32_t rank_post_pad =
        tokens_cnts[index(num_experts, threadIdx.x, expert_id)] + cumsum[expert_id];
...
void moe_align_block_size(torch::Tensor topk_ids, int num_experts, int block_size,
  ...
  VLLM_DISPATCH_INTEGRAL_TYPES(topk_ids.scalar_type(), "moe_align_block_size_kernel", [&] {
    ...
    AT_CUDA_CHECK(VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize((void*)kernel, shared_mem));
    kernel<<<1, num_experts, shared_mem, stream>>>(
        topk_ids.data_ptr<scalar_t>(), sorted_token_ids.data_ptr<int32_t>(),
        experts_ids.data_ptr<int32_t>(), num_tokens_post_pad.data_ptr<int32_t>(), num_experts,
