 ...
 */
 ...
 constexpr uint32_t LARGE_THRESHOLD = 32768;
 ...
 __device__ __noinline__ void fast_topk_cuda_tl(const float* __restrict__ logits,
    ...
    // __shared__ that would inflate the persistent kernel's smem footprint.
    extern __shared__ char medium_smem[];
  
    int (*shared_histogram)[RADIX + 128] =
        reinterpret_cast<int(*)[RADIX + 128]>(medium_smem);
    int* medium_scalars =
        reinterpret_cast<int*>(medium_smem + kMediumHistBytes);
