...
inline int getMultiProcessorCount()
{
    ...
    checkCuda(cudaDeviceGetAttribute(&multi_processor_count, cudaDevAttrMultiProcessorCount, device_id));
...
struct KVCacheListParams{
    ...
    KVCachePageIndex const* block_indices = nullptr; // shape: [batchSize][beamWidth][2][maxNbPagesPerSeq].
    SeqLenDataType const* sequence_lengths = nullptr; // shape: [batchSize][beamWidth] (for compatibility)
    ...
    SeqLenDataType const* _sequence_lengths, uint32_t _capacity) : 
     pool(_pool), block_indices(_block_indices), sequence_lengths(_sequence_lengths), capacity(_capacity) {}
...
template <typename T>
std::optional<T> getGlobalVar(CUmodule hmod,
    char const* const name, bool required = false)
{
    ...
    case CUDA_ERROR_NOT_FOUND:
        ...
        [[fallthrough]];
    default: cuErrCheck(("Failed to retrieve global variable from cubin.", error));
    }
