...
class CacheDevicePtr : public torch::CustomClassHolder {
 ...
 public:
  ...
  int64_t pageSize =
      2 * _MB;  // page size, the minimum unit of kvcache memory allocation

  std::vector<CUmemGenericAllocationHandle>
      handles;  // used to store the allocated physical memory handles

  // std::vector<size_t> handle_sizes; // used to store the number of pages of
  // each allocated handle
...
class CacheAllocator : public torch::CustomClassHolder {
 private:
  ...
  size_t granurality = 2 * _MB;  // memory allocation granularity supported by
                                 // the gpu, 2MB for nvidia gpu
  size_t pageSize = 2 * _MB;  // page size, the minimum unit of kvcache memory
                              // allocation, default 2MB
 ...
 public:
  ...
  // address space of dptr, and set access permission int
  // allocCachePtr(CacheDevicePtr& ptr, int64_t pageNum=1, int64_t offset=0);
  ...
  // handles and free virtual address space int freeCachePtr(CacheDevicePtr&
  // ptr);
  ...
  // physical memory handles but not free virtual address space int
  // releaseCachePtr(CacheDevicePtr& ptr);
...
// torch::Tensor wrap_cache_ptr_to_tensor(CacheDevicePtr &ptr, const std::string
// dtype, at::ArrayRef<int64_t> shape);
