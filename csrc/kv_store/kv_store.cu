...
void KVStore::CopyIncompleteBlocks(
  ...
  for (size_t i = 0; i < num_items; i++) {
    ...
    char* dst_ptr =
        reinterpret_cast<char*>(dst[dst_block][0][layer_id].data_ptr());
...
void KVStore::CopyBlocks2CPU(torch::Tensor& src, torch::Tensor& dst,
  ...
  for (size_t i = 0; i < num_blocks; i++) {
    ...
    // key
    char* src_ptr =
        reinterpret_cast<char*>(src[0][src_block_number].data_ptr());
    char* dst_ptr =
        reinterpret_cast<char*>(dst[dst_block_number][0][layer_id].data_ptr());
    CHECK_CUDA(cudaMemcpyAsync(dst_ptr, src_ptr, block_size_in_bytes,
                               memcpy_type, stream));
    // value
    src_ptr = reinterpret_cast<char*>(src[1][src_block_number].data_ptr());
    dst_ptr =
        reinterpret_cast<char*>(dst[dst_block_number][1][layer_id].data_ptr());
    CHECK_CUDA(cudaMemcpyAsync(dst_ptr, src_ptr, block_size_in_bytes,
                               memcpy_type, stream));
  }
}
...
__global__ void kv_store_copy_blocks_kernel(
    scalar_t* src, int64_t* key_cache_ptrs, int64_t* value_cache_ptrs,
    const int64_t* block_mapping, const int64_t* block_offsets,
    const int request_idx, const int64_t numel_per_block) {
  ...
  scalar_t* src_value_block =
      (src + (pair_idx * 2 + 1) * num_layer * numel_per_block +
       layer_idx * numel_per_block);
  for (int i = threadIdx.x; i < numel_per_block; i += blockDim.x) {
...
void KVStore::CopyBlocks2GPUBatch(torch::Tensor& src,
  ...
  TORCH_CHECK(block_mapping.device().is_cpu(), "block_mapping must be on CPU");
  TORCH_CHECK(block_offsets.device().is_cpu(), "block_offsets must be on CPU");
  const at::cuda::OptionalCUDAGuard device_guard(dst_device);
  auto stream = at::cuda::getCurrentCUDAStream();
  auto block_mapping_gpu =
      block_mapping.to(dst_device, block_mapping.scalar_type(),
                       /*non_blocking=*/true);
  auto block_offsets_gpu =
      block_offsets.to(dst_device, block_offsets.scalar_type(),
                       /*non_blocking=*/true);
  ...
  for (size_t i = 0; i < num_requests; i++) {
    ...
    auto options =
        torch::TensorOptions().dtype(kv_caches[0].dtype()).device(dst_device);
    ...
    for (size_t j = 0; j < num_blocks; j++) {
      ...
      CHECK_CUDA(
          cudaMemcpyAsync(dst_ptr, src_ptr, trans_nbytes, memcpy_type, stream));
    }
        ...
        src.scalar_type(), "kv_store_copy_blocks_kernel", ([&] {
          vllm::kv_store_copy_blocks_kernel<scalar_t>
              <<<grid, block, 0, stream>>>(
                  ...
                  block_offsets_gpu.data_ptr<int64_t>(), i, numel_per_block);
        }));
    CHECK_CUDA(
        cudaEventRecord(reinterpret_cast<cudaEvent_t>(events[i]), stream));
  }
}
...
template <typename scalar_t>
__global__ void kv_store_copy_blocks_kernel(
    scalar_t* src, scalar_t* key_cache, scalar_t* value_cache,
    const int64_t* block_mapping, const int64_t* block_offsets,
    const int request_idx, const int64_t layer_id, const int64_t num_layer,
  ...
  scalar_t* dst_key_block = key_cache + dst_block_number * numel_per_block;
  for (int i = threadIdx.x; i < numel_per_block; i += blockDim.x) {
    dst_key_block[i] = src_key_block[i];
  }
  scalar_t* src_value_block =
      src + (src_block_number * 2 + 1) * num_layer * numel_per_block +
      layer_id * numel_per_block;
  scalar_t* dst_value_block = value_cache + dst_block_number * numel_per_block;
  for (int i = threadIdx.x; i < numel_per_block; i += blockDim.x) {
...
void CopyLayerBlocks2GPUKernelFunc(
    ...
    const int64_t num_layer, const torch::Tensor& block_mapping,
  ...
  TORCH_CHECK(block_mapping.device().is_cpu(), "block_mapping must be on CPU");
  TORCH_CHECK(block_offsets.device().is_cpu(), "block_offsets must be on CPU");
  TORCH_CHECK(req_ids.device().is_cpu(), "req_ids must be on CPU");
  const at::cuda::OptionalCUDAGuard device_guard(dst_device);
  auto block_mapping_gpu =
      block_mapping.to(dst_device, block_mapping.scalar_type(),
                       /*non_blocking=*/true);
            ...
            src.scalar_type(), "kv_store_copy_blocks_kernel", ([&] {
              vllm::kv_store_copy_blocks_kernel<scalar_t>
                  <<<grid, block, 0, stream>>>(
                      ...
                      block_mapping_gpu.data_ptr<int64_t>(),
                      block_offsets_gpu.data_ptr<int64_t>(), i, layer_id,
                      num_layer, numel_per_block);
            }));
      } else {
        for (size_t j = 0; j < num_blocks; j++) {
          int64_t idx = (start_idx + j);
          ...
          CHECK_CUDA(cudaMemcpyAsync(dst_ptr, src_ptr, block_nbytes,
                                     memcpy_type, stream));
...
void CopyLayerBlocks2GPUThreadFunc(
    ...
    const int64_t num_layer, const torch::Tensor& block_mapping,
    const torch::Tensor& block_offsets, const torch::Tensor& req_ids,
    const std::vector<long>& events, const at::cuda::CUDAStream& stream) {
  size_t num_requests = req_ids.size(0);
  ...
  for (size_t i = 0; i < num_requests; i++) {
    ...
    for (int64_t layer_id = 0; layer_id < num_layer; layer_id++) {
      for (size_t j = 0; j < num_blocks; j++) {
        ...
        char* dst_ptr = reinterpret_cast<char*>(
            kv_caches[layer_id][0][dst_block_number].data_ptr());
        CHECK_CUDA(cudaMemcpyAsync(dst_ptr, src_ptr, block_nbytes, memcpy_type,
                                   stream));
        src_ptr = reinterpret_cast<char*>(
            src[src_block_number][1][layer_id].data_ptr());
        dst_ptr = reinterpret_cast<char*>(
...
void KVStore::CopyLayerBlocks2GPU(torch::Tensor& src,
  ...
  _copy_thread = std::thread(CopyLayerBlocks2GPUThreadFunc,
      ...
      events, stream);
  */
  CopyLayerBlocks2GPUKernelFunc(src, kv_caches, num_layer, block_mapping,
                                block_offsets, req_ids, events, stream);
}
