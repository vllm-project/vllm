...
torch::Tensor LLMM1(at::Tensor& mat, at::Tensor& vec,
  ...
  AT_DISPATCH_REDUCED_FLOATING_TYPES(vec.scalar_type(), "LLGemm1", [&] {
    ...
    switch (rows_per_block) {
      case 2:
        LLGemm1_kernel<scalar_t, 2><<<NUM_BLOCKS, NUM_THREADS, 0, stream>>>(
            mat_ptr, vec_ptr, out_ptr, K);
        break;
      case 4:
        LLGemm1_kernel<scalar_t, 4><<<NUM_BLOCKS, NUM_THREADS, 0, stream>>>(
            mat_ptr, vec_ptr, out_ptr, K);
        break;
      case 8:
        LLGemm1_kernel<scalar_t, 8><<<NUM_BLOCKS, NUM_THREADS, 0, stream>>>(
            mat_ptr, vec_ptr, out_ptr, K);
        break;
      case 16:
        LLGemm1_kernel<scalar_t, 16><<<NUM_BLOCKS, NUM_THREADS, 0, stream>>>(
            mat_ptr, vec_ptr, out_ptr, K);
        break;
      default:
