...
__launch_bounds__(NUM_THREADS, 5) void paged_attention_ll4mi_QKV_mfma16_kernel(
  ...
  // write out partition max_logits and exp_sum (only needed for
  // multi-partition)
  ...
  if (warpid == 0) {
    ...
    if (head_elem_idx < HEAD_SIZE) {
      ...
      if constexpr (SINGLE_PARTITION) {
        // Write directly to final_out - layout: [num_seqs, num_heads,
        // head_size]
        for (int h = 0; h < GQA_RATIO4; h++) {
          const int local_head_idx = 4 * h + rowid;
...
void paged_attention_custom_launcher_navi(
  ...
  if (use_single_partition) {
    ...
    switch (gqa_ratio) {
      case 1:
        LAUNCH_CUSTOM_ATTENTION_MFMA16_SINGLE(1);
        break;
      case 2:
        LAUNCH_CUSTOM_ATTENTION_MFMA16_SINGLE(2);
        break;
      case 3:
        LAUNCH_CUSTOM_ATTENTION_MFMA16_SINGLE(3);
        break;
      case 4:
        LAUNCH_CUSTOM_ATTENTION_MFMA16_SINGLE(4);
        break;
      case 5:
        LAUNCH_CUSTOM_ATTENTION_MFMA16_SINGLE(5);
        break;
