// Per-N kernel instantiation shard for wvSplitK_w8a8 (N=1).
...
void launch_w8a8_n1(dim3 grid, cudaStream_t stream, int K, int M, int Bx,
                    ...
                    int achunk) {
  dispatch_w8a8<scalar_t, 1>(grid, stream, K, M, Bx, By, B, A_raw, w_scale,
                             a_scale, BIAS, C, CuCount, quant_mode, thrds,
                             ytile, unrl, achunk);
}

template void launch_w8a8_n1<half>(dim3, cudaStream_t, int, int, int, int,
                                   const int8_t*, const void*, const half*,
                                   const float*, const half*, half*, int, int,
                                   int, int, int, int);
template void launch_w8a8_n1<__hip_bfloat16>(
