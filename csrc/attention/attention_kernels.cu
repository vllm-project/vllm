...
__device__ void paged_attention_kernel(
  ...
  if (USE_PARTITIONING && partition_idx * PARTITION_SIZE >= seqlen) {
    // No work to do. Terminate the thread block.
    return;
  }

  const int num_seq_blocks = DIVIDE_ROUND_UP(seqlen, BLOCK_SIZE);
  ...
  const int end_token_idx = MIN(start_token_idx + num_blocks * BLOCK_SIZE, seqlen);
...
#pragma unroll
      ...
      qk += (alibi_slope != 0) ? alibi_slope * (token_idx - seqlen + 1) : 0;

      if (thread_group_offset == 0) {
        // Store the partial reductions to shared memory.
        // NOTE(woosuk): It is required to zero out the masked logits.
        const bool mask = token_idx >= seqlen;
...
#pragma unroll
          for (int j = 0; j < V_VEC_SIZE; j++) {
            v_vec_ptr[j] = token_idx + j < seqlen ? v_vec_ptr[j] : zero_value;
          }
        }
...
__global__ void paged_attention_v1_kernel(
  ...
  paged_attention_kernel<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, IS_FP8_KV_CACHE>(
    ...
    out, q, k_cache, v_cache, num_kv_heads, scale, block_tables, seqlens,
...
__global__ void paged_attention_v2_kernel(
  ...
  paged_attention_kernel<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, IS_FP8_KV_CACHE, PARTITION_SIZE>(
    ...
    block_tables, seqlens, max_num_blocks_per_seq, alibi_slopes,
...
__global__ void paged_attention_v2_reduce_kernel(
  ...
  const int seq_idx = blockIdx.y;
  const int seqlen = seqlens[seq_idx];
  const int num_partitions = DIVIDE_ROUND_UP(seqlen, PARTITION_SIZE);
  if (num_partitions == 1) {
...
void paged_attention_v1_launcher(
  ...
  constexpr int NUM_WARPS = NUM_THREADS / WARP_SIZE;
  int padded_max_seqlen = DIVIDE_ROUND_UP(max_seqlen, BLOCK_SIZE) * BLOCK_SIZE;
  int logits_size = padded_max_seqlen * sizeof(float);
...
void paged_attention_v2_launcher(
  ...
  constexpr int NUM_WARPS = NUM_THREADS / WARP_SIZE;
  int max_num_partitions = DIVIDE_ROUND_UP(max_seqlen, PARTITION_SIZE);
