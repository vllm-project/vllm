...
#pragma unroll
      for (int j = 0; j < NUM_VECS_PER_THREAD; j++) {
        ...
        if constexpr (IS_FP8_KV_CACHE) {
          ...
          // FP8_E4M3 quantized k cache vec to k vec in higher precision (FP16, BFloat16, etc.)
...
__global__ void paged_attention_v1_kernel(
  ...
  paged_attention_kernel<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, IS_FP8_KV_CACHE>(
      ...
      scale, block_tables, seq_lens, max_num_blocks_per_seq, alibi_slopes, q_stride,
...
#define LAUNCH_PAGED_ATTENTION_V1(HEAD_SIZE)                                                       \
  VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize(                                            \
      ((void*)vllm::paged_attention_v1_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS,      \
                                              IS_FP8_KV_CACHE>),                                   \
      shared_mem_size);                                                                            \
  vllm::paged_attention_v1_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, IS_FP8_KV_CACHE> \
      <<<grid, block, shared_mem_size, stream>>>(                                                  \
          out_ptr, query_ptr, key_cache_ptr, value_cache_ptr, num_kv_heads, scale,                 \
          block_tables_ptr, seq_lens_ptr, max_num_blocks_per_seq, alibi_slopes_ptr, q_stride,      \
          kv_block_stride, kv_head_stride, kv_scale);

// TODO(woosuk): Tune NUM_THREADS.
template <typename T, typename CACHE_T, int BLOCK_SIZE, bool IS_FP8_KV_CACHE, int NUM_THREADS = 128>
...
#define CALL_V1_LAUNCHER(T, CACHE_T, BLOCK_SIZE, IS_FP8_KV_CACHE)                      \
  paged_attention_v1_launcher<T, CACHE_T, BLOCK_SIZE, IS_FP8_KV_CACHE>(                \
      out, query, key_cache, value_cache, num_kv_heads, scale, block_tables, seq_lens, \
      max_seq_len, alibi_slopes, kv_scale);
...
#define LAUNCH_PAGED_ATTENTION_V2(HEAD_SIZE)                                                       \
  vllm::paged_attention_v2_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, IS_FP8_KV_CACHE, \
...
void paged_attention_v2_launcher(torch::Tensor& out, torch::Tensor& exp_sums,
...
#define CALL_V2_LAUNCHER(T, CACHE_T, BLOCK_SIZE, IS_FP8_KV_CACHE)                             \
  paged_attention_v2_launcher<T, CACHE_T, BLOCK_SIZE, IS_FP8_KV_CACHE>(                       \
      ...
      block_tables, seq_lens, max_seq_len, alibi_slopes, kv_scale);
