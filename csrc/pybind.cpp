...
void rotary_embedding_dispatch(torch::Tensor &positions, torch::Tensor &query,
  ...
  VLLM_DISPATCH_DEVICES(key.device(), rotary_embedding, positions, query, key,
...
void paged_attention_v2_dispatch(
    ...
    int max_context_len, const c10::optional<torch::Tensor> &alibi_slopes,
    const std::string& kv_cache_dtype) {
  VLLM_DISPATCH_DEVICES(out.device(), paged_attention_v2, out, exp_sums,
                        max_logits, tmp_out, query, key_cache, value_cache,
                        num_kv_heads, scale, block_tables, context_lens,
                        block_size, max_context_len, alibi_slopes, kv_cache_dtype);
}
...
void rms_norm_dispatch(torch::Tensor &out, torch::Tensor &input,
                       torch::Tensor &weight, float epsilon) {
  VLLM_DISPATCH_DEVICES(out.device(), rms_norm, out, input, weight, epsilon);
}

void fused_add_rms_norm_dispatch(torch::Tensor &input, torch::Tensor &residual,
                                 torch::Tensor &weight, float epsilon) {
  VLLM_DISPATCH_DEVICES(input.device(), fused_add_rms_norm, input, residual,
                        weight, epsilon);
}

torch::Tensor awq_gemm_dispatch(torch::Tensor _in_feats, torch::Tensor _kernel,
                                torch::Tensor _scaling_factors,
                                torch::Tensor _zeros, int split_k_iters) {
  VLLM_DISPATCH_DEVICES(_in_feats.device(), awq_gemm, _in_feats, _kernel,
                        _scaling_factors, _zeros, split_k_iters);
}

torch::Tensor awq_dequantize_dispatch(torch::Tensor _kernel,
                                      torch::Tensor _scaling_factors,
                                      torch::Tensor _zeros, int split_k_iters,
                                      int thx, int thy) {
...
void squeezellm_gemm_dispatch(torch::Tensor vec, torch::Tensor mat,
                              torch::Tensor mul, torch::Tensor lookup_table) {
  VLLM_DISPATCH_DEVICES(vec.device(), squeezellm_gemm, vec, mat, mul,
                        lookup_table);
}

void swap_blocks_dispatch(torch::Tensor &src, torch::Tensor &dst,
                          const std::map<int64_t, int64_t> &block_mapping) {
  VLLM_DISPATCH_DEVICES(src.device(), swap_blocks, src, dst, block_mapping);
}

void copy_blocks_dispatch(
    std::vector<torch::Tensor> &key_caches,
    std::vector<torch::Tensor> &value_caches,
    const std::map<int64_t, std::vector<int64_t>> &block_mapping) {
  VLLM_DISPATCH_DEVICES(key_caches[0].device(), copy_blocks, key_caches,
                        value_caches, block_mapping);
}

void reshape_and_cache_dispatch(torch::Tensor &key, torch::Tensor &value,
                                torch::Tensor &key_cache,
                                torch::Tensor &value_cache,
                                torch::Tensor &slot_mapping,
                                const std::string& kv_cache_dtype) {
  VLLM_DISPATCH_DEVICES(key.device(), reshape_and_cache, key, value, key_cache,
                        value_cache, slot_mapping, kv_cache_dtype);
}

void gather_cached_kv_dispatch(torch::Tensor &key, torch::Tensor &value,
                               torch::Tensor &key_cache,
                               torch::Tensor &value_cache,
                               torch::Tensor &slot_mapping) {
  VLLM_DISPATCH_DEVICES(key.device(), gather_cached_kv, key, value, key_cache,
                        value_cache, slot_mapping);
}

torch::Tensor gptq_gemm_dispatch(torch::Tensor a, torch::Tensor b_q_weight,
                                 torch::Tensor b_gptq_qzeros,
                                 torch::Tensor b_gptq_scales,
                                 torch::Tensor b_g_idx, bool use_exllama) {
  VLLM_DISPATCH_DEVICES(a.device(), gptq_gemm, a, b_q_weight, b_gptq_qzeros,
                        b_gptq_scales, b_g_idx, use_exllama);
}

void gptq_shuffle_dispatch(torch::Tensor q_weight, torch::Tensor q_perm) {
  VLLM_DISPATCH_DEVICES(q_weight.device(), gptq_shuffle, q_weight, q_perm);
}


void moe_align_block_size_dispatch(
  ...
  VLLM_DISPATCH_DEVICES(topk_ids.device(), moe_align_block_size, topk_ids,
                        num_experts, block_size, sorted_token_ids, experts_ids, num_tokens_post_pad);
}
