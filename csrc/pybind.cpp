...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  ops.def(
      "batched_rotary_embedding", &batched_rotary_embedding,
...
// Quantization ops
  ...
  ops.def("moe_align_block_size", &moe_align_block_size,
          "Aligning the number of tokens to be processed by each expert such that it is divisible "
          "by the block size.");
  ...
  cache_ops.def("reshape_and_cache_flash", &reshape_and_cache_flash,
                "Reshape the key and value tensors and cache them");
  cache_ops.def("convert_fp8", &convert_fp8, "Convert the key and value cache to fp8 data type");
