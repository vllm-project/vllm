...
#pragma unroll
        ...
        if (tid == 0) {
            ...
            dst[batch_id * vocab_size + ite] = total.u;
            s_sum += total.u;
...
#define CASE_K(K_MIN, K_MAX, BLOCK_SIZE, BLOCK_PER_SEQ)                                                                \
    case K_MIN ... K_MAX:                                                                                              \
        topk_stage1<T, BLOCK_SIZE, BLOCK_PER_SEQ><<<batch_size * BLOCK_PER_SEQ, BLOCK_SIZE, 0, stream>>>(              \
            src, temp_log_probs, topk_tmp_id_buf, topk_tmp_val_buf, max_top_k, top_ks, vocab_size);                    \
        topk_stage2_sampling<T, BLOCK_SIZE, BLOCK_PER_SEQ><<<batch_size, BLOCK_SIZE, 0, stream>>>(                     \
            topk_tmp_id_buf, topk_tmp_val_buf, dst, max_top_k, top_ks, top_ps, vocab_size);                            \
        break;
                ...
                cudaStream_t stream)
{
    ...
    invokeBatchTopKSampling<T>(
        nullptr, workspace_size, nullptr, nullptr, max_top_k, nullptr, nullptr, vocab_size, batch_size, stream);

    cudaMalloc(&workspace, workspace_size);

    invokeBatchTopKSampling<T>(
        workspace, workspace_size, src, dst, max_top_k, top_ks, top_ps, vocab_size, batch_size, stream);
           ...
           torch::Tensor       top_ps)
{
    ...
    top_k_cuda(src_ptr, dst_ptr, max_top_k, top_ks_ptr, top_ps_ptr, batch_size, vocab_size, stream);
}
