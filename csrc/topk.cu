...
void persistent_topk(const torch::Tensor& logits, const torch::Tensor& lengths,
  ...
  const bool use_large =
      (stride > P::LARGE_THRESHOLD) ||
      (stride >= P::LARGE_THRESHOLD && num_rows >= 8);
  ...
  if (!use_large) {
    // ---- Non-large path (round-robin, decode + medium) ----
    params.row_states = nullptr;
    params.chunk_size = 0;
    params.ctas_per_group = 1;

    uint32_t grid_size = std::min(static_cast<uint32_t>(num_sms),
                                  static_cast<uint32_t>(num_rows));
    size_t smem_size = P::kSmemMedium;

    static bool medium_smem_configured = false;
    if (!medium_smem_configured) {
      cudaError_t err = cudaFuncSetAttribute(
          P::persistent_topk_kernel<false>,
          cudaFuncAttributeMaxDynamicSharedMemorySize,
          smem_size);
      TORCH_CHECK(err == cudaSuccess,
                  "Failed to set smem: ", cudaGetErrorString(err));
      medium_smem_configured = true;
    }

    P::persistent_topk_kernel<false>
        <<<grid_size, P::kThreadsPerBlock, smem_size, stream>>>(params);

  } else {
    // ---- Large path (multi-CTA groups) ----
    ...
    uint32_t num_groups = std::min(
        static_cast<uint32_t>(num_sms) / ctas_per_group,
        static_cast<uint32_t>(num_rows));
    ...
    TORCH_CHECK(workspace.size(0) >= static_cast<int64_t>(state_bytes),
                "workspace too small for large path, need ",
                state_bytes, " bytes");
...
#define LAUNCH_LARGE(VS)                                                       \
    do {                                                                       \
      void (*kernel)(P::PersistentTopKParams) =                                \
          &P::persistent_topk_kernel<true, VS>;                                \
      cudaError_t err = cudaFuncSetAttribute(                                  \
          kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);      \
      TORCH_CHECK(err == cudaSuccess,                                          \
                  "Failed to set large smem: ", cudaGetErrorString(err));       \
      kernel<<<total_ctas, P::kThreadsPerBlock, smem_size, stream>>>(params);  \
    } while (0)

    if (vec_size == 4) { LAUNCH_LARGE(4); }
    else if (vec_size == 2) { LAUNCH_LARGE(2); }
    else { LAUNCH_LARGE(1); }
