...
void swap_blocks(torch::Tensor& src, torch::Tensor& dst, const torch::Tensor& block_mapping) {
  ...
  for (size_t i = 0; i < num_blocks; i++) {
    ...
    cudaMemcpyAsync(dst_ptr + dst_offset, src_ptr + src_offset, block_size_in_bytes, memcpy_type,
                    stream);
  }
}
  ...
  VLLM_DISPATCH_FLOATING_AND_BYTE_TYPES(
      key_caches[0].scalar_type(), "copy_blocks_kernel", ([&] {
        vllm::copy_blocks_kernel<scalar_t><<<grid, block, 0, stream>>>(
            key_cache_ptrs_tensor.data_ptr<int64_t>(), value_cache_ptrs_tensor.data_ptr<int64_t>(),
...
__global__ void reshape_and_cache_kernel(
    ...
    const int block_size, const int x, const float kv_scale) {
  ...
  for (int i = threadIdx.x; i < n; i += blockDim.x) {
    ...
    if constexpr (is_fp8_kv_cache) {
      ...
      key_cache[tgt_key_idx] =
          fp8_e4m3::scaled_vec_conversion<uint8_t, scalar_t>(tgt_key, kv_scale);
      value_cache[tgt_value_idx] =
          fp8_e4m3::scaled_vec_conversion<uint8_t, scalar_t>(tgt_value, kv_scale);
#else
      assert(false);
#endif
    } else {
...
__global__ void reshape_and_cache_flash_kernel(
    ...
    const int64_t* __restrict__ slot_mapping,  // [num_tokens]
    const int block_stride, const int key_stride, const int value_stride, const int num_heads,
  ...
  for (int i = threadIdx.x; i < n; i += blockDim.x) {
    ...
    const int head_idx = i / head_size;
    const int head_offset = i % head_size;
    const int64_t tgt_value_idx = block_idx * block_stride + block_offset * num_heads * head_size +
...
#define CALL_RESHAPE_AND_CACHE(KV_T, CACHE_T, IS_FP8_KV_CACHE)                                \
  vllm::reshape_and_cache_kernel<KV_T, CACHE_T, IS_FP8_KV_CACHE><<<grid, block, 0, stream>>>( \
      reinterpret_cast<KV_T*>(key.data_ptr()), reinterpret_cast<KV_T*>(value.data_ptr()),     \
      reinterpret_cast<CACHE_T*>(key_cache.data_ptr()),                                       \
      reinterpret_cast<CACHE_T*>(value_cache.data_ptr()), slot_mapping.data_ptr<int64_t>(),   \
      key_stride, value_stride, num_heads, head_size, block_size, x, kv_scale);
...
void reshape_and_cache_flash(
  ...
  const at::cuda::OptionalCUDAGuard device_guard(device_of(key));
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  VLLM_DISPATCH_FLOATING_TYPES(key.scalar_type(), "reshape_and_cache_flash", [&] {
    vllm::reshape_and_cache_flash_kernel<scalar_t><<<grid, block, 0, stream>>>(
        key.data_ptr<scalar_t>(), value.data_ptr<scalar_t>(), k_cache.data_ptr<scalar_t>(),
        v_cache.data_ptr<scalar_t>(), slot_mapping.data_ptr<int64_t>(), block_stride, key_stride,
...
__global__ void convert_fp8_kernel(const Tin* __restrict__ src_cache, Tout* __restrict__ dst_cache,
...
#define CALL_CONVERT_FP8(Tout, Tin)                                              \
  vllm::convert_fp8_kernel<Tout, Tin>                                            \
      <<<grid, block, 0, stream>>>(reinterpret_cast<Tin*>(src_cache.data_ptr()), \
                                   reinterpret_cast<Tout*>(dst_cache.data_ptr()), block_stride);

void convert_fp8(torch::Tensor& src_cache, torch::Tensor& dst_cache) {
