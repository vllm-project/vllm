...
// Weightless variants skip the per-channel multiply for FlashNorm-folded
// checkpoints where the RMSNorm scale has been folded into the next linear.
template <typename scalar_t, int VEC_SIZE, int NUM_DIMS>
__global__ void rms_norm_weightless_kernel(
    scalar_t* __restrict__ out,           // [..., hidden_size]
    const scalar_t* __restrict__ input,   // [..., hidden_size]
    const int64_t input_stride_d2,        // input.stride(-2)
    const int64_t input_stride_d3,        // input.stride(-3)
...
template <typename scalar_t, int width>
__global__ std::enable_if_t<(width > 0) && _typeConvert<scalar_t>::exists>
fused_add_rms_norm_weightless_kernel(
    scalar_t* __restrict__ input,  // [..., hidden_size]
    const int64_t input_stride,
    scalar_t* __restrict__ residual,  // [..., hidden_size]
    const float epsilon, const int num_tokens, const int hidden_size) {
...
void rms_norm_weightless(torch::stable::Tensor& out,     // [..., hidden_size]
                         torch::stable::Tensor& input,   // [..., hidden_size]
                         double epsilon) {
  STD_TORCH_CHECK(out.is_contiguous());
  if (input.stride(-1) != 1) {
  ...
  VLLM_STABLE_DISPATCH_RANK234(num_dims, [&] {
    VLLM_STABLE_DISPATCH_FLOATING_TYPES(
        input.scalar_type(), "rms_norm_weightless_kernel", [&] {
          const int calculated_vec_size =
              std::gcd(16 / sizeof(scalar_t), hidden_size);
          const int block_size =
              std::min(hidden_size / calculated_vec_size, max_block_size);
          dim3 block(block_size);
          VLLM_STABLE_DISPATCH_VEC_SIZE(calculated_vec_size, [&] {
            vllm::rms_norm_weightless_kernel<scalar_t, vec_size, tensor_rank>
                <<<grid, block, 0, stream>>>(
                    out.mutable_data_ptr<scalar_t>(),
                    input.const_data_ptr<scalar_t>(), input_stride_d2,
                    input_stride_d3, input_stride_d4, input_shape_d2,
                    input_shape_d3, epsilon, num_tokens, hidden_size);
          });
        });
  });
}

#define LAUNCH_FUSED_ADD_RMS_NORM_WEIGHTLESS(width)                          \
  VLLM_STABLE_DISPATCH_FLOATING_TYPES(                                       \
      input.scalar_type(), "fused_add_rms_norm_weightless_kernel", [&] {     \
        vllm::fused_add_rms_norm_weightless_kernel<scalar_t, width>          \
            <<<grid, block, 0, stream>>>(                                    \
                input.mutable_data_ptr<scalar_t>(), input_stride,          \
                residual.mutable_data_ptr<scalar_t>(), epsilon, num_tokens,  \
                hidden_size);                                                \
      });

void fused_add_rms_norm_weightless(
    torch::stable::Tensor& input,     // [..., hidden_size]
    torch::stable::Tensor& residual,  // [..., hidden_size]
    double epsilon) {
  STD_TORCH_CHECK(input.scalar_type() == residual.scalar_type());
  ...
  if (ptrs_are_aligned && offsets_are_multiple_of_vector_width &&
      !batch_invariant_launch) {
    LAUNCH_FUSED_ADD_RMS_NORM_WEIGHTLESS(8);
  } else {
    LAUNCH_FUSED_ADD_RMS_NORM_WEIGHTLESS(0);
  }
}
