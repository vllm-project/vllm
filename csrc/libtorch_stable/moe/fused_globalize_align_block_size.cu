...
static constexpr long FGAS_SB_MAX_EFFECTIVE_WORK = 4096;
...
static void fgas_launch(long* p_topk, const int* p_psum, int* p_sorted,
  ...
  long effective_work = (long)mntp + 2L * numel + (long)mnmb * local_e;
  // Histogram [local_e] + one scalar (phase-2 padded-run total).
  size_t smem = (size_t)(local_e + 1) * sizeof(int);

  // Small decode: one non-cooperative block, __syncthreads barriers. Account
  // for both numel passes and the expert_ids owner scan in the crossover.
  if (effective_work <= FGAS_SB_MAX_EFFECTIVE_WORK) {
    fused_gas_kernel<false><<<1, FGAS_THREADS, smem, stream>>>(
