...
static constexpr int GLM_5_2_HIDDEN_DIM = 6144;
...
void dsv3_router_gemm(
  ...
  STD_TORCH_CHECK(
      hidden_dim == DEFAULT_HIDDEN_DIM || hidden_dim == GLM_5_2_HIDDEN_DIM,
      "Expected hidden_dim=", DEFAULT_HIDDEN_DIM,
      " or hidden_dim=", GLM_5_2_HIDDEN_DIM,
  ...
  if (output.scalar_type() == torch::headeronly::ScalarType::Float) {
    ...
    } else {  // GLM_5_2_HIDDEN_DIM
      LoopUnroller<1, 16, DEFAULT_NUM_EXPERTS,
                   GLM_5_2_HIDDEN_DIM>::unroll_float_output(num_tokens, out_ptr,
                                                            a_ptr, b_ptr,
                                                            stream);
    }
    ...
    if (hidden_dim == DEFAULT_HIDDEN_DIM) {
      ...
      LoopUnroller<1, 16, DEFAULT_NUM_EXPERTS,
                   GLM_5_2_HIDDEN_DIM>::unroll_bf16_output(num_tokens, out_ptr,
                                                           a_ptr, b_ptr,
                                                           stream);
    }
  }
