...
static void launchFusedDeepseekV4Templated(
  ...
  // The reduced grid pays off by iterating a token's Q heads in one CTA; with
  // no Q slots at all there is nothing to iterate, so KV-only always takes
  // the warp-per-slot grid.
