...
__global__ void direct_dcp_kv_gather_multimem_kernel(
    ...
    int64_t slice_bytes, int64_t slot_stride_bytes) {
  uint32_t epoch = static_cast<uint32_t>(epoch_ptr[0]);
  int64_t buffer_slot = static_cast<int64_t>(epoch & 1u);
...
void direct_dcp_kv_gather(const torch::stable::Tensor& local_kv,
                          ...
                          torch::stable::Tensor& gathered_kv,
  ...
  STD_TORCH_CHECK(
      received_kv.is_cuda() && received_kv.scalar_type() == dtype &&
  ...
  STD_TORCH_CHECK(
      gathered_kv.is_cuda() && gathered_kv.scalar_type() == dtype &&
  ...
  bool vectorized =
      ...
      reinterpret_cast<uintptr_t>(gathered_kv.data_ptr()) % alignof(uint4) ==
          0 &&
      slice_bytes % sizeof(uint4) == 0 && slot_stride_bytes % sizeof(uint4) == 0;
  ...
  auto launch = [&]<typename copy_t>() {
    ...
    if (use_multimem) {
      ...
      blocks_per_peer =
          blocks_per_peer < kMaxBlocksPerPeer ? blocks_per_peer
                                              : kMaxBlocksPerPeer;
