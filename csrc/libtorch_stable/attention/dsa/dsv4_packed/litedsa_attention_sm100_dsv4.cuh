...
struct TmaParams {
    ...
    CUtensorMap tensor_map_kv;      // K and V gathers: bf16 box {64,1} = 128 B, SWIZZLE_128B
};
...
static constexpr int B_TOPK = 64;  // For 2 CTAs (bf16: halved to hold the smem budget)
...
static_assert(sizeof(SharedMemoryPlan) <= 232448,
              "SharedMemoryPlan exceeds the SM100 dynamic smem cap -- reduce NUM_BUFS or B_TOPK");
...
using TiledMMA_O = decltype(make_tiled_mma(
    ...
    Tile<Int<128>, Layout<Shape<_128, _2, _2>, Stride<_1, _256, _128>>, _16>{}  // CTA0 takes V[:, 0:256], CTA1 takes V[:, 256:512]; K-mode = 16 (16bit)
));
...
KernelTemplate<D_QK>::sparse_attn_fwd_kernel_devfunc(const SparseAttnFwdParams &params, const TmaParams &tma_params) {
    ...
    if (warpgroup_idx == 0) {
        ...
        uint32_t qm_cur = 0u, qm_nx = 0u;   // B_TOPK=64: one 32-bit word per thread-half
        ...
        constexpr int U128_PER_THREAD = (B_TOPK/2)*sizeof(bf16)/16;   // = 4 at B_TOPK=64
        uint128_t* sS_base0 = (uint128_t*)plan.s[0].data() + idx_in_warpgroup%64 + 64*((idx_in_warpgroup/64)*U128_PER_THREAD);
        uint128_t* sS_base1 = (uint128_t*)plan.s[1].data() + idx_in_warpgroup%64 + 64*((idx_in_warpgroup/64)*U128_PER_THREAD);
        ...
        for (int k = 0; k < num_k_blocks; ++k) {
            ...
            const uint32_t kvw = *(uint32_t*)(kv_valid_base + (idx_in_warpgroup>=64?B_TOPK/8/2:0));
        ...
        float output_scale = __fdividef(params.out_scale, li + exp2f(attn_sink - mi));
        ...
        if (elect_one_sync()) {
            ...
            for (int k = 0; k < num_k_blocks; ++k) {
                    ...
                    for (int local_row = 0; local_row < NUM_LOCAL_ROWS_PER_WARP; ++local_row) {
                        ...
                        for (int c = c0; c < c1; ++c)
                            ku::tma_gather4_cta_group_2<true>(
                                ...
                                bar,
                                sK_base + local_row*(4*NUM_WARPS)*64 + c*((B_TOPK/2)*64),
                                c*64,
                                indices[local_row],
                ...
                if (!should_skip_tma) {
                    ...
                    plan.bar_k_part0_ready[cur_buf].complete_transaction(0u, NUM_LOCAL_ROWS_PER_WARP*4*D_PART0*sizeof(bf16), 1u);
                }

                if (!should_skip_tma) {
                    load_part_ki(plan.bar_k_part1_ready[cur_buf], NUM_P0_TILES, D_K/64);
                } else {
                    plan.bar_k_part1_ready[cur_buf].complete_transaction(0u, NUM_LOCAL_ROWS_PER_WARP*4*D_PART1*sizeof(bf16), 1u);
                }
                CUTE_UNROLL
                for (int r = 0; r < NUM_LOCAL_ROWS_PER_WARP; ++r)
        ...
        if (cta_idx == 0 && warp_idx == 12 && elect_one_sync()) {
            ...
            for (int k = 0; k < num_k_blocks+1; ++k) {
                if (k < num_k_blocks) {
                    ...
                    Tensor sQ1 = make_tensor(make_smem_ptr(plan.u.s.q_full.data() + (B_H/2)*D_PART0), SmemLayoutQTiles<NUM_P1_TILES>{});
                    Tensor sK0 = make_tensor(make_smem_ptr(plan.u.s.k[k & 1].data()), SmemLayoutKTiles<NUM_P0_TILES>{});
                    Tensor sK1 = make_tensor(make_smem_ptr(plan.u.s.k[k & 1].data() + (B_TOPK/2)*D_PART0), SmemLayoutKTiles<NUM_P1_TILES>{});

                    plan.bar_k_part0_ready[cur_buf].arrive_and_expect_tx(B_TOPK*D_PART0*sizeof(bf16));
                    ...
                    plan.bar_k_part1_ready[cur_buf].arrive_and_expect_tx(B_TOPK*D_PART1*sizeof(bf16));
                ...
                if (k > 0) {
                    ...
                    Tensor sS_divided = flat_divide(sS, Tile<Int<B_H/2>, Int<B_TOPK/2>>{})(_, _, _0{}, _);    // (B_H/2, 32, 2)
                    Tensor sV_divided = flat_divide(sV, Tile<Int<D_V/2>, Int<B_TOPK/2>>{})(_, _, _0{}, _);  // (D_V/2, 32, 2)
