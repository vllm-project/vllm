...
void dsa_litetopk_seed_prep(const Tensor& slog, int64_t num_buckets,
                            int64_t topk, int64_t cand_cap, int64_t emit_limit,
                            double headroom, int64_t probe_stride_tok,
                            int64_t hist_stride, Tensor& origin,
                            Tensor& inv_delta, Tensor& th_bucket,
                            Tensor& bcount, Tensor& cand_val, Tensor& cand_idx,
                            Tensor& cand_cnt) {
  ...
  if (seed_smem > 48 * 1024) {
    ...
    if (!attr_set) {
      cudaFuncSetAttribute((void*)seed_prep_kernel,
  ...
  const int emit_lim =
      emit_limit == 0 ? 0
                      : (emit_limit > 0 ? static_cast<int>(emit_limit) : head);
  ...
  seed_prep_kernel<<<Q, 1024, seed_smem, stream>>>(
      fptr(slog), static_cast<int>(slog.stride(0)), head, NB, K, cap, emit_lim,
      pst, hst, static_cast<float>(headroom), fptr(origin), fptr(inv_delta),
      iptr(th_bucket), iptr(bcount), fptr(cand_val), iptr(cand_idx),
      iptr(cand_cnt));
}

void dsa_litetopk_scan(const Tensor& q, const Tensor& kv,
                       const Tensor& kv_scales, const Tensor& weights,
                       const Tensor& cu_start, const Tensor& cu_end,
                       Tensor& origin, Tensor& inv_delta, Tensor& th_bucket,
                       Tensor& cand_val, Tensor& cand_idx, Tensor& cand_cnt,
                       Tensor& bcount, int64_t num_buckets, int64_t topk,
                       int64_t refresh_every, int64_t num_kv_splits_override,
  ...
  auto tm_q = make_2d(ptr(q), CU_TENSOR_MAP_DATA_TYPE_UINT8, esz_fp8, HEAD_DIM,
                      seq_len * NUM_HEADS, HEAD_DIM, BLOCK_Q * NUM_HEADS,
                      HEAD_DIM, HEAD_DIM);
  auto tm_kv =
      make_2d(ptr(kv), CU_TENSOR_MAP_DATA_TYPE_UINT8, esz_fp8, HEAD_DIM,
              seq_len_kv, HEAD_DIM, BLOCK_KV, HEAD_DIM, HEAD_DIM);
  auto tm_ks = make_2d(ptr(kv_scales), CU_TENSOR_MAP_DATA_TYPE_FLOAT32, esz_f32,
                       ks_aligned, 1, BLOCK_KV, 1, 0, 0);
  auto tm_w = make_2d(ptr(weights), CU_TENSOR_MAP_DATA_TYPE_FLOAT32, esz_f32,
                      NUM_HEADS, seq_len, NUM_HEADS, BLOCK_Q, NUM_HEADS, 0);

  const int smem = compute_smem_bytes();
  auto kernel =
      &dsa_litetopk::sm100_dsa_litetopk<NUM_HEADS, HEAD_DIM, BLOCK_Q, BLOCK_KV,
                                        NUM_Q_STAGES, NUM_KV_STAGES, NUM_SMS,
                                        SPEC_THREADS, MATH_THREADS>;
  cudaFuncSetAttribute((void*)kernel,
  ...
  if (num_kv_splits_override > 0) {
    ...
    const int max_useful_splits =
        total_kv_blocks > 0 ? (total_kv_blocks + 1) / 2 : 1;
  ...
  kernel<<<grid, SPEC_THREADS + MATH_THREADS, smem, stream>>>(
      ...
      reinterpret_cast<uint32_t*>(iptr(cu_start)),
      reinterpret_cast<uint32_t*>(iptr(cu_end)), fptr(origin), fptr(inv_delta),
      iptr(th_bucket), iptr(bcount), (uint32_t)num_buckets_i, (uint32_t)topk_i,
      (uint32_t)refresh_every_i, (uint32_t)num_kv_splits, (uint32_t)probe_group,
      probe_group > 0
          ? (((1ULL << 42) + (uint64_t)probe_group - 1) / (uint64_t)probe_group)
          : 0ULL,
      (uint32_t)probe_add_max, fptr(cand_val), iptr(cand_idx), iptr(cand_cnt),
...
void dsa_litetopk_select(const Tensor& cand_val, const Tensor& cand_idx,
                         const Tensor& cand_cnt, const Tensor& origin,
                         const Tensor& inv_delta, const Tensor& th_bucket,
                         int64_t num_buckets, int64_t topk, Tensor& out_val,
                         Tensor& out_idx) {
  ...
  compact_topk_min_thr_litetopk_kernel<<<R, 256, 0, stream>>>(
      fptr(cand_val), iptr(cand_idx), iptr(cand_cnt), fptr(origin),
      fptr(inv_delta), iptr(th_bucket), R, CAP, K, NB, fptr(out_val),
      iptr(out_idx), /*probe_group=*/0u, 0ULL, /*probe_add_max=*/0u, nullptr);
}
