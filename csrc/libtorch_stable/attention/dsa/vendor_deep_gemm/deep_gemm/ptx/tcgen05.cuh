...
struct SM100_MMA_F16BF16_SS {
  CUTLASS_DEVICE static void fma(uint64_t const& desc_a, uint64_t const& desc_b,
    ...
    asm volatile(
        ...
        "}\n" ::"r"(tmem_c),
        "l"(desc_a), "l"(desc_b), "r"(static_cast<uint32_t>(desc >> 32)),
        "r"(scale_c));
  }
};
...
struct SM100_MMA_MXF8F6F4_SS {
  CUTLASS_DEVICE static void fma(uint64_t const& desc_a, uint64_t const& desc_b,
                                 uint32_t const& tmem_c,
                                 uint32_t const& scale_c, uint64_t const& desc,
    ...
    asm volatile(
        ...
        : "r"(tmem_c), "l"(desc_a), "l"(desc_b),
          "r"(static_cast<uint32_t>(desc >> 32)), "r"(scale_c), "r"(tmem_sfa),
          "r"(tmem_sfb));
  }
};
