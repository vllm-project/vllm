...
CUTLASS_HOST_DEVICE longlong4_t make_longlong4_t(const long long& x,
                                                 const long long& y,
                                                 const long long& z,
                                                 const long long& w) {
  return {x, y, z, w};
...
struct SM90_U32x2_LDSM_N {
  CUTLASS_DEVICE static void copy(uint32_t& dst_0, uint32_t& dst_1,
                                  void* smem_src) {
...
struct SM90_U32x2_STSM_N {
  CUTLASS_DEVICE static void copy(dtype_t src_0, dtype_t src_1,
                                  void* smem_dst) {
...
struct SM90_U32x4_STSM_T {
  CUTLASS_DEVICE static void copy(dtype_t src_0, dtype_t src_1, dtype_t src_2,
...
struct SM100_U8x8_STSM_T {
  __device__ __forceinline__ static void copy(dtype_t src_0, dtype_t src_1,
                                              void* smem_dst) {
    DG_STATIC_ASSERT(sizeof(dtype_t) == sizeof(uint32_t), "Invalid dtype");
    const uint32_t src[2] = {*reinterpret_cast<uint32_t*>(&src_0),
                             *reinterpret_cast<uint32_t*>(&src_1)};
...
CUTLASS_DEVICE void st_shared(const float* ptr, float val) {
  asm volatile("st.shared.f32 [%0], %1;" ::"l"(__cvta_generic_to_shared(ptr)),
               "f"(val));
}

CUTLASS_DEVICE void st_shared(const float2* ptr, float2 val) {
  asm volatile(
      "st.shared.v2.f32 [%0], {%1, %2};" ::"l"(__cvta_generic_to_shared(ptr)),
      "f"(val.x), "f"(val.y));
}

CUTLASS_DEVICE void st_shared(const uint32_t* ptr, uint32_t val) {
  asm volatile("st.shared.u32 [%0], %1;" ::"l"(__cvta_generic_to_shared(ptr)),
               "r"(val));
}

CUTLASS_DEVICE void st_shared(const void* ptr, uint32_t x, uint32_t y) {
  asm volatile(
      "st.shared.v2.u32 [%0], {%1, %2};" ::"l"(__cvta_generic_to_shared(ptr)),
      "r"(x), "r"(y));
}
...
CUTLASS_DEVICE void st_relaxed_sys(const uint64_t* ptr, const uint64_t& value) {
  asm volatile("st.L1::no_allocate.relaxed.sys.global.u64 [%0], %1;" ::"l"(ptr),
               "l"(value));
}
...
CUTLASS_DEVICE uint64_t atomic_add_sys(const uint64_t* ptr,
...
CUTLASS_DEVICE uint32_t atomic_add_rel(const uint32_t* ptr,
...
CUTLASS_DEVICE void red_or_rel_sys(const uint64_t* ptr, const uint64_t& value) {
  asm volatile("red.release.sys.global.or.b64 [%0], %1;" ::"l"(ptr),
               "l"(value));
}

CUTLASS_DEVICE void red_or_rel_gpu(uint64_t* ptr, const uint64_t& value) {
  asm volatile("red.release.gpu.global.or.b64 [%0], %1;" ::"l"(ptr),
               "l"(value));
}
...
/// Predicated loads
CUTLASS_DEVICE longlong4_t ld_gez_pred(const longlong4_t* ptr,
                                       const int& pred) {
