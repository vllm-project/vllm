...
ensure_ray_tmp_for_node() {
  ...
  # This is job-specific and node-local. Remove stale content from a prior failed
  # attempt with the same job id/node only if it exists.
  rm -rf "${root}" "${link}" 2>/dev/null || true

  mkdir -p "${parent}" "${root}" "${root}/spill" "${root}/py_tmp"

  # Link lives in /tmp for short Ray socket paths, but real storage is ARC/SLURM tmp.
  ln -sfn "${root}" "${link}"
...
# The batch script runs on the head node. Reuse the head node's short Ray temp
# path for Python/Ray-client temp files. Do not call ensure_ray_tmp_for_node here,
...
#   4. Run fallback copies from node-local ARC/SLURM tmp into TRACE_RUN_DIR.
# -----------------------------------------------------------------------------
