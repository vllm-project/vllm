...
SCRIPT_VERSION="stop-server-then-nsight"
...
# Avoid letting post-run diagnostic/copy srun steps hang indefinitely.
SRUN_DIAG_TIMEOUT="${SRUN_DIAG_TIMEOUT:-60s}"
...
nsight_audit_on_node() {
  ...
  run_with_timeout "${SRUN_DIAG_TIMEOUT}" \
    srun \
      --overlap \
      --nodelist "${node}" \
      --nodes=1 \
      ...
      " || nsight_copy_msg "${label}: srun audit failed or timed out on ${node} (exit $?)"
}
...
echo "SRUN_DIAG_TIMEOUT=${SRUN_DIAG_TIMEOUT}"
...
# Important ordering:
#   1. Stop the profiled vLLM API server so the outer nsys profile can finalize.
#   2. Stop Ray background srun steps so the nodes are free for later srun copy jobs.
#   3. Sleep briefly for file flush.
#   4. Only then launch srun-based worker Nsight copy/audit steps.
# -----------------------------------------------------------------------------
...
# 4. Only now do srun-based copy/audit.
