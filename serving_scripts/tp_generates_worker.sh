...
#SBATCH --job-name=vllm-host-qwen3-30b-tp4-2node
#SBATCH --nodes=2
#SBATCH --partition=short
#SBATCH --constraint=gpu_sku:H100&fabric:HDR
...
SCRIPT_VERSION="arc-ray-qwen3-30b-a3b-instruct-tp4-2x2h100"
...
CPUS_PER_TASK="${CPUS_PER_TASK:-${SLURM_CPUS_PER_TASK:-32}}"
SERVE_SCRIPT="${REPO_ROOT}/serving_scripts/serve_ShareGPT_multi_node.sh"

EXPECTED_GPUS="$((TP * PP * EP))"
if [ "${EXPECTED_GPUS}" -ne "${TOTAL_GPUS}" ]; then
  echo "Error: TP*PP*EP=${EXPECTED_GPUS} but NUM_NODES*GPUS_PER_NODE=${TOTAL_GPUS}." >&2
  echo "  For 2-node TP=4 use GPUS_PER_NODE=2, PP=1, EP=1 (not PP=2 with 1 GPU/node)." >&2
  exit 1
fi
...
if [ "${NSYS_ENABLE}" = "1" ] && [ "${NSYS_PROFILE_SERVER}" = "1" ]; then
  ...
  nsight_copy_msg "TP=${TP} PP=${PP} on ${HEAD_NODE} + ${WORKER_NODES:-<none>} (${GPUS_PER_NODE} GPU(s)/node)"
