...
SCRIPT_VERSION="arc-ray-qwen3-30b-workers-use-nsight-only-v3"

# This variant removes every explicit nsys wrapper.
#
# There is NO:
#   nsys profile python -m vllm.entrypoints.openai.api_server ...
#
# There is NO:
#   nsys profile ray start --block ...
#
# The only Nsight profiling mechanism left is vLLM's worker-side flag:
#   --ray-workers-use-nsight
#
# Goal:
#   isolate vLLM/Ray worker Nsight behavior without nesting or mixing it with
#   API-server profiling or Ray-process profiling.
#
# Important ARC/Slurm observations:
#   Worker Nsight files may appear under either:
#     1. /tmp/ray/session_*/logs/nsight/*
#     2. /tmp/slurm-${SLURM_JOB_ID}/tmp/ray/session_*/logs/nsight/*
#
#   Do not trust /tmp/ray/session_latest because it can point to an old job.
#
#   Do not copy empty.nsys-rep or zero-byte reports. Those are placeholders.
...
#
#   2. /tmp/ray sessions that are referenced by currently live Ray/vLLM
...
copy_ray_nsight_locally_once() {
  ...
  live_sessions="$(
    ps -eo args 2>/dev/null |
      grep -E 'ray|vllm|EngineCore|worker_process' |
...
# The only profiling trigger in this script is --ray-workers-use-nsight.
...
# These env vars are for vLLM worker-side Nsight only.
# No explicit nsys profile command is used in this script.
...
echo "RAY_PROCESS_NSYS_WRAPPER=0"
echo "ONLY_EXPLICIT_WORKER_NSYS_FLAG=--ray-workers-use-nsight"
...
echo "=== vLLM api_server (background process; only --ray-workers-use-nsight) ==="
echo "Starting vLLM server on head node WITHOUT API-server or Ray-process nsys wrappers..."
...
echo "RAY_PROCESS_NSYS_WRAPPER=0"
if [ "${NSYS_ENABLE}" = "1" ] && [ "${NSYS_PROFILE_WORKERS}" = "1" ]; then
  nsight_copy_msg "worker Nsight enabled only via --ray-workers-use-nsight"
  ...
  nsight_copy_msg "worker Nsight disabled because NSYS_ENABLE=${NSYS_ENABLE}, NSYS_PROFILE_WORKERS=${NSYS_PROFILE_WORKERS}"
fi
...
#      There is no API-server nsys wrapper.
#
...
# Do not use any explicit nsys profile wrapper in this script.
# -----------------------------------------------------------------------------
...
echo "NSYS_DIR after server shutdown, expected empty/no API/Ray wrapper reports:"
...
nsight_copy_msg "NSYS_DIR, expected empty/no explicit wrapper reports in this variant: ${NSYS_DIR}"
ls -la "${NSYS_DIR}/" 2>/dev/null || true
