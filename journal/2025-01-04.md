...
- Device-side gating landed for FlashAttention: `_apply_eps_union` now invokes `union_select_for_request` to filter block tables, updates decode counters, and merges telemetry with CPU fallback (`vllm/v1/attention/backends/flash_attn.py`, `vllm/v1/eps/device_union.py`, `vllm/v1/worker/gpu_model_runner.py`).
- CPU union pre-pass now defers to backend-owned groups, device counters merge via `EpsStepCounters.add_from`, and strict guardrails keep uninitialised groups safe.
- Torch import currently crashes in this sandbox (`OMP SHM2`); need to rerun `pytest tests/v1/eps/test_device_union.py` (and rest of EPS suite) once shared memory is available.
- Device-side EPS gating now covers Triton and ROCm backends via shared helper `apply_device_union`, keeping block tables on-device before launch.
- CPU pre-pass skips backend-owned groups while device telemetry aggregates through `EpsStepCounters.add_from`; unit coverage added in `tests/v1/eps/test_device_union.py`.
- Next test run still blocked by torch SHM (`OMP: SHM2`) in this sandbox; rerun the EPS suite once shared memory is available.
- EPS device gating tests (`pytest tests/v1/eps`) pass locally with `OMP_NUM_THREADS=1`; torch SHM error resolved on host.
- Added `modal_app.py` to run EPS pytest/decode smokes on Modal with reusable CPU/GPU functions and automatic editable installs.



### 2025-11-06 Modal debugging notes
- Goal: capture `/tmp/eps_off.jsonl` and `/tmp/eps_on.jsonl` via `vllm.entrypoints.openai.run_batch` on Modal CPU using `facebook/opt-125m`.
- Fixes landed:
  - `modal_app.py` now installs `libnuma-dev`, `numba`, and runs pip installs from `/workspace` so `requirements/common.txt` resolves. Sets CPU-friendly env vars and forces `CpuPlatform` for modal runs.
  - `EngineArgs.from_cli_args` merges all `--eps-*` CLI flags into `EngineArgs.eps_config`; without this, `--eps-method` alone left `enabled=False`.
  - `_init_eps_layer_lookup` now handles nested `attn_groups`, fixing the CPU-only `AttributeError` when union mode initializes.
- Roadblock: even with `--eps-enabled`, no `eps_on.jsonl` is emitted on CPU; reporter logs never fire, so `latest_eps_counters` likely stays `None` (possibly because CPU attention backend never marks any group as device-union-capable).
- Next steps tomorrow:
  1. Instrument `_run_eps_prepass` / `run_union_prepass` and CPU attention stack to confirm where counters drop on CPU.
  2. If CPU can only run host union, ensure we still call `EpsReporter.add_step` immediately after pre-pass so JSONL writes even without device telemetry.
  3. Rerun Modal commands for both EPS on/off once JSONLs appear and archive the metrics.
