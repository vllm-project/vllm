...
def test_voxtral_streaming_forward():
    files = ["/mnt/vast/ci/fixtures/audio/obama.mp3"]
    ...
    def from_file(file_path: str):
        ...
        audio.resample(16_000)
        print(np.abs(audio.audio_array[:9000]).sum())
    ...
    tokenized_list = [
        from_file(file) for file in files
    ]
    ...
    for tokens, audio_array in tokenized_list:
        ...
        import torch
        hey = torch.from_numpy(audio_array[:9000]).to(torch.bfloat16)
        print(hey.abs().sum())
        input_dict = {
...
if __name__ == "__main__":
    test_voxtral_streaming_forward()
