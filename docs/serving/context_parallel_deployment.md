...
`await engine_client.get_effective_attention_block_size()` returns the initialized
full-attention block size in tokens, including DCP scaling: a physical block of
...
engine replicas disagree. Clients can retain their existing behavior when it is
unavailable. This describes a full block; partial cache events carry their own size.
