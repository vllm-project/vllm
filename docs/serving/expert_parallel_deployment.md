...
3. **For disaggregated serving**: Install `gdrcopy` by running the [`install_gdrcopy.sh`](gh-file:tools/install_gdrcopy.sh) script (e.g., `gdrcopy_install.sh "${GDRCOPY_OS_VERSION}" "12.8" "x64/aarch64"`). You can find available OS versions [here](https://developer.download.nvidia.com/compute/redist/gdrcopy/CUDA%2012.8/).
...
1. **Install gdrcopy/ucx/nixl**: Run the [`install_gdrcopy.sh`](gh-file:tools/install_gdrcopy.sh) script to install `gdrcopy` (e.g., `gdrcopy_install.sh "${GDRCOPY_OS_VERSION}" "12.8" "<x64/aarch64>"`). You can find available OS versions [here](https://developer.download.nvidia.com/compute/redist/gdrcopy/CUDA%2012.8/). `nixl` and `ucx` are installed as dependencies via pip.
