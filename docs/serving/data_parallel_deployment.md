...
- `--data-parallel-size-local` has to be at least 1 even if a single DP group requires multiple nodes, *e.g.* in case a single model replica needs to run on at least two nodes
