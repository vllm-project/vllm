...
This document focuses on online deployments (with the API server). DP + EP is also supported for offline usage (via the LLM class), for an example see [examples/basic/offline_inference/data_parallel.py](../../examples/basic/offline_inference/data_parallel.py).
