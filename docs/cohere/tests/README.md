...
| `asr` | ~15m | Speech-to-text regression bucket for Cohere ASR. See [Cohere ASR](./features/asr.md). Currently routed only to the 1xH100 runner. Downloads `cohere-transcribe-03-2026`, covers ASR preprocess-worker config checks, per-request VAD parsing / chunking / Whisper regressions, short- and long-form WER regressions, the long-audio streaming WER gate, plus transcription spacing and cancellation endpoint tests. |
