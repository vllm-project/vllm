...
The [classify][vllm.entrypoints.pooling.PoolingOfflineMixin.classify] method outputs a probability vector for each prompt.
...
The [embed][vllm.entrypoints.pooling.PoolingOfflineMixin.embed] method outputs an embedding vector for each prompt.
...
The [encode][vllm.entrypoints.pooling.PoolingOfflineMixin.encode] method is available to all pooling models in vLLM.
