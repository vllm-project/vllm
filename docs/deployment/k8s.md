...
vLLM supports graceful shutdown via the `--shutdown-timeout` flag. When
the server receives a `SIGTERM` signal (e.g. from Kubernetes during pod
termination), it stops accepting new requests and drains in-flight
requests for up to `--shutdown-timeout` seconds before exiting. Any
...
Set `terminationGracePeriodSeconds` to be at least
`--shutdown-timeout` plus a small buffer (e.g. +30 s) to ensure
Kubernetes does not `SIGKILL` the pod before the drain completes:
  ...
  template:
    metadata:
      labels:
        app: vllm
    spec:
      terminationGracePeriodSeconds: 150  # >= shutdown-timeout + buffer
      containers:
      - name: vllm
        image: vllm/vllm-openai:latest
        command: ["/bin/sh", "-c"]
        args:
        - "vllm serve mistralai/Mistral-7B-Instruct-v0.3 --shutdown-timeout 120"
        ports:
        - containerPort: 8000
        livenessProbe:
          httpGet:
