...
- `NUMEXPR_MAX_THREADS`: specifies maximum number of threads to be used by `numexpr`. The variable only takes effect when `VLLM_CPU_OMP_THREADS_BIND` is set to `nobind`. Default value is determined by `os.sched_getaffinity` (if available) or `os.cpu_count` otherwise.
- `OMP_NUM_THREADS`: specified number of OpenMP threads to use. The variable only takes effect when `VLLM_CPU_OMP_THREADS_BIND` is set to `nobind`. Default value is equal to `torch.get_num_threads` result.
