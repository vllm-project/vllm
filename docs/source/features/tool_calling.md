...
vLLM currently supports named function calling, as well as the `auto`, `required` and `none` options for the `tool_choice` field in the chat completion API.
...
vLLM supports the `tool_choice='required'` option in the chat completion API. Similar to the named function calling, it also uses guided decoding, so this is enabled by default and will work with any model.
