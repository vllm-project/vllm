...
You can either install existing plugins or implement your own. By default, vLLM comes with a [resolver plugin to load LoRA adapters from a local directory on disk.](https://github.com/vllm-project/vllm/tree/main/vllm/plugins/filesystem_resolver)
To enable this resolver, set `VLLM_ALLOW_RUNTIME_LORA_UPDATING` to True, and then set `VLLM_PLUGIN_LORA_CACHE_DIR` to a local directory. When vLLM receives a request using a LoRA adapter `foobar`,
