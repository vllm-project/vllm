...
Note, over mainstream platforms and models (<=70B), it should not take more than **one hour** to load the model, usually just several minutes. So if you have wait for more than one hour, it is likely that the vLLM instance is stuck somewhere.

One notorious crash issue is CUDA OOM (Out-of-Memory) error. If you encounter this issue, please refer to :ref:`CUDA OOM Section (Keep Updating) <cuda-oom>`.
...
- **Too small max_num_seqs**: If you set ``max_num_seqs`` too small (e.g., ``max_num_seqs=4``), after loading the model, you might see the process hanging and not processing any sequence. This is possibly because some racing condition would happen and multiple processes can starve to death. You can set ``max_num_seqs`` to a larger value to see if the issue is resolved.
...
--------------------------------
Usually when you encounter an OOM issue, you will see an error message like ``CUDA out of memory``. Here are some common reasons for OOM issues:
- **Model is too large to fit in GPU memory**: As a rough calculation (here is a `simple tutorial (page 5) <https://docs.google.com/presentation/d/1AduB4KWxX_vLuPmHGsy5r2NiGz8o1w7G/edit?usp=sharing&ouid=111912319459945992784&rtpof=true&sd=true>`_), under common fp16 precision, one `X-B` model would consume about `X*2` GB GPU memory. This is the minimum memory requirement excluding CUDA graph overhead and KV Cache overhead. If your GPU memory summed up is even less than the model size, you will encounter an OOM issue. Try to increase ``tensor_parallel_size`` when you have more GPUs available.
- **Too large gpu_memory_utilization**: This is especially severe when you want to obtain Top-K logits, as it involves unpredictable GPU memory utilization on sorting. Counterintuitively, `gpu_memory_utilization` is not saying how much GPU memory is **used in total**, but how much GPU memory is **reserved for model weights + KV Cache + CUDA Graph** (not including running time memory consumption). If you set `gpu_memory_utilization` to a large value, it will reserve a large amount of memory for the model, which can cause an OOM issue during inference time for non-predictable memory consumption. You can set `gpu_memory_utilization` to a smaller value (e.g., ``0.5``) to reduce the memory usage. But take care not overdo it, otherwise you might also encounter the problem of insufficient reserved memory, which will report as "ValueError: No available memory for the cache blocks. Try increasing `gpu_memory_utilization` when initializing the engine.".
