...
Note: This doc is written based on commit [458e74eb907f96069e6d8a4f3c9f457001fef2ea]. This feature is still in its early stage and things may change.

## What’s This?

Many recent “hybrid” LLMs combine multiple attention types within one model. For example:

1. Sliding window attention (sw) + full attention (full): gpt-oss, Gemma 2/3, Ministral, cohere …  
2. Mamba + full: Bamba, Jamba, Minimax, …  
...
2. Support layer-specific prefix-cache rules, for example:  
    - Full attention: a cache hit prefix requires **all** tokens’ KV remain unevicted.  
    - Sliding window: a cache hit prefix only requires the last **sliding_window_size** tokens’ KV to be unevicted.
...
We use a single memory pool for all layer types. The memory pool is splitted into multiple blocks with the same page size. KVCacheManager allocates different numbers of blocks to different layers according to its attention type.
...
$$
In hybrid models, however, `num_hidden_layers` varies by attention type, which would normally produce mismatched page sizes. The cases below show how we unify them.

### Case 1: toy example

Let’s start with a toy example: a model has 1 full attention layer and 3 sliding window layers. All layers have the same kv hidden size. 
...
### Case 2: model with the same kv\_hidden\_size

When the model has more layers, e.g., 20 sliding window layers and 10 full attention layers with the same kv\_hidden\_size. Calling the allocator once per layer (30 calls) is OK but becomes inefficient. As a solution, we group the allocation of layers that need the same number of blocks to reduce the number of calls.
...
Assume block\_size 16, sliding window size 32, request length 112, then for the above example model, we need to allocate 11 blocks (0-6 for full, 7-8 for sw group 1, 9-10 for sw group 2).
...
Here is the formal definition: the layers are divided into multiple *KV Cache Groups* so that:
...
### Case 3: model with the same kv\_hidden\_size, but bad ratio
...
### Case 4: models with different kv\_hidden\_size (mainly hybrid mamba models)
...
$$
This padding strategy is still WIP.

### Case 5: KV sharing

KV sharing refers to a layer using the kv cache of another layer, e.g., gemma-3n.  
In these models, KVCacheManager ignores all layers with kv sharing and only allocates KV cache for layers that need kv cache, and some patches are made in model runner to apply the allocation result to kv sharing layers.

## Prefix caching

For simplicity, we assume block\_size=1 in this section.
...
### Case 0: models only with full attention layers
...
### Case 1: models only with sliding window layers

For sliding window layers, a naive implementation for memory allocation is to allocate sliding\_window\_size blocks and fill in the blocks in a round-robin way. But this naive implementation is not compatible with prefix caching so we didn’t pick this design. In vLLM,  we allocate different blocks for different tokens and free blocks that are outside the sliding window.

For a new request, the cache hit prefix only requires the last window size \- 1 tokens being cached.   
Let’s say sliding window size 4 and block size 1, and the request is a 15-token prompt (blue blocks are cached)
...
### Case 2: sliding window layer + full attention layer
...
1\. Get the longest cache hit for global attention (scanning from left to right)  
2\. Get the longest cache hit for SWA that is within that length. Implemented by checking cache hits from right to left starting from the cache hit length of global attention.

It can be ensured that the resulting cache hit of SWA layers is also a cache hit of full attention layers. This is more efficient than finding all possible prefixes of each group and doing the intersection, because our approach can exit early if there is no cache hit.
...
The prefix caching support of the mamba model is WIP. Once implemented, models with mamba layer + full attention layer can be supported via the full attention + X algorithm in case 2.
...
With such an example, the physical memory is divided into 10 buffers (KVCacheTensor 0 - KVCacheTensor 9). Each buffer is shared by 3 layers (e.g., KVCacheTensor 0 is shared by full.0 from group 0, sw.0 from group 1, and sw.10 from group 2) and is divided into pieces with size block\_size \* kv\_hidden\_size. The KV cache of these 3 attention layers are saved to different pieces of the buffer based on the allocated block\_ids. Note that one logic “block” is mapped to 10 pieces in the 10 buffers of the physical memory.
