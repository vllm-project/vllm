...
- `vllm:time_to_first_token_seconds` (Histogram) - Time to first token (TTFT). Labeled by `operation` (`chat` / `text_completion` / `unknown`). Not recorded for `embeddings`.
- `vllm:inter_token_latency_seconds` (Histogram) - Inter-token latency. Labeled by `operation` (`chat` / `text_completion` / `unknown`). Not recorded for `embeddings`.
- `vllm:e2e_request_latency_seconds` (Histogram) - End-to-end request latency. Labeled by `operation` (`chat` / `text_completion` / `embeddings` / `unknown`).
