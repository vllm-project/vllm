...
- `vllm:kv_cache_token_usage_perc` (Gauge) - Fraction of used KV cache *token slots* (0–1). Always at most `vllm:kv_cache_usage_perc`; the difference is space lost to partially filled blocks. Equal to `vllm:kv_cache_usage_perc` for hybrid models.
