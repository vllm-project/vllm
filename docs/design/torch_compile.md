...
By default, the cache saves compiled artifacts as binary files. If you would like to interact with the generated code for debugging purposes, set the field `compile_cache_save_format=unpacked` in the compilation config, or override this with the env variable `VLLM_COMPILE_CACHE_SAVE_FORMAT=unpacked`.
