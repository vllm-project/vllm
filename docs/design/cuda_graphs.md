...
O2/O3 optimization defaults enable some of the above fusion passes, so they may require explicit overrides. Launch example (use either the dotted form or a single JSON blob for `--compilation-config`; mixing the two forms keeps only the last one):
...
vllm serve <model> --tensor-parallel-size 2 \
  ...
  --compilation-config.pass_config.enable_sp false \
  --compilation-config.pass_config.fuse_gemm_comms false \
  --compilation-config.pass_config.fuse_allreduce_rms false \
  --compilation-config.pass_config.fuse_attn_quant false
```
