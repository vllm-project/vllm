...
Ragged decode batches, where each request carries a different number of query tokens read from the device `query_start_loc` (as in adaptive verification), are declared separately and not ordered against the enum: `AttentionMetadataBuilder.get_varlen_decode_cudagraph_max_query_len()` returns the largest per-request query length a FULL decode graph can replay, or 0 if it cannot. Builders reporting `ALWAYS` support every length and return `max_num_batched_tokens`; FlashInfer returns `1 + num_speculative_tokens` when its TRTLLM-GEN ragged decode path is active.
