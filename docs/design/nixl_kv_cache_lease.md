...
**Reclaiming expired leases:** P's `get_finished()` sweeps **every** outstanding lease each step rather than stopping at the first unexpired one. Deadlines in that map are not ordered by expiry: heartbeats extend an existing entry in place, and two independent TTLs (`kv_lease_duration` and, in bidirectional mode, `decoder_kv_blocks_ttl`) feed the same map, so a long deadline can sit ahead of a short one. An early-exit sweep would strand those blocks --- and the request-plane slot holding them --- indefinitely.
