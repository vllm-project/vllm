...
| `fuse_attn_quant`\*          | FP8 static\*, FP8 per-group‡\*, NVFP4\*  | FP8 static\*, FP8 per-group‡\*           | FP8 static\*, FP8 per-group‡\*           | —             | FP8 static\*, FP8 per-group‡\* (untested)|
...
- `TRITON_ATTN`: CUDA, ROCm (untested) (requires `head_size % group_size == 0`)
