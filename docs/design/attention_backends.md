...
> **†** FlashInfer Native is the regular FlashInfer path. XQA is the SM90 decode path exposed through FlashInfer's TRTLLM decode API. trtllm-gen is used on SM100 and supports sinks. MM Prefix uses native FlashInfer prefill and is not supported with TRTLLM-only block sizes or attention sinks. Disable XQA/trtllm-gen via `--attention-config.use_trtllm_attention=0`.
>
