...
| `VLLM_ENABLE_FLASHINFER_AUTOTUNE_CACHE` | `0` | Enables the persistent FlashInfer autotune config cache. Disabled by default. |
| `VLLM_FLASHINFER_AUTOTUNE_CACHE_DIR` | `$VLLM_CACHE_ROOT/flashinfer_autotune_cache/<flashinfer-version>/<arch>/<cache-hash>/` | FlashInfer autotune config cache location, used when `VLLM_ENABLE_FLASHINFER_AUTOTUNE_CACHE=1`. |
