...
Which of the four services are mounted is controlled by `--grpc-services`. The default, `all`, mounts all four whatever the engines are configured for; RPCs for an unconfigured feature answer `FailedPrecondition`. Hardening the port is opt-in: `configured` mounts `Inference` and `Control`, adds `KvTransfer` when a KV connector or ZMQ KV cache events are configured, and adds `RlControl` when every engine has weight transfer or sleep mode configured. A comma-separated list of `inference`, `control`, `kv-transfer`, `rl-control` pins the set exactly, and naming a service the engines are not configured for fails startup. An unmounted service answers `Unimplemented`. `Control.GetServerInfo` reports the mounted set in its `services` field, which is readable only when `Control` itself is mounted. The RPCs that moved off `Control` to `KvTransfer` and `RlControl` remain callable at their old `Control` paths for one release, deprecated, and follow the mounted set: they answer `Unimplemented` when the service they moved to is not mounted, so a mesh policy that filters on the `vllm.Control` prefix must still allow for them until they are removed.

**Warning:** The gRPC interface implements **no authentication or authorization of its own**. Encryption is available: the gRPC listener uses the same TLS settings as the HTTP server (`--ssl-certfile`, `--ssl-keyfile`, `--ssl-ca-certs`, `--ssl-cert-reqs`), so with `--ssl-cert-reqs 2` only clients presenting a certificate from your CA can connect, but every such client can call every RPC. Treat it as a private, internal interface intended for use only between services within a trusted network. Do not expose the gRPC port to the public internet or untrusted clients. Protect it with mTLS or network-level access controls such as firewall rules, network segmentation, or a service mesh policy that restricts which identities may call which methods.
...
- Only enable `--grpc-port` when you have a specific need for it: gRPC-based inference, or NIXL prefill/decode disaggregation on the Rust frontend, which uses the `KvTransfer` service for the handshake
- Narrow the mounted set with `--grpc-services`, which mounts everything by default. `--grpc-services configured` drops the services the engines are not configured for. For prefill/decode disaggregation without RL, `--grpc-services control,kv-transfer` is the recommended prefill setting: it keeps the handshake plus whatever the router needs from `Control`, and removes the `Generate` and RL weight-update RPCs from the port entirely rather than relying on a mesh policy to reject them. On an instance no router queries, `--grpc-services kv-transfer` leaves only the handshake
- With a KV connector configured, the `KvTransfer` service serves the NIXL handshake metadata, which includes the RDMA registration of the KV cache, and decode instances call it to pair with a prefiller. Peer instances then share a port with `Generate`, `PauseGeneration`, `Sleep` and the weight-update RPCs, so restrict them **per method**: a peer only calls `GetKvHandshakeMetadata` (and `GetKvTransferInfo` to check compatibility), so a service mesh policy can admit the peer identity to those two paths and the router identity to the rest. `KvTransfer` also serves `GetKvEventSources`, which reports the KV event stream endpoint, so the service prefix alone is wider than a peer needs. See [Router discovery over the gRPC control plane](../features/nixl_connector_usage.md#router-discovery-over-the-grpc-control-plane)
- The NIXL ZMQ side channel keeps listening while `handshake_transport` is `auto`. Once every peer runs a control plane, set `handshake_transport` to `grpc` on the prefillers so the unauthenticated side-channel port closes
...
- **The Rust frontend bundles its own OpenSSL.** `vllm-rs` is built with a vendored `native-tls`, so TLS on its HTTP and gRPC listeners, including the KV transfer control plane, does not use the host's FIPS provider the way the Python API server does. Treat it like the dependencies below until it can link the host library.
