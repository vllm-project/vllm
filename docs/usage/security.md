...
vLLM provides optional gRPC `Inference`, `Control`, `KvTransfer`, `RlControl` and `Lora` services on a separate TCP port, enabled via the `--grpc-port` flag. When not specified, no gRPC server is started. The gRPC listener binds to the same host address as the HTTP server.

Which of the five services are mounted is controlled by `--grpc-services`. The default, `configured`, mounts `Inference` and `Control`, adds `KvTransfer` when ZMQ KV cache events are configured, adds `RlControl` when every engine has weight transfer or sleep mode configured, and adds `Lora` when every engine has LoRA enabled, so an engine that is not set up for RL or LoRA does not expose the pause, sleep, weight-update or adapter-loading RPCs. A comma-separated list of `inference`, `control`, `kv-transfer`, `rl-control`, `lora` pins the set exactly, and naming a service the engines are not configured for fails startup. `all` mounts all five whatever the engines are configured for; RPCs for an unconfigured feature then answer `FailedPrecondition`. An unmounted service answers `Unimplemented`. `Control.GetServerInfo` reports the mounted set in its `services` field, which is readable only when `Control` itself is mounted. The RPCs that moved off `Control` to `Lora`, `KvTransfer` and `RlControl` remain declared at their old `Control` paths for one release, deprecated, and are served only under `--grpc-services all`; with `configured` or an explicit list they answer `Unimplemented`, so each RPC is reachable only under the prefix of the service it belongs to. `Control.ListLoras` is the exception: it is a read-only view that stays on `Control` permanently so a router admitted only to `Control` can see which adapters are loaded, while `LoadLora` and `UnloadLora` load weights from disk and belong to `Lora`. Use `all` only while existing clients still call the old paths, and note that it lets a caller admitted to `/vllm.Control/*` reach every `Lora`, `KvTransfer` and `RlControl` RPC through the aliases.
