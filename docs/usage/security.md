...
## Built-in Tool Security (GPT-OSS)

vLLM integrates with [GPT-OSS](https://github.com/openai/gpt-oss) to provide built-in tools such as a code interpreter (Python execution via Docker) and a web browser (via Exa API). These tools are activated by passing `--tool-server demo` and enabling specific tool labels through `VLLM_GPT_OSS_SYSTEM_TOOL_MCP_LABELS`.

**Important:** No built-in tools are enabled by default. They must be explicitly opted into via configuration.

### Code Interpreter (Python Tool) Security Risks

The GPT-OSS Python tool executes model-generated code inside a Docker container. However, the container is **not configured with network isolation by default**. It inherits the host's Docker networking configuration (e.g., default bridge network or `--network=host`), which means:
...
Built-in tools are controlled by two settings:

1. **`--tool-server`**: Must be set to `demo` to enable built-in tools, or set to a `host:port` pair for external MCP tool servers.

2. **`VLLM_GPT_OSS_SYSTEM_TOOL_MCP_LABELS`**: A comma-separated allowlist of tool labels to enable. Valid values are:
   - `container` - Container tool
   - `code_interpreter` - Python code execution tool
   - `web_search_preview` - Web search/browser tool

   If this variable is not set or is empty, no system tools from MCP servers will be enabled.
