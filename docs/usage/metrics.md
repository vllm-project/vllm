...
The Prometheus endpoint records the distribution of request and scheduled-token
counts in every model-executing engine iteration, split by `phase="prefill"`
and `phase="decode"`. These cumulative `vllm:iteration_requests` and
...
that correlation is required. They are unavailable with `--disable-log-stats`.
