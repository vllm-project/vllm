...
For instructions on installing appropriate NIC userspace libraries, see [Installing NIC userspace libraries](#installing-nic-userspace-libraries).

### Configuration

The connector is configured in two levels: on the application level, and on the transport level.

### Application-level Configuration

**Modes:** MoRI has two different modes of operation: WRITE and READ mode.

- In WRITE mode (default), the producer actively pushes computed KV blocks after every layer into the consumer's memory.
- In READ mode the consumer pulls the KV blocks from the producer all at once, as soon it has been notified those blocks are ready.
...
**Control-plane configuration:** While MoRI moves the actual KV bytes over an RDMA (or xGMI) fast-path, the producer and consumer instances still need a set of out-of-band TCP channels to exchange block ids, RDMA queue-pair metadata, report liveness, and signal transfer completions.

The following keys are used to configure this metadata exchange, all part of the `kv_connector_extra_config`:

- `proxy_ip`: IP address of the disaggregation proxy/router that fronts the prefiller and decoder. Each vLLM instance uses it to register itself and to send heartbeats so the proxy knows where it can route incoming requests.
...
- `notify_port`: TCP port used for control and synchronization messages between prefiller and decoder. Used differently in the two modes.
    - WRITE mode: (1) Block allocation: decoder notifies prefiller about its block ids, so prefiller can push its computed KV blocks into the correct place on the decoder instance. (2) Completion: once all blocks have been transferred, prefiller notifies decoder that it's safe to use its blocks.
    - READ mode: once decoder has read all blocks from the prefiller, it notifies the prefiller so it can free its KV cache blocks.
...
The configuration options for each backend is as follows.
...
> [!TIP]
> Example: to set the number of QP's per transfer to 4, use the flag `--kv-transfer-config.kv_connector_extra_config.qp_per_transfer 4`. All other flags are
set analagously.
...
**Fix:** The installed RDMA userspace libraries does not match the driver and firmware version installed on the host. You must install NIC userspace libraries corresponding to you RDMA kernel module and firmware version. See [Installing NIC userspace
libraries](#installing-nic-userspace-libraries) for more information.

## Installing NIC userspace libraries
