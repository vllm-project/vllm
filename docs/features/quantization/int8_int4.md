...
??? code
    ...
    recipe = [
        GPTQModifier(targets="Linear", scheme="W4A8", ignore=["lm_head"], dampening_frac=0.01),
    ]
    ...
    # Save the compressed model: Meta-Llama-3-8B-Instruct-W4A8-Dynamic-Per-Token
    SAVE_DIR = MODEL_ID.split("/")[1] + "-W4A8-Dynamic-Per-Token"
    model.save_pretrained(SAVE_DIR, save_compressed=True)
    tokenizer.save_pretrained(SAVE_DIR)
    ```

This process creates a W4A8 model with weights quantized to 4-bit integers and activations to 8-bit integers.
...
llm = LLM("./Meta-Llama-3-8B-Instruct-W4A8-Dynamic-Per-Token")
```
...
```bash
lm_eval --model vllm \
  --model_args pretrained="./Meta-Llama-3-8B-Instruct-W4A8-Dynamic-Per-Token",add_bos_token=true \
  --tasks gsm8k \
  --num_fewshot 5 \
  --limit 250 \
  --batch_size 'auto'
