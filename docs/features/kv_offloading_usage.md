...
| `calibrate_load_path` | no | `false` | both | Opt-in, CUDA only. When `true`, each worker times the two CPU→GPU load paths (`cuMemcpyBatchAsync` and the Triton kernel) for its own copy size when the offloading handler is created, and uses the measured batch-size crossover instead of the built-in constants, which were tuned on a single GPU. Adds about 0.5–1 s to startup, most of which is the first Triton launch that the first load would otherwise pay. Copy sizes above 64 KiB are not measured and keep the defaults. |
