...
| `store_threshold` | no | `0` | single-tier | Min distinct requests that produce a storable block before it is offloaded. Values ≥ 2 are rejected by `TieringOffloadingSpec`. |
| `max_tracker_size` | no | `64000` | single-tier | Max entries in the admission tracker. |
