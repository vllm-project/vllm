...
| `pin_in_flight_chunks` | no | `false` | single-tier | Prefer evicting chunks of finished requests over chunks an unfinished request will resume from. Falls back to normal eviction when every chunk is in flight. Useful for colocated RL, where rollouts stay in flight across a weight update. |
