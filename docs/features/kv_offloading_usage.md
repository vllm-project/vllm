...
| `host` | no | `$VLLM_P2P_SIDE_CHANNEL_HOST` (`0.0.0.0`) | Address the control socket binds to. When omitted, resolves from the env var below. |
...
- `VLLM_P2P_SIDE_CHANNEL_HOST` (default `0.0.0.0`): address the P2P control socket binds to. Defaults to `0.0.0.0` (bind all interfaces); set to the node IP for cross-host P/D.
