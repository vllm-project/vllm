...
- `--reasoning-config` defines the reasoning boundary tokens (e.g., `think_start_str`, `think_end_str`). If not set, vLLM will attempt to automatically initialize these tokens from the reasoning parser.
