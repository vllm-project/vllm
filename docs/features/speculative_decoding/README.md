...
| `num_speculative_tokens` | `integer > 0` | `None` | Number of speculative tokens to propose per step. Loaded from checkpoint metadata when available (speculators-format declarations, MTP `n_predict`, or DFlash/DSpark block-size defaults); otherwise required. |
