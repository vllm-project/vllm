...
3. **`--compiled` is for speed experiments only.** It applies best-effort
   ...
   current stack.
4. **Scoring runs on the V1 model runner.** Score mode is implemented in
   `vllm/v1/worker/gpu_model_runner.py` only. The V2 runner's
   `PromptLogprobsWorker` does not know about `score_mode`, `kld_mode`, or
   `return_prompt_logits`, so it would silently return ordinary prompt
   logprobs. The example scripts set `VLLM_USE_V2_MODEL_RUNNER=0` for you;
   API users must set it themselves, because V2 is selected automatically for
   some architectures.
...
# Must be set before the engine resolves its config.
...
- Requires the V1 model runner (`VLLM_USE_V2_MODEL_RUNNER=0`).
