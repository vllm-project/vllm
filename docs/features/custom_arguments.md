...
repeating an n-gram of that size; `0` or an omitted value disables the feature.
Prompt tokens are not included in the n-gram history.

``` python
SamplingParams(extra_args={"no_repeat_ngram_size": 100})
```
