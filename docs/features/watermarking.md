...
When a generated-token context has already appeared, generation can use ordinary
sampling for that occurrence. Reusing the keyed random vector would otherwise
correlate token choices and weaken the sequence-level distribution-preserving
guarantee. The detector independently deduplicates contexts so repeated keyed
random vectors are not treated as independent evidence.

`deduplicate_contexts` controls the generation history scope:

- `"none"` disables context deduplication.
- `"single_turn"` checks contexts generated by the current request. This is the
  default and provides the sequence-level guarantee within one generation.
- `"all"` checks the supplied token history, including the prompt and prior
  turns. When the full conversation is supplied and the history limit covers
  it, this preserves the single-sequence non-distortion guarantee across
  multiple generated turns. Scanning prompt history can reduce inference speed,
  and contexts from non-generated prompt tokens are conservatively treated as
  used, so substantially fewer generated tokens may be watermarked.

`deduplicate_contexts_max_history` limits each check to the most recent prior
positions and defaults to 8,192. A smaller value reduces scanning cost but only
provides the guarantee within that window.
...
Context deduplication can be disabled for comparison or compatibility with
earlier behavior:

```bash
vllm serve MODEL \
  --watermark-config \
  '{"algorithm":"gumbel","key":42,"deduplicate_contexts":"none"}'
```
