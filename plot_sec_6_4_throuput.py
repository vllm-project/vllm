...
def plot_throughput(
    ...
    # num_shot -> system -> (request_rate, normalized_latency)
    ...
    for save_dir in save_dirs:
        ...
        for seq in results:
            ...
            per_seq_num_tokens.append(total_num_tokens)
            per_seq_latencies.append(latency)
            if get_request_rate(save_dir) == 1.0 and get_num_shot(save_dir) == 5:
                print(f'total_num_tokens {total_num_tokens} = {prompt_len} + {output_len}')

        request_rate = get_request_rate(save_dir)
        normalized_latency = np.mean(per_seq_norm_latencies)
        throughput_token = sum(per_seq_num_tokens) / sum(per_seq_latencies)
        throughput_request = len(results) / sum(per_seq_latencies)
        num_shot = get_num_shot(save_dir)
        print(f'num_shot {num_shot} + request_rate {request_rate}:\n {throughput_token} = {sum(per_seq_num_tokens)} / {sum(per_seq_latencies)} tokens/s\n  {throughput_request} = {len(results)} / {sum(per_seq_latencies)} requests/s')

        
        if num_shot not in perf:
            perf[num_shot] = {}
            plot_names.append(num_shot)
        system_name = get_system(save_dir)
        if system_name not in perf[num_shot]:
            perf[num_shot][system_name] = ([], [])
        perf[num_shot][system_name][0].append(request_rate)
        perf[num_shot][system_name][1].append(normalized_latency)

        if num_shot not in x_top:
            x_top[num_shot] = 0
        else:
            x_top[num_shot] = max(x_top[num_shot], request_rate)
    ...
    for i, (num_shot, ax) in enumerate(zip(plot_names, axs)):
        curves = []
        legends = []
        for system_name in SYSTEMS:
            ...
            request_rates, normalized_latencies = perf[num_shot][system_name]
            ...
            curve = ax.plot(request_rates, normalized_latencies, label=label, color=color, marker=marker, markersize=2)
            curves.append(curve[0])
            legends.append(label)
            for x, y in zip(request_rates, normalized_latencies):
                ax.annotate(f'({x}, {y:.6f})', (x, y), textcoords="offset points", xytext=(5, 5), ha='center', fontsize=4)

        enum = get_alpha_enum(i + label_offset)
        if subset == 'prefix':
            sub_name = num_shot + '-shot prefix prompt'
        ax.set_xlabel(f'Request rate (req/s)\n\n{enum} {sub_name}', fontsize=14)
        ...
        if xlim is not None:
            ...
            ax.set_xlim(left=0, right=x_top[num_shot] * 1.1)
        if ylim is not None:
            if log_scale:
    ...
    fig.text(-0.03, 0.5, 'Throughput\n       (tokens/s)', va='center', rotation='vertical', fontsize=14)
    if subset == 'prefix':
        fig.legend(curves, legends, loc="upper center", ncol=5, bbox_to_anchor=(0.5, 1.18), fontsize=14, frameon=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--subset', choices=['prefix'], default='prefix')
