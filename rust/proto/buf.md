...
`Control` still declares the RPCs that moved to `KvTransfer` and `RlControl`, marked deprecated, for one release. The frontend serves them only under `--grpc-services all`.
