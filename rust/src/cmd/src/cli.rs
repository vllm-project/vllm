...
pub struct SharedRuntimeArgs {
    ...
    /// Static URL path prefix for reverse-proxy deployments (e.g., `/api`).
    /// When set, all routes are also served under this prefix so the server
    /// responds on both the bare and prefixed paths.
    #[arg(long)]
    #[serde(default)]
