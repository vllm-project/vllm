...
`tests/cli_parity.rs` keeps the CLI a superset of Python `vllm bench serve` flags
(snapshot `tests/python_serve_flags.txt`, regenerated by
`tests/benchmarks/test_rust_bench_cli_parity.py` at the repo root; deliberate gaps
live in its `PYTHON_ONLY` allowlist). When adding a flag that exists in Python
under another name, add a clap `visible_alias` for the Python spelling.
