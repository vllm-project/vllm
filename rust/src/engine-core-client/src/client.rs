...
impl EngineCoreClient {
    ...
    ///
    /// A closing ADD is a marker only: the engine discards its `prompt_token_ids`
    /// and `mm_features`, so the last chunk of input has to go out in a
    /// `resumable: true` continuation before it.
