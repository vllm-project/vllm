...
/// The cleanup abort is sent from a detached task, so it has to be awaited
/// rather than polled. Bounded so a regression fails instead of hanging.
async fn decode_abort(dealer: &mut DealerSocket) -> Vec<String> {
    let message = timeout(Duration::from_secs(10), recv_engine_message(dealer))
        .await
        .expect("timed out waiting for the client to abort the retired session");
    assert_eq!(
        message[0].as_ref(),
        EngineCoreRequestType::Abort.to_frame().as_ref(),
        "expected an ABORT frame"
    );
    rmp_serde::from_slice(&message[1]).expect("decode ABORT")
}
...
/// Expect the client to drop engine state it can no longer reach through its
/// own registry.
fn expect_abort() -> EngineStep {
    EngineStep::ExpectAbort
}

/// Emit a stop the session cannot resume from, which ends it outright.
fn emit_unresumable_stop(tokens: Vec<u32>) -> EngineStep {
    EngineStep::Emit {
        tokens,
            ...
            Box::pin(async move {
                for step in script {
                    match step {
                        ...
                        EngineStep::ExpectAbort => {
                            assert_eq!(
                                decode_abort(dealer).await,
                                vec![RESUMABLE_SESSION_ID.to_string()],
                                "the client must abort exactly the retired session"
                            );
                        }
                    }
                }
...
/// A stop the session cannot resume from retires it client-side while the engine
/// still holds the request, so the client has to tell the engine to drop it.
/// Without that, the session would sit in the scheduler holding its KV blocks and
/// a request slot with nothing left to abort it.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn resumable_session_aborts_leftover_engine_state_on_an_unresumable_stop() {
    init_tracing();
    let fixture = resumable_session_fixture(vec![
        expect_add(true),
        emit_unresumable_stop(vec![5]),
        expect_abort(),
    ])
    .await;
    ...
    assert_eq!(drain_resumable_stream(stream).await, vec![5]);
    fixture.shutdown().await;
}
