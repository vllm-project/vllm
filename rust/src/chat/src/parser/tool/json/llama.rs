...
use crate::parser::tool::utils::{JsonObjectScanState, parse_buffered_event};
use crate::parser::tool::{
    Result, ToolCallDelta, ToolParseResult, ToolParser, ToolParserError, parsing_failed,
};
...
mod tests {
    ...
    use crate::parser::tool::{ToolParseResult, ToolParser, ToolParserFactory, names};
  ...
  "flag": true
    ...
    #[test]
    fn llama_json_factory_registers_llama3_and_llama4_aliases() {
        let tools = test_tools();

        ToolParserFactory::global().create(names::LLAMA3_JSON, &tools).unwrap();
        ToolParserFactory::global().create(names::LLAMA4_JSON, &tools).unwrap();
    }
}
