...
@torch.compile(fullgraph=True, dynamic=False, backend="openxla")
def bgmv_shrink(inputs: torch.Tensor, loras: torch.Tensor, idxs: torch.IntTensor):
    return torch.ops.xla.bgmv_shrink(inputs, loras, idxs)

@torch.compile(fullgraph=True, dynamic=False, backend="openxla")
def bgmv_expand(inputs: torch.Tensor, loras: torch.Tensor, idxs: torch.IntTensor, enable_laning: bool):
...
def shrink_and_expand(inputs: torch.Tensor, loras_a: torch.Tensor, loras_b: torch.Tensor, idxs: torch.IntTensor):
    return bgmv_expand(
        bgmv_shrink(inputs, loras_a, idxs),
        loras_b,
        idxs,
        enable_laning=True
    )
...
def test_bmark_shrink(benchmark, T, D, L, N, func):
    ...
    benchmark.pedantic(partial(run_and_wait_torch, func), args=(inputs, loras, idxs), rounds=5, warmup_rounds=5, iterations=10)
...
def test_bmark_shrink_and_expand(benchmark, T, D, L, N, func):
    ...
    benchmark.pedantic(partial(run_and_wait_torch, func), args=(inputs, loras_a, loras_b, idxs), rounds=5, warmup_rounds=5, iterations=10)
