...
def process_channel_group(orig_img, channels):
    ...
    orig_img = torch.clamp(
        (orig_img - min_value) / (max_value - min_value), 0, 1
    )
...
def run_model(
    ...
    num_batches = (
        windows.shape[0] // batch_size if windows.shape[0] > batch_size else 1
    )
    ...
    if temporal_coords:
        temporal_coords = torch.tensor(
            temporal_coords, device=device
        ).unsqueeze(0)
    else:
        temporal_coords = None
    if location_coords:
        location_coords = torch.tensor(
            location_coords[0], device=device
        ).unsqueeze(0)
    else:
