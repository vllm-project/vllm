...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--gpqa-jsonl",
        default=None,
        help="Optional GPQA JSONL containing prompt/answer records.",
    )
    parser.add_argument("--gpqa-index", type=int, default=17)
        ...
        steps.append(
            {
                ...
                "retrieved_count_mean": _mean(retrieved),
                "retrieved_unique_count_mean": _mean(unique),
            }
        )
...
def main() -> None:
    ...
    if args.gpqa_jsonl:
        with open(args.gpqa_jsonl, encoding="utf-8") as fh:
            rows = [json.loads(line) for line in fh if line.strip()]
        question = rows[args.gpqa_index]["prompt"]
            ...
            [
                {
                    ...
                    "content": (
                        ...
                        "then end with exactly `Answer: X`, where X is A, B, C, or D."
                        f"\n\n{question}"
                    ),
                }
            ],
    ...
    if prompt_tokens + args.output_tokens <= args.threshold:
        raise RuntimeError(
            f"Request ends at {prompt_tokens + args.output_tokens} tokens, below "
            f"threshold {args.threshold}; increase --output-tokens"
        )
    ...
    summary["gpqa_jsonl"] = args.gpqa_jsonl
    summary["gpqa_index"] = args.gpqa_index if args.gpqa_jsonl else None
