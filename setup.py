...
def _list_reachable_release_tags(
    release_version: str,
) -> list[tuple[Version, str, str]]:
    tag_pattern = f"v{release_version}.post*"
    tags_output = _git_maybe_output(
        "tag",
        "--merged",
        "HEAD",
        "--list",
...
def vllm_version_scheme(version: ScmVersion) -> str:
    metadata = load_upstream_metadata()
...
def vllm_local_scheme(version: ScmVersion) -> str:
...
def get_vllm_version() -> str:
    ...
    if env_version := os.getenv("VLLM_VERSION_OVERRIDE"):
        ...
        return get_version(
            ...
            write_to_template=vllm_version_template(),
            version_scheme=vllm_version_scheme,
            local_scheme=vllm_local_scheme,
            parse=parse_vllm_scm_version,
            dist_name="vllm-hust",
        )
