# Example usage:
# Without token parallelism: torchrun --nproc-per-node=8 vllm-inference-bench/simple_generate.py --tensor-parallel-size 8 --pipeline-parallel-size 1 --data-parallel-size 1 --enable-expert-parallel
# With token parallelism: torchrun --nproc-per-node=8 vllm-inference-bench/simple_generate.py --tensor-parallel-size 8 --pipeline-parallel-size 1 --data-parallel-size 1 --enable-token-parallel --token-parallel-size 2

# torchrun --nproc-per-node=8 vllm-inference-bench/simple_generate.py --tensor-parallel-size 1 --pipeline-parallel-size 1 --data-parallel-size 8 --enable-expert-parallel --model deepseek-ai/DeepSeek-V2-Chat-0628
# VLLM_ALL2ALL_BACKEND=pplx VLLM_USE_DEEP_GEMM=1 torchrun --nproc-per-node=8 simple_generate.py --tensor-parallel-size 1 --pipeline-parallel-size 1 --data-parallel-size 8 --enable-expert-parallel --model deepseek-ai/DeepSeek-V2-Chat-0628
...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default="deepseek-ai/DeepSeek-V2-Chat-0628",
                        help="Model name (default: deepseek-ai/DeepSeek-V2-Chat-0628)")
...
def main():
    ...
    llm_kwargs = {
        ...
        "quantization": None,  # Explicitly set to None
    }
