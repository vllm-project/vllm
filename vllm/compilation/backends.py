...
def make_compiler(compilation_config: CompilationConfig) -> CompilerInterface:
    if compilation_config.use_inductor:
        ...
        # 2. AOT compile is enabled (which requires serialization support), OR
        # 3. Backend with inductor cache is enabled (which requires serialization support)
        ...
        should_use_standalone = (
            ...
            or envs.VLLM_USE_BACKEND_WITH_INDUCTOR_CACHE
        )
        if (
            should_use_standalone
            ...
            if (
                envs.VLLM_USE_AOT_COMPILE or envs.VLLM_USE_BACKEND_WITH_INDUCTOR_CACHE
            ) and not envs.VLLM_USE_STANDALONE_COMPILE:
                logger.info(
                    "AOT compile or inductor cache backend is enabled, automatically using "
                    "InductorStandaloneAdaptor for serialization support"
                )
            logger.debug("Using InductorStandaloneAdaptor")
            return InductorStandaloneAdaptor()
        else:
            if envs.VLLM_USE_AOT_COMPILE or envs.VLLM_USE_BACKEND_WITH_INDUCTOR_CACHE:
                logger.warning(
                    "VLLM_USE_AOT_COMPILE or VLLM_USE_BACKEND_WITH_INDUCTOR_CACHE is set but "
                    "standalone compile is not available. These features require PyTorch 2.8.0+ "
                    "with standalone_compile support. Falling back to InductorAdaptor without "
...
class VllmBackendWithCache:
    ...
    def __init__(
        self,
        inductor_cache: Any,  # VllmSerializableFunction.InductorCache
        vllm_config: VllmConfig,
        prefix: str = "",
        submod_names: list[str] = None,
        ...
        Args:
            inductor_cache: The inductor cache containing compiled artifacts
            vllm_config: The vLLM configuration
    ...
    def build_dispatch_callable(self):
        ...
        for submod_name in self.submod_names:
            ...
            for cache_key in self.inductor_cache.submodule_bytes:
                if cache_key.startswith(f"{submod_name}_"):
                    shape_str = cache_key[len(submod_name) + 1 :]
                    compiled_fn = self.inductor_cache.get_loaded(submod_name, shape_str)
    ...
    def create_split_gm_from_cache(self, split_gm: fx.GraphModule):
...
class VllmBackend:
    ...
    def _collect_inductor_cache(self, submod_names: list[str]):
        """Collect inductor cache artifacts from all piecewise backends.

        Returns:
            tuple: (inductor_cache, sym_shape_indices_map, returns_tuple_map)
                - inductor_cache: InductorCache with compiled artifacts
                - sym_shape_indices_map: dict mapping submod_name to
                  sym_shape_indices
                - returns_tuple_map: dict mapping submod_name to returns_tuple
        """
            ...
            if has_serialize:
                ...
                if bytes_dict:
                    for shape_str, bytes_data in bytes_dict.items():
                        inductor_cache.insert(submod_name, shape_str, bytes_data)
                        logger.info(
                ...
                logger.warning(
                    ...
                    "(missing 'serialize' method). bundled_autograd_cache "
        ...
        logger.info(
            ...
            inductor_cache.size_bytes(),
        )

        logger.info(
            "Inductor cache keys: %s",
            list(inductor_cache.submodule_bytes.keys()),
        )

        return inductor_cache, sym_shape_indices_map, returns_tuple_map
    ...
    def __call__(self, graph: fx.GraphModule, example_inputs):
        ...
        (
            ...
            sym_shape_indices_map,
            returns_tuple_map,
        ) = self._collect_inductor_cache(submod_names)
