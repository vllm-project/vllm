...
class VllmCompiledFunction(SerializableCallable):
    ...
    class InductorCache:
        ...
        def load_all(self) -> None:
            ...
            def _load_entry(entry) -> torch._inductor.CompiledArtifact:
                try:
                    loaded = None
                    while loaded is None:
                        loaded = torch._inductor.CompiledArtifact.from_bytes(
                            entry)
                    return loaded
                except Exception as e:
                    logger.debug("retrying after fx graph cache miss: %s", e)
