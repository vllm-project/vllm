...
class VllmSerializableFunction(SerializableCallable):
    ...
    def deserialize_compile_artifacts(cls, data: bytes) -> "VllmSerializableFunction":
        ...
        if envs.VLLM_USE_BACKEND_WITH_INDUCTOR_CACHE:
            from vllm.compilation.backends import VllmBackendWithCache

            # Check if inductor cache has actual artifacts
            inductor_cache = state.get("inductor_cache")
            submod_names = state.get("submod_names")

            has_artifacts = (
                inductor_cache is not None
                and hasattr(inductor_cache, "num_artifacts")
                and inductor_cache.num_artifacts() > 0
            )

            num_artifacts = inductor_cache.num_artifacts() if inductor_cache else 0
            ...
            if not has_artifacts or not submod_names:
                ...
                logger.warning(
                    "VLLM_USE_BACKEND_WITH_INDUCTOR_CACHE is set but inductor cache "
                    "is empty (artifacts=%d) or submod_names is missing (len=%d). "
                    "Falling back to standard compilation path to populate cache.",
                    num_artifacts,
                    num_submods,
                )
                ...
                vllm_backend = VllmBackendWithCache(
                ...
                populated_split_gm = vllm_backend.create_split_gm_from_cache(split_gm)
