...
class VllmSerializableFunction(SerializableCallable):
    """
    PyTorch's precompile with custom backend requires vllm to implement this
    interface. Right now serialization for the custom backend is done via
    ...
    def __init__(self, graph_module, example_inputs, prefix, optimized_call):
        ...
        self.shape_env = None
        sym_input = next(
            (i for i in self.example_inputs if isinstance(i, torch.SymInt)),
            None)
    ...
    def serialize_compile_artifacts(
            cls, compiled_fn: "VllmSerializableFunction") -> bytes:
        import sympy
    ...
    @classmethod
    def deserialize_compile_artifacts(
            cls, data: bytes) -> "VllmSerializableFunction":
