...
class HelionAllGatherCutlassScaledMMPattern(BasePattern):                                                   
    def get_inputs(self) -> list[torch.Tensor]:
        x = torch.empty([8, 16], device=self.device, dtype=FP8_DTYPE)
        weight = (
            torch.empty([16, 16], device=self.device, dtype=FP8_DTYPE)
...
class AsyncTPPass(VllmPatternMatcherPass):
    ...
    def __init__(self, config: VllmConfig) -> None:
        ...
        if self.model_dtype == torch.bfloat16:
            ...
            if envs.VLLM_USE_HELION_BACKEND:
                HelionAllGatherScaledMMPattern(self.model_dtype, self.device).register(self.patterns)
                HelionAllGatherCutlassScaledMMPattern(self.model_dtype, self.device).register(self.patterns)   
            else:
