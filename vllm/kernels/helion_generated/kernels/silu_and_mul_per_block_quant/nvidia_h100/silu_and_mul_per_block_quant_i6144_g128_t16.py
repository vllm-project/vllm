...
def _triton_silu_and_mul_per_block_quant(input_1, scale_ub, scales, out, input_1_stride_0, input_1_stride_1, input_1_stride_2, out_stride_0, out_stride_1, out_stride_2, scales_stride_0, scales_stride_1):
    ...
    num_blocks_1 = tl.cdiv(48, _BLOCK_SIZE_1)
    num_pid_m = tl.cdiv(128, _BLOCK_SIZE_2)
    num_pid_n = tl.cdiv(48, _BLOCK_SIZE_1)
    ...
    load = tl.load(input_1 + (indices_0[:, None, None] * input_1_stride_0 + indices_1[None, :, None] * input_1_stride_1 + indices_2[None, None, :] * input_1_stride_2), None, eviction_policy='evict_last')
    v_0 = tl.cast(load, tl.float32)
    v_1 = tl.full([], 48, tl.int32)
    v_2 = indices_1 + v_1
    v_3 = tl.full([], 48, tl.int32)
    v_4 = indices_1 + v_3
    v_5 = tl.full([], 96, tl.int32)
    v_6 = v_4 < v_5
    subscript = v_6[None, :, None]
    load_1 = tl.load(input_1 + (indices_0[:, None, None] * input_1_stride_0 + (indices_1 + 48)[None, :, None] * input_1_stride_1 + indices_2[None, None, :] * input_1_stride_2), subscript, other=0, eviction_policy='evict_first')
    ...
    scale_ub_s = tl.load(scale_ub + tl.zeros([], tl.int32), None, eviction_policy='evict_first')
...
def call(out: torch.Tensor, input: torch.Tensor, scales: torch.Tensor, group_size: int, scale_ub: torch.Tensor | None=None, is_scale_transposed: bool=False, *, _launcher=_default_launcher):
    ...
    _BLOCK_SIZE_2 = 128
    _BLOCK_SIZE_1 = 4
    _launcher(_triton_silu_and_mul_per_block_quant, ((128 + _BLOCK_SIZE_2 - 1) // _BLOCK_SIZE_2 * ((48 + _BLOCK_SIZE_1 - 1) // _BLOCK_SIZE_1) * num_tokens,), input, scale_ub, scales, out, input.stride(0), input.stride(1), input.stride(2), out.stride(0), out.stride(1), out.stride(2), scales.stride(0), scales.stride(1), num_warps=4, num_stages=8)
