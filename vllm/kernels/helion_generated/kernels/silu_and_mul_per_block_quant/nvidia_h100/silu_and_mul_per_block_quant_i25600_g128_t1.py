...
@triton.jit
def _triton_silu_and_mul_per_block_quant(input_1, scale_ub, scales, out, input_1_stride_1, input_1_stride_2, out_stride_1, out_stride_2, scales_stride_0, scales_stride_1):
    ...
    load = tl.broadcast_to(tl.load(input_1 + (indices_1[None, :, None] * input_1_stride_1 + indices_2[None, None, :] * input_1_stride_2), None), [_BLOCK_SIZE_0, _BLOCK_SIZE_1, _BLOCK_SIZE_2])
    ...
    load_1 = tl.broadcast_to(tl.load(input_1 + ((indices_1 + 200)[None, :, None] * input_1_stride_1 + indices_2[None, None, :] * input_1_stride_2), subscript, other=0, eviction_policy='evict_last'), [_BLOCK_SIZE_0, _BLOCK_SIZE_1, _BLOCK_SIZE_2])
    ...
    tl.store(out + tl.broadcast_to(indices_1[None, :, None] * out_stride_1 + indices_2[None, None, :] * out_stride_2, [_BLOCK_SIZE_0, _BLOCK_SIZE_1, _BLOCK_SIZE_2]), v_23, None)
...
def call(out: torch.Tensor, input: torch.Tensor, scales: torch.Tensor, group_size: int, scale_ub: torch.Tensor | None=None, is_scale_transposed: bool=False, *, _launcher=_default_launcher):
    ...
    _BLOCK_SIZE_2 = 128
    _BLOCK_SIZE_1 = 4
    _launcher(_triton_silu_and_mul_per_block_quant, ((128 + _BLOCK_SIZE_2 - 1) // _BLOCK_SIZE_2 * ((200 + _BLOCK_SIZE_1 - 1) // _BLOCK_SIZE_1) * 1,), input, scale_ub, scales, out, input.stride(1), input.stride(2), out.stride(1), out.stride(2), scales.stride(0), scales.stride(1), num_warps=4, num_stages=1)
