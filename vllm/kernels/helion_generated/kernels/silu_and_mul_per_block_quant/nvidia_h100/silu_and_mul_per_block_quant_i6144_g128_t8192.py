...
def call(out: torch.Tensor, input: torch.Tensor, scales: torch.Tensor, group_size: int, scale_ub: torch.Tensor | None=None, is_scale_transposed: bool=False, *, _launcher=_default_launcher):
    ...
    _BLOCK_SIZE_2 = 128
    _BLOCK_SIZE_1 = 16
    _launcher(_triton_silu_and_mul_per_block_quant, ((128 + _BLOCK_SIZE_2 - 1) // _BLOCK_SIZE_2 * ((48 + _BLOCK_SIZE_1 - 1) // _BLOCK_SIZE_1) * num_tokens,), input, scales, out, input.stride(0), input.stride(1), input.stride(2), out.stride(0), out.stride(1), out.stride(2), scales.stride(0), scales.stride(1), num_warps=8, num_stages=1)
