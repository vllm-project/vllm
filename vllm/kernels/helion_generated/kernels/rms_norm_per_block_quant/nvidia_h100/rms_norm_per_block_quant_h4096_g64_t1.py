...
def _triton_rms_norm_per_block_quant(input_1, residual, weight, scale_ub, scale, result, input_1_stride_0, input_1_stride_1, residual_stride_0, residual_stride_1, result_stride_0, result_stride_1, scale_stride_0, scale_stride_1, weight_stride_0, num_tokens, epsilon, _NUM_SM: tl.constexpr):
    ...
    for virtual_pid in tl.range(tl.program_id(0), total_pids, _NUM_SM * 4):
        ...
        for offset_1 in tl.range(0, 4096, _BLOCK_SIZE_1, disallow_acc_multi_buffer=True):
        ...
        tl.debug_barrier()
        for offset_2 in tl.range(0, 64, _BLOCK_SIZE_2, disallow_acc_multi_buffer=False, flatten=True):
            ...
            for offset_3 in tl.range(0, 64, _BLOCK_SIZE_3):
                ...
                n_blk = v_13[None, :, :]
                v_14 = tl.full([], 64, tl.int32)
                v_15 = indices_2 < v_14
                mask = v_15[None, :, None]
                load_2 = tl.load(input_1 + (m_blk_copy_0 * input_1_stride_0 + n_blk * input_1_stride_1), mask, other=0, eviction_policy='evict_last')
...
def call(result: torch.Tensor, input: torch.Tensor, weight: torch.Tensor, scale: torch.Tensor, epsilon: float, scale_ub: torch.Tensor | None, residual: torch.Tensor | None, group_size: int, is_scale_transposed: bool, *, _launcher=_default_launcher):
    ...
    _launcher(_triton_rms_norm_per_block_quant, (_NUM_SM * 4,), input, residual, weight, scale_ub, scale, result, input.stride(0), input.stride(1), residual.stride(0), residual.stride(1), result.stride(0), result.stride(1), scale.stride(0), scale.stride(1), weight.stride(0), num_tokens, epsilon, _NUM_SM, num_warps=16, num_stages=6, maxnreg=128)
