...
def _triton_rms_norm_per_block_quant(input_1, weight, scale, result, input_1_stride_0, input_1_stride_1, result_stride_0, result_stride_1, scale_stride_0, scale_stride_1, weight_stride_0, epsilon):
    ...
    for offset_1 in tl.range(0, 8192, _BLOCK_SIZE_1, disallow_acc_multi_buffer=True, flatten=True):
    ...
    v_3 = tl.full([], 0.0001220703125, tl.float32)
    v_4 = rms * v_3
    v_5 = v_4 + epsilon
    v_6 = tl.rsqrt(v_5)
    ...
    m_blk = v_8[:, None, None]
    tl.debug_barrier()
    for offset_2 in tl.range(0, 128, _BLOCK_SIZE_2, flatten=True):
...
def call(result: torch.Tensor, input: torch.Tensor, weight: torch.Tensor, scale: torch.Tensor, epsilon: float, scale_ub: torch.Tensor | None, residual: torch.Tensor | None, group_size: int, is_scale_transposed: bool, *, _launcher=_default_launcher):
    ...
    _launcher(_triton_rms_norm_per_block_quant, (num_tokens,), input, weight, scale, result, input.stride(0), input.stride(1), result.stride(0), result.stride(1), scale.stride(0), scale.stride(1), weight.stride(0), epsilon, num_warps=32, num_stages=2)
