...
def _triton_rms_norm_per_block_quant(input_1, weight, scale, result, input_1_stride_0, input_1_stride_1, result_stride_0, result_stride_1, scale_stride_0, scale_stride_1, weight_stride_0, epsilon):
    ...
    for offset_1 in tl.range(0, 5120, _BLOCK_SIZE_1, disallow_acc_multi_buffer=False):
    ...
    m_blk = v_8[:, None, None]
    tl.debug_barrier()
    for offset_2 in tl.range(0, 40, _BLOCK_SIZE_2, disallow_acc_multi_buffer=True):
        ...
        for offset_3 in tl.range(0, 128, _BLOCK_SIZE_3):
            ...
            w_blk = tl.load(weight + n_blk * weight_stride_0, mask_2[None, :, None] & mask, other=0, eviction_policy='evict_first')
            subscript_4 = v_6_copy_0[:, None, None]
            v_15 = v_14 * subscript_4
            v_16 = tl.cast(v_15, tl.bfloat16)
            v_17 = v_16 * w_blk
            v_18 = tl.abs(v_17)
            _mask_to = tl.where(tl.broadcast_to(mask_2[None, :, None], [_BLOCK_SIZE_0, _BLOCK_SIZE_2, _BLOCK_SIZE_3]), v_18, tl.full([], float('-inf'), tl.bfloat16))
            ...
            tl.store(scale + (indices_0[:, None] * scale_stride_0 + indices_2[None, :] * scale_stride_1), v_23, mask_2[None, :])
...
def call(result: torch.Tensor, input: torch.Tensor, weight: torch.Tensor, scale: torch.Tensor, epsilon: float, scale_ub: torch.Tensor | None, residual: torch.Tensor | None, group_size: int, is_scale_transposed: bool, *, _launcher=_default_launcher):
    ...
    _launcher(_triton_rms_norm_per_block_quant, (num_tokens,), input, weight, scale, result, input.stride(0), input.stride(1), result.stride(0), result.stride(1), scale.stride(0), scale.stride(1), weight.stride(0), epsilon, num_warps=4, num_stages=7)
