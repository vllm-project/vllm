...
@triton.jit
def _triton_fused_qk_norm_rope(qkv, q_weight, k_weight, position_ids, cos_sin_cache, cos_sin_cache_stride_0, cos_sin_cache_stride_1, k_weight_stride_0, position_ids_stride_0, q_weight_stride_0, qkv_stride_0, qkv_stride_1, qkv_stride_2, num_tokens, eps, is_neox, _NUM_SM: tl.constexpr, _BLOCK_SIZE_2: tl.constexpr, head_dim: tl.constexpr, qk_heads: tl.constexpr, num_heads_q: tl.constexpr, embed_dim: tl.constexpr):
    total_pids = tl.cdiv(head_dim, _BLOCK_SIZE_2) * tl.cdiv(qk_heads, _BLOCK_SIZE_1) * num_tokens
    block_size = tl.cdiv(total_pids, _NUM_SM * 64)
    start_pid = tl.program_id(0) * block_size
    end_pid = start_pid + block_size
    if end_pid > total_pids:
        end_pid = total_pids
    for virtual_pid in tl.range(start_pid, end_pid, loop_unroll_factor=1, flatten=True):
