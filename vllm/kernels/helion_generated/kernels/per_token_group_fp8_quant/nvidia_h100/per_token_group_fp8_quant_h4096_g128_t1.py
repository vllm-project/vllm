...
@triton.jit
def _triton_per_token_group_fp8_quant(input_1, output_s, output_q, input_1_stride_1, input_1_stride_2, output_q_stride_1, output_q_stride_2, output_s_stride_0, output_s_stride_1, eps, fp8_max, scale_ue8m0, fp8_min, _BLOCK_SIZE_2: tl.constexpr, group_size: tl.constexpr, groups_per_row: tl.constexpr):
    num_blocks_0 = groups_per_row
    num_blocks_1 = 1
    num_pid_m = groups_per_row
    num_pid_n = 1
    ...
    x_blk = tl.broadcast_to(tl.load(input_1 + (indices_1[None, :, None] * input_1_stride_1 + indices_2[None, None, :] * input_1_stride_2), mask_2[None, None, :], other=0, eviction_policy='evict_first'), [_BLOCK_SIZE_0, _BLOCK_SIZE_1, _BLOCK_SIZE_2])
    v_0 = tl.abs(x_blk)
    _mask_to = tl.where(tl.broadcast_to(mask_2[None, None, :], [_BLOCK_SIZE_0, _BLOCK_SIZE_1, _BLOCK_SIZE_2]), v_0, tl.full([], float('-inf'), tl.bfloat16))
    ...
    tl.store(output_q + tl.broadcast_to(indices_1[None, :, None] * output_q_stride_1 + indices_2[None, None, :] * output_q_stride_2, [_BLOCK_SIZE_0, _BLOCK_SIZE_1, _BLOCK_SIZE_2]), v_14, mask_2[None, None, :])
...
def call(input: torch.Tensor, output_q: torch.Tensor, output_s: torch.Tensor, group_size: int, eps: float, fp8_min: float, fp8_max: float, scale_ue8m0: bool, dummy_is_scale_transposed: bool=False, dummy_is_tma_aligned: bool=False, *, _launcher=_default_launcher):
    ...
    _launcher(_triton_per_token_group_fp8_quant, (groups_per_row * 1 * ((group_size + _BLOCK_SIZE_2 - 1) // _BLOCK_SIZE_2),), input, output_s, output_q, input.stride(1), input.stride(2), output_q.stride(1), output_q.stride(2), output_s.stride(0), output_s.stride(1), eps, fp8_max, scale_ue8m0, fp8_min, _BLOCK_SIZE_2, group_size, groups_per_row, num_warps=1, num_stages=4)
