...
TokenizerRegistry = _TokenizerRegistry(
    {
        mode: (f"vllm.frontend.processing.tokenizers.{mod_relname}", cls_name)
...
def get_tokenizer(
    ...
    **kwargs,
    ...
    # Ensure that, if the config were to come from
    # vllm.foundation.integrations.transformers_utils.config, it is
    ...
    if model_type in _MODEL_TYPES_WITH_INCORRECT_TOKENIZER_CLASS:
        from vllm.frontend.processing.tokenizers.hf import get_cached_tokenizer
