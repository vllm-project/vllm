...
non-streaming), see ``vllm.frontend.processing.tool_parsers.gemma4_tool_parser``.
For thinking/reasoning output parsing, see
``vllm.frontend.processing.reasoning.gemma4_utils``.
...
def _parse_tool_arguments(args_str: str) -> dict[str, str]:
    ...
    ``vllm.frontend.processing.parser.gemma4``, which handles internal quotes, nested
    ...
    from vllm.frontend.processing.parser.gemma4 import _parse_gemma4_args
