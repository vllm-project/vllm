...
def register_vllm_serve_api_routers(app: FastAPI):
    ...
    from vllm.frontend.entrypoints.serve.lora.api_router import (
        attach_router as attach_lora_router,
    )

    attach_lora_router(app)

    from vllm.frontend.entrypoints.serve.profile.api_router import (
        attach_router as attach_profile_router,
    )

    attach_profile_router(app)

    from vllm.frontend.entrypoints.serve.tokenize.api_router import (
