...
class RankingComputer:
    def __init__(self, classi_idx):
        ...
        ranking_params = filter_missing_classis(MODEL_SERVER_RANKING, classi_idx,
                                                warn=True)
        ...
        if not self.classi_indices:
            logger.warning(
                f"No ranking classifiers {pnames} found. Candidates will not be ranked.")
    ...
    def compute(self, logit_GC, debug_infos_G: list[BeamDebugInfo] = None):
        ...
        ranking_GC = torch.where(self.negation_R, 1.0 - probs_GC, probs_GC).pow(
            self.pow_R)
