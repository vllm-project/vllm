...
"""
Worker Controller's AsyncLLM - Uses in-process EngineCore to avoid subprocess overhead.

This is a modified version of vllm.v1.engine.async_llm.AsyncLLM that runs the
...
class InprocAsyncEngineCore:
    """
    Async wrapper around in-process EngineCore.

    This avoids the overhead of spawning an EngineCore subprocess by running
    the EngineCore in-process. Blocking operations are run in a thread pool
    ...
    def __init__(
        ...
        self.resources = InprocResources()
    ...
    def get_output(self) -> EngineCoreOutputs:
        """Get output synchronously (called from output handler thread)."""
    ...
    async def get_output_async(self) -> EngineCoreOutputs:
        ...
        return await self._run_in_executor(self.get_output)
    ...
    async def profile_async(self, is_start: bool = True) -> None:
        await self._run_in_executor(self.engine_core.profile, is_start)
    ...
    async def sleep_async(self, level: int = 1) -> None:
        await self._run_in_executor(self.engine_core.sleep, level)

    async def wake_up_async(self, tags: list[str] | None = None) -> None:
        await self._run_in_executor(self.engine_core.wake_up, tags)

    async def is_sleeping_async(self) -> bool:
        return self.engine_core.is_sleeping()

    async def add_lora_async(self, lora_request: LoRARequest) -> bool:
        return await self._run_in_executor(self.engine_core.add_lora, lora_request)

    async def remove_lora_async(self, lora_id: int) -> bool:
        return await self._run_in_executor(self.engine_core.remove_lora, lora_id)
    ...
    async def collective_rpc_async(
        ...
        return await self._run_in_executor(
            self.engine_core.collective_rpc, method, timeout, args, kwargs
        )
...
class AsyncLLM(EngineClient):
    def __init__(
        ...
        # EngineCore (runs in-process to avoid subprocess overhead).
        # This is the key optimization - we avoid spawning a subprocess
        # which saves ~8-10s of startup time.
        self.engine_core = InprocAsyncEngineCore(
            vllm_config=vllm_config,
            executor_class=executor_class,
