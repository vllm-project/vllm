...
logger = init_logger("vllm.logits_processor")
...
def _apply_logits_processors(
    ...
    for seq_group in sampling_metadata.seq_groups:
        ...
        if (
            logits_processors or guidance_scale is not None
            ...
            for seq_id, logits_row_idx in zip(seq_ids, seq_group.sample_indices):
                ...
                if guidance_scale is not None:
                    ...
                    conditional_logits_row = logits[2 * logits_row_idx].to(
                        torch.float32
                    )
                    unconditional_logits_row = logits[2 * logits_row_idx + 1].to(
                        torch.float32
                    )

                    # try doing full sampling here, and masking output logits in both sequences to make it deterministic
                    cfg_logits = conditional_logits_row + guidance_scale * (
                        conditional_logits_row - unconditional_logits_row
                    )

                    conditional_logits_row = (
                        conditional_logits_row / sampling_params.temperature
                    )
                    _, topk_idx = torch.topk(cfg_logits, sampling_params.top_k)
                    masked_logits = torch.full_like(
                        conditional_logits_row, -float("inf")
                    )
                    masked_logits.scatter_(
                        -1, topk_idx, conditional_logits_row.gather(-1, topk_idx)
                    )

                    probs = torch.softmax(masked_logits, dim=-1)
                    selected_token_idx = torch.multinomial(probs, num_samples=1).item()

                    # hacky -- sample logits deterministically and let sampling code run over it ;p
                    deterministic_logits = torch.full_like(
                        conditional_logits_row, -float("inf")
                    )
                    deterministic_logits[
                        selected_token_idx
                    ] = 0.0  # Make only our chosen token have non-negative logit
                    logits[2 * logits_row_idx] = deterministic_logits
                    logits[2 * logits_row_idx + 1] = deterministic_logits.clone()

                    logits_row = deterministic_logits
                    logits_row_idx *= 2
                # --- FLAGSCALE MODIFICATION END ---
                else:
    ...
    if found_logits_processors:
        ...
        if has_negative:
            assert (
                logits_processed == logits.shape[0] // 2
            )  # --- FLAGSCALE MODIFICATION ---
        else:
