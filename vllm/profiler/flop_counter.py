...
class FlopCount:
    ...
    def total(self) -> int:
        ...
        return sum(getattr(self, field.name) for field in self.__dataclass_fields__.values())
...
class DetailedFlopCount:
    ...
    def add_operation(self, op_name: str, flops: int, layer_name: Optional[str] = None):
        ...
        """
        self.operation_counts[op_name] = self.operation_counts.get(op_name, 0) + flops
        ...
        if layer_name:
            ...
            if flop_field:
                current_value = getattr(self.layer_counts[layer_name], flop_field)
                setattr(self.layer_counts[layer_name], flop_field, current_value + flops)
...
class FlopCounter(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        if self.enabled:
            ...
            if flops > 0:
                op_name = str(func)
                layer_name = ".".join(self._module_stack) if self._module_stack else None
    ...
    def _compute_flops(self, func, args: Tuple, kwargs: Dict[str, Any]) -> int:
        ...
        elif func_packet in [torch.ops.aten.softmax, torch.ops.aten.log_softmax]:
            return self._softmax_flops(args)
        elif func_packet in [torch.ops.aten.gelu, torch.ops.aten.silu, torch.ops.aten.relu]:
    ...
    def _mm_flops(self, args: Tuple) -> int:
        ...
        if len(args) >= 2:
            a, b = args[0], args[1]
            if hasattr(a, 'shape') and hasattr(b, 'shape') and len(a.shape) >= 2 and len(b.shape) >= 2:
    ...
    def _addmm_flops(self, args: Tuple) -> int:
        ...
        if len(args) >= 3:
            ...
            if hasattr(result_tensor, 'shape') and len(result_tensor.shape) >= 2:
        ...
        if len(args) >= 3:
            ...
            if all(hasattr(t, 'shape') for t in [query, key, value]):
                ...
                if len(query.shape) == 4:
                    ...
                    num_heads = query.shape[1] if query.shape[1] < query.shape[2] else query.shape[2]
                else:
    ...
    def _conv2d_flops(self, args: Tuple) -> int:
        ...
        if len(args) >= 2:
            ...
            if hasattr(input_tensor, 'shape') and hasattr(weight, 'shape'):
                ...
                output_h, output_w = input_tensor.shape[2], input_tensor.shape[3]
                
                return batch_size * out_channels * output_h * output_w * in_channels * kernel_h * kernel_w
        return 0
