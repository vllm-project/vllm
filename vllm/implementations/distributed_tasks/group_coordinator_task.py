...
class GroupCoordinatorDistributedTask(DistributedTask):
    def run(self, *, coordinator_type: CoordinatorType, communicator_type: CommunicatorType, groups: List[List[int]], **kwargs):
        ...
        group = [g for g in groups if self.global_coordinator.get_rank() in g][0]
        self.group_coordinator : Coordinator = coordinator_cls(group)
        self.group_coordinator.initialize()

        self.communicator : Communicator = communicator_cls(self.group_coordinator)
    ...
    def post_init_distributed(self, **kwargs):
        ...
        They can use `self.global_coordinator` for global communication over the whole process group.
        They can use `self.group_coordinator` for communication within a subgroup.
        They can use `self.communicator` for communication between devices in a subgroup.
        """
        return
