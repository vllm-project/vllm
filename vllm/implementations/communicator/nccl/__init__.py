...
def get_communicator_class(communicator_type: CommunicatorType) -> type:
    ...
    if communicator_type == CommunicatorType.PYNCCL:
        from vllm.implementations.communicator.nccl.pynccl.pynccl_communicator import NCCLCommunicator
        return NCCLCommunicator
    else:
        raise ValueError(f"Communicator type {communicator_type} not regonized.")
