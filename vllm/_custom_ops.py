...
def paged_attention_v1(
    ...
    torch.ops._C.paged_attention_v1(out, query, key_cache, value_cache, num_kv_heads,
...
def paged_attention_v2(
    ...
    torch.ops._C.paged_attention_v2(
        out, exp_sum, max_logits, tmp_out, query, key_cache, value_cache, num_kv_heads,
...
def mla_decode_kvcache_cpu(
    ...
    torch.ops._C_cpu.mla_decode_kvcache(out, query, kv_cache, scale, block_tables,
                                        seq_lens)
...
def merge_attn_states(output: torch.Tensor,
    ...
    torch.ops._C.merge_attn_states(output, output_lse, prefix_output, prefix_lse,
...
def batched_rotary_embedding(positions: torch.Tensor, query: torch.Tensor,
                             ...
                             cos_sin_cache: torch.Tensor, is_neox: bool, rot_dim: int,
                             cos_sin_cache_offsets: torch.Tensor) -> None:
    # TODO: Remove this contiguous call when the kernel is updated to support tensor slices
    query_contiguous = query.contiguous()
    key_contiguous = key.contiguous()
    torch.ops._C.batched_rotary_embedding(positions, query_contiguous, key_contiguous,
                                          head_size, cos_sin_cache, is_neox, rot_dim,
...
def advance_step_flashattn(num_seqs: int, num_queries: int, block_size: int,
    ...
    return torch.ops._C.advance_step_flashattn(num_seqs, num_queries, block_size,
...
def advance_step_flashinfer(num_seqs: int, num_queries: int, block_size: int,
    ...
    return torch.ops._C.advance_step_flashinfer(num_seqs, num_queries, block_size,
                                                input_tokens, sampled_token_ids,
                                                input_positions, seq_lens, slot_mapping,
                                                block_tables, paged_kv_indices,
...
# quantization ops
# awq
def awq_dequantize(qweight: torch.Tensor, scales: torch.Tensor, zeros: torch.Tensor,
                   split_k_iters: int, thx: int, thy: int) -> torch.Tensor:
...
# gptq
def gptq_gemm(a: torch.Tensor, b_q_weight: torch.Tensor, b_gptq_qzeros: torch.Tensor,
              b_gptq_scales: torch.Tensor, b_g_idx: torch.Tensor, use_exllama: bool,
              bit: int) -> torch.Tensor:
    return torch.ops._C.gptq_gemm(a, b_q_weight, b_gptq_qzeros, b_gptq_scales, b_g_idx,
                                  use_exllama, bit)
...
# marlin
def marlin_gemm(a: torch.Tensor, b_q_weight: torch.Tensor, b_scales: torch.Tensor,
                workspace: torch.Tensor, size_m: int, size_n: int,
                size_k: int) -> torch.Tensor:
    return torch.ops._C.marlin_gemm(a, b_q_weight, b_scales, workspace, size_m, size_n,
                                    size_k)


# marlin_24
def gptq_marlin_24_gemm(a: torch.Tensor, b_q_weight: torch.Tensor, b_meta: torch.Tensor,
                        b_scales: torch.Tensor, workspace: torch.Tensor,
                        b_q_type: ScalarType, size_m: int, size_n: int,
                        size_k: int) -> torch.Tensor:
    return torch.ops._C.gptq_marlin_24_gemm(a, b_q_weight, b_meta, b_scales, workspace,
                                            b_q_type.id, size_m, size_n, size_k)
...
if hasattr(torch.ops._C, "gptq_marlin_24_gemm"):
    ...
    def _awq_gemm_fake(input: torch.Tensor, qweight: torch.Tensor, qzeros: torch.Tensor,
                       scales: torch.Tensor,
    ...
    def _aqlm_dequant_fake(codes: torch.Tensor, codebooks: torch.Tensor,
...
if hasattr(torch.ops._C, "allspark_w8a16_gemm"):
    ...
    def _allspark_w8a16_gemm_fake(a: torch.Tensor, b_qweight: torch.Tensor,
                                  b_scales: torch.Tensor,
                                  b_qzeros: Optional[torch.Tensor], n: torch.SymInt,
                                  group_size: torch.SymInt, sm_count: torch.SymInt,
                                  sm_version: torch.SymInt,
                                  CUBLAS_M_THRESHOLD: torch.SymInt, has_zp: bool,
...
if hasattr(torch.ops._C, "ggml_dequantize"):
    ...
    def _ggml_dequantize_fake(W: torch.Tensor,
                              quant_type: int,
                              m: torch.SymInt,
                              n: torch.SymInt,
...
def cutlass_scaled_fp4_mm(a: torch.Tensor, b: torch.Tensor, block_scale_a: torch.Tensor,
...
def get_cutlass_moe_mm_data(topk_ids: torch.Tensor, expert_offsets: torch.Tensor,
                            problem_sizes1: torch.Tensor, problem_sizes2: torch.Tensor,
                            input_permutation: torch.Tensor,
                            output_permutation: torch.Tensor, num_experts: int, n: int,
                            k: int):
    """
    ...
    """
    torch.ops._C.get_cutlass_moe_mm_data(topk_ids, expert_offsets, problem_sizes1,
...
def cutlass_moe_mm(out_tensors: torch.Tensor, a_tensors: torch.Tensor,
    ...
    """
    torch.ops._C.cutlass_moe_mm(out_tensors, a_tensors, b_tensors, a_scales, b_scales,
                                expert_offsets, problem_sizes, a_strides, b_strides,
                                c_strides)


# aqlm
def aqlm_gemm(input: torch.Tensor, codes: torch.Tensor, codebooks: torch.Tensor,
              scales: torch.Tensor, codebook_partition_sizes: list[int],
              bias: Optional[torch.Tensor]) -> torch.Tensor:
...
# gptq_marlin
def gptq_marlin_repack(b_q_weight: torch.Tensor, perm: torch.Tensor, size_k: int,
...
def gptq_marlin_moe_repack(b_q_weight: torch.Tensor, perm: torch.Tensor, size_k: int,
                           size_n: int, num_bits: int) -> torch.Tensor:
    ...
    for e in range(num_experts):
        output[e] = torch.ops._C.gptq_marlin_repack(b_q_weight[e], perm[e], size_k,
                                                    size_n, num_bits)
    return output


def awq_marlin_moe_repack(b_q_weight: torch.Tensor, perm: torch.Tensor, size_k: int,
                          size_n: int, num_bits: int) -> torch.Tensor:
    num_experts = b_q_weight.shape[0]
    assert size_k % 16 == 0
    output = torch.empty((num_experts, size_k // 16, size_n * (num_bits // 2)),
                         ...
                         dtype=b_q_weight.dtype)
    for e in range(num_experts):
        output[e] = torch.ops._C.awq_marlin_repack(b_q_weight[e], size_k, size_n,
                                                   num_bits)
    return output
...
def gptq_marlin_gemm(a: torch.Tensor,
    ...
    return torch.ops._C.gptq_marlin_gemm(a, b_q_weight, b_scales, b_zeros, g_idx, perm,
                                         workspace, b_q_type.id, size_m, size_n, size_k,
                                         is_k_full, has_zp, use_atomic_add,
                                         use_fp32_reduce, is_zp_float)


# fp8 marlin
def fp8_marlin_gemm(a: torch.Tensor, b_q_weight: torch.Tensor, b_scales: torch.Tensor,
                    workspace: torch.Tensor, num_bits: int, size_m: int, size_n: int,
                    size_k: int) -> torch.Tensor:
    return torch.ops._C.fp8_marlin_gemm(a, b_q_weight, b_scales, workspace, num_bits,
                                        size_m, size_n, size_k)


# machete
def machete_supported_schedules(a_type: torch.dtype,
                                b_type: ScalarType,
    ...
    return torch.ops._C.machete_supported_schedules(a_type, b_type.id,
                                                    group_scales_type, group_zeros_type,
                                                    channel_scales_type,
                                                    token_scales_type, out_type)
...
def machete_mm(
    ...
    return torch.ops._C.machete_mm(a, b_q, b_type.id, out_type, b_group_scales,
                                   b_group_zeros, b_group_size, b_channel_scales,
                                   a_token_scales, schedule)


def machete_prepack_B(b_q_weight: torch.Tensor, a_type: torch.dtype, b_type: ScalarType,
                      group_scales_type: Optional[torch.dtype]) -> torch.Tensor:
...
def allspark_repack_weight(
        ...
        has_zp: bool = False) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    ...
    torch.ops._C.rearrange_kn_weight_as_n32k16_order(qweight, scale, zero_point, has_zp,
                                                     qweight_reorder, scale_reorder,
                                                     zero_point_reorder, K, N,
                                                     N_32align)
...
def allspark_w8a16_gemm(a: torch.Tensor, b_qweight: torch.Tensor,
    ...
    return torch.ops._C.allspark_w8a16_gemm(a, b_qweight, b_scales, b_qzeros, n,
                                            group_size, sm_count, sm_version,
                                            CUBLAS_M_THRESHOLD, has_zp, n32k16_reorder)
...
# qqq ops
def marlin_qqq_gemm(a: torch.Tensor, b_q_weight: torch.Tensor, s_tok: torch.Tensor,
                    s_ch: torch.Tensor, s_group: torch.Tensor, workspace: torch.Tensor,
                    size_m: int, size_n: int, size_k: int) -> torch.Tensor:
    return torch.ops._C.marlin_qqq_gemm(a, b_q_weight, s_tok, s_ch, s_group, workspace,
                                        size_m, size_n, size_k)
...
def ggml_moe_a8(
    ...
    return torch.ops._C.ggml_moe_a8(X, W, sorted_token_ids, expert_ids,
                                    num_tokens_post_padded, quant_type, row, top_k,
                                    tokens)
...
def causal_conv1d_fwd(x: torch.Tensor, weight: torch.Tensor,
                      ...
                      has_initial_state: Optional[torch.Tensor], silu_activation: bool,
                      pad_slot_id: int):
    torch.ops._C.causal_conv1d_fwd(x, weight, bias_, conv_states, query_start_loc,
...
def causal_conv1d_update(x: torch.Tensor, conv_state: torch.Tensor,
                         ...
                         conv_state_indices: Optional[torch.Tensor], pad_slot_id: int):
    torch.ops._C.causal_conv1d_update(x, conv_state, weight, bias_, silu_activation,
...
def selective_scan_fwd(u: torch.Tensor, delta: torch.Tensor, A: torch.Tensor,
    ...
    torch.ops._C.selective_scan_fwd(u, delta, A, B, C, D_, z_, delta_bias_,
                                    delta_softplus, query_start_loc, cache_indices,
...
def sgl_moe_align_block_size(topk_ids: torch.Tensor, num_experts: int, block_size: int,
                             sorted_token_ids: torch.Tensor, experts_ids: torch.Tensor,
                             num_tokens_post_pad: torch.Tensor) -> None:
    torch.ops._moe_C.sgl_moe_align_block_size(topk_ids, num_experts, block_size,
                                              sorted_token_ids, experts_ids,
                                              num_tokens_post_pad)


def moe_wna16_gemm(input: torch.Tensor, output: torch.Tensor, b_qweight: torch.Tensor,
                   b_scales: torch.Tensor, b_qzeros: Optional[torch.Tensor],
                   topk_weights: Optional[torch.Tensor], sorted_token_ids: torch.Tensor,
                   experts_ids: torch.Tensor, num_tokens_post_pad: torch.Tensor,
                   top_k: int, BLOCK_SIZE_M: int, BLOCK_SIZE_N: int, BLOCK_SIZE_K: int,
                   bit: int) -> torch.Tensor:
    if not current_platform.is_cuda():
        raise NotImplementedError("The optimized moe_wna16_gemm kernel is only "
    ...
    torch.ops._moe_C.moe_wna16_gemm(input, output, b_qweight, b_scales, b_qzeros,
                                    ...
                                    num_tokens_post_pad, top_k, BLOCK_SIZE_M,
...
def topk_softmax(topk_weights: torch.Tensor, topk_ids: torch.Tensor,
    ...
    torch.ops._moe_C.topk_softmax(topk_weights, topk_ids, token_expert_indicies,
                                  gating_output)
...
def moe_wna16_marlin_gemm(input: torch.Tensor, output: Optional[torch.Tensor],
                          ...
                          g_idx: Optional[torch.Tensor], perm: Optional[torch.Tensor],
                          workspace: torch.Tensor, sorted_token_ids: torch.Tensor,
                          expert_ids: torch.Tensor,
                          num_tokens_past_padded: torch.Tensor,
                          topk_weights: torch.Tensor, moe_block_size: int, top_k: int,
                          mul_topk_weights: bool, is_ep: bool, b_q_type: ScalarType,
    ...
    return torch.ops._moe_C.moe_wna16_marlin_gemm(
        ...
        moe_block_size, top_k, mul_topk_weights, is_ep, b_q_type.id, size_m, size_n,
...
if supports_moe_ops and hasattr(torch.ops._moe_C, "marlin_gemm_moe"):
    ...
    def marlin_gemm_moe_fake(a: torch.Tensor, b_q_weights: torch.Tensor,
                             sorted_ids: torch.Tensor, topk_weights: torch.Tensor,
    ...
    def moe_wna16_marlin_gemm_fake(input: torch.Tensor, output: Optional[torch.Tensor],
                                   b_qweight: torch.Tensor, b_scales: torch.Tensor,
                                   b_qzeros: Optional[torch.Tensor],
...
def concat_and_cache_mla(
    ...
    torch.ops._C_cache_ops.concat_and_cache_mla(kv_c, k_pe, kv_cache, slot_mapping,
...
def gather_cache(src_cache: torch.Tensor,
    ...
    torch.ops._C_cache_ops.gather_cache(src_cache, dst, block_table, cu_seq_lens,
...
# custom ar
def init_custom_ar(ipc_tensors: list[torch.Tensor], rank_data: torch.Tensor, rank: int,
                   fully_connected: bool) -> int:
...
def get_flash_mla_metadata(
    ...
    """
    return torch.ops._C.get_flash_mla_metadata(cache_seqlens, num_heads_per_head_k,
                                               num_heads_k)
...
def cutlass_mla_decode(out: torch.Tensor, q_nope: torch.Tensor, q_pe: torch.Tensor,
                       kv_c_and_k_pe_cache: torch.Tensor, seq_lens: torch.Tensor,
                       page_table: torch.Tensor, scale: float) -> torch.Tensor:
    torch.ops._C.cutlass_mla_decode(out, q_nope, q_pe, kv_c_and_k_pe_cache, seq_lens,
                                    page_table, scale)
    return out
