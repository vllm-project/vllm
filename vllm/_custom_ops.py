...
def gptq_marlin_24_gemm(
    ...
    return torch.ops._C.gptq_marlin_24_gemm(
        a, b_q_weight, b_meta, b_scales, workspace, b_q_type, size_m, size_n, size_k
    )
...
if hasattr(torch.ops._C, "gptq_marlin_24_gemm"):
    ...
    def _ggml_dequantize_fake(
        W: torch.Tensor, quant_type: int, m: int, n: int
    ) -> torch.Tensor:
...
def machete_gemm(
    ...
    return torch.ops._C.machete_gemm(
        a, b_q, b_type, b_scales, b_zeros, b_group_size, c, alpha, beta, schedule
    )
...
def causal_conv1d_fwd(
    ...
    return torch.ops._C.causal_conv1d_fwd(
        x,
        weight,
        bias_,
        conv_states,
...
def causal_conv1d_update(
    ...
    return torch.ops._C.causal_conv1d_update(
        x, conv_state, weight, bias_, silu_activation, cache_seqlens, conv_state_indices
    )
