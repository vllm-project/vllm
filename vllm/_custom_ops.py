...
def _raise_ptx_error(original_error: Exception) -> None:
    ...
    incompatibility (e.g. a pre-built wheel compiled with a newer CUDA
    toolkit than the GPU driver supports)."""
    import subprocess
    ...
    try:
        ...
        pass
    raise RuntimeError(
        "Marlin repack kernel failed with a CUDA error that usually indicates "
        "a PTX version mismatch: the pre-built vLLM wheel was compiled with a "
        "CUDA toolkit newer than what your GPU driver supports (driver "
        f"version: {driver_version}).\n\n"
        "To fix this, build vLLM from source with your system's CUDA toolkit:\n"
