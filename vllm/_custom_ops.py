...
if hasattr(torch.ops._C, "allspark_a16w8_gemm"):

    @register_fake("_C::allspark_a16w8_gemm")
    def _allspark_a16w8_gemm_fake(a: torch.Tensor, b_qweight: torch.Tensor,
                                  b_scales: torch.Tensor,
...
# gptq allspark
def gptq_allspark_rearrange_weight(
    ...
    """
    Rearrange qweight, weight_scale, and weight_zero_point(if asymmetric) to n32k16 format 
    for Ampere A16W8 Fused Gemm kernel
    ...
    Returns:
        Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]] : rearranged weight, scale, and optionally zero_point.
    """
    K = qweight.shape[0]
    N = qweight.shape[1]
    ...
    qweight_reorder = torch.empty((N_32align, K),
                                  ...
                                  dtype=qweight.dtype)
    scale_reorder = torch.empty((N_32align, ),
                                device=scale.device,
                                dtype=scale.dtype)
    zero_point_reorder = None
    if has_zp and zero_point is not None:
        zero_point_reorder = torch.empty((N_32align, ),
...
def allspark_a16w8_gemm(a: torch.Tensor, b_qweight: torch.Tensor,
                        b_scales: torch.Tensor,
    ...
    return torch.ops._C.allspark_a16w8_gemm(a, b_qweight, b_scales, b_qzeros,
