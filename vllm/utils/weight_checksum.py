...
def merge_finish_checksums(
    ...
    A finish that was not asked for digests reports None from every worker, so
    that case stays None rather than becoming an empty result that a caller
    could mistake for a successful empty check.

    Args:
        per_worker: One worker's rank-qualified digests, or None when the finish
