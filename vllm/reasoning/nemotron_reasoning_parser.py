...
class NemotronReasoningParser(ReasoningParser):
    ...
    Nvidia nemotron models currently use "<think>"
    and "</think>" to separate its thinking / response outputs.
    """

    def __init__(self, tokenizer: PreTrainedTokenizerBase):
        super().__init__(tokenizer)

        # Regex to match the reasoning content and response content when in
        # non-streaming mode.
    ...
    def extract_reasoning_content_streaming(
        ...
        """Extract the reasoning content / content emitted by nemotron models;
        If the sequence doesn't match what we expect, i.e., the model generates
        something else, all content is considered non-reasoning content.

        NOTE: nemotron models do not use a special token to start their 
        reasoning and response sections; instead they have token sequences, 
        e.g.,
        ...
        """
        reasoning_content, content = self._get_content_sections(current_text)
        ...
        # We've finished both the start of reasoning and start of response seq.
            ...
            delta_message = self._get_delta_message_with_both_bounds(
                delta_text, reasoning_content, content)
        if not delta_message.content and not delta_message.reasoning_content:
            return None
        return delta_message

    #### Implementation details of stream parsing for nemotron models
    ...
    def _is_response_start_substr(self, text: str) -> bool:
        ...
        return self.response_start.startswith(text)
    ...
    def _get_delta_message_with_no_response_bounds(
        ...
        ends_with_start_response_seq = current_text.endswith(
            self.response_start)
    ...
    def _get_delta_message_with_both_bounds(
        ...
        reasoning_end_idx = len(delta_text) - (len(response_content) +
                                               self.response_start_len)

        if reasoning_end_idx < 0:
            delta_reasoning_content = None
        else:
            reasoning_start_idx = -(len(reasoning_content) +
                                    self.response_start_len +
                                    len(response_content))
            delta_reasoning_content = delta_text[
                reasoning_start_idx:reasoning_end_idx]
    ...
    def _get_content_sections(
        ...
        Returns:
            tuple[Optional[str], Optional[str]]: Tuple of len 2
            containing the reasoning content and the non-reasoning content.
        """
        ...
        for current_chunk_end in delimiter_idxs:
            ...
            elif not parsed_content and current_chunk[
                    -self.response_start_len + 1:] == self.response_start[:-1]:
                # Mark end of reasoning and start response content
                # after the start of response sequence.
                end_reasoning_content = (current_chunk_end -
                                         self.response_start_len + 1)
