    ...
    * thinking disabled → ``content`` (NO marker at all — ``</think>`` already
    ...
    def __init__(self, tokenizer: "TokenizerLike", *args, **kwargs):
        ...
        if not self.model_tokenizer:
            raise ValueError("XingChen4ReasoningParser requires a valid tokenizer.")
    ...
    def count_reasoning_tokens(self, token_ids: Sequence[int]) -> int:
        if self.end_token_id in token_ids:
            end_idx = token_ids.index(self.end_token_id)
            if self.start_token_id not in token_ids[:end_idx]:
                return end_idx
