...
def get_string_based_reasoning_parser(
    reasoning_start: str,
    response_start: str,
    ...
    class StringBasedReasoningParser(ReasoningParser):
        """
        String based reasoning parser.

        Some models (e.g., Nemotron) use a specific string sequence to
        indicate the start of reasoning and response content (e.g. <think> and 
        </think>). This requires special handling when the string sequences are
        not tokenized as singular tokens, but rather as regular text.
        """
        ...
        def __init__(self, tokenizer: PreTrainedTokenizerBase):
            ...
            self.reasoning_regex = re.compile(
                fr"{self.reasoning_start}(.*?){self.response_start}(.*)",
                re.DOTALL)

            # Substrings to match for sequence boundaries on raw text
            self.seq_boundary_end = seq_boundary_end
            self.seq_boundary_start = seq_boundary_start

        def extract_reasoning_content(
            self, model_output: str, request: ChatCompletionRequest
        ) -> tuple[Optional[str], Optional[str]]:
            """Extract the reasoning content & content sections, respectively.
            If the sequence doesn't match what we expect, i.e., the model 
            generates something else, all content is considered non-reasoning 
            content.
        ...
        def extract_reasoning_content_streaming(
            ...
            """Extract the reasoning content / content emitted by models;
            If the sequence doesn't match what we expect, i.e., the model 
            generates something else, all content is considered non-reasoning 
            content.

            NOTE: some models do not use a special token to start their
            reasoning and response sections; instead they have token sequences,
            e.g.,

                    <think>Foo</think>Bar

            This increases the complexity of correctly handling streams, since
            we need to watch for specific sequences and correctly parse them 
            without dropping content that is potentially overlapping & spanning
            multiple delta messages.
            ...
            Returns:
                Union[DeltaMessage, None]
                    DeltaMessage with either reasoning content or content, or
                    None.
            """
            reasoning_content, content = self._get_content_sections(
                current_text)
            # Either we haven't finished the start of the reasoning sequence,
            # or the model is generating something unexpected.
            if not reasoning_content:
                delta_message = self._get_delta_message_with_no_reasoning_bounds(  # noqa: E501
            ...
            # We've finished both the start of reasoning and start of response
            # sequence.
            else:
                delta_message = self._get_delta_message_with_both_bounds(
                    delta_text, reasoning_content, content)
            if not delta_message.content and not delta_message.reasoning_content:  # noqa: E501
                return None
        ...
        def _get_delta_message_with_no_reasoning_bounds(
            ...
            """Parse the delta message when the current text has not yet
            completed its start of reasoning sequence.
        ...
        def _get_delta_message_with_no_response_bounds(
            ...
            content with no (response) content. NOTE that we may have 
            overlapping tokens with the start of reasoning / start of response
            sequences on either side of the delta text.
            ...
            Returns:
                DeltaMessage: Message containing the parsed content.
            """
            # If we have no reasoning content or explicitly end with the start
            # of response sequence, we are in transition to the response; need
            # to be careful here, since the final token (:) will match the
            # reasoning content and fully parse it out; we should not pass
            # the : back.
            ...
            # Consider previous / current text only within context of the
            # reasoning
            ...
            if not prev_was_substr:
                # Delta may be starting a new response seq but has other text
                # too.
                if delta_new_substr:
                    return DeltaMessage(
                        reasoning_content=delta_text[:delta_idx], content=None)
                # Normal case for most reasoning text (no potential special
                # seqs).
            ...
            # and also slice off the potential response sequence
                ...
                return DeltaMessage(reasoning_content=reasoning_content,
                                    content=None)
            # No new substring yet, and we broke our old one; take the whole
            # delta
            return DeltaMessage(
        ...
        def _get_content_sections(
                self,
                current_text: str) -> tuple[Optional[str], Optional[str]]:
    ...
    return StringBasedReasoningParser


# Register known string-based reasoning parsers

NemotronReasoningParser = get_string_based_reasoning_parser(
