...
class Xing4_0ReasoningParser(ReasoningParser):
    """Reasoning parser for Xing4_0 models.
    ...
    * thinking disabled → ``content`` (NO marker at all — ``</think>`` already
    ...
    def __init__(self, tokenizer: "TokenizerLike", *args, **kwargs):
        ...
        if not self.model_tokenizer:
            raise ValueError("Xing4_0ReasoningParser requires a valid tokenizer.")
        ...
        if start_id is None:
            raise RuntimeError(
                f"Xing4_0ReasoningParser: '{self.start_token}' not found "
                "in tokenizer vocabulary."
            )
        if end_id is None:
            raise RuntimeError(
                f"Xing4_0ReasoningParser: '{self.end_token}' not found "
                "in tokenizer vocabulary."
            )
