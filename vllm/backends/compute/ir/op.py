...
from vllm.backends.compute.ir.tolerances import DEFAULT_TOLERANCES, ToleranceSpec
from vllm.backends.compute.ir.util import hash_source, weak_cache
...
def register_op(
    ...
    *,
    ...
    Example usage:
    ```python
    @vllm.backends.compute.ir.register_op
    def my_add(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        return x + y


    @vllm.backends.compute.ir.register_op(name="custom_mul")
...
class IrOp:
    ...
    def set_default(self, priority: list[str]) -> None:
        ...
        logger.debug(
            "Priority for vllm.backends.compute.ir.%s set to %s",
            self.name,
