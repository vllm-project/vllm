...
class CudaPlatformBase(Platform):
    ...
    def graph_pool_handle(cls):
        ...
        memory can be offloaded and restored on sleep/wake. Otherwise fall
        back to the native ``torch.cuda.graph_pool_handle()``.

        The "is sleep mode on" signal is the existence of the
        ``CuMemAllocator`` singleton: it is created the first time a cumem
        memory pool is entered (weight loading under sleep mode), which
        always precedes CUDA graph capture. We deliberately do NOT use
        ``get_current_vllm_config()`` here because graph_pool_handle() is
        also called during ``torch.compile``, where the current-config
        context is not set and the lookup raises.
        """
        try:
