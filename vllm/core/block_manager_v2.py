...
from vllm.core.block.cpu_gpu_block_allocator import (
    CpuGpuBlockAllocator, CpuGpuExternalBlockAllocator)
...
class BlockSpaceManagerV2(BlockSpaceManager):
    ...
    def __init__(
        ...
        if not self.enable_external_swapper:
            ...
            self.block_allocator = CpuGpuExternalBlockAllocator.create(
                allocator_type="prefix_caching" if enable_caching else "naive",
                num_gpu_blocks=num_gpu_blocks,
                num_cpu_blocks=num_cpu_blocks,
    ...
    def swap_in(
        ...
        Returns:
            List[Tuple[BlockSwapParam, BlockSwapParam]]: The mapping of
            swapping block from CPU to GPU.
        """
        ...
        for seq in seq_group.get_seqs(status=SequenceStatus.SWAPPED):
            ...
            seq_physical_block_id_mapping = {
                BlockSwapParam(
                    ...
                    Device.CPU): BlockSwapParam(
                        self.block_allocator.get_physical_block_id(
                            Device.GPU, gpu_block.block_id),  # type: ignore
                        Device.GPU)
                for cpu_block, gpu_block in seq_swap_mapping.items()
            }
    ...
    def swap_in_from_external(
        ...
        """Returns the block id mapping (from External Swapper to GPU)
        generated by swapping in the given seq_group with
        num_lookahead_slots.
        ...
        Returns:
            List[Tuple[BlockSwapParam, BlockSwapParam]]: The mapping of
            swapping block from External Swapper to GPU.
        """
        physical_block_id_mapping = []
        for seq in seq_group.get_seqs(status=SequenceStatus.SWAPPED):
            blocks = self.block_tables[seq.seq_id].blocks
            ...
            seq_swap_mapping = self.block_allocator.swap(
                blocks=blocks,
                src_device=self._external_device,
                dst_device=Device.GPU)
            ...
            seq_physical_block_id_mapping = {
                BlockSwapParam(
                    self.block_allocator.get_physical_block_id(
                        ...
                        external_block.block_id),  # type: ignore
                    self._external_device): BlockSwapParam(
                        self.block_allocator.get_physical_block_id(
                            Device.GPU, gpu_block.block_id),  # type: ignore
                        Device.GPU)
                for external_block, gpu_block in seq_swap_mapping.items()
            }
    ...
    def is_swap_in_from_external(
        ...
        for seq in seq_group.get_seqs(status=SequenceStatus.SWAPPED):
            if len(self.block_tables[seq.seq_id].blocks) == 0:
                continue

            block = self.block_tables[seq.seq_id].blocks[0]
            return self.block_allocator.get_block_device(
                block) == self._external_device
    ...
    def can_swap_out_to_external(self, seq_group: SequenceGroup) -> bool:
        """Returns whether we can swap out the given external swapper
        with num_lookahead_slots.
        ...
        Returns:
            bool: Whether it's possible to swap out current sequence group.
        """
        alloc_status = self._can_swap(seq_group, self._external_device,
                                      SequenceStatus.RUNNING)
        if alloc_status == AllocStatus.OK:
            return True
        return False
    ...
    def swap_out(
        ...
        Returns:
            List[Tuple[BlockSwapParam, BlockSwapParam]]: The mapping of
            swapping block from GPU to CPU.
        """
        physical_block_id_mapping = []
        for seq in seq_group.get_seqs(status=SequenceStatus.RUNNING):
            blocks = self.block_tables[seq.seq_id].blocks
            ...
            seq_physical_block_id_mapping = {
                BlockSwapParam(
                    self.block_allocator.get_physical_block_id(
                        Device.GPU, gpu_block.block_id),  # type: ignore
                    Device.GPU): BlockSwapParam(
                        self.block_allocator.get_physical_block_id(
                            Device.CPU, cpu_block.block_id),  # type: ignore
                        Device.CPU)
                for gpu_block, cpu_block in seq_swap_mapping.items()
            }
    ...
    def swap_out_to_external(
        ...
        """Returns the block id mapping (from GPU to External swapper)
        generated by swapping out the given sequence_group with
        num_lookahead_slots.
        ...
        Returns:
            List[Tuple[BlockSwapParam, BlockSwapParam]]: The mapping of
            swapping block from GPU to External Swapper.
        """
        physical_block_id_mapping = []
        for seq in seq_group.get_seqs(status=SequenceStatus.RUNNING):
            blocks = self.block_tables[seq.seq_id].blocks
            ...
            seq_swap_mapping = self.block_allocator.swap(
                blocks=blocks,
                src_device=Device.GPU,
                dst_device=self._external_device)
                ...
                BlockSwapParam(
                    ...
                    Device.GPU): BlockSwapParam(
                        self.block_allocator.get_physical_block_id(
                            ...
                            external_block.block_id),  # type: ignore
                        self._external_device)
                for gpu_block, external_block in seq_swap_mapping.items()
            }
    ...
    def get_num_free_external_blocks(self) -> int:
        return self.block_allocator.get_num_free_blocks(self._external_device)
