...
"""Fused Qwen4Exp PLE kernels for Intel XPU.

N-gram IDs hash each suffix with
``offset[h] + (xor_i(token[t-i] * multiplier[i]) % size[h])``.

Ported from the NVIDIA kernel; the CUDA programmatic-dependent-launch hints
have no XPU equivalent and are dropped.
"""
...
def can_use_ple_ngram_ids(input_ids: torch.Tensor) -> bool:
    return input_ids.device.type == "xpu" and HAS_TRITON
...
__all__ = ["can_use_ple_ngram_ids", "ple_ngram_ids"]
