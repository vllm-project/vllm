...
class Qwen4ExpNGramEmbedding(nn.Module):
    ...
    def compute_ngram_ids(
        ...
        """Compute n-gram IDs using CPU Torch operations."""
        ...
        context = torch.cat(
            [
                ngram_context[:num_reqs].to(device=input_ids.device, dtype=torch.long),
                packed,
            ],
            dim=-1,
        )
        ...
        for ngram in range(2, self.ngram_size + 1):
            ...
            ids = (
                torch.remainder(
                    mixed.unsqueeze(-1),
                    self.ngram_heads_vocab_sizes[start:end],
                )
                + self.ngram_heads_offsets[start:end]
            )
            id_blocks.append(ids[request_indices, adjusted_columns])
        result = torch.cat(id_blocks, dim=-1)
