...
"""Shared Triton paged attention with Inkling's query-dependent relative bias."""
...
def _inkling_rel_attention_kernel(
    ...
    # KV head, reusing each KV tile across its grouped query heads.
...
def inkling_triton_rel_attention(
    ...
    *,
    ...
    if not causal or window_size[1] not in (0, -1):
        raise NotImplementedError("Inkling Triton attention requires causal masking")
    ...
    if (
        ...
        and max_seqlen_q == 1
        and q.shape[0] == batch
        and q.stride(-1) == out.stride(-1) == 1
        and use_split_kv_decode(
    ...
    if current_platform.is_cuda():
        # CUDA tensor-core dot products require at least 16 rows.
        block_q = max(block_q, 16 // triton.next_power_of_2(gqa_group_size))
