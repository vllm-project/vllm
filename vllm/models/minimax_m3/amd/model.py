...
class MiniMaxM3SparseAttention(nn.Module, AttentionLayerBase):
    ...
    def _run_attention(
        ...
        if not self.skip_index_topk:
            ...
            # buffers. The Triton indexer can instead fuse decode top-k with
            # main's per-forward sparse-table allocation.
            if self.indexer.impl.emits_sparse_block_table:
                self.indexer(index_query)
            else:
                attn_metadata = get_forward_context().attn_metadata
                if not (
                    self.use_aiter_sparse_pa and isinstance(attn_metadata, dict)
                ):
                    self.indexer(index_query)
                    attn_metadata = None
