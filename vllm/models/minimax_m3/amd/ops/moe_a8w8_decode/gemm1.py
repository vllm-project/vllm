...
The workgroup stages bf16 activations through LDS and streams MXFP8 weights
through a per-wave register ring:
...
* a block of ``BM`` sorted rows is ``RT = BM/16`` MFMA row tiles: every W
  fragment a wave unpacks feeds all of them, so an expert with more than 16
  rows streams its weights ``RT`` times less often. Two block widths coexist
  in the sorted layout (``wide=True``): the shared expert, which owns every
  token, comes first in ``WIDE_BM``-row blocks run with ``WIDE_TILE``; the
  routed experts (a few rows each) follow in ``BM``-row blocks. The kernel
  carries both bodies and picks one by block index.
...
BM = 16  # default rows per m-block (one MFMA M tile); compile_gemm1(BM=32) runs two
...
# Wide blocks of the shared expert (sorted layout, ``wide=True``): WIDE_BM rows per
# block, four N-waves of 16 columns, one A chunk per batch (70 KB LDS, the same
# 2 workgroups per CU as the 16-row tile). Its W13 then streams ceil(M/128)
# times instead of M/16 times; the wide workgroups come first in the grid and
# overlap the routed ones.
WIDE_BM = 128
WIDE_TILE = (64, 1, 1)
...
def compile_gemm1(
    *,
    ...
    grid. ``BM`` (16 or 32) is the sort's row block of the routed experts. ``wide``
    (sorted mode) adds the WIDE_BM-row body for the shared expert's blocks, which
    the sort places first: the grid is ``ceil(n_tokens/WIDE_BM) *
    launch.wide_n_blocks`` wide workgroups followed by the routed ones
    (``launch.wide_bm``). ``large_m`` / ``prefetch`` / ``waves_per_eu`` override
    the defaults for lab sweeps only (the kernel name carries them)."""
