...
"""Fused Gemma-style RMSNorm — portable Triton implementation.

Gemma RMSNorm = normalize(x) * (1 + weight), computed in fp32.  Used as a
fallback on platforms where FlashInfer's CUDA kernels are unavailable (CPU,
Neuron, TPU, XPU, …).  The AMD path uses this same implementation via
``amd/ops/gemma_rmsnorm.py``, which is a verbatim copy; keeping a shared copy
here avoids duplication and lets the NVIDIA path import it without a cross-
platform dependency.
