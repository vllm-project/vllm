    ...
    def build(  # type: ignore[override]
        ...
        if num_spec_decodes == 0:
            ...
            if self.use_recoverssm:
                ...
                # A row can report 0 accepted tokens when its sampled tokens
                # were discarded (stale async-scheduling step) while its
                # drafts were still scheduled. Such a row has no valid spec
                # state to resume from, and its recurrent state must not
                # advance this step: null its state slots so the kernels
                # skip both the initial-state read and the final-state
                # write, and clamp the count so the slot index
                # (num_accepted_tokens - 1) stays in bounds. This is done
