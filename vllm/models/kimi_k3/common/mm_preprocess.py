...
class KimiK3MultiModalProcessor(BaseMultiModalProcessor[KimiK3ProcessingInfo]):
    ...
    def preprocess_messages(
        ...
        Supports matching by ``tool_call_id``, by synthetic
        ``"{tool}:{zero_based_index}"`` alias, or by an explicit
        ``(tool, index)`` pair carried on the message.  When any tool
        message in a block cannot be resolved the whole block is left
        in its original order (graceful fallback).
        ...
        while i < len(messages):
            ...
            # Build lookup tables from the assistant's tool_calls.
            targets_by_id: dict[str, tuple[str, int, int]] = {}
            targets_by_pair: dict[tuple[str, int], tuple[str, int, int]] = {}
            for index, tool_call in enumerate(tool_calls, start=1):
                ...
                call_target = (tool, index, index - 1)
                targets_by_pair[(tool, index)] = call_target

                aliases = [f"{tool}:{index - 1}"]
            ...
            unresolved = False
            for order, tool_message in enumerate(messages[block_start:i]):
                resolved: tuple[str, int, int] | None = None

                # 1. Match by tool_call_id
                tool_call_id = tool_message.get("tool_call_id")
                if tool_call_id is not None:
                    resolved = targets_by_id.get(str(tool_call_id))

                # 2. Match by (tool, index) pair
                if resolved is None:
                    tool = tool_message.get("tool") or tool_message.get("name")
                    raw_index = tool_message.get("index")
                    if tool is not None and raw_index is not None:
                        try:
                            resolved = targets_by_pair.get((str(tool), int(raw_index)))
                ...
                tool_name, xtml_index, sort_order = resolved
                enriched = dict(tool_message)
                enriched["tool"] = tool_name
                enriched["index"] = xtml_index
                keyed.append((sort_order, order, enriched))

            if unresolved:
                normalized.extend(messages[block_start:i])
            else:
                keyed.sort(key=lambda item: (item[0], item[1]))
                normalized.extend(msg for _, _, msg in keyed)
