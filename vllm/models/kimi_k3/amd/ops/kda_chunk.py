...
@cache
def _chunk_groups(chunks_per_seq: int, num_heads: int) -> int:
    ...
    not grow with group length. ``C_g`` is still stored in bf16, so G must not
    depend on batch composition: the same sequence would otherwise produce
    different numerics when packed with different neighbours.

    Occupancy therefore uses ``num_heads`` (a model/TP constant) rather than
    ``num_seqs * num_heads``. ``chunks_per_seq`` is the longest sequence in the
    batch, so adding shorter sequences does not change G.

    * ``fill1``/``fill2``: the largest G whose pass-one / pass-two grid still
      fits in one scheduling batch for a single sequence. Rows are launched in
      blocks of at most ``kBV = 128``, so pass one (``kV + kK`` rows) needs two
      blocks per ``(n, h, g)`` and pass two one.
...
def fused_kda_chunk(
    ...
    if output_final_state:
        # Sequences with T == 0 never write ht. Seed from the initial state
        # (or zeros) so those slots are not uninitialized fp32 committed into
        # the persistent SSM cache.
    ...
    # Longest sequence in the batch, not the average: G must be independent of
    # how many short neighbours the scheduler packed in.
    if num_seqs > 0:
        max_tokens = int((cu_seqlens[1:] - cu_seqlens[:-1]).amax().item())
        chunks_per_seq = (max_tokens + CHUNK_SIZE - 1) // CHUNK_SIZE
    else:
        chunks_per_seq = 0
    groups = _chunk_groups(chunks_per_seq, u.shape[2])
