...
"""MLA attention and lightning indexer for HY V4 on AMD."""
...
from .rocm import HYV4ROCmMLASparseBackend, HYV4ROCmSparseIndexerCache
...
class Indexer(nn.Module):
    ...
    def __init__(
        ...
        self.k_cache = HYV4ROCmSparseIndexerCache(
...
class HYV4MLAAttention(nn.Module):
    ...
    The sink is applied by the HY V4 ROCm sparse MLA backend in `.rocm`.
    """
    ...
    def __init__(
        ...
        if self.learnable_sink and not hasattr(config, "index_topk"):
            raise ValueError("HYV4 learnable sinks on ROCm require index_topk")
        self.is_sparse = requested_sparse or self.learnable_sink
        self.create_indexer = self.is_sparse and (not self.skip_topk or is_mtp_layer)
        ...
        if self.learnable_sink:
            ...
            sink_backend = HYV4ROCmMLASparseBackend
