...
"""iHC (independent Hyper-Connections) layers for HY V4 (ROCm).
...
in-tree Triton kernels on ROCm when HPC is unavailable, then to the eager path.
The AMD Triton implementation also provides a fused cross-layer post+pre path.
"""
