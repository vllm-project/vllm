...
def _fp8_quant_and_cache_write(
    ...
    if OWNER_SHARDED_CACHE:
        tl.store(
            kv_cache_ptr
            + owner_peer_rank * cache_peer_stride
            + block_start
        ...
        tl.store(
            kv_cache_scale_ptr
            + owner_peer_rank * scale_peer_stride
            + scale_byte_off // 4,
            scale,
        )
    else:
        # Ordinary non-peer cache insertion.
        tl.store(
...
def _fused_norm_rope_kernel(
    ...
    slot_mapping_ptr,
    owner_slot_mapping_ptr,
    owner_slot_mapping_stride0,
    owner_slot_mapping_stride1,
    # Index K FP8 cache
    indexer_cache_ptr,
    ...
    OWNER_SLOT_CAPACITY: tl.constexpr,
    OWNER_SHARDED_CACHE: tl.constexpr,
    ...
    if pid == 2:
        # Q RMS norm is compute, not a cache write. Profiling may use a negative
        # dummy slot, and PCP may mask replicated decode cache writes on
        # nonzero ranks, but every rank still needs valid Q for local attention.
    ...
    if not MATERIALIZE_CACHE_INPUTS:
        if OWNER_SHARDED_CACHE:
            owner_mapping = (
                owner_slot_mapping_ptr + tok_idx * owner_slot_mapping_stride0
            )
            owner_peer_rank = tl.load(owner_mapping)
            slot_idx = tl.load(owner_mapping + owner_slot_mapping_stride1)
            if (
                (owner_peer_rank < 0)
                | (owner_peer_rank >= PCP_PEER_COUNT)
                | (slot_idx < 0)
                | (slot_idx >= OWNER_SLOT_CAPACITY)
            ):
                # PAD and malformed device-resident mappings must not address
                # any peer allocation.
                return
        else:
    ...
    if pid == 1:
        ...
        if MLA_CACHE_DS_MLA:
            ...
            if OWNER_SHARDED_CACHE:
                tl.store(
                    mla_cache_ptr
                    + owner_peer_rank * mla_cache_peer_stride
                    + byte_base
                    + kv_block,
                    kv_c_fp8,
                )
                tl.store(
                    mla_cache_ds_scale_ptr
                    + owner_peer_rank * mla_cache_ds_scale_peer_stride
                    + byte_base // 4
                    + KV_DIM // 4
                    + tile_off,
                ...
                rope_dst = (
                    mla_cache_ds_rope_ptr
                    + owner_peer_rank * mla_cache_ds_rope_peer_stride
                    + byte_base // 2
                    + (KV_DIM // 2 + 8)
                )
        ...
        if OWNER_SHARDED_CACHE:
            peer_dst = dst + owner_peer_rank * mla_cache_peer_stride
            if MLA_CACHE_FP8:
...
def _fused_norm_rope_impl(
    ...
    # Cache params for fused writes (one mapping is shared by both caches)
    ...
    owner_sharded_cache = pcp_owner_slot_mapping is not None
    owner_slot_capacity = 0
    ...
    if use_pcp_peer_cache:
        ...
        if owner_sharded_cache:
            # Each producer row names one destination as
            # [owner_peer_rank, owner_local_slot]. [-1, -1] is PAD. Keeping
            # ownership explicit avoids a producer-side fanout and lets the
            # scheduler choose any owner-local block layout.
            assert pcp_owner_slot_mapping is not None
            assert pcp_owner_slot_mapping.ndim == 2
            assert pcp_owner_slot_mapping.shape == (num_tokens, 2)
            assert pcp_owner_slot_mapping.device == device
            assert pcp_owner_slot_mapping.dtype in (torch.int32, torch.int64)
        else:
            raise ValueError(
                "A peer-mapped cache requires owner-sharded slot mappings."
            )
    ...
    if use_pcp_peer_cache and has_indexer:
        ...
        owner_slot_capacity = min(
            owner_slot_capacity,
            pcp_peer_indexer_k_cache.shape[1] * pcp_peer_indexer_k_cache.shape[2],
        )
        ...
        idx_cache_scale_peer_stride = 0
        if (
            not use_pcp_peer_cache
            and mla_kv_cache is None
            and not materialize_cache_inputs
        ):
    ...
    if mla_kv_cache is not None:
        ...
        # the cache-address calculation safely.
        mla_block_stride = 1
        mla_entry_stride = 1
    ...
    owner_slot_mapping = pcp_owner_slot_mapping if owner_sharded_cache else slot_mapping
    owner_slot_mapping_stride0 = (
        pcp_owner_slot_mapping.stride(0) if owner_sharded_cache else 0
    )
    owner_slot_mapping_stride1 = (
        pcp_owner_slot_mapping.stride(1) if owner_sharded_cache else 0
    )
    ...
    _fused_norm_rope_kernel[(4, num_tokens)](
        ...
        OWNER_SLOT_CAPACITY=owner_slot_capacity,
        OWNER_SHARDED_CACHE=owner_sharded_cache,
...
def _fused_norm_rope_op(
    ...
    pcp_size: int,
    pcp_owner_slot_mapping: torch.Tensor | None,
...
def fused_norm_rope(
    ...
    if q_c_out is None:
        q_c_out = torch.empty_like(q_c)
    # The local cache is a slice of the rank-major peer mapping. Passing both
    # tensors to a registered custom op exposes overlapping mutable arguments
    # to functionalization. The implementation consumes the peer mapping when
    # present, so omit its local alias at the dispatcher boundary.
