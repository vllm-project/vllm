...
class DequantGatherKCacheKernel(
    ...
    def kernel(compile_key: CompileKey) -> Any:
        ...
        def load_g2s(
            ...
            idx += 32
            if idx < cutlass.const_expr(data_dim // 16):
                src = k_data_slice[page_id, block_offset, (None, idx)]
                cute.copy(
                    cp16_atom,
                    cute.recast_tensor(src, Uint32),
        ...
        def device_kernel(
            ...
            s_kdata = smem.allocate_tensor(
                Uint32,
                cute.make_layout((data_dim // 4, num_warps, num_stages)),
                byte_alignment=16,
            )[None, warp_id, None]
            ...
            for i in range(
                ...
                cute.copy(
                    cp8_atom, s_kdata_8B_slice[(None, lane_id), compute_stage], data0
                )
                cute.copy(
                    cp8_atom,
                ...
                scale0_u32 = Uint32(s_kscale[lane_id * 8 // group_size, compute_stage])
                scale0_bf16x2 = (scale0_u32 << Uint32(23)) | (scale0_u32 << Uint32(7))
                scale1_u32 = Uint32(
                    s_kscale[(lane_id + 32) * 8 // group_size, compute_stage]
                )
        ...
        def host_entrypoint(
            ...
            k_data = cute.make_tensor(
                ...
                layout=cute.make_layout(
                    (k_cache.shape[0], block_size, data_dim),
            ...
            num_workers = cutlass.max(
                1,
                cutlass.min(cute.ceil_div(max_rows, 4), cute.ceil_div(8192, num_reqs)),
            )
    ...
    def warmup_inputs(self, compile_key: CompileKey) -> tuple[Any, ...]:
        ...
        gather_lens = (
            make_fake_tensor(Int32, (num_reqs,))
            if compile_key.has_gather_lens
            else None
        )
