...
"""Small helpers vendored from upstream model files.

This module is the home for tiny stand-alone symbols that DSv4
hw-agnostic uses but live in upstream model implementations the
isolation lint forbids:

  * ``SharedHead`` — vendored from
    ``vllm/model_executor/models/deepseek_mtp.py``.
  * ``get_spec_layer_idx_from_weight_name`` — vendored from
    ``vllm/model_executor/models/deepseek_v2.py``.
  * ``SupportsPP`` — vendored from
    ``vllm/model_executor/models/interfaces.py``. The pp-detection
    helpers (``supports_pp``, ``_supports_pp_attributes``, etc.) live
    upstream and are not used directly by DSv4 hw-agnostic; only the
    Protocol class is needed.

All three are pure data / protocol definitions with no hardware
dispatch — they were forbidden purely by the broad
``model_executor.models.*`` lint scope.
"""
