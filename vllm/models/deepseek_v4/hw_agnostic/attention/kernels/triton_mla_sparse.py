...
def triton_bf16_mla_sparse_interface(
    ...
    """BF16 sparse MLA — interface used by both decode and prefill.

    Args:
        q: [num_tokens, num_heads_q, dim_qk]
        kv: [num_tokens, num_heads_kv, dim_qk]; the kernel reuses ``kv``
            as both K and V (V is sliced as ``kv[..., :d_v]``).
        indices: [num_tokens, num_heads_kv, topk]; use ``-1`` to mark
            padding past each token's valid prefix.
        sm_scale: pre-LOG2E softmax scale (the kernel multiplies in
