...
"""MTP draft model for DeepSeek V4 on Turing (SM75).

Ported from ``vllm/models/deepseek_v4/xpu/mtp.py``. Uses only portable ops:
``fused_mtp_input_rmsnorm`` / ``mtp_shared_head_rmsnorm``, the Turing
``TuringMLAAttention`` via ``DeepseekV4DecoderLayer``, FP16 compute, and
the SM75 FP16/FP8 fused-MoE paths.
"""
...
class DeepSeekV4MultiTokenPredictor(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Disable aux streams on Turing — no multi-stream overlap support.
