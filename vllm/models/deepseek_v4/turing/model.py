...
"""Turing (SM75) DeepSeek V4 model.

Ported from ``vllm/models/deepseek_v4/xpu/model.py``: FP16 compute
(dtype literals are ``torch.float16``), the Turing sparse MLA attention
(``TuringMLAAttention``), and MegaMoE gated off (deep-gemm kernels are
sm90+). The fused-MoE path keeps FP8/FP4 weights packed on device.
"""
...
class DeepseekV4MegaMoEExperts(nn.Module):
    ...
    def _check_runtime_supported(self) -> None:
        raise NotImplementedError("DeepSeek V4 MegaMoE is not supported on SM75.")
...
class DeepseekV4MoE(nn.Module):
    def __init__(
        ...
        self.prefix = prefix
        # Deep-gemm MegaMoE kernels are sm90+ and unsupported on SM75.
        if vllm_config.kernel_config.moe_backend == "deep_gemm_mega_moe":
            raise NotImplementedError(
                "DeepSeek V4 MegaMoE is not supported on Turing (SM75). "
                "Use the default (non-MegaMoE) fused MoE backend."
            )
