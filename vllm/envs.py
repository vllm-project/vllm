...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    "VLLM_USE_TRTLLM_CONTEXT_ATTENTION": lambda: os.getenv(
        "VLLM_USE_TRTLLM_CONTEXT_ATTENTION", None
    ),
    # If set to 1, use the TRTLLM Decode Attention backend in flashinfer.
    "VLLM_USE_TRTLLM_DECODE_ATTENTION": lambda: os.getenv(
    ...
    "VLLM_USE_FLASHINFER_MXFP4_MOE": lambda: bool(
        int(os.getenv("VLLM_USE_FLASHINFER_MXFP4_MOE", "0"))
    ),
    # If set to 1, use the FlashInfer MXFP4 x BF16 MoE backend.
    "VLLM_USE_FLASHINFER_MXFP4_BF16_MOE": lambda: bool(
        int(os.getenv("VLLM_USE_FLASHINFER_MXFP4_BF16_MOE", "0"))
    ),
    ...
    "VLLM_QUARK_EMU_MEM_OPT": lambda: bool(
        int(os.getenv("VLLM_QUARK_EMU_MEM_OPT", "0"))
    ),
    ...
    "VLLM_USE_HPU_CONTIGUOUS_CACHE_FETCH": lambda: os.environ.get(
        "VLLM_CONTIGUOUS_PA", "true"
    ).lower()
    in ("1", "true"),
    # Use delayed sampling for HPU to reduce host cpu overhead
    # between each step.
    "VLLM_HPU_USE_DELAYED_SAMPLING": lambda: os.environ.get(
        "VLLM_DELAYED_SAMPLING", "false"
    ).lower()
    in ("1", "true"),
    # Converts model weights to FP8UZ format.
    "VLLM_HPU_CONVERT_TO_FP8UZ": lambda: os.environ.get(
    ...
    "VLLM_HPU_FORCE_CHANNEL_FP8": lambda: os.environ.get(
        "VLLM_HPU_FORCE_CHANNEL_FP8", "true"
    ).lower()
    in ("1", "true")
