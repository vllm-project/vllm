...
if TYPE_CHECKING:
    ...
    VLLM_DEEPSEEK_V41_SWA_BOUNDED_REPLAY: bool = True
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # DeepSeek-V4.1: keep the sliding-window KV out of prefix caching and
    # recompute the last `sliding_window` tokens of every prefix-cache or
    # KV-connector hit to rebuild it (encoder SWA bounded replay). Replayed
    # tokens see a truncated window, so a hit is not bit-identical to a cold
    # run. Needs model runner V2; off on V1.
    "VLLM_DEEPSEEK_V41_SWA_BOUNDED_REPLAY": lambda: bool(
        int(os.getenv("VLLM_DEEPSEEK_V41_SWA_BOUNDED_REPLAY", "1"))
    ),
