...
def extra_keys_to_typed(
    ...
    def classify_str(value: str, remaining_lora: bool, remaining_salt: bool):
        """Classify a bare string extra key against the known request values.

        The producer emits, per block, at most one LoRA name first, then at
        most one ``cache_salt`` (block 0 only) later. When ``cache_salt``
        equals the LoRA name the two bare strings collide, so classification
        must consume each known value once per *block* (not once globally:
        the LoRA name repeats on every block).
        """
        if remaining_lora and remaining_salt and value == lora_name == cache_salt:
            # Equal lora_name/cache_salt: the first occurrence is the LoRA
            # entry (producer order), flipping to the salt role on the second.
            return LoRAKey(name=value), remaining_lora and False, remaining_salt
        if remaining_lora and value == lora_name:
            return LoRAKey(name=value), False, remaining_salt
        if remaining_salt and value == cache_salt:
            return CacheSaltKey(salt=value), remaining_lora, False
        return None, remaining_lora, remaining_salt
    ...
    for block_keys in extra_keys_list:
        if block_keys is None:
            ...
            continue
        # Per-block consumption: each known string is matched at most once
        # within the block. The LoRA name repeats across blocks.
        remaining_lora = lora_name is not None
        remaining_salt = cache_salt is not None
        ...
        for key in block_keys:
            ...
            if (
                ...
                known, remaining_lora, remaining_salt = classify_str(
                    key, remaining_lora, remaining_salt
                )
                if known is None:
...
class BlockStored(KVCacheEvent):
    ...
    Stored as ``None`` until published; ``_publisher_thread`` stamps the
    concrete version on the wire instead of relying on ``omit_defaults``,
    which would drop a default-valued field from the payload.
    """
    ...
    def to_typed(self) -> "BlockStored":
        """Return a copy with ``event_version`` stamped for publishing.

        ``omit_defaults=True`` on ``KVCacheEvent`` drops any field left at
        its default, so a typed event (one carrying ``extra_keys``) must
        explicitly set ``event_version`` before encode to keep the version
        on the wire. Legacy events (``extra_keys=None``) keep the exact
        legacy payload by leaving the version unset.
        """
        if not self.extra_keys or self.event_version is not None:
            return self
        event = copy(self)
    ...
    def _publisher_thread(self) -> None:
        ...
        while self._running or self._event_queue.qsize() > 0:
            ...
            try:
                ...
                # Stamp the typed schema version on the wire: plain defaults
                # are dropped by ``EventBatch.omit_defaults``, so only typed
                # events (those carrying ``extra_keys``) get ``event_version``.
                if isinstance(event, KVEventBatch):
                    event = KVEventBatch(
                        ts=event.ts,
                        events=[
                            e.to_typed() if hasattr(e, "to_typed") else e
