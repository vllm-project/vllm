...
from .rebalance_execute import move_from_buffer,eplb_worker
...
#--------------------------------------新增的----------------------------------------------------   
    ...
    def step(self,
        ...
        """
        if not self.ep_buffer_ready:
            self.move_to_workspace(model)
    ...
    def pre_calculate(self,
        ...
        num_devicess = ep_group.size()

        if num_devices % num_nodes != 0:
            logger.warning_once(
                f"num_deviceus % num_nodes != 0, "
    ...
    """创建子线程异步事件循环，支持多GPU分布式通信"""
        def thread_target():
            ...
            try:
            # 循环执行任务
                loop.run_until_complete(self._run_periodically(model, experts_stream))
    ...
    async def _run_periodically(self,model,stream):
        """
        ...
        while not _stop_event.is_set():
            if not self.ep_buffer_ready and self.cp:
                ...
                self.layer+=1
                self.ep_buffer_ready = True  # 执行完f后设置A为True
            else:
                await asyncio.sleep(0.1)  # 短暂休眠避免CPU占用过高


    def move_to_workspace(self, model: MixtureOfExperts):
        async_move_from_buffer(
