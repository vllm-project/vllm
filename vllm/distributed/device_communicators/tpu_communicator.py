...
class TpuCommunicator(DeviceCommunicatorBase):
    ...
    def __init__(self,
        ...
        # Set required attributes from the base implementation.
        self.groups = self._base_communicator.groups
    ...
    def all_reduce(self, input_: torch.Tensor) -> torch.Tensor:
        ...
        """Delegate all_reduce to the base implementation."""
        return self._base_communicator.all_reduce(input_)

    def all_gather(self, input_: torch.Tensor, dim: int = -1) -> torch.Tensor:
        # assert dim == -1, "TPUs only support dim=-1 for all-gather."
        # return xm.all_gather(input_, dim=dim)
        """Delegate all_gather to the base implementation."""
        return self._base_communicator.all_gather(input_, dim=dim)
