...
# Feature toggle for AutoPerf Plan Step 2
_PLAN_STEP = 2
_PLAN_DESCRIPTION = "AllReduce push-based 2-buffer protocol"
_DISABLE_ENV_VAR = f"VLLM_DISABLE_AUTO_PERF_PLAN_{_PLAN_STEP}"
...
class PushAllReduce:
    ...
    def __init__(
        ...
        if os.environ.get(_DISABLE_ENV_VAR) == "1":
            logger.info(
                "[AutoPerf] Plan step %d: %s is DISABLED (env override)",
                _PLAN_STEP,
                _PLAN_DESCRIPTION,
            )
            self.disabled = True
        ...
        try:
            ...
            logger.info(
                "[AutoPerf] Plan step %d: %s is ENABLED",
                _PLAN_STEP,
                _PLAN_DESCRIPTION,
            )
