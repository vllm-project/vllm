...
class ElasticScalingState:
    def __init__(
        ...
        if scale_type == "scale_up":
            self.state = (
                ScaleUpNewWorkerState.EPLB_RESHUFFLE
                if worker_type == "new"
                else ScaleUpExistingWorkerState.WAIT_NEW_WORKERS_INIT
            )
        else:
            self.state = (
                ScaleDownShutdownWorkerState.EPLB_RESHUFFLE
    ...
    def _progress_existing_worker(self) -> bool:
        ...
        if state == ScaleUpExistingWorkerState.WAIT_NEW_WORKERS_INIT:
            ...
            if (
                int(self.old_dp_store.get("elastic_ep_request_first_barrier"))
            ...
            if (
                int(self.old_dp_store.get("elastic_ep_request_second_barrier"))
                < self.old_dp_group.size()
            ):
                return False
            ...
            if (
                int(self.new_dp_store.get("elastic_ep_request_third_barrier"))
    ...
    def _switch_and_prepare(self):
        ...
        logger.info(
            "[Elastic EP] Switched to new comm group and prepare model for new setup"
        )
