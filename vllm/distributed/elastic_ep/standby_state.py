...
_STANDBY_DP_PCP: StatelessGroupCoordinator | None = None
...
def create_standby_groups(
    ...
    with defer_comm_warmup_on_rocm():
        ...
        if new_dp_size > 1 and pcp_size > 1 and tp_size > 1:
            standby_dp_pcp_ranks = (
                all_ranks.permute(0, 2, 4, 1, 3)
                .reshape(-1, new_dp_size * pcp_size)
                .unbind(0)
            )
            standby_dp_pcp_ranks = [x.tolist() for x in standby_dp_pcp_ranks]
            _STANDBY_DP_PCP = _init_stateless_group(
                standby_dp_pcp_ranks,
                "dp_pcp",
                master_ip,
                backend,
