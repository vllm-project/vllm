...
wait_for_kv_cache_ready_timeout = 60
...
async def proxy_request(request: Request):
    ...
    try:
        ...
        prefill_req_data = copy.deepcopy(req_data)
        prefill_req_data["max_tokens"] = 1  # Only set for prefill
        await send_request_to_vllm(app.state.prefill_client, prefill_req_data)
        ...
        success = await asyncio.wait_for(wait_for_kv_cache(), timeout=wait_for_kv_cache_ready_timeout)

        if not success:
            request_store.pop(request_id, None)  # Cleanup to prevent stale data
        ...
        # Ensure cleanup in case of any failure
        request_store.pop(request_id, None)
        kv_cache_ready_flags.pop(request_id, None)
