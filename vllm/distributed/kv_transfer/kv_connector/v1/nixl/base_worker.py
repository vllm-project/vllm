        ...
        with zmq_ctx(zmq.REQ, path) as sock:
            for remote_pp_rank, remote_worker_key in itertools.product(
                ...
                msg = msgspec.msgpack.encode(
                    (GET_META_MSG, remote_pp_rank, remote_flat_idx)
                )
                ...
                if (
                    ...
                    raise RuntimeError(
                        "Remote handshake topology mismatch: expected "
                        f"worker={remote_worker_key}, tp={remote_tp_size}, "
                        f"dcp={remote_dcp_size}, pcp={remote_pcp_size}; got "
                        f"worker={(metadata.tp_rank, metadata.dcp_rank)}, "
                        f"tp={metadata.tp_size}, dcp={metadata.dcp_size}, "
                        f"pcp={metadata.pcp_size}."
                    )
