...
"""NIXL connector worker-side implementation."""
...
class NixlConnectorWorker:
    ...
    def __init__(
        ...
        # TODO (ZhanqiuHu): _mamba_phys_ratio and _transfer_configs are only
        # used for Mamba hybrid models for now. They should be merged/combined
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        self.kv_topo = TpKVTopology(
            ...
            tensor_shape=self.block_transfer_policy.get_tensor_shape(kv_caches),
            is_mamba=self.block_transfer_policy.is_mamba,
        )
        ...
        for layer_name, cache_or_caches in xfer_buffers.items():
            ...
            physical_page_size = self.block_transfer_policy.compute_page_size(
            ...
            num_blocks = self.block_transfer_policy.get_num_blocks(
            ...
            for cache in cache_list:
                ...
                self.block_len_per_layer.append(
                    self.block_transfer_policy.compute_layer_block_bytes(
                        layer_spec,
                        physical_page_size,
                        self._physical_blocks_per_logical_kv_block,
                    )
                ...
                # TODO (ZhanqiuHu): Refactor: non-MLA model-specific logics
                if not self.use_mla:
        ...
        # TODO (ZhanqiuHu): Refactor: Mamba model-specific logics
        if self.block_transfer_policy.is_mamba:
            logger.info(
                ...
                self.num_regions,
                self.num_descs,
                self.block_transfer_policy.ssm_sizes,
    ...
    def register_local_xfer_handler(
        ...
        blocks_data = self.block_transfer_policy.build_local_descs(
    ...
    def add_remote_agent(
        ...
        if self.block_transfer_policy.is_mamba:
            ...
            self._mamba_phys_ratio[engine_id] = sct.compute_mamba_phys_ratio(
                nixl_agent_meta.ssm_sizes,
                nixl_agent_meta.block_lens[0],
            )
            if engine_id not in self._transfer_configs:
                ...
                self._transfer_configs[engine_id] = cfg
        ...
        if (
            ...
            for handle_data in self.block_transfer_policy.build_src_split_handles(
                self.src_blocks_data,
                self.num_descs,
                abs_tp,
                self._transfer_configs.get(engine_id),
            ):
        ...
        blocks_data = self.block_transfer_policy.build_remote_descs(
            ...
            transfer_config=self._transfer_configs.get(engine_id),
            physical_blocks_per_logical=self._mamba_phys_ratio.get(
                engine_id,
                1,
            ),
        )
    ...
    def _validate_remote_agent_handshake(
        ...
        # TODO (ZhanqiuHu): Refactor: Mamba model-specific logics
        if not self.block_transfer_policy.is_mamba:
            assert not (
        ...
        if self.use_mla or self.kv_topo.is_kv_replicated(remote_engine_id):
            ...
            # TODO (ZhanqiuHu): Refactor: Mamba model-specific logics
            # For mamba models, validate FA and mamba block_lens separately.
            if not self.block_transfer_policy.is_mamba:
    ...
    def save_kv_to_host(self, metadata: NixlConnectorMetadata):
        ...
        for req_id, meta in metadata.reqs_to_save.items():
            meta.local_physical_block_ids = (
                self.block_transfer_policy.logical_to_kernel_block_ids(
    ...
    def _read_blocks_for_req(self, req_id: str, meta: ReqMeta):
        ...
        read_specs = self.block_transfer_policy.compute_read_specs(
            local_block_ids=meta.local_physical_block_ids,
        ...
        # MLA: notify remote ranks we did NOT read from.
        # TODO (ZhanqiuHu): Refactor: MLA model-specific logics
        if self.use_mla and tp_ratio < 0:
    ...
    def _read_blocks(
        ...
        for i, remote_group in enumerate(remote_block_ids):
            ...
            if (
                ...
                and not self.block_transfer_policy.is_mamba_group(i)
            ):
    ...
    def _get_block_descs_ids(
        ...
        """
        return self.block_transfer_policy.get_block_descs_ids(
            block_ids=block_ids,
