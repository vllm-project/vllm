...
class NixlPromMetrics(KVConnectorPromMetrics):
    def __init__(
        ...
        config_labelnames = [*labelnames, "kv_connector", "kv_role",
                             "compatibility_hash"]
        self.gauge_nixl_config_info = self._gauge_cls(
            name="vllm:nixl_config_info",
            documentation="NIXL KV transfer configuration "
    ...
    def record_config_info(
        self, vllm_config: VllmConfig, engine_idx: int = 0
    ) -> None:
        ...
        label_values.extend([
            ...
            (kv_transfer_config.compatibility_hash or "unknown") if kv_transfer_config else "unknown",
        ])
