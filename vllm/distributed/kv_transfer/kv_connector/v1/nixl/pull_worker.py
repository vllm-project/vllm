...
class NixlPullConnectorWorker(NixlBaseConnectorWorker):
    ...
    def _read_hisparse_host_blocks(
        self,
        req_id: str,
        meta: ReqMeta,
        plan: "TPMapping",
        ...
        assert meta.remote is not None and self.transfer_topo is not None
        if self._has_mamba or not self.use_mla:
            raise NotImplementedError(
        ...
        local_by_region = self._block_ids_by_region(
            meta.local_physical_block_ids, self.region_group_ids
        )
        ...
        for region, local_blocks in zip(
            self._hisparse_host_regions, local_by_region, strict=True
        ):
            if region is None:
                destination_pages.append(list(local_blocks))
                continue
            _, _, host_page_capacity = region
            if host_page_capacity % host_num_blocks:
                raise ValueError("HiSparse host region capacity is not block aligned")
            pages_per_block = host_page_capacity // host_num_blocks
        ...
        trimmed_destination_pages, trimmed_remote_by_region = (
            self._apply_prefix_caching_by_region(destination_pages, remote_by_region)
        )
        ...
        remote_desc_ids = self._compute_desc_ids(
            block_ids=remote_by_region,
            dst_num_blocks=self.dst_num_blocks[engine_id],
            block_size_ratio=None,
            physical_blocks_per_logical=(
                self.transfer_topo.get_engine_info(
                    engine_id
                ).remote_physical_blocks_per_logical
            ),
            region_num_blocks=self.dst_region_num_blocks[engine_id],
            region_group_ids=list(range(self.num_regions)),
        )
        ...
        for region_idx, (pages, mirrors) in enumerate(
            ...
            host_region = self._hisparse_host_regions[region_idx]
            block_len = self.block_len_per_layer[region_idx]
            ...
            for page_idx, page_id in enumerate(pages):
                if host_region is None:
                    dst_addrs.append(
                        ...
                        + page_id * self.region_strides[region_idx]
                    )
                    ...
                    mirror_addrs.append(
                        ...
                        + mirror_block * self.region_strides[region_idx]
                    )
        ...
        notif_id = f"{meta.remote.request_id}:{self.world_size}".encode()
        agents = self._remote_agents[engine_id]
        if self.transfer_topo.tp_ratio(meta.tp_size) < 0:
            pending_notifs = [(agent, notif_id) for agent in agents.values()]
        else:
            pending_notifs = [(agents[(0, remote_rank)], notif_id)]
        self._pending_recv_notifs.setdefault(req_id, []).extend(pending_notifs)
        try:
            self._get_hisparse_host_stager().submit(
                req_id,
                remote_desc_ids,
