...
class NixlPushConnectorScheduler(NixlBaseConnectorScheduler):
    ...
    def __init__(
        ...
        self._lw_need_save: dict[ReqId, tuple[Any, BlockIds]] = {}
        self._lw_pending_save: dict[ReqId, tuple[Any, list]] = {}
    ...
    def update_state_after_alloc(
        ...
        # main uses a 3-tuple (request, block_ids, cached); keep it so
        # base_scheduler's ``add_new_req_to_recv`` unpacks correctly.
    ...
    def build_connector_meta(
        ...
        for _rid, (_req, _bids, _cached) in self._reqs_need_recv.items():
    ...
    def _lw_emit(
        ...
        blocks = self.get_sw_clipped_blocks(tuple(list(g) for g in blocks))
    ...
    def _lw_build_reqs_to_save(
        ...
        for i, req_id in enumerate(req_ids):
            ...
            new_blocks = (
                new_block_ids_list[i] if i < len(new_block_ids_list) else None
            )
