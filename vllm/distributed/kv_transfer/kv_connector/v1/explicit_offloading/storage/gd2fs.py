...
class GD2FSStorage(ExOffloadingStorage):
    ...
    def register_kvcache(self, config: ExOffloadingStorageKVCacheConfig) -> None:
        assert config.split_k_and_v, "GD2FS only supports split k and v"
        assert not config.is_block_first, "GD2FS does not support block first layout"
    ...
    def _get_mem_tensors(self, block_ids: list[int]) -> list[torch.Tensor]:
        return get_mem_tensors(self.kv_caches, block_ids)
