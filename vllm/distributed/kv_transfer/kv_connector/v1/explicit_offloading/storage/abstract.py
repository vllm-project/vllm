...
def build_mem_regions(
    ...
    for _, cache_list in kv_caches.items():
    ...
    return (
        mem_regions_per_layer[::2] + mem_regions_per_layer[1::2],
        block_bytes_per_layer[::2] + block_bytes_per_layer[1::2],
    )
...
def get_mem_tensors(
    kv_caches: dict[str, torch.Tensor], block_ids: list[int]
) -> list[torch.Tensor]:
    group_block_ids = group_block_contiguous(block_ids)
