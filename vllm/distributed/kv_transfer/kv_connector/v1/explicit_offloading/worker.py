...
class ExOffloadingConnectorWorker:
    ...
    async def _start_load_task(self, ctx: ExOffloadingRequestContext):
        assert id not in self._finished_load_req_ids

        try:
            ctx.stored_exkvcache.update_kv_layout(tp_rank=self._tp_rank)
            await ctx.stored_exkvcache.prefetch(self._kvcache_config)
        except Exception as e:
            logger.error(
                "(%s:%s) Failed loading kv cache: %s", ctx.id, ctx.request_id, e
            )
            assert ctx.stored_exkvcache.block_ids is not None
            self._invalid_block_ids.update(ctx.stored_exkvcache.block_ids)
    ...
    async def _start_save_task(self, ctx: ExOffloadingRequestContext):
        assert id not in self._finished_save_req_ids

        try:
            ctx.fresh_exkvcache.update_kv_layout(tp_rank=self._tp_rank)
            await ctx.fresh_exkvcache.backup(self._kvcache_config)
        except Exception as e:
            logger.error(
