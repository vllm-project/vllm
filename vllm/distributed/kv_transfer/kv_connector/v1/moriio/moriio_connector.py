...
def _split_kv_cache_group_kinds(
    ...
    if kv_cache_config is not None:
        ...
        for gi, group in enumerate(kv_cache_config.kv_cache_groups):
            if not group.enable_kv_transfer:
                continue
...
class MoRIIOConnectorScheduler:
    ...
    def __init__(
        ...
        # a single running state in the last non-speculative slot.
        ...
        # hybrid (mamba/KDA) models (unpack with split_attn_mamba_block_ids). The mamba
    ...
    def update_state_after_alloc(
        ...
        if params.get("do_remote_decode") and self.mode == MoRIIOMode.WRITE:
            attn_block_ids, _ = self.split_block_groups(blocks.get_block_ids())
            # Hybrid WRITE is rejected in worker registration, so this
            # chunked-prefill bookkeeping always carries flat attention ids.
                ...
                if remote_block_ids := params.get("remote_block_ids"):
                    ...
                    if "remote_engine_id" in params:
                        ...
                        if num_external_tokens > 0:
                            ...
                            # externally allocated destination blocks before
    ...
    def split_block_groups(
        ...
        into the recurrent-state slot list. A scheduler constructed without
        ``kv_cache_config`` keeps the legacy ``block_ids[0]`` behavior.

        Mamba groups are clipped to the slots that actually carry state, so
        both legs derive the same slot list from their own allocation: the
        trailing speculative scratch slots always go, and outside
        ``mamba_cache_mode="all"`` only the last (running-state) slot remains.
        Without this the prefiller's and the decoder's lists differ in length
    ...
    def _clip_mamba_group(self, group_index: int, blocks: list[int]) -> list[int]:
        ...
        if not blocks:
            return blocks
        n_spec = self._ssm_spec_blocks[group_index] if self._ssm_spec_blocks else None
        if n_spec:
            # Never strip the running state itself.
            drop = min(n_spec, len(blocks) - 1)
            if drop:
                blocks = blocks[:-drop]
    ...
    def _align_read_blocks(
        ...
        recomputed final token and speculative lookahead; those blocks have no
        remote source and are not transferred. The caller derives the maximum
        attention tail from the configured lookahead and strips speculative
        mamba slots before calling this helper.
        """
    ...
    def request_finished(
        ...
        ``block_ids`` is the flat attention list; hybrid callers pass the
        recurrent-state slot separately in ``mamba_block_ids`` (they have
        already split the per-group tuple, so it must not be split again).
        """
        ...
        if params.get("do_remote_prefill"):
            ...
            if self.mode == MoRIIOMode.WRITE:
                ...
                if mamba_block_ids:
                    recv_blocks: list = [list(block_ids), list(mamba_block_ids)]
                elif block_ids:
                    recv_blocks = [list(block_ids)]
                else:
                    recv_blocks = []
...
class MoRIIOConnectorWorker:
    ...
    def _write_blocks_for_req(self, req_id: ReqId, meta: ReqMeta, layer_name, kv_layer):
        ...
        # Hybrid WRITE is rejected once at cache registration, so metadata
        # reaching this per-layer hot path is necessarily attention-only.
        local_attn = cast(list[int], meta.local_block_ids)
        remote_attn = cast(list[int], meta.remote_block_ids)
        self.schedule_write_blocks(
            request_id=req_id,
            transfer_id=meta.transfer_id,
            dst_engine_id=meta.remote_engine_id,
            local_block_ids=local_attn,
            remote_block_ids=remote_attn,
            layer_name=layer_name,
            kv_layer=kv_layer,
            remote_notify_port=meta.remote_notify_port,
    ...
    def _mamba_tp_ratio(self, remote_tp_size: int | None) -> int:
        """Return the signed TP ratio used by the fail-closed KDA gate."""
