    ...
    def _process_deferred_tasks(self) -> None:
        ...
        for task in self._deferred_tasks:
            ...
            if now - task.enqueue_time > defer_timeout:
                # The consumer simply has not allocated destination blocks yet,
                # which is expected while it is at KV capacity. Marking the
                # transfer done here would free the producer blocks without
                # notifying the consumer, hanging it in WAITING_FOR_REMOTE_KVS.
