...
class MoRIIOWrapper:
    ...
    def _wait_all_transfers(self, transfers_to_wait: list[Any]) -> None:
        ...
        if wait_all is None:
            raise TransferError("MoRIIO engine does not support wait_all")
        rc = wait_all(transfers_to_wait, timeout_ms=timeout_ms)
    ...
    def _poll_transfers_until_done(self, transfers_to_wait: list[Any]) -> None:
        ...
        Gives failure precedence over pending, the same way the batched wait
        does: once any status has failed the request is lost, so waiting out the
        remaining ones would only delay the barrier by up to transfer_timeout.
        """
        timeout = self._transfer_timeout
