...
@dataclass
class P2PXferAgentMetadata(KVConnectorHandshakeMetadata):
    engine_id: str
...
class P2PXferConnectorMetadata(KVConnectorMetadata):
    def __init__(self):
...
class P2PXferConnector(KVConnectorBase_V1):
    def __init__(
        self,
    ...
    def get_required_kvcache_layout(cls, vllm_config: VllmConfig):
        ...
        if use_mla:
            ...
            return None
        logger.info_once(
            "P2PXferConnector setting KV cache layout to HND for better xfer performance."
        )
        return "HND"
    ...
    def get_block_ids_with_load_errors(self) -> set[int]:
        """Get block IDs that failed to load via P2PXFER."""
        assert self.connector_worker is not None
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs) -> None:
        assert self.connector_worker is not None
        assert isinstance(self._connector_metadata, P2PXferConnectorMetadata)
        self.connector_worker.start_load_kv(self._connector_metadata)

    def wait_for_layer_load(self, layer_name: str) -> None:
        """P2PXferConnector does not do layerwise saving."""
        pass
    ...
    def save_kv_layer(
        ...
        **kwargs,
        ...
        """P2PXferConnector does not save explicitly."""
        pass
...
class P2PXferConnectorScheduler:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str):
        ...
        self.side_channel_host = envs.VLLM_P2PXFER_SIDE_CHANNEL_HOST
        self.side_channel_port = (
            envs.VLLM_P2PXFER_SIDE_CHANNEL_PORT
            + vllm_config.parallel_config.data_parallel_rank
        )
        self.abort_request_timeout = envs.VLLM_P2PXFER_ABORT_REQUEST_TIMEOUT
        ...
        logger.info("Initializing P2PXfer Scheduler %s", engine_id)

        # Background thread for handling new handshake requests.
        self._p2pxfer_handshake_listener_t: threading.Thread | None = None
    ...
    def shutdown(self):
        ...
        if self._p2pxfer_handshake_listener_t is not None:
            self._p2pxfer_handshake_listener_t.join()
            self._p2pxfer_handshake_listener_t = None
    ...
    def set_xfer_handshake_metadata(
        ...
        for tp_rank, rank_metadata in metadata.items():
            if not isinstance(rank_metadata, P2PXferAgentMetadata):
                raise ValueError(
                    "P2PXferConnectorScheduler expects P2PXferAgentMetadata for "
                    "handshake metadata."
                )
            encoded_data[tp_rank] = encoder.encode(rank_metadata)
            logger.debug(
                "Tp rank %d: encoded P2PXferAgentMetadata size: %s bytes",
                tp_rank,
        ...
        if self._p2pxfer_handshake_listener_t is None:
            ready_event = threading.Event()
            self._p2pxfer_handshake_listener_t = threading.Thread(
                target=self._p2pxfer_handshake_listener,
                args=(
                    encoded_data,
                    ready_event,
                    self._stop_event,
                ...
                daemon=True,
                name="p2pxfer_handshake_listener",
            )
            self._p2pxfer_handshake_listener_t.start()
            ready_event.wait()  # Wait for listener ZMQ socket to be ready.

    @staticmethod
    def _p2pxfer_handshake_listener(
        encoded_data: dict[int, Any],
        ready_event: threading.Event,
        stop_event: threading.Event,
        port: int,
    ):
        """Background thread for getting new handshakes."""
        # NOTE(rob): this is a simple implementation. We will move
        # to a better approach via HTTP endpoint soon.

        # Listen for new requests for metadata.
        host = envs.VLLM_P2PXFER_SIDE_CHANNEL_HOST
    ...
    def get_num_new_matched_tokens(
        ...
        logger.debug(
            "P2PXferConnector get_num_new_matched_tokens: "
    ...
    def update_state_after_alloc(
        ...
        params = request.kv_transfer_params
        logger.debug(
            "P2PXferConnector update_state_after_alloc: "
        ...
        if self.use_host_buffer and params.get("do_remote_decode"):
            # NOTE: when accelerator is not directly supported by comm backend,
    ...
    def request_finished(
        ...
        params = request.kv_transfer_params
        logger.debug(
            "P2PXferConnector request_finished(%s), request_status=%s, "
        ...
        if delay_free_blocks:
            ...
            logger.debug(
                "P2PXferConnector request_finished(%s) waiting for %d seconds "
                "for remote decode to fetch blocks",
                request.request_id,
                self.abort_request_timeout,
            )
            self._reqs_need_send[request.request_id] = (
                time.perf_counter() + self.abort_request_timeout
            )
...
class P2PXferConnectorWorker:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str):
        ...
        if vllm_config.kv_transfer_config is None:
            raise ValueError("kv_transfer_config must be set for P2PXferConnector")
        ...
        # KV Caches and tracking data.
        self.device_type = current_platform.device_type
        self.kv_buffer_device: str = vllm_config.kv_transfer_config.kv_buffer_device
        self.device_kv_caches: dict[str, torch.Tensor] = {}

        # cpu kv buffer for xfer
        # used when device memory can not be registered with the comm backend
        ...
        # Number of regions. Currently one region per cache
        ...
        # invalid blocks from failed transfer operations
        self._invalid_block_ids: set[int] = set()
        # requests that skipped transfer (handshake or transfer failures)
        self._failed_recv_reqs: set[ReqId] = set()

        # Handshake metadata of this worker for transfers.
        self.xfer_handshake_metadata: P2PXferAgentMetadata | None = None

        # Background thread for initializing new P2PXFER handshakes.
        self._handshake_initiation_executor = ThreadPoolExecutor(
            # NIXL is not guaranteed to be thread-safe, limit 1 worker.
            max_workers=1,
            thread_name_prefix="vllm-p2p-xfer-handshake-initiator"
        )
    ...
    def _p2pxfer_handshake(
        ...
        """Do a P2PXfer handshake with a remote instance."""
        ...
        with zmq_ctx(zmq.REQ, path) as sock:
            ...
            decoder = msgspec.msgpack.Decoder(P2PXferAgentMetadata)
            metadata = decoder.decode(metadata_bytes)
            got_metadata_time = time.perf_counter()
            logger.debug(
                "P2PXFER handshake: get metadata took: %s", got_metadata_time - start_time
            )

            # Ensure engine id matches.
            if metadata.engine_id != expected_engine_id:
                raise RuntimeError(
                    f"Remote P2PXFER agent engine ID mismatch. "
            ...
            logger.debug(
                "P2PXFER handshake: add agent took: %s",
    ...
    def initialize_host_xfer_buffer(self, kv_caches: dict[str, torch.Tensor]) -> None:
        ...
        NOT directly supported by transfer backend (e.g., tpu)
        """
        ...
        try:
            ...
            logger.error("P2PXferConnectorWorker gets %s.", e)
            raise
    ...
    def _background_p2pxfer_handshake(
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        """Register the KV Cache data in p2p xfer backend."""
        ...
        # Disadvantage is that the encoded P2PXferAgentMetadata is now larger
        # (roughly 8KB vs 5KB).
        ...
        self.xfer_impl.register_memory(caches_data)
        ...
        # Register local/src descr for xfer backend.
        blocks_data = []
        for i, base_addr in enumerate(seen_base_addresses):
            kv_block_len = self.get_backend_aware_kv_block_len(layer_idx=i)
            # NOTE With heter-TP, more blocks are prepared than what are
            # needed as self.num_blocks >= xfer_agent_meta.num_blocks. We
        ...
        self.xfer_impl.register_local_blocks(blocks_data)

        # After KV Caches registered, listen for new connections.
        self.xfer_handshake_metadata = P2PXferAgentMetadata(
            engine_id=self.engine_id,
            agent_metadata=self.xfer_impl.get_agent_metadata(),
    ...
    def add_remote_agent(
        self,
        xfer_agent_meta: P2PXferAgentMetadata,
        remote_tp_rank: int = 0,
        remote_tp_size: int = 1,
    ) -> str:
        """
        Add the remote P2P Xfer agent and prepare the descriptors for reading cache
        blocks from remote.
        ...
        """  # noqa: E501
        engine_id = xfer_agent_meta.engine_id
        ...
        remote_agent_name = self.xfer_impl.add_remote_agent(
            xfer_agent_meta.agent_metadata
        )
        ...
        if engine_id not in self.dst_num_blocks:
            self.dst_num_blocks[engine_id] = xfer_agent_meta.num_blocks

        # Keep track of remote agent kv caches base addresses.
        self.kv_caches_base_addr[engine_id] = xfer_agent_meta.kv_caches_base_addr

        self._validate_remote_agent_handshake(xfer_agent_meta, remote_tp_size)
        ...
        for i, base_addr in enumerate(xfer_agent_meta.kv_caches_base_addr):
            kv_block_len = self.get_backend_aware_kv_block_len(layer_idx=i)
            rank_offset = (
                self.tp_rank % tp_ratio * kv_block_len if not replicates_kv_cache else 0
            )
            for block_id in range(xfer_agent_meta.num_blocks):
                block_offset = block_id * xfer_agent_meta.block_lens[i]
                # For each block, grab the heads chunk belonging to rank_i
                # of size remote_nheads // tp_ratio, which correspond to
                # self.block_len == remote_block_len//tp_ratio bytes.
                addr = base_addr + block_offset + rank_offset
                # (addr, len, device id)
                blocks_data.append((addr, kv_block_len, xfer_agent_meta.device_id))
            ...
            if self.kv_topo.is_kv_layout_blocks_first:
                ...
                for block_id in range(xfer_agent_meta.num_blocks):
                    block_offset = block_id * xfer_agent_meta.block_lens[i]
                    addr = base_addr + block_offset + rank_offset
                    v_addr = addr + xfer_agent_meta.block_lens[i] // 2
                    blocks_data.append(
                        (v_addr, kv_block_len, xfer_agent_meta.device_id)
                    )
        ...
        self.xfer_impl.register_xfer_list(engine_id, remote_agent_name, blocks_data)

        return remote_agent_name

    def _validate_remote_agent_handshake(
        self, p2pxfer_agent_meta: P2PXferAgentMetadata, remote_tp_size: int
    ):
        """
        Validate the remote agent handshake metadata ensuring the
        invariants hold true.
        """
        remote_engine_id = p2pxfer_agent_meta.engine_id

        assert self._tp_size[remote_engine_id] == remote_tp_size
        # TODO We may eventually want to skip enforcing the same attn backend.
        assert p2pxfer_agent_meta.attn_backend_name == self.backend_name
        ...
        if not self.use_mla and p2pxfer_agent_meta.kv_cache_layout != kv_cache_layout:
            if (
                self.kv_transfer_config.enable_permute_local_kv
                and p2pxfer_agent_meta.kv_cache_layout == "HND"
            ):
                logger.info(
        ...
        assert self.dst_num_blocks[remote_engine_id] == p2pxfer_agent_meta.num_blocks

        assert len(p2pxfer_agent_meta.kv_caches_base_addr) == len(self.block_len_per_layer)
    ...
    def save_kv_to_host(self, metadata: P2PXferConnectorMetadata):
    ...
    def _get_new_notifs(self) -> set[str]:
        ...
        for notifs in self.xfer_impl.get_new_notifs().values():
            for notif in notifs:
    ...
    def _pop_done_transfers(
        ...
        for req_id, handles in list(transfers.items()):
            ...
            for handle, _xfer_stime in handles:
                xfer_state = self.xfer_impl.check_xfer_state(handle)
                if xfer_state == "DONE":
                    # Get telemetry from transfer backend
                    if self.xfer_stats is not None:
                        res = self.xfer_impl.get_xfer_telemetry(handle)
                        self.xfer_stats.record_transfer(res)
                    self.xfer_impl.release_xfer_handle(handle)
                    ...
                    logger.error(
                        "P2PXFER transfer failed for request %s with state %s. "
    ...
    def start_load_kv(self, metadata: P2PXferConnectorMetadata):
        """
        Start loading by triggering non-blocking xfer.
        ...
        for req_id, meta in metadata.reqs_to_recv.items():
            ...
            if remote_engine_id not in self._remote_agents:
                ...
                with self._handshake_lock:
                    if remote_engine_id not in self._remote_agents:
                        self._background_p2pxfer_handshake(req_id, remote_engine_id, meta)
                        continue
    ...
    def _read_blocks(
        ...
        if num_local_blocks == 0:
            ...
            try:
                self.xfer_impl.send_notif(agent_name, notif_msg=notif_id)
            except Exception:
                logger.exception(
                    "P2PXFER send_notif failed for request %s: "
        ...
        # Prepare transfer with P2PXFER.
        handle = None
        try:
            handle = self.xfer_impl.read_kv_blocks(
                dst_engine_id,
            ...
            logger.exception(
                "P2PXFER transfer setup/initiation failed for request %s. "
                "Marking blocks as invalid.",
                request_id,
            )
            # mark all blocks for this request as invalid
    ...
    def get_kv_connector_stats(self) -> KVConnectorStats | None:
        ...
        if self.xfer_stats and not self.xfer_stats.is_empty():
    ...
    def get_block_ids_with_load_errors(self) -> set[int]:
        ...
        to be retried after a transfer backend failure.
        """
    ...
    def shutdown(self):
        ...
        self.xfer_impl.shutdown(self._remote_agents)
