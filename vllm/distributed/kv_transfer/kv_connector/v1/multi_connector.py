...
class MultiConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def __init__(
        ...
        self._failed_async_loads: set[str] = set()
    ...
    def get_transfer_results(
        ...
        if self._range_load and self._async_load_sources:
            for req_id in finished_req_ids:
                ...
                self._failed_async_loads.discard(req_id)
            ...
            if not self._range_load:
                ...
                for req_id in child_results.finished_recving:
                    ...
                    if sources is not None and pending is None:
                        continue
                    if req_id in child_results.failed_recving:
                        self._failed_async_loads.add(req_id)
                    ...
                    if req_id in self._failed_async_loads:
                        self._failed_async_loads.remove(req_id)
