...
class RegisterEngineCorePayload(BaseModel):
    engine_id: EngineId
    dp_rank: int
    addr: EngineCoreAddr
...
class EngineEntry:
    engine_id: EngineId
    worker_addr: dict[int, dict[int, WorkerAddr]] # {tp_rank: {pp_rank: worker_addr}}
    engine_addr: str
...
class MooncakeBootstrapServer:
    ...
    def _register_routes(self):
        ...
        self.app.post("/register_engine")(self.register_engine_core)
    ...
    async def register_engine_core(self, payload: RegisterEngineCorePayload):
        ...
        async with self._lock:
            if payload.dp_rank not in self.workers:
                ...
                if self.workers[payload.dp_rank].engine_addr is not None:
                    raise HTTPException(
                        status_code=400,
                        detail=(
                            f"Worker with dp_rank={payload.dp_rank}, "
                            f"is already registered at "
                            f"{self.workers[payload.dp_rank]}, "
                            f"but still want to register at {payload.addr}"
                        ),
                    )

            dp_entry = self.workers[payload.dp_rank]
            dp_entry.engine_addr = payload.addr

            logger.info(
                "Registered worker: engine_id=%s, dp_rank=%d at %s",
                payload.engine_id,
                payload.dp_rank,
                payload.addr,
            )
