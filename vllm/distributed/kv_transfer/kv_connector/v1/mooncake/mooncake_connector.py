...
class MooncakeConnectorWorker:
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        for layer_name, cache_or_caches in kv_caches.items():
            ...
            if layer_spec is None:
                ...
                continue
            # K/V (and Mamba conv/ssm state) are packed into one blocks-first
            ...
            for cache in cache_list:
                ...
                # Each entry is (state byte offset within the block, real
                # payload bytes per block). The region expander may still
                # sub-split attention entries into K/V halves.
                if isinstance(layer_spec, (MLAAttentionSpec, SlidingWindowMLASpec)):
                    region_entries = [(0, layer_spec.page_size_bytes)]
                elif isinstance(layer_spec, MambaSpec):
                    if self.transfer_topo.virtually_split_kv_in_blocks:
                        # The block page packs each state contiguously before
                        # any padding. Register every state with its real byte
                        # length so heterogeneous-TP slicing stays head-aligned
                        # instead of cutting across padding.
                        conv_bytes = prod(layer_spec.shapes[0]) * get_dtype_size(
                            layer_spec.dtypes[0]
                        )
                        if is_conv_state_dim_first():
                            # The conv state concatenates sub-projections
                            # (GDN: Q, K, V) along the dim axis, and each
                            # sub-projection shards across TP independently.
                            # Register one region per sub-projection so the
                            # per-region heterogeneous-TP split never cuts
                            # across a projection boundary. Only the DS
                            # (dim, state_len) layout keeps each
                            # sub-projection contiguous.
                            split = derive_mamba_conv_split(layer_spec, self.tp_size)
                            region_entries = [
                                (offset, size)
                                for offset, size in split.local_conv_offsets
                            ]
                        else:
                        ...
                        for shape, dtype in zip(
                            layer_spec.shapes[1:], layer_spec.dtypes[1:]
                        ):
                            state_bytes = prod(shape) * get_dtype_size(dtype)
                            region_entries.append((state_offset, state_bytes))
                            state_offset += state_bytes
                    else:
                        region_entries = [(0, block_len)]
                elif self.transfer_topo.virtually_split_kv_in_blocks:
                    region_entries = [(0, block_len // 2)]
                else:
                    region_entries = [(0, block_len)]
    ...
    def _get_transfer_regions(
        ...
        if self.transfer_topo.virtually_split_kv_in_blocks:
            # Remote (peer PP stage) layers may be absent from the local spec
            # table; their split flag is unused because region alignment only
            # matches locally registered layers.
            split_kv_regions = [
                not isinstance(
                    self._layer_specs.get(layer_name),
