...
class MooncakeLayerwiseXferMetadata(msgspec.Struct, omit_defaults=True, dict=True):
    # agent_info: {engine_id: {tp: (worker_addr, rpc_port, kv_cache_base_addr)}}
    # req_blocks: dict[ReqId, tuple[TransferId, list[int]]]
    agent_info: dict[EngineId, dict[int, tuple[str, int, list[int]]]]
    req_blocks: dict[ReqId, tuple[TransferId, list[int]]]
...
class MooncakeConnectorScheduler:
    ...
    def __init__(self, vllm_config: VllmConfig, engine_id: str):
        ...
        self.decode_ready_req = dict() # req_iq
        self.agent_meta_info = dict() # {engine_id : agent_meta}
        self.agent_meta_queue = asyncio.Queue[bytes]()
        if self.is_kv_producer:
            self.sender_loop = asyncio.new_event_loop()
            # self.sender_loop.set_exception_handler(handle_asyncio_exception)
            ...
            if self.is_kv_producer:
                print(f"Event loop running: {self.sender_loop.is_running()}")
    ...
    async def _mooncake_sender_listener(self, ready_event: threading.Event):
        ...
        try:
            while True:
                ...
                # logger.info(f"[+++] engine core recv {frames}")
                identity, metadata_bytes = await sock.recv_multipart()
                sock.send_multipart((identity, b"", b"ACK"), flags=zmq.NOBLOCK)
                logger.info(f"[+++] engine core recv {metadata_bytes}")
                await self.agent_meta_queue.put(metadata_bytes)
        ...
        while True:
            try:
                async with httpx.AsyncClient() as client:
                    # response = await client.post(url, json=payload.model_dump(), timeout=httpx.Timeout(0.5))
    ...
    def get_num_new_matched_tokens(
        ...
        if not params:
            return 0, False
        while not self.agent_meta_queue.empty():
            agent_meta = self.agent_meta_queue.get_nowait()
            xfermetadata_decoder = msgspec.msgpack.Decoder(MooncakeLayerwiseXferMetadata)
            agent_info = xfermetadata_decoder.decode(agent_meta)
            logger.info(f"[===] get_num_new_matched_tokens recv {agent_info}")
            for req_id, (transfer_id, block_ids) in agent_info.req_blocks.items():
                self.decode_ready_req[transfer_id] = block_ids

            for engine_id, agent in agent_info.agent_info.items():
                self.agent_meta_info[engine_id] = agent
        ...
        if self.use_layerwise:
            ...
            if params.get("transfer_id") in self.decode_ready_req:
                self.decode_ready_req.pop(params.get("transfer_id"))  # TODO 要从这里获取出remote block_ids
                logger.info(f"MooncakeConnector get_num_new_matched_tokens: return (0, False) for layerwise, {request.request_id}")
                return 0, False
            return None, False
        else:
            return 0, False

    def handle_request(self, request_id: str, remote_bootstrap_addr: str, params: dict, block_ids: list[int]):
        """
        协程：负责与远程 P 节点进行控制面交互
        1. 查询 remote P 的 bootstrap server，获取 worker addr
        2. 推送本地的 agent metadata 和请求信息给 P 节点
        """
        ...
        remote_engine_addr = None
        import requests
        try:
            response = requests.get(f"{remote_bootstrap_addr}/query", timeout=5)
            response.raise_for_status()
            data: dict = response.json()
            
            for engine_id, dp_entry in data.items():
                found_engine_id = dp_entry.get("engine_id")
                engine_addr = dp_entry.get("engine_addr")  # 假设有 worker_addr 字段
                if remote_engine_id == found_engine_id:
                    remote_engine_addr = engine_addr
                    logger.info(f"[+++] engine_id={found_engine_id}, worker_addr={remote_engine_addr}, dp_entry={dp_entry}")
                    break
            ...
            # ================= 步骤 2：推送 metadata 到远程 P 节点 =================
            decoder = msgspec.msgpack.Decoder(MooncakeLayerwiseXferMetadata)
            # 构造要推送的数据
            push_data = MooncakeLayerwiseXferMetadata(
                agent_info=self.agent_info,
                req_blocks={request_id:(transfer_id, block_ids)}
            )
            
            logger.info(f"[===] {push_data=}")
            path = remote_engine_addr
            encoder = msgspec.msgpack.Encoder()
            encoded_data = encoder.encode(push_data)
            decode_data = decoder.decode(encoded_data)
            with zmq_ctx(zmq.DEALER, path) as sock:  # type: ignore
                # sock.connect(path)
                logger.info(f"[===] {decode_data=}")
                logger.info(f"[===] D node send agent data to {path=} , {encoded_data}")
                sock.send_multipart([encoded_data])
                sock.recv()
                # If something goes wrong, let P wait timeout first (in asyncio.wait()).
        except zmq.ContextTerminated:
            logger.debug("ZMQ context terminated, exiting Mooncake receiver thread.")
        except Exception as e:
            logger.error(f"MooncakeXferMetadata transfer failed for {e}")
            return
    ...
    def build_connector_meta(
        ...
        if not self.is_kv_consumer:
            ...
            # while not self.agent_meta_queue.empty():
            #     agent_meta = self.agent_meta_queue.get_nowait()
            #     meta.agents_meta.add(agent_meta)
    ...
    def request_finished(
        ...
        if delay_free_blocks:
            print(f"[===] req id do prefill done {request.request_id}")
    ...
    def set_xfer_handshake_metadata(
        ...
        # handshake_payload = handshake_decoder.decode(encoded_data[0])
        ...
        # print(f"[===] {metadata=}")
        # print(f"[===] {self._encoded_xfer_handshake_metadata=}")
        import copy
        ...
        for i in range(len(encoded_data_copy)):
            ...
            agent_mata = (metadata.worker_addr, metadata.rpc_port, metadata.kv_caches_base_addr)
...
class MooncakeConnectorWorker:
    ...
    async def send_kv_to_decode(
        ...
        while wait_tasks:
            ...
            for task in done:
                ...
                logger.info(f"[===] P add req_id in d node ready {d_req_id}")
    ...
    async def _build_transfer_params(
        ...
        for d_req_id, send_meta in ready_reqs:
            ...
            logger.info(f"[===] {d_req_id=} {local_block_ids=} {remote_block_ids=}")
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        # TODO 2. move sender listen to shchedule connector，listen for req block_ids from decoder shchedule connector
        if self.is_kv_producer:
            ready_event = threading.Event()
            asyncio.run_coroutine_threadsafe(
                self._mooncake_sender_listener(ready_event), self.sender_loop
        ...
        agent_metadata = MooncakeAgentMetadata(
            engine_id=self.engine_id,
            worker_addr=f"{self.hostname}:{self.side_channel_port}",
