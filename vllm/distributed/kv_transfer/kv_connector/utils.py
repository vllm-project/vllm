...
class KVOutputAggregator:
    ...
    def aggregate(self,
        ...
        def update_finished_set(req_ids: Optional[set[str]],
                                ...
                                kv_transfer_stats: Optional[dict[KVConnectorType, Optional[KVTransferStats]]],
                                aggregated_kv_transfer_stats: dict[KVConnectorType, Optional[KVTransferStats]]) -> None:
            ...
            if kv_transfer_stats is not None:
                for connector_id, xfer_stats in kv_transfer_stats.items():
                    if connector_id not in aggregated_kv_transfer_stats:
                        ...
                        aggregated_kv_transfer_stats[connector_id].aggregate(xfer_stats)

        finished_sending = set[str]()
        finished_recving = set[str]()
        aggregated_kv_transfer_stats: dict[KVConnectorType, Optional[KVTransferStats]] = {}
        for output in outputs:
            output = output.kv_connector_output
            if not output:
                continue
            update_finished_set(output.finished_sending,
                                self._send_remaining_count, finished_sending, output.kv_transfer_stats, aggregated_kv_transfer_stats)
            update_finished_set(output.finished_recving,
                                self._recv_remaining_count, finished_recving, output.kv_transfer_stats, aggregated_kv_transfer_stats)
