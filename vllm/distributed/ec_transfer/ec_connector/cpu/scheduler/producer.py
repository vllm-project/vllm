...
"""Producer role for ECCPUConnector — encoding pipeline only.

The XferReq serving, pin management, and NIXL notification handling that
previously lived here now live in ProducerSession (session.py). ECCPUProducer
handles only the encoding side: reserving space for new encodings, allocating
blocks, promoting them to the local cache, and announcing them to consumers.
"""
...
class ECCPUProducer:
    """Encoding pipeline for the producer side.

    Runs on the main (scheduler) thread. Shared state (_local_encodings,
    _blocks) is accessed under _lock; ProducerSession reads the same dicts
    ...
    def ensure_cache_available(
        self, request: "Request", num_computed_tokens: int, first_in_batch: bool = False
    ) -> bool:
        """Return False if the region cannot accommodate this request's saves.

        Performs a speculative alloc-then-free for each unseen feature to check
        whether there is room. The actual allocation happens in build_saves().
        """
    ...
    def request_finished(self, request: "Request") -> dict[str, Any] | None:
        """Return the transfer params the consumer needs to open an XferReq."""
    ...
    def build_saves(self) -> dict[str, list[int]]:
        """Advance encode pipeline. Returns {mm_hash: block_indices} for worker."""
    ...
    def shutdown(self) -> None:
        pass  # Pin cleanup is handled by ProducerSession.stop().
