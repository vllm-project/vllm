...
class TensorMemoryPool:
    ...
    def _split_block(self, block: MemoryBlock, required_size: int):
        while block.size > required_size and block.size // 2 \
                >= self.min_block_size:
    ...
    def store_tensor(self, tensor: torch.Tensor) -> int:
        """Stores a HBM tensor in pinned host memory.

        Args:
            tensor (torch.Tensor): HBM tensor to store

        Returns:
            int: Address where the tensor is stored

        Raises:
            ValueError: If tensor is not on HBM or allocation fails
        """
