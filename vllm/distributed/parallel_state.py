...
def initialize_model_parallel(
    ...
    # world, PP, DP, DCP, EP, and other groups on the stock path. Read the
    # flag defensively so partial configs (e.g. in unit tests) keep working.
    compilation_config = getattr(config, "compilation_config", None)
    enable_eager_tp_all_reduce = (
        compilation_config is not None and compilation_config.enable_eager_tp_all_reduce
    )
    ...
    # Engram reduces slices of a persistent prefetch buffer and relies on the
    # functional out-of-place contract, so an Engram model must not alias the
    # eager in-place TP group; give it its own coordinator in that case.
    if engram_tensor_parallel_size == tensor_model_parallel_size and not (
        enable_eager_tp_all_reduce and config.engram_config is not None
    ):
        _ETP = _TP
    else:
        group_ranks = (
