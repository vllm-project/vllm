...
class GroupCoordinator:
    ...
    def isend_tensor_dict(
        ...
        In async mode the returned handles are additionally self-retained in
        ...
        if is_async:
            ...
            # Synchronous path: block on metadata send before posting
            # tensors. No self-retention is needed because ``send_object``
            # has returned by the time we exit, and the caller is expected
            # to ``wait()`` the tensor handles before the buffers can be
            # reused.
