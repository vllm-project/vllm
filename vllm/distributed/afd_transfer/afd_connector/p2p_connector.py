...
class P2PAFDConnector(AFDConnectorBase):
    ...
    def update_state_from_dp_metadata(
        ...
        # Pre-allocate fixed recv buffers for each A in each stage.
        # For asymmetric A/F: Each A has its own buffer to avoid data overwrite.
        # Buffer key: (stage_idx, src_rank, tuple(size))
        if self.config.afd_config.afd_role == "ffn" and not self.config.model_config.enforce_eager:
            for stage_idx in range(num_of_stages):
                ...
                # For each A (src_rank from 1 to group_size-1), allocate a buffer
                for src_rank in range(1, self.group_size):
                    buffer_key = (stage_idx, src_rank, tuple(meta.size))
    ...
    def send_ffn_output(
        ...
        For asymmetric A/F: F (rank 0) slices the output tensor and sends
        each slice to the corresponding A (ranks 1 to ratio).
        The tensor is split evenly along dimension 0.
        """
        total_tokens = hidden_states.shape[0]

        if self.ratio == 1:
            # Symmetric case: send full tensor to the single A
            self._send_hidden_states(hidden_states, 1, self.e2a_group)
        else:
    ...
    def recv_attn_output(
        ...
        Each A has its own pre-allocated buffer to avoid data overwrite.
        """
        hidden_states_list = []
        ...
        for src in range(1, self.group_size):
            ...
            if not self.config.model_config.enforce_eager:
                # Each A has its own buffer with unique key (stage_idx, src_rank, size)
                buffer_key = (ubatch_idx, src, tuple(meta.size))
