...
def determine_default_log2phy_map(global_expert_num, world_size, rank_id):
    """
    Determines a default logical-to-physical expert mapping for a specific rank.
    This function sets up an initial, balanced distribution where experts are
    partitioned across ranks, and then uses `generate_log2phy_map` to create
    the final mapping, including replication for unassigned experts.
