...
@njit
def compute_piece_counts(X: np.ndarray, P: int, stage_weights: np.ndarray):
    """
...
@njit
def lpt_placement(X: np.ndarray, pieces: np.ndarray, M: int, stage_weights: np.ndarray):
    """
...
@njit
def slice_values(X: np.ndarray, pieces: np.ndarray):
    """
...
@njit
def auto_fix_new_placement(
    old_placement: np.ndarray, new_placement: np.ndarray
) -> np.ndarray:
    """
...
@njit
def compute_objective(deployment: np.ndarray, X: np.ndarray, pieces: np.ndarray):
    """
...
class FlashlbEplbPolicy(AbstractEplbPolicy):
    ...
    def need_update(self, current_par: float, layer_id: int = 0):
        """
    ...
    def compute_stage_weight(self, hotness: np.ndarray):
        """
    ...
    def register_hotness(self, num_layer: int, hotness: np.ndarray):
        """
    ...
    def compress_by_avg_pooling_fast_nd(self, arr: np.ndarray, m: int):
        """
    ...
    def rebalance_experts(
        ...
        for i, layer in enumerate(layers_need_update):
            ...
            if old_global_expert_indices is None:
                ...
                new_deployment[layer], expert_count[layer] = (
                    self.group_based_adaptive_searching(
                        hotness, num_replicas, num_ranks, stage_weights, recorsive=False
                    )
                )
            else:
                (
        ...
        if old_global_expert_indices is None:
            physical_to_logical_map = np.array(new_deployment, dtype=np.int32).reshape(
                (num_layer, num_replicas)
            )
            maxlogcnt = int(expert_count.max())
            logical_to_physical_map = compute_logical_to_physical_map(
                physical_to_logical_map, num_layer, num_expert, num_replicas, maxlogcnt
            )
            physical_to_logical_map = torch.tensor(
                physical_to_logical_map, dtype=torch.int32, device=weight.device
            )
            logical_to_physical_map = torch.tensor(
                logical_to_physical_map, dtype=torch.int32, device=weight.device
            )
            expert_count = torch.tensor(
                expert_count, dtype=torch.int32, device=weight.device
            )
        ...
        logical_to_physical_ndarray = compute_logical_to_physical_map(
            physical_to_logical_ndarray, num_layer, num_expert, num_replicas, maxlogcnt
        )
        ...
        logical_to_physical_map = torch.tensor(
            logical_to_physical_ndarray, dtype=torch.int32, device=weight.device
        )
        expert_count_tensor = torch.tensor(
            expert_count, dtype=torch.int32, device=weight.device
        )
        return (physical_to_logical_map, logical_to_physical_map, expert_count_tensor)
