...
class EplbPolicyFactory:
    ...
    @classmethod
    def register_policy(cls, policy_name: EPLBPolicyOption, policy_cls: Type[AbstractEplbPolicy]):
        """
        ...
        if not issubclass(policy_cls, AbstractEplbPolicy):
            raise TypeError(f"Policy class {policy_cls.__name__} must inherit from AbstractEplbPolicy")
        cls._POLICY_REGISTRY[policy_name] = policy_cls

    @classmethod
    def create_policy(cls, policy_type: EPLBPolicyOption = "default") -> AbstractEplbPolicy:
        """
