...
def _allocate_peer_group_buffers(
    ...
    per_peer_target_bytes = target_total_bytes // (2 * num_peers)
    ...
    if max_group_layers <= 0:
        raise ValueError(
            "Not enough free memory to allocate per-peer send/recv buffers for EPLB. "
            f"layer_bytes: {layer_bytes}, max_group_layers: {max_group_layers}, "
            f"free_bytes: {free_bytes}, target_total_bytes: {target_total_bytes}"
        )
...
def shuffle_layer(
    ...
    for peer_rank, experts in sorted(recv_peers.items()):
        ...
        if not experts_sorted:
            continue
        pre_buf = peer_recv_buffers[peer_rank]
        need_rows = len(experts_sorted)
        if need_rows > pre_buf.shape[0]:
            logger.debug(
                "EPLB recv: peer %d needs %d rows > capacity %d; using temp buffer",
                peer_rank,
                need_rows,
                pre_buf.shape[0],
            )
    ...
    for peer_rank, experts in sorted(send_peers.items()):
        ...
        if not experts_sorted:
            continue
        pre_buf = peer_send_buffers[peer_rank]
        need_rows = len(experts_sorted)
        if need_rows > pre_buf.shape[0]:
            logger.debug(
                "EPLB send: peer %d needs %d rows > capacity %d; using temp buffer",
                peer_rank,
                need_rows,
                pre_buf.shape[0],
            )
    ...
    # 5. Unpack received buffers directly into expert weights
    ...
    # 6. Copy the weights from the local buffer back to the original weights.
    for dst in range(num_local_experts):
        if is_unchanged[dst]:
            continue
        if is_received_locally[dst]:
