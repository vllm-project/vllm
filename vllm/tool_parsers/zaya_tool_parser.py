...
class StreamingXMLToolCallParser:
    ...
    def __init__(self):
        ...
        self.tool_call_start_token: str = '<zyphra_tool_call>'
        self.tool_call_end_token: str = '</zyphra_tool_call>'
    ...
    def parse_single_streaming_chunks(self, xml_chunk: str) -> DeltaMessage:
        ...
        if found_elements:
            ...
            try:
                ...
                # If this chunk contains </zyphra_tool_call>
                ...
                if (self.current_call_id is not None
                    ...
                    if not has_toolcall_close:
                        ...
                        if self.current_function_name:
                            self._end_element('function')
                        self._end_element('zyphra_tool_call')
            ...
            # in multi <zyphra_tool_call> scenarios
            if (self.current_call_id is not None
                    and (self.function_end_token in xml_chunk
    ...
    def _process_complete_xml_elements(self) -> bool:
        ...
        while self.last_processed_pos < len(self.streaming_buffer):
            ...
            try:
                ...
                if ((preprocessed_element.strip().startswith('<zyphra_tool_call>') or
                ...
                if (preprocessed_element.strip().startswith('<zyphra_tool_call>')
    ...
    def _find_next_complete_element(
        ...
        if buffer.startswith('<'):
            ...
            if tag_end != -1 and tag_end2 != -1:
                ...
                # check if starts with <zyphra_tool_call>
                if self.current_call_id is None:
                    # Check if might be start of <zyphra_tool_call>
                    if buffer == '<zyphra_tool_call>'[:len(buffer)]:
                        # Might be start of <zyphra_tool_call>, wait for more data
                        return None, start_pos
                    else:
                        # Not start of <zyphra_tool_call>, treat as text
    ...
    def _auto_close_open_parameter_if_needed(self,
        ...
        if there are unclosed tool_calls, complete `</zyphra_tool_call>`.
        """
        ...
        if incoming_tag in ('function',
                            'zyphra_tool_call') and self.current_function_name:
            self._end_element('function')

        # If about to start new tool_call,
        # and there are unclosed tool_calls, close tool_call first
        if incoming_tag == 'zyphra_tool_call' and self.current_call_id:
            self._end_element('zyphra_tool_call')
    ...
    def _start_element(self, name: str, attrs: dict[str, str]):
        ...
        if name == 'zyphra_tool_call':
            # Before opening new tool_call,
            # automatically complete previous unclosed tags
            self._auto_close_open_parameter_if_needed('zyphra_tool_call')
            ...
            if not self.current_call_id:
                self._start_element('zyphra_tool_call', {})
    ...
    def _end_element(self, name: str):
        ...
        if (name.startswith('function') or name == 'function'
                or name == 'zyphra_tool_call') and self.current_param_name:
