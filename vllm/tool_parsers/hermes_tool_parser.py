...
class Hermes2ProToolParser(ToolParser):
    def __init__(self, tokenizer: TokenizerLike):
        ...
        # Cache token IDs per tokenizer so tokenizer.encode() is called
        # only once per model. A new Hermes2ProToolParser is created per
        # tool-call request (_preprocess_chat → tool_parser(tokenizer)),
        # and under high concurrency the repeated encode() calls race on
        # the HF fast tokenizer's internal Rust RefCell
        # (set_truncation_and_padding → no_truncation), causing
        # "RuntimeError: Already borrowed".
        # The cache is keyed by tokenizer identity so different models
        # get their own entries. Double-checked locking: the outer check
        # is the fast path; the lock serializes only the first call per
        # tokenizer.
            ...
            with _hermes_cache_lock:
                if tok_key not in cls._token_cache:
                    ...
                    cls._token_cache[tok_key] = {
                        ...
                        "end_ids": end_ids,
                        "start_array": [
                            self.model_tokenizer.decode([tid]) for tid in start_ids
                        ],
                        "end_array": [
                            self.model_tokenizer.decode([tid]) for tid in end_ids
                        ],
                    }
