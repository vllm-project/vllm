...
def _tool_parameters(
    tool: ChatCompletionToolsParam | Mapping[str, Any],
) -> Mapping[str, Any]:
    function = _tool_function(tool)
...
class XingChen4ToolParser(ToolParser):
    ...
    def extract_tool_calls(
        ...
        try:
            for match in TOOL_CALL_REGEX.finditer(model_output):
                tool_name, arguments = _parse_payload(match.group(1), request.tools)
                if not tool_name:
                    continue
    ...
    def extract_tool_calls_streaming(
        ...
        while True:
            ...
            if start_idx == -1:
                partial_len = _partial_suffix_len(self._buffer, self.tool_start_token)
                if partial_len:
            ...
            if not extracted_tool_calls.tool_calls:
                logger.warning("Failed to extract any tool calls from %r.", tool_text)
                content += tool_text
            else:
