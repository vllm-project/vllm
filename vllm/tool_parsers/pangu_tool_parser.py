...
class PanguToolParser(ToolParser):
    def __init__(self, tokenizer: TokenizerLike):
        ...
        self.tool_call_start_token: str = "<|tool_call_start|>" if self.vocab.get("<|tool_call_start|>") else "[unused11]"
    ...
    def extract_tool_calls(
        ...
        if not (self.tool_call_start_token in model_output and self.tool_call_end_token in model_output):
    ...
    def send_current_tool_name_or_none(self, current_tool_call):
        ...
        cur_args_json = json.dumps(cur_arguments, ensure_ascii=False)
        argument_diff = None
        if self.is_json_complete and cur_args_json and not self.streamed_args_for_tool[-1]:
            ...
            delta = DeltaMessage(
                tool_calls=[
                    DeltaToolCall(
                        ...
                        type="function",
                        id=make_tool_call_id(),
                        function=DeltaFunctionCall(name=function_name, arguments=argument_diff).model_dump(
                            exclude_none=True
                        ),
                    )
                ]
    ...
    def stream_arguments(self, current_tool_call):
        cur_arguments = current_tool_call.get("arguments")
        delta = None
        if self.is_json_complete and not cur_arguments and not self.streamed_args_for_tool[-1]:
            argument_diff = "{}"
            delta = DeltaMessage(
                tool_calls=[
                    DeltaToolCall(
