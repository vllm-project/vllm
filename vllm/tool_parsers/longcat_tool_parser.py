...
class LongcatFlashToolParser(Hermes2ProToolParser):
    def __init__(self, tokenizer: TokenizerLike):
        ...
        self.tool_call_start_token_ids = self._cached_encode(self.tool_call_start_token)
        self.tool_call_end_token_ids = self._cached_encode(self.tool_call_end_token)

        self.tool_call_start_token_array = [
            self._cached_decode(token_id) for token_id in self.tool_call_start_token_ids
        ]

        self.tool_call_end_token_array = [
            self._cached_decode(token_id) for token_id in self.tool_call_end_token_ids
        ]
