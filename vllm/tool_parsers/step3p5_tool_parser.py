...
class StreamingXMLToolCallParser:
    ...
    def _fix_incomplete_tag_in_chunk(self, chunk: str) -> str:
        """
        Fallback: fix incomplete <parameter=xxx or <function=xxx tags (missing >)
        Examples: <parameter=-C: -> <parameter=-C>, <parameter=parameter=-n: -> <parameter=-n>
        Also handles missing = cases: <function xxx> -> <function=xxx>, <functionxxx> -> <function=xxx>
        ...
        for tag_type in ["parameter", "function"]:
            ...
            if (gt_pos != -1 and (lt_pos == -1 or gt_pos < lt_pos)
            ...
            while tag_name and not (tag_name[-1].isalnum()
                                    or tag_name[-1] in ('-', '_')):
                tag_name = tag_name[:-1]
            ...
            if tag_type == "parameter" and not self._validate_parameter_name(
                    tag_name):
                continue

            # Apply fix
            chunk = chunk.replace(f"<{tag_type}={content[:end_pos]}",
                                  f'<{tag_type}={tag_name}>', 1)
    ...
    def _find_next_complete_element(self,
        ...
        if buffer.startswith("<"):
            ...
            is_incomplete_param = (buffer.startswith("<parameter=")
                                   and ">" not in buffer.split("\n")[0])
            is_incomplete_func = (buffer.startswith("<function=")
                                  and ">" not in buffer.split("\n")[0])
            ...
            if is_incomplete_param or is_incomplete_func:
                ...
                if closing_pos != -1:
                    ...
                    return complete_element, start_pos + closing_pos + len(
                        closing_tag)
    ...
    def _preprocess_xml_chunk(self, chunk: str) -> str:
        ...
        if (chunk.startswith(self.function_start_token)
                or chunk.startswith(self.function_end_token)
                or chunk.startswith("<function ")
                or re.match(r'^<function[a-zA-Z_]',
                            chunk)):  # <functionXXX without space or =
            is_tool_call = True
        if chunk.startswith(self.parameter_start_token) or chunk.startswith(
                self.parameter_end_token):
            is_tool_call = True

        # Fallback: fix incomplete <parameter= or <function= tags without closing >
        # This handles cases like: <parameter=-C:\n or <parameter=-B 5\n
        # Apply when parsing tool calls OR when chunk looks like a function/parameter tag
        if (self.current_call_id is not None or chunk.startswith("<function")
    ...
    def _end_element(self, name: str):
        ...
        if (name.startswith("parameter")
            ...
            # return empty object
                ...
                self._emit_delta(delta)
            self.current_function_open = False
            self.current_function_name = None  # Clear function name to prevent duplicate closing
