...
class MultiModalPlaceholderMap:
    ...
    def from_seq_group(
        ...
        for key, placeholders in seq_group.multi_modal_placeholders.items():
            mm_items = mm_data.pop(key)
            if not isinstance(mm_items, list):
                mm_items = [mm_items]

            if positions:
                intersecting_items = placeholder_maps[
                    key].append_items_from_seq_group(positions, mm_items,
                                                     placeholders)

                if intersecting_items:
                    mm_data[key] = intersecting_items
