...
try:
    ...
    WhisperFeatureExtractor = None  # type: ignore[misc, assignment]


# ============================================================
# Audio Channel Normalization Framework
# ============================================================
...
def get_audio_spec(feature_extractor: object) -> AudioSpec:
    """Detect audio format requirements from a feature extractor.

    This function examines the feature extractor type to determine
    what audio format the model expects.

    Args:
        feature_extractor: A HuggingFace feature extractor instance.

    Returns:
        AudioSpec describing the required audio format.
        Returns PASSTHROUGH_AUDIO_SPEC for unknown extractors.
    """
    if feature_extractor is None:
        return PASSTHROUGH_AUDIO_SPEC

    # WhisperFeatureExtractor requires mono audio
    # Used by: Whisper, Qwen2-Audio, Qwen2.5-Omni, Qwen3-Omni, Ultravox
    if WhisperFeatureExtractor is not None and isinstance(
        feature_extractor, WhisperFeatureExtractor
    ):
        return MONO_AUDIO_SPEC

    # Default: passthrough (no normalization)
    return PASSTHROUGH_AUDIO_SPEC


def get_target_channels(feature_extractor: object) -> int | None:
    """Get the target number of channels for a feature extractor.

    This is a convenience function that returns just the target_channels
    value from get_audio_spec(). Use this when you only need to know
    the channel count, not the full AudioSpec.

    Args:
        feature_extractor: A HuggingFace feature extractor instance.

    Returns:
        Number of target channels, or None for passthrough.
    """
    return get_audio_spec(feature_extractor).target_channels
