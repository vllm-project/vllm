...
class MultiModalProfiler(Generic[_I]):
    ...
    def get_mm_max_tokens(
        ...
        """
        Returns the maximum number of embeddings per item of each modality.

        This excludes any break/text tokens in-between multimodal embeddings/
        encoder outputs. For encoder cache sizing, we only need to track actual
        embeddings, not placeholder tokens.
        """
        if mm_counts is None:
            mm_counts = self.get_mm_limits()
