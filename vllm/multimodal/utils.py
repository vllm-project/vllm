...
def run_dp_sharded_vision_model(image_input: torch.Tensor,
    ...
    """Run a vision model with data parallelism (DP) sharding. The function will 
    shard the input image tensor on the first dimension and run the vision model
    ...
    image_input_per_rank = image_input_padded[rank * num_chunks_per_rank:(rank + 1) *
                                        num_chunks_per_rank, ...].clone()
                                        
    vision_embeddings = vision_model(image_input_per_rank)
    vision_embeddings = tensor_model_parallel_all_gather(
        vision_embeddings, dim=0)
