...
class RayTPUExecutor(TPUExecutor):
    ...
    def _init_workers_ray(self, placement_group: "PlacementGroup", **ray_remote_kwargs):
        ...
        for bundle_id, bundle in enumerate(placement_group.bundle_specs):
            ...
            if "TPU_CHIPS_PER_HOST_BOUNDS" in os.environ:
                override_env.update(
                    {
                        "TPU_CHIPS_PER_HOST_BOUNDS": os.environ[
                            "TPU_CHIPS_PER_HOST_BOUNDS"
                        ]
                    }
                )
            if "TPU_HOST_BOUNDS" in os.environ:
                override_env.update({"TPU_HOST_BOUNDS": os.environ["TPU_HOST_BOUNDS"]})
    ...
    def _run_workers(
        ...
        **kwargs,
        ...
        all_worker_args = (
            repeat(args, count) if all_args is None else islice(all_args, 1, None)
        )
        all_worker_kwargs = (
            repeat(kwargs, count) if all_kwargs is None else islice(all_kwargs, 1, None)
        )
