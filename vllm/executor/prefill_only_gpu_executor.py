...
from vllm.model_executor.prefill_only.execute_io import (ExecuteInput,
                                                         ExecuteOutput)
from vllm.model_executor.prefill_only.workflow import Workflow
from vllm.worker.prefill_only_gpu_worker import WorkerBase, create_worker
...
class GPUAsyncExecutor(GPUExecutor):
    ...
    def __init__(self, engine_config: EngineConfig, workflow: Workflow,
                 attn_backend: PrefillOnlyAttentionBackend, executor_in: Queue,
...
def simple_execute_loop(worker: WorkerBase,
    ...
    def execute_model(executor_input: ExecuteInput) -> ExecuteOutput:
...
def double_buffer_execute_loop(worker: WorkerBase,
    ...
    from vllm.model_executor.prefill_only.engine_io import SchedulerOutput
    ...
    while go_on:
        ...
        if output_to_cpu:
            with torch.cuda.stream(io_stream):
                assert current_task.executor_output is not None
