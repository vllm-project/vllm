...
class RayExecutorBase(ExecutorBase, ABC):
    ...
    def _init_workers_ray(self, placement_group: "PlacementGroup",
        ...
        driver_node_id = ray.get(self.driver_dummy_worker.get_node_id.remote())
        worker_node_ids = ray.get(
            [worker.get_node_id.remote() for worker in self.workers])

        node_workers = defaultdict(list)

        node_workers[driver_node_id].append(0)
        for i, node_id in enumerate(worker_node_ids, start=1):
        ...
        for rank, (worker, node_id) in enumerate(
                zip(self.workers, worker_node_ids),
                start=1,
        ):
            local_rank = node_workers[node_id].index(rank)
            self._init_worker_ray(
                worker,
        ...
        self.driver_worker = self._create_driver_worker(
...
class RayExecutorAsyncBase(RayExecutorBase, ExecutorAsyncBase, ABC):
