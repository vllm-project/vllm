...
class RayDistributedExecutor(DistributedExecutorBase):
    ...
    non_carry_over_env_vars_file = os.path.join(config_home,
    ...
    def _init_executor(self) -> None:
        ...
        if self.use_ray_compiled_dag:
            assert self.use_ray_spmd_worker, ("VLLM_USE_RAY_COMPILED_DAG=1 requires "
                                              "VLLM_USE_RAY_SPMD_WORKER=1")
        if self.use_ray_spmd_worker:
            # TODO: Support SPMD worker for non-DAG Ray executor.
            assert self.use_ray_compiled_dag, ("VLLM_USE_RAY_SPMD_WORKER=1 requires "
    ...
    def _init_workers_ray(self, placement_group: "PlacementGroup", **ray_remote_kwargs):
        ...
        if not self.use_ray_spmd_worker and self.driver_dummy_worker is None:
            raise ValueError("Ray does not allocate any GPUs on the driver node."
        ...
        logger.info("Copying the following environment variables to workers: %s",
