...
from vllm.entrypoints.janitorai.nats.worker import AsyncEngineNatsWorker

logger = init_logger(__name__)


async def serve_http_and_consume(app: FastAPI, engine: AsyncEngineClient,
                     **uvicorn_kwargs: Any):
    ...
    worker = AsyncEngineNatsWorker(engine)
    worker_task = loop.create_task(worker.run())

    def signal_handler() -> None:
        # prevents the uvicorn signal handler to exit early
        # do we need this worker task cancel? 
        worker_task.cancel()
        server_task.cancel()
    ...
    try:
        ...
        # first try to close the job consumer
        logger.info("trying to shut down NATS consumer")
        worker.cleanup()
        logger.info("shut down the NATS consumer")
