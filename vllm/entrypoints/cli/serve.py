...
def run_multi_api_server(args: argparse.Namespace):
    ...
    orig_mm_ipc_cache_gb = args.mm_ipc_cache_gb

    if num_api_servers > 1:
        setup_multiprocess_prometheus()

        # Not compatible with API server scale-out
        args.mm_ipc_cache_gb = 0
    ...
    if num_api_servers > 1:
        ...
        if model_config.is_multimodal_model and orig_mm_ipc_cache_gb > 0:
            logger.warning("Multi-modal IPC cache is disabled because it "
                           "is not compatible with `api_server_count > 1`.")
