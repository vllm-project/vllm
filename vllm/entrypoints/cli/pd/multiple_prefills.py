...
from vllm.entrypoints.cli.pd.base import (
    KV_IP_PLACE_HOLDER,
    NODE_NAME_PLACE_HOLDER,
    PORT_PLACE_HOLDER,
    BasePDJob,
    RankIDGenerator,
    Service,
...
class MultiplePrefillsPDJob(BasePDJob):
    ...
    def _start_single_service(
        ...
        handler = _VllmService.options(  # type: ignore[attr-defined]
            num_cpus=0,
            num_gpus=num_gpus,
            name=name,
    ...
    def _start_prefill_services(self) -> tuple[str | None, list[Service]]:
        """
        ...
        for idx in range(prefill_config.replicas):
            ...
            service = self._start_single_service(
                ...
                ServiceType.prefill,
                runtime_env,
                prefill_config.role_params,
                prefill_config.role_num_gpus,
                prefill_config.unique_role_extra_params,
                prefill_config.envs,
                kv_ip,
            )
            ...
            logger.info(
                "Starting prefill service(rank_id=%s), url: %s",
                service.rank_id,
                service.url,
            )
    ...
    def _start_decode_services(self, kv_ip: str) -> list[Service]:
        ...
        for _ in range(decode_config.replicas):
            ...
            service = self._start_single_service(
                ...
                ServiceType.decode,
                runtime_env,
                decode_config.role_params,
                decode_config.role_num_gpus,
                decode_config.unique_role_extra_params,
                decode_config.envs,
                kv_ip,
            )

            logger.info(
                "Starting decode service(rank_id=%s), url: %s",
                service.rank_id,
                service.url,
            )
            # Start the service asynchronously
    ...
    def start_vllm_services(self) -> tuple[list[Service], list[Service]]:
        ...
        # Ensure kv_ip is available before starting decode services
        assert kv_ip is not None, "KV IP must be available from prefill services"
