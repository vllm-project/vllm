...
class ProxyServer:
    ...
    def _prepare_command(self):
        ...
        - {PORT}: Port number
        - $PREFILL_HOST_{idx}, $PREFILL_PORT_{idx}, $PREFILL_URL_{idx}: Prefill service info
        - $DECODE_HOST_{idx}, $DECODE_PORT_{idx}, $DECODE_URL_{idx}: Decode service info
        ...
        # Replace from high index to low index to ensure no incorrect matches
    ...
    def start(self):
        ...
        # Use shell=True to execute command, capturing stdout and stderr
...
class BasePDJob:
    ...
    def _create_placement_group(self, role_num_gpus: int) -> PlacementGroup:
        ...
        # pg = placement_group([{"CPU": config.gpus_per_worker}] * num_bundles)  # CPU version for debugging
            ...
            if time.time() - start_time > self.config.timeout:
                ...
                for result, service in zip(check_result, services):
                    if isinstance(result, Exception):
                        logger.error(
                            f"The last exception occurred checking health("
                            f"type: {service_type}, rank_id: {service.rank_id}).\n"
                            f"{str(result)}")
