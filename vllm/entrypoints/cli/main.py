...
# [startup] NOTE: The background torch+transformers preload thread from
# upstream PR #40331 was removed locally because it races with the lazy
# __init__ pattern used by transformers 5.x. The bg thread places a
# partially initialized `transformers` module in sys.modules before the
# `_LazyModule` swap at the end of transformers/__init__.py; the main
# thread's `from transformers import GenerationConfig, PretrainedConfig`
# in vllm/transformers_utils/config.py then fast-paths on sys.modules
# without waiting, and fails the attribute lookup. If re-introduced,
# ensure transformers is either fully imported on the main thread first
# or that the bg thread's import completes before any `from transformers
# import X` on the main thread.
