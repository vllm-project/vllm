...
class PDDisaggregatedJobCommand(CLISubcommand):
    ...
    def cmd(args: argparse.Namespace) -> None:
        ...
        ray.init(log_to_driver=not args.disable_log_to_driver, runtime_env=runtime_env)
        ...
        # Dynamically get supported connectors from KVConnectorFactory registry
        # This avoids maintaining a hardcoded list that needs manual updates
        supported_connectors = tuple(
            name.lower() for name in KVConnectorFactory.get_registered_connectors()
        )
        ...
        if config.kv_connector in supported_connectors:
            ...
            raise ValueError(
                f"Unrecognized kv_connector: {config.kv_connector}. "
