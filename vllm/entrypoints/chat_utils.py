...
def _resolve_vision_chunk_items(
    ...
    filtered_items = [
        (idx, item) for idx, item in enumerate(vision_chunk_items) if item is not None
    ]

    assert len(filtered_items) == len(vision_chunks_modality_order), (
    ...
    for i, (idx, item) in enumerate(filtered_items):
        inner_modality = vision_chunks_modality_order[i]
        data, uuid = item
        uuid_val = uuid if idx < len(vision_chunks_uuids) else None
