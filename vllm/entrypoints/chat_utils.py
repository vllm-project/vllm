...
class CustomChatCompletionMessageParam(TypedDict, total=False):
    ...
    tool: str | None
    """The XTML tool name for tool-role messages."""

    index: int | str | None
    """The XTML tool-call index for tool-role messages."""
...
def _parse_chat_message_content(
    ...
    for result_msg in result:
        if role == "assistant":
            ...
            if "tool" in message and message["tool"] is not None:
                result_msg["tool"] = cast(str, message["tool"])
            if "index" in message and message["index"] is not None:
                result_msg["index"] = cast(int | str, message["index"])
...
def parse_chat_messages(
    ...
    # Apply model-specific message preprocessing (e.g. tool result reordering)
    if model_config.is_multimodal_model:
        mm_processor = mm_tracker.mm_processor
        if hasattr(mm_processor, "preprocess_messages"):
            messages = mm_processor.preprocess_messages(messages)
