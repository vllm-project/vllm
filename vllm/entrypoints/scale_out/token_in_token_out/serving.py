...
class ServingTokens(GenerateBaseServing):
    ...
    async def serve_tokens(
        ...
        if request.return_token_logprobs:
            ...
            if self.model_config.logprobs_mode not in ("raw_logprobs", "processed_logprobs"):
                # The field is named for log-probabilities; do not hand back
                # logits under that name.
                return self.create_error_response(
                    "return_token_logprobs requires --logprobs-mode raw_logprobs or "
                    f"processed_logprobs (server runs {self.model_config.logprobs_mode})"
                )
