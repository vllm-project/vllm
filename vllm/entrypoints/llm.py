...
from vllm.config import CompilationConfig, is_init_field, ModelDType, TokenizerMode
...
from vllm.utils import Counter, deprecate_kwargs, Device, is_list_of
...
class LLM:
    ...
    Args:
        ...
        kv_cache_memory: Size of KV Cache per GPU in bytes. By default, this is
            ...
            on gpu_memory_utilization. However, users may want to manually
            specify the kv cache memory size. kv_cache_memory allows more
            fine-grain control of how much memory gets used when compared with
            using gpu_memory_memory_utilization. Note that kv_cache_memory
    ...
    def _get_modality_specific_lora_reqs(
        ...
        optional_loras = (
            [lora_request] * len(parsed_prompts)
        ...
        return [
            ...
            for parsed_prompt, opt_lora_req in zip(parsed_prompts, optional_loras)
        ]
    ...
    def _resolve_single_prompt_mm_lora(
        ...
        if (
            ...
            or not isinstance(parsed_prompt, dict)
        ...
        intersection = set(parsed_prompt["multi_modal_data"].keys()).intersection(
