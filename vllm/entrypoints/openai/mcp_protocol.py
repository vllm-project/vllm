...
class BasicMCPServer(MCPServer):
    ...
    async def read_resource(self, uri: str) -> MCPResult:
        ...
        return MCPResult(content=f"Resource not found: {uri}", is_error=True)
        ...
        try:
            if name == "echo":
                ...
                try:
                    ...
                    return MCPResult(
                        content=f"Calculation error: {str(e)}", is_error=True
                    )
                ...
                try:
                    ...
                    return MCPResult(
                        content=f"Current time: {current_time.isoformat()}"
                    )
                except Exception as e:
                    return MCPResult(content=f"Time error: {str(e)}", is_error=True)

            else:
                return MCPResult(content=f"Unknown tool: {name}", is_error=True)

        except Exception as e:
            return MCPResult(content=f"Tool execution error: {str(e)}", is_error=True)
        ...
        try:
            if method == "resources/list":
                ...
                if result.is_error:
                    return {
                        "content": [
                            {"type": "text", "text": f"Error: {result.content}"}
                        ],
                        "isError": True,
                    }
                else:
