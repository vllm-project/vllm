...
async def debug_profile_start(request: Request, prefix: str = "benchmark"):
    """Start profiling with optional trace prefix."""
...
@router.get("/step_stats")
async def debug_step_stats(request: Request):
    """Return accumulated engine step timings (prefill/decode)."""
    stats = await engine_client(request).engine_core.call_utility_async(
        "get_step_stats")
    return JSONResponse(content=stats)


@router.post("/step_stats/reset")
async def debug_reset_step_stats(request: Request):
    """Reset step stats accumulator."""
    await engine_client(request).engine_core.call_utility_async(
        "reset_step_stats")
