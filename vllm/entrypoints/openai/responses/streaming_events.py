...
class SimpleStreamingState:
    ...
    is_custom_tool_call: bool = False
...
def emit_simple_tool_call_open(
    ...
    state.is_custom_tool_call = is_custom
...
def emit_simple_tool_call_delta(
    ...
    state.accumulated_text += delta
    if not state.is_custom_tool_call:
    ...
    decoded = decode_custom_tool_input_prefix(state.accumulated_text)
    inc = (
        decoded[len(state.custom_unwrapped) :]
        if decoded.startswith(state.custom_unwrapped)
        else ""
    )
    if not inc:
        return []
    state.custom_unwrapped = decoded
    state.has_emitted_tool_call_delta = True
    return [
        ResponseCustomToolCallInputDeltaEvent(
...
def emit_simple_tool_call_done(
    ...
    events: list[StreamingResponsesResponse] = []
    if state.is_custom_tool_call:
        unwrapped = decode_custom_tool_input(state.accumulated_text)
        extra = (
            unwrapped[len(state.custom_unwrapped) :]
            if unwrapped.startswith(state.custom_unwrapped)
            else ""
        )
        if extra:
            events.append(
            ...
            state.has_emitted_tool_call_delta = True
        state.custom_unwrapped = unwrapped
        ...
        done_item: ResponseCustomToolCall | ResponseFunctionToolCall = (
            ResponseCustomToolCall(
                type="custom_tool_call",
    ...
    state.is_custom_tool_call = False
    state.current_state = _StateType.NONE
    return events
...
class SimpleStreamingEventProcessor:
    ...
    def __init__(
        ...
        self.custom_tool_names = extract_custom_tool_names(tools)
    ...
    def open(
        ...
        if target_state == _StateType.TOOL_CALL:
            ...
            return handlers.open_fn(
                ...
                tool_call.index,
                call_name.namespace,
                tool_call.function.name in self.custom_tool_names,
            )
