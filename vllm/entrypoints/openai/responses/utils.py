...
def construct_chat_messages_with_tool_call(
    ...
    A single assistant turn is emitted by the Responses API as several ordered
    items (a reasoning item, an optional output message, one or more function
    calls). They carry no shared turn id, so they are grouped by position: a
    reasoning / message / function_call item is merged into the immediately
    preceding assistant message (filling its empty ``reasoning`` / ``content``
    slot, or appending to its ``tool_calls``). A non-assistant item (a tool
    result or a user message) ends the run, so the next assistant item starts a
    fresh message. This reconstructs the single ``think + content + tool_call``
    assistant turn the models were trained on, instead of leaving each item as
    its own bare assistant message.
    """
    messages: list[ChatCompletionMessageParam] = []
...
def _construct_message_from_response_item(
    ...
    """Returns a new message or None. If ``None``, ``prev_msg`` was updated in
    place (the item was merged into the preceding assistant message). If
    ``prev_msg`` is not an assistant message, a new message is always returned.
    """
    prev_assistant_msg = (
    ...
    if isinstance(item, ResponseFunctionToolCall):
        ...
        if prev_assistant_msg:
            ...
            logger.warning(
                ...
                "Tool call merging is skipped and a new assistant message is "
                "created. Item %s",
                item.id,
            )
        ...
        if prev_assistant_msg:
            ...
            if previous_reasoning is None:
                prev_assistant_msg["reasoning"] = reasoning_content
                return None
        return {
            "role": "assistant",
