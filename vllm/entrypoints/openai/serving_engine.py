...
class OpenAIServing:
    ...
    async def beam_search(
        ...
        for _ in range(max_tokens):
            ...
            all_beams_token_id = []
            # Store which parent beam each token comes from.
            # NOTE: Do NOT assume each beam returns exactly `logprobs_num`
            # candidates. The returned dict may contain a variable number of
            # entries (e.g. top-k plus sampled token), so we must keep an
            # explicit parent mapping for correctness.
            all_beams_parent_beam_idx = []
            ...
            for i, result in enumerate(output):
                ...
                if result.outputs[0].logprobs is not None:
                    ...
                    all_beams_token_id.extend(token_ids)
                    all_beams_parent_beam_idx.extend([i] * len(token_ids))
            ...
            all_beams_parent_beam_idx = np.array(all_beams_parent_beam_idx)
            ...
            if not ignore_eos:
                ...
                for idx in eos_idx:
                    parent_i = int(all_beams_parent_beam_idx[idx])
                    current_beam = all_beams[parent_i]
                    result = output[parent_i]
            ...
            # Get indices of the top beam_width probabilities (exclude -inf).
            valid_idx = np.where(np.isfinite(all_beams_logprob))[0]
            if valid_idx.size == 0:
                # No non-EOS candidates left; stop expanding beams.
                all_beams = []
                break
            topk = min(beam_width, int(valid_idx.size))
            valid_logprob = all_beams_logprob[valid_idx]
            # np.argpartition expects kth to be in [0, N-1].
            topn_local = np.argpartition(-valid_logprob, topk - 1)[:topk]
            topn_idx = valid_idx[topn_local]
