...
ParsedInvoke = tuple[str | None, dict[str, str] | None]
...
class StepDeepResearchToolParser(ToolParser):
    ...
    def _parse_all_invokes(
        ...
        invoke_pattern = r'<steptml:invoke name="[^"]+"[\s\S]*?</steptml:invoke>'
        invoke_matches = re.findall(invoke_pattern, xml_text)

        for invoke_text in invoke_matches:
            func_name, params = StepDeepResearchToolParser._parse_steptml_invoke(
                invoke_text
            )
            if func_name:
    ...
    def _cast_single_param(
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Failed to cast parameter '%s' with value %r to type '%s'. "
                    "Initial error: %s. Fallback error: %s",
                    key,
                    value,
                    typ,
                    e1,
    ...
    def _check_partial_start_tag(self, text: str) -> int:
        """
        Check if text ends with a partial start tag.

        Returns:
            The length of the partial tag suffix if found, 0 otherwise.
        """
        # Check if text ends with any prefix of TOOLCALL_START
        for i in range(1, len(self.TOOLCALL_START)):
            if text.endswith(self.TOOLCALL_START[:i]):
                return i
        return 0

    def _handle_before_tool_block(
        self, unprocessed_text: str, current_text: str
    ) -> tuple[bool, DeltaMessage | None]:
        """
        ...
        if start_pos == -1:
            # Check if text ends with partial start tag
            partial_len = self._check_partial_start_tag(unprocessed_text)
            if partial_len > 0:
                # Output content before the partial tag, keep partial for later
                content_len = len(unprocessed_text) - partial_len
                if content_len > 0:
                    content = unprocessed_text[:content_len]
                    self.position += content_len
                    return False, DeltaMessage(content=content)
                # Only partial tag, wait for more
                return False, None
            # No toolcall start found and no partial, output everything
    ...
    def _handle_new_invoke_block(self, unprocessed_text: str) -> bool | None:
        """
        ...
        if unprocessed_text.startswith(self.INVOKE_START):
            # Always increment, current_tool_id starts at -1
            self.current_tool_id += 1
            self.current_tool_name_sent = False
    ...
    def _parse_active_tool_call(
        ...
        if end_invoke_pos == -1 and self._check_partial_end_tag(unprocessed_text):
            return None
    ...
    def extract_tool_calls_streaming(
        ...
        while True:
            ...
            if self.tool_block_finished:
                # Check if there's another toolcall block starting
                next_toolcall_pos = unprocessed_text.find(self.TOOLCALL_START)
                if next_toolcall_pos != -1:
                    # There's another toolcall block, output content before it
                    # and reset state
                    content_before = unprocessed_text[:next_toolcall_pos]
                    self.position += next_toolcall_pos
                    # Reset state for new toolcall block
                    self.tool_block_started = False
                    self.tool_block_finished = False
                    if content_before:
                ...
                partial_len = self._check_partial_start_tag(unprocessed_text)
                if partial_len > 0:
                    # Output content before the partial tag
                    content_len = len(unprocessed_text) - partial_len
                    if content_len > 0:
                        content = unprocessed_text[:content_len]
                        self.position += content_len
            ...
            if is_active:
                result = self._parse_active_tool_call(unprocessed_text, request)
                return result
    ...
    def _parse_tool_block(
        ...
        for func_name, params in parsed_tools:
            if func_name and params is not None:
                casted_params = self._cast_arguments(func_name, params, request)
                args_json = json.dumps(casted_params, ensure_ascii=False)
                tool_calls.append(
                    ToolCall(function=FunctionCall(name=func_name, arguments=args_json))
                )
        return tool_calls
    ...
    def extract_tool_calls(
        ...
        if self.TOOLCALL_START not in model_output:
            ...
            return ExtractedToolCallInformation(
                tools_called=False, tool_calls=[], content=final_content
            )
        ...
        for part in parts:
            is_tool_block = part.startswith(self.TOOLCALL_START) and part.endswith(
                self.TOOLCALL_END
            )
            if is_tool_block:
...
if __name__ == "__main__":
    ...
    parser = StepDeepResearchToolParser(tokenizer=tokenizer)
    ...
    parser2 = StepDeepResearchToolParser(tokenizer=None)
    current_text = ""
            ...
            if delta.tool_calls:
                for dtc in delta.tool_calls:
                    ...
                    if func and func.name:
                        print(f"  ToolCall[{dtc.index}]: name={func.name}")
                    if func and func.arguments:
                        print(f"  ToolCall[{dtc.index}]: args={func.arguments}")

    # Test Case 3: Streaming with multiple toolcall blocks
    print("\n--- TEST 3: Streaming with multiple toolcall blocks ---")
    streaming_simple = (
        "<steptml:toolcall>\n"
        '<steptml:invoke name="web_search">\n'
        '<steptml:parameter name="query">'
        ...
        "</steptml:invoke>\n"
        "</steptml:toolcall>\n"
        "Now let me open a specific URL for more details...\n"
        "<steptml:toolcall>\n"
        '<steptml:invoke name="open_url">\n'
        '<steptml:parameter name="url">'
    ...
    stream_tokens = [tokenizer.decode(id) for id in ids]
    parser3 = StepDeepResearchToolParser(tokenizer=None)
    current_text = ""
    print("Streaming output:")
    for delta_text in stream_tokens:
        previous_text = current_text
    ...
    # Test Case 4: Streaming with think tag before toolcall, "><" as single token
    print("\n--- TEST 4: Streaming with think tag, >< as single token ---")
    manual_tokens = [
        "<think>",
        "think content",
        "</think",  # </think without >
        "><",  # >< as single token (end of </think> and start of <steptml:toolcall>)
        "steptml:toolcall>\n",
        '<steptml:invoke name="web_search">\n',
        '<steptml:parameter name="query">',
        '["python", "tutorial", "beginner", "examples"]',
    ...
    parser4 = StepDeepResearchToolParser(tokenizer=None)
    current_text = ""
    print("Streaming output:")
    for delta_text in manual_tokens:
        previous_text = current_text
        current_text += delta_text
        delta = parser4.extract_tool_calls_streaming(
            previous_text=previous_text,
            current_text=current_text,
            delta_text=delta_text,
            previous_token_ids=[],
