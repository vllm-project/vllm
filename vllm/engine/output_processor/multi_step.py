...
from vllm.engine.output_processor.single_step import single_step_process_prompt_logprob
...
class MultiStepOutputProcessor(SequenceGroupOutputProcessor):
    ...
    def process_outputs(
        ...
        seq = seqs[0]
        seq_id = seq.seq_id
        assert all([seq_id == output.samples[0].parent_seq_id for output in outputs])
        ...
        if is_async:
            ...
            self._process_seq_outputs(
                seq, valid_samples, sequence_group.sampling_params
            )

    def _process_decode_and_stop(
        self, seq: Sequence, sampling_params: SamplingParams
    ) -> None:
        new_char_count = 0
