...
class EngineArgs:
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parser.add_argument(
            ...
            choices=["auto", "triton", "flashkda", "flashinfer", "hipkda"],
            default=None,
    ...
    def create_engine_config(
        ...
        if self.kda_prefill_backend is not None:
            if (
                self.kda_prefill_backend in ("flashkda", "flashinfer")
                and not current_platform.is_cuda()
            ):
                raise ValueError(
                    f"--kda-prefill-backend={self.kda_prefill_backend} is "
                    "only available on CUDA."
                )
            if self.kda_prefill_backend == "hipkda" and not current_platform.is_rocm():
                raise ValueError(
                    "--kda-prefill-backend=hipkda is only available on ROCm."
                )
