...
class EngineArgs:
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        # Factory defaults become argparse.SUPPRESS on the serve-help path
        # because the heavy ones must not run there:
        # CompilationConfig.__post_init__ resolves the platform, which imports
        # torch. The factories below are import-light, so re-instantiate them
        # and let help render their defaults as it does on the normal path.
        # The no-torch serve-help test is what enforces the lightness claim.
