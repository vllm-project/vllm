...
class EngineArgs:
    ...
    enable_sparse_hot_cache: bool = CacheConfig.enable_sparse_hot_cache
    sparse_topk: int | None = CacheConfig.sparse_topk
    copy_method: str = CacheConfig.copy_method.value
    cache_policy: str = CacheConfig.cache_policy.value
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        cache_group.add_argument(
            "--enable-sparse-hot-cache",
            action="store_true",
            default=CacheConfig.enable_sparse_hot_cache,
            help="Enable the experimental sparse hot KV cache migration path.",
        )
        cache_group.add_argument(
            "--sparse-topk",
            type=int,
            default=CacheConfig.sparse_topk,
            help="Maximum number of KV tokens selected per layer.",
        )
        cache_group.add_argument(
            "--copy-method",
            type=str,
            choices=[method.value for method in CopyMethod],
            default=CacheConfig.copy_method.value,
            help="CPU<->GPU KV block transfer method for sparse hot cache.",
        )
        cache_group.add_argument(
            "--cache-policy",
            type=str,
            choices=[policy.value for policy in CachePolicy],
            default=CacheConfig.cache_policy.value,
            help="GPU hot-cache eviction policy for sparse hot cache.",
        )
    ...
    def create_engine_config(
        ...
        cache_config = CacheConfig(
            ...
            enable_sparse_hot_cache=self.enable_sparse_hot_cache,
