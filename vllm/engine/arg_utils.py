...
class EngineArgs:
    ...
    @staticmethod
    def add_cli_args_for_tools(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        """
        CLI arguments to configure tool use for the OpenAI API-style endpoint.
        At this point, only a template for taking the provided tools and formatting them
        into a model-specific system prompt format is supported, but others may be added
        in the future, e.g. for decoding the tool call generated by the model into the
        OpenAI API style.
        """
        parser.add_argument(
    ...
    def add_cli_args(
        ...
        # Related to OpenAI API-style entrypoint
        parser = EngineArgs.add_cli_args_for_tools(parser)
