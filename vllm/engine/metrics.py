...
gauge_scheduler_swapped = Gauge("vllm_requests_stopped_total",
...
_gpu_cache_usage_fn = lambda stats: _cache_usage(stats.total_gpu_blocks, stats.
                                                 free_gpu_blocks)
_cpu_cache_usage_fn = lambda stats: _cache_usage(stats.total_cpu_blocks, stats.
                                                 free_cpu_blocks)
    ...
    def __init__(self) -> None:
        ...
        self.system_metrics: List[PrometheusMetric] = [
            GaugeMetric(gauge=gauge_scheduler_running,
                        fn=_scheduler_running_fn),
            GaugeMetric(gauge=gauge_scheduler_swapped,
                        fn=_scheduler_swapped_fn),
            GaugeMetric(gauge=gauge_scheduler_waiting,
                        fn=_scheduler_waiting_fn),
            GaugeMetric(gauge=gauge_gpu_cache_usage, fn=_gpu_cache_usage_fn),
            GaugeMetric(gauge=gauge_cpu_cache_usage, fn=_cpu_cache_usage_fn),
        ]
        self.iteration_metrics: List[PrometheusMetric] = [
            CounterMetric(counter=counter_prompt_tokens, fn=_prompt_tokens_fn),
            CounterMetric(counter=counter_generation_tokens,
                          fn=_generation_tokens_fn),
            HistogramMetric(histogram=histogram_time_to_first_token,
                            fn=_time_to_first_token_fn),
            HistogramMetric(histogram=histogram_inter_token_latency,
                            fn=_inter_token_latency_fn),
        ]
