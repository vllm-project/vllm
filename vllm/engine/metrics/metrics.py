...
class TrackedStats:
    ...
    def get_throughput(self, now: float) -> float:
        return np.sum(self.stats) / (now - self.last_reset_time)

    def get_mean(self) -> float:
        return np.mean(self.stats) if len(self.stats) > 0 else 0
...
class PrometheusMetric(ABC):
    ...
    def __init__(self, attr: str, template: str, log_local: bool=True) -> None:
