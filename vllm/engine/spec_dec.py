...
class SpecDecWorker(Worker):
    ...
    def __init__(self, config: SpecDecConfig, scheduler: Scheduler) -> None:
        ...
        logger.info("Initializing speculative decoding worker: "
    ...
    def _prepare_inputs(
        ...
        input_ids_list = [
            _pad_left_to_max(input_ids, max_len, PAD_TOKEN_ID)
    ...
    def _sample_method(self, logits) -> torch.Tensor:
        temperature = 0.0001
    ...
    def set_draft_tokens(self, seq_group_list: List[SequenceGroupMetadata],
        ...
        for _ in range(self.propose_cnt):
            ...
            attention_mask = torch.cat([
                attention_mask,
                torch.ones(input_tensor.shape[0], 1, device='cuda')
            ],
                                       dim=1)
        ...
        for i, seq_group_metadata in enumerate(seq_group_list):
            ...
            seq_group_metadata.block_tables[
                seq_id] = self.scheduler.block_manager.get_block_table(
                    seqs[seq_id])
    ...
    def _accept_tokens(self, seq: Sequence,
                       seq_group_output: SequenceOutput) -> List[int]:
        accepted_token_ids = []
        for i, token_prob in enumerate(seq.data.draft_token_probs):
            ...
            if len(self.alphas) % 20 == 0:
                logger.warning(
                    f"alpha: {len(self.alphas)}, {sum(self.alphas) / len(self.alphas)}"
                )
            r = torch.rand(1).item()
    ...
    def accept(self, target_outputs: List[SamplerOutput],
               scheduler_outputs: SchedulerOutputs) -> None:
        scheduled_seq_groups = scheduler_outputs.scheduled_seq_groups
        assert len(scheduled_seq_groups) == len(target_outputs)
        for seq_group, seq_group_output in zip(scheduled_seq_groups,
                                               target_outputs):
            assert seq_group.num_seqs() == 1
            sample: SequenceOutput = seq_group_output.samples[0]
            seq_id = list(seq_group.seqs_dict.keys())[0]
            cur_seq = seq_group.seqs_dict[seq_id]
            logger.info(
                f"sample output: {sample.output_token}, {cur_seq.data.get_draft_token_ids()}"
            )
