...
class HfHubResolver(FilesystemResolver):
    ...
    async def resolve_lora(self, base_model_name: str,
        ...
        if maybe_repo is not None and maybe_repo not in self.adapter_dirs:
            self.adapter_dirs[maybe_repo] = await self._get_adapter_dirs(
                maybe_repo)

        repo_path = await asyncio.to_thread(snapshot_download,
                                            repo_id=maybe_repo,
                                            allow_patterns=f"{maybe_subpath}/*"
                                            if maybe_subpath != "." else "*")
    ...
    async def _resolve_repo(self, lora_name: str) -> Optional[str]:
    ...
    async def _resolve_repo_subpath(
            self, lora_name: str, maybe_repo: Optional[str]) -> Optional[str]:
        if maybe_repo is None:
            return None
        repo_len = len(maybe_repo)
        if lora_name == maybe_repo or len(
                lora_name) == repo_len + 1 and lora_name[-1] == "/":
    ...
    async def _get_adapter_dirs(self, repo_name: str) -> set[str]:
        ...
        adapter_dirs = {
            ...
            for name in repo_files if name.endswith("adapter_config.json")
        }
