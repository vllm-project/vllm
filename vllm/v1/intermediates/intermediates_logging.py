...
def save_tensors_metadata_if_too_large(tensor: torch.Tensor,
    ...
    if intermediate_log_config is None:
        return False
    if (intermediate_log_config.max_tensor_size is not None
...
def safe_reload_tensor(save_path: str, tensor: Any,
                       reload_dir: Optional[str]) -> Any:
    ...
    try:
        ...
        logger.debug("reload tensor of shape %s from %s", tensor.shape,
                     reload_path)
...
def save_tensors(tensor: Any,
                 file_path: str,
                 reload_input_dir: Optional[str] = None) -> Any:
    ...
    if isinstance(tensor, torch.Tensor):
        ...
        try:
            ...
            reloaded_tensor = safe_reload_tensor(pt_path, tensor,
                                                 reload_input_dir)
...
def log_pre_fwd_hook(module: torch.nn.Module,
    ...
    if log_dir := prepare_log_current_fwd(module, is_pre_fwd=True):
        ...
        reloaded_inputs = save_tensors(inputs, str(log_dir / "inputs"),
                                       reload_input_dir)
