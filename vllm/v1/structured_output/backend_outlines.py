...
def validate_structured_output_request_outlines(params: SamplingParams):
    ...
    if gd_params.regex:
        ...
        raise ValueError(
            "Outlines guided decoding backend "
...
def validate_regex_is_buildable(pattern: str) -> None:
    ...
    try:
        ...
        if _prefix_needs_context(parsed):
            raise ValueError(
                "anchored universal start state doesn't exist for this regex")
    except Exception as e:
        raise ValueError(f"Error parsing guided regex: {e}") from e
