...
"""Deterministic Uno noise preserving the released batch-row hash contract.

The hash includes the flat batch-row offset, so deterministic replay assumes a
fixed request row order as well as a fixed request seed and draft step.
"""
...
def fill_uno_noise(
    ...
    The hash includes each flat batch-row position.  Reordering requests
    changes their noise values; this preserves the released Uno hash semantics.
