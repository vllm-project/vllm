...
class EagleProposer:
    ...
    def _reshard_draft_tp_tensor(self, target_model, target_name):
        """
        ...
        for name, param in self.model.named_parameters():
            if target_name in name:
                src_tensor = sd[name]
                full_tensor_shape = (
                    self.speculative_config.draft_tensor_parallel_size * param.shape[0],
                    *param.shape[1:],
                )
                ...
                torch.distributed.all_gather_into_tensor(
                    full_tensor, src_tensor, group=self.tp_group
                )
