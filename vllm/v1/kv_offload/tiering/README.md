...
错误跳过。默认仅对至少第二次出现、经验复用概率达到阈值的 Session 自动卸载；
请求可通过 `force_kv_offload=true` 显式覆盖热度和阶段筛选，但仍受迁移额度保护。
...
迁移 Block/字节/时延、迁移预算占用、复用信号、过期 Session 数和 CPU 主动回收数量。指标统一使用
`vllm:kv_tiering_*` 前缀，可通过 vLLM 的 `/metrics` 端点观察。
