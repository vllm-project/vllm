...
class TQSlidingWindowSpec(SlidingWindowSpec):
    ...
    Sliding-window analogue of ``TQFullAttentionSpec``. Overrides
    real_page_size_bytes to use TQ packed slot bytes instead of the raw
    head_size * dtype formula, so sliding-window layers can share the
    packed uint8 cache layout used by full-attention TQ layers.
    """
