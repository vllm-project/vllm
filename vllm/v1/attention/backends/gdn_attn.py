...
class GDNAttentionMetadata:
    ...
    # Chunk-kernel inputs for the non-spec prefill region: the prefill tail
    # when 1-token decodes are peeled off in GatedDeltaNet._forward_core, else
    # the full non-spec batch. Precomputed to avoid per-layer recomputation.
...
class GDNAttentionMetadataBuilder(AttentionMetadataBuilder[GDNAttentionMetadata]):
    ...
    def build(  # type: ignore[override]
        ...
        if num_prefills > 0:
            ...
            # In a mixed non-spec batch (prefills + 1-token decodes), the
            # decodes are peeled off to the recurrent decode kernel in
            # GatedDeltaNet._forward_core (they are the contiguous decode-first
            # front slice), so the chunk kernel only processes the prefill tail.
            # Build chunk metadata from the prefill-only, rebased cu_seqlens in
            # that case; otherwise use the full non-spec one. _forward_core keys
            # off the same condition, so they agree.
