...
def split_decodes_and_prefills(
    ...
    # For PCP, query_start_loc contains local (partitioned) counts. Use
    # global_num_scheduled_tokens for prefill/decode classification, and
    # consult num_computed_tokens to distinguish new prefills (which must
    # not be classified as decode regardless of query length).
...
def pcp_kv_allgather_and_restore(
    ...
    """All-gather key and value tensors across PCP ranks and restore order.
...
def extend_all_queries_by_1(
    ...
    """Return a new CommonAttentionMetadata with all query lens increased by 1.

    Also seq_lens are increased by 1. Useful e.g. in speculative decoding
    where each sequence is extended by 1 draft token. The slot mapping is
    computed externally as it requires more information.
    """
...
def fused_pcp_qkv_select(
    ...
    """Fused Q/K/V head/tail selection for PCP DualChunkSwap.
    ...
    k_head = torch.empty(
        (q.size(0) // 2 * (pcp_rank + 1),) + k.shape[1:],
        device=k.device,
        dtype=k.dtype,
    )
    v_head = torch.empty(
        (q.size(0) // 2 * (pcp_rank + 1),) + v.shape[1:],
        device=v.device,
        dtype=v.dtype,
    )
    k_tail = torch.empty(
