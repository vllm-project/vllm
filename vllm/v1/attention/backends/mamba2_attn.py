...
class Mamba2AttentionMetadataBuilder(
    ...
    def build(
        ...
        print(f"[MAMBA2-ATTN-BUILDER] Building Mamba2AttentionMetadata")
        print(f"[MAMBA2-ATTN-BUILDER] num_decode_draft_tokens_cpu: {num_decode_draft_tokens_cpu}")
        print(f"[MAMBA2-ATTN-BUILDER] num_accepted_tokens: {num_accepted_tokens}")
        ...
        if spec_sequence_masks is None:
            ...
            if state_indices_tensor.ndim == 1:
                # No prefix caching: simple case
                # Allocate num_spec+2 cache slots per spec sequence for checkpointing
                # (1 verified state + num_spec drafts + 1 bonus token)
                spec_state_indices_tensor = state_indices_tensor[spec_sequence_masks].unsqueeze(1).expand(
                    -1, self.num_spec + 1
                ).contiguous()
                # Add offset to create unique slots: [base, base+1, base+2, ..., base+num_spec+1]
                offsets = torch.arange(self.num_spec + 1, device=state_indices_tensor.device, dtype=torch.int32)
                spec_state_indices_tensor = spec_state_indices_tensor + offsets.unsqueeze(0)
