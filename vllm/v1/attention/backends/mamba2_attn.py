...
class Mamba2AttentionMetadataBuilder(
    ...
    def build(self,
        ...
        if self.vllm_config.cache_config.enable_prefix_caching:
            ...
            block_idx_last_computed_token = cdiv(num_computed_tokens,
                                                 mamba_block_size) - 1
            # which is <= block index for the first scheduled token
            block_idx_first_scheduled_token = cdiv(num_computed_tokens + 1,
                                                   mamba_block_size) - 1
            # which is <= block index of the last scheduled token
            block_idx_last_scheduled_token = cdiv(
                common_attn_metadata.seq_lens, mamba_block_size) - 1
            # -1 in case it's non-computed and causes later issues with indexing
            block_idx_last_computed_token = block_idx_last_computed_token.clamp(
                min=0)
        else:
