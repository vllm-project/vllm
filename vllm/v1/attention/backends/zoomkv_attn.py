...
def _decode_region_sparse_eligible(
    *,
    ...
    """True when every request in the reordered decode prefix is long enough."""
    if cfg.dense_fallback or num_decodes <= 0:
        return False
    if cfg.full_attention_threshold <= 0:
        return True
    if seq_lens_cpu is None or seq_lens_cpu.numel() < num_decodes:
        # Missing host lengths must fail closed to dense attention.
        return False
    return bool(
        torch.all(
            seq_lens_cpu[:num_decodes] >= int(cfg.full_attention_threshold)
        ).item()
    )
...
def _should_use_sparse_decode(
    *,
    ...
    if num_prefills > 0:
        return False
    return _decode_region_sparse_eligible(
        cfg=cfg,
...
class ZoomKVMetadataBuilder(FlashAttentionMetadataBuilder):
    ...
    def get_cudagraph_support(
        ...
        # The model runner disables full-graph replay until every active
        # sequence reaches the sparse threshold.
