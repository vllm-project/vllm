...
class TurboQuantAttentionBackend(AttentionBackend):
    ...
    def indexes_kv_by_block_stride(cls) -> bool:
        ...
        # The Triton decode kernel addresses blocks via block_table indices and
        # kv_cache.stride(0), so a page padded to a larger physical size is
        # safe: stride(0) reflects the actual allocation, and the kernel reads
        # only tq_slot_size bytes within each block. Returning True allows
        # unify_kv_cache_spec_page_size to use page_size_padded when layers
        # have different slot sizes (e.g. Gemma4's head_dim=256 vs 512).
        return True
...
class TurboQuantAttentionImpl(AttentionImpl["TurboQuantMetadata"]):
    ...
    def _chunked_causal_attn(
        ...
        for q_start in range(0, q_len, chunk):
            ...
            if is_causal:
                ...
                k_pos = torch.arange(
                    kv_len, device=q.device
                ).unsqueeze(0)  # (1, kv_len)
                ...
                ).unsqueeze(0).unsqueeze(0)  # (1, 1, c, kv_len)
    ...
    def _prefill_attention(
        ...
        if self._use_flash_attn and attn_metadata.max_query_len == attn_metadata.max_seq_len:
    ...
    def _continuation_prefill(
        ...
        if self._use_flash_attn:
            ...
            additive_mask = torch.where(
                ...
                torch.full(
                    (q_len, seq_len), float("-inf"),
                    device=device, dtype=query.dtype,
                ),
            ).unsqueeze(0).unsqueeze(0)  # (1, 1, q_len, seq_len)
            out = self._chunked_causal_attn(
                q_t, k_t, v_t, attn_mask=additive_mask, enable_gqa=(Hk < Hq),
            )  # (1, Hq, q_len, D)
