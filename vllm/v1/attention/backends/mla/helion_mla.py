...
from vllm.kernels.helion.ops.mla_decode_fp8_per_token import mla_decode
...
class HelionMLAMetadataBuilder(MLACommonMetadataBuilder[MLACommonMetadata]):
    _cudagraph_support: ClassVar[AttentionCGSupport] = AttentionCGSupport.UNIFORM_BATCH


class HelionMLABackend(MLACommonBackend):
    ...
    @staticmethod
    def get_impl_cls() -> type["HelionMLAImpl"]:
        return HelionMLAImpl

    @staticmethod
    def get_builder_cls() -> type["HelionMLAMetadataBuilder"]:
        return HelionMLAMetadataBuilder
...
class HelionMLAImpl(MLACommonImpl[MLACommonMetadata]):
    ...
    def forward_mqa(
        ...
        assert latent_kv_cache.numel() > 0
        ...
        # Add a head dim of 1
        latent_kv_cache = latent_kv_cache.unsqueeze(2)
        # latent_kv_cache = latent_kv_cache[..., : self.kv_lora_rank]
        PAGE_SIZE = latent_kv_cache.size(1)
