...
class TritonMLASparseImpl(XPUMLASparseImpl):
    ...
    # SM80 path has no masked-MHA prefill kernel and no DCP; these are
    # probed unconditionally by the unified forward_impl.
    ...
    def _forward_bf16_kv(
        ...
        # Chunk large prefill batches so the split-KV workspace stays small;
        # topk indices are absolute KV positions, so slicing along the query
        # dim is independent per chunk.
        max_chunk = 4096
        if num_tokens > max_chunk:
            parts = []
            for start in range(0, num_tokens, max_chunk):
                end = min(start + max_chunk, num_tokens)
                parts.append(
                    triton_sparse_mla_attention(
                        q[start:end],
                        kv_c_and_k_pe_cache,
