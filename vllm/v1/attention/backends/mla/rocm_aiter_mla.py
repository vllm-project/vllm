...
class AiterMLAHelper:
    ...
    def use_gluon_verify(num_heads: int, max_qo_len: int, kv_cache_dtype: str) -> bool:
        """Whether a small-head multi-token verify goes to Gluon.

        bf16 has no gqa<16, qseqlen>1 asm kernel, so the verify goes to Gluon
        as a native 4-D MTP query. fp8 has an asm kernel via the q-row fold and
        must not come here: Gluon's only fp8-KV regime, bh16bn128, takes a bf16
        query. Shared by the builder and forward_mqa, each with its own head
        count.
        """
...
class AiterMLAImpl(MLACommonImpl[AiterMLAMetadata]):
    ...
    def forward_mqa(
        ...
        if decode.use_gluon_decode or AiterMLAHelper.use_gluon_verify(
            ...
            assert num_tokens % qlen == 0, (
                f"Gluon MLA requires uniform query lengths. "
                f"Got {num_tokens=} and {qlen=}."
            )
