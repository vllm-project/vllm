...
def _convert_req_index_to_global_index_kernel(
    ...
    if HAS_PREFILL:
        ...
        if is_prefill != 0:
            out_tile_base = (out_ptr + token_id * out_stride0 +
                             (tile_id * BLOCK_N) * out_stride1)
...
class FlashMLASparseMetadataBuilder(
    ...
    def build(self,
        ...
        (num_decodes, num_prefills, num_decode_tokens,
         num_prefill_tokens) = split_decodes_and_prefills(
             common_attn_metadata,
             decode_threshold=self.reorder_batch_threshold or 0)
        ...
        assert num_prefill_reqs + num_decode_reqs == \
...
class FlashMLASparseImpl(MLACommonBaseImpl[FlashMLASparseMetadata]):
    ...
    def _ensure_prefill_buffers(self, num_slots: int,
                                device: torch.device) -> None:
        if (self.prefill_bf16_workspace is None
                or self.prefill_bf16_workspace.shape[0] < num_slots
                or self.prefill_bf16_workspace.device != device):
            self.prefill_bf16_workspace = torch.empty(
                (num_slots, self.head_size),
                dtype=torch.bfloat16,
                device=device)

        if (self.prefill_seen_buffer is None
                or self.prefill_seen_buffer.shape[0] < num_slots
                or self.prefill_seen_buffer.device != device):
            self.prefill_seen_buffer = torch.zeros((num_slots, ),
                                                   dtype=torch.int32,
                                                   device=device)
        else:
            self.prefill_seen_buffer.zero_()

        if (self.prefill_unique_output is None
                or self.prefill_unique_output.shape[0] < num_slots
                or self.prefill_unique_output.device != device):
            self.prefill_unique_output = torch.empty((num_slots, ),
                                                     dtype=torch.int32,
                                                     device=device)

        if (self.prefill_unique_count is None
                or self.prefill_unique_count.device != device):
            self.prefill_unique_count = torch.zeros((1, ),
                                                    dtype=torch.int32,
                                                    device=device)
        else:
            self.prefill_unique_count.zero_()
    ...
    def forward(
        ...
        if (use_fp8_cache and attn_metadata.num_prefill_tokens > 0):
            if kv_cache.numel() == 0:
                raise RuntimeError("Expected non-empty kv_cache for fp8_ds_mla"
                                   " prefill handling")
            num_slots = kv_cache.shape[0] * kv_cache.shape[1]
            ...
            prefill_token_mask = torch.zeros(num_actual_toks,
                                             dtype=torch.int32,
        ...
        if not use_fp8_cache:
            ...
            if num_prefill_tokens > 0:
                ...
                if (unique_prefill_indices is not None
