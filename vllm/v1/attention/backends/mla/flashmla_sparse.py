...
class FlashMLASparseMetadataBuilder(
    ...
    def build(
        ...
        if self.compress_ratio > 1:
            metadata.slot_mapping = get_compressed_slot_mapping(
        ...
        if self.is_deepseek_v4 and self.compress_ratio == 128:
            c128a_fields = self._build_c128a_metadata(
                common_attn_metadata, metadata.req_id_per_token
            )
            metadata.c128a_global_decode_topk_indices = c128a_fields.get(
                "c128a_global_decode_topk_indices"
            )
            metadata.c128a_decode_topk_lens = c128a_fields.get("c128a_decode_topk_lens")
            metadata.c128a_prefill_topk_indices = c128a_fields.get(
