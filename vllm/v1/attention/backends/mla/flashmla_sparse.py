...
class FlashMLASparseImpl(SparseMLACommonImpl[FlashMLASparseMetadata]):
    ...
    def forward_mqa(
        ...
        # DCP splits the KV across ranks and needs a rank-local filter on the
        # request-local indices each step, so it bypasses the cross-layer cache
        # and is handled inside _forward_fp8_kv_mixed_batch.
        if self.dcp_world_size > 1:
            topk_length = None
        else:
