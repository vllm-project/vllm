...
class MLACommonMetadataBuilder(Generic[M]):
    ...
    def __init__(
        self, runner: "GPUModelRunner", metadata_cls: Optional[type[M]] = None
    ):
        self.metadata_cls = (
        ...
        if self._num_prefills > 0:
            ...
            if (
                ...
                chunk_starts = (
                    ...
                    .unsqueeze(1)
                    .expand(-1, self._num_prefills)
                    * max_context_chunk
                )
                chunk_ends = torch.min(
                    context_lens.unsqueeze(0), chunk_starts + max_context_chunk
                )
                ...
                zero = torch.zeros(
                    num_chunks, dtype=torch.int32, device=device
                ).unsqueeze(-1)
...
class MLACommonImpl(MLAAttentionImpl[M], Generic[M]):
    ...
    def _q_proj_and_k_up_proj(self, x):
        if envs.VLLM_MLA_PERFORM_MATRIX_ABSORPTION:
            ...
            return torch.matmul(x, self.W_Q_UK).view(
                -1, self.num_heads, self.kv_lora_rank
            )
        else:
            x = torch.matmul(x, self.W_Q).view(
                -1, self.num_heads, self.qk_nope_head_dim
            )
            return torch.einsum("bnp,lnp->bnl", x, self.W_UK).view(
                -1, self.num_heads, self.kv_lora_rank
            )
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        ...
        def get_and_maybe_dequant_weights(layer: LinearBase):
            if not isinstance(layer.quant_method, UnquantizedLinearMethod):
                ...
                dequant_weights = layer.quant_method.apply(
                    layer, eye, bias=None
                )
                del eye
    ...
    def _forward_prefill(
        ...
        output = output.view(-1, self.num_heads, q.shape[-1])[
            ..., : v.shape[-1]
    ...
    def forward(
        ...
        if has_decode:
            ...
            decode_q_pe = torch.matmul(decode_hs_or_q_c, self.W_QR).view(
