...
# Conservative floor for amortizing the exchange latency. An H20 TP=4 sweep
# regressed at 512 rows/rank and became net-positive at 1024; the exact
# crossover is hardware-specific.
MIN_TP_SHARD_ROWS_PER_RANK = 1024


def _prefill_uses_mqa(max_seq_len: int, index_topk: int, force_mqa: bool) -> bool:
    """Whether sparse MQA consumes prefill Top-K indices."""
    return max_seq_len > index_topk or force_mqa
...
class DeepseekV32IndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(
        ...
        if num_prefills > 0:
            ...
            prefill_uses_mqa = _prefill_uses_mqa(
                prefill_max_seq_len,
                self.vllm_config.model_config.hf_config.index_topk,
                self.vllm_config.attention_config.sparse_mla_force_mqa,
            )
