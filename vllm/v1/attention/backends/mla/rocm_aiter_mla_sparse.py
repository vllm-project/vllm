...
class ROCMAiterMLASparseImpl(
    FusedQKRopeCacheMLAMixin,
    MLAAttentionImpl[ROCMAiterMLASparseMetadata],
    SharedTopkIndicesBuffer,
):
    is_sparse = True
    supports_dense_mha_prefill = False
    supports_dcp = False
    # Sparse routes every token through MQA, so the fused call covers the whole
    # batch and the wrapper can safely skip the eager RoPE.
    always_fuses_qk_rope_cache = True
