...
class _PinnedPlanWorkspaces:
    ...
    over from plans of the same shape. A full ring waits for its oldest copy.
    """
    ...
    def acquire(
        ...
        if ring[i][1].query():
            pass
        elif len(ring) >= self.MAX_BUFFERS:
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        # Whether the wrappers have the FlashInfer-internal buffers that
        # planning from the bounds relies on. Checked on first use.
        self._wrappers_have_plan_buffers: bool | None = None
        # Whether the prefill / decode wrapper last planned runs FlashInfer's
        # fa2 kernels, which read each request's KV length from the device.
        self._prefill_runs_fa2 = False
        self._decode_runs_fa2 = False
    ...
    def _seq_lens_cpu_from_bounds(
        ...
        The plan uses the upper bound. fa2 wrappers get the exact lengths on
        the device after plan(), so their rows may count a page too many,
        except rows with several query tokens: fa2 lays out split-KV from the
        planned lengths. Other FlashInfer kernels need exact bounds. TRTLLM
        reads the lengths from the device.
        """
        upper = common_attn_metadata.seq_lens_cpu_upper_bound
        lower = common_attn_metadata.seq_lens_cpu_lower_bound
        if upper is None or lower is None:
            return None
        if self._wrappers_have_plan_buffers is None:
            self._wrappers_have_plan_buffers = all(
                hasattr(wrapper, name)
                for wrapper in (
                    self._get_prefill_wrapper(),
                    self._get_decode_wrapper(0),
                )
                for name in (
        ...
        if not self._wrappers_have_plan_buffers:
            return None
        upper_np, lower_np = upper.numpy(), lower.numpy()
        ...
        all_exact = True
        for rows, uses_trtllm, runs_fa2 in (
            (slice(num_decodes), decode_uses_trtllm, self._decode_runs_fa2),
            (slice(num_decodes, None), prefill_uses_trtllm, self._prefill_runs_fa2),
        ):
            if uses_trtllm or exact[rows].all():
                continue
            if not (runs_fa2 and fa2_ok[rows].all()):
                return None
            all_exact = False
        return upper, all_exact

    def _plan(self, wrapper, plan: Callable[..., None], **kwargs) -> bool:
        """Run ``plan(**kwargs)`` for ``wrapper`` with a pinned buffer from its
        ring (see _PinnedPlanWorkspaces). Returns whether the wrapper runs fa2.

        A wrapper picks its kernel on the first plan and keeps it. The choice
        follows from the device, the dtypes and the head dims, so it is the same
        for every wrapper of this builder.
        """
        buf, event = self._plan_workspaces.acquire(
            wrapper, wrapper._pin_memory_int_workspace_buffer
        )
        wrapper._pin_memory_int_workspace_buffer = buf
        plan(**kwargs)
        event.record()
        return getattr(wrapper, "_backend", None) == "fa2"
    ...
    def _write_exact_last_page_len(self, wrapper, start: int, num: int) -> None:
        ...
        fa2 derives each request's KV length from its last-page length."""
    ...
    def build(
        ...
        if num_prefills > 0:
            ...
            if prefill_use_trtllm:
                ...
                if self.use_dcp:
                    ...
                    self._prefill_runs_fa2 = self._plan(
                        prefill_wrapper,
                        prefill_wrapper.plan,
        ...
        if num_decodes > 0:
            if decode_with_flashinfer_trtllm_api:
                ...
                self._decode_runs_fa2 = self._plan(
                    decode_wrapper,
