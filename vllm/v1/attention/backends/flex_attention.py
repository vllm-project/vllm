...
class FlexAttentionMetadata:
    ...
    def _build_block_mask_direct(self) -> BlockMask:
        ...
        # used_pages = self.block_table[self.doc_ids, :self.max_seq_len]
        ...
        # # WHY?
        # used_pages_padded_1 = used_pages_padded // page_to_block_ratio + 1
        # kv_indices = unique_static_unsorted((used_pages_padded_1.long()),
        #                                     M=self.num_blocks).to(torch.int32)
...
class FlexAttentionMetadataBuilder(
    ...
    def build(self,
              ...
              direct_build: bool = True) -> FlexAttentionMetadata:
        ...
        # For our direct block mask construction, we require that the unused
        # blocks in the block table are zeroed out. This is not needed for
        # the general create_block_mask path.
        # if direct_build:
        #     block_table_tensor_mask = torch.arange(
        #         block_table_tensor.shape[-1],
        #         device=block_table_tensor.device,
        #         dtype=block_table_tensor.dtype)[
        #             None, :] < num_blocks_per_seq[:, None]
        #     block_table_tensor = torch.where(block_table_tensor_mask,
