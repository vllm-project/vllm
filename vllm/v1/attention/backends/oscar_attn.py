...
#   [key_packed (kps bytes) | key_scale_zero | pad | value_packed | value_scale_zero]
#   For oscar_int2 head_dim=128: [32 bytes key | 4 bytes sz | 0 pad | 32 bytes value | 4 bytes sz] = 72
"""
...
class OscarAttentionImpl(AttentionImpl["OscarMetadata"]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Precompute cache layout parameters for OSCAR INT2 (head_size)
        self.k_q_bytes = self.head_size // 4
        self.sz_bytes = 4
        # 16-byte alignment
        self.k_aligned_size = (self.k_q_bytes + self.sz_bytes + 15) // 16 * 16
    ...
    def forward(
        ...
        if not attn_metadata.is_prefill:
            ...
            attn_out = self._decode_attention(
                q, kv_cache, attn_metadata, Pi, layer
            )
        elif num_decodes == 0:
            # Pure prefill batch
            k = key[:N].view(N, self.num_kv_heads, self.head_size)
            ...
            attn_out[:num_decode_tokens] = self._decode_attention(
                q[:num_decode_tokens], kv_cache, decode_meta, Pi, layer
            )
    ...
    def _store_kv(
        ...
        D = self.head_size
        k_q_bytes = self.k_q_bytes
        sz_bytes = self.sz_bytes
        k_aligned_size = self.k_aligned_size
    ...
    def _prefill_attention(
        ...
        # cu_seqlens are allocated on-demand above instead of caching _arange_cache.
