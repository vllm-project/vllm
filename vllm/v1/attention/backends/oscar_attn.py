...
Per-head per-position slot layout:
  ...
  For oscar_k3v4_nc head_dim=256: [100 bytes key | 512 bytes value] = 612
"""
...
from vllm.v1.attention.ops.triton_oscar_store import (
    quantized_set_kv_int2_oscar_rotate_k_clip_triton as triton_oscar_store,
)
...
class OscarAttentionBackend(AttentionBackend):
    ...
    def get_kv_cache_shape(
        ...
        This is safe because OSCAR has its own get_kv_cache_shape override and
        ...
        from the OSCAR config to ensure correct cache allocation for all head dims.
        """

        oscar_config = OscarConfig.from_cache_dtype(cache_dtype_str, head_size)
        return (num_blocks, block_size, num_kv_heads, oscar_config.slot_size_aligned)
...
class OscarMetadataBuilder(AttentionMetadataBuilder[OscarMetadata]):
    ...
    def build(self, common_prefix_len, common_attn_metadata, fast_build=False):
        """Build OscarMetadata from common attention metadata."""
...
class OscarAttentionImpl(AttentionImpl["OscarMetadata"]):
    """Oscar attention implementation.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.oscar_config = OscarConfig.from_cache_dtype(kv_cache_dtype, head_size)
    ...
    def _ensure_on_device(self, layer, device):
        """One-time derivation of OSCAR buffers (rotation matrix, midpoints).
        ...
        if not hasattr(layer, "_oscar_cached"):
            ...
            layer._oscar_Pi_half = H.to(torch.float16)

            # OSCAR INT2 uses uniform 4-level scalar quantization.
            # Centroids: [-1.5, -0.5, 0.5, 1.5] (symmetric around 0).
    ...
    def do_kv_cache_update(
        ...
        """Store compressed K/V into the combined OSCAR cache.
    ...
    def forward(
        ...
        # Get OSCAR buffers, ensure on device (one-time migration).
        # Use Any-typed alias for dynamic _oscar_* attrs set by _ensure_on_device.
        oscar_layer: Any = layer
        device = q.device
        self._ensure_on_device(oscar_layer, device)
        Pi = oscar_layer._oscar_Pi
        PiT = oscar_layer._oscar_PiT
        centroids = oscar_layer._oscar_centroids
    ...
    def _store_kv(
        ...
        Uses the OSCAR fused rotate(K) + clip + INT2-pack kernel.
        R_k is applied in-kernel; V is passed pre-rotated (identity rotation
        when real rotation files are not provided).
        """

        D = self.head_size
        # Cache layout (uint8): shape (num_blocks*block_size, Hk, slot_size_aligned)
        k_q_bytes = D // 4  # packed INT2 (4 values/byte)
        sz_bytes = 4  # 2 x fp16

        # We align the V start offset to 16 bytes to prevent illegal memory accesses
        # during 128-bit vectorized Triton stores.
        ...
        # Flatten spatial dims: (num_blocks, block_size, Hk, S)
        # -> (num_blocks*block_size, Hk, S)
        nb, bs, Hk, S = kv_cache.shape
        cache_flat = kv_cache.view(nb * bs, Hk, S)

        # Views into the sub-regions (uint8 arithmetic; fp16 views for sz)
        k_buf = cache_flat[:, :, k_start : k_start + k_q_bytes]  # (slots, Hk, D//4)
        ksz = cache_flat[:, :, ksz_start : ksz_start + sz_bytes]  # (slots, Hk, 4)
        v_buf = cache_flat[:, :, v_start : v_start + k_q_bytes]  # (slots, Hk, D//4)
        vsz = cache_flat[:, :, vsz_start : vsz_start + sz_bytes]  # (slots, Hk, 4)

        # Reinterpret sz as float16: each slot has [scale, zero]
        # ksz is (slots, Hk, 4) uint8. Reinterpreting to float16 gives (slots, Hk, 2).
        ksz_f16 = ksz.view(torch.float16).view(nb * bs, Hk, 2)
        vsz_f16 = vsz.view(torch.float16).view(nb * bs, Hk, 2)

        # R_k must match input dtype; cache the converted copy on layer.
        if (
            not hasattr(layer, "_oscar_R_k_cached")
            or layer._oscar_R_k_cached.dtype != key.dtype
        ):
            layer._oscar_R_k_cached = layer._oscar_Pi.to(key.dtype)
        R_k = layer._oscar_R_k_cached

        triton_oscar_store(
            cache_k_unrotated=key.contiguous(),
            cache_v_rotated=value.contiguous(),
            R_k=R_k,
            loc=slot_mapping,
    ...
    def _prefill_attention(
        ...
        # previously cached K/V from the OSCAR cache, not just the current
        # chunk's raw K/V.
        Hk = key.shape[1]
        use_gqa = Hk < Hq
        ...
        for i in range(num_reqs):
            ...
            if q_len == seq_len:
                ...
                # Continuation chunk: tokens already stored to OSCAR cache
                # by do_kv_cache_update.
                cached_len = seq_len - q_len
                if q_len == 1:
                    # Single-token continuation: use the fused decode kernel
                    # directly (avoids O(cached_len) dequant allocation).
                    d_seq_lens = torch.tensor(
                        [seq_len], dtype=torch.int32, device=device
                    )
                    d_block_table = attn_metadata.block_table[i : i + 1]
                    out = triton_oscar_decode_attention(
                        query=q_seq.unsqueeze(0) if q_seq.ndim == 2 else q_seq,
                        kv_cache=kv_cache,
                    ...
                    output[q_start:q_end] = out.squeeze(0).to(query.dtype)
                else:
                    # Multi-token continuation: dequant cache + flash_attn.
    ...
    def _continuation_prefill(
        ...
        """Handle continuation chunk by dequanting cached K/V from OSCAR cache."""
        ...
        # Generate slot mapping for the cached sequence
        ...
        # Dequantize cached tokens
        Pi_half = layer._oscar_Pi_half
        k_cached_trim, v_cached_trim = dequant_oscar_kv_cache(
            kv_cache, slot_mapping, Pi_half, D
        )
    ...
    # ------------------------------------------------------------------ #
    #  Decode: Triton OSCAR decode attention                                 #
    # ------------------------------------------------------------------ #
    @torch.compiler.disable
    def _decode_attention(
        self,
        query: torch.Tensor,  # (num_reqs, num_heads, head_size)
        kv_cache: torch.Tensor,  # (num_blocks, block_size, Hk, slot_size)
        attn_metadata: OscarMetadata,
        Pi: torch.Tensor,
        centroids: torch.Tensor,
