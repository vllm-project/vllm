    ...
    def _compute_common_metadata(
        ...
        *,
        ...
        if fast_split is not None:
            ...
            # First prompt chunks have no prior Mamba state and must stay
            # prefills.
            ...
            # The scheduler may pad a one-token remote prompt tail with
            # placeholder drafts to retain the uniform K+1 decode graph. This is
            # a speculative decode transaction even though the real token is
            # still in the prompt: the decode kernels keep h(N) in the running
            # slot and h(N+i) in scratch slots, so normal acceptance rollback
            # remains valid. The prefill kernels only return h(N+K) and cannot
            # roll the placeholders back.
