...
class DirectDCPA2AWorkspace:
    ...
    def lse_reduce(
        ...
        """Cross-rank LSE-weighted combine of per-rank partial MLA outputs.

        ``seq_lens`` holds this rank's local KV shard lengths (one entry per
        request; the token count must be a multiple of its length). Rows whose
        local shard is empty carry undefined attention output/LSE; the kernel
        masks them to a zero-weight contribution before any exp/max, and rows
        empty on every rank combine to zeros.
        """
