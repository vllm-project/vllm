...
is the core kernel), ``int4_per_token_head`` (packed write + split-dot read),
and the shared ``_hadamard`` transform. INT4 vs. everything-else is
dispatched explicitly in ``triton_reshape_and_cache_flash`` (write) and
``triton_unified_attention`` (read).
"""
