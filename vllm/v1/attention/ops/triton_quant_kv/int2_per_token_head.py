...
# ---------------------------------------------------------------------------
# Attention kernel (2D): 4-way split-dot, Lloyd-Max centroid lookup
# ---------------------------------------------------------------------------
@triton.jit
def _attn_int2_2d(
    output_ptr,
...
# ---------------------------------------------------------------------------
# Attention kernel (3D): same as 2D but with parallel softmax segments
# ---------------------------------------------------------------------------
@triton.jit
def _attn_int2_3d(
    segm_output_ptr,
...
class Int2PerTokenHeadBackend(QuantKVBackend):
    ...
    def unified_attention(
        ...
        *,
        ...
        if use_3d:
            _attn_int2_3d[(total_num_q_blocks, num_kv_heads, num_par_softmax_segments)](
            ...
            _attn_int2_2d[(total_num_q_blocks, num_kv_heads)](
                output_ptr=out,
                query_ptr=q,
                key_cache_ptr=k_cache,
                value_cache_ptr=v_cache,
