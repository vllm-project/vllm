...
def _fused_indexer_q_rope_quant_kernel(
    ...
    if INDEX_Q_NOPE_DIM > 0:
        tl.store(
            ...
            tl.div_rn(x_nope, index_q_scale).to(tl.float8e4b8 if IS_FNUZ else tl.float8e4nv),
        )
    fp8_rot_base = fp8_base_ptr + INDEX_Q_NOPE_DIM
    tl.store(
        fp8_rot_base + half_offset * 2,
        tl.div_rn(r_even, index_q_scale).to(tl.float8e4b8 if IS_FNUZ else tl.float8e4nv),
    )
    tl.store(
        fp8_rot_base + half_offset * 2 + 1,
        tl.div_rn(r_odd, index_q_scale).to(tl.float8e4b8 if IS_FNUZ else tl.float8e4nv),
    )
