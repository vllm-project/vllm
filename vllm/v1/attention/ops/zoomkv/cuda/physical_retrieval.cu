...
__global__ void quest_parent_score_physical_bf16_d128_kernel(
  ...
  // Lane-parallel load of up to 16 child physical ids into registers, then
  // broadcast via shfl so every lane can aggregate parent min/max.
  int32_t my_phys = -1;
  bool my_valid = false;
  if (lane < factor) {
...
__global__ void quest_sub_score_physical_bf16_d128_kernel(
  ...
  bool valid = child64 >= 0 && child64 < n_chunks;
  int32_t physical = -1;
  if (valid) {
...
__global__ void density_score_physical_bf16_d128_kernel(
  ...
  bool valid = chunk >= 0 && chunk < n_chunks;
  int32_t physical = -1;
  if (valid) {
    physical = physical_ids[static_cast<int64_t>(b) * pid_stride_b +
                            chunk * pid_stride_n];
...
__global__ void kivi_physical_kernel(PhysicalKiviParams p) {
  ...
  if (valid_slot) {
    ...
    if (logical_chunk >= 0 && logical_chunk < p.n_chunks) {
      physical =
...
#pragma unroll
  ...
  if (valid_slot) {
    const int out_base =
        (static_cast<int64_t>(b) * p.kv_heads + h) * p.nk * CHUNK_SIZE +
        static_cast<int64_t>(slot) * CHUNK_SIZE + lane_in_chunk;
...
void quest_parent_score_physical_cuda(
  ...
  if (q.scalar_type() == at::ScalarType::BFloat16 && dim == HEAD_DIM_128 &&
    ...
    quest_parent_score_physical_bf16_d128_kernel<<<grid, threads, 0, stream>>>(
        ...
        global_valid.data_ptr<bool>(), scores.data_ptr<float>(), batch, heads,
        ...
        "quest_parent_score_physical", [&] {
          quest_parent_score_physical_kernel<scalar_t>
              <<<grid, SCORE_THREADS, 0, stream>>>(
                  ...
                  global_valid.data_ptr<bool>(), scores.data_ptr<float>(),
                  batch, heads, dim, global_min.size(0), n_chunks, factor,
                  n_parent, physical_ids.stride(0), physical_ids.stride(1),
                  global_min.stride(0), global_min.stride(1), scores.stride(0),
                  scores.stride(1));
        });
  }
