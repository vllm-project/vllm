...
def _aligned_local_start(
    seq: torch.Tensor,
    sink_len: torch.Tensor,
    local_size: int,
    block_size: int,
) -> torch.Tensor:
...
def _aligned_local_start_scalar(
    seq_len: int,
    sink_len: int,
    local_size: int,
    block_size: int,
) -> int:
    """Scalar equivalent of :func:`_aligned_local_start`."""
    raw_start = max(sink_len, seq_len - local_size)
    return max(sink_len, (raw_start // block_size) * block_size)
...
def _validate_topk_gather_mapping(
    ...
    local_start = _aligned_local_start(
        seq, sink_len.view(batch), local_size, block_size
    ).view(batch, 1, 1)
...
def assemble_sparse_context_indices(
    ...
    *,
    ...
    sink_len = min(sink_size, seq_len)
    raw_local_start = max(sink_len, seq_len - local_size)
    base_local_len = seq_len - raw_local_start
    sink_local = build_sink_local_indices(
        seq_len, sink_size, local_size, device, block_size
    )
    kv, tk = topk_logical.shape
    # Keep the fast-path context width fixed. Boundary tokens replace the
    # lowest-ranked retrieved tokens instead of leaving a 1..block_size-1 gap.
    n_ctx = sink_len + base_local_len + tk
    ...
    topk_slots = max(0, n_ctx - sink_local.numel())
    toks = topk_logical[:, :topk_slots]
