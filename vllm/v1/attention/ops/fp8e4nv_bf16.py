...
# ---------------------------------------------------------------------------
# Encode: bf16 -> fp8e4m3  (write path). Rounding mode selected by round= :
#   "rn"    round-to-nearest, ties away (half-up); <=1 fp8-code ULP.
#   "rne"   round-to-nearest-even; bit-exact vs torch over all finite bf16.
...
# ---- encode: bf16 -> fp8e4m3, RN (round-nearest, half-up) ----
_BF16_TO_FP8E4M3_RN_ASM = """\
{
    .reg .u16 b<4>;
    .reg .u32 raw<4>;
...
_BF16_TO_FP8E4M3_RN_ASM = tl.constexpr(_BF16_TO_FP8E4M3_RN_ASM)
_BF16_TO_FP8E4M3_RNE_ASM = tl.constexpr(_BF16_TO_FP8E4M3_RNE_ASM)
_BF16_TO_FP8E4M3_TRUNC_ASM = tl.constexpr(_BF16_TO_FP8E4M3_TRUNC_ASM)
...
@triton.jit
def bf16_to_fp8e4m3(x, round: tl.constexpr):
    """4 bf16 -> 4 packed uint8 fp8e4m3 bytes (pack-4, saturating).
    ...
    if round == "rne":
        ...
        return tl.inline_asm_elementwise(
            _BF16_TO_FP8E4M3_RN_ASM, "=r,r,r", [x], dtype=tl.uint8, is_pure=True, pack=4
        )
    return tl.inline_asm_elementwise(
        _BF16_TO_FP8E4M3_TRUNC_ASM, "=r,r,r", [x], dtype=tl.uint8, is_pure=True, pack=4
    )
