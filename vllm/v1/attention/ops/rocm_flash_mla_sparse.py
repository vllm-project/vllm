...
def _batched_query_key(q: torch.Tensor, k: torch.Tensor) -> torch.Tensor:
    if _env_enabled("VLLM_DSV4_ROCM_SPARSE_MLA_BMM"):
        return torch.bmm(q, k.transpose(1, 2).contiguous())
    return torch.einsum("thd,tcd->thc", q, k)


def _batched_scores_value(scores: torch.Tensor, values: torch.Tensor) -> torch.Tensor:
    if _env_enabled("VLLM_DSV4_ROCM_SPARSE_MLA_BMM"):
...
def flash_mla_with_kvcache_rocm(
    ...
    * ``extra_k_cache`` + ``extra_indices_in_kvcache`` / ``extra_topk_length``
    ...
    if topk_length is not None:
        lens = topk_length.to(torch.long).view(-1, 1)
        arange = torch.arange(swa_idx.shape[-1], device=swa_idx.device).view(1, -1)
        swa_idx = swa_idx.masked_fill(arange >= lens, -1)
    ...
    if extra_k_cache is not None:
        ...
        if extra_topk_length is not None:
            lens = extra_topk_length.to(torch.long).view(-1, 1)
            arange = torch.arange(extra_idx.shape[-1], device=extra_idx.device).view(1, -1)
            extra_idx = extra_idx.masked_fill(arange >= lens, -1)
    else:
        extra_idx = None
