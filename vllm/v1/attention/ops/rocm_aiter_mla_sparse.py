...
def _use_aiter_native_paged_mqa() -> bool:
    ...
    The native deepgemm decode is used on gfx942/gfx950 whenever aiter exposes
    it. Otherwise we take the Triton fallback, which is correct at every block
    size and self-adapts to the cache layout via ``_indexer_cache_layout``.
    Set ``VLLM_ROCM_SPARSE_MLA_FORCE_TRITON=1`` to force the fallback (e.g. to
    validate a new aiter, or if a future aiter regresses).
    """
