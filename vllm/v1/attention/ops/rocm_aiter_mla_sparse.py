...
def _load_v41_nvfp4_gfx950_chunk(
    ...
    """One chunk of a V4.1 NVFP4 row: e2m1 pairs plus one e4m3 scale per 16
    dims. V4.1 RoPEs before quantizing, so there is no bf16 tail to splice in.
    """
    ...
    # Byte j holds dims 2j and 2j+1, so both take the scale of tile (j // 8):
    # a 16-dim tile starts on an even dim and never splits a byte.
    ...
    # gfx950 decodes a nibble pair in one instruction. Its scale operand is
    # E8M0, so this record's e4m3 scale is applied separately; the destination
    # is a register pair, so the result comes back as one 64-bit value whose
    # halves are the even and odd dim of the byte.
...
def _rocm_sparse_attn_decode_ragged_triton(
    ...
    # DeepSeek-V4.1 stores the compressed cache as NVFP4 (288 B/token) next to a
    # V4 SWA cache; the record width is what the KV-cache spec publishes.
    extra_fp4 = has_extra and extra_cache.shape[-1] == _V41_NVFP4_BYTES_PER_TOKEN
    # Only the gfx950 tile reads the NVFP4 record.
    assert not extra_fp4 or _ON_GFX950, (
        "NVFP4 compressed KV cache needs the gfx950 sparse decode path"
    )
