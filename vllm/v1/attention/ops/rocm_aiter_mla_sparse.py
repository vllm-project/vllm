...
def _use_aiter_native_paged_mqa() -> bool:
    ...
    decode on gfx942/gfx950, its stage1 form elsewhere. The Triton fallback,
    which is correct at every block size and self-adapts to the cache layout
    via ``_indexer_cache_layout``, is taken when aiter does not expose the
    module at all, and when ``VLLM_ROCM_SPARSE_MLA_FORCE_TRITON=1`` forces it
    -- the lever for validating a new aiter, or for working around a future
    aiter regression.
    """
...
def rocm_fp8_paged_mqa_logits(
    ...
    # Prefer aiter's native deepgemm decode whenever aiter exposes it; the
