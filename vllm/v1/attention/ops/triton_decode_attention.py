...
def _fwd_kernel_stage1(
    ...
    if split_kv_end > split_kv_start:
        for start_n in range(split_kv_start, split_kv_end, BLOCK_N):
            ...
            if k.dtype.is_fp8():
                k = (k.to(tl.float32) * tl.load(k_scale)).to(q.dtype)
            ...
            if v.dtype.is_fp8():
                v = (v.to(tl.float32) * tl.load(v_scale)).to(q.dtype)
    ...
    if split_kv_end > split_kv_start:
        for start_n in range(split_kv_start, split_kv_end, BLOCK_N):
            ...
            if BLOCK_DPE > 0:
                ...
                if kpe.dtype.is_fp8():
                    kpe = (kpe.to(tl.float32) * tl.load(k_scale)).to(qpe.dtype)
...
def decode_attention_fwd(
    ...
    # When k_scale/v_scale are None (e.g. standalone / test usage without
    # an AttentionLayer), provide a dummy 1.0 scalar so the Triton kernel
    # always receives a consistent pointer arg.  The kernel's compile-time
    # `if dtype.is_fp8()` check skips dequant for BF16 tensors, so the
    # scale value is never read in that case.
    if k_scale is None:
