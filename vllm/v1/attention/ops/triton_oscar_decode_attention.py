...
def triton_oscar_decode_attention(
    ...
    if (
        buf_holder is not None
        and hasattr(buf_holder, "_oscar_mid_o_buf")
        and buf_holder._oscar_mid_o_buf.shape[0] >= B
        and buf_holder._oscar_mid_o_buf.shape[2] >= NUM_KV_SPLITS
    ):
        mid_o = buf_holder._oscar_mid_o_buf[:B, :Hq, :NUM_KV_SPLITS, :]
    else:
        mid_o = torch.empty(
            B,
            Hq,
    ...
    if (
        buf_holder is not None
        and hasattr(buf_holder, "_oscar_output_buf")
        and buf_holder._oscar_output_buf.shape[0] >= B
        and buf_holder._oscar_output_buf.dtype == out_dtype
    ):
        output = buf_holder._oscar_output_buf[:B, :Hq, :D]
    else:
        output = torch.empty(B, Hq, D, dtype=out_dtype, device=device)
        if buf_holder is not None:
            buf_holder._oscar_output_buf = output

    if buf_holder is not None and hasattr(buf_holder, "_oscar_lse_buf") and buf_holder._oscar_lse_buf.shape[0] >= B:
        lse = buf_holder._oscar_lse_buf[:B, :Hq]
    else:
