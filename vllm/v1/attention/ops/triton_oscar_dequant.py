...
def _oscar_dequant_kernel(
    ...
    # out_k will reconstruct the correct layout:
    # 1) form [4, BLOCK_QUARTER] where the rows are v0, v1, v2, v3
    # 2) reshape to [4, BLOCK_QUARTER] (already is)
    # 3) permute to [BLOCK_QUARTER, 4]
    # 4) flatten to [D]
    
    d_offs = tl.arange(0, D)

    k_stacked = tl.zeros([4, BLOCK_QUARTER], dtype=tl.float32)
    k_stacked = tl.where(tl.arange(0, 4)[:, None] == 0, k_v0[None, :], k_stacked)
    k_stacked = tl.where(tl.arange(0, 4)[:, None] == 1, k_v1[None, :], k_stacked)
    k_stacked = tl.where(tl.arange(0, 4)[:, None] == 2, k_v2[None, :], k_stacked)
    k_stacked = tl.where(tl.arange(0, 4)[:, None] == 3, k_v3[None, :], k_stacked)
    
    k_permuted = tl.permute(k_stacked, (1, 0)) # [BLOCK_QUARTER, 4]
    out_k_sum = tl.reshape(k_permuted, [D])
    ...
    v_stacked = tl.zeros([4, BLOCK_QUARTER], dtype=tl.float32)
    v_stacked = tl.where(tl.arange(0, 4)[:, None] == 0, v_v0[None, :], v_stacked)
    v_stacked = tl.where(tl.arange(0, 4)[:, None] == 1, v_v1[None, :], v_stacked)
    v_stacked = tl.where(tl.arange(0, 4)[:, None] == 2, v_v2[None, :], v_stacked)
    v_stacked = tl.where(tl.arange(0, 4)[:, None] == 3, v_v3[None, :], v_stacked)
    
    v_permuted = tl.permute(v_stacked, (1, 0)) # [BLOCK_QUARTER, 4]
    out_v_sum = tl.reshape(v_permuted, [D])
