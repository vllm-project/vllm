...
class GPUModelRunner(
    ...
    def propose_draft_token_ids(
        ...
        # preprocess
        if spec_config.method == "ngram_gpu":
            (
                next_token_ids,
        ...
        elif spec_config.method in ["eagle", "dflash", "draft_model"]:
        ...
        extra_inputs = {
            ...
            # Ngram-gpu inputs
            "num_tokens_no_spec": self.num_tokens_no_spec_gpu[:batch_size],
            "token_ids_gpu": self.token_ids_gpu_tensor[:batch_size],
            "valid_sampled_token_ids_gpu": valid_sampled_token_ids_gpu,
        ...
        propose_input = self.drafter.prepare_inputs(
            sampled_token_ids, self.input_batch, **extra_inputs
        )
        # propose
        draft_token_ids, num_valid_draft_tokens = self.drafter.propose(propose_input)

        # postprocess
        if spec_config.method == "extract_hidden_states":
            next_token_ids, valid_sampled_tokens_count = (
                self.drafter.prepare_next_token_ids_padded(
                    self.optimistic_seq_lens_cpu,
                    sampled_token_ids,
