...
class GPUModelRunner(
    ...
    def __init__(
        ...
        self.gpu_cache_manager: GPUCacheManager | None = None
    ...
    def execute_model(
        ...
        with (
            ...
            if self.gpu_cache_manager is not None and self.gpu_cache_manager.enabled:
                block_tables_cpu_by_group = {
                    gid: self.input_batch.block_table[gid].get_cpu_tensor()
                    for gid, _ in enumerate(self.kv_cache_config.kv_cache_groups)
                }
                self.gpu_cache_manager.begin_batch(
                    num_reqs=num_reqs,
                    num_tokens=num_tokens_unpadded,
                    query_start_loc_cpu=self.query_start_loc.cpu,
                    seq_lens_cpu=self.seq_lens.cpu,
                    block_tables_cpu_by_group=block_tables_cpu_by_group,
        ...
        try:
            with (
                set_forward_context(
                    ...
                    gpu_cache_manager=self.gpu_cache_manager,
            ...
            if self.gpu_cache_manager is not None and self.gpu_cache_manager.enabled:
                self.gpu_cache_manager.end_batch()
    ...
    def _allocate_sparse_hot_kv_cache_tensors(
        self, kv_cache_config: KVCacheConfig
        ...
        if num_gpu_blocks is None or num_gpu_blocks <= 0:
            raise ValueError("Sparse hot KV cache requires positive num_gpu_blocks.")
        if num_cpu_blocks is None or num_cpu_blocks <= 0:
            raise ValueError("Sparse hot KV cache requires positive num_cpu_blocks.")

        gpu_raw_tensors: dict[str, torch.Tensor] = {}
        cpu_raw_tensors: dict[str, torch.Tensor] = {}
        for group in self._kv_cache_spec_attn_group_iterator():
            kv_cache_spec = group.kv_cache_spec
            if not isinstance(kv_cache_spec, AttentionSpec):
                raise NotImplementedError(
                    "Sparse hot-cache MVP only supports attention KV caches."
                )
            for layer_name in group.layer_names:
                if layer_name in self.runner_only_attn_layers:
                    continue
                gpu_raw_tensors[layer_name] = torch.zeros(
                    kv_cache_spec.page_size_bytes * num_gpu_blocks,
                    dtype=torch.int8,
                    device=self.device,
                )
                cpu_raw_tensors[layer_name] = torch.zeros(
                    kv_cache_spec.page_size_bytes * num_cpu_blocks,
                    dtype=torch.int8,
                    device="cpu",
                    pin_memory=self.pin_memory,
                )
        return gpu_raw_tensors, cpu_raw_tensors

    def _validate_sparse_hot_cache_support(
        self, kv_cache_config: KVCacheConfig, kernel_block_sizes: list[int]
    ) -> None:
        if self.cache_config.kv_offloading_size is not None:
            raise ValueError(
                "Sparse hot-cache MVP is not compatible with kv_offloading_size."
            )
        if self.use_async_scheduling:
            raise ValueError(
                "Sparse hot-cache MVP does not support async scheduling."
            )
        if self.parallel_config.use_ubatching:
            raise ValueError("Sparse hot-cache MVP does not support DBO/ubatching.")
        if self.compilation_config.cudagraph_mode != CUDAGraphMode.NONE:
            raise ValueError(
                "Sparse hot-cache MVP requires cudagraph_mode=NONE."
            )
        if self.model_config.is_encoder_decoder:
            raise ValueError(
                "Sparse hot-cache MVP only supports decoder-only models."
            )
        if self.shared_kv_cache_layers:
            raise ValueError(
                "Sparse hot-cache MVP does not support shared_kv_cache_layers."
            )
        if self.cache_config.kv_sharing_fast_prefill:
            raise ValueError(
                "Sparse hot-cache MVP does not support kv_sharing_fast_prefill."
            )
        if any(
            isinstance(group.kv_cache_spec, (EncoderOnlyAttentionSpec, MambaSpec))
            for group in kv_cache_config.kv_cache_groups
        ):
            raise ValueError(
                "Sparse hot-cache MVP only supports decoder full-attention KV groups."
            )

        for group in self._kv_cache_spec_attn_group_iterator():
            if not isinstance(group.kv_cache_spec, FullAttentionSpec):
                raise ValueError(
                    "Sparse hot-cache MVP only supports FullAttentionSpec layers."
                )
            if group.backend.get_name() != "FLASH_ATTN":
                raise ValueError(
                    "Sparse hot-cache MVP only supports the FlashAttention backend."
                )
            group_id = group.kv_cache_group_id
            if group_id >= len(kernel_block_sizes):
                raise ValueError("Missing kernel block size for sparse hot-cache group.")
            if kernel_block_sizes[group_id] != group.kv_cache_spec.block_size:
                raise ValueError(
                    "Sparse hot-cache MVP does not support kernel block splitting."
                )
    ...
    def initialize_kv_cache_tensors(
        ...
        self.cpu_kv_caches.clear()

        cpu_kv_caches: dict[str, torch.Tensor] | None = None

        # Try creating KV caches optimized for kv-connector transfers
        cache_dtype = self.cache_config.cache_dtype
        if self.cache_config.enable_sparse_hot_cache:
            gpu_kv_cache_raw_tensors, cpu_kv_cache_raw_tensors = (
                self._allocate_sparse_hot_kv_cache_tensors(kv_cache_config)
            )
            kv_caches = self._reshape_kv_cache_tensors(
                kv_cache_config, gpu_kv_cache_raw_tensors, kernel_block_sizes
            )
            cpu_kv_caches = self._reshape_kv_cache_tensors(
                kv_cache_config, cpu_kv_cache_raw_tensors, kernel_block_sizes
            )
            ...
            kv_caches, cross_layers_kv_cache, attn_backend = self.allocate_uniform_kv_caches(
                kv_cache_config,
                self.attn_groups,
                cache_dtype,
                self.device,
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        if self.cache_config.enable_sparse_hot_cache:
            self._validate_sparse_hot_cache_support(
        ...
        if self.cache_config.enable_sparse_hot_cache:
            ...
            for gid, kv_cache_group in enumerate(kv_cache_config.kv_cache_groups):
                for layer_name in kv_cache_group.layer_names:
                    layer_name_to_group[layer_name] = gid
            cpu_kv_cache_map = {
                layer_name: layer.cpu_kv_cache[0]
                for layer_name, layer in attn_layers.items()
            }
            self.gpu_cache_manager = GPUCacheManager(
                cache_config=self.cache_config,
                layer_names=tuple(attn_layers.keys()),
                num_gpu_blocks=kv_cache_config.num_gpu_blocks or kv_cache_config.num_blocks,
                num_cpu_blocks=kv_cache_config.num_cpu_blocks or kv_cache_config.num_blocks,
                block_size=self.cache_config.block_size,
                max_num_reqs=self.max_num_reqs,
                max_num_tokens=self.max_num_tokens,
                max_model_len=self.max_model_len,
            )
            self.gpu_cache_manager.attach_caches(
                kv_caches, cpu_kv_cache_map, layer_name_to_group
            )
        else:
