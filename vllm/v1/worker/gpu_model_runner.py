...
class GPUModelRunner(
    ...
    def propose_draft_token_ids(
        ...
        """
        Propose draft tokens using the unified SpecDecodeProposer interface.

        This method delegates all method-specific input preparation to the
        drafter's prepare_inputs() method, then calls propose() with the
        prepared inputs.
        """
        assert self.speculative_config is not None
        assert self.drafter is not None

        # Prepare inputs using the drafter's unified interface
        prepare_output = self.drafter.prepare_inputs(
        ...
        draft_token_ids = self.drafter.propose(prepare_output.inputs)

        # Run post-processing if needed (e.g., for extract_hidden_states)
        if prepare_output.post_process_fn is not None:
            prepare_output.post_process_fn(self)
