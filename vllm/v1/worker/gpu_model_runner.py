        ...
        try:
            with DeviceMemoryProfiler() as m:
                ...
                if getattr(self, "sampler", None) is not None:
                    # Preallocate sampler logits workspace inside the profiler
                    # block so its footprint is deducted from KV-cache bounds.
                    # Sampler.forward sees one row per request for normal
                    # decode/prefill, plus target rows for speculative tokens.
                    max_tokens = self.max_num_reqs * (1 + self.num_spec_tokens)
