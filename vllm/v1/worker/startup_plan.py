...
computes the ``--kv-cache-memory-bytes`` value that reproduces that
allocation.
...
def maybe_apply_startup_plan(worker: "Worker") -> None:
    """If enabled and ``--kv-cache-memory-bytes`` was not set explicitly, apply
    a persisted plan by setting ``worker.cache_config.kv_cache_memory_bytes``.
