...
class GPUModelRunner(LoRAModelRunnerMixin):
        ...
        with self.maybe_setup_dummy_loras(self.lora_config):
            with self.maybe_select_dummy_loras(
                ...
                num_scheduled_tokens=np.array(
                    # Provide a dummy scheduled token count array; size does not strictly matter
                    # here as prepare_tensors only checks `num_active_loras`.
                    [self.max_num_tokens],
                    dtype=np.int32,
                ),
                num_active_loras=self.lora_config.max_loras if self.lora_config else 0,
    ...
    def execute_model(
        ...
        if not dummy_run:
            ...
            if self.lora_config:
                # Assuming `_dummy_run` or `capture_model` has already wrapped the
                # execution with `maybe_select_dummy_loras` to set up `num_active_loras`.
                # We can just fetch the inputs that were generated by that context.
                lora_inputs = self.lora_state.make_lora_inputs(
                    input_batch.req_ids,
                    input_batch.idx_mapping_np,
                    input_batch.num_scheduled_tokens,
