...
class DSparkSpeculator(DFlashSpeculator):
    ...
    def set_attn(
        ...
        # A sliding-window drafter never attends context beyond each request's
        # last ``window`` tokens (and under the target's decoder-side replay no
        # hidden states exist beyond them), so only those rows get context KV.
        windows = [
    ...
    def _context_rows(self, input_batch: InputBatch) -> torch.Tensor | None:
        """Target rows that get context KV, or None for all (see set_attn)."""
        window = self.context_window
        if window is None:
            return None
        # Adaptive verification redistributes the leading decodes' tokens on the
        # GPU, but their span as a whole and the prefill boundaries after it are
        # as on the CPU; decodes are never trimmed, so the CPU rows hold.
