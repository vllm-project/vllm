...
class BaseSpeculator(ABC):
    ...
    def set_draft_sampling_params(
        self,
        # [max_num_reqs] persistent per-request top-k (vocab_size if unused)
        top_k: torch.Tensor,
        # [max_num_reqs] persistent per-request top-p (1.0 if unused)
        top_p: torch.Tensor,
    ) -> None:
        """Give the drafter the sampler's per-request top-k / top-p tensors for
        probabilistic draft sampling. Called once, before CUDA graph capture;
        the tensors are updated in place as requests come and go.
...
class DraftModelSpeculator(BaseSpeculator):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # Per-request top-k / top-p for probabilistic drafting: the sampler's
        # persistent [max_num_reqs] tensors, None until the model runner sets
        # them and for greedy drafting.
        self.draft_top_k: torch.Tensor | None = None
        self.draft_top_p: torch.Tensor | None = None
    ...
    def sample_draft(
        ...
        if draft_logits is not None:
            ...
            threshold = None
            if self.draft_top_k is not None and self.draft_top_p is not None:
                # Runs for every batch, with or without top-k / top-p, so that
                # a captured CUDA graph applies it too; rows without top-k are
                # skipped on the GPU.
                if self.draft_watermarker is None:
                    # The sampling kernel applies the threshold in its vocab pass.
                    ...
                    # The watermarker resamples from the logits, so mask them.
                    logits = apply_draft_top_k_top_p(
                        logits,
                        idx_mapping,
                        self.draft_top_k,
                        self.draft_top_p,
    ...
    def set_draft_sampling_params(
        ...
        if self.draft_logits is None:
            ...
            return
        self.draft_top_k = top_k
        self.draft_top_p = top_p
