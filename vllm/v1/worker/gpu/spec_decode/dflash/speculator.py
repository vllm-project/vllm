...
def context_kv_needs_eager(
    target_dp_sync: DPSyncState | None,
    dummy_run: bool,
    num_reqs: int,
    ...
    """Whether a drafter capturing the context-KV precompute must run this
    step eagerly.

    The captured graph precomputes exactly its own shape's context rows,
    which covers a real batch only at uniform decode: 1+N target tokens per
    request in both admitted layouts (``num_query_per_req`` plus one anchor
    row). Under DP the drafter reuses the target's sync and runs no collective
    of its own, so the decision must come from a value every rank holds
    alike: the target's agreed ``uniform_token_count``. It is None once any
    rank has a prefill or the ranks disagree, forcing eager everywhere; at
    pure uniform decode every rank sees 1+N and keeps the graph, idle ranks
    included (their dummy batch is decode-shaped). Without DP the rank's own
    batch decides, and dummy batches stay replayable (the caller PAD-fills
    their slots).
    """
    context_rows_per_req = num_query_per_req + (1 if sample_from_anchor else 0)
    if target_dp_sync is not None:
        return target_dp_sync.uniform_token_count != context_rows_per_req
    return not dummy_run and num_target_tokens != num_reqs * context_rows_per_req
...
class DFlashSpeculator(DraftModelSpeculator):
    ...
    def propose(
        ...
        batch_desc, batch_sync = dispatch_cg_and_sync_dp(
            ...
            # rows, which covers a real batch only at uniform decode. Decided
            # from the target's rank-agreed sync under DP (batch_sync reuses
            # it, so no collective reconciles per-rank votes here). Dummy
            # batches stay replayable: their slots are PAD-filled below.
            need_eager=is_profile
            or (
                self._context_kv_in_graph
                and context_kv_needs_eager(
                    dp_sync,
                    dummy_run,
                    num_reqs,
                    num_target_tokens,
