...
class DFlashSpeculator(DraftModelSpeculator):
    ...
    def propose(
        ...
        # A FULL replay never reads the metadata this builds; the build was
        # kept because it also refreshes the builders' persistent buffers, which
        # the captured graph reads by address. Graphs that refresh those buffers
        # themselves make the build, and the slot mappings only the eager path
        # consumes, dead work.
        refresh_in_graph = (
            batch_desc.cg_mode == CUDAGraphMode.FULL
            and self.query_cudagraph_manager is not None
            and self.query_cudagraph_manager.has_metadata_refresh(batch_desc)
        )
        draft_attn_metadata = None
        draft_slot_mappings_by_layer = None
        if not refresh_in_graph:
