...
def load_dflare_model(target_model: nn.Module, vllm_config: VllmConfig) -> nn.Module:
    ...
    causal = get_dflare_causal(draft_model_config)
    ...
    with set_model_tag("dflare_head"):
        dflare_model = get_model(
    ...
    draft_inner = dflare_model.model
    ...
    if get_pp_group().world_size == 1:
        ...
        if target_embed is not None and _should_share(
            dflare_model, "has_own_embed_tokens", draft_embed, target_embed
        ):
    ...
    draft_lm_head = getattr(dflare_model, "lm_head", None)
    if target_lm_head is not None and _should_share(
        dflare_model, "has_own_lm_head", draft_lm_head, target_lm_head
    ):
        if draft_lm_head is not None:
            del dflare_model.lm_head
        dflare_model.lm_head = target_lm_head
