...
def _logprob_token_ranks_kernel(
    ...
    req_idx = tl.program_id(0).to(tl.int64)
    token_idx = tl.program_id(1).to(tl.int64)
    row_ptr = logits_ptr + req_idx * logits_stride
...
def _drop_sampled_token_from_topk(
    ...
    n_keep = min(num_logprobs, topk_token_ids.shape[1] - 1)
    return (
        topk_token_ids.gather(1, perm)[:, :n_keep],
        topk_ranks.gather(1, perm)[:, :n_keep],
    )
