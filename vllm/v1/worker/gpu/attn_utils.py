...
def _allocate_kv_cache(
    ...
    for kv_cache_tensor in kv_cache_config.kv_cache_tensors:
        peer_block_stride = (
            _peer_cache_block_stride(kv_cache_tensor, kv_cache_config)
            if pcp_manager is not None and pcp_manager.peer_kv_enabled
            else 0
        )
        if kv_cache_tensor.block_stride > 0:
            ...
            if packed_backing is None:
                if pcp_manager is not None and pcp_manager.peer_kv_enabled:
                    packed_peer_allocation = pcp_manager.allocate_peer_cache(
                        kv_cache_tensor.size, peer_block_stride
                    )
            ...
            if pcp_manager is not None and pcp_manager.peer_kv_enabled:
                peer_allocation = pcp_manager.allocate_peer_cache(
                    kv_cache_tensor.size, peer_block_stride
                )
...
def _peer_cache_block_stride(
    kv_cache_tensor: KVCacheTensor,
    kv_cache_config: KVCacheConfig,
) -> int:
    """Return the byte stride that must divide each VMM rank segment."""
    if kv_cache_tensor.block_stride > 0:
        return kv_cache_tensor.block_stride

    page_sizes: set[int] = set()
    shared_layers = set(kv_cache_tensor.shared_by)
    for group in kv_cache_config.kv_cache_groups:
        for layer_name in shared_layers.intersection(group.layer_names):
...
def _init_kv_cache(
    ...
    if peer_allocations:
        assert pcp_manager is not None and pcp_manager.peer_kv_enabled
