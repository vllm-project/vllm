...
class RejectionSampler(nn.Module):
    ...
    def forward(
        ...
        if sampling_metadata.max_num_logprobs:
            ...
            final_logits[target_logits_indices] = (
                target_logits if self.return_processed_logprobs else raw_target_logits
            )
            final_logits[bonus_logits_indices] = (
                bonus_sampler_output.logprobs_tensors.logprobs.to(torch.float32)
            )
            ...
            logprobs_tensors = self.compute_logprobs(
                final_logits[accepted_logit_indices],
    ...
    def compute_logprobs(
        ...
        return self.sampler.gather_logprobs(
            logprobs, num_logprobs, token_ids.to(dtype=torch.int64)
        )
