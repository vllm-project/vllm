...
class TreeRejectionSampler(nn.Module):
    ...
    def __init__(
        ...
        # Precompute offsets for tree-decoding batches.
        num_tree_tokens = self.cu_tokens_per_level[-1]
        self.batch_offsets = self.batch_indices * num_tree_tokens
        ...
        self.tree_internal_indices = self.batch_offsets.unsqueeze(
            1) + torch.arange(num_tree_internal_tokens, device=device)
    ...
    def forward(
        ...
        Returns:
            ...
            accepted_token_indices (torch.Tensor):
        ...
        batch_offsets = self.batch_offsets[:num_tree_decodes]
        tree_internal_indices = self.tree_internal_indices[:num_tree_decodes]
        ...
        num_logits_per_batch = tree_internal_logits.shape[
            0] // num_tree_decodes
        ...
        output_token_ids[:num_tree_decodes, :draft_tree_size][
            draft_path_masks] = draft_token_ids[draft_path_masks]

        # Sample and add a bonus token to the accepted paths.
        bonus_logit_indices = batch_offsets + accepted_paths
        bonus_target_logits = target_logits[bonus_logit_indices]
        ...
        accepted_token_indices = torch.where(path_masks.view(-1))[0]
