...
class ThinkingBudgetStateHolder:
    ...
    def _apply_forcing_to_logits(
        ...
        # avoid per-iteration scalar writes to GPU tensors (each of which
        # forces a synchronizing H2D copy).
        ...
        if active_indices_cpu:
            ...
            # Build the fill tensor on-device via `torch.full` (scalar-fill
            # kernel, no H2D) so `index_put_` has a tensor value and won't
            # need to upload a CPU scalar.
