...
class TopKTopPKernel(VllmTritonJitKernel["TopKTopPKernel.CompileKey"]):
    ...
    def dispatch(  # type: ignore[override]
        ...
        *,
        ...
        topk_enabled: bool,
        topp_enabled: bool,
        use_large_tiles: bool,
        split_enabled: bool,
    ) -> CompileKey:
        return self.CompileKey(
            logits_stride=triton_scalar_specialization_rep(logits_stride),
            ...
            split_covers_ponly=split_enabled
            and topp_enabled
    ...
    def get_warmup_keys(self, vllm_config: Any) -> list[CompileKey]:
        ...
        return self._trace_dispatch(self.dispatch)(
            zip_inputs(
                dict(topk_enabled=True, topp_enabled=False),
                dict(topk_enabled=True, topp_enabled=True),
                dict(topk_enabled=False, topp_enabled=True),
            ),
            logits_stride=(16, 2),
            batch_size=WarmupIntRange(1, max_batch_size + 1),
            vocab_size=vocab_size,
            use_large_tiles=use_large_tiles,
    ...
    def warmup_inputs(self, compile_key: CompileKey) -> dict[str, Any]:
        logits = TritonWarmupTensor(
            torch.float32,
            shape=(1, compile_key.vocab_size),
            strides=(compile_key.logits_stride, 1),
        )
        return dict(
            logits=logits,
            logits_stride=compile_key.logits_stride,
            ...
            block_size_trunc=compile_key.block_size_trunc,
            topk_enabled=compile_key.topk_enabled,
            topp_enabled=compile_key.topp_enabled,
            split_covers_ponly=compile_key.split_covers_ponly,
...
class TopPSplitStatsKernel(
    ...
    def get_warmup_keys(self, vllm_config: Any) -> list[CompileKey]:
        ...
        return self._trace_dispatch(self.dispatch)(
            logits_stride=(16, 2),
            batch_size=WarmupIntRange(
                1,
                min(vllm_config.scheduler_config.max_num_seqs, _SPLIT_MAX_BATCH) + 1,
            ),
    ...
    def warmup_inputs(self, compile_key: CompileKey) -> dict[str, Any]:
        ...
        return dict(
            ...
            k=TritonWarmupTensor(torch.int32) if compile_key.has_k else logits,
            p=TritonWarmupTensor(torch.float32),
            has_k=compile_key.has_k,
            vocab_size=compile_key.vocab_size,
            splits=compile_key.splits,
        )
...
class TopPSplitStepKernel(VllmTritonJitKernel["TopPSplitStepKernel.CompileKey"]):
    ...
    def dispatch(  # type: ignore[override]
        ...
        *,
        ...
        return self.CompileKey(
            logits_stride=triton_scalar_specialization_rep(logits_stride),
            round=triton_scalar_specialization_rep(round),
            has_k=has_k,
            vocab_size=vocab_size,
            splits=_topp_split_count(batch_size, num_sm),
        )
    ...
    def get_warmup_keys(self, vllm_config: Any) -> list[CompileKey]:
        ...
        return self._trace_dispatch(self.dispatch)(
            ...
            num_sm=num_compute_units(),
            round=WarmupIntRange(0, _SPLIT_ROUNDS),
            has_k=(False, True),
            vocab_size=vocab_size,
        )
