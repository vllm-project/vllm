...
def dry_core(
    ...
    Args:
      ...
      max_exp: [R] int64 per-request exponent ceiling.
      mult: [R] float32 per-request multiplier, already rounded through
        float32, as llama.cpp stores it.
      base: [R] float32 per-request base, rounded the same way.
    ...
    # host int now that the caller computes it. The routing predicate in
    # DryState.apply is what actually bounds it - this assert is a second reader's
    # sanity check, and -O erases it - but it is free and it fails loudly rather
    # than wrapping a run length.
    ...
    # this path exists to avoid, so nothing checks them. Nothing has to: use_dry() is
    # False for a base below 1.0, so add_request leaves use_dry[req_idx] False and
    # DryState.apply builds no row for that request, and validate_params rejects a
    # negative multiplier at admission. use_dry's docstring argues both.
    ...
    if any_breakers:
        ...
        # on it. For a row that has a mask the stack costs one byte per (row, vocab)
        # entry next to the 4 the penalty accumulator already costs, and copies a mask
        # that is cached and resident anyway (DryState._breaker_masks). A row with no
        # breakers has nothing cached and gets a fresh [vocab] zeros here every step,
        # so a batch mixing the two costs up to two bytes per (row, vocab); that case
        # is unmeasured.
        bmask = torch.stack(
            [
                bm
    ...
    # requests enabled DRY. Those figures are measured, not derived: the full-width
    # version was profiled next to this one. It is not in the tree, so the comparison
