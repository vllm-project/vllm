...
def dry_core(
    ...
    Args:
      ...
      max_exp: [R] int64 per-request exponent clamp.
      mult: [R] float32 per-request multiplier (already rounded
        through float32, as llama.cpp stores them).
      base: [R] float32 per-request base (likewise float32-rounded).
    ...
    # this path exists to avoid. DryState.apply checks them on the host instead.
