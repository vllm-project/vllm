...
class OffloadingWorker:
    ...
    def __init__(self, completion_queue: CompletionQueue,
                 transfer_type: TransferType, transfer_fn: TransferFunction):
        ...
        logger.info("Started thread for transfer type %r", transfer_type)

    def run(self):
        while True:
            job_id, transfer_spec = self.submission_queue.get()
            if self._shutdown_event.is_set():
                logger.info("Thread for transfer type %r finished",
                            self.transfer_type)
                return
