...
def _get_configs() -> list:
    """Autotune grid over (BLOCK_Q, BLOCK_D); `_prune_configs` filters by
    SMEM budget and Lq at runtime, so one list is safe across GPUs."""
    return [
...
def _smem_budget() -> int:
    """Per-block opt-in shared-memory budget of the active device (bytes):
    Triton driver query, else a conservative capability table."""
    try:
