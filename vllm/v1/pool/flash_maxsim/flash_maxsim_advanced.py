...
def _maxsim_splitk_phase1(
    ...
    for q_start in range(0, Lq, BLOCK_Q):
        ...
        for d_start in range(split_start, split_end, BLOCK_D):
            ...
            D_ptrs = (D_ptr + doc_id64 * stride_d_b
        ...
        partial_ptrs = (partial_ptr + doc_id64 * stride_p_b
                        + split_id * stride_p_s
                        + q_off * stride_p_q)
        tl.store(
            partial_ptrs,
...
def _maxsim_splitk_phase2(
    ...
    for q_start in tl.static_range(0, Lq, BLOCK_Q):
        ...
        for s in tl.static_range(0, num_splits):
            pm_ptrs = (partial_ptr + doc_id64 * stride_p_b
                       + s * stride_p_s + q_off * stride_p_q)
            pm = tl.load(
                pm_ptrs,
