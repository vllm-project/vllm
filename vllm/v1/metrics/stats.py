...
class IterationStats:
    ...
    def update_from_finished_request(
        ...
        iv = compute_timing_intervals(req_stats, req_stats.num_generation_tokens)

        def _z(value: float | None) -> float:
            # FinishedRequestStats fields feed Prometheus histograms, which
            # require floats; None (unmeasured) becomes 0.0.
            return value if value is not None else 0.0

        queued_time = _z(iv.queue)
        prefill_time = _z(iv.prefill)
        decode_time = _z(iv.decode)
        inference_time = _z(iv.inference)
        mean_time_per_output_token = _z(iv.mean_per_output_token)
