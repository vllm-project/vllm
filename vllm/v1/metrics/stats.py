...
class PrefillStats:
    ...
    def finalize(self, num_cached_tokens_after: int) -> None:
        assert num_cached_tokens_after >= 0
        # vLLM doesn't currently deduplicate cached blocks, so tokens excluded
        # from num_cached_tokens may still be cached again. Revisit if cache dedupped.
        self.num_cache_creation_tokens = max(
            0, num_cached_tokens_after - self.num_cached_tokens
        )
