...
def interval(start_ts: float, end_ts: float) -> float:
    """Duration between two engine-core timestamps.

    The timestamps in ``RequestStateStats`` default to ``0.0`` and are only
    filled in when the corresponding event fires, so an unset field means
    "this never happened" rather than "this happened at time zero" -- the same
    sentinel ``update_from_events`` uses to ignore preemptions.  Subtracting
    against an unset endpoint yields a value on the order of the host's
    uptime, in whichever direction the sentinel happens to fall on.  Such an
    interval is undefined rather than very large, so report it as zero.
    """
...
class IterationStats:
    ...
    def update_from_finished_request(
        ...
        queued_time = interval(req_stats.queued_ts, req_stats.scheduled_ts)

        # Prefill interval is from first SCHEDULED to first NEW_TOKEN
        # Any preemptions during prefill is included in the interval
        prefill_time = interval(req_stats.scheduled_ts, req_stats.first_token_ts)

        # Decode interval is from first NEW_TOKEN to last NEW_TOKEN
        # Any preemptions during decode are included
        decode_time = interval(req_stats.first_token_ts, req_stats.last_token_ts)

        # Inference interval is from first SCHEDULED to last NEW_TOKEN
        # Any preemptions during prefill or decode are included
        inference_time = interval(req_stats.scheduled_ts, req_stats.last_token_ts)
