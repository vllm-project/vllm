...
def _normalize_metric_value(value: object) -> ExternalMetricValue:
    ...
    if isinstance(value, int) and not isinstance(value, bool):
        if not -(2**63) <= value <= 2**64 - 1:
            raise ValueError("External metrics integer values must fit in MessagePack.")
        return value
...
def register_external_metrics_provider(
    ...
    *,
    ...
    collection interval keeps provider work off the per-step hot path. The
    scheduler also collects a final snapshot before becoming idle.
    """
...
def collect_external_metrics(
    *, now: float | None = None, force: bool = False
    ...
    last snapshot they received for each provider. ``force`` bypasses the
    interval for the final snapshot when the scheduler becomes idle.
    """
    ...
    with _providers_lock:
        for name, state in _providers.items():
            if not force and collection_time < state.next_collection_time:
                continue
