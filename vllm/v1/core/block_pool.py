...
class BlockPool:
    ...
    def cache_full_blocks(
        ...
        new_hashes: list[int] | None = [] if self.enable_kv_cache_events else None
        for i, blk in enumerate(new_full_blocks):
            ...
            self.cached_block_hash_to_block[block_hash_with_group_id][blk.block_id] = (
                blk
            )
    ...
    def _maybe_evict_cached_block(self, block: KVCacheBlock) -> bool:
        ...
        if self.enable_kv_cache_events:
            ...
            self.kv_event_queue.append(
                BlockRemoved(
                    block_hashes=[block_hash.get_hash_value()], medium=MEDIUM_GPU
                )
            )
        return True
