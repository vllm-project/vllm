...
class HybridKVCacheCoordinator(KVCacheCoordinator):
    ...
    def find_longest_cache_hit(
        ...
        def _get_block_hashes(kv_cache_spec: KVCacheSpec) -> BlockHashList:
            logical_block_size = self._logical_block_size(kv_cache_spec)
            if logical_block_size == self.hash_block_size:
                return block_hashes
            return BlockHashListWithBlockSize(
                block_hashes, self.hash_block_size, logical_block_size
            )
        ...
        while True:
            ...
            for idx, group in enumerate(self.attention_groups):
                ...
                if isinstance(spec, FullAttentionSpec) and cached_blocks is not None:
                    ...
                    curr_hit_length = (
                        curr_hit_length // logical_block_size * logical_block_size
                    )
                    continue
                ...
                if drop_eagle_block:
                    ...
                    _max_length = min(
                        curr_hit_length + logical_block_size,
                ...
                _new_hit_length = len(hit_blocks[0]) * logical_block_size
                if drop_eagle_block:
        ...
        if isinstance(first_group.spec, FullAttentionSpec):
            num_blocks = hit_length // self._logical_block_size(first_group.spec)
