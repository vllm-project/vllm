...
class KVCacheCoordinator(ABC):
    ...
    def __init__(
        ...
        for i, kv_cache_group in enumerate(self.kv_cache_config.kv_cache_groups):
            ...
            group_dcp = 1 if is_replicated else dcp_world_size
            group_pcp = 1 if is_replicated else pcp_world_size
            managers.append(
                get_manager_for_kv_cache_spec(
                    kv_cache_spec=spec,
                    ...
                    enable_caching=enable_caching,
                    kv_cache_group_id=i,
                    dcp_world_size=group_dcp,
                    pcp_world_size=group_pcp,
...
class HybridKVCacheCoordinator(KVCacheCoordinator):
    ...
    def __init__(
        ...
        self.group_dcp_world_sizes: list[int] = []
        self.group_pcp_world_sizes: list[int] = []
        for g in kv_cache_config.kv_cache_groups:
            spec = g.kv_cache_spec
            if isinstance(spec, SlidingWindowSpec):
                self.group_dcp_world_sizes.append(1)
                self.group_pcp_world_sizes.append(1)
            elif isinstance(spec, FullAttentionSpec):
                self.group_dcp_world_sizes.append(dcp_world_size)
                self.group_pcp_world_sizes.append(pcp_world_size)
            else:
                assert dcp_world_size == 1 and pcp_world_size == 1, (
                    f"DCP/PCP not supported for {type(spec).__name__}"
                )
                self.group_dcp_world_sizes.append(1)
                self.group_pcp_world_sizes.append(1)
    ...
    def find_longest_cache_hit(
        ...
        def _get_block_hashes(target_block_size: int) -> BlockHashList:
        ...
        while True:
            ...
            for idx, (spec, group_ids, manager_cls, use_eagle) in enumerate(
                ...
                group_dcp = self.group_dcp_world_sizes[group_ids[0]]
                group_pcp = self.group_pcp_world_sizes[group_ids[0]]
                effective_block_size = spec.block_size * group_dcp * group_pcp
                ...
                hit_blocks = manager_cls.find_longest_cache_hit(
                    block_hashes=_get_block_hashes(effective_block_size),
        ...
        if isinstance(first_group.spec, FullAttentionSpec):
            trunc_dcp = self.group_dcp_world_sizes[first_group.group_ids[0]]
            trunc_pcp = self.group_pcp_world_sizes[first_group.group_ids[0]]
            trunc_block_size = first_group.spec.block_size * trunc_dcp * trunc_pcp
            num_blocks = hit_length // trunc_block_size
    ...
    def find_longest_cache_hit_per_group(
        ...
        for spec, group_ids, manager_cls, use_eagle in self.attention_groups:
            ...
            group_hit = len(blocks[0]) * effective_block_size
