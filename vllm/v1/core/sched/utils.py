...
def determine_stop_status(
    request: Request, max_model_len: int
) -> RequestStatus | None:
    ...
    if (
        ...
        return RequestStatus.FINISHED_LENGTH_CAPPED
    ...
    if repetition_detection is not None and (
        ...
        return RequestStatus.FINISHED_REPETITION

    return None


def check_stop(request: Request, max_model_len: int) -> bool:
    """Compatibility wrapper; Scheduler uses determine_stop_status for Q2 v3."""
    status = determine_stop_status(request, max_model_len)
    if status is None:
        return False
