...
def self_test():
    ...
    def make(flags):
        ...
        for b, cached in zip(blocks, flags):
            if cached:
                b.set_block_hash(b'prototype-' + str(b.block_id).encode())
        return FreeKVCacheBlockQueue(blocks), blocks

    def snapshot(q, bs):
        def bid(b):
            return None if b is None else b.block_id
        return (q.num_free_blocks, bid(q.fake_free_list_head.next_free_block),
                bid(q.fake_free_list_tail.prev_free_block),
                tuple((b.block_id, b.ref_cnt, b.block_hash,
                       bid(b.prev_free_block), bid(b.next_free_block)) for b in bs))

    cases = [
        ('current chunk fits', [0,0,1,1], 2, set(), 512, 'no_exposure_in_snapshot'),
        ('full prompt exposes cache', [0,0,1,1], 3, set(), 512, 'cached_block_exposed'),
        ('own hit excluded', [1,0,0], 2, {0}, 512, 'no_exposure_in_snapshot'),
        ('other cached block still exposed', [1,0,1], 2, {0}, 512, 'cached_block_exposed'),
