...
class RiskPerfScheduler(Scheduler):
    ...
    def enable_risk_policy(self):
        self._qwenfuse_risk_enabled = False
        if not self.cache_aware_window:
            return
        groups = self.kv_cache_config.kv_cache_groups
        if (
            len(groups) != 1
            or not isinstance(groups[0].kv_cache_spec, FullAttentionSpec)
            or self.connector is not None
            or (self.lora_config is not None)
            or self.is_encoder_decoder
            or self.num_spec_tokens
            or self.prefix_replay_tokens
            or self.num_lookahead_tokens
            or self.scheduler_config.async_scheduling
        ):
            raise ValueError(
                "Risk experiment requires synchronous single-group full attention, "
                "no KV connector, LoRA, encoder, speculation or prefix replay"
            )
        self._qwenfuse_risk_enabled = True
    ...
    def _qwenfuse_risk_reorder(
        ...
        result = predict_fresh(
            ...
            self.kv_cache_manager.block_pool.free_block_queue,
            self.block_size,
        )
        if result is None:
            return False
