...
def pack_scheduler_output_for_execute_model_fast_path(
    ...
    """Pack SchedulerOutput as positional tuples for MultiprocExecutor RPC.

    MultiprocExecutor serializes execute_model broadcasts with this fixed schema
    instead of pickling the dataclass object graph directly. array('i') is used
    for int lists to speed up pickle/unpickle on the TP hot path.
    """
    ...
    def pack_kv_connector_block_state(
        ...
        return (
            value.req_ids,
            {
                req_id: pack_block_ids(value.resolve_block_ids(req_id))
                for req_id in value.req_ids
            },
...
def unpack_scheduler_output_from_execute_model_fast_path(
    ...
    def unpack_kv_connector_block_state(value):
        if value is None:
            return None
        req_ids, block_ids_payload, boundary_state_offloads = value
        block_ids = {
