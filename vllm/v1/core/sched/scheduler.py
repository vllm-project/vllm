...
class Scheduler(SchedulerInterface):
    ...
    def reset_prefix_cache(
        ...
        """
        # Free GPU blocks for requests whose async KV store transfers were
        # synchronously flushed by the worker before this call.
        if flushed_sending_req_ids:
            flushed_output = KVConnectorOutput(
                finished_sending=flushed_sending_req_ids,
            )
            if self.connector is not None:
                self.connector.update_connector_output(flushed_output)

                # Publish any KV cache events generated by the flush
                # (e.g. BlockStored with medium="CPU") so that external
                # subscribers see them before the prefix cache resets.
                connector_events = list(self.connector.take_events())
                if connector_events:
                    self.kv_event_publisher.publish(
                        KVEventBatch(ts=time.time(), events=connector_events)
                    )

            for req_id in flushed_sending_req_ids:
