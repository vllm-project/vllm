    ...
    def __init__(
        ...
        if speculative_config is not None:
            ...
            if speculative_config.uses_draft_model():
                self.num_lookahead_tokens = self.num_spec_tokens
            if speculative_config.use_dflash() or speculative_config.use_orthrus():
                # Block-diffusion drafters reserve one extra query for the last
                # sampled token, plus one query for each speculative token.
