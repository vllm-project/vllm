    ...
    def _mamba_block_aligned_split(
        ...
        if num_computed_tokens < last_cache_position:
            ...
            if num_computed_tokens % block_size != 0:
                ...
                chunk_end = min(
                    chunk_end, round_down(num_computed_tokens, block_size) + block_size
                )
        elif (
            self.mamba_partial_cache_hit
            and num_computed_tokens
            < (
                tail_boundary := round_down(
            ...
            # Prefill of the final partial block: stop once at the prompt's
            # last hash boundary so the mamba partial tail entry can be
            # registered.
            chunk_end = tail_boundary
