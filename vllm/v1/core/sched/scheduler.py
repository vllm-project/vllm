...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        self.tier_scoped_clear_events = (
            self.kv_events_config is not None
            and self.kv_events_config.enable_kv_cache_events
            and self.kv_events_config.tier_scoped_clear_events
        )
    ...
    def _publish_kv_cache_events(self, all_tiers_cleared: bool = False) -> None:
        ...
        if all_tiers_cleared and self.enable_kv_cache_events:
            events = [
                event
        ...
        elif events and not self.tier_scoped_clear_events:
            # Legacy wire contract: a scoped clear is widened to the all-tier
            # event so subscribers that predate `TierBlocksCleared` still see
            # the (over-broad) invalidation instead of dropping the batch.
            events = [
                AllBlocksCleared() if isinstance(event, TierBlocksCleared) else event
                for event in events
            ]
