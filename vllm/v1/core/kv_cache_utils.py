...
from vllm.models.deepseek_v4.nvidia import kv_cache_planner_utils
...
def estimate_max_model_len(
    ...
    def fits_in_memory(model_len: int) -> bool:
        ...
        memory_needed = kv_cache_planner_utils.max_memory_usage_bytes(
...
def check_enough_kv_cache_memory(
    ...
    """
    Check whether `available_memory` is enough for the KV cache to hold at
    ...
    if kv_cache_spec:
        kv_cache_planner_utils.check_enough_kv_cache_memory(
            available_memory,
            lambda: kv_cache_planner_utils.max_memory_usage_bytes(
                vllm_config, kv_cache_spec.values()
            ),
...
def get_kv_cache_configs(
    ...
    """Generate per-worker KV cache configs using the resolved planner."""
