...
def _get_kv_cache_bytes_per_block(kv_cache_groups: list[KVCacheGroupSpec]) -> int:
    """Return the largest cache group's bytes per block.

    This is the layout bytes one global block ID covers, used to size the
    shared KV cache backing allocation.
    """
    bytes_per_block = max(
        sum(
            _get_per_layer_spec(group, layer_name).page_size_bytes
...
class DefaultKVCacheConfigBuilder(KVCacheConfigBuilder):
    ...
    def _get_max_memory_usage_bytes_from_groups(
        ...
        if (glm5_layout := _glm5_next_tensor_layout(kv_cache_groups)) is not None:
            ...
            if tail_names:
                total_blocks += 1
            return total_blocks * self._get_pool_bytes_per_block(
                vllm_config, kv_cache_groups
            )
