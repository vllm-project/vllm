...
class KVCacheManager:
    ...
    def __init__(
        ...
        # A Block pool of all gpu kv-cache blocks.
        self.block_pool: List[KVCacheBlock] = [
            KVCacheBlock(idx) for idx in range(num_gpu_blocks)
        ]
        # A block pool of all cpu kv-cache blocks.
        ...
        # {block_hash: cpu block}. Cached cpu blocks are evicted gpu blocks.
        # Unlike the gpu counter part, it has no duplicate blocks.
    ...
    def get_computed_blocks(
        ...
        Returns:
            A tuple containing:
                ...
                - The number of computed tokens including both gpu and cpu blocks.
        """
        ...
        for block_hash in block_hashes:
            ...
            elif cached_cpu_block := self._get_cached_cpu_block(block_hash):
    ...
    def allocate_slots(
        ...
        Args:
            ...
            new_computed_blocks: A list of new computed blocks just hitting the
                prefix caching.
            new_computed_cpu_blocks: A list of new computed cpu blocks just hitting
                the prefix caching.
        ...
        Prefill:
        -----------------------------------------------------------------------------
        | < new computed > | < new computed cpu > |  < new >     | < pre-allocated > |
        -----------------------------------------------------------------------------
        |                   < required >                         |
        ---------------------------------------------------------
        |                   < full >                            |
        ...
        # Swap in the cpu blocks. Metadata update happens in cache_full_blocks below.
        ...
        self.step_h2d_swap_map.update({
            ...
            for cpu_block, new_block in zip(new_computed_cpu_blocks, new_blocks)
        })
        ...
        num_computed_full_blocks_on_device = (
            request.num_computed_tokens // self.block_size +
            len(new_computed_blocks))
        new_full_blocks = req_blocks[
            num_computed_full_blocks_on_device:num_full_blocks]
        if new_full_blocks:
            self._cache_full_blocks(
                request=request,
                blk_start_idx=num_computed_full_blocks_on_device,
                # The new full blocks are the full blocks that are not computed.
                full_blocks=new_full_blocks,
                prev_block=(req_blocks[num_computed_full_blocks_on_device -
                                       1] if num_computed_full_blocks_on_device
                            > 0 else None),
            )
    ...
    def _get_new_cpu_block(self) -> Optional[KVCacheBlock]:
        ...
        while self.next_available_cpu_block_id in self.step_cpu_block_in_use:
            self.next_available_cpu_block_id = (
                self.next_available_cpu_block_id + 1) % len(self.cpu_block_pool)
    ...
    def _maybe_evict_cached_block(self,
                                  block: KVCacheBlock,
                                  swap_out: bool = False) -> bool:
        """
        If a block is cached in `cached_block_hash_to_block`, we reset its hash
        metadata and evict it from the cache. Optionally, we can swap out the block
        to cpu cache.
