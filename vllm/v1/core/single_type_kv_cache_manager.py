    ...
    def remove_skipped_blocks(self, request_id, num_computed_tokens):
        if not envs.VLLM_USE_LIGHTER_MAMBA_CACHE:
            ...
            if num_blocks > 2 + self.kv_cache_spec.num_speculative_blocks:
                ...
                assert (
                    num_blocks == 0
                    or num_blocks == 2 + self.kv_cache_spec.num_speculative_blocks
                )
        ...
        if envs.VLLM_USE_LIGHTER_MAMBA_CACHE:
            ...
            if request_id not in self.num_cached_block:
                if new_computed_blocks:
                    assert (
                        len(new_computed_blocks) == 1 or new_computed_blocks[-2].is_null
                    )
                    new_computed_blocks = list(new_computed_blocks[-1:])
                else:
    ...
    def allocate_new_blocks(
        ...
        if not envs.VLLM_USE_LIGHTER_MAMBA_CACHE:
            ...
            if num_new_blocks <= 0:
                ...
                new_blocks: list[KVCacheBlock] = self.block_pool.get_new_blocks(
                    num_new_blocks
                )
                req_blocks.extend(new_blocks)
                return new_blocks
    ...
    def cache_blocks(self, request: Request, num_tokens: int) -> None:
        ...
        if envs.VLLM_USE_LIGHTER_MAMBA_CACHE:
            ...
            if (
                ...
                assert (
                    len(self.req_to_blocks[request.request_id])
                    == 3 + self.kv_cache_spec.num_speculative_blocks
                )
