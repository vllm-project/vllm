...
class SingleTypeKVCacheManager(ABC):
    ...
    def __init__(
        ...
        # Blocks registered in the prefix cache during the current scheduling
        # step. Another request in the same step must not reuse them: the GPU
        # forward pass that writes their KV data has not executed yet.
        # Cleared at the start of each new step via new_step_starts().
    ...
    def get_num_blocks_to_allocate(
        ...
        # If the last prefix-cache hit block was just registered this step by
        # another request, defer: the GPU hasn't written those blocks yet.
        if (
            self.enable_caching
...
class MambaManager(SingleTypeKVCacheManager):
    ...
    def get_num_blocks_to_allocate(
        ...
        if self.mamba_cache_mode != "align":
            ...
            # align mode does not call super(), so check the intra-step deferral
            # guard here — the base class handles it for the non-align path via super().
            if (
                self.enable_caching
                and new_computed_blocks
                and not new_computed_blocks[-1].is_null
    ...
    def cache_blocks(self, request: Request, num_tokens: int) -> None:
        # Tracking newly-cached blocks in cached_blocks_this_step is handled
        # by the base class cache_blocks implementation.
