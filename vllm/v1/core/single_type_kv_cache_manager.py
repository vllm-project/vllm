...
class SingleTypeKVCacheManager(ABC):
    ...
    def commit_step(self, step_seq: int = 0) -> None:
        """Notify that the step scheduled in a given pass has committed
        (its GPU writes have landed)."""
        return None
...
class MambaManager(SingleTypeKVCacheManager):
    ...
    def __init__(
        ...
        # Publish generation per state hash: a hash is only safe to hit once
        # the step that scheduled its state write has committed. Replaces the
        # old cached_blocks_this_step guard, which was cleared at the next
        # schedule pass and so missed the async-scheduling window where the
        # publishing step's writes are still in flight.
        self._hash_publish_step: dict[BlockHashWithGroupId, int] = {}
        self._sched_step_seq = 0
        self._committed_step_seq = 0
    ...
    def get_num_blocks_to_allocate(
        ...
        if (
            len(new_computed_blocks) > 0
            and self._hash_publish_step.get(
                new_computed_blocks[-1].block_hash, 0
            )
            > self._committed_step_seq
        ):
            # Mamba can't rely on blocks whose state write has not committed
            # yet. To put it in the next step, we return num_gpu_blocks + 1 so
            # that kv_cache_manager will think there is no enough blocks to
            # allocate now and don't schedule it in the current step.
                ...
                if partial_hit is not None:
                    ...
                    if blocks_allocated:
                        ...
                        if cow_block.block_hash is not None:
                            ...
                            # copy, so defer hits on it until this step
                            # commits.
                            self._hash_publish_step[cow_block.block_hash] = (
                                self._sched_step_seq
                            )
                    else:
                        self._apply_cow(request_id, block_idx, source_block, cow_block)
    ...
    def pop_blocks_for_free(self, request_id: str) -> list[KVCacheBlock]:
        ...
        # Drop the publish-guard entries for this request's hashes; uncommitted
        # ones are evicted from the cache by the caller (abort/preemption), so
        # they must not defer future lookups.
        for block in self.req_to_blocks.get(request_id, ()):
            if block.block_hash is not None:
                self._hash_publish_step.pop(block.block_hash, None)
        ...
        *,
        ...
        if self.mamba_cache_mode == "align":
            ...
            if partial_hash is not None:
                self._hash_publish_step[partial_hash] = self._sched_step_seq
        if num_cached_blocks_after > num_cached_blocks_before:
            ...
            for idx in range(num_cached_blocks_before, num_cached_blocks_after):
                ...
                # no hash and must not be recorded as published.
                if block.is_null or block.block_hash is None:
                    continue
                self._hash_publish_step[block.block_hash] = self._sched_step_seq
                if self.mamba_cache_mode == "align":
    ...
    def new_step_starts(self, step_seq: int = 0) -> None:
        self._sched_step_seq = step_seq

    def commit_step(self, step_seq: int = 0) -> None:
        self._committed_step_seq = step_seq
        # Everything still unpublished-after-commit stays guarded; entries at
        # or below the committed seq are hittable and can be dropped.
        self._hash_publish_step = {
            h: g for h, g in self._hash_publish_step.items() if g > step_seq
        }
