...
class MultiCascadeManager:
    ...
    def alloc_leaf_pass(self, kv_prefix_trie: KVPrefixTrie) -> Optional[KVPrefixAlignedGroups]:
        ...
        for block_id in kv_prefix_trie.block_id_to_req_id:
            groups_list.append((kv_prefix_trie.block_id_to_leaf_node[block_id].depth,
                                len(request_list),
                                len(request_list) + len(kv_prefix_trie.block_id_to_req_id[
                                                            block_id])))
            request_list.extend(list(map(lambda k: (k, kv_prefix_trie.req_to_scheduled[k]),
    ...
    def alloc_full_pass(self, kv_prefix_trie: KVPrefixTrie) -> Optional[KVPrefixAlignedGroups]:
        ...
        stack: List[Tuple[KVPrefixTrieNode, bool]] = [(kv_prefix_trie.sentinel, False)]
        while stack:
            ...
            if visited:
                ...
                if node.weight >= self.absorption_threshold * child_group_weights:
                    ...
                    groups_list.append((node.depth,
                                        node.min_accessible_leaf_id, start_of_next_group))
                    node.num_groups = 1
                else:
                    node.groups_weight = child_group_weights
                    groups_list.extend([(node.depth, i, i + 1) for i in
                                        range(start_direct_leaf, start_of_next_group)])
                    node.num_groups = num_groups + start_of_next_group - len(request_list)
                request_list.extend(list(map(lambda k: (k,
                                                        kv_prefix_trie.req_to_scheduled[k]),
                                                        node.node_req_ids)))
            else:
