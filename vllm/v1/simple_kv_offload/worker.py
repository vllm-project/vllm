...
class SimpleCPUOffloadWorker:
    ...
    def __init__(
        ...
        # (get_finished runs once per step, copy queue is FIFO). Both loads and
        # stores need this barrier: a load can overwrite a block that is still
        # being read by compute after the block is reused.
    ...
    def get_finished(
        ...
        (CPU->GPU) can overwrite a reused GPU block while compute is still
        reading it, so they use the same barrier. See #45704 and #47282 for
        the cross-stream hazards, and #39306 for the srcAccessOrder rationale.
