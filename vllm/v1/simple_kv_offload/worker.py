...
from vllm.utils.host_memory import (
    HOST_REGISTER_CHUNK_SIZE_BYTES,
    alloc_page_aligned_zeros,
    host_unregister,
)
...
def _allocate_cpu_caches(
    ...
    """Allocate zeroed CPU tensors and pin them in batches.

    The page-aligned start plus block-aligned batch borders keep every block
    copy inside one pinned region; see `host_register_chunked`.
    """
    tensors: dict[str, torch.Tensor] = {}
    registered: list[int] = []
    try:
        for name, (shape, dtype) in plan.items():
            ...
            tensor = alloc_page_aligned_zeros(nbytes).view(dtype).view(shape)
            if pin_memory:
                registered += pin_tensor(
                    tensor, chunk_size_bytes, copy_unit_bytes=row_bytes
                )
            tensors[name] = tensor
