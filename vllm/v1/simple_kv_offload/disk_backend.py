...
class _DiskIOError(OSError):
    ...
    failed_block_indices: list[int] | None = None
...
class DiskBackend:
    ...
    def pop_failed_events(self, is_store: bool, hwm: int) -> dict[int, list[int]]:
        """Return failed transfer indices after completion events are consumed."""
        with self._error_lock:
            failures = (
            ...
            while True:
                ...
                try:
                    ...
                    with self._error_lock:
                        ...
                        failed_indices = list(range(len(src_blocks)))
                        if not is_store and error.failed_block_indices is not None:
                            failed_indices = error.failed_block_indices
                        failures[event_idx] = failed_indices
                    logger.warning(
    ...
    def _do_load(
        ...
        if first_error is not None:
            first_error.failed_block_indices = failed_block_indices
            raise first_error
