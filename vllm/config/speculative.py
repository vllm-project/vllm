...
class SpeculativeConfig:
    ...
    def hf_config_override(hf_config: PretrainedConfig) -> PretrainedConfig:
        ...
        is_v32_dsa = hf_config.model_type in ("deepseek_v32", "glm_moe_dsa")
        ...
        if is_v32_dsa:
            # DSA checkpoints (DeepSeek-V3.2, GLM-5.2) use the hardware-isolated
            # vllm.models.deepseek_v32 MTP, not the legacy deepseek_mtp.
            hf_config.model_type = "deepseek_mtp"
            n_predict = getattr(hf_config, "num_nextn_predict_layers", None)
            hf_config.update(
                {"n_predict": n_predict, "architectures": ["DeepseekV32MTPModel"]}
            )
