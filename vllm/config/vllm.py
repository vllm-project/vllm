...
def _uses_full_rank_kda(model_config: ModelConfig | None) -> bool:
    ...
    is only chosen when the checkpoint asks for the full-rank gate: the same
    architecture also serves standalone Kimi-Linear through a different,
    unguarded implementation.
    """
    if model_config is None:
        return False
    kda_config = getattr(model_config.hf_text_config, "linear_attn_config", None)
    return bool((kda_config or {}).get("use_full_rank_gate", False))
