        ...
        *,
        ...
        if watermark_config is None:
            if config is not None and config.watermarking is True:
                raise VLLMValidationError(
                    "Watermarking was explicitly requested, but the engine has no "
        ...
        if config.trace_decode_token_ids is not None:
            raise VLLMValidationError(
                ...
                value=config.trace_decode_token_ids,
            )
        if config.temperature == 0:
            if config.watermarking is True:
                raise VLLMValidationError(
                    "Watermarking was explicitly requested, but greedy decoding "
                    "(temperature=0) cannot be watermarked. Set temperature > 0 or "
                    "watermarking=false.",
