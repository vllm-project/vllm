...
class CacheConfig:
    ...
    With `mamba_cache_mode` "all" the effective block size is a multiple of
    both this value and the model's mamba chunk size, since that is where the
    cached states are read from, and it also sets the attention block size.
    With "align" it is ignored: the mamba block size follows the attention
    block size."""
