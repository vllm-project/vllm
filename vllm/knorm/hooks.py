...
def collect_knorm_scores(runner, input_batch) -> None:
    ...
    positions_np = input_batch.positions.cpu().numpy()
...
def attach_knorm_scores(runner, result) -> None:
    ...
    actual = result.get_output() if hasattr(result, "get_output") else result
    actual.knorm_block_scores = knorm_scores  # type: ignore[attr-defined]
