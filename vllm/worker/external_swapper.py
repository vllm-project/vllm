...
class LocalFileSwapper(ExternalSwapperBase):
    ...
    def __init__(
        ...
        self.external_swapper_space_bytes = cache_config.external_swapper_space_bytes
    ...
    def _allocate_kv_cache(self) -> List[Tuple[str, str]]:
        ...
        if kv_attention_layer_bytes % 2 != 0:
            raise ValueError(
                f"Invalid kv_attention_layer_bytes size: {kv_attention_layer_bytes}"
            )
        key_attention_layer_bytes = int(kv_attention_layer_bytes / 2)

        kv_cache: List[Tuple[str, str]] = []
        for i in range(self.num_attention_layers):
            key_file_name = f"{self.directory}/external_{self.cache_engine_identifier}_layer_{i}_key"
            val_file_name = f"{self.directory}/external_{self.cache_engine_identifier}_layer_{i}_val"
