...
def seq_output_builder():
    return SequenceOutput(
        0, 0, {0: Logprob(logprob=float("inf"), rank=None, decoded_token=None)}
    )
    ...
    def pythonize(
        ...
        if not self.pythonized:
            self._pythonize_sampler_output(
                input_metadata, copy_stream, pinned_sampled_token_buffer, True
            )
...
class MultiStepModelRunner(GPUModelRunnerBase[StatefulModelInput]):
    ...
    def __init__(self, base_model_runner: GPUModelRunnerBase, *args, **kwargs):
        ...
        supported_attention_backends: List[str] = _get_supported_attention_backends(
    ...
    def _async_process_outputs(
        self, model_input: StatefulModelInput, output_proc_callback: Callable
    ):
        ...
        for step_num, model_output in enumerate(model_input.cached_outputs):
            if not model_output.pythonized:
                model_output.maybe_pythonize(
                    model_input, self._copy_stream, self.pinned_sampled_token_ids
                )
    ...
    def _final_process_outputs(
        self, model_input: StatefulModelInput, output_proc_callback: Optional[Callable]
    ):
    ...
    def execute_model(
        ...
        output = self._base_model_runner.execute_model(
            frozen_model_input, kv_caches, intermediate_tensors, num_steps=1
        )
    ...
    def _advance_step(
        ...
        turn_prefills_into_decodes: bool = (
            model_input.current_step == 1 and model_input.num_single_step_prefills != 0
        )
...
DeferredLogprobsReturnType = Tuple[
    Optional[List[Optional[PromptLogprobs]]], Optional[List[SampleLogprobs]]
]
...
def _pythonize_sampler_output(
    ...
    for sgdx, (seq_group, sample_result) in enumerate(zip(seq_groups, samples_list)):
        ...
        for tdx, (parent_id, next_token_id) in enumerate(
            zip(parent_ids, next_token_ids)
        ):
            if cache is not None:
                        ...
                        (
                            ...
                            if any_logprobs_are_requested
                                ...
                                next_token_id: Logprob(
                                    logprob=float("inf"), rank=None, decoded_token=None
                                )
                            }
                        ),
                    )
