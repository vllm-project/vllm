...
class GPUModelRunnerBase(ModelRunnerBase[TModelInputForGPU]):
    ...
    def capture_model(self, kv_caches: List[List[torch.Tensor]]) -> None:
        ...
        # if self.model_config.model_part in ("full", "encoder"):
            ...
            for virtual_engine in range(
                ...
                cudagraph_inputs_embeds = ((
                    True, False) if self.model_config.enable_prompt_embeds else
                                           (False, ))
                if self.model_config.model_part in ("middle", "decoder"):
                    # For decoder-only models, we only capture inputs_embeds.
                    cudagraph_inputs_embeds = (True, )
...
class DecoderModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    """
    GPU model runner with sampling step.
    """
    _model_input_cls: Type[ModelInputForGPUWithSamplingMetadata] = (
    ...
    def _dummy_run(self, max_num_batched_tokens, max_num_seqs=1):
        with self.set_in_profile_run():
            # Enable top-k sampling to reflect the accurate memory usage.
            sampling_params = \
                SamplingParams(top_p=0.99, top_k=self.vocab_size - 1)
            ...
            for group_id in range(max_num_seqs):
                ...
                dummy_data = self.input_registry \
                    .dummy_data_for_profiling(self.model_config,
                                              seq_len,
                                              self.mm_registry)
                # Create a sequence group metadata object with dummy data.
                # Since we don't have a sampler in this runner,
                # we don't need to create sampling params.
                seq = SequenceGroupMetadata(
                    request_id=str(group_id),
                    is_prompt=True,
                    seq_data={group_id: dummy_data.seq_data},
...
class MiddleModelRunner(GPUModelRunnerBase[ModelInputForGPU]):
    """
    GPU model runner with sampling step.
    """
    _model_input_cls: Type[ModelInputForGPU] = (
        ModelInputForGPU)
    _builder_cls: Type[ModelInputForGPUBuilder] = ModelInputForGPUBuilder
    ...
    def make_model_input_from_broadcasted_tensor_dict(
        ...
        model_input = \
            ModelInputForGPU.from_broadcasted_tensor_dict(
                tensor_dict,
                attn_backend=self.attn_backend,
            )
        return model_input
    ...
    def _dummy_run(self, max_num_batched_tokens, max_num_seqs = 1):
        with self.set_in_profile_run():
            # We don't have a sampler in this runner, so we don't
            # need to create sampling params.
