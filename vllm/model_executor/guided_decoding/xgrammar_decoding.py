...
class XGrammarLogitsProcessor:
    ...
    def __call__(self, input_ids: list[int],
        ...
        if not self.prefilled or len(input_ids) == 0:
            ...
            for i, matcher in enumerate(self.matchers):
                if not matcher.is_terminated():
                    ...
                    assert self.matchers[i].accept_token(
                        sampled_token), f"token {sampled_token} not accepted"
        ...
        # decoding
        is_speculative_decoding = not self.config.num_lookahead_slots
        xgr.apply_token_bitmask_inplace(
            scores,
            self.token_bitmask.to(scores.device,
                                  non_blocking=is_speculative_decoding))
        if device_type != "cuda":
