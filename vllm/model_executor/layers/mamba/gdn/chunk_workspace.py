...
# The output ``o`` stays concurrent with them (the caller's references keep
# the five alive until return), and solve_tril's bf16 copy of the fp32 ``A``
# coexists during the cast; both are counted below. SSM ``final_state`` is
# GDN cache, not this workspace. Synthetic measurement (H20, T=16384): this
# analytic peak is ~90% of the true CUDA peak; the residual is allocator
# fragmentation and kernel scratch, which cannot be modeled analytically.
...
def estimate_gdn_chunk_workspace(
    ...
    Covers ``h + v_new + w + u + A``, the attention output ``o`` (allocated
    while the others are still referenced by the caller), and the bf16 copy
    of the fp32 ``A`` made by ``solve_tril``. ``h`` scales with chunk count,
    not raw tokens. Packed varlen layout is ``B = 1``.
    ...
    # The output is allocated while the caller still references h/v_new/w/u,
    # so it adds to the peak instead of reusing freed blocks. Same size as
    # v_new.
    o_bytes = v_new_bytes
    # solve_tril reads the fp32 A while writing its bf16 copy; both are live
    # during the cast.
    a_cast_bytes = a_bytes // 2
    hv_only = h_bytes + v_new_bytes
    peak_live = (
        h_bytes + v_new_bytes + w_bytes + u_bytes + a_bytes + o_bytes + a_cast_bytes
    )
