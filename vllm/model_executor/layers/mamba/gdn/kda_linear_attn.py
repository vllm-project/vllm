...
class KDAAttentionBase(GatedDeltaNetAttention):
    """Shared KDA implementation for model-specific adapters."""
    ...
    def __init__(
        ...
        *,
        ...
        head_dim: int,
        conv_size: int,
        conv_params_dtype: torch.dtype,
        norm_eps: float = 1e-5,
        norm_dtype: torch.dtype | None = None,
        allow_neg_eigval: bool = False,
        reduce_results: bool = True,
        dt_bias_weight_loader: LoaderFunction | None = None,
        a_log_weight_loader: LoaderFunction | None = None,
    ) -> None:
        ...
        if self.num_heads % self.tp_size != 0:
            raise ValueError(
                f"KDA num_heads ({self.num_heads}) must be divisible by "
        ...
        if dt_bias_weight_loader is None:
            dt_bias_weight_loader = sharded_weight_loader(0)
        set_weight_attrs(
            self.dt_bias,
            {"weight_loader": dt_bias_weight_loader},
        )
        ...
        if a_log_weight_loader is None:
            a_log_weight_loader = sharded_weight_loader(2)
        set_weight_attrs(
            self.A_log,
    ...
    def _forward_kda(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        q, k, v, g1, beta, output_gate = self._project_inputs(hidden_states)
        ...
        return self._project_output(core_attn_out, output_gate)
