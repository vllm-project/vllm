...
class KimiGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        # otherwise (shared base is also used by KimiLinearConfig).
        self.head_dim = getattr(config, "linear_head_dim", None)
        self.num_heads = getattr(config, "linear_num_heads", None)
        self.conv_size = getattr(config, "linear_conv_kernel_dim", None)
        if (
            self.head_dim is None
            or self.num_heads is None
            or self.conv_size is None
        ):
