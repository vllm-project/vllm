...
@jitcache(
    ...
    # for variables that cannot be labeled constexpr because range > 32 bit
    assume_const=[
        "hdim", "dstate", "chunk_size", "batch", "nheads_ngroups_ratio"
    ],
    # cache_launch_grid=True,
)
@triton.jit
...
@jitcache(
    ...
    # for variables that cannot be labeled constexpr because range > 32 bit
    assume_const=["hdim", "dstate", "chunk_size", "nheads_ngroups_ratio"],
    # cache_launch_grid=True,
)
@triton.jit
...
def _chunk_cumsum_fwd(dt,
    ...
    with torch.cuda.device(dt.device.index):
        _chunk_cumsum_fwd_kernel[grid_chunk_cs](
            ...
            stride_dt_seqlen=dt.stride(1),
            stride_dt_head=dt.stride(2),
            stride_A_head=A.stride(0),
            stride_dt_bias_head=dt_bias.stride(0)
            if dt_bias is not None else 0,
            stride_dt_out_batch=dt_out.stride(0),
            stride_dt_out_head=dt_out.stride(1),
            stride_dt_out_chunk=dt_out.stride(2),
            stride_dt_out_csize=dt_out.stride(3),
            stride_dA_cs_batch=dA_cumsum.stride(0),
            stride_dA_cs_head=dA_cumsum.stride(1),
            stride_dA_cs_chunk=dA_cumsum.stride(2),
            stride_dA_cs_csize=dA_cumsum.stride(3),
...
def _chunk_state_fwd(B,
    ...
    with torch.cuda.device(x.device.index):
        _chunk_state_fwd_kernel[grid](
            ...
            stride_b_dstate=B.stride(-1),
            stride_states_batch=states.stride(0),
            stride_states_chunk=states.stride(1),
            stride_states_head=states.stride(2),
            stride_states_hdim=states.stride(3),
            stride_states_dstate=states.stride(4),
            stride_dt_batch=dt.stride(0),
            stride_dt_head=dt.stride(1),
            stride_dt_chunk=dt.stride(2),
            stride_dt_csize=dt.stride(3),
            stride_dA_cs_batch=dA_cumsum.stride(0),
            stride_dA_cs_head=dA_cumsum.stride(1),
            stride_dA_cs_chunk=dA_cumsum.stride(2),
            stride_dA_cs_csize=dA_cumsum.stride(3),
...
def chunk_state_varlen(B,
    ...
    with torch.cuda.device(x.device.index):
        _chunk_state_varlen_kernel[grid](
            ...
            stride_init_states_batch=init_states_strides[0],
            stride_init_states_head=init_states_strides[1],
            stride_init_states_hdim=init_states_strides[2],
            stride_init_states_dstate=init_states_strides[3],
