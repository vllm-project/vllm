...
@triton.heuristics(
    values={
        'NEED_MASK_HD':
        lambda args: args['hdim'] / args['BLOCK_SIZE_HD'] != args['hdim'] //
        args['BLOCK_SIZE_HD'],
        'NEED_MASK_CS_DS':
        lambda args: args['dstate'] / args['CS_BLOCK_SIZE_DS'] != args[
            'dstate'] // args['CS_BLOCK_SIZE_DS'] or args['dstate'] != args[
                'BLOCK_SIZE_DSTATE'],
        'NEED_MASK_CS_CS_inner':
        lambda args: args['HAS_INITSTATES'] or args['chunk_size'] / args[
            'CS_BLOCK_SIZE_CS_inner'] != args['chunk_size'] // args[
                'CS_BLOCK_SIZE_CS_inner'],
        'NEED_MASK_CS_CS_outer':
        lambda args: args['HAS_INITSTATES'] or args['chunk_size'] / args[
            'CS_BLOCK_SIZE_CS_outer'] != args['chunk_size'] // args[
                'CS_BLOCK_SIZE_CS_outer'],
        'NEED_MASK_1_DS':
        lambda args: args['dstate'] / args['BLOCK_SIZE_DS'] != args[
            'dstate'] // args['BLOCK_SIZE_DS'],
    })
@triton.jit
def _fused5_ssd_kernel(
    ...
    * NEED_MASK_*=True is not tested, and this kernel was only tested on an A100 and H100.
    ...
    if pid_og < num_pids_css:
        ...
        dt_ptrs = css_dt_ptr + (offs_h[:, None] * stride_dt_orig_head +
                                offs_c[None, :] * stride_dt_orig_seqlen)
        A_ptrs = A_ptr + offs_h * stride_A_head
        dt_out_ptrs = css_dt_out_ptr + (offs_h[:, None] * stride_dt_head +
                                        offs_c[None, :] * stride_dt_csize)
        dA_cs_ptrs = css_dA_cumsum_ptr + (offs_h[:, None] * stride_dA_cs_head +
        ...
        # mark progress
        tl.atomic_add(first2_wait_ptr + pid_c * first2_wait_stride_chunk,
                      CCS_BLOCK_SIZE_H,
                      sem='release')
    ########################################
    # BMM (CB)
    ########################################
        ...
        if not IS_CAUSAL or pid_n * BMM_BLOCK_SIZE_N < (pid_m +
                                                        1) * BMM_BLOCK_SIZE_M:
            ...
            a_ptrs = a_ptr + (offs_m[:, None] * stride_C_seqlen +
                              offs_k[None, :] * stride_C_dstate)
            b_ptrs = b_ptr_bmm + (offs_k[:, None] * stride_b_dstate +
                                  offs_n[None, :] * stride_b_seqlen)
            chunk_size_limit = min(chunk_size, seqlen - pid_c * chunk_size)

            # acc = tl.zeros((BMM_BLOCK_SIZE_M, BMM_BLOCK_SIZE_N), dtype=cb_ptr.dtype.element_ty)
            acc = tl.zeros((BMM_BLOCK_SIZE_M, BMM_BLOCK_SIZE_N),
                           dtype=tl.float32)
            ...
            chunk_size_limit = min(chunk_size, seqlen - pid_c * chunk_size)
            seq_idx_m = tl.load(bmm_seq_idx_ptr +
                                offs_m * stride_seq_idx_seqlen,
                                mask=offs_m < chunk_size_limit,
                                other=-1)
            seq_idx_n = tl.load(bmm_seq_idx_ptr +
            ...
            out_ptrs_cb = out_ptr_cb + (stride_cb_csize_m * offs_m[:, None] +
                                        offs_n[None, :] * stride_cb_csize_k)
            tl.store(out_ptrs_cb,
                     acc,
    ...
    C_ptr += pid_c * chunk_size * stride_C_seqlen + (
    ...
    for pid_ds in range(0, num_pid_ds, 1):
        ...
        x_ptrs_cs = x_ptr + (offs_hd[:, None] * stride_x_hdim +
                             offs_cs[None, :] * stride_x_seqlen)
        b_ptrs_cs = b_ptr + (offs_ds[None, :] * stride_b_dstate +
                             offs_cs[:, None] * stride_b_seqlen)
        ...
        for k in range(0, chunk_size_limit, BLOCK_SIZE_CS):
            if (not NEED_MASK_HD) and (not NEED_MASK_1_DS):
                ...
                b = tl.load(b_ptrs_cs,
                            ...
                            other=0.0,
                            eviction_policy='evict_first')
            dA_cs_k = tl.load(dA_cumsum_ptrs_cs,
                              mask=offs_cs < chunk_size_limit - k,
                              other=0.0).to(tl.float32)

            seq_idx_k = tl.load(seq_idx_ptrs_cs,
                                mask=offs_cs < chunk_size_limit - k,
                                other=-1)

            dt_k = tl.load(dt_ptrs_cs,
                           mask=offs_cs < chunk_size_limit - k,
                           other=0.0).to(tl.float32)
        ...
        # State Passing
        ########################################
        state_G_ptrs = states_G_ptr + offs_hd[:,
                                              None] * stride_states_G_hdim + offs_ds[
                                                  None, :] * stride_states_G_dstate

        main_mask = None if (
            (not NEED_MASK_HD) and (not NEED_MASK_1_DS)
        ) else (offs_hd < hdim)[:, None] & (offs_ds < dstate)[None, :]

        # offset gets us to the end of each chunk
        dA_cs = tl.load(dA_cumsum_ptr +
                        (chunk_size - 1) * stride_dA_cs_csize).to(tl.float32)
        scale = tl.exp(dA_cs)

        seq_idx = tl.load(seq_idx_ptr + (min(pid_c * chunk_size, seqlen) - 1) *
                          stride_seq_idx_seqlen)
    ...
    if not HAS_INITSTATES:
        ...
        chunk_idx_end_excl = tl.load(
            chunk_inv_start + (pid_c + 1))  # has nchunks+1 size so no mask
    ...
    for logic_chunk in range(chunk_idx_start, chunk_idx_end_excl, 1):
        ...
        if not HAS_INITSTATES:
            ...
            c_off = tl.load(chunk_offsets_ptr + logic_chunk,
                            mask=logic_chunk > -1,
                            other=0)
        ...
        if HAS_INITSTATES:
            ...
            if next_chunk_same_pid_c or c_off > 0:
                ...
                c_off_n = tl.load(chunk_offsets_ptr + (logic_chunk + 1),
                                  mask=logic_chunk > -1
                                  and (logic_chunk + 1) < num_logic_chunks,
                                  other=chunk_size)
        ...
        for pid_cs in range(0, cs_num_pid_cs, 1):
            ...
            if not NEED_MASK_CS_CS_outer:
                dA_cs_m = tl.load(dA_cumsum_ptr +
                                  offs_cs * stride_dA_cs_csize).to(tl.float32)
            else:
                dA_cs_m = tl.load(dA_cumsum_ptr + offs_cs * stride_dA_cs_csize,
                                  mask=offs_cs < chunk_size,
                                  other=0.0).to(tl.float32)

            if not HAS_INITSTATES:
                # - handle seq idx when HAS_INITSTATES==False
                seq_idx_m = tl.load(seq_idx_ptr +
            ...
            if IS_TRITON_22 or pid_c > -1:
                ...
                offs_k_dstate = tl.arange(
                    0, BLOCK_SIZE_DSTATE if BLOCK_SIZE_DSTATE
                    <= CS_WHOLEBLOCK_DS else CS_BLOCK_SIZE_DS)
                C_ptrs = C_ptr + (offs_cs[:, None] * stride_C_seqlen +
            ...
            cb_ptrs = cb_ptr + (offs_cs[:, None] * stride_cb_csize_m +
            ...
            K_MAX = chunk_size_limit if not IS_CAUSAL else min(
                (pid_cs + 1) * CS_BLOCK_SIZE_CS_outer, chunk_size_limit)
            for k in range(0, K_MAX, CS_BLOCK_SIZE_CS_inner):
                ...
                acc += tl.dot(cb, x.to(cb.dtype), out_dtype=acc.dtype)
            ...
            if HAS_D:
                if D_HAS_HDIM:
                    D = tl.load(D_ptr + pid_h * stride_D_head + offs_hd,
                                mask=offs_hd < hdim,
                                other=0.0)
                else:
                    D = tl.load(D_ptr + pid_h * stride_D_head)
                if not NEED_MASK_HD:
                    x_residual = tl.load(x_ptr +
                                         (offs_cs[:, None] * stride_x_seqlen +
                                          offs_hd[None, :] * stride_x_hdim),
                    ...
                    x_residual = tl.load(
                        x_ptr + (offs_cs[:, None] * stride_x_seqlen +
                                 offs_hd[None, :] * stride_x_hdim),
                        mask=(offs_cs[:, None] >= c_off) &
                        (offs_cs[:, None] < chunk_size_limit) &
                        (offs_hd[None, :] < hdim),
            ...
            out_ptrs = out_ptr + (stride_out_seqlen * offs_cs[:, None] +
...
def _fused5_ssd(
    ...
    * Could have slightly different output, but very close
    ...
    # setup from bmm
    CB = torch.empty((nchunks, ngroups, chunk_size, chunk_size),
                     device=C.device,
                     dtype=torch.float16)
    ...
    states_G = torch.empty((nchunks + 1, nheads, hdim, dstate),
                           device=x.device,
                           dtype=states_dtype)
    ...
    grid = lambda META: (
        ...
        # BMM grid
        triton.cdiv(chunk_size, META['BMM_BLOCK_SIZE_N']) * triton.cdiv(
            chunk_size, META['BMM_BLOCK_SIZE_M']) * nchunks * ngroups +
        # fused3 grid
    ...
    _fused5_ssd_kernel[grid](
        ...
        sync_atomic[states_ready_size + grid_atomic_size:states_ready_size +
                    grid_atomic_size + 1],
        32,
        ...
        dA_cumsum.stride(0),
        dA_cumsum.stride(
            2),  # stride_dA_cs_chunk, stride_dA_cs_head, stride_dA_cs_csize,
        seq_idx.stride(0)
        if seq_idx is not None else 0,  # stride_seq_idx_seqlen,
        states_G.stride(0),
        states_G.stride(1),
        states_G.stride(2),
        states_G.stride(3),
    ...
    final_states = states_G[nchunks].to(
        states_G.dtype, copy=True)  # copy and convert to expected dtype
