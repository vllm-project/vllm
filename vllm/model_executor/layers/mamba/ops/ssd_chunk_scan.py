...
def _chunk_scan_fwd_kernel(
    ...
    #            && "Unexpected mma -> mm a layout conversion"' failed.
    # With Triton 2.2.0, this works
    if IS_TRITON_22 or pid_c > -1:
        # Faster to just do 1 iteration with larger BLOCK_SIZE_K,
        # up to block size 128
    ...
    for k in range(0, K_MAX, BLOCK_SIZE_K):
        ...
        dA_cs_k = tl.load(dA_cumsum_ptrs,
                          ...
                          other=0.0).to(tl.float32)
        # If there's seq_idx, we already set cb[i, j] = 0
        # for seq_idx[i] != seq_idx[j].
