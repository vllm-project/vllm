...
def _bmm_chunk_fwd_kernel(
    ...
    if IS_CAUSAL and pid_n * BLOCK_SIZE_N >= (pid_m + 1) * BLOCK_SIZE_M:
        return
...
def _bmm_chunk_fwd(a,
    ...
    Argument:
        ...
        seq_idx: (batch, seqlen) or None. 
                 out[i, j] for seq_idx[i] != seq_idx[j] will be zeroed out.
        causal: if True, then out[i, j] for i > j will be arbitrary.
                Only out[i, j] for i <= j are guaranteed to be correct.
    Return:
        out: (batch, nchunks, chunk_size, chunk_size)
