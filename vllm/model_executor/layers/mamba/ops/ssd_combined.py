...
def _mamba_chunk_scan_combined_fwd(x,
    ...
    # either (batch, nchunks, chunk_size, chunk_size)
    # or     (batch, nchunks, nheads, chunk_size, chunk_size)
    ...
    if cu_seqlens is None:
        ...
        assert batch == 1, "passing cu_seqlens to get the varlen states is \
                            only supported if batch dimension is 1"
...
class MambaChunkScanCombinedFn(torch.autograd.Function):
    ...
    def apply(x,
        ...
        if not return_varlen_states:
            ...
            assert cu_seqlens is not None, "cu_seqlens must be provided \
                                            if return_varlen_states is True"

        (out, out_x, dt_out, dA_cumsum, states, final_states,
         *rest) = _mamba_chunk_scan_combined_fwd(x,
                                                 dt,
                                                 A,
                                                 B,
                                                 C,
...
def mamba_chunk_scan_combined(x,
    ...
    Argument:
        ...
        cu_seqlens: (num_sequences + 1) or None, 
                    only used if return_varlen_states is True
