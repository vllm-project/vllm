...
def _sparse_indexer_topk_row_ends(
    seq_lens: torch.Tensor, next_n: int, num_rows: int
) -> torch.Tensor:
    """Per-row exclusive end offsets (int32, (num_rows,)) for top-k kernels
    that take ragged lengths (DeepSelect, FlashInfer, torch reference).

    seq_lens is (B, next_n) per-row effective lens for native spec decode and
    (B, 1) otherwise, in which case per-row lens are derived the same way as
    the other decode top-k kernels.
    """
    if seq_lens.numel() == num_rows:
        row_ends = seq_lens.reshape(-1)
    else:
        next_n_offsets = torch.arange(next_n, dtype=torch.int32, device=seq_lens.device)
        row_ends = (seq_lens.reshape(-1, 1) - next_n + 1 + next_n_offsets).reshape(-1)
    assert row_ends.dtype == torch.int32
    return row_ends


def _sparse_indexer_cooperative_topk_constraints(
    ...
    if topk_tokens not in (512, 1024, 2048):
        failures.append(f"topk_tokens must be in (512, 1024, 2048), got {topk_tokens}")
    if num_rows > 64:
...
def _resolve_sparse_indexer_topk_backend(
    logits: torch.Tensor, topk_tokens: int, num_rows: int
) -> str:
    """Resolve the decode top-k implementation from the configured backend.

    "auto" applies the heuristic chain deep_select -> cooperative -> persistent
    -> per_row. Explicit values are validated against their constraints and
    raise RuntimeError when unmet.
    """
    backend = get_current_vllm_config().kernel_config.sparse_indexer_topk_backend
    if backend == "auto":
        if (
            current_platform.is_cuda()
            and deep_select_topk.is_available()
            and deep_select_topk.supports(logits, topk_tokens)
            and num_rows >= deep_select_topk.DEEP_SELECT_MIN_ROWS
        ):
            return "deep_select"
        if not _sparse_indexer_cooperative_topk_constraints(
            logits, topk_tokens, num_rows
        ):
            return "cooperative"
    ...
    if backend == "cooperative":
        failures = _sparse_indexer_cooperative_topk_constraints(
            logits, topk_tokens, num_rows
        )
    ...
    elif backend == "deep_select":
        # The 32-row threshold of "auto" is a performance heuristic and does
        # not apply to the forced backend.
        if not current_platform.is_cuda():
            failures.append("requires a CUDA platform")
        elif not deep_select_topk.is_available():
            failures.append(
        ...
        if not current_platform.is_cuda():
            failures.append("requires a CUDA platform")
        if _fi_top_k_ragged_transform is None:
            failures.append("flashinfer.topk.top_k_ragged_transform is not importable")
    if failures:
        raise RuntimeError(
            f"sparse_indexer_topk_backend='{backend}' was requested, but: "
            + "; ".join(failures)
        )
    return backend


def run_sparse_indexer_decode_topk(
    backend: str,
    logits: torch.Tensor,
    ...
    """Run the selected decode top-k implementation, writing into
    topk_indices (int32, -1 fill for rows shorter than topk_tokens)."""
    num_rows = logits.shape[0]
    if backend == "deep_select":
        row_ends = _sparse_indexer_topk_row_ends(seq_lens, next_n, num_rows)
        ...
        # Restricts each row to [0, row_ends[i]) and returns int32 indices
        # with -1 fill past the row length; write into the int32 buffer.
...
def sparse_attn_indexer(
    ...
    if has_decode:
        ...
        topk_backend = _resolve_sparse_indexer_topk_backend(
            logits, topk_tokens, num_rows
        )
