...
and a per-query query-major membership bitmask restores EXACT per-query
attend-sets inside the fp8 kernel — LSE-equal to the per-query path, so
semantics are bit-identical to stock sparse attention.
...
(PACK=16) 116.01s (1.28x, exact official attend-sets).
"""
...
def litedsa_available() -> bool:
    """True iff the SM100 grouped-attention kernels were compiled."""
