...
def sparse_mqa_attn_indexer(
    ...
    if metadata.num_prefills > 0:
        ...
        k_quant_full, k_scale_full = get_prefill_k_workspaces(
            total_seq_lens, head_dim, use_fp4_cache=True
        )
...
class SparseMQAIndexer(SparseAttnIndexer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            not has_deep_select()
            and self.topk_tokens not in SPARSE_TOPK_KERNEL_SUPPORTED
        ):
            raise ValueError(
                "SparseMQAIndexer without DeepSelect requires topk in "
                f"{SPARSE_TOPK_KERNEL_SUPPORTED}."
            )
        if not (
