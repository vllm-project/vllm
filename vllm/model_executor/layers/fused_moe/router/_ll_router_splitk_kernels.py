...
def _fill_pred(pred_flat, coord_ktile, dim_limit, k_base, K_total):
    ...
    for v in range(num_vec):
        ...
        for j in range(num_mn):
            ...
            pred_flat[v, j] = (
                cute.elem_less(coord_ktile[(0, v), j][0], dim_limit) & k_valid # [0] extracts the M-coordinate from the (M, K) coordinate tuple
            )
...
def _make_pred(tXcX, dim_limit, k_base, K_total):
    ...
    tXcX_ktile = tXcX[None, None, 0, 0] #Slice to one K-tile: None keeps CopyOp and CopyM, 0 selects CopyK=0 and k_tile=0. Result shape: ((8,1), 4).
    num_vec = tXcX.shape[0][1] # = 8 — from the vector part of the hierarchical CopyOp shape (8, 1).
    num_mn = cute.size(tXcX, mode=[1]) # = 4 — flattened size of mode 1 (CopyM or CopyN).
    ...
    _fill_pred(pred_flat, tXcX_ktile, dim_limit, k_base, K_total) # Fill with initial values (M/N-bounds AND K-bounds for the first tile).
...
class LLRouterSplitK:
    ...
    def __init__(
        ...
        *,
        ...
        self.num_threads = self.num_dma_threads + self.num_mma_threads # total per CTA: 256
    ...
    def _make_smem_layout_AB(self, dtype, copy_bits, smem_tiler):
        ...
        swizzle_bits = min(swizzle_bits,
                           3) # Capped at 3: 64 * 16 / 128 = 8, log2(8) = 3
    ...
    def call_splitk(
        ...
        perm_mnk = (
            ...
            self.atom_layout[1] * self.mma_shape[1] * (self.tile_n // 8), # 1 * 8 * 2 = 16
        ...
        self.kernel_splitk(
            ...
            grid=[cute.size(grid_m), cute.size(grid_n), self.split_k], # [1, 16, 8] = 128 CTAs
            block=[self.num_threads, 1, 1], # [256, 1, 1]
            cluster=[1, 1, self.split_k], # [1, 1, 8] -- the 8 CTAs along Z form one cluster
            stream=stream,
            use_pdl=True,
        )
