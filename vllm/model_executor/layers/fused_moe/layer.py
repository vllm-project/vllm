...
def FusedMoEFactory(
    ...
    if expert_replacement is not None:
        logical_num_experts = expert_replacement.num_logical_experts
    ...
    if expert_replacement is not None:
        if moe_backend == "auto":
            moe_backend = "triton"
            logger.info(
                "Using the Triton MoE backend for %s because expert "
                "substitution requires invalid-route skipping.",
                prefix,
            )
