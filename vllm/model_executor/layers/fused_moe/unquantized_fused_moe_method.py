...
class UnquantizedFusedMoEMethod(FusedMoEMethodBase, CustomOp):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if self.unquantized_backend in [
            ...
            # When the expert LRU cache is active, skip the normal kernel setup
            # and go straight to cache initialization which allocates the small
            # GPU scratch buffer. CachedWeightProvider handles both CPU and GPU
            # source weights (device_loading_context may have moved them to GPU).
            cache_active = (
                self.supports_expert_lru_cache and layer._moe_expert_cache_size > 0
            )
            if cache_active:
                self.moe_quant_config = self.get_fused_moe_quant_config(layer)
