...
class DeepEPPrepareAndFinalize(mk.FusedMoEPrepareAndFinalize):
    ...
    def prepare(
        ...
        # TODO (varun) : application of router weights and quantization is common
        # to all implementations - factor out
        ...
        (num_tokens_per_rank, num_tokens_per_rdma_rank,
         expert_num_tokens, is_token_in_rank, event) = self.buffer.get_dispatch_layout(
        ...
        (token_data, expert_topk_ids,  expert_topk_weights, expert_num_tokens_per_expert_list,
         self.handle, event) = self.buffer.dispatch(
                                x = token_data,
                                handle = None,
                                ...
                                expert_alignment = 1, # TODO (varun) : set this properly and avoid moe_alighn kernel ?
                                config = None,
                                previous_event = None,
                                async_finish = False,
                                allocate_on_comm_stream = False)
        ...
        if do_debug:
            ...
            s += f"expert_num_tokens_per_expert_list : {expert_num_tokens_per_expert_list} \n"
            print (s, flush=True)
        ...
        # The existing MOE kernels assume that all entries of topk_ids are valid. To that
        # effect, set the -1s in expert_topk_ids to some expert outside this rank so
        # the expert_map can remap it to -1 when safe.
        # With Expert Parallel, the experts are divided amongst the rank sequentially.
        # For rank 0, set it to num_experts - 1 and for all other ranks set it to 0 as
        # we know that expert_map will have a -1 in those regions for those ranks.
        expert_topk_ids = torch.where(expert_topk_ids == -1, num_experts - 1 if self.rank == 0 else 0, expert_topk_ids)

        return expert_x, expert_x_scale, expert_num_tokens, expert_topk_ids, expert_topk_weights
    ...
    def finalize(self,
        ...
        if not apply_router_weight_on_input:
            ...
            fused_expert_output.mul_(topk_weights.view(fused_expert_output.size(0), -1, 1))
            num_tokens = topk_ids.size(0)
            hidden_dim = fused_expert_output.size(-1)
            local_out_shape = (num_tokens, hidden_dim)
            local_out = torch.zeros(local_out_shape, device="cuda", dtype=torch.float32)
            ops.moe_sum(fused_expert_output, local_out)
            fused_expert_output = local_out

        combined_x, combined_topk_weights, event = self.buffer.combine(x = fused_expert_output,
                            handle = self.handle, 
