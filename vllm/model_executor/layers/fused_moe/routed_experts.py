...
class RoutedExperts(PluggableLayer):
    ...
    def __init__(
        ...
        expert_replacement: ConstantExpertReplacement | None = None,
    ):
        super().__init__()
    ...
    def load_weights(
        ...
        for expert_name, loaded_weight in weights:
            ...
            for param_name, weight_name, expert_id, shard_id in expert_mapping:
                ...
                if (
                    param is None
                    and getattr(self, "expert_replacement", None) is not None
                ):
    ...
    def make_expert_params_mapping(
        ...
        if any(module.expert_replacement is not None for _, module in routed_modules):
            mapping: list[tuple[str, str, int, str]] = []
            for module_name, module in routed_modules:
                assert module_name.endswith(".routed_experts")
                moe_prefix = module_name.removesuffix(".routed_experts")
                replacement = module.expert_replacement
                if replacement is not None:
                    target_prefix = replacement.target.module_path
                    assert target_prefix.endswith(moe_prefix)
                    mapping.extend(
