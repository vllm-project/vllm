...
class DeepEPLLPrepareAndFinalize(mk.FusedMoEPrepareAndFinalize):
    ...
    def prepare_async(
        ...
        logger.info(
            f"[UCCL-EP DEBUG] low_latency_dispatch call:\n"
            f"  a1.shape: {a1.shape}\n"
        ...
        logger.info(
            f"[UCCL-EP DEBUG] low_latency_dispatch OUTPUT:\n"
            f"  expert_x.shape: {expert_x.shape if isinstance(expert_x, torch.Tensor) else (expert_x[0].shape, expert_x[1].shape)}\n"
            f"  expert_num_tokens.shape: {expert_num_tokens.shape}\n"
            f"  expert_num_tokens values: {expert_num_tokens}"
        )
