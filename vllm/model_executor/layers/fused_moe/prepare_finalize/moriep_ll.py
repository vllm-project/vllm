...
class MoriEPLLPrepareAndFinalize(mk.FusedMoEPrepareAndFinalizeModular):
    """
    MoRI EP low-latency Prepare/Finalize.

    Uses the split ``op.dispatch_send`` / ``op.dispatch_recv`` and
    ``op.combine_send`` / ``op.combine_recv`` path (AsyncLL kernel), which is
    overlap- / DBO-capable. Supports bf16/fp8/fp4 dispatch and bf16/fp8 combine.
    Each DBO micro-batch uses its own EpDispatchCombineOp because the op's
    receive buffers are internal/non-reentrant.
    """
    ...
    def supports_async(self) -> bool:
        # TODO(billishyahao): enable async
        return False
    ...
    def prepare_async(
        ...
        op = self._op()
        dispatch_out = op.dispatch_send(a1, topk_weights, scale, dispatch_ids)

        return (
            op.dispatch_recv,
            lambda: self._build_prepare_result(dispatch_out),
        )
    ...
    def _finalize(
        ...
        dbo_maybe_run_recv_hook()
        combine_out = op.combine_send(fused_expert_output, None, combine_ids)

        def receiver() -> None:
            result = combine_out[0]
            output.copy_(result[:num_token])

        return op.combine_recv, receiver
