...
class FlashInferCFTCountedWritePrepareAndFinalize(
    mk.FusedMoEPrepareAndFinalizeModular
):
    """FlashInfer CFT counted-write MoE all-to-all prepare/finalize.

    Uses CUDA Logical Endpoints and ``fabric.try_put.counted`` operations
    to write directly into peer receive buffers over the NVLink fabric.
    The counted write atomically increments a destination-side counter by
    the number of bytes delivered, so the receiver can wait on the counter
    rather than a separate fence-and-flag handshake.

    Requires sm_100+ (Blackwell), CUDA 13.4+, and a compatible NVLink
    fabric environment.  Falls back behavior is handled upstream via the
    ``has_flashinfer_cft_counted_write()`` capability check.
    """
    ...
    def prepare(
        ...
        assert self.all2all_manager.cft_moe is not None  # type: ignore[attr-defined]
        recv_payloads = self.all2all_manager.cft_moe.dispatch(  # type: ignore[attr-defined]
