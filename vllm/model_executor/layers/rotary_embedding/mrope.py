...
class MRotaryEmbedding(RotaryEmbeddingBase):
    ...
    def forward_native(
        ...
        # Cos and sin remain unchanged most of the time,
        # and adding constraint updates below can greatly improve reasoning performance.
        is_need_update_cos_sin = False
        if self.cos is None:
            is_need_update_cos_sin = True
        elif self.positions_shape != positions.shape:
            is_need_update_cos_sin = True
            self.positions_shape = positions.shape
        if is_need_update_cos_sin:
            cos_sin = self.cos_sin_cache[positions]
            self.cos, self.sin = cos_sin.chunk(2, dim=-1)
            if positions.ndim == 2:
                assert self.mrope_section
                if self.mrope_interleaved:
                    self.cos = apply_interleaved_rope(self.cos, self.mrope_section)
                    self.sin = apply_interleaved_rope(self.sin, self.mrope_section)
                else:
                    self.cos = torch.cat(
                        [m[i] for i, m in enumerate(self.cos.split(self.mrope_section, dim=-1))],
                        dim=-1,
                    )
                    self.sin = torch.cat(
                        [m[i] for i, m in enumerate(self.sin.split(self.mrope_section, dim=-1))],
                        dim=-1,
                    )
        ...
        query_rot = apply_rotary_emb_dispatch(query_rot, self.cos, self.sin, self.is_neox_style)
        ...
        key_rot = apply_rotary_emb_dispatch(key_rot, self.cos, self.sin, self.is_neox_style)
