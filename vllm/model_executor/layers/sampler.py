...
class SamplerOutput(
    ...
    def __repr__(self) -> str:
        ...
        """
        sampled_token_probs_repr = ("None" if self.sampled_token_probs is None else
                                    self.sampled_token_probs.shape)
...
class Sampler(nn.Module):
    ...
    def _init_sampling_tensors(
        ...
        (sampling_tensors, do_penalties, do_top_p_top_k,
         do_min_p) = SamplingTensors.from_sampling_metadata(sampling_metadata,
...
def _random_sample(
    ...
    for seq_group in selected_seq_groups:
        ...
        if is_prompt:
            ...
            # Generation phase.
            parent_ids = list(range(num_parent_seqs))
            next_token_ids = random_samples[sample_idx:sample_idx + num_parent_seqs,
                                            0].tolist()
        results.append((next_token_ids, parent_ids))
        sample_idx += num_parent_seqs
    return results
    ...
    * Defer Pythonization & preserve GPU-side
    ...
    for sampling_type in SamplingType:
        ...
        if sampling_type == SamplingType.GREEDY:
            ...
            seq_groups_arg = (None
                              if sampling_type == SamplingType.RANDOM else seq_groups)
...
def get_logprobs(
    ...
    if len(query_indices) == 0:
        ...
        return [empty_prompt_logprob] * num_seq_groups, [empty_sampled_logprob
                                                         ] * num_seq_groups
...
def _build_sampler_output(
    ...
    if skip_sampler_cpu_output:
        ...
        for (seq_group, sample_result, group_prompt_logprobs,
            ...
            for parent_id, next_token_id, logprobs in zip(parent_ids, next_token_ids,
