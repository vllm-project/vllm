...
class SpecDecodeBaseSampler(nn.Module):
    ...
    def _create_output(
            ...
            target_token_ids: torch.Tensor,  # [batch_size, k + 1]
            recovered_token_ids: torch.Tensor,  # [batch_size, 1]
    ) -> torch.Tensor:
        ...
        Args:
            ...
            draft model.
            target_token_ids: A tensor of token ids generated by the
            target model.
            recovered_token_ids: A tensor of token ids generated by 
            re-sampling or target model.
        Returns:
        ...
        output_with_bonus_tokens = -torch.ones((batch_size, k + 1),
    ...
    def _raise_if_incorrect_shape(
        ...
        target_token_batch_size, num_target_tokens = target_token_ids.shape
        assert target_token_batch_size == target_batch_size
        assert num_target_tokens == num_draft_token_ids + 1
    ...
    def _raise_if_incorrect_dtype(
        ...
        assert target_token_ids.dtype == self.token_id_dtype
        if draft_probs is not None:
    ...
    def _raise_if_inconsistent_device(
        ...
        devices = [
            t.device for t in [
                target_with_bonus_probs, target_token_ids, draft_probs,
                draft_token_ids
            ] if t is not None
        ]
    ...
    def _raise_if_out_of_bounds_vocab(
        ...
        assert torch.all(target_token_ids < vocab_size)
        assert torch.all(target_token_ids >= 0)
