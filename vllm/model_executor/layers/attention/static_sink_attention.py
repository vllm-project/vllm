...
class StaticSinkMLAAttention(MLAAttention):
    ...
    def populate_sink_kv(self, self_kv_cache: torch.Tensor):
        ...
        # if "model.layers.0" in self.layer_name:
        #     print(f"self_kv_cache.shape: {self_kv_cache.shape}, self_kv_cache.sum(): {self_kv_cache.float().sum()}, self_kv_cache.flatten()[:5]: {self_kv_cache.flatten()[:5]}")
