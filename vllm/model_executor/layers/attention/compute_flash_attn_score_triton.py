...
def _compute_key_importance_kernel(
    ...
    for start_m in range(0, Q_LEN, BLOCK_M):
        ...
        q_ptrs = q_start_ptr + (
            offs_m[:, None] * stride_q_n + offs_d[None, :] * stride_q_d
        )
    ...
    out_start_ptr = Out_Score + (
        cur_batch_idx * stride_out_b + cur_head_idx * stride_out_h
    )
...
def _compute_key_importance_varlen_kernel(
    ...
    stride_q_tok,
    stride_q_h,
    stride_q_d,  # Q: [Total_Tokens, Heads, Head_Dim]
    stride_k_tok,
    stride_k_h,
    stride_k_d,  # K: [Total_Tokens, Heads, Head_Dim]
    stride_lse_h,
    stride_lse_tok,  # LSE: [Heads, Total_Tokens] (Packed 格式)
...
def compute_varlen_importance(
    ...
    _compute_key_importance_varlen_kernel[grid](
        ...
        k.stride(2),
        softmax_lse.stride(0),
        softmax_lse.stride(1),  # [Heads, Total_Tokens]
        out_score.stride(0),
        # Meta
        softmax_scale,
        ACTUAL_HEAD_DIM=head_dim,
