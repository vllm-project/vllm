    ...
    def _omni_get_input_positions_tensor(
        ...
        while idx < len(src_item):
            ...
            if src_item[idx] not in [
                ...
                t_index = (torch.arange(grid_t) * second_per_grids[video_idx] *
                           position_id_per_seconds).long()
                llm_pos_ids = cls._get_llm_pos_ids_for_vision(
...
class OmniMRotaryEmbedding(MRotaryEmbedding):
    ...
    def get_llm_pos_ids_for_vision(
        ...
        llm_grid_h = grid_hs[vision_idx] // spatial_merge_size
        llm_grid_w = grid_ws[vision_idx] // spatial_merge_size
        h_index = (torch.arange(llm_grid_h, device=llm_grid_h.device).view(
            1, -1, 1).expand(len(t_index), -1, llm_grid_w).flatten())
        w_index = (torch.arange(llm_grid_w, device=llm_grid_w.device).view(
            1, 1, -1).expand(len(t_index), llm_grid_h, -1).flatten())
        t_index = torch.Tensor(t_index).to(llm_grid_h.device).view(
            -1, 1).expand(-1, llm_grid_h * llm_grid_w).flatten().long()
        _llm_pos_ids = torch.stack([t_index, h_index, w_index])
        llm_pos_ids_list.append(_llm_pos_ids +
                                start_idx)  # + 1 ) # 12.09 by malinhan
        llm_pos_ids = torch.cat(llm_pos_ids_list, dim=1)
        return llm_pos_ids

    @staticmethod
    def split_list_into_ranges(lst, interval):
        ranges = [[] for _ in range((max(lst) // interval) + 1)]
        for num in lst:
            index = num // interval
            ranges[index].append(num)
    ...
    def get_input_positions(
        ...
        seconds_per_chunk = hf_config.seconds_per_chunk
        spatial_merge_size = hf_config.vision_config.spatial_merge_size
        tokens_per_second = getattr(hf_config.vision_config,
                                    "tokens_per_second", 25)
        ...
        while idx < len(src_item):
            ...
            if src_item[idx] not in [
                ...
                llm_pos_ids = OmniMRotaryEmbedding.get_llm_pos_ids_for_vision(
                    start_idx, image_idx, spatial_merge_size, t_index, grid_hs,
                    grid_ws)
                llm_pos_ids_list.append(llm_pos_ids)
                ...
                t_index = (torch.arange(grid_t) * second_per_grids[video_idx] *
                           position_id_per_seconds).long()
                # t_index = torch.arange(grid_t).long
                # llm_pos_ids_list.append(torch.LongTensor([start_idx]).to(raw_input_ids.device).expand(3, -1))  ## 12.09 by malinhan
                llm_pos_ids = OmniMRotaryEmbedding.get_llm_pos_ids_for_vision(
                    start_idx, video_idx, spatial_merge_size, t_index, grid_hs,
                    grid_ws)
                ...
                t_ntoken_per_chunk = int(position_id_per_seconds *
                                         seconds_per_chunk)
                t_index = (torch.arange(grid_t) * second_per_grids[video_idx] *
                           position_id_per_seconds).long()  #每格对应的时间戳
                t_index_split_chunk = OmniMRotaryEmbedding.split_list_into_ranges(
                    t_index, t_ntoken_per_chunk)  #每个chunk每格对应的时间戳
                ...
                for t_chunk in t_index_split_chunk:
                    ...
                    new_src_item.extend([video_token_id] *
                                        vision_ntoken_per_chunk)
                    vision_llm_pos_ids_list = OmniMRotaryEmbedding.get_llm_pos_ids_for_vision(
                        start_idx + 1,
                        video_idx,
                    ...
                    llm_pos_ids_list.extend(
                        vision_llm_pos_ids_list)  # img_token
                    new_src_item.extend(
                        min(t_ntoken_per_chunk, pure_audio_len -
                            added_audio_len) * [audio_token_id])
                    audio_start_idx = start_idx if len(
                    ...
                    added_audio_len += min(t_ntoken_per_chunk,
                                           pure_audio_len - added_audio_len)
                    llm_pos_ids_list.extend(
                        audio_llm_pos_ids_list)  # audio_token
                if added_audio_len < pure_audio_len:
                    new_src_item.extend(
                        (pure_audio_len - added_audio_len) * [audio_token_id])
                    audio_llm_pos_ids_list = (
        ...
        return torch.cat(llm_pos_ids_list, dim=1).tolist(), torch.cat(
