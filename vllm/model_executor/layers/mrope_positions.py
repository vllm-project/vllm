    ...
    if use_numba:
        ...
        if is_omni:
            if audio_feature_lengths is None:
                ...
                audio_feature_lengths = audio_feature_lengths.numpy()
...
def _vl_get_input_positions_torch(
    ...
    tokens_per_second = getattr(hf_config.vision_config, "tokens_per_second",
                                1.0)
...
def _omni_get_input_positions_torch(
    ...
    while idx < len(src_item):
        ...
        if src_item[idx] not in [
            ...
            llm_pos_ids = _get_llm_pos_ids_for_vision(start_idx, image_idx,
                                                      spatial_merge_size,
                                                      t_index, grid_hs,
                                                      grid_ws)
            llm_pos_ids_list.append(llm_pos_ids)
            ...
            t_index = (torch.arange(grid_t) * second_per_grid_ts[video_idx] *
                       tokens_per_second).long()
            llm_pos_ids = _get_llm_pos_ids_for_vision(start_idx, video_idx,
                                                      spatial_merge_size,
                                                      t_index, grid_hs,
                                                      grid_ws)
            llm_pos_ids_list.append(llm_pos_ids)
            ...
            if added_audio_len < pure_audio_len:
                ...
                audio_llm_pos_ids_list = (
                    torch.arange(pure_audio_len - added_audio_len).expand(
                        3, -1) + llm_pos_ids_list[-1].max() + 1).split(1,
                                                                       dim=1)
                llm_pos_ids_list.extend(audio_llm_pos_ids_list)
            audio_idx += 1
            video_idx += 1
...
def _omni_get_input_positions_numba(
    ...
    while i < len(input_tokens):
        ...
        if token_id == image_token_id:
            ...
            # audio and vision position ids split into chunks and interleaved.
            #
            # |V_1 ...    V_n|A_1 ...   A_n|V_n+1 ... V_2n|A_n+1 ... A_2n|...
            # |vision chunk 1|audio chunk 1|vision chunk 2|audio chunk 2 |...
            ...
            cur_audio_idx += 1
            if cur_audio_idx >= len(audio_feature_lengths):
                with numba.objmode():
                    _raise_missing_mm_item_error(MM_TYPE_AUDIO, cur_audio_idx)
            ...
            i, cur_t = _emit_1d_tokens(
                mrope_pos,
                i=i,
                start_t=cur_t + 1,
                num_tokens=_calc_audio_token_num(
                    audio_feature_lengths[cur_audio_idx]),
            )

            if i == ERR_EXCEEDED:
                with numba.objmode():
                    _raise_tokens_out_of_bound_error(MM_TYPE_AUDIO,
                                                     cur_audio_idx)
