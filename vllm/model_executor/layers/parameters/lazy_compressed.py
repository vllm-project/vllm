...
class LazyCompressedParameter(torch.Tensor):
    ...
    def __torch_dispatch__(cls, func, types, args, kwargs):
        ...
        def unwrap(e):
            ...
            return e.uncompressed_data if isinstance(e, LazyCompressedParameter) else e
        
        rs = func(*tree_map(unwrap, args), **tree_map(unwrap, kwargs))
        
        def wrap(e):
            if isinstance(e, torch.Tensor) and ret_storage_format_cls is not None:
                return LazyCompressedParameter(e, storage_format_cls=ret_storage_format_cls)
            return e
    ...
    def compress(self) -> None:
        density = torch.count_nonzero(self.uncompressed_data).item() / numpy.prod(self.shape)
        print("density: ", density, torch.count_nonzero(self.uncompressed_data), numpy.prod(self.shape))
        
        if self.uncompressed_data is None:
            raise ValueError("Called compress() but uncompressed_data does not exist.")
        self.compressed_data = self.storage_format_cls.compress(
            self.uncompressed_data.t() if self.compress_transposed else self.uncompressed_data
        )
