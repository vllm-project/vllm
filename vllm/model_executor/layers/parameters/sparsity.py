...
from magic_wand import SparseTensor, CompressedStorageFormat, SparseSemiStructuredStorageFormat
...
class SemiStructuredSparseParameter(torch.Tensor):
    ...
    def __new__(
        ...
        assert torch.__version__ > (1,
                                    10), "SparseTensor requires PyTorch 1.11+"
        assert storage_format_cls == SparseSemiStructuredStorageFormat
        self = torch.Tensor._make_wrapper_subclass(cls,
                                                   size=shape,
                                                   dtype=dtype,
                                                   requires_grad=False)
    ...
    def copy_(self, src, non_blocking=False):
        return self.__class__._copy(self, src)
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        # Forward the call to the original torch function with the same arguments
