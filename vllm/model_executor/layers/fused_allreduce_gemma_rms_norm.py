...
def _can_use_flashinfer(hidden_states: torch.Tensor, tp_size: int) -> tuple[bool, int]:
    ...
    if envs.VLLM_BATCH_INVARIANT:
        # The fused path picks one-shot vs two-shot by token count, so the
        # reduction order would track batch composition. Callers fall back to
        # an unfused all-reduce, which goes through the communicator.
        return False, 0
    if (
