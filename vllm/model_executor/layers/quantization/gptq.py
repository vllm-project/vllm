...
class GPTQConfig(QuantizationConfig):
    ...
    def __repr__(self) -> str:
        return (
            f"GPTQConfig(quant_type={self.quant_type}, "
    ...
    def get_quant_method(
        ...
        if isinstance(layer, FusedMoE):
            ...
            moe_quant_method = get_moe_quant_method(self, layer, prefix, GPTQMoEMethod)
            if moe_quant_method is None:
                return None
            moe_quant_method.input_dtype = get_marlin_input_dtype(prefix)
            return moe_quant_method

        quant_method = get_linear_quant_method(self, layer, prefix, GPTQLinearMethod)
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        if kernel_type.__name__ not in self._kernel_backends_being_used:
            logger.info("Using %s for GPTQLinearMethod", kernel_type.__name__)
...
class GPTQMoEMethod(FusedMoEMethodBase):
    """MoE method for GPTQ with Marlin kernels."""
...
GPTQMarlinLinearMethod = GPTQLinearMethod
GPTQMarlinMoEMethod = GPTQMoEMethod
