...
class ModelOptMixedPrecisionConfig(ModelOptQuantConfigBase):
        ...
        if (
            ...
            for shard_name in self.packed_modules_mapping[proj_name]:
                ...
                if shard_prefix in self.quantized_layers:
                    algos.add(
                        self.quantized_layers[shard_prefix][
                            "quant_algo"
                        ].upper()
                    )
            if len(algos) == 1:
    ...
    # ------------------------------------------------------------------
    # Layer → quantize-method dispatch
    # ------------------------------------------------------------------
    ...
    def get_quant_method(
        ...
        if isinstance(layer, LinearBase):
            if quant_algo == "FP8":
                method = ModelOptFp8LinearMethod(self.fp8_config)
                if getattr(method, "backend", "") == "marlin":
                    method.marlin_input_dtype = get_marlin_input_dtype(
                        prefix
                    )
                return method
            if quant_algo == "NVFP4":
                method = ModelOptNvFp4LinearMethod(self.nvfp4_config)
                if getattr(method, "backend", "") == "marlin":
                    method.marlin_input_dtype = get_marlin_input_dtype(
                        prefix
                    )
    ...
    def apply_vllm_mapper(self, hf_to_vllm_mapper: "WeightsMapper"):
        ...
        if self.quantized_layers:
            self.quantized_layers = hf_to_vllm_mapper.apply_dict(
