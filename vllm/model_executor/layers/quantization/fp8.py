...
class Fp8LinearMethod(LinearMethodBase):
    ...
    def apply(
        ...
        if vllm_kernel_override_batch_invariant():
            # DEBUG: Print shapes at the very start
            # import os
            # from vllm.distributed import get_tensor_model_parallel_rank
            # tp_rank = get_tensor_model_parallel_rank()
            ##if tp_rank == 0:
            # print(f"[FP8 Dequant START] weight shape: {layer.weight.shape}, "
            #       f"scale shape: {layer.weight_scale.shape}")
            # print(f"[FP8 Dequant START] block_quant: {self.block_quant}, "
