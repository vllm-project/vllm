...
class ComposedQuantizationConfig(QuantizationConfig):
    """Combine a checkpoint quantizer with online methods for its unquantized layers."""
    ...
    def __init__(
        ...
        self.online_config = OnlineQuantizationConfig(online_args)
        self.set_packed_modules_mapping(checkpoint_config.packed_modules_mapping)

    def set_packed_modules_mapping(
        self, packed_modules_mapping: dict[str, list[str]]
    ) -> None:
        self.packed_modules_mapping = packed_modules_mapping
        self.checkpoint_config.packed_modules_mapping = packed_modules_mapping
        self.online_config.packed_modules_mapping = packed_modules_mapping

    def get_name(self) -> QuantizationMethods:
        return self.checkpoint_config.get_name()

    def get_supported_act_dtypes(self) -> list[torch.dtype]:
        checkpoint_dtypes = self.checkpoint_config.get_supported_act_dtypes()
    ...
    @classmethod
    def from_config(cls, config: dict[str, Any]) -> "ComposedQuantizationConfig":
        raise NotImplementedError(
    ...
    def apply_vllm_mapper(self, hf_to_vllm_mapper) -> None:
        ...
        self.online_config.apply_vllm_mapper(hf_to_vllm_mapper)

    def maybe_update_config(self, *args, **kwargs) -> None:
        self.checkpoint_config.maybe_update_config(*args, **kwargs)
        self.online_config.maybe_update_config(*args, **kwargs)

    def get_quant_method(
        self, layer: torch.nn.Module, prefix: str
    ) -> "QuantizeMethodBase | None":
        checkpoint_method = self.checkpoint_config.get_quant_method(layer, prefix)
        if not isinstance(layer, (LinearBase, RoutedExperts)):
            return checkpoint_method

        online_method = self.online_config.get_quant_method(layer, prefix)
        # The user did not select this layer for online quantization.
        if online_method is None or isinstance(
            online_method, (UnquantizedLinearMethod, UnquantizedFusedMoEMethod)
        ):
            return checkpoint_method
        assert online_method is not None

        # Replace a checkpoint-unquantized layer with the requested online method.
        if checkpoint_method is None or isinstance(
            checkpoint_method, (UnquantizedLinearMethod, UnquantizedFusedMoEMethod)
        ):
            return online_method

        raise ValueError(
            "Online quantization requires the checkpoint to leave selected "
