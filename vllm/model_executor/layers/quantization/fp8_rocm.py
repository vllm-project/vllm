...
class Fp8RocmConfig(QuantizationConfig):
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> "Fp8RocmConfig":
...
class Fp8RocmLinearMethod(LinearMethodBase):
    ...
    def __init__(self, quant_config: Fp8RocmConfig):
    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        ...
        if not self.quant_config.is_checkpoint_fp8_serialized:
            qweight, weight_scale = ops.scaled_fp8_quant(layer.weight,
                                                         scale=None)
            qweight = qweight.to(torch.float16) / 2.0
            qweight = qweight.to(torch.float8_e4m3fnuz)
            layer.weight = Parameter(qweight.t(), requires_grad=False)
            layer.weight_scale = Parameter(weight_scale * 2.0, requires_grad=False)
        ...
        # weights into a single fp8 weight with a single weight scale.
            ...
            for idx, logical_width in enumerate(layer.logical_widths):
                ...
                start = end
            layer.weight_scale = Parameter(max_w_scale * 2.0, requires_grad=False)

            # WEIGHT
            #   Transpose weight for passing to torch._scaled_mm
            weight = layer.weight.to(torch.float16) / 2.0
            weight = weight.to(torch.float8_e4m3fnuz)
    ...
    def apply(self,
        ...
        qinput, x_scale = ops.scaled_fp8_quant(x,
                                               layer.act_scale)
        qinput = qinput.to(torch.float16) / 2.0
        qinput = qinput.to(torch.float8_e4m3fnuz)
        x_scale *= 2.0
