...
class QuarkMoEMethod(FusedMoEMethodBase):
    ...
    def get_moe_method(
        ...
        elif quant_config._is_w_ocp_mx_a_ocp_mx(weight_config, input_config):
            return QuarkOCP_MX_MoEMethod(weight_config, input_config, module.moe_config)
        elif quant_config._is_w_ocp_mx_weight_only(weight_config, input_config):
            return Quark_OCPMX_W4A16_MoEMethod(
...
# TODO (xuebwang-amd): unified with QuarkOCP_MX_MoEMethod as one?
class Quark_OCPMX_W4A16_MoEMethod(QuarkMoEMethod):
    def __init__(
        self,
        weight_config: dict[str, Any],
        input_config: dict[str, Any],
        ...
        if not weight_qscheme == "per_group":
            raise ValueError(
                "For MX(FP4) Fused MoE layers, only per-group scales "
                "for weights are supported. Found "
                f"{weight_qscheme}"
            )  # noqa E501
        ...
        if self.emulate:
            logger.warning_once(
                f"The current mode (supports_mx={current_platform.supports_mx()}, "
                f"use_mxfp4_aiter_moe={self.use_rocm_aiter_moe}, "
                f"mxfp4_backend={self.mxfp4_backend}, "
                f"ocp_mx_scheme={self.ocp_mx_scheme}) "
                "does not support native weight-only MXFP4/MXFP6 "
                "computation. Simulated weight dequantization "
                "will be used, with the linear layers computed in high precision."
            )
        else:
            logger.warning_once(
                "The current mode supports native MoE weight-only MXFP4 computation"
            )
...
class Quark_OCPMX_FP8_MoEMethod(QuarkMoEMethod):
    def __init__(
        ...
        weight_qscheme = self.weight_quant.get("qscheme")
        input_qscheme = self.input_quant.get("qscheme")
        if not (weight_qscheme == "per_group" and input_qscheme == "per_tensor"):
            raise ValueError(
                "For MX(FP4) Fused MoE layers, only per-group scales "
                "for weights and activations are supported. Found "
                f"{weight_qscheme}, {input_qscheme}"
        ...
        self.weight_dtype = self.weight_quant["dtype"].replace("fp", "mxfp")
        self.input_dtype = self.input_quant["dtype"].replace("fp8_e4m3", "fp8")
        self.fp4_dtype = getattr(torch, "float4_e2m1fn_x2", None)
        ...
        self.emulate = not current_platform.supports_mx() or not (
            self.use_rocm_aiter_moe and self.ocp_mx_scheme == "w_mxfp4"
        )
        if self.emulate:
            logger.warning_once(
                f"The current mode (supports_mx={current_platform.supports_mx()}, "
    ...
    def process_weights_after_loading(self, layer):
        # firstly, process fp8 activation
        if self.static_input_scales:
            if layer.w13_input_scale is None or layer.w2_input_scale is None:
                raise ValueError(
                    "QuantConfig has static quantization, but found "
