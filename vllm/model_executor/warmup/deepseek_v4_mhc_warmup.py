...
def _warmup_hc_head(
    ...
    # ``hc_head`` from a free function exported by ``deepseek_v4`` into the
    # ``HCHeadOp`` :class:`CustomOp` instance attached to the model as
    # ``hc_head_op``. We call through that instance so the warmup exercises
    # the same dispatched implementation as the inference path.
