...
def rms_norm_quant(input: torch.Tensor,
                                weight: torch.Tensor,
                                scale: torch.Tensor) -> torch.Tensor:
    hidden_size = input.size(-1);
    num_tokens = input.numel() // hidden_size;

    tmp = torch.empty((num_tokens, hidden_size), dtype=torch.float32, device="cuda")
    out = torch.empty((num_tokens, hidden_size), dtype=torch.float8_e4m3fn, device="cuda")
    torch.ops._C.rms_norm_quant(out, input, tmp, weight,
                                             scale, 1e-05)
    return out

def rms_norm_quant_meta(input: torch.Tensor,
                                weight: torch.Tensor,
                                scale: torch.Tensor) -> torch.Tensor:
    hidden_size = input.size(-1);
    num_tokens = input.numel() / hidden_size;

    return torch.empty((num_tokens, hidden_size), dtype=torch.float8_e4m3fn, device="cuda")

def rms_norm_quant_2(input: torch.Tensor,
                                weight: torch.Tensor,
                                scale: torch.Tensor) -> torch.Tensor:
    hidden_size = input.size(-1);
    num_tokens = input.numel() // hidden_size;
    ...
    torch.ops._C.rms_norm_quant(out, input, tmp, weight,
                                             scale, 1e-05)
    second_output = torch.empty(out.size(0), 6144, dtype=torch.float16, device="cuda")
    return (out, second_output)

def rms_norm_quant_meta_2(input: torch.Tensor,
                                weight: torch.Tensor,
                                scale: torch.Tensor) -> torch.Tensor:
    hidden_size = input.size(-1);
    num_tokens = input.numel() / hidden_size;
...
def setup_fused_rms_norm(cc: CodeCache):
    ...
    torch.library.impl(f"{ns_op}",
                       "Meta",
                       func=rms_norm_quant_meta)
    cc.add(rms_norm_quant_name,
           torch.ops.dogfood.rms_norm_quant)
...
def setup_fused_rms_norm_2(cc: CodeCache):
    ...
    torch.library.impl(f"{ns_op}",
                       "Meta",
                       func=rms_norm_quant_meta_2)
    cc.add(rms_norm_quant_name_2,
           torch.ops.dogfood.rms_norm_quant_2)
