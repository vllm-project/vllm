...
def triton_w4a16_skinny_fmt_gemm(
    ...
    if _on_gfx12x():
        ...
        if override is not None:
            ...
            if K >= 2 * N:  # tall K (e.g. down_proj)
                # BLOCK_N=512 gathers 512 rows of B per K-step on top of
                # BLOCK_M=128 rows of A, so both operands must be off the
                # stride cliff for it to pay off -- see _pad_activation_rows.
...
# multiple, which costs up to a fifth of the achievable read bandwidth. Nudging
# the stride one cache line off that multiple recovers it. The two operands sit
# on different cliffs, both measured on gfx1151: the packed weight row (K/2 B,
# streamed whole by the decode kernel) on 1024 B, the activation row (K*2 B,
# gathered BLOCK_M rows at a time by the prefill tile) on 2048 B.
_STRIDE_CLIFF_BYTES = 1024
_ACT_CLIFF_BYTES = 2048
