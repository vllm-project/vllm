...
"""POSIX shared memory double buffer with ROCm host registration for multi-rank
direct I/O.
...
def unregister_rocm_host_memory(buf: memoryview) -> bool:
    """Unregisters host memory buffer from ROCm page tables
    (hipHostUnregister).
    """
    lib = _get_hip_lib()
    if lib is None:
...
class SharedPinnedBufferPool:
    ...
    def _register_signal_traps(self) -> None:
        """Installs signal traps to ensure shared memory segments
    ...
    def _init_shared_memory(self) -> None:
        ...
        for slot_idx, name in enumerate(self.slot_names):
            if self.is_creator:
                ...
                shm = shared_memory.SharedMemory(
                    create=True, size=self.slot_size, name=name
                )
            else:
                ...
                if shm is None:
                    raise TimeoutError(
                        f"Timed out waiting for shared memory segment {name}"
                    )
        ...
        # Initialize or attach to control segment (128 bytes: active_slot,
        # shard_idx, seq, flags)
        ctrl_size = 128
        if self.is_creator:
            with contextlib.suppress(FileNotFoundError):
            ...
            self.ctrl_shm = shared_memory.SharedMemory(
                create=True, size=ctrl_size, name=self.ctrl_name
            )
            ...
            while time.time() < deadline:
                try:
                    self.ctrl_shm = shared_memory.SharedMemory(
                        create=False, name=self.ctrl_name
                    )
                    break
                except FileNotFoundError:
                    time.sleep(0.01)
            if self.ctrl_shm is None:
                raise TimeoutError(
                    f"Timed out waiting for control segment {self.ctrl_name}"
                )
        ...
        if self.tp_rank == 0:
            while True:
                ...
                if time.time() - start_time > timeout:
                    raise TimeoutError(
                        f"Rank 0 timed out waiting for barrier {next_gen}. "
                        f"Arrivals: {arrivals}"
                    )
                time.sleep(0.0005)
            ...
            while True:
                ...
                if time.time() - start_time > timeout:
                    raise TimeoutError(
                        f"Rank {self.tp_rank} timed out waiting for barrier "
                        f"{next_gen}. Current gen: {gen}"
                    )
                time.sleep(0.0005)
