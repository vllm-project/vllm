...
def initialize_dummy_weights(
    ...
    """
    # Check if we're doing online quantization with meta device weights.
    # This requires: quant method supports it AND checkpoint is not pre-quantized.
    quant_cls = get_quantization_config(model_config.quantization)
    uses_online_meta_device = (
        quant_cls.supports_online_meta_device and not model_config.is_quantized
    )

    for param in model.state_dict().values():
        if uses_online_meta_device and param.device == torch.device("meta"):
