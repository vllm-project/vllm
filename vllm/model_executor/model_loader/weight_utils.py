...
def gguf_quant_weights_iterator_multi(
    ...
    """
    Like gguf_quant_weights_iterator but handles multiple shard files.
    Yields all weight types from all shards first, then all weights.
    """
    import numpy as np

    # First pass: yield weight types from all shards
    ...
    # Second pass: yield weights from all shards
    for gguf_file in gguf_files:
        reader = gguf.GGUFReader(gguf_file)
        for tensor in reader.tensors:
            if tensor.name in gguf_to_hf_name_map:
