...
_NEURON_SUPPORTED_MODELS: Dict[str, Tuple[str, str, str]] = {
    "LlamaForCausalLM":
    ("transformers_neuronx.llama.model", "LlamaForSampling", "LlamaForCausalLM"),
    "MistralForCausalLM":
    ("transformers_neuronx.mistral.model", "MistralForSampling", "MistralForCausalLM")
}
    ...
    def sample(
        ...
        for seq_group in sampling_metadata.seq_groups:
            ...
            for seq_id in seq_group.seq_ids:
                ...
                sample_idx += 1
            next_tokens.append(
                CompletionSequenceGroupOutput(samples=samples, prompt_logprobs=None))
...
def _get_model_architecture(config: PretrainedConfig) -> str:
    ...
    for arch in architectures:
        if arch in _NEURON_SUPPORTED_MODELS:
            return arch
    raise ValueError(f"Model architectures {architectures} are not supported on Neuron "
...
def _get_buckets(env: str, default_value: List[int]) -> List[int]:
    ...
    buckets_remove_empty = filter(lambda x: x is not None and len(x.strip()) > 0,
                                  env_value.split(","))
...
def _get_default_neuron_config(model_config: ModelConfig,
    ...
    quant_config = dict(dequant_dtype=TORCH_DTYPE_TO_NEURON_AMP[model_config.dtype],
    ...
    default_neuron_args = dict(
        ...
        on_device_generation=_get_neuron_on_device_generation_config(model_config))
...
def _get_neuron_config_after_override(default_neuron_config, overridden_neuron_config):
    ...
    if continuous_batching:
        overridden_neuron_config["continuous_batching"] = ContinuousBatchingConfig(
            **continuous_batching)
...
def get_neuron_model(model_config: ModelConfig, parallel_config: ParallelConfig,
    ...
    default_neuron_config_args = _get_default_neuron_config(model_config,
                                                            parallel_config,
                                                            scheduler_config)
