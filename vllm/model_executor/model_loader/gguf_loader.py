...
class GGUFModelLoader(BaseModelLoader):
    ...
    def _get_all_gguf_files(model_path: str) -> list[str]:
        ...
        E.g. 00001-of-00005.gguf -> [00001, 00002, ..., 00005]
        """
        import re as re_module
        match = re_module.search(r'-(\d+)-of-(\d+)\.gguf$', model_path)
        if not match:
            return [model_path]
        ...
        for i in range(1, total + 1):
            shard_path = f"{prefix}{i:05d}-of-{total:05d}{suffix}"
    ...
    def _get_gguf_weight_type(
        ...
        for f in gguf_files:
            weight_type_map.update(
                get_gguf_weight_type_map(f, gguf_to_hf_name_map)
            )
