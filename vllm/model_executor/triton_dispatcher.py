...
class KernelOverride:
    ...
    def warmup(self, *args: Any, **kwargs: Any) -> None:
        """No-op: platform implementations need no Triton JIT compilation."""
        pass
...
def _rebind_kernels(
    ...
    * JIT warmup owners, whose ``kernel`` attribute is the kernel they
    ...
    for module in list(sys.modules.values()):
        ...
        if not namespace or not any(lookup(v) for v in namespace.values()):
            continue
        for attr, value in list(namespace.items()):
            ...
            if wrapper is not None:
                ...
                continue
            try:
                wrapper = lookup(getattr(value, "kernel", None))
            except Exception:
                # Module-level __getattr__ hooks may raise on unknown
                # attributes (e.g. torch.cuda lazily initializing CUDA).
                continue
            if wrapper is None:
                continue
