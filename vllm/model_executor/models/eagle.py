...
class EAGLE(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if (
            not hasattr(self.config.model, "skip_prenorm")
        ...
        if (
            not hasattr(self.config.model, "skip_output_norm")
        ...
        if (
            hasattr(self.config.model, "add_para_norm")
    ...
    def forward(
        ...
        if (
            previous_hidden_states.size(0) == 0
            or previous_hidden_states.size(0) != batch_size
        ):
            hidden_dim = self.config.model.hidden_size
            device = inputs_embeds.device
            # Create zero tensor with matching batch size
            previous_hidden_states = torch.zeros(batch_size, hidden_dim, device=device)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]):
        ...
        for name, loaded_weight in weights:
            if name == "token_map":
                ...
                weight_loader = getattr(
                    self.enorm.weight, "weight_loader", default_weight_loader
                )
                weight_loader(self.enorm.weight, loaded_weight)
            elif name.startswith("hnorm.weight"):
                weight_loader = getattr(
                    self.hnorm.weight, "weight_loader", default_weight_loader
                )
