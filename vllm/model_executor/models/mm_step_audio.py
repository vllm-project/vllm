...
try:
    ...
    raise ModuleNotFoundError(
        "StepAudio2ForCausalLM requires librosa. Install with `pip install librosa`."
    ) from exc
...
class StepAudio2ForCausalLM(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def _parse_and_validate_audio_input(
        ...
        if audio_lens is None:
            raise ValueError("audio_lens is required for StepAudio2 inputs.")

        if isinstance(audio_lens, torch.Tensor):
            if audio_lens.ndim == 0:
                audio_lens_list = [int(audio_lens.item())]
            else:
                audio_lens_list = audio_lens.flatten().tolist()
        elif isinstance(audio_lens, list):
            if not audio_lens:
                audio_lens_list = []
            elif all(isinstance(item, torch.Tensor) for item in audio_lens):
                audio_lens_list = torch.cat(
                    [item.flatten() for item in audio_lens]
                ).tolist()
            else:
                audio_lens_list = [int(item) for item in audio_lens]
        else:
            raise ValueError(
                "audio_lens must be a torch.Tensor or list for StepAudio2 inputs."
            )

        audio_mels_list: list[torch.Tensor] = []
        if isinstance(audio_mels, torch.Tensor):
            if audio_mels.ndim == 2:
                if audio_lens_list and sum(audio_lens_list) > audio_mels.size(0):
                    raise ValueError(
                        "audio_lens sum exceeds audio_mels length for StepAudio2 "
                        "inputs."
                    )
                if not audio_lens_list and audio_mels.numel() > 0:
                    raise ValueError(
                        "audio_lens is empty but audio_mels has data for "
                        "StepAudio2 inputs."
                    )
                cur_idx = 0
                for audio_len in audio_lens_list:
                    audio_mels_list.append(audio_mels[cur_idx : cur_idx + audio_len])
                    cur_idx += audio_len
                if cur_idx != audio_mels.size(0):
                    raise ValueError(
                        "audio_lens sum does not match audio_mels length for "
                        "StepAudio2 inputs."
                    )
            elif audio_mels.ndim == 3:
                if len(audio_lens_list) != audio_mels.size(0):
                    raise ValueError(
                        "audio_lens length does not match audio_mels batch size "
                        "for StepAudio2 inputs."
                    )
                audio_mels_list = [
                    audio_mels[i, : audio_lens_list[i]]
                    for i in range(len(audio_lens_list))
                ]
            else:
                raise ValueError(
                    "audio_mels must be a 2D or 3D torch.Tensor for StepAudio2 inputs."
                )
        elif isinstance(audio_mels, list):
            if not all(isinstance(item, torch.Tensor) for item in audio_mels):
                raise ValueError(
                    "audio_mels list must contain torch.Tensor items "
                    "for StepAudio2 inputs."
                )
            for item in audio_mels:
                if item.ndim != 2:
                    raise ValueError(
                        "audio_mels list items must be 2D tensors "
                        "for StepAudio2 inputs."
                    )
            audio_mels_list = [item for item in audio_mels]
        else:
            raise ValueError(
                "audio_mels must be a torch.Tensor or list for StepAudio2 inputs."
            )

        if not audio_mels_list:
            if audio_lens_list:
                raise ValueError(
                    "audio_mels is empty but audio_lens is not for StepAudio2 inputs."
                )
            return None

        if len(audio_lens_list) != len(audio_mels_list):
            raise ValueError(
                "audio_lens length does not match audio_mels items for StepAudio2 "
                "inputs."
            )

        max_len = max(x.size(0) for x in audio_mels_list)
        audio_mels = torch.stack(
            [F.pad(x, (0, 0, 0, max_len - x.size(0))) for x in audio_mels_list],
            dim=0,
        )

        return Step1fAudioInputs(
            audio_mels=audio_mels.to(self.dtype).to(self.device),
            audio_lens=audio_lens_list,
        )
    ...
    def embed_multimodal(self, **kwargs: object) -> MultiModalEmbeddings:
        ...
        return [] if audio_embeddings is None else audio_embeddings
    ...
    def get_input_embeddings(
        ...
        if multimodal_embeddings is not None:
            is_multimodal = input_ids == AUDIO_PATCH_TOKEN_ID
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]):
        ...
        params_need_to_load = set(params_dict)
        if params_need_to_load != loaded_params:
            param_name_example = list(params_need_to_load - loaded_params)[0]
            raise RuntimeError(
                "Some parameters like {} are not in the checkpoint and will "
                "falsely use random initialization".format(param_name_example)
            )
