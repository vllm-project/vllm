...
XingChen4 reuses the DeepSeek-V2/V3 backbone (MLA attention, MoE block) and
replaces the standard residual connection with Manifold-constrained
Hyper-Connections (mHC): the residual stream is expanded into ``hc_mult``
parallel streams that are mixed by input-dependent, doubly-stochastic
matrices produced by a Sinkhorn-Knopp projection.
...
class XingChen4Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if self.num_residual_streams > 1:
            ...
            if model_dtype != torch.bfloat16:
                raise ValueError(
                    "XingChen4 mHC (hc_mult > 1) requires "
            ...
            if vllm_config.parallel_config.pipeline_parallel_size > 1:
                raise NotImplementedError(
                    "XingChen4 mHC (hc_mult > 1) does not "
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        The mHC weights (``{attn,ffn}_hc.*``) live on the ``XingChen4MHC``
