...
class DeepseekV2MoE(nn.Module):
    def __init__(
        ...
        self.is_fused_shared_expert_enabled = bool(fuse_shared_experts)
        if config.n_shared_experts is not None and fuse_shared_experts is None:
            self.is_fused_shared_expert_enabled = resolve_layer_fused_shared_expert(
                quant_config, prefix
            )
        if self.is_fused_shared_expert_enabled and config.n_shared_experts:
            logger.info_once(
