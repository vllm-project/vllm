...
# ===================== V4.6 Specific Modules =====================


class ViTMergerAttention4_6(nn.Module):
...
class ViTMergerMLP4_6(nn.Module):
...
class ViTWindowAttentionMerger4_6(nn.Module):
    ...
    def __init__(self, vision_config: PretrainedConfig):
        ...
        self.self_attn = ViTMergerAttention4_6(hidden_size, num_heads)
        self.layer_norm1 = nn.LayerNorm(hidden_size)
        self.mlp = ViTMergerMLP4_6(hidden_size, intermediate_size, hidden_act)
        self.layer_norm2 = nn.LayerNorm(hidden_size)

        hidden_4x = hidden_size * 4
        inter_4x = intermediate_size * 4
        self.pre_norm = nn.LayerNorm(hidden_4x)
        self.linear_1 = nn.Linear(hidden_4x, inter_4x, bias=True)
        self.act = nn.GELU(approximate="tanh")
        self.linear_2 = nn.Linear(inter_4x, hidden_size, bias=True)
...
class DownsampleMLP4_6(nn.Module):
...
class MiniCPMV4_6(MiniCPMVBaseModel, SupportsLoRA, HasInnerState, IsHybrid):
    packed_modules_mapping = {
        "qkv_proj": [
            "q_proj",
            "k_proj",
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.vit_merger = ViTWindowAttentionMerger4_6(vision_config)
    ...
    def init_llm(
        ...
        try:
            llm = Qwen3_5ForCausalLM(vllm_config=vllm_config, prefix=prefix)
        finally:
    ...
    def get_vision_hidden_states(
        ...
        flat_mask = patch_attn_mask
        if self.vpm.apply_encoder_attention_mask and torch.any(~flat_mask):
            min_val = torch.finfo(hidden_states.dtype).min
            attention_mask = (
                (~flat_mask).to(dtype=hidden_states.dtype) * min_val
            )
    ...
    def _process_vision_input(
        ...
        result = []
        start = 0
        for ns in num_slices.tolist():
            group = vision_outputs[start : start + ns]
...
class MiniCPMV(
    MiniCPMVBaseModel, SupportsMultiModal, SupportsLoRA,
    ...
    def get_mamba_state_dtype_from_config(
        ...
        return MiniCPMV4_6.get_mamba_state_dtype_from_config(vllm_config)

    @classmethod
    def get_mamba_state_shape_from_config(
        cls, vllm_config: VllmConfig,
    ) -> tuple[tuple[int, int], tuple[int, int]]:
        return MiniCPMV4_6.get_mamba_state_shape_from_config(vllm_config)

    @classmethod
    def get_mamba_state_copy_func(
        cls,
    ) -> tuple[MambaStateCopyFunc, MambaStateCopyFunc]:
        return MiniCPMV4_6.get_mamba_state_copy_func()
