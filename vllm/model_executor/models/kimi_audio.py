...
class KimiAudioMultiModalProcessor(
    ...
    def _get_prompt_updates(
        ...
        # TODO: Cause lack of official examples, 
        # directly insert token should be a handled case.
...
class KimiAudioForConditionalGeneration(nn.Module, SupportsMultiModal,
    ...
    def _parse_and_validate_audio_input(self, **kwargs:
                                        object) -> Optional[KimiAudioInputs]:
    ...
    def _process_audio_input(self,
        ...
        if getattr(self.config, "use_whisper_feature"):
            ...
            media_start_idx = (audio_input_ids ==
                               self.kimia_media_begin).nonzero()
            media_end_idx = (audio_input_ids ==
                             self.kimia_media_end).nonzero()
            # shape: batch, seq_len, hidden_size
            ...
            expanded_whisper = torch.zeros(
                audio_emb.shape[1], whisper_input_dim,
                dtype=whisper_dtype,
    ...
    def _merge_multimodal_embeddings(
        ...
        audio_emb: MultiModalEmbeddings
    ) -> torch.Tensor:
