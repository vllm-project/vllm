...
from vllm.transformers_utils.processors import KimiAudioProcessor, WhisperEncoder
...
class KimiAudioMultiModalProcessor(BaseMultiModalProcessor[KimiAudioProcessingInfo]):
    ...
    def _get_prompt_updates(
        ...
        audio_tokenizer = processor.audio_tokenizer
        audio_placeholder_id = getattr(
            processor, "audio_placeholder_id", tokenizer.vocab_size + 10
        )
...
class MoonshotKimiaForCausalLM(
    ...
    def _process_audio_input(self, audio_input: KimiAudioInputs) -> torch.Tensor:
        ...
        # Audio is already loaded as numpy arrays in main process (in __call__)
        # Now we only need to extract whisper features in worker process
        if whisper_input_feature and isinstance(whisper_input_feature[0], np.ndarray):
            processed_features = []
            for audio_data in whisper_input_feature:
                # Convert numpy to tensor
                audio_tensor = torch.from_numpy(audio_data).unsqueeze(0)
                # Extract whisper features (in worker, has GPU and model)
