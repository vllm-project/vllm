...
class Qwen3MoeModel(nn.Module):
    ...
    def _load_weights_mxfp4(
        ...
        for name, loaded_weight in weights:
            ...
            if "gate_up_proj_blocks" in name:
                ...
                # flat loaded_weight from (E, 2 * N, block_size, entry_per_block)
                ...
                if use_ep:
                    ...
                    # Gate and up projection weights are stacked on top in the
                    # checkpoint. With TP we need to extract the relevant slices.
                    slice_idx_gate = torch.arange(tp_rank_start, tp_rank_end)
                    narrow_weight = loaded_weight[
                        :,
                        torch.cat([slice_idx_gate, intermediate_size + slice_idx_gate]),
                        ...,
                    ]
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        stacked_params_mapping_qkv = [
            # (param_name, shard_name, shard_id)
            ("qkv_proj", "q_proj", "q"),
            ("qkv_proj", "k_proj", "k"),
            ("qkv_proj", "v_proj", "v"),
        ]
        stacked_params_mapping_gate_up = [
        ...
        if quant_method == "mxfp4":
            return self._load_weights_mxfp4(
                weights, stacked_params_mapping_qkv, ignore_suffixes
            )
        else:
            return self._load_weights_other(
                weights,
                stacked_params_mapping_qkv + stacked_params_mapping_gate_up,
                ignore_suffixes,
            )
