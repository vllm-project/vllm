...
class DiffusionSampler:
    ...
    def __call__(
        ...
        if max_num_logprobs >= 0:
            ...
            # Commit steps: emit the stashed logprobs of requests committing
            # this step. Only a committing request may pop its own stash; a
            # co-batched request that merely converged this step keeps its
            # stash until its own commit, otherwise a peer's commit would
            # consume it one step early and the response would return fewer
            # logprob rows than tokens (#45689).
