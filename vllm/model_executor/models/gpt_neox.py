...
class GPTNeoXAttention(nn.Module):
    def __init__(self, config: GPTNeoXConfig):
        ...
        self.query_key_value = ColumnParallelLinear(
            config.hidden_size, 3 * config.hidden_size, gather_output=False, perform_initialization=False
        )
        self.dense = RowParallelLinear(
            config.hidden_size, config.hidden_size, input_is_parallel=True, perform_initialization=False
        )
...
class GPTNeoXMLP(nn.Module):
    def __init__(self, config: GPTNeoXConfig):
        ...
        self.dense_h_to_4h = ColumnParallelLinear(
            config.hidden_size, config.intermediate_size, gather_output=False, perform_initialization=False
        )
        self.dense_4h_to_h = RowParallelLinear(
            config.intermediate_size, config.hidden_size, input_is_parallel=True, perform_initialization=False
        )
...
class GPTNeoXModel(nn.Module):
    def __init__(self, config: GPTNeoXConfig):
        ...
        self.embed_in = VocabParallelEmbedding(config.vocab_size, config.hidden_size, perform_initialization=False)
...
class GPTNeoXForCausalLM(nn.Module):
    def __init__(self, config):
        ...
        self.embed_out = ColumnParallelLinear(
            config.hidden_size, config.vocab_size, bias=False, gather_output=False, perform_initialization=False
        )
    ...
    def forward(
        ...
        hidden_states = self.gpt_neox(input_ids, positions, kv_caches, input_metadata, cache_events)
