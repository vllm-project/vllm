...
IS_COMPUTE_8_OR_ABOVE =  (torch.cuda.is_available() and torch.cuda.get_device_capability()[0] >= 8)
...
class LocalStridedBlockSparseAttn(torch.nn.Module):
    def __init__(self, n_heads, max_seqlen, local_blocks, vert_stride, block_size,
        ...
        sparse_layout, sparse_pattern, self.dense_attn_mask = self.get_attn_pattern(dtype, device)
    ...
    def get_attn_pattern(self, dtype, device):
        sparse_layout, sparse_pattern, dense_attn_mask = get_sparse_attn_mask(self.n_heads,
                                                           self.max_seqlen,
                                                           self.max_seqlen,
                                                           dtype,
                                                           device,
...
class LocalStridedBlockSparsePagedAttn(torch.nn.Module):
    def __init__(self, n_heads, max_seqlen,
        ...
        self.local_blocks = local_blocks
        self.vert_stride = vert_stride
        sparse_layout, sparse_pattern, _ = get_sparse_attn_mask(n_heads, max_seqlen, max_seqlen, dtype, device,
    ...
    def get_remote_sparse_layout(self, n_heads, max_seqlen, dtype, device, block_size, local_blocks, vert_stride,
        ...
        _, sparse_pattern, _ = get_sparse_attn_mask(n_heads, max_seqlen, max_seqlen, dtype, device,
                                                block_size=block_size,
                                                local_blocks=local_blocks, vert_stride=vert_stride,
                                                homo_head=homo_head, return_dense=False)
    
        _, sparse_pattern_local, _ = get_sparse_attn_mask(n_heads, max_seqlen, max_seqlen, dtype, device,
                                                block_size=block_size,
