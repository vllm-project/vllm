...
@triton.jit
def _fwd_kernel_inner_with_blocktable(acc, l_i, m_i, q, k, v, Q, k_block_id,
                                      offs_n, sm_scale, q_pid,
...
def _fwd_kernel_batch_inference_with_blocktable(
    ...
    if is_local:
        ...
        for k_block_id in range(k_block_start, k_block_end - 1):
            k, v = _load_kv(k_block_id, K, V, Q, block_tables, stride_kb,
                            stride_kd, stride_kt, stride_kx, stride_vb,
                            stride_vd, stride_vt, stride_btb, stride_btt,
                            off_z, kv_scale, BLOCK_N, D_HEAD, X, IS_FP8,
                            BLOCK_D, EVEN_D)

            acc, l_i, m_i = _fwd_kernel_inner_with_blocktable(
                acc, l_i, m_i, q, k, v, Q, k_block_id, offs_n, sm_scale, q_pid,
                False, BLOCK_M_LOADING, BLOCK_N)

        k_block_id = k_block_end - 1
        k, v = _load_kv(k_block_id, K, V, Q, block_tables, stride_kb,
                        stride_kd, stride_kt, stride_kx, stride_vb, stride_vd,
                        stride_vt, stride_btb, stride_btt, off_z, kv_scale,
                        BLOCK_N, D_HEAD, X, IS_FP8, BLOCK_D, EVEN_D)

        acc, l_i, m_i = _fwd_kernel_inner_with_blocktable(
            acc, l_i, m_i, q, k, v, Q, k_block_id, offs_n, sm_scale, q_pid,
            True, BLOCK_M_LOADING, BLOCK_N)
        ...
        Out += off_z * stride_ot + (off_h_for_kv * q_k_ratio +
                                    start_local_head_idx) * stride_oh
        q_mask_h = tl.arange(0, BLOCK_M_LOADING)
        tl.store(Out + q_mask_h[:, None] * stride_oh +
                 offs_d[None, :] * stride_od,
                 acc,
                 mask=q_mask_h[:, None] < q_k_ratio)

        M += off_z * stride_mt + (off_h_for_kv * q_k_ratio +
                                  start_local_head_idx) * stride_mh
