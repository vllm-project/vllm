...
def _get_sparse_attn_mask_homo_head(q_len,
                                    max_seqlen,
                                    dtype,
                                    device,
                                    block_size=128,
    ...
    with torch.no_grad():
        ...
        block_mask_dense_output = block_mask_dense[-num_blocks_q:].contiguous(
        ).to_sparse_csr()
    if return_dense:
        ...
        return (block_mask_dense_output.crow_indices(),
                block_mask_dense_output.col_indices()
                ), block_mask_dense, mask_dense
    else:
        return (block_mask_dense_output.crow_indices(),
                block_mask_dense_output.col_indices()), block_mask_dense, None
...
@lru_cache
def get_sparse_attn_mask(n_heads,
                         q_len,
                         max_seqlen,
                         dtype,
                         device,
    ...
    if homo_head:
        with torch.no_grad():
            (crow, col
                 ...
                 q_len, max_seqlen, dtype, device, block_size, local_blocks,
                 vert_stride, return_dense)
    ...
    if return_dense:
        ...
        return dense_to_crow_col(
            block_mask_dense_output), block_mask_dense, mask_dense
    else:
        return dense_to_crow_col(
            block_mask_dense_output), block_mask_dense, None
