...
class CheXagentImagePixelInputs(TypedDict):
    type: Literal["pixel_values"]
    data: torch.Tensor
    """Shape: `(batch_size * num_images, num_channels, height, width)`"""


class CheXagentImageEmbeddingInputs(TypedDict):
...
class CheXagentQFormerAttention(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.attention = CheXagentQFormerMultiHeadAttention(
            config,
        ...
        self.output = CheXagentQFormerSelfOutput(config, prefix=f"{prefix}.output")
...
class CheXagentQFormerLayer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.attention = CheXagentQFormerAttention(
            config,
            quant_config=quant_config,
            cache_config=cache_config,
            prefix=f"{prefix}.attention",
        ...
        if self.has_cross_attention:
            self.crossattention = CheXagentQFormerAttention(
                config,
                quant_config=quant_config,
                cache_config=cache_config,
        ...
        self.intermediate = CheXagentQFormerIntermediate(
            config, prefix=f"{prefix}.intermediate")
        self.output = CheXagentQFormerOutput(config, prefix=f"{prefix}.output")

        self.intermediate_query = CheXagentQFormerIntermediate(
            config, prefix=f"{prefix}.intermediate_query")
        self.output_query = CheXagentQFormerOutput(config,
                                                   prefix=f"{prefix}.output_query")

        self.chunk_size_feed_forward = getattr(config,
...
class CheXagentProcessingInfo(BaseProcessingInfo):
    ...
    def get_num_image_tokens(self) -> int:
        ...
        return getattr(config, "num_query_tokens", 32)


class CheXagentDummyInputsBuilder(BaseDummyInputsBuilder[CheXagentProcessingInfo]):
...
class CheXagentMultiModalProcessor(BaseMultiModalProcessor[CheXagentProcessingInfo]):
...
@MULTIMODAL_REGISTRY.register_processor(CheXagentMultiModalProcessor,
                                        info=CheXagentProcessingInfo,
                                        dummy_inputs=CheXagentDummyInputsBuilder)
class CheXagentForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP,
                                        SupportsQuant):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Vision model for processing medical images
        self.vision_model = BlipVisionModel(config.vision_config, quant_config)

        # Query tokens for QFormer
        self.query_tokens = nn.Parameter(
            torch.zeros(1, config.num_query_tokens,
                        config.qformer_config.hidden_size))

        # QFormer for processing vision features
    ...
    def _parse_and_validate_image_input(
            self, **kwargs: object) -> Optional[CheXagentImageInputs]:
        ...
        if pixel_values is not None:
            ...
            return CheXagentImagePixelInputs(
                type="pixel_values",
        ...
        if image_embeds is not None:
            ...
            return CheXagentImageEmbeddingInputs(
                type="image_embeds",
                data=image_embeds,
            )
    ...
    def _process_image_pixels(self,
                              inputs: CheXagentImagePixelInputs) -> torch.Tensor:
    ...
    def _process_image_input(self,
                             image_input: CheXagentImageInputs) -> torch.Tensor:
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        return AutoWeightsLoader.load_weights(self, weights) 