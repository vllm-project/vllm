...
def fused_qk_rmsnorm_rope_gate(
    ...
    Replaces the unfused (split + GemmaRMSNorm + RoPE + gate chunk) sequence
    ...
    The RoPE is NeoX-style (the rotated dims are split into two contiguous
    halves) and the norm matches ``GemmaRMSNorm`` (``q_weight``/``k_weight`` are
    the effective gamma, i.e. ``weight + 1``).
