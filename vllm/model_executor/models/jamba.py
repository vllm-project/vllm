...
class JambaMambaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mamba(hidden_states, attn_metadata, conv_state, ssm_state)
        # Fully Connected
...
class JambaForCausalLM(nn.Module, HasInnerState, SupportsLoRA):
    ...
    def _swap_mamba_cache(self, from_index: int, to_index: int):
        ...
        for cache_t in self.mamba_cache:
            cache_t[:, [to_index, from_index]] = cache_t[:, [from_index, to_index]]
    ...
    def _move_out_if_already_occupied(
        self, index: int, all_occupied_indices: List[int]
    ):
        if index in all_occupied_indices:
            ...
            self._move_cache_index_and_mappings(
                from_index=index, to_index=first_free_index
            )

    def _assign_seq_id_to_mamba_cache_in_specific_dest(
        self, cur_rid: str, seq_id: int, destination_index: int
    ):
        """
        ...
        if cur_rid not in self.mamba_cache_indices_mapping:
            self._move_out_if_already_occupied(
                index=destination_index, all_occupied_indices=all_occupied_indices
            )
            ...
            # already exists
            cache_index_already_exists = self.mamba_cache_indices_mapping[cur_rid][
                seq_id
            ]
            if cache_index_already_exists != destination_index:
                ...
                self._swap_pair_indices_and_mappings(
                    from_index=cache_index_already_exists, to_index=destination_index
                )
    ...
    def _prepare_current_run_mamba_cache(
        ...
        for dest_index, (request_id, seq_id) in enumerate(
            request_ids_to_seq_ids_flatten
        ):
    ...
    def _clean_up_first_bs_blocks(
        self, batch_size: int, indices_for_current_run: List[int]
    ):
        ...
        for destination_index in destination_indices:
            if (
                ...
                all_other_indices = list(range(batch_size, max_possible_batch_size))
                first_avail_index = self._first_free_index_in_mamba_cache(
                    all_other_indices
                )
                self._swap_indices(
                    from_index=destination_index, to_index=first_avail_index
                )
    ...
    def _release_finished_and_prepare_mamba_cache(
        self, finished_requests_ids, request_ids_to_seq_ids
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        self._release_mamba_cache(finished_requests_ids)
        return self._prepare_current_run_mamba_cache(
    ...
    def _first_free_index_in_mamba_cache(
        self, indices_range: Optional[List[int]] = None
    ) -> int:
        ...
        for i in indices_range:
            if i not in all_occupied_indices:
                return i
        raise Exception(
            "Couldn't find a free spot in the mamba cache! This" "should never happen"
        )
    ...
    def _prepare_mamba_cache(self):
        ...
        self.mamba_cache = (
            torch.empty(
                size=(mamba_layers, max_batch_size) + conv_state_shape,
                dtype=dtype,
                device="cuda",
            ),
            torch.empty(
                size=(mamba_layers, max_batch_size) + temporal_state_shape,
                dtype=dtype,
                device="cuda",
            ),
        )
