...
class HyperCLOVAXOmniProcessingInfo(BaseProcessingInfo):
    ...
    def get_num_audio_tokens(
        ...
        **kwargs: object,
        ...
        if audio_processor is not None and hasattr(audio_processor, "get_num_audio_tokens"):
    ...
    def get_num_image_tokens(
        ...
        **kwargs: object,
        ...
        return image_processor.get_number_of_image_patches(
            image_height, image_width, images_kwargs=images_kwargs,
        )
...
class HyperCLOVAXOmniDummyInputsBuilder(BaseDummyInputsBuilder[HyperCLOVAXOmniProcessingInfo]):
    ...
    def get_dummy_mm_data(
        ...
        if audio_processor is not None:
            target_audio_length = min(getattr(audio_processor, "chunk_length", 30), 30) * audio_processor.sampling_rate
        else:
    ...
    def get_dummy_processor_inputs(
        ...
        tokenizer = self.info.get_tokenizer()
        added_vocab = tokenizer.get_added_vocab()
        image_token = "<|IMAGE_PAD|>" if "<|IMAGE_PAD|>" in added_vocab else getattr(hf_processor, "image_token", "<|IMAGE_PAD|>")
        video_token = "<|VIDEO_PAD|>" if "<|VIDEO_PAD|>" in added_vocab else getattr(hf_processor, "video_token", "<|VIDEO_PAD|>")
...
class HyperCLOVAXOmniMultiModalProcessor(
    ...
    def _call_hf_processor(
        ...
        # HCXVisionV2Processor requires that image/video tokens in text
        # have corresponding mm data. During profiling, text-only tokenization
        # may include placeholders without actual mm data — remove them.
    ...
    def _get_prompt_updates(
        ...
        placeholder = {}
        for mod, tok_str in [("image", "<|IMAGE_PAD|>"), ("video", "<|VIDEO_PAD|>"), ("audio", "<|AUDIO_PAD|>")]:
...
class HyperCLOVAXOmniForCausalLM(
    ...
    def _init_discrete_vision(self, config, vllm_config):
        ...
        if isinstance(raw_dvc, (dict, PretrainedConfig)):
            ...
            can_load = not (isinstance(raw_dvc, dict) and not raw_dvc.get("model_name_or_path"))
            if can_load:
    ...
    def _init_discrete_audio(self, config, vllm_config):
        ...
        if isinstance(raw_dac, (dict, PretrainedConfig)):
            can_load = not (isinstance(raw_dac, dict) and not raw_dac.get("model_name_or_path"))
            if can_load:
    ...
    def _process_image_input(
        ...
        merge_size = getattr(self.vision_model, "spatial_merge_size",
