...
"""Inference-only Xing4_0 model.

Xing4_0 reuses the DeepSeek-V2/V3 backbone (MLA attention, MoE block,
...
into ``hc_mult`` parallel streams that are mixed by
...
A local custom op (``xing4_0_transpose_contiguous``) ensures the transposed
...
if os.getenv("USE_FLAGOS") == "1":
    ...
    logger.info("Xing4_0: Flaggems is enabled.")
...
def _xing4_0_transpose_contiguous(x: torch.Tensor) -> torch.Tensor:
    """Transpose last two dims and enforce C-contiguity."""
    return x.transpose(-1, -2).contiguous()


def _xing4_0_transpose_contiguous_fake(x: torch.Tensor) -> torch.Tensor:
...
direct_register_custom_op(
    op_name="xing4_0_transpose_contiguous",
    op_func=_xing4_0_transpose_contiguous,
    mutates_args=[],
    fake_impl=_xing4_0_transpose_contiguous_fake,
)


def _xing4_0_ensure_contiguous(x: torch.Tensor) -> torch.Tensor:
    """Dispatch to the registered custom op (opaque to torch.compile)."""
    return torch.ops.vllm.xing4_0_transpose_contiguous(x)


# ============================== mHC adapter ==============================


class Xing4_0MHC(nn.Module):
    ...
    def __init__(self, config) -> None:
        super().__init__()
        # ``hc_mult`` (a.k.a. num_residual_streams on older checkpoints) is the
        # number of parallel mHC streams; <=1 disables mHC and falls back to the
        # standard DeepseekV2 residual path.
        self.n = getattr(config, "hc_mult", 1)
        self.hidden_size = config.hidden_size
        self.sinkhorn_iterations = getattr(config, "hc_sinkhorn_iters", 20)
        self.norm_eps = getattr(config, "hc_eps", 1e-6)
        self.pre_eps = getattr(config, "hc_eps", 1e-6)
        self.post_mult_value = getattr(config, "mhc_post_mult_value", 2.0)
        self.h_res_clamp_min = getattr(config, "mhc_h_res_clamp_min", None)
        self.h_res_clamp_max = getattr(config, "mhc_h_res_clamp_max", None)
        ...
        self.hc_base = nn.Parameter(torch.zeros(mix_hc, dtype=torch.float32))
    ...
    def forward(
        ...
        comb_mix = _xing4_0_ensure_contiguous(comb_mix)
...
class Xing4_0DecoderLayer(nn.Module):
    def __init__(
        ...
        if self.enable_mhc:
            self.attn_hc = Xing4_0MHC(config)
            self.ffn_hc = Xing4_0MHC(config)
    ...
    def forward(
        ...
        if self.enable_mhc:
            ...
            # --- attention sub-block ---
            aggregated, attn_h_res, attn_h_post = self.attn_hc(origin_hidden_states)
            attn_kwargs = {
                "positions": positions,
            ...
            if (
                not isinstance(self.self_attn, DeepseekV2Attention)
            ...
            # --- MLP sub-block ---
            aggregated, mlp_h_res, mlp_h_post = self.ffn_hc(hidden_states)
            mlp_out = self.mlp(self.post_attention_layernorm(aggregated))
            if isinstance(self.mlp, DeepseekV2MLP) and mlp_out.dtype == torch.float16:
...
class Xing4_0Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Backward compatibility: older Xing4_0 configs describe RoPE with
        ...
        if not hasattr(config, "rope_parameters") and hasattr(config, "rope_scaling"):
        ...
        if self.is_v32:
            ...
            for _attr in ("index_topk", "index_n_heads", "index_head_dim"):
                if hasattr(config, _attr) and getattr(config, _attr) is None:
                    ...
                    logger.info(
                        "xing4_0: non-DSA checkpoint has stale '%s' "
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: Xing4_0DecoderLayer(
                vllm_config,
                prefix,
                topk_indices_buffer=topk_indices_buffer,
            ),
        ...
        self.num_residual_streams = getattr(config, "hc_mult", 1)
        ...
        if self.num_residual_streams > 1:
            ...
            if model_dtype != torch.bfloat16:
                raise ValueError(
                    "Xing4_0 mHC (hc_mult > 1) requires "
            ...
            if vllm_config.parallel_config.pipeline_parallel_size > 1:
                raise NotImplementedError(
                    "Xing4_0 mHC (hc_mult > 1) does not "
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        """Load Xing4_0 weights.

        The mHC weights (``{attn,ffn}_hc.*``) live on the ``Xing4_0MHC``
        modules, which the DeepseekV2 loader does not know about, so load
        them directly here and delegate everything else to
        ...
        loaded_params.update(DeepseekV2Model.load_weights(self, other_weights))
...
class Xing4_0ForCausalLM(DeepseekV2ForCausalLM):
    model_cls = Xing4_0Model
    packed_modules_mapping = {
        "gate_up_proj": ["gate_proj", "up_proj"],
    }

    def set_moe_parameters(self):
        """Override for Xing4_0DecoderLayer (not a DeepseekV2DecoderLayer
        instance)."""
        self.expert_weights = []
