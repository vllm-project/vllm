...
class Llama4Model(LlamaModel):
    ...
    def load_moe_expert_weights(
        ...
        for param_name, weight_name, expert_id, shard_id in expert_params_mapping:
            ...
            # When fused=True, the transpose() on line 444 creates a
            # non-contiguous view of the 3D weight tensor. Subsequent
            # operations (chunk + indexing) preserve this non-contiguity.
            # Non-contiguous CPU tensors cause extremely slow DMA transfers
            # to GPU (3-4 seconds per tensor vs milliseconds), because the
            # transfer cannot use a single bulk memory copy. Making just
            # the final sliced tensor contiguous here is more efficient
            # than making the entire weight tensor contiguous after
            # transpose, since this operates on only the per-expert slice.
                ...
                if "experts." in name and any(
                    ...
                    if getattr(weight_loader, "supports_moe_loading", False):
                        ...
                        if (
                            ...
                            # contiguous() is needed because transpose()
                            # returns a non-contiguous view, which causes
