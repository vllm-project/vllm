...
class PerceptionEncoderRope2D(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, grid_hw: tuple[int,
                                                                       int]):
        if grid_hw[0] != self.max_grid_height or grid_hw[
                1] != self.max_grid_width:
...
class PerceptionEncoderMLP(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.fc1 = ColumnParallelLinear(input_dim,
                                        hidden_dim,
                                        bias=True,
        ...
        self.fc2 = RowParallelLinear(hidden_dim,
                                     input_dim,
                                     bias=True,
                                     quant_config=quant_config,
                                     prefix=f"{prefix}.fc2",
...
class PerceptionEncoderVisionAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.qkv_proj = QKVParallelLinear(embed_dim,
                                          self.head_dim,
        ...
        self.attn = MMEncoderAttention(self.num_heads, self.head_dim,
                                       self.scale)
        self.rope = PerceptionEncoderRope2D(dim=self.head_dim,
                                            max_grid_height=max_grid_height,
                                            max_grid_width=max_grid_width,
                                            use_cls_token=use_cls_token)

    def forward(self, x: torch.Tensor, grid_hw: tuple[int,
                                                      int]) -> torch.Tensor:
        ...
        q = q.permute(0, 2, 1, 3).reshape(bsz, seq_len,
                                          self.num_heads * self.head_dim)
        k = k.permute(0, 2, 1, 3).reshape(bsz, seq_len,
                                          self.num_heads * self.head_dim)
...
class PerceptionEncoderVisionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.ls_1 = (PerceptionEncoderLayerScale(d_model, ls_init_value)
                     if ls_init_value is not None else nn.Identity())
        self.ls_2 = (PerceptionEncoderLayerScale(d_model, ls_init_value)
                     if ls_init_value is not None else nn.Identity())
        self.ln_1 = norm_layer(d_model)
        self.ln_2 = norm_layer(d_model)
        hidden_dim = int(d_model * mlp_ratio)
        self.mlp = PerceptionEncoderMLP(d_model,
                                        hidden_dim,
                                        act_layer,
                                        quant_config=quant_config,
                                        prefix=f"{prefix}.mlp",
...
class PerceptionEncoderAttentionPooling(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp = PerceptionEncoderMLP(embed_dim,
                                        hidden_dim,
                                        act_layer,
                                        quant_config=quant_config,
                                        prefix=f"{prefix}.mlp",
    ...
    def forward(self, x: torch.Tensor):
        batch = x.shape[0]
        probe = self.probe.expand(batch, -1, -1).to(dtype=x.dtype,
                                                    device=x.device)
...
class PerceptionEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.ln_pre = (norm_layer(config.width)
                       if config.use_ln_pre else nn.Identity())
        self.ln_post = (norm_layer(self.width)
                        if config.use_ln_post else nn.Identity())
        ...
        self.vit_downsampler1 = nn.Conv2d(config.width,
                                          config.width * 2,
                                          kernel_size=3,
                                          stride=2,
                                          padding=1)
        self.vit_downsampler2 = nn.Conv2d(config.width * 2,
                                          config.width * 4,
                                          kernel_size=3,
                                          stride=2,
                                          padding=1)
...
class StepVLForConditionalGeneration(Step3VLForConditionalGeneration):
    ...
    def _get_vision_model_output(
            self,
            input_tensor: Optional[torch.Tensor]) -> Optional[torch.Tensor]:
