...
class Gemma4Config(VerifyAndUpdateConfig):
    @staticmethod
    def verify_and_update_config(vllm_config: "VllmConfig") -> None:
        """Prefer explicit TurboQuant, otherwise leave backend selection alone."""
        ...
        if (
            ...
            if cache_dtype.startswith("turboquant_"):
                ...
                logger.info(
                    ...
                    "TURBOQUANT backend for quantized layers; skipped "
                    "layers will fall back to TRITON_ATTN.",
                    head_dim,
                    global_head_dim,
                )
