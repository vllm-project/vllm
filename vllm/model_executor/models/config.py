...
class PanguUltraMoEForCausalLMConfig(VerifyAndUpdateConfig):
    ...
    def verify_and_update_config(cls, vllm_config: "VllmConfig") -> None:
        ...
        mla_page_size_1_token = (hf_config.kv_lora_rank + hf_config.qk_rope_head_dim) * dtype_size
        ...
        if (
            ...
            needed_block_size = round_up(
                needed_block_size, mla_block_alignment_size)
            cache_config.block_size = needed_block_size
            logger.info("Setting attention block size to %d to align hybrid KV.", cache_config.block_size)

        # Final page size for alignment
        target_page_size = cache_config.block_size * max_page_size_1_token
        if cache_config.mamba_page_size_padded is None or cache_config.mamba_page_size_padded < target_page_size:
