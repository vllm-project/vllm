...
if __name__ == "__main__":
    ...
    model = GemmaForCausalLM.from_pretrained(MODEL_NAME, config=config)
    ...
    backend = get_attn_backend(torch.bfloat16)

    bs = 8
    seq_len = 1
    context_len = 128
    max_num_blocks_per_seq = context_len // 16
    num_blocks = 128

    dummy_input_ids = torch.arange(bs * seq_len,
                                   dtype=torch.long,
                                   device=device)
    dummy_input_ids = dummy_input_ids.view(bs, seq_len)
    # NOTE: Currently, position_ids should be 1D due to index_select
    dummy_position_ids = torch.arange(bs * seq_len,
                                      dtype=torch.long,
                                      device=device)
