...
class AyaVisionForConditionalGeneration(nn.Module, SupportsMultiModal):
    ...
    def _parse_and_validate_image_input(
        ...
        if pixel_values.ndim == 5:
            ...
            num_patches = pixel_values.shape[1]
            embed_is_patch = torch.full((batch_size, ),
                                        True,
                                        dtype=torch.bool,
                                        device=pixel_values.device)
            num_embeds = torch.full((batch_size, ),
                                    num_patches,
                                    dtype=torch.int,
                                    device=pixel_values.device)
            num_patches = torch.full((batch_size, ),
                                     num_patches,
                                     dtype=torch.int,
                                     device=pixel_values.device)
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        if input_ids is not None:
            ...
            special_image_mask = torch.zeros_like(inputs_embeds,
                                                  dtype=torch.bool).to(
