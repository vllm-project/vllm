...
"""DFlareV2AllNorm Qwen3 draft model for speculative decoding."""
...
@triton.jit
def _fused_layer_context_allnorm_kernel(
    base_ptr,
    stacked_ptr,
    fusion_ptr,
    base_norm_weight_ptr,
    fusion_norm_weight_ptr,
    context_norm_weight_ptr,
    output_ptr,
    num_ctx,
    ...
    """Normalize the base and fusion branches, add, then normalize again."""
    row = tl.program_id(0)
    layer = row // num_ctx
    ...
    context_weight = tl.load(
        context_norm_weight_ptr + offsets, mask=mask, other=0.0
    ).to(tl.float32)
    tl.store(
        output_ptr + row * hidden_size + offsets,
        values * context_weight,
        mask=mask,
    )


class DFlareV2AllNormQwen3Model(DFlareV2Qwen3Model):
    """DFlareV2 with norms on both context branches before their residual add."""
    ...
    def __init__(
        ...
        *,
        ...
        self.base_context_norm = RMSNorm(
            self.config.hidden_size, eps=self.config.rms_norm_eps
        )
        self.fusion_context_norm = RMSNorm(
            self.config.hidden_size, eps=self.config.rms_norm_eps
        )
    ...
    def _project_context_kv(
        ...
        if num_layers != L:
            raise ValueError(
                f"DFlareV2AllNorm expected {L} draft layers, got {num_layers}."
            )
        if context_states.shape[-1] != T * D:
            raise ValueError(
                "DFlareV2AllNorm context width mismatch: "
        ...
        if context_states.is_cuda:
            ...
            _fused_layer_context_allnorm_kernel[(L * num_ctx,)](
                base_context,
                stacked,
                fusion_probs,
                self.base_context_norm.weight,
                self.fusion_context_norm.weight,
            ...
            base_flat = torch.empty_like(base_context)
            ops.rms_norm(
                base_flat,
                base_context,
                self.base_context_norm.weight,
            ...
            fusion = torch.einsum("lt,ntd->lnd", fusion_probs, stacked)
            fusion_flat = fusion.reshape(L * num_ctx, D)
            fusion_normed_flat = torch.empty_like(fusion_flat)
            ops.rms_norm(
                fusion_normed_flat,
                fusion_flat,
                self.fusion_context_norm.weight,
...
class DFlareV2AllNormQwen3ForCausalLM(DFlareV2Qwen3ForCausalLM):
    """Top-level AllNorm wrapper with native training-key remapping."""
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        self.model = DFlareV2AllNormQwen3Model(
            vllm_config=vllm_config,
