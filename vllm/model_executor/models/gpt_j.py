...
class GPTJModel(nn.Module):
    ...
    def __init__(self, config: GPTJConfig):
        ...
        self.wte = VocabParallelEmbedding(config.vocab_size, config.hidden_size,
                                               perform_initialization=False)
        self.h = nn.ModuleList([GPTNeoXLayer(config) for _ in range(config.n_layer)])
...
class GPTJForCausalLM(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.gpt_neox = GPTJModel(config)
    ...
    _column_parallel_weights = ["wte.weight", "lm_head.weight", "dense_h_to_4h.weight", "dense_h_to_4h.bias"]
