...
def input_processor_for_phio(
    ...
    # for offline_inference, we will get str input and we parse MM special tokens from it
    # (ignore prompt_token_ids)
    # for OAI server, we will get prompt_token_ids, where MM special tokens are already parsed
    ...
    if 'audio' in multi_modal_data:
        ...
        if prompt_str is not None:
            audio_id_to_input_ids = _get_audio_id_to_input_ids(audios, ctx, prompt_str=prompt_str)
            audio_embed_sizes = []
    ...
    if 'image' in multi_modal_data:
        ...
        if not isinstance(images, list):
            images = [images]
        if prompt_str is not None:
            image_id_to_input_ids = _get_image_id_to_input_ids(images, prompt_str, ctx)
            image_token_counts = []
        elif prompt_token_ids is not None:
            image_id_to_input_ids = {}
            image_token_counts = _count_image_tokens(images, ctx)
    ...
    # Handle the case where the prompt is a string and we need to manually tokenize it.
    # In this case, the `audio_id_to_input_ids` dict will be mapping from an audio placeholder
    # string (e.g. `<|audio_1|>`) to the audio placeholder tokens for the given audio length.
    if prompt_str:
    ...
    # Handle the case where the prompt is already tokenized
        ...
        assert prompt_token_ids is not None, "If string prompt isn't provided, prompt_token_ids must be"

        i = 0
        input_ids = prompt_token_ids
        img_cnt, audio_cnt, user_text_input_cnt = 0, 0, 0  # only needed for later assertion
        ...
        while i < len(input_ids):
            ...
            if token_id == _AUDIO_PLACEHOLDER_TOKEN_ID:
                ...
                user_text_input_cnt += 1 if token_id not in NON_USER_INPUT_TOKENS else 0
                i += 1
                continue
        ...
        assert (
            img_cnt == len(image_token_counts)
        ), (
...
class PhiOForCausalLM(nn.Module, SupportsLoRA, SupportsMultiModal):
    ...
    def _audio_features_to_embeddings(
        ...
        kwargs = {"wte": self.model.embed_tokens, 'audio_projection_mode': audio_projection_mode}
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]) -> None:
        ...
        for name, weight in weights.items():
            ...
            name = (
                ...
                .replace("model.embed_tokens_extend.image_embed.", "vision_encoder.")
            )
