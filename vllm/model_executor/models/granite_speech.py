...
class GraniteSpeechMultiModalProcessor(
    ...
    def _get_mm_fields_config(
        ...
        return dict(
            ...
            # Only consumed as Python split sizes / scalars; keep CPU-resident.
...
class GraniteSpeechForConditionalGeneration(
    ...
    def _build_input_features_mask(
        ...
        """
        # `audio_embed_sizes` is CPU-resident (see `_get_mm_fields_config`).
        # Build the mask on CPU and pin+non-blocking upload to the encoder's
        # device — the boolean mask is later used to index GPU
        # `projected_embeds` in `_process_audio_input`.
