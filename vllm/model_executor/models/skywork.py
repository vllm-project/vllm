...
class SkyWorkLMHeadModel(nn.Module):
    ...
    def load_weights(self,
        ...
        print("SkyWork load_weights")
        tensor_model_parallel_world_size = (
            get_tensor_model_parallel_world_size())
        tensor_model_parallel_rank = get_tensor_model_parallel_rank()
        state_dict = self.state_dict()
        print("tensor_model_parallel_world_size: ", tensor_model_parallel_world_size)
        print("tensor_model_parallel_rank: ", tensor_model_parallel_rank)

        def pytorch_model_weights_iterator(
            model_name_or_path: str,
            cache_dir: Optional[str] = None,
            use_np_cache: bool = False,
        ) -> Iterator[Tuple[str, torch.Tensor]]:
            # Be sure that The pytorch model is located on local dir
            pytorch_model_files = [
                x for x in glob.glob(os.path.join(model_name_or_path, "*.pt"))
            ]
            print("pytorch_model_files: ", pytorch_model_files)
            for model_file in pytorch_model_files:
                print("Start load model file.")
                start = time.time()
                model = torch.load(model_file, map_location="cpu")
                elasped = time.time() - start
                print(f"Load finished! Elasped {elasped / 60.0} min.")
                for name, param in model.named_parameters():
                    yield name, param
        
        start = time.time()
        print("Start split tensor into Multi-GPUs.")
        for name, loaded_weight in pytorch_model_weights_iterator(
        ...
        elasped = time.time() - start
        print(f"Split finished! Elasped {elasped / 60.0} min.")