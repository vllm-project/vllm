...
Follows the DeepSeek-V3 NextN layout (checkpoint weights under
``model.layers.{num_hidden_layers + i}.*``), with GigaChat's gated norms and a
dense full-attention draft block. The draft has no GDN state, so the model is
neither ``IsHybrid`` nor ``MixtureOfExperts``.
"""
...
class GigaChat35MultiTokenPredictorLayer(nn.Module):
    def __init__(self, vllm_config: VllmConfig, prefix: str) -> None:
        ...
        if getattr(config, "nextn_is_sparse", False):
            raise NotImplementedError(
                "GigaChat 3.5 MTP with nextn_is_sparse=True is not supported: "
                "expert weights are not loaded for the draft model."
            )
    ...
    def forward(
        ...
        # compute_logits applies shared_head's norm to the pre-norm element.
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                # embed_tokens is hoisted to model.*; skip the copies shipped
                # with the later NextN layers.
                if (
        ...
        # A checkpoint may declare more NextN layers than it actually ships.
        ...
        for layer_idx in range(
            ...
            if layer_idx not in loaded_layers:
                raise ValueError(
                    ...
                    f"from the checkpoint; set num_nextn_predict_layers to "
                    f"the number of NextN layers actually present."
                )
