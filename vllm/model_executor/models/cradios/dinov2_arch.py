...
try:
    if XFORMERS_ENABLED:
        from xformers.ops import (SwiGLU, fmha, index_select_cat,
...
class NestedTensorBlock(Block):
    ...
    def forward_nested(self, x_list: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            ...
            def ffn_residual_func(x: torch.Tensor,
                                  attn_bias=None) -> torch.Tensor:
            ...
            x_list = drop_add_residual_stochastic_depth_list(
                ...
                residual_func=attn_residual_func,
                sample_drop_ratio=self.sample_drop_ratio,
                scaling_vector=self.ls1.grandma if isinstance(
            ...
            x_list = drop_add_residual_stochastic_depth_list(
                ...
                residual_func=ffn_residual_func,
                sample_drop_ratio=self.sample_drop_ratio,
                scaling_vector=self.ls2.grandma if isinstance(
                    self.ls1, LayerScale) else None,
            )
            return x_list
        else:
