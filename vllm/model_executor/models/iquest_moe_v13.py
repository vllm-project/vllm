...
class IquestMoeAttention(nn.Module):
    ...
    def sinks_k_weight_loader(self, param: nn.Parameter, loaded_weight: torch.Tensor):
        ...
        # the shape of loaded_weight is [head_num, head_dim]
        new_loaded_weight = loaded_weight[None, :]
        loaded_weight = new_loaded_weight
...
class IquestMoeV13ForCausalLM(nn.Module, SupportsLoRA):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        loader = AutoWeightsLoader(self, skip_prefixes=["mtp_layers."])
