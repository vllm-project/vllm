...
class VoxtralRealtimeGeneration(VoxtralForConditionalGeneration, SupportsRealtime):
    ...
    async def buffer_realtime_audio(
        ...
        # raised an uncaught TimeoutError on any silence longer than the timeout,
        # silently killing this feeder task while the websocket stayed open --
        # the session hung with no further tokens (vllm-project/vllm#36015,
        # #35863). The v1 engine has no per-iteration timeout watchdog, and the
