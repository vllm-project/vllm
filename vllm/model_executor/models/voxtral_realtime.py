...
class VoxtralRealtimeGeneration(VoxtralForConditionalGeneration, SupportsRealtime):
    ...
    async def buffer_realtime_audio(
        ...
        # idle wait here is normal (e.g. gaps between tokens or utterances) and
        # must not be treated as a hang. A real engine failure instead makes
        # generate() raise and aborts the request, which cancels this task via
        # the generator's finally block below, so this wait has no timeout.
