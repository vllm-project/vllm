...
class LocateAnythingSlowLogitsProcessor:
    ...
    def _allowed_token_ids(
        ...
        coords = set(range(ids.coord_start, ids.coord_end + 1))
        if not box_tokens:
...
class LocateAnythingMultiModalProcessor(
    ...
    def _get_prompt_updates(
        ...
        # Look up the <img> / </img> wrapper token IDs from the tokenizer.
        hf_processor = self.info.get_hf_processor(**hf_processor_mm_kwargs)
        img_start_id = hf_processor.tokenizer.convert_tokens_to_ids("<img>")
        img_end_id = hf_processor.tokenizer.convert_tokens_to_ids("</img>")
        unk_id = hf_processor.tokenizer.unk_token_id
...
class LocateAnythingForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        self.image_token_id: int = config.image_token_index
        # Cache vision tower dtype — never changes after init.
        self._vision_dtype = next(self.vision_tower.parameters()).dtype
    ...
    def _process_image_pixels(
        ...
        pixel_values = pixel_values.to(dtype=self._vision_dtype)
