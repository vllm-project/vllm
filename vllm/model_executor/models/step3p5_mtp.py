...
class Step3p5MTP(nn.Module):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                for mapping in expert_params_mapping:
                    ...
                    for idx in range(
                        mtp_start_layer_idx, mtp_start_layer_idx + num_mtp_layers
                    ):
                        if f"{idx}.transformer." in name:
        ...
        if params_need_to_load != loaded_params:
            ...
            raise RuntimeError(
                f"Some parameters like {param_name_example} are not in the checkpoint"
                f" and will falsely use random initialization"
            )
