...
class SPLADESparsePooler(Pooler):
    ...
    SPLADE sparse pooling:
      logits = mlm_head(hidden_states) -> log1p(relu(logits)) -> (max|sum over L) -> [V]
    ...
    def forward(
        ...
        if self.remove_cls_sep and token_ids is not None:
            for i, L in enumerate(lens):
                ...
                if L <= 0:
                    continue
                if self.cls_token_id is not None and int(token_ids[i, 0].item()) == self.cls_token_id:
                    attn_mask[i, 0] = False
                if self.sep_token_id is not None and int(token_ids[i, L - 1].item()) == self.sep_token_id:
        ...
        if self.pooling == "sum":
            ...
            pooled = torch.where(torch.isneginf(pooled), torch.zeros_like(pooled), pooled)

        if self.sparsify_threshold > 0.0:
            th = self.sparsify_threshold
            pooled = torch.where(pooled > th, pooled, torch.zeros_like(pooled))
...
class BertSpladeSparseEmbeddingModel(BertEmbeddingModel):
    ...
    - pooler: "embed-sparse" → SPLADESparsePooler(mlm_head...)
    """
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "", splade_pooling: str = "max"):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]):
        ...
        loader_top = AutoWeightsLoader(self, skip_prefixes=["pooler.", "mlm_head.", "lm_head."])
