...
_I = TypeVar("_I", bound=BaseProcessingInfo, default=Qwen3OmniMoeThinkerProcessingInfo)


class Qwen3OmniMoeThinkerMultiModalProcessor(
    Qwen2_5OmniThinkerMultiModalProcessor[_I],
):
    def _get_hf_mm_data(
        self,
    ...
    def _apply_hf_processor_main(
        ...
        if mm_items.get_all_counts().get("audio", 0):
            ...
            for key in ("audio_kwargs", "text_kwargs"):
                nested_kwargs = hf_processor_mm_kwargs.get(key) or {}
                assert isinstance(nested_kwargs, Mapping)
                hf_processor_mm_kwargs[key] = dict(nested_kwargs)
