...
#
# This code is based on sarvam.py and bailing_moe.py implementations
# in this library. It has been modified from its original forms to
# accommodate Param2MoE's GQA-based MoE architecture.
#
...
class Param2MoEAttention(nn.Module):
    ...
    def __init__(
        ...
        # individual base=/rope_scaling= kwargs (see sarvam.py for reference).
...
class Param2MoEForCausalLM(
    ...
    def load_weights(
        ...
        """
        Load weights from a Param2MoE HuggingFace checkpoint.

        The method:
          1. Renames HF weight names → vLLM internal names.
          2. Zero-means the expert-score bias for load balance.
          3. Splits the fused ``query_key_value`` into separate q/k/v shards
             that ``QKVParallelLinear`` can ingest.
          4. Fuses ``gate_proj`` + ``up_proj`` → ``gate_up_proj`` for both
             the dense MLP layers and the shared-expert MLP layers.
          5. Routes routed-expert weights via the fused-MoE mapping.
        """
        config = self.config
        ...
        # checkpoint into the single gate_up_proj in vLLM.
        # This applies to both dense layers AND shared-expert MLPs.
        ...
        for name, loaded_weight in _rename_and_normalize_weights(weights):
            ...
            #    We split it into q / k / v slices and load each separately
            #    so that QKVParallelLinear handles TP sharding correctly.
            # ----------------------------------------------------------------
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                if weight_name not in name:
                    continue
                # Skip routed expert weights (handled below via fused MoE)
            ...
            # ----------------------------------------------------------------
            # 4. All other weights: direct load (layernorms, lm_head, etc.)
            # ----------------------------------------------------------------
