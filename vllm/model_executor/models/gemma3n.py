    ...
    def __init__(
        ...
        if self.is_kv_shared:
            ...
            if kv_shared_layer_index >= 0:
                # Different model wrappers expose layer parameters under different parent attributes.
                # For example:
                #   - Gemma3nForCausalLM → parameters live under "model.layers"
                #   - Gemma3nForConditionalGeneration → under "language_model.model.layers"
                # This logic extracts the portion of the parameter name *before* ".layers."
                # so downstream code can consistently reference the correct model root
                # regardless of which wrapper class was used.
                if ".layers." in prefix:
