...
class ZayaAttention(nn.Module):
    def __init__(
        ...
        self.cca_num_q_heads = config.cca_num_q_heads
...
class ZayaRouter(nn.Module):
    ...
    def forward(
        ...
        if self.router_softmax_fp32:
            # Keep router selection numerically stable without changing expert compute dtype.
...
class ZayaForCausalLM(nn.Module, HasInnerState, IsHybrid):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # assert not vllm_config.scheduler_config.enable_chunked_prefill, "Chunked prefill is not supported by Zaya for now!"
        ...
        self.model = ZayaModel(vllm_config=vllm_config,
