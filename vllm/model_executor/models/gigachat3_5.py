...
class GigaChat35ZeroCenteredRMSNorm(nn.Module):
    def __init__(self, hidden_size: int, eps: float = 1e-6, scale: float = 1.0) -> None:
        ...
        # MLA alpha_q/alpha_kv. Applied to the fp32 norm output since a scalar
        # cannot be folded into fp8 weights.
...
class GigaChat35Attention(nn.Module):
    ...
    def __init__(
        ...
        # Applied at the q_a/kv_a layernorm output via _build_norm(scale=...).
...
class GigaChat35Model(nn.Module):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        # NextN layers are dropped in GigaChat35ForCausalLM.load_weights.
