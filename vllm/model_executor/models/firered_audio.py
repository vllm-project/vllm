...
class FireRedAudioProcessingInfo(BaseProcessingInfo):
...
class FireRedAsrDummyInputsBuilder(BaseDummyInputsBuilder[FireRedAudioProcessingInfo]):
...
class FireRedAsrMultiModalProcessor(
    BaseMultiModalProcessor[FireRedAudioProcessingInfo]
):
...
@MULTIMODAL_REGISTRY.register_processor(
    ...
    info=FireRedAudioProcessingInfo,
...
class FireRedAudioForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def _validate_and_reshape_mm_tensor(
        self,
        mm_input: torch.Tensor | list[torch.Tensor] | None,
        name: str,
    ) -> torch.Tensor:
    ...
    def embed_multimodal(self, **kwargs: object) -> MultiModalEmbeddings:
        ...
        max_length = int(projected_lengths.max()) if projected_lengths.numel() else 0
        ...
        return tuple(
            audio_embeddings[i, : int(projected_lengths[i].item())]
            for i in range(batch_size)
        )
