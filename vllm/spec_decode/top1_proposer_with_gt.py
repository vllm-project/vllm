...
class Top1Proposer(SpeculativeProposer):
    ...
    def get_spec_proposals(
        ...
        if self._is_fake:
            ...
            for k, v in execute_model_req.seq_group_metadata_list[0].seq_data.items():
                ib = len(v.output_token_ids)
                break

            ie = ib + self.spec_steps

            proposals = self._gt_tokens[ib:ie]
            probs = torch.zeros((1, self.spec_steps, self._vocab_size))
            for i, token in enumerate(proposals):
                probs[0, i, token] = 1.0

            proposals = SpeculativeProposals(
                proposal_token_ids=torch.Tensor([proposals]).long().cuda(),
                proposal_probs=probs.cuda(),
                proposal_lens=torch.Tensor([self.max_proposal_len]).long().cuda(),
                no_proposals=False,
            )
