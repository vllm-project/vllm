...
def get_sampled_token_logprobs(
    ...
    sampled_token_ids_ranks = (
        (logprob_tensor > expanded_selected_logprobs).sum(-1).add_(1)
    )
    ...
    logprobs.update(
        {
            ...
            for topk_index, (topk_token_id, topk_logprob) in enumerate(
                zip(topk_token_ids, topk_logprobs)
            )
...
def create_sequence_group_output(
    ...
    return CompletionSequenceGroupOutput(
        samples=[
            SequenceOutput(
                parent_seq_id=seq_id, output_token=token_id, logprobs=logprobs
            )
        ],
...
def maybe_mock_device_tensors(
    sampler_output: SamplerOutput, batch_size: int, vocab_size: int, device: str
) -> None:
    ...
    sampler_output.sampled_token_probs = torch.nn.functional.softmax(
        torch.rand(batch_size, vocab_size, dtype=torch.float32, device=device), dim=-1
    )

    sampler_output.sampled_token_ids = torch.randint(
        low=10, high=100, size=(batch_size,), dtype=torch.long, device=device
    )
