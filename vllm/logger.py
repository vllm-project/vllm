...
class VllmJsonFormatter(jsonlogger.JsonFormatter if HAS_JSON_LOGGER else logging.Formatter):
    ...
    def add_fields(self, log_record: dict, record: logging.LogRecord, message_dict: dict) -> None:
        ...
        if record.stack_info:
            ...
            log_record['stack_info'] = [line.strip() for line in self.formatStack(record.stack_info).split('\n') if
                                        line.strip()]
...
def _install_json_exception_handler() -> None:
    ...
    if hasattr(sys.excepthook, '_vllm_json_installed'):
        return
    ...
    def json_excepthook(exc_type, exc_value, exc_traceback):
        ...
        # Log the exception through vllm logger
        logger.critical(
            "Uncaught exception",
    ...
    # Mark as installed
    json_excepthook._vllm_json_installed = True
    ...
    if sys.version_info >= (3, 8):
        ...
        def json_threading_excepthook(args):
            ...
            # args is a named tuple: (exc_type, exc_value, exc_traceback, thread)
            if issubclass(args.exc_type, KeyboardInterrupt):
                original_threading_excepthook(args)
                return

            # Log the exception with thread info
            logger.critical(
...
DEFAULT_LOGGING_CONFIG = {
    ...
    "handlers": {
        "vllm": {
            ...
            "formatter": (
                "vllm_json" if _use_json_logging()
...
def _configure_vllm_root_logger() -> None:
    ...
    if hasattr(envs, 'VLLM_LOGGING_JSON') and envs.VLLM_LOGGING_JSON == "1" and not HAS_JSON_LOGGER:
        logging.warning(
