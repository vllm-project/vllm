...
from typing import Dict, AbstractSet, Type, Union
...
class _LoRAResolverRegistry:
    ...
    def register_resolver(
        ...
        if not (isinstance(resolver_cls, type) and issubclass(resolver_cls, LoRAResolver)):
            raise TypeError(
                f"`resolver_cls` should be a LoRAResolver subclass, not a {type(resolver_cls)}")
