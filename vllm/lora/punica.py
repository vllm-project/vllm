...
def convert_mapping(
    ...
    if long_lora_context:
        long_lora_offsets = torch.zeros(
            len(index_mapping_indices), device="cuda", dtype=torch.long
        )
...
class PunicaWrapper:
    ...
    def __init__(self, max_num_batched_tokens: int, max_batches: int, device: str):
    ...
    def expand_slice_decode(
        ...
        bgmv_expand_slice(
            x, w_t_all, y, self.token_lora_indices, y_offset, y_slice_size, add_input
        )
    ...
    def add_shrink(
        ...
        """
        shrink_fun: Callable = (
            self.shrink_prefill if self.is_prefill else self.shrink_decode
        )
    ...
    def add_expand(
        ...
        expand_fun: Callable = (
            self.expand_prefill if self.is_prefill else self.expand_decode
        )
    ...
    def add_expand_slice(
        ...
        expand_slice_fun: Callable = (
            self.expand_slice_prefill if self.is_prefill else self.expand_slice_decode
        )
        ...
        *,
        ...
        if y_offset is None and y_slice_size is None:
            ...
            self.add_expand_slice(
                y, buffer, wb_t_all, y_offset, y_slice_size, add_input=True
            )
        y = y.view_as(y_org)
