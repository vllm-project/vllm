...
import torch
from vllm_hpu_extension.ops import dispatch_bgmv_embedding, dispatch_bgmv_linear
...
class PunicaWrapperHPU(PunicaWrapperBase):
    def __init__(
        ...
        **kwargs,
        ...
        PunicaWrapperBase.__init__(
            self, 3 * max_num_batched_tokens, max_batches, device
        )
    ...
    def _update_base_metadata(
        ...
        if long_lora_context:
            ...
            long_lora_offsets_tensor = torch.tensor(
                long_lora_offsets, device=self.device, dtype=torch.long
            )
