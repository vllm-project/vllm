...
def _bgmv_embed_kernel(
    ...
    offsets_embed = cur_tokens[:, None] * HIDDEN_DIM + hidden_range[None, :]  # Shape: (BLOCK_N, HIDDEN_DIM)

    # Load embeddings from embed_tokens_base
    embeddings_base = tl.load(embed_tokens_base + offsets_embed, mask=mask[:, None], other=0.0)
    ...
    base_offsets_all = tl.where(mask_all, lora_index * HIDDEN_DIM * VOCAB_SIZE, 0)
    ...
    embeddings_all = tl.load(embed_tokens_all + full_offsets_all, mask=mask_all[:, None], other=0.0)
    # Overwrite embeddings_block where lora_index != -1
    embeddings_block = tl.where(mask_all[:, None], embeddings_all, embeddings_block)
