...
def _sgmv_expand_slice_kernel(
    ...
    c_mask = (offset_cm[:, None] < (cur_seq_start + M)) & (offset_cn[None, :] <
                                                           (slice_offset + N))
    if ADD_INPUTS:
        # explicitly pass in other=None to tell triton that masked values can be uninitialized. This is OK because the later tl.store operation uses the same mask, eliminating the risk of garbage values propagating
