...
class FusedMoEWithLoRA(BaseLayerWithLoRA):
    ...
    def register_jit_warmups(
        ...
        *,
        ...
        """Register the fused-MoE kernels selected by this LoRA layer."""
        if not HAS_TRITON:
            return
        ...
        top_k_num = self.moe_config.experts_per_token
        sm_count = current_platform.num_compute_units(self.device.index)
        weight_pairs = (
            (
        ...
        for lora_a, lora_b, shrink_op, expand_op, output_size in weight_pairs:
            ...
            if not self.fully_sharded:
                max_npid_factor = min(
                    ...
                    max(
                        ...
                        int(
                            1.5
                            / max(
                                a0.shape[3] / max(a0.shape[3] + b0.shape[2], 1),
                                1e-3,
                            )
                        )
                        + 1,
                ...
                _FUSED_MOE_LORA_ONE_SHOT_KERNEL.register_warmup(
                    ...
                    config_hidden_size=self.hidden_size,
                    moe_intermediate_size=self.intermediate_size_per_partition,
                    max_shared_memory=get_max_shared_memory_bytes(self.device.index),
                    **common,
                )
