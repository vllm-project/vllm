...
class DoRALayerWeights(LoRALayerWeights):
    def __init__(
        self,
        module_name: str,
        rank: int,
        ...
        self.magnitude_param = magnitude_param
    ...
    def create_dummy_dora_weights(
        ...
        magnitude_param = torch.zeros(
            [rank], dtype=dtype, device=device, pin_memory=pin_memory
        )
        if bias_enabled:
            bias = torch.zeros(
                [output_dim], dtype=dtype, device=device, pin_memory=pin_memory
        ...
        return cls(
            ...
            lora_a=lora_a,
            lora_b=lora_b,
            magnitude_param=magnitude_param,  # for magnitude_param
            bias=bias,
            embeddings_tensor=embeddings_tensor,
        )
...
class PackedDoRALayerWeights(DoRALayerWeights):
    """DoRA used for packed layers (eg. qkv_proj)."""
    ...
    def __init__(
        ...
        lora_a: List[Optional[torch.Tensor]],
        lora_b: List[Optional[torch.Tensor]],
        magnitude_params: List[Optional[torch.Tensor]],
        bias: Optional[List[Optional[torch.Tensor]]] = None,
        scaling: Optional[List[float]] = None,
    ) -> None:
        # Initialize with dummy values that will be overridden
        super().__init__(
            module_name=module_name,
            rank=rank,
            lora_alpha=0,
            lora_a=lora_a,
            lora_b=lora_b,
            magnitude_param=magnitude_params[0] if magnitude_params else None,
    ...
    @classmethod
    def pack(
        cls, loras: GenericSequence[Optional["DoRALayerWeights"]]
    ) -> "PackedDoRALayerWeights":
        """Pack a list of DoRAs into a single DoRA.

        If DoRA is None, it signifies that the submodule does not have a DoRA.
        """
        first_lora = next(lora for lora in loras if lora is not None)
        for lora in loras:
            if lora is None:
        ...
        obj = cls(
            ...
            [lora.lora_a if lora is not None else None for lora in loras],
            [lora.lora_b if lora is not None else None for lora in loras],
            [lora.magnitude_param if lora is not None else None for lora in loras],
            [lora.bias if lora is not None else None for lora in loras],
            scaling=[1 if lora is not None else None for lora in loras],
        )
        return obj

    def optimize(self) -> "PackedDoRALayerWeights":
        """Optimize the DoRA by merging the scaling into lora_b."""
        for i in range(len(self.lora_b)):
