...
# TODO move this back to entrypoints/openai/protocol.py once lora not found bug is resolved
@dataclass
...
# TODO remove this once lora not found bug is resolved
class LoraNotFoundError(Exception):
    def __init__(self, lora_request: LoRAModulePath, error_code=500):
        message = f"lora local path was not found or incorrectly loaded {lora_request.lora_local_path}"
    ...
    def from_local_checkpoint(
        ...
        *,
        ...
        new_embeddings_bin_file_path = os.path.join(lora_dir,
                                                    "new_embeddings.bin")
        # TODO remove try except clause once scale up bug is resolved
        try:
            with open(lora_config_path) as f:
                config = json.load(f)
        except Exception as e:
            raise LoraNotFoundError(LoRAModulePath(os.path.dirname(lora_dir), lora_dir))
