...
class BaseLinearLayerWithLoRA(BaseLayerWithLoRA):
    ...
    def set_lora(
        ...
        if lora_magnitudes is not None:
            ...
            self.lora_magnitudes_stacked[0][index, 0, : lora_magnitudes.shape[0]].copy_(
                lora_magnitudes.T, non_blocking=True
            )
...
class ColumnParallelLinearWithLoRA(BaseLinearLayerWithLoRA):
    ...
    def slice_lora_b(self, lora_b: torch.Tensor) -> torch.Tensor:
        ...
        if self.is_merged_col_linear:
            ...
            right_weight = lora_b[
                :, offset + tp_rank * shard_size : offset + (tp_rank + 1) * shard_size
            ]
...
class LinearScalingRotaryEmbeddingWithLora(BaseLayerWithLoRA):
    ...
    def create_lora_weights(
        ...
        scaling_factors = (
            list(lora_config.long_lora_scaling_factors)
            if lora_config.long_lora_scaling_factors
            else []
        )
        base_scaling_factor = (
            self.base_layer.scaling_factor
            if isinstance(self.base_layer, LinearScalingRotaryEmbedding)
            else 1.0
        )
