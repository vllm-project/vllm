...
def _not_fully_sharded_or_block_diagonal_sharded_can_replace(can_replace):
    """
    decorator which adds the condition of not using fully sharded loras
    intended to wrap can_replace_layer()
    """
    ...
    def dec(*args, **kwargs):
        decorate = kwargs.pop("decorate") if "decorate" in kwargs else True
        condition = (not (kwargs["lora_config"].fully_sharded_loras or kwargs["lora_config"].block_diagonal_sharded_loras)
                     if decorate else True)
        return can_replace(*args, **kwargs) and condition
...
class ReplicatedLinearWithLoRA(BaseLinearLayerWithLoRA):
    ...
    @classmethod
    @_not_fully_sharded_or_block_diagonal_sharded_can_replace
    def can_replace_layer(
        cls,
        source_layer: nn.Module,
        lora_config: LoRAConfig,
...
class MergedColumnParallelLinearWithLoRA(ColumnParallelLinearWithLoRA):
    ...
    def create_lora_weights(
        ...
        #
        self.lora_a_out_size = [lora_a_output_size_per_partition for _ in range(self.n_slices)]
        self.lora_a_in_size = [self.input_size for _ in range(self.n_slices)]
        self.lora_b_out_size = [output_size for output_size in self.output_slices]
        self.lora_b_in_size = [lora_config.max_lora_rank for _ in self.output_slices]
