...
def attn_fwd(Q, K, V, bias, sm_scale, L, Out, stride_qz, stride_qh, stride_qm,
    ...
    if IS_CAUSAL:
        ...
        # This captures the decrease in n_blocks if we have a rectangular
        # attn matrix
        ...
        if n_blocks <= 0:
            ...
            # We store inf to LSE, not -inf because in the bwd pass, we subtract
            # this from qk which makes it -inf, such that exp(qk - inf) = 0 for
            # these masked blocks.
            l = tl.full(  # NOQA: E741
                [BLOCK_M], value=float("inf"), dtype=tl.float32)
            l_ptrs_mask = offs_m < MAX_SEQLENS_Q
            tl.store(l_ptrs, l, mask=l_ptrs_mask)
            # TODO: Should dropout & return encoded softmax be handled here too?
            return
    ...
    # We can ask to return the dropout mask without actually doing dropout. In
    ...
    # Compute for full blocks. Here we set causal to false unconditionally
    # because there is no masking. Similarly we do not need padding.
    if n_full_blocks > 0:
    ...
    # rows. This is only true for the last M block. For others, overflow_size
    # will be -ve
