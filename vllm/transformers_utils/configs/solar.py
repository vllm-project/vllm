...
class SolarConfig(PretrainedConfig):
    ...
    def _rope_scaling_validation(self):
        ...
        if (not isinstance(self.rope_scaling, dict) or len(self.rope_scaling) != 2):
            raise ValueError("`rope_scaling` must be a dictionary with two fields,"
                             " `type` and `factor`, "
        ...
        if (rope_scaling_factor is None or not isinstance(rope_scaling_factor, float)
                or rope_scaling_factor <= 1.0):
            raise ValueError(f"`rope_scaling`'s factor field must be a float > 1,"
