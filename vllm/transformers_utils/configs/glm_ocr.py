        ...
        **kwargs: Any,
        ...
        if isinstance(text_config, dict):
            ...
            if model_type == "glm_ocr_text":
                model_type = "glm4"
            # Extract partial_rotary_factor from rope_parameters so that
            # Glm4Config receives it as a top-level kwarg instead of using
            # its default (0.5). This prevents standardize_rope_params()
            # from overwriting the correct value in rope_parameters.
            rope_params = text_config.get("rope_parameters")
            if (
                isinstance(rope_params, dict)
                and "partial_rotary_factor" in rope_params
                and "partial_rotary_factor" not in text_config
            ):
                text_config["partial_rotary_factor"] = rope_params[
