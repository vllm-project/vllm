...
def resolve_text_config(text_config):
    """Unwrap a (possibly nested) text config into a ``Qwen2VLTextConfig``.

    Transformers v5 split ``Qwen2VLConfig`` into an outer composite config and
    an inner ``Qwen2VLTextConfig``. Combined with Tarsier2's nested
    ``config.json``, ``get_text_config`` can return an object that still lacks
    the text attributes vLLM expects (e.g. ``num_attention_heads``). This
    recursively descends ``text_config`` until it finds the real text config.
    """
