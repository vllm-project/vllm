        ...
        if backbone_config is not None:
            ...
            if not isinstance(backbone_config, PretrainedConfig):
                ...
                backbone_config = AutoConfig.for_model(model_type,
                                                       **backbone_config)
        self.vision_config = backbone_config
    ...
    def __init__(self,
        ...
        if visual_tokenizer_config is not None:
            ...
            if not isinstance(visual_tokenizer_config, PretrainedConfig):
                visual_tokenizer_config = SiglipVisualTokenizerConfig(
    ...
    def format(self, conversations: List[Dict], generation_preface=None):
        if self.gpt_token_num is None:
            self.gpt_token_num = len(
                self.tokenizer(self.from2role["gpt"],
                               add_special_tokens=False).input_ids)

        if self.bos_token_ids is None:
            self.bos_token_ids = self.tokenizer(
                self.bos_token, add_special_tokens=False).input_ids

        if conversations[0]["from"] == "system":
            raise ValueError("Gemma does not support system prompt")

        if generation_preface is not None:
            conversations.append({"from": "gpt", "value": generation_preface})
        ...
        for i, conversation in enumerate(conversations):
            ...
            if frm == "gpt":
                ...
                label_ids[self.gpt_token_num:-1] = token_ids[self.
                                                             gpt_token_num:-1]
