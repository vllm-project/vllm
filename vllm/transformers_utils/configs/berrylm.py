...
class BerryLMConfig(PreTrainedConfig):
    ...
    attn_output_gate (`bool`, *optional*, defaults to `True`):
        Whether the full-attention layers gate their output with a sigmoid branch of the
        query projection.
    ...
    linear_num_value_heads (`int`, *optional*, defaults to 32):
        Number of value heads used in linear attention layers. Must be a multiple of
        `linear_num_key_heads`.
    attn_res_block_size (`int`, *optional*, defaults to 8):
        Block size of the Gated Block AttnRes mixer: every decoder layer reads a softmax
        mixture of the residual
        streams committed every `attn_res_block_size` layers (the embeddings count as
        block 0) and the current
        stream, gated back toward the identity by a per-layer scalar. `0` disables the
        mixer.
    attn_res_gated (`bool`, *optional*, defaults to `True`):
        Whether the mixer output is gated toward the identity (`y = x + tanh(g) * (mix -
        x)`, `g = 0` is the exact
    ...
    kda_gate_bottleneck (`int`, *optional*, defaults to 128):
        ...
        (`g = -exp(A_log) * softplus(f_up(f_down(x)) + dt_bias)`, shape
    ...
    ```
    """  # noqa: D212 -- transformers.auto_docstring parses the parameter list

    model_type = "berrylm"
    keys_to_ignore_at_inference = ["past_key_values"]

    # The small KDA gate projections (f_down_proj / f_up_proj) and the AttnRes
    ...
    def __post_init__(self, **kwargs):
        ...
        # `full_attention_interval` only seeds `layer_types` (one full-attention layer
        # per `interval` layers).
        ...
        if bad:
            raise ValueError(
                "`layer_types` must contain only 'linear_attention'/'full_attention', "
                f"got {bad}"
            )
        ...
        if self.kda_safe_gate and self.kda_gate_lower_bound is None:
            raise ValueError(
                "`kda_gate_lower_bound` is required when `kda_safe_gate=True`"
            )
