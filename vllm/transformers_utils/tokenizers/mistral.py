...
class MistralTokenizer:
    def __init__(self, tokenizer: PublicMistralTokenizer) -> None:
        ...
        assert isinstance(
            self.tokenizer, (Tekkenizer, SentencePieceTokenizer)
        ), type(self.tokenizer)
        self._is_tekken = isinstance(self.tokenizer, Tekkenizer)

    @classmethod
    def from_pretrained(
        cls, repo_id: str, *, revision: Optional[str] = None
    ) -> "MistralTokenizer":
    ...
    def _download_mistral_tokenizer_from_hf(
        ...
        tokenizer_file = hf_hub_download(
            tokenizer_name, filename=matched_files[0], revision=revision
        )
    ...
    def encode_messages(
        ...
        request = ChatCompletionRequest(messages=messages)  # type: ignore[type-var]
        encoded = self.mistral.encode_chat_completion(request)
        return encoded.tokens

    def encode_chat_completion(
        self, oai_request: "OAIChatCompletionRequest"
    ) -> Dict[str, Any]:
    ...
    def convert_ids_to_tokens(
        self, ids: List[int], skip_special_tokens: Optional[bool] = True
    ) -> List[str]:
