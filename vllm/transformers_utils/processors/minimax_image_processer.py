...
from transformers.utils import (is_torch_device, is_torch_dtype,
                                requires_backends)
...
def select_best_resolution(original_size, possible_resolutions):
    """
    Selects the best resolution from a list of possible 
    resolutions based on the original size.

    Args:
        original_size (tuple): The original size of the 
        image in the format (width, height).
        possible_resolutions (list): A list of possible 
...
def image_size_to_num_patches(image_size, grid_pinpoints, patch_size):
    if not isinstance(grid_pinpoints, list):
        raise TypeError("grid_pinpoints should " +
                        "be a list of tuples or lists")

    # ! VERY IMPORTANT if image_size is tensor,
    # must convert to into tuple, otherwise it will cause wrong calculate
...
def get_anyres_image_grid_shape(image_size, grid_pinpoints, patch_size):
    """
    Calculate the shape of the image patch grid 
    after the preprocessing for images of any resolution.

    Args:
        image_size (`tuple`):
            The size of the input image in the format (width, height).
        grid_pinpoints (`List`):
            A list containing possible resolutions. 
            Each item in the list should be a tuple or list
            of the form `(height, width)`.
        patch_size (`int`):
            The size of each image patch.

    Returns:
        tuple: The shape of the image patch 
        grid in the format (width, height).
    """
    if not isinstance(grid_pinpoints, list):
    ...
    if not isinstance(image_size, (list, tuple)):
        if not isinstance(image_size, (torch.Tensor, np.ndarray)):
            raise TypeError(
                "image_size invalid type " + f"{type(image_size)} not valid," +
...
def resize_multiple_of(image, multiple, max_size=None):
    ...
    """
    width, height = image.size
    new_width, new_height = get_hw_multiple_of((width, height), multiple,
                                               max_size)
...
class CustomBatchFeature(BatchFeature):
    ...
    def convert_to_tensors(self,
        ...
        Args:
            tensor_type (`str` or [`~utils.TensorType`], *optional*):
                The type of tensors to use. If `str`, should be one 
                of the values of the enum [`~utils.TensorType`]. If
    ...
    def to(self, *args, **kwargs) -> "BatchFeature":
        ...
        different `dtypes` and sending the 
        `BatchFeature` to a different `device`.
        ...
        if device is None and len(args) > 0:
            ...
            if is_torch_dtype(arg):
                ...
                pass
            elif isinstance(arg, str) or is_torch_device(arg) or isinstance(
                    arg, int):
                device = arg
            else:
                # it's something else
                raise ValueError("Attempting to cast a BatchFeature to type " +
                                 f"{str(arg)}. This is not supported.")
        # We cast only floating point tensors to avoid issues
        # with tokenizers casting `LongTensor` to `FloatTensor`
...
def as_tensor(value):
    if isinstance(value, (list, tuple)) and len(value) > 0 and isinstance(
            value[0], np.ndarray) or (isinstance(value[0], (list, tuple))
                                      and len(value[0]) > 0
...
class ImageProcessor(BaseImageProcessor):
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        if self.process_image_mode == 'resize':
            return self.resize_preprocess(images, return_tensors, data_format,
                                          input_data_format, **kwargs)
        elif self.process_image_mode == 'anyres':
            if processor_for_vllm == 1:
                return self.anyres_for_vllm_preprocess(images, return_tensors,
                                                       data_format,
                                                       input_data_format,
                                                       **kwargs)
            return self.anyres_preprocess(images, return_tensors, data_format,
                                          input_data_format, **kwargs)
        elif self.process_image_mode == 'keepratio_resize':
            return self.keepratio_resize_preprocess(images, return_tensors,
                                                    data_format,
                                                    input_data_format,
                                                    **kwargs)
        elif self.process_image_mode == 'dynamic_res':
            return self.dynamic_res_preprocess(images, return_tensors,
                                               data_format, input_data_format,
                                               **kwargs)
        else:
            raise ValueError(
                f"Invalid process_image_mode: {self.process_image_mode}")
    ...
    def keepratio_resize_preprocess(
            self,
            images,
            return_tensors: Optional[Union[str, TensorType]] = None,
            data_format: Optional[ChannelDimension] = ChannelDimension.FIRST,
    ...
    def dynamic_res_preprocess(
        ...
        for image in images:
            ...
            resized_image = resize_multiple_of(image,
                                               self.patch_size,
                                               max_size=self.size)
            resized_w, resized_h = resized_image.size
            transform_img = _transform(resized_h, resized_w, self.image_mean,
                                       self.image_std)(resized_image)
            all_images.append(to_numpy_array(transform_img))
    ...
    def anyres_for_vllm_preprocess(
            self,
            images,
            return_tensors: Optional[Union[str, TensorType]] = None,
            data_format: Optional[ChannelDimension] = ChannelDimension.FIRST,
        ...
        for image in images:
            ...
            for image in image_patches:
                transform_img = _transform(self.size[0], self.size[1],
                                           self.image_mean,
                                           self.image_std)(image)
