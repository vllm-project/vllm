...
class KimiAudioProcessor:
    ...
    def _tokenize_audio(self, audio_path: str) -> list[int]:
        ...
        wav_tokens = torch.full(
            (count,), self.audio_token_id, dtype=torch.long, device="cpu"
        )
        ...
        **kwargs,
        ...
        if whisper_input_feature and isinstance(whisper_input_feature[0], str):
            ...
            for audio_path in whisper_input_feature:
                # Use load_audio (ffmpeg-based, safe and efficient)
                audio_data = load_audio(audio_path)
                loaded_audios.append(audio_data)  # numpy array (float32)
            whisper_input_feature = loaded_audios
        ...
        for audio_list in audio_input_ids:
            ...
            for idx, tok in enumerate(audio_list):
                ...
                if (
                    ...
                    audio_tokens = (
                        audio_list[:start_idx] + wav_tokens_list + audio_list[end_idx:]
                    )
                    break
