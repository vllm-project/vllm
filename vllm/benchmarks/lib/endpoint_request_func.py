        ...
        async with session.post(url=api_url, json=payload, headers=headers) as response:
            if response.status == 200:
                ...
                async for chunk_bytes in response.content.iter_any():
                    for message in handler.add_chunk(chunk_bytes):
                        ...
                        if event_type in _RESPONSES_TOKEN_DELTA_EVENTS:
                            ...
                            # Only token events advance the end of the
                            # request, so `latency - ttft` stays equal to
                            # `sum(itl)` and TPOT means what serve.py says.
                            most_recent_timestamp = timestamp
                ...
                if stream_error is not None:
                    ...
                    # Without the terminal event the response is truncated and
                    # its usage block is missing.
                    output.success = False
                    output.error = (
