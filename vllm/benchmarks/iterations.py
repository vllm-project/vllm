...
async def run_compilation_warmup(
    ...
    # Send 1 request with matching token count for each unique total.
    ...
    for ctx_len in config.context_lens:
        for in_len in input_lens:
            for bs in config.batch_sizes:
                per_req = ctx_len + in_len + 2  # +2 for BOS/tokenization overhead
                num_reqs = bs * dp_size
        ...
        for ctx_len, in_len, batch_size_per_dp in param_combos:
            ...
            if config.mode == "decode" and config.profile and trace_prefix:
                ...
                (
                    ...
                    trace_prefix=None, use_sleep_wake=False,
                )
            ...
            logger.info(
                ...
                "%.2fms total (client), %.2fms/iter (client), %.2f tok/s",
                server_total_ms,
                latency_per_iter,
                decode_elapsed_ms,
                client_latency_per_iter,
...
def print_results_summary(
    ...
    print(
        ...
        f"{'Clnt Total':>12} {'Clnt/Iter':>12} {'Tokens/s':>12}"
    )
    print("-" * 130)
    ...
    for r in results:
        print(
            ...
            f"{r.iteration:>6} {r.total_latency_ms:>12.2f} {r.latency_per_iter_ms:>12.2f} "
