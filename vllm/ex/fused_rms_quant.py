...
def add_residual_rms_norm_quant(input: torch.Tensor,
    ...
    torch.ops._C.add_residual_rms_norm_quant(out, 
                                             input,
                                             residual,
                                             tmp,
                                             weight,
...
def setup_fused_rms_norm(cc: CodeCache):
    ...
    torch.library.impl(f"{ns_op}", "Meta", func=add_residual_rms_norm_quant_meta)
    cc.add(add_residual_rms_norm_quant_name, torch.ops.dogfood.add_residual_rms_norm_quant)