...
def flash_attn_varlen_func(
    ...
    if fa_version == 2:
        ...
        # The SM90 FA4 fp8-KV path consumes (bf16 or fp16) Q + fp8 e4m3 paged K/V,
        # dequantizes K/V to fp16, computes attention in fp16, and writes O back in Q's
        # dtype. The bf16<->fp16 Q/O casts happen inside the kernel (Q is narrowed
        # bf16->fp16 in-place, the fp32 accumulator is cast to O's dtype in the epilogue),
        # so we hand Q and the preallocated `out` straight through -- no DRAM round-trips.
        # We pass q_descale=None (the cute interface materializes an identity descale) and
        # forward the (batch, num_kv_heads) float32 k/v descales.
        fa4_fp8_kv_dequant = (
