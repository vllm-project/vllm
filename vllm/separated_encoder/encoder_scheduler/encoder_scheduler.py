...
class EncoderScheduler(SchedulerInterface):
    ...
    def __init__(
        ...
        assert self.epd_disagg_config.instance_type == "encode", \
            "Encoder scheduler usage is not allowed on non-encoder isntances"
        ...
        self.encoder_cache_connector = ECConnector(self.connector_workers_num)
        self.finished_injects = queue.Queue()
        for _ in range(self.connector_workers_num // 2):
            self.encoder_cache_connector.create_recv_inject_notif_req(
                self.receive_inject_notif)
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        while self.waiting and encoder_budget > 0:
            ...
            assert request.has_encoder_inputs,\
                "Requests on encoder instance must have mm data."

            if request.has_encoder_inputs:
                mm_positions = request.mm_positions

                assert (mm_positions is not None) and (len(mm_positions) > 0)
                ...
                if (not
                    self.encoder_cache_manager.can_allocate_disag(total_size) \
                    or required_budget > new_encoder_budget):
                    # don't have a budget or space for encode
                    break
                ...
                for i, pos_info in enumerate(mm_positions):
                    ...
                    if (not self.encoder_cache_manager.can_allocate(
                            request, i)
                            or num_encoder_tokens > new_encoder_budget):
                        break
                    new_encoder_budget -= num_encoder_tokens
                    encoder_inputs_to_schedule.append(i)
                assert (new_encoder_budget + required_budget == encoder_budget)
                # We can handle multiple mm inputs more efficiently, but
                # for simplicity we left this implementation
            ...
            if encoder_inputs_to_schedule:
                ...
                for input_id in encoder_inputs_to_schedule:
                    ...
                    self.encoder_cache_connector.\
                        create_send_encoder_cache_metas_req(
                        request.request_id,
                        input_id,
                        request.get_num_encoder_tokens(input_id)
                    )
                encoder_budget = new_encoder_budget

        # Check if the scheduling constraints are satisfied.
        assert len(self.running) <= self.max_num_running_reqs
        assert (len(scheduled_new_reqs) <= len(self.running))

        new_reqs_data = [req for req in scheduled_new_reqs]

        for _ in range(self.finished_injects.qsize()):
            req_id, input_id = self.finished_injects.get()
            self.encoder_cache_manager.free_encoder_input(
                self.requests.get(req_id), input_id)
            if not self.encoder_cache_manager.has_cached_inputs_disag(req_id):
    ...
    def _free_blocks(self, request: Request):
        assert request.is_finished()
        # We're deleting request in update_from_output
    ...
    def receive_inject_notif(self, request_id, input_id) -> None:
        self.finished_injects.put((request_id, input_id))
        self.encoder_cache_connector.create_recv_inject_notif_req(
            self.receive_inject_notif)
