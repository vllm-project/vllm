...
class DelegatingParser(Parser):
    ...
    def _apply_structural_tag(
        ...
        output_format = None
        if resolved_tools is not None and is_auto and structured_outputs is not None:
            output_format = structured_outputs_to_format(structured_outputs)

        if resolved_tools is not None:
            tag_request_update: dict[str, object] = {"tools": list(resolved_tools)}
            if output_format is not None:
                # The auto tag permits unconstrained final text. Use a required
                # tag here so the structured-output branch remains meaningful.
                tag_request_update["tool_choice"] = "required"
            structural_tag_request = request.model_copy(update=tag_request_update)
            tools_structural_tag = tool_parser.get_structural_tag(
                structural_tag_request,
                reasoning=False,
            )
        else:
        ...
        if output_format is not None:
            ...
            logger.warning_once(
                "structured outputs are ignored because tool_choice=%s forces "
                "tool call.",
                request.tool_choice,
                scope="local",
            )
