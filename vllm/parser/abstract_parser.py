        ...
        if (
            ...
            and (
                ...
                or getattr(
                    ...
                    "keeps_reasoning_markup_in_content",
                    False,
                )
            )
            and hasattr(self._tool_parser, "skip_reasoning_parsing")
        ):
            # With no reasoning parser configured, or one that keeps reasoning
            # markup inside content, the markup is plain content: an
            # engine-based tool parser should pass it through verbatim where
            # its grammar allows, not consume or reclassify it. The engine
            # ignores the flag for markers shared with non-reasoning structure.
