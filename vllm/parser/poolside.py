...
"""poolside (Laguna) parser for reasoning and tool calls.

Laguna uses the same XML-like surface format as GLM-4.7::

    <tool_call>func_name<arg_key>key</arg_key><arg_value>value</arg_value></tool_call>

with the function name followed directly by the first ``<arg_key>`` (no newline
separator), optional empty-argument calls, and ``<think>...</think>`` reasoning.

This module intentionally mirrors ``vllm/parser/glm47_moe.py`` but defines its
own ``poolside_v1_config`` / ``PoolsideParser`` so Laguna owns a self-contained
config rather than inheriting GLM's. If Laguna's format later diverges from
GLM-4.7, change it here without touching GLM. (Trade-off: GLM-4.7 engine-config
fixes do not auto-propagate; keep the two in sync if the shared format changes.)
"""
...
_PARTIAL_ARG_RE = re.compile(
    r"<arg_key>(?P<key>.*?)</arg_key>\s*<arg_value>(?P<value>.*)$",
    re.DOTALL,
)


def _poolside_arg_converter(raw_args: str, partial: bool) -> str:
...
@functools.cache
def poolside_v1_config(thinking: bool = True) -> ParserEngineConfig:
    arg_tag_transitions = {
    ...
    reasoning_terminals = (
        {"THINK_START": THINK_START, "THINK_END": THINK_END} if thinking else {}
    )
    ...
    return ParserEngineConfig(
        ...
        arg_converter=_poolside_arg_converter,
...
class PoolsideParser(ParserEngine):
    """poolside (Laguna) parser backed by the declarative parser engine."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs.setdefault(
            ...
            poolside_v1_config(thinking=self.thinking_enabled),
        )
