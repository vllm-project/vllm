...
def convert_mapping(
    ...
    pa_indices = [
        id_to_index.get(id_, -1) if id_ > 0 else -1 for id_ in mapping.index_mapping
    ]
...
class PromptAdapterModel(AdapterModel):
    ...
    def __init__(
        self, prompt_adapter_id=None, num_virtual_tokens=None, prompt_embedding=None
    ) -> None:
    ...
    def _create_prompt_adapter_modules(self):
        for module_name, module in self.model.named_modules(remove_duplicate=False):
            if "VocabParallel" in module.__class__.__name__:
                ...
                replaced_module.set_mapping(
                    self.base_indices, self.base_embedding_indices
                )
                break
...
class PromptAdapterLRUCache(AdapterLRUCache[PromptAdapterModel]):
    ...
    def __init__(
        self, capacity: int, deactivate_prompt_adapter_fn: Callable[[int], bool]
    ):
...
class LRUCachePromptAdapterModelManager(PromptAdapterModelManager):
    ...
    def __init__(
        ...
        self.prompt_adapter_config = prompt_adapter_config
        super().__init__(
            model, max_num_seqs, max_num_batched_tokens, prompt_adapter_config
        )
    ...
    def activate_adapter(
        ...
        if (
            prompt_adapter_id not in self._active_adapters
