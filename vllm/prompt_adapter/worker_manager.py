...
class WorkerPromptAdapterManager(AbstractWorkerManager):
    ...
    def _load_adapter(
            self, prompt_adapter_request: PromptAdapterRequest) -> PromptAdapterModel:
        try:
            ...
            raise RuntimeError(f"Loading prompt_adapter "
...
class LRUCacheWorkerPromptAdapterManager(WorkerPromptAdapterManager):
    ...
    def _apply_adapters(self,
        ...
        if len(prompt_adapters_map) > self._adapter_manager.prompt_adapter_slots:
            raise RuntimeError(f"Number of requested prompt_adapters "
