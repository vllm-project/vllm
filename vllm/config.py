...
class ParallelConfig:
    ...
    def __post_init__(self) -> None:
        ...
        # For certain platforms, only Ray is supported.
        self._configure_ray_only_devices()
    ...
    def _configure_ray_only_devices(self):
        from vllm.platforms import current_platform

        # TODO: Add device types that *must* use Ray for distributed execution
        ray_only_devices: list[str] = []
        if current_platform.device_type in \
                ray_only_devices and self.world_size > 1:
            if self.distributed_executor_backend \
                    not in (None, "ray"):
                raise ValueError(
                    f"Distributed inference with "
                    f"{current_platform.device_type.upper()} "
                    "requires the 'ray' distributed executor backend.")
            if self.distributed_executor_backend is None:
