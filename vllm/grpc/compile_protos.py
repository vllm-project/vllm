...
e.g., after modifying vllm_engine.proto or vllm_render.proto.
...
def _get_well_known_protos_path() -> Path:
    """Get the path to well-known protobuf types (google/protobuf/*.proto)."""
    import grpc_tools
...
def compile_proto(proto_name: str, script_dir: Path, vllm_package_root: Path) -> int:
    ...
    well_known_path = _get_well_known_protos_path()
    result = protoc.main(
        [
            "grpc_tools.protoc",
...
def compile_protos():
    ...
    try:
        for proto_name in ["vllm_engine", "vllm_render"]:
