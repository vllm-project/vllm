...
class cmake_build_ext(build_ext):
    ...
    def build_extensions(self) -> None:
        ...
        def target_name(s: str) -> str:
            return s.removeprefix("vllm_kernels.").removeprefix(
                "vllm_flash_attn.")
...
class repackage_wheel(build_ext):
    ...
    def run(self) -> None:
        ...
        with zipfile.ZipFile(wheel_path) as wheel:
            files_to_copy = [
                "vllm_kernels/_C.abi3.so",
                "vllm_kernels/_moe_C.abi3.so",
                "vllm_kernels/_flashmla_C.abi3.so",
                "vllm_kernels/vllm_flash_attn/_vllm_fa2_C.abi3.so",
                "vllm_kernels/vllm_flash_attn/_vllm_fa3_C.abi3.so",
                "vllm_kernels/cumem_allocator.abi3.so",
            ...
            compiled_regex = re.compile(
                r"vllm_kernels/vllm_flash_attn/(?:[^/.][^/]*/)*(?!\.)[^/]*\.py"
            )
            file_members += list(
...
def get_vllm_version() -> str:
    # version = get_version(write_to="vllm_kernels/_version.py")
    version = "0.1.0"
...
if _is_cuda() or _is_hip():
    ext_modules.append(CMakeExtension(name="vllm_kernels._moe_C"))

if _is_hip():
    ext_modules.append(CMakeExtension(name="vllm_kernels._rocm_C"))

if _is_cuda():
    ext_modules.append(
        CMakeExtension(name="vllm_kernels.vllm_flash_attn._vllm_fa2_C"))
    if envs.VLLM_USE_PRECOMPILED or get_nvcc_cuda_version() >= Version("12.3"):
        # FA3 requires CUDA 12.3 or later
        ext_modules.append(
            CMakeExtension(name="vllm_kernels.vllm_flash_attn._vllm_fa3_C"))
        # Optional since this doesn't get built (produce an .so file) when
        # not targeting a hopper system
        ext_modules.append(
            CMakeExtension(name="vllm_kernels._flashmla_C", optional=True))
    ext_modules.append(CMakeExtension(name="vllm_kernels.cumem_allocator"))

if _build_custom_ops():
    ext_modules.append(CMakeExtension(name="vllm_kernels._C"))
