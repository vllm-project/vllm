...
def compare_tensors(tensor1: torch.Tensor, tensor2: torch.Tensor, rtol: float, atol: float) -> Dict:
    ...
    try:
        ...
        return {
            "match": False,
            "error": f"Error comparing tensors: {e}"
        }
    ...
    for step_name, modules in tensor_files.items():
        ...
        for module_name, file_types in modules.items():
            ...
            if compiled_patterns:
                if not any(pattern.search(module_name) for pattern in compiled_patterns):
                    continue
...
def compare_directories(
            ...
            for json_type in ["inputs_json", "outputs_json"]:
                ...
                if json_files1 and json_files2:
                    ...
                    if not json_comparison.get("match", True):
                        module_report[f"{json_name}_metadata"]["file1"] = str(json_files1[0])
                        module_report[f"{json_name}_metadata"]["file2"] = str(json_files2[0])
    ...
    report["summary"] = {
        ...
        "total_modules": sum(step_report["summary"]["total_modules"] for step_report in report["steps"].values()),
        "matching_modules": sum(step_report["summary"]["matching_modules"] for step_report in report["steps"].values()),
        "mismatched_modules": sum(step_report["summary"]["mismatched_modules"] for step_report in report["steps"].values()),
        "missing_modules": sum(step_report["summary"]["missing_modules"] for step_report in report["steps"].values()),
        "total_tensors": sum(
...
def generate_markdown_report(report: Dict, verbose: bool = False) -> str:
    ...
    lines.append("")
    lines.append(f"Comparing intermediate logging outputs between:")
    ...
    lines.append(f"| Modules | {summary['total_modules']} | {summary['matching_modules']} | {summary['mismatched_modules']} | {summary['missing_modules']} |")
    lines.append(f"| Tensors | {summary['total_tensors']} | {summary['matching_tensors']} | {summary['mismatched_tensors']} | {summary['missing_tensors']} |")
    lines.append("")
    ...
    for step_name, step_report in sorted(report["steps"].items()):
        ...
        lines.append("")
        lines.append(f"**Summary**: {step_summary['matching_modules']} matching modules, {step_summary['mismatched_modules']} mismatched modules, {step_summary['missing_modules']} missing modules")
        lines.append("")
        
        # Add module details
        for module_name, module_report in sorted(step_report["modules"].items()):
            ...
            lines.append("")
            lines.append(f"**Summary**: {module_summary['matching_tensors']} matching tensors, {module_summary['mismatched_tensors']} mismatched tensors, {module_summary['missing_tensors']} missing tensors")
            lines.append("")
            ...
            for metadata_type in ["inputs_metadata", "outputs_metadata"]:
                if metadata_type in module_report:
                    ...
                    if not metadata_comparison.get("match", True):
                        ...
                        lines.append(f"**{metadata_type.capitalize()}**: Mismatch detected{file_paths}")
                        if verbose and "mismatches" in metadata_comparison:
                            lines.append("```json")
                            lines.append(json.dumps(metadata_comparison["mismatches"], indent=2))
                            lines.append("```")
                        lines.append("")
            ...
            if module_summary["mismatched_tensors"] > 0 or verbose:
                ...
                if module_report["inputs"]:
                    ...
                    for tensor_name, comparison in sorted(module_report["inputs"].items()):
                        if comparison.get("match", False):
                            status = "✅"
                ...
                if module_report["outputs"]:
                    ...
                    for tensor_name, comparison in sorted(module_report["outputs"].items()):
                        if comparison.get("match", False):
                            status = "✅"
                            details = "Tensors match"
                        elif "error" in comparison:
...
def main():
    parser = argparse.ArgumentParser(description="Compare intermediate logging outputs from two different runs.")
    parser.add_argument("--dir1", required=True, help="First intermediate logging directory")
    parser.add_argument("--dir2", required=True, help="Second intermediate logging directory")
    parser.add_argument("--output", help="Output file for the report (default: stdout)")
    parser.add_argument("--rtol", type=float, default=1e-5, help="Relative tolerance for tensor comparison (default: 1e-5)")
    parser.add_argument("--atol", type=float, default=1e-8, help="Absolute tolerance for tensor comparison (default: 1e-8)")
    parser.add_argument("--steps", help="Comma-separated list of steps to compare (default: all)")
    parser.add_argument("--modules", help="Comma-separated list of module name patterns to compare (default: all)")
    parser.add_argument("--verbose", action="store_true", help="Include detailed information about each tensor")
