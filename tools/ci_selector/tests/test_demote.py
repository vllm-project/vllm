...
class TestSubtractiveInvariants:
    ...
    def test_no_leaf_origin_lazy_edge_was_dropped(self, state):
        """A lazy import from tests/ or benchmarks/ into a claimed target must
        never be finalized away -- the flashinfer silent-unlinking class."""
        from helpers import leaf_origin_drops

        assert leaf_origin_drops(state) == []

    def test_the_demotion_detector_has_not_collapsed(self, state):
        """Zero demotions makes every check below pass vacuously."""
        assert state.full.dispatch.demotions, "dispatch produced zero demotions"
