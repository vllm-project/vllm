...
"""Work out what the docs build depends on, from the docs sources.

The docs build runs on Read the Docs and not Buildkite, so it declares no
source_file_dependencies. We derive them instead: a wide floor (api-autonav
re-renders the whole non-excluded vllm/ tree, plus docs/, examples/ and the
build config) extended with the out-of-tree files docs pages reference by
snippet or link. The reference extractor is shared with the docs-refs audit.
"""
...
from ..handwritten import DOCS_FLOOR_PREFIXES, DOCS_INFRA_FILES
...
class _MkdocsLoader(yaml.SafeLoader):
    """SafeLoader that turns mkdocs' custom tags into None instead of raising.

    Still safe: the base never builds arbitrary objects, and both constructors
    return None without touching the node, so a python-object tag resolves to
    None and never runs. We only read plain string lists, so dropping the
    tagged values loses nothing.
    """
...
def load_mkdocs(repo: Path) -> dict | None:
    """Parsed mkdocs.yaml, or None on any failure, which sends the caller to
    the wide floor."""
    try:
        data = yaml.load((repo / MKDOCS_CONFIG).read_text(), Loader=_MkdocsLoader)
...
# [identifier][] collapsed form: the identifier is in the first bracket.
...
def extract_refs(repo: Path) -> list[Ref]:
    """Every autoref, snippet and pylink reference in the docs pages. Autoref
    and pylink matches inside a code fence are skipped as inert; snippets are
    expanded even inside a fence, so those are always collected."""
    refs: list[Ref] = []
    for md in sorted((repo / DOCS_DIR).rglob(DOCS_PAGE_GLOB)):
...
def resolve_autoref(target: str, index) -> tuple[str | None, str | None]:
    """Longest importable prefix of a dotted target -> (module file, first
    trailing symbol). The symbol is None when the target is a module itself.
    `vllm` always resolves, so the file is None only for a non-vllm target."""
    parts = target.split(".")
...
def is_literal_snippet(target: str) -> bool:
    """A snippet target naming a real file, not a marker or a placeholder."""
...
def _module_stmts(body: list[ast.stmt]) -> Iterator[ast.stmt]:
    """Module-level statements, descending into a module-level if or try so
    guarded imports count, but never into a function or class body."""
    for node in body:
        yield node
...
def symbol_status(module_file: str, symbol: str, repo: Path) -> str:
    """PRESENT / BROKEN / UNCERTAIN for a top-level symbol in a module.

    BROKEN only when we are sure: the module parses, has no `__getattr__` or
    star import, and the name appears nowhere at module level. Anything
    ambiguous stays UNCERTAIN, which keeps the audit free of false alarms.
    """
    try:
...
def _parse_excludes(
    ...
    """api-autonav excludes as (path prefixes, module-name regexes). Empty when
    the parse failed, which leaves the vllm floor covering everything."""
    if data is None:
        return (), ()
    prefixes: list[str] = []
...
def build_docs_deps(repo: Path) -> DocsDeps:
    ...
    deps = DocsDeps(
        infra_files=DOCS_INFRA_FILES,
        floor_prefixes=DOCS_FLOOR_PREFIXES,
...
def _add_pylink_dep(deps: DocsDeps, ref: Ref, repo: Path) -> None:
    """A relative link to a file outside the floor makes that file a dep:
    renaming or removing it breaks the build. A link to a DIRECTORY breaks only
    if the directory itself moves, and flooring its whole subtree would cover
    far too much, so those are left unmodeled."""
    try:
