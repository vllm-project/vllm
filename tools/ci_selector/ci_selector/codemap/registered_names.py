...
SUBSTRING_KEY_MIN_LEN = 12  # shorter risks matching inside unrelated words
RAW_KEY_MIN_LEN = 8  # table-diff archs are 8-11 chars; higher drops them
...
class KeyIndex:
    ...
    # The subtractive-invariant tests read this to tell "refused on purpose"
    # from "should have been routable and is not", which is the shape of a
    # real routing gap.
    ...
    def steps_naming_raw(self, keys: set[str]) -> set[str]:
        ...
        The bar is deliberately LOWER than SUBSTRING_KEY_MIN_LEN: raw keys
        ...
        archs."""
        candidates = {k for k in keys if "/" in k or len(k) >= RAW_KEY_MIN_LEN}
        return {
            sid
