...
- `runtime_tuning.py` owns hardware-policy selection and independent parameter
  tuning functions.
- `sweep_generation.py` generates optional benchmark candidates around the
  initial scheduler suggestion.
...
Runtime tuning always produces **one initial suggested `config.yml`**. Users can
deploy that configuration directly without running a benchmark.

When workload information is available, `--generate-sweep` can additionally
create a small vLLM-native benchmark package:
...
This still writes the single initial deployment suggestion:
...
The optional sweep only varies `max-num-seqs` and
`max-num-batched-tokens`. TP and memory utilization remain fixed at the initial
hardware-derived suggestion, and DP is not swept.

`serve_params.json` starts with the exact initial scheduler suggestion and then
adds a small directed set of nearby benchmark candidates:

- lower scheduler concurrency,
- lower token budget,
- lower concurrency plus lower token budget, and
- higher token budget.

The generated fixed workload uses the supplied input/output lengths and
concurrency. A server `max-num-seqs` larger than that concurrency is not added
because it would not exercise additional active requests in this fixed-workload
sweep.
...
For a faster coarse pass, use one run per configuration:

```bash
sweep/run_sweep.sh --num-runs 1
```

The sweep uses `vllm bench sweep serve`, so command-line sweep values override
the corresponding values from the generated `config.yml`. Users can review the
measured latency/throughput tradeoff and replace the initial scheduler values
only when the sweep demonstrates a better configuration for the target workload.
