...
find . -name .git -prune -o -type f -name "*.sh" -not -path "./.buildkite/scripts/hardware_ci/run-amd-test.sh" -print0 \
  | xargs -0 -I {} sh -c 'git check-ignore -q "{}" || shellcheck -S error -s bash "{}"'
