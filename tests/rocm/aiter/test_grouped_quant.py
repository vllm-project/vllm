...
# This is a test for the vllm_aiter.dynamic_group_quant_fp8 op.
...
# ensures the ops are registered
...
def test_dynamic_group_quant_fp8_fake_implementation():
    """Test that the fake implementation is correctly
    defined for torch.ops.vllm_aiter.dynamic_group_quant_fp8."""
    # Create test tensors
    M = 128
    N = 4096
    group_size = 128
    ...
    torch.library.opcheck(
        torch.ops.vllm_aiter.dynamic_group_quant_fp8,
        (input_tensor, group_size),
        test_utils=("test_faketensor",),
    )


def test_dynamic_group_quant_fp8_torch_compile_with_cudagraph():
    """Test that dynamic_group_quant_fp8 with group size 128 can be used
    with torch.compile in cudagraph mode."""
    M = 128
    N = 4096
    group_size = 128
    ...
    def group_fp8_quant_fn(x):
        return torch.ops.vllm_aiter.dynamic_group_quant_fp8(x, group_size)
...
def test_dynamic_group_quant_fp8_different_shapes():
    """Test dynamic_group_quant_fp8 with different input shapes."""
    group_size = 128
    ...
    for M, N in test_shapes:
        ...
        x_fp8, scales = torch.ops.vllm_aiter.dynamic_group_quant_fp8(
            input_tensor, group_size
        )
