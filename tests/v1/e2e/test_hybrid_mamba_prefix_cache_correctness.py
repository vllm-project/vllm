...
because its sub-2-block trigger geometry is expected to be legitimately
uncacheable once a fix lands (eagle boundary reservation).
...
XFAIL_REASON = (
    ...
    "it does not correspond to (pending #45477/#47861/#45614/#46281); "
    "remove after one green calibration run on a fixed tree"
)
...
class CorruptionDetected(AssertionError):
    ...
    The xfail markers are restricted to this type via ``raises=``, so a
    failed engagement guard, control-quality gate, geometry assert, or an
    environment/boot fault can never masquerade as the expected corruption
    failure: anything else hard-fails the test despite the marker.
...
@create_new_process_for_each_test()
@pytest.mark.xfail(reason=XFAIL_REASON, raises=CorruptionDetected, strict=False)
...
def test_cold_concurrent_prefill_mamba_prefix_cache(
    ...
    Marked xfail (non-strict, raises=CorruptionDetected): only the
    corruption checks are xfail-eligible; engagement, control-quality,
    and environment failures hard-fail. Not yet calibrated on a tree
    with the open fixes applied.
    """
    ...
    # Engagement is asserted BEFORE the corruption checks (and is not
    # xfail-eligible: only CorruptionDetected is), so a run that never
    # engaged APC+MTP hard-fails instead of xfailing via a corruption
    # check it had no standing to make.
...
def test_multi_turn_decode_written_mamba_prefix_cache(
    ...
    Marked xfail (non-strict, raises=CorruptionDetected): only the
    corruption check is xfail-eligible; engagement, control-quality, and
    environment failures hard-fail. Non-strict because pytest-level red
    has not been calibrated as deterministic per model (see module
    docstring); the marker should be removed after a green calibration
    run on a fixed tree.
    """
    monkeypatch.setenv("VLLM_ENABLE_V1_MULTIPROCESSING", "0")
    kwargs = _engine_kwargs(tp_size)
    ...
    # Engagement is asserted BEFORE the corruption check (and is not
    # xfail-eligible: only CorruptionDetected is), so a run that never
    # engaged APC+MTP hard-fails instead of xfailing via a corruption
