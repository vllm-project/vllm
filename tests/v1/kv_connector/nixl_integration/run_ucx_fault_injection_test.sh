...
# script to run ucx fault injection tests for disaggregated prefill/decode
# injects faults on the decode instances to test recovery behavior
...
echo "Running UCX fault injection tests with kv_buffer_device=$KV_BUFFER_DEVICE"
...
# Build the kv-transfer-config (uses default retry policy of recomputation)
...
# Install ucx-fault-injector from latest release
...
run_tests_for_model() {
  local model_name=$1
  echo "================================"
  echo "Testing model with fault injection: $model_name"
  echo "================================"
  ...
  # Start decode instances WITH fault injection
  for i in $(seq 0 $((NUM_DECODE_INSTANCES-1))); do
    ...
    # Base command with fault injection enabled via LD_PRELOAD
    BASE_CMD="CUDA_VISIBLE_DEVICES=$GPU_ID \
  ...
  TEST_MODEL=$model_name python3 -m pytest -s -x ${GIT_ROOT}/tests/v1/kv_connector/nixl_integration/test_ucx_fault_injection.py
