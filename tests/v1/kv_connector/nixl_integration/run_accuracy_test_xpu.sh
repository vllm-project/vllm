...
KV_BUFFER_DEVICE="xpu"  # Default to xpu
...
get_num_gpus() {
  if [[ -n "${ZE_AFFINITY_MASK:-}" ]]; then
    echo "${ZE_AFFINITY_MASK}" | tr ',' '\n' | grep -c .
  elif command -v xpu-smi &> /dev/null; then
    xpu-smi discovery 2>/dev/null | grep -ci "device"
  else
    # Works for non-xpu platforms,
    # assuming at least 1 device and
    # let system decide which card to use.
    echo "1"
  fi
}
...
run_tests_for_model() {
  ...
  for i in $(seq 0 $((NUM_PREFILL_INSTANCES-1))); do
    # Calculate XPU ID - we'll distribute across available XPUs
    ...
    echo "Starting prefill instance $i on XPU $GPU_ID, port $PORT"
  ...
  for i in $(seq 0 $((NUM_DECODE_INSTANCES-1))); do
    # Calculate XPU ID - we'll distribute across available XPUs, starting from after prefill XPUs
    ...
    echo "Starting decode instance $i on XPU $GPU_ID, port $PORT"
