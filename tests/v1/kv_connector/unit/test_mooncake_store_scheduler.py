...
def test_finished_partial_tail_is_pinned_as_store_job():
    scheduler = _make_bare_scheduler(hash_block_size=4, enable_partial_hash_hits=True)
    ...
    # The request holds the table source until the next metadata build pins it.
    ...
    assert req_meta.partial_tail_offloads == [(1, 9, 12)]
    # The partial tail is also in the request table, so it is pinned once.
