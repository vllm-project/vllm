...
class DummyModelRunnerOutput(ModelRunnerOutput):
    def __init__(
        ...
        self.kv_connector_output = KVConnectorOutput(
            ...
            expected_finished_reqs=expected_finished_reqs,
        )
...
def test_aggregate_workers_output_with_expected_finished_reqs():
    # We create the aggregator expecting to collect from 4 workers
    aggregator = KVOutputAggregator(world_size=4)
    assert aggregator._expected_finished_reqs == 4
    ...
    # collect from 2
    output1 = DummyModelRunnerOutput(
        finished_sending={"req1"}, expected_finished_reqs=2
    )
    output2 = DummyModelRunnerOutput(
        finished_recving={"req2"}, expected_finished_reqs=2
    )
    ...
    assert aggregated.kv_connector_output.expected_finished_reqs == 2
    ...
    # Req1 is still waiting for 2 more acks (expected_finished_reqs has no effect)
