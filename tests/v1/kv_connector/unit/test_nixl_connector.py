...
def test_register_kv_caches(
    ...
    nixl_module = "vllm.distributed.kv_transfer.kv_connector.v1.nixl.utils"
    with (
...
def test_kv_buffer_to_nixl_memory_types(
    ...
    with (
        ...
        patch(
            "vllm.distributed.kv_transfer.kv_connector.v1.nixl.utils.threading.Event"
        ),
        patch(
            "vllm.distributed.kv_transfer.kv_connector.v1.nixl.utils.threading.Thread"
        ),
        patch(
            "vllm.distributed.kv_transfer.kv_connector.v1.nixl.utils.current_platform",
            FakePlatform,
        ),
        patch(
...
def test_transfer_failure_logging(
    ...
    # Capture logs from the nixl.utils logger specifically
...
def test_compatibility_hash_validation(
    ...
    with contextlib.ExitStack() as stack:
        if "vllm_version" in version_override:
            ...
            stack.enter_context(
                patch(
                    "vllm.distributed.kv_transfer.kv_connector.v1.nixl.metadata.NIXL_CONNECTOR_VERSION",
    ...
    with (
        ...
        patch(
            "vllm.distributed.kv_transfer.kv_connector.v1.nixl.worker.zmq_ctx"
        ) as mock_zmq_ctx,
    ):
