...
def test_prefix_cache_hit_returns_finished(dist_init):
    """Test that prefix cache hits (no blocks to transfer) return via get_finished."""
    vllm_config = create_vllm_config()
    ...
    connector.connector_worker.nixl_wrapper.fail_send_notif = True

    request_id = "test_prefix_cache_hit"
    metadata = NixlConnectorMetadata()
    # Empty local_block_ids simulates full prefix cache hit
    metadata.add_new_req(
        request_id=request_id,
        local_block_ids=[],
        kv_transfer_params={
