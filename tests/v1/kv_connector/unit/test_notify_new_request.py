...
"""
Tests for the notify_new_request() hook added to fix GPU idle under load.
...
The fix is a single call in add_request() so the connector gets notified
as soon as a request arrives, not when the scheduler eventually gets around
to looking at the waiting queue.
"""
...
def test_notify_called_when_request_arrives():
    """
    Core regression test for #41784.

    connector.notify_new_request() must be called the moment add_request()
    ...
    mock_conn.notify_new_request.assert_called_once_with(req)


def test_notify_called_for_each_distinct_request():
    """
    Three separate requests -> three notify calls, one each, in order.
    """
    scheduler, mock_conn = _scheduler_with_mock_connector()
    ...
    assert mock_conn.notify_new_request.call_count == 3
    mock_conn.notify_new_request.assert_has_calls(
        [call(r) for r in reqs], any_order=False
    )


def test_no_notify_on_streaming_update_to_existing_request():
    """
    A streaming chunk update re-uses the same request_id. We should not
    call notify_new_request again - the connector already knows the request
    and a second notify could trigger a redundant disk fetch.
    """
    scheduler, mock_conn = _scheduler_with_mock_connector()

    req = create_request(num_tokens=16)
    scheduler.add_request(req)
    count_after_first = mock_conn.notify_new_request.call_count
    ...
    assert mock_conn.notify_new_request.call_count == count_after_first
...
def test_notify_not_called_on_internal_reenqueue():
    """
    When the scheduler preempts a request and re-enqueues it internally
    via _enqueue_waiting_request(), notify must NOT fire again.

    That path bypasses add_request() entirely - the connector already
    knows the request. A double-notify there would be wrong.
    """
    scheduler, mock_conn = _scheduler_with_mock_connector()

    req = create_request(num_tokens=16)
    scheduler.add_request(req)
    assert mock_conn.notify_new_request.call_count == 1
