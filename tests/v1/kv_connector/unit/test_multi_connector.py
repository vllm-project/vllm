...
def _make_policy_connector(
    ...
    connector._failed_async_loads = set()
...
def test_range_aware_load_queries_all_connectors_while_lookup_pending():
    ...
    assert connector._request_load_ranges == {}
    assert connector._request_async_loads == {}

    connector._connectors[0].get_num_new_matched_tokens.return_value = (64, True)
    assert connector.get_num_new_matched_tokens(request, 0) == (96, True)
    assert connector._request_load_ranges == {
        "req": {
            0: KVLoadRange(0, 64, is_terminal=False),
            1: KVLoadRange(64, 96),
        }
    }
...
def test_range_transfer_results_wait_for_failed_and_successful_sources(failed_source):
    ...
    worker._async_load_sources = {"req": {0, 1}}
    worker._pending_async_loads = {"req": {0, 1}}
    ...
    assert worker._failed_async_loads == set()
    assert worker.get_transfer_results(set()) == KVConnectorTransferResults()


def test_range_transfer_results_clear_failed_state_on_request_finish():
    worker = _make_policy_connector([(0, False)] * 2, [True] * 2)
    worker._async_load_sources = {"req": {0, 1}}
    worker._pending_async_loads = {"req": {1}}
    worker._failed_async_loads = {"req"}
    ...
    assert worker._async_load_sources == {}
    assert worker._pending_async_loads == {}
    assert worker._failed_async_loads == set()
