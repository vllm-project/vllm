...
def test_merge_seeds_pending_finished_recving():
    """Recv votes completed in a failed step must surface at the next
    aggregate(), even if no worker reports them again."""
    aggregator = KVOutputAggregator(expected_finished_count=2)

    # Failed step: req1's recv votes completed (2/2).
