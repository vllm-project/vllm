...
from vllm.distributed.kv_transfer.kv_connector.v1.cpu_connector import (
    d2h_page_copy, h2d_copy_leading_tokens, h2d_copy_trailing_tokens, h2d_page_copy
)
...
def test_h2d_copy_leading_tokens():
    ...
    dst_layer = torch.full((2, 1, page_size, head_size, hidden_size), 
                          fill_value=2.0, dtype=torch.float32, device='cuda')
    
    # Copy first 8 tokens (half of page_size)
    end_position = 8
    h2d_copy_leading_tokens(
        src_buffer, dst_layer,
    ...
    assert torch.allclose(
        dst_layer[0, 0, :end_position].cpu(),
        src_buffer[0, 0, :end_position]
    )
    assert torch.allclose(
        dst_layer[1, 0, :end_position].cpu(), 
        src_buffer[1, 0, :end_position]
    )
    
    # Verify remaining tokens are unchanged (should still be 2.0)
    expected_unchanged = torch.full((page_size - end_position, head_size, hidden_size), 
                                  fill_value=2.0, dtype=torch.float32)
    assert torch.allclose(
        dst_layer[0, 0, end_position:].cpu(),
        expected_unchanged
    )
...
def test_h2d_copy_trailing_tokens():
    ...
    assert torch.allclose(
        dst_layer[0, 0, start_position:].cpu(),
        src_buffer[0, 0, start_position:]
    )
    assert torch.allclose(
        dst_layer[1, 0, start_position:].cpu(),
...
def test_h2d_page_copy():
    ...
    h2d_page_copy(
        src_buffer, dst_layer, block_ids,
        start_token_idx, stop_token_idx, block_size
    )
    ...
    for i in range(num_blocks):
        if i < start_block or i >= end_block:
            ...
            assert torch.allclose(
                dst_layer[:, block_ids[i], :start_pos].cpu(),
            ...
            assert torch.allclose(
                dst_layer[:, block_ids[i], :end_pos].cpu(),
...
def test_h2d_page_copy_edge_cases():
    ...
    assert torch.allclose(
        dst_layer[:, block_ids[1], start_pos:end_pos].cpu(),
    ...
    for i in range(num_blocks):
        if i == 0 or i == 3:
            ...
            assert torch.allclose(
                dst_layer[:, block_ids[i]].cpu(),
                expected_unchanged
            ), f"Block {i} should be unchanged"
        else:
            # Middle blocks (1 and 2) should be fully copied
            assert torch.allclose(
                dst_layer[:, block_ids[i]].cpu(),
                src_buffer[:, i]
            ), f"Block {i} should be fully copied"
