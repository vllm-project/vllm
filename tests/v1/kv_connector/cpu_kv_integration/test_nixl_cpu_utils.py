...
from vllm.distributed.kv_transfer.kv_connector.v1.nixl_cpu_utils import (
    NixlCPUSender, NixlCPUReceiver, SourceSpec, DestinationSpec,
    RingBufferAllocator
)
...
def run_receiver(buffer_config, host, base_port, rank, ready_event, stop_event):
...
def run_sender_with_protocol(buffer_config, host, base_port, rank, receiver_ready_event, success_event):
...
class TestNixlCPUUtils:
    ...
    def buffer_config(self):
        ...
        torch_buffer = torch.zeros(buffer_size, dtype=torch.uint8, device='cpu')
    ...
    def test_creation_with_invalid_buffer_size(self, buffer_config):
        ...
        with pytest.raises(Exception):  # Specific exception type depends on NIXL implementation
    ...
    def test_nixl_handshake_multiprocess(self, buffer_config):
        ...
        receiver_process = mp.Process(
            target=run_receiver,
            args=(buffer_config, test_host, test_base_port, 
                  test_rank, receiver_ready, stop_receiver)
        )
    ...
    def test_nixl_protocol_communication(self, buffer_config):
        ...
        receiver_process = mp.Process(
            target=run_receiver_with_progress,
            args=(buffer_config, test_host, test_base_port, 
                  test_rank, receiver_ready, stop_receiver)
        )
        receiver_process.start()
        ...
        sender_process = mp.Process(
            target=run_sender_with_protocol,
            args=(buffer_config, test_host, test_base_port, 
                  test_rank, receiver_ready, protocol_success)
        )
        sender_process.start()
