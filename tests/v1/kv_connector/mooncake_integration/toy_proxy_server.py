...
"""Minimal disaggregated P/D proxy for the Mooncake connector.

Adapted from examples/disaggregated/mooncake_connector/mooncake_connector_proxy.py.
Unlike the NIXL proxy, the decoder discovers the prefiller through the Mooncake
bootstrap server: the proxy queries each prefiller's bootstrap `/query` endpoint
to learn its engine_id, then injects ``remote_bootstrap_addr`` /
``remote_engine_id`` / ``transfer_id`` into the decode request.
"""
