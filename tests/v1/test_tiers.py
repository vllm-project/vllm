...
class SLOTestSetup:
    ...
    def create_async_llm(self, mock_output_processor_instance: Mock) -> AsyncLLM:
        ...
        with patch('vllm.v1.engine.async_llm.EngineCoreClient') as mock_engine_core_client, \
             patch('vllm.v1.engine.async_llm.OutputProcessor') as mock_output_processor:
            
            # Setup mocks
            mock_engine_core = AsyncMock()
            mock_engine_core_client.return_value = mock_engine_core
...
async def test_max_num_reqs_slo_validation():
    ...
    mock_output_processor_instance.get_num_unfinished_requests.return_value = 2  # Already at limit
    ...
    # Test 1: Request with tier=1.0 should be rejected (2/2 > 1.0 is false, but 2+1 > 2 is true)
...
async def test_max_pending_context_tokens_slo_validation():
    ...
    mock_output_processor_instance.get_num_pending_context_tokens.return_value = 1000  # At limit
