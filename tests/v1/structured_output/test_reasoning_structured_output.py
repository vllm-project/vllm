...
class TestReasoningStructuredOutput:
    ...
    def test_should_advance_trims_nothing_when_reasoning_never_started(
        self,
        manager_with_reasoner,
        mock_request_with_structured_output,
    ):
        """A parser reporting reasoning ended without a marker in the delta
        (e.g. Mistral when the model never generated ``[THINK]``) must not
        misclassify the first content token as the end marker: the whole
        step is content and must reach the grammar untrimmed.
        """
        structured_req = mock_request_with_structured_output.structured_output_request
        structured_req.reasoning_ended = False
        structured_req.structured_output_key = (
        ...
        start_token, end_token = 111, 222

        class OptionalThinkingReasoner:
            def __init__(self, *_, **__):
                pass

            def is_reasoning_end_streaming(self, input_ids, delta_ids):
                if end_token in list(delta_ids):
                    return True
                return start_token not in input_ids

        structured_req.reasoner = OptionalThinkingReasoner()
        ...
        assert structured_req.reasoning_ended is True
        assert result is True
        assert structured_req.reasoning_end_token_index == 2
        assert (
