...
def test_unsupported_json_features_by_type(schema_type, omit_type, request):
...
def test_schema_maps_do_not_treat_property_names_as_constraints(keyword):
    """Names in schema maps are not validation keywords."""
    schema = {
        keyword: {"pattern": {"type": "string"}, "maxLength": {"type": "integer"}}
    }
    assert not has_xgrammar_unsupported_json_features(schema)
...
def test_schema_data_is_not_checked_as_a_subschema(keyword):
...
def test_implicit_type_constraints_in_subschemas_are_detected(schema):
    assert has_xgrammar_unsupported_json_features(schema)
...
def test_referenced_schema_data_is_checked(container, reference, omit_type):
...
def test_recursive_referenced_schema_is_checked(unsupported):
    properties: dict[str, dict[str, str | int]] = {"next": {"$ref": "#/x-schema"}}
    if unsupported:
        properties["code"] = {"pattern": "^a+$", "maxLength": 3}
...
def test_boolean_schemas_and_inapplicable_constraints_are_supported(schema):
    assert not has_xgrammar_unsupported_json_features(schema)
