...
def _create_mtp_proposer(
    ...
    proposer_cls: type[EagleProposer] = EagleProposer,
) -> EagleProposer:
...
def test_step3p5_mtp_propose_uses_spec_step_idx():
    """Step3.5 MTP should select the matching draft layer per token."""
    ...
    proposer = _create_mtp_proposer(
        ...
        proposer_cls=Step3p5MTPProposer,
    )
    hidden_size = proposer.hidden_size
    ...
    assert result.shape == (batch_size, num_speculative_tokens)
    assert [call.kwargs["spec_step_idx"] for call in model_mock.call_args_list] == [
        0,
        1,
        2,
    ]
    assert [
        call.kwargs["spec_step_idx"]
