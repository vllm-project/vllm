...
count, so correctness rests on four properties, each tested here:
  * the opt-in is fail-closed (``context_kv_capture_supported``),
  * under a reused DP sync every rank reaches the same eager decision
    (``context_kv_needs_eager``),
...
@pytest.mark.parametrize(
    ("num_query_per_req", "sample_from_anchor"),
    [(4, False), (3, True)],  # 1+N layout and anchor layout, N=3
)
def test_context_kv_eager_gate_is_dp_agreed(num_query_per_req, sample_from_anchor):
    """Under a reused DP sync the drafter runs no collective, so the eager
    decision must be identical on every rank whatever its local batch."""
    spec = 3

    def target_sync(uniform_token_count):
        return DPSyncState(
            num_tokens_across_dp=torch.tensor([40, 4, 8], dtype=torch.int32),
            uniform_token_count=uniform_token_count,
            eager=False,
            num_reqs=2,
        )

    # (dummy_run, num_reqs, num_target_tokens) as seen by three DP ranks.
    prefill_rank = (False, 2, 40)
    decode_rank = (False, 1, 1 + spec)
    idle_rank = (True, 2, 2 * (1 + spec))
    ranks = (prefill_rank, decode_rank, idle_rank)

    def decide(sync, rank):
        return context_kv_needs_eager(
            sync, *rank, num_query_per_req, sample_from_anchor
        )

    # Some rank has a prefill (or ranks disagree): every rank goes eager.
    assert {decide(target_sync(None), r) for r in ranks} == {True}
    # Pure uniform decode at 1+N: every rank keeps the graph.
    assert {decide(target_sync(1 + spec), r) for r in ranks} == {False}
    # Agreed but not the graph's layout (e.g. truncated near max_model_len).
    assert {decide(target_sync(1), r) for r in ranks} == {True}

    # Without DP the rank's own batch decides; dummy batches keep the graph.
    assert decide(None, prefill_rank)
    assert not decide(None, decode_rank)
    assert not decide(None, idle_rank)
    assert not decide(None, (True, 2, 40))
