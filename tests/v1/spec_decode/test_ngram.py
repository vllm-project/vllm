...
def test_ngram_proposer():
    ...
    # No match.
    result = ngram_proposer(2, 2,
                            2).propose(context_token_ids=np.array([1, 2, 3, 4, 5]))
    ...
    result = ngram_proposer(
        3, 4,
        2).propose(context_token_ids=np.array([2, 3, 4, 5, 1, 2, 3, 4, 1, 2, 3, 4]))
    assert np.array_equal(result, np.array([1, 2]))  # Not [5, 1]

    # Match for 2-gram and 3-gram, but not 4-gram.
    result = ngram_proposer(
        2, 4, 2).propose(context_token_ids=np.array([3, 4, 5, 2, 3, 4, 1, 2, 3, 4]))
