...
def test_peagle_speculators_model(vllm_runner, example_prompts, monkeypatch):
    """
    Test PEagle speculators model properly initializes speculative decoding.

    Verifies:
    1. Speculative config is automatically initialized from speculators config
    2. Method is detected as 'eagle3' (peagle maps to eagle3)
    3. parallel_drafting is enabled
    ...
    with vllm_runner(
        ...
        assert spec_config.method == "eagle3", (
            f"Expected method='eagle3', got '{spec_config.method}'"
        )
        assert spec_config.parallel_drafting is True, (
            "Expected parallel_drafting=True for PEagle model"
        )
...
def test_peagle_speculators_correctness(monkeypatch):
    """
    E2E correctness test for PEagle via the speculators auto-detect path.
    ...
    assert acceptance_len >= al_threshold, (
        f"PEagle speculators acceptance length too low: "
