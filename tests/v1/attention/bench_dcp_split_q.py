...
configs = [
    triton.testing.Benchmark(
        x_names=["num_decodes"],
        x_vals=[1, 4, 16, 64, 128, 256, 512, 1024, 1234, 2000, 3000, 5000, 5120],
        line_arg="provider",
        line_vals=["triton", "pytorch", "compiled", "cudagraph"],
        line_names=["Triton fused", "PyTorch eager", "torch.compile", "CUDAGraph"],
        styles=[
            ("blue", "-"),
            ("red", "--"),
            ("green", "-."),
...
def bench_dcp_split_q(
    ...
    if provider == "triton":
        ...
        # Warmup compilation
        for _ in range(3):
            compiled_fn(global_seq_lens, block_table, num_decodes, tokens_per_req)
