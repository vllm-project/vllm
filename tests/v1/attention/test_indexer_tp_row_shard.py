...
def _bound_tables(chunks, num_tokens):
    ...
    chunk_ks = torch.cat([c.cu_seqlen_ks for c in chunks])
    chunk_ke = torch.cat([c.cu_seqlen_ke for c in chunks])
    rows = slice(_DECODE_ROWS, _DECODE_ROWS + chunk_ks.numel())
    ks[rows] = chunk_ks
    ke[rows] = chunk_ke
    return ks, ke
...
def _run_group(monkeypatch, world, chunks, num_tokens, logits, split=None):
    ...
    rows = sum(chunk.token_end - chunk.token_start for chunk in chunks)
...
def test_tp_row_shard_prefill_ignores_padded_tail(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The gathered real rows must not be assigned across padded tokens."""
    chunks, unpadded_end = _build_chunks([2049, 849, 651, 549])
    num_tokens = unpadded_end + 13
    logits = torch.randn(
        num_tokens, _NUM_KV, generator=torch.Generator().manual_seed(53691)
    )

    def no_exchange(local, sizes):
        raise AssertionError("the replicated reference must not exchange")
    ...
    for rank, (buffer, gathers) in enumerate(
        _run_group(
            ...
            num_tokens=num_tokens,
            logits=logits,
            split=[2049, 849, 651, 549],
        )
    ):
        torch.testing.assert_close(
            buffer[:unpadded_end], baseline[:unpadded_end], msg=f"rank {rank} diverged"
        )
        assert torch.all(buffer[unpadded_end:] == -1)
...
@pytest.mark.parametrize(
    "max_seq_len,index_topk,force_mqa,expected",
    [
        (2047, 2048, False, False),
        (2048, 2048, False, False),
        (2049, 2048, False, True),
        (2047, 2048, True, True),
        (2048, 2048, True, True),
    ],
)
def test_prefill_uses_mqa_matches_attention_routing(
    max_seq_len: int, index_topk: int, force_mqa: bool, expected: bool
) -> None:
    """Row sharding only runs when sparse MQA consumes the Top-K output."""
    assert indexer._prefill_uses_mqa(max_seq_len, index_topk, force_mqa) is expected
...
def _set_sharding_env(
    monkeypatch: pytest.MonkeyPatch, overrides: dict[str, bool] | None = None
) -> None:
    """Set every lazy environment flag read by the sharding gate."""
    values = {
...
def test_row_sharding_gate_rejects_unsupported_configurations(
    ...
    _set_sharding_env(monkeypatch, env)
