...
def _reference_dcp_split_q(
    ...
    """Pure-PyTorch reference, originally from FlashInferMLAImpl._split_q_for_dcp."""
...
def test_dcp_split_q_sum_across_ranks() -> None:
    ...
    # Last token in each request (offset=0) should have local seq_lens summing
    # to the global seq_len.
    rank_locals = []
    for dcp_rank in range(dcp_world_size):
        out_sl, _ = dcp_split_q(
            global_seq_lens=global_seq_lens,
