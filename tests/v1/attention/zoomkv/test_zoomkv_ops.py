...
def test_assemble_context_includes_unaligned_local_boundary():
    device = torch.device("cpu")
    seq_len, sink, local, block_size, tk = 3000, 64, 256, 16, 100
    topk = torch.arange(sink, sink + tk, device=device).view(1, -1)

    idx, valid = assemble_sparse_context_indices(
        seq_len,
        topk,
        sink,
        local,
    ...
    # raw local_start=2744, aligned local_start=2736: all eight boundary
    # tokens are always attended, replacing the eight lowest-ranked Top-K slots.
    assert idx.shape == (1, sink + local + tk)
    assert bool(valid.all())
    assert torch.equal(idx[0, :sink], torch.arange(sink))
    assert torch.equal(idx[0, sink : sink + 264], torch.arange(2736, 3000))
    assert torch.equal(idx[0, sink + 264 :], topk[0, :92])
...
def test_sparse_decode_gate():
    ...
    metadata.seq_lens_cpu = torch.tensor([512])
    metadata.seq_lens = torch.tensor([512])
    ...
    assert _should_use_mixed_sparse_decode(
        ...
        num_decodes=1,
        num_prefills=1,
        seq_lens_cpu=torch.tensor([512, 100]),
    )
    ...
    assert not _should_use_mixed_sparse_decode(
        ...
        num_decodes=1,
        num_prefills=1,
        seq_lens_cpu=torch.tensor([4096, 100]),
    )
...
def test_use_sparse_gate_matches_builder_helper():
    ...
    assert _should_use_sparse_decode(
        ...
        num_prefills=0,
        num_reqs=2,
        seq_lens_cpu=torch.tensor([512, 4096]),
    )
    assert not _should_use_sparse_decode(
        cfg=cfg,
        max_query_len=1,
        ...
        num_prefills=0,
        num_reqs=2,
        seq_lens_cpu=torch.tensor([511, 4096]),
    )
    ...
    assert _should_use_mixed_sparse_decode(
        ...
        num_decodes=2,
        num_prefills=1,
        seq_lens_cpu=torch.tensor([512, 4096, 100]),
    )
    assert not _should_use_sparse_decode(
