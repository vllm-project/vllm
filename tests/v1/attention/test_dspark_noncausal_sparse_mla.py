...
def test_dspark_metadata_replay_refreshes_decode_state(backend):
    ...
    config = SimpleNamespace(
        model_config=SimpleNamespace(
            max_model_len=1024,
            hf_config=SimpleNamespace(sliding_window=128, compress_ratios=[0]),
        ),
        scheduler_config=SimpleNamespace(max_num_batched_tokens=20, max_num_seqs=4),
        speculative_config=SimpleNamespace(
            num_speculative_tokens=5, parallel_drafting=False, use_dspark=lambda: True
        ),
    )
    kv_spec = SlidingWindowMLASpec(
        block_size=128,
        num_kv_heads=1,
        head_size=512,
        dtype=torch.bfloat16,
    ...
    for num_reqs, length in [(4, 20), (3, 137), (1, 900), (4, 513)]:
        ...
        common.query_start_loc.copy_(qsl)
