...
def test_route_needs_dcp():
    ...
    assert _configured(dcp_world_size=8, cp_interleave=1) is True


def test_round_robin_interleave_other_than_one_is_excluded():
    """The kernel's global-position causal window assumes interleave 1, the
    same restriction the segmented gate carries."""
    assert _configured(dcp_world_size=8, cp_interleave=2) is False
    assert _configured(dcp_world_size=8, cp_interleave=16) is False
