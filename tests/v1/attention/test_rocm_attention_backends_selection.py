...
@pytest.mark.parametrize("use_dcp", [False])
@pytest.mark.parametrize(
...
def test_aiter_fa_preferred_over_rocm_attn(
    ...
    VLLM_ROCM_USE_AITER=1 on CDNA, auto-selection must pick ROCM_AITER_FA
    (the AITER FlashAttention path) rather than the slower non-AITER
    ROCM_ATTN, while leaving the non-AITER ordering unchanged when AITER
    (MHA) is off.
    """
    for key, value in env_vars.items():
        monkeypatch.setenv(key, value)
