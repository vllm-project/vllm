...
These exercise the pure decision logic: which values are accepted, what the
scheduler is left holding, and what a read-back reports. Nothing here allocates
a KV cache or touches a GPU, so it runs in the default CI lane; the reinit path
itself is covered by the hardware bench in the PR description.
"""
...
class _Scheduler:
    """Enough of a scheduler for the knob code: the two caps it writes and the running
    list it must respect."""
...
class _Core:
    """EngineCore's knob methods bound to a bare object, so no engine has to be
    started."""
...
def test_bad_values_are_rejected_and_change_nothing(knobs):
    core = _Core(_Scheduler())
    before = (
        core.scheduler.max_num_running_reqs,
        core.scheduler.max_num_scheduled_tokens,
    )
...
def test_lowering_below_occupancy_stops_admission_without_breaking_the_step_assertion():
    """The scheduler asserts len(running) <= cap every step, so the cap cannot drop
    below what is already running.

    The target is recorded, the enforced cap stays at the occupancy, and the
    read-back reports the target, so a caller can see the difference between
    what it asked for and what is in force."""
    core = _Core(_Scheduler(max_num_seqs=64, running=20))
    out = core.trimtab_set_knobs({"max_num_seqs": 4})
    assert out["ok"]
    assert (
        core.scheduler.max_num_running_reqs == 20
    )  # unchanged while those 20 are in flight
...
def test_log_level_is_case_insensitive_and_recorded():
    core = _Core(_Scheduler())
    assert (
        core.trimtab_set_knobs({"log_level": "debug"})["applied"]["log_level"]
        == "DEBUG"
    )
...
def test_reinit_validation_refuses_before_anything_is_released(fields, bad):
    """A bad value caught during the rebuild would leave the engine with no KV cache, so
    it is caught first."""
...
def test_reinit_validation_accepts_a_reasonable_request():
    assert (
        _trimtab_validate_reinit({"gpu_memory_utilization": 0.85, "max_num_seqs": 32})
        == {}
    )
