...
def test_incremental_detokenization(
    ...
    requests = [
        ...
        for idx, (prompt, prompt_tokens) in enumerate(
            zip(
                dummy_test_vectors.prompt_strings,
        ...
        if num_sample_logprobs is not None:
            ...
            for idx, (sampled_token, pos_logprob_dict) in enumerate(
                ...
                for jdx in range(1, len(ref_pos_logprob_toks)):
                    ...
                    # present
                    rank_one_appears = (
                        True if lp_rank == 1 else rank_one_appears
                    )
        ...
        if num_prompt_logprobs is not None:
            ...
            for idx, (prompt_token, pos_logprob_dict) in enumerate(
                ...
                for jdx in range(1, len(ref_pos_prompt_logprob_toks)):
                    ...
                    # present
                    rank_one_appears = (
                        True if plp_rank == 1 else rank_one_appears
                    )
...
@pytest.mark.parametrize(
    "num_sample_logprobs", [None, NUM_SAMPLE_LOGPROBS_UNDER_TEST]
)
@pytest.mark.parametrize(
    "num_prompt_logprobs", [None, NUM_PROMPT_LOGPROBS_UNDER_TEST]
)
...
def test_stop_token(
    ...
    * else, the detokenized string should be
    ...
    generation_tokens = (
        dummy_test_vectors.generation_tokens[0] + 2 * suffix_token
    )
    if do_logprobs:
...
def test_iteration_stats(dummy_test_vectors):
    ...
    output_processor.process_outputs(
        outputs, engine_core_timestamp, iteration_stats
    )
