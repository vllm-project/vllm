...
def _create_random_top_token_test_matrix(
    ...
    matrix = torch.cat((torch.tensor(
        tokens_list, dtype=torch.int).unsqueeze(-1), choice_tensor.view(shape)),
                       dim=1)
...
def generate_dummy_prompt_logprobs_tensors(
    ...
    (
        token_vector,
        prompt_token_ranks,
    ) = _create_random_top_token_test_matrix((num_prompt_logprobs, num_logprobs), 0,
                                             len(tokenizer.vocab) - 1,
                                             prompt_tokens_list[1:])
    return LogprobsTensors(
        token_vector,
        _create_random_top_logprob_test_matrix((num_prompt_logprobs, num_logprobs + 1),
                                               -100, 0), prompt_token_ranks)
...
class MockEngineCore:
    ...
    def __init__(
        ...
        generated_logprobs_raw: Optional[list[list[tuple[list[int], list[float],
                                                         int]]]] = None,
