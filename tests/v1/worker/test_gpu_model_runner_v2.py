...
def test_custom_sampler_rejects_sampling_masks():
    """Custom samplers do not implement the sampling-mask protocol."""
    custom_sampler = (object(), None)
    with pytest.raises(
        ValueError, match="return_sampling_mask is not supported with custom samplers"
    ):
        model_runner_module._validate_custom_sampler_sampling_mask(
            custom_sampler, return_sampling_mask=True
        )
