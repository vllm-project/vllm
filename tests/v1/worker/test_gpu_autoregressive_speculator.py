...
def _make_speculator(
    ...
    # Only forwarded to the patched `set_forward_context`, never dereferenced.
    speculator.vllm_config = cast(VllmConfig, None)
...
def test_load_model_keeps_mm_support_for_capable_drafter(monkeypatch):
    ...
    speculator.vllm_config = cast(VllmConfig, SimpleNamespace(model_config=object()))
    speculator.max_num_tokens = 4
    speculator.hidden_size = 3
    speculator.dtype = torch.float32
...
def test_multi_step_decode_replays_captured_graph_as_expected(
    ...
    speculator.decode_cudagraph_manager = cast(
        SpeculatorCudaGraphManager, SimpleNamespace(run_fullgraph=run_fullgraph)
    )
...
def test_update_draft_decode_metadata_updates_fa3_scheduler_metadata(
    ...
    builder.cache_config = cast(CacheConfig, SimpleNamespace(cache_dtype="bfloat16"))
