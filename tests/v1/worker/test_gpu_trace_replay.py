...
def test_replay_overwrites_sampled_at_each_step():
    ...
    trace_len = _i32([3, 3])
    trace_token_ids = torch.zeros(2, MAX_TRACE_LEN, dtype=torch.int32, device=DEVICE)
...
def test_past_end_of_trace_leaves_sampled_untouched():
    ...
    trace_token_ids = torch.zeros(1, MAX_TRACE_LEN, dtype=torch.int32, device=DEVICE)
...
def test_trace_too_long_rejected():
    ...
    with pytest.raises(ValueError, match="too long"):
        state.add_request(
            0, SamplingParams(trace_decode_token_ids=[0] * (MAX_TRACE_LEN + 1))
        )
