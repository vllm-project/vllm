...
from vllm.v1.worker.gpu_model_runner import InputBatch, CachedRequestState
...
def _construct_expected_sampling_metadata(
    ...
    for req in reqs:
        if not req.req_id in req_ids_retained:
            continue
        ...
        stop_token_ids[index_in_input_batch] = req.sampling_params.all_stop_token_ids
    ...
    return SamplingMetadata(
        ...
        frequency_penalties=torch.tensor(
            frequency_penalties, dtype=torch.float, device=device).unsqueeze(dim=1),
        presence_penalties=torch.tensor(
            presence_penalties, dtype=torch.float, device=device).unsqueeze(dim=1),
        repetition_penalties=torch.tensor(
            repetition_penalties, dtype=torch.float, device=device).unsqueeze(dim=1),
...
def _construct_cached_request_state(req_id_suffix:int):
    prompt_token_ids = [np.random.randint(0, VOCAB_SIZE) for _ in range(
        np.random.randint(0, MAX_PROMPT_SIZE))]
    output_token_ids = [np.random.randint(0, VOCAB_SIZE) for _ in range(
        np.random.randint(0, NUM_OUTPUT_TOKENS))]
...
def test_sampling_metadata_in_input_batch(
    ...
    sampling_metadata = input_batch.make_sampling_metadata(
        req_id_reqs, skip_copy=False)
    
    # Create expected output.
    expected_sampling_metadata = _construct_expected_sampling_metadata(
        reqs, req_ids_retained,
        input_batch.req_id_to_index, device=torch.device(device)
    )
    # Assert the actual and expected output.
    assert torch.allclose(
        expected_sampling_metadata.temperature, sampling_metadata.temperature)
    assert torch.allclose(expected_sampling_metadata.top_p, sampling_metadata.top_p)
    assert torch.allclose(
        expected_sampling_metadata.top_k, sampling_metadata.top_k)
    assert torch.allclose(
    ...
    assert (
        expected_sampling_metadata.output_token_ids == sampling_metadata.output_token_ids)
    assert (
        expected_sampling_metadata.min_tokens == sampling_metadata.min_tokens)    
    assert (
        expected_sampling_metadata.stop_token_ids == sampling_metadata.stop_token_ids)
    assert (
        expected_sampling_metadata.no_penalties == sampling_metadata.no_penalties)
    assert (
        expected_sampling_metadata.no_top_p == sampling_metadata.no_top_p)
    assert (
        expected_sampling_metadata.no_top_k == sampling_metadata.no_top_k)
