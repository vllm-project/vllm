...
def test_input_batch_reinitialized_after_late_interleave_adjustment(monkeypatch):
    ...
    # jit_warmup_registry is injected onto the instance by
    # @JitWarmupRegistry.capture, so it is not declared on the class.
    jit_warmup_registry = Mock()
    jit_warmup_registry.activate.return_value = nullcontext()
    runner.jit_warmup_registry = jit_warmup_registry  # type: ignore[attr-defined]
