...
def test_custom_processor_add_request_gates_pipeline_flag(admitted: bool):
    """A custom processor's add_request() return value is OR-ed into both
    per-request pipeline flags; the head-dtype verifier gates on the second."""
    sampler = _make_sampler(custom_logits_processors=[_GateProcessor(admitted)])
    sampler.add_request(3, SamplingParams())

    assert sampler.needs_logits_processing[3] == admitted
    assert sampler.needs_stored_logits_processing[3] == admitted
...
def test_head_dtype_processing_reuses_input(sampling_params: SamplingParams):
    sampler = _make_sampler()
    sampler.add_request(0, sampling_params)
