...
class _Runner:
    def __init__(self, num_reserved: int) -> None:
        self.req_states = RequestState(
            ...
            num_reserved_slots=num_reserved,
        )
...
def _fill_serving_pool(runner: _Runner) -> None:
    for i in range(MAX_NUM_REQS - NUM_RESERVED):
        idx = _add_request(runner, f"live-{i}")
        runner.block_tables.append_block_ids(idx, ([i + 1],), overwrite=True)
...
def test_reserved_warmup_preserves_live_state():
    runner = _Runner(NUM_RESERVED)
    _fill_serving_pool(runner)
    ...
    with preserve_serving_state(runner):
        assert runner.req_states.free_indices == runner.req_states.reserved_indices
    ...
    live_rows = slice(0, MAX_NUM_REQS - NUM_RESERVED)
    assert _pool_state(runner) == pool_before
    assert torch.equal(
        runner.block_tables.block_tables[0].gpu[live_rows], blocks_before[live_rows]
    )
...
def test_reserved_warmup_restores_state_on_error():
    runner = _Runner(NUM_RESERVED)
    _fill_serving_pool(runner)
    pool_before = _pool_state(runner)
...
def test_full_pool_warmup_uses_all_rows_and_null_blocks():
    runner = _Runner(num_reserved=0)

    with preserve_serving_state(runner, full_pool=True):
        assert len(runner.req_states.free_indices) == MAX_NUM_REQS
        idx = _add_request(runner, "_warmup")
        runner.block_tables.append_block_ids(idx, ([5, 6, 7],), overwrite=True)
        runner.block_tables.apply_staged_writes()
        assert runner.block_tables.block_tables[0].gpu[idx, :3].tolist() == [0, 0, 0]


def test_full_pool_warmup_rejects_live_requests():
    runner = _Runner(num_reserved=0)
    _add_request(runner, "live")

    with (
        pytest.raises(AssertionError, match="empty request pool"),
        preserve_serving_state(runner, full_pool=True),
    ):
        pass
