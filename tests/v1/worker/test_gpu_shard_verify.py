...
"""Routing-math tests for batch-sharded speculative verification.
...
from vllm.v1.worker.gpu.spec_decode import batch_shard
from vllm.v1.worker.gpu.spec_decode.batch_shard import (
    BatchSharder,
...
def _make_batch(
    ...
    return InputBatch(
        ...
        expanded_idx_mapping=torch.from_numpy(expanded_idx_np),
        expanded_local_pos=torch.from_numpy(expanded_pos_np),
        ...
        logits_indices=torch.from_numpy(logits_indices_np),
...
def test_local_batch_partition(tp_size: int, seed: int):
    ...
    for rank, (local, _, metadata) in enumerate(results):
        ...
        assert local.idx_mapping.numpy().tolist() == expected_idx.tolist()
        assert (np.diff(local.cu_num_logits_np) == np.diff(cu)[owned]).all()
        assert local.cu_num_logits.numpy().tolist() == local.cu_num_logits_np.tolist()
        assert (
            local.expanded_idx_mapping.numpy()
            == batch.expanded_idx_mapping.numpy()[rows]
        ).all()
        assert (
            local.expanded_local_pos.numpy() == batch.expanded_local_pos.numpy()[rows]
        ).all()
        assert (
            local.logits_indices.numpy() == batch.logits_indices.numpy()[rows]
        ).all()
        assert (local.seq_lens.numpy() == batch.seq_lens.numpy()[owned]).all()
    ...
    sorted_logits_indices = results[0][1].numpy()
    for _, gi, _ in results:
        assert (gi.numpy() == sorted_logits_indices).all()
    rows_all = np.concatenate(
        [_owned_rows(cu, _owned_batch_indices(local)) for local, _, _ in results]
    )
    assert (
        sorted_logits_indices.tolist()
        == batch.logits_indices.numpy()[rows_all].tolist()
    )
...
def test_all_to_all_reassembly_simulated(tp_size: int):
    ...
    token_logits = torch.randn(batch.num_tokens, vocab)
...
def test_shard_no_spec():
    ...
    for local, _, metadata in results:
        ...
        owned = _owned_batch_indices(local)
        assert local.logits_indices.numpy().tolist() == (
            batch.logits_indices.numpy()[owned].tolist()
        )
...
def test_gather_src_indices_round_trip(tp_size: int):
    ...
    for local, _, _ in results:
        owned = _owned_batch_indices(local)
        block = torch.zeros(pad, 1, dtype=torch.int64)
        block[: len(owned), 0] = torch.from_numpy(owned)
        blocks.append(block)
