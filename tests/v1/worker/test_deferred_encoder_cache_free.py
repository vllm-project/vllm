...
"""Tests for deferred encoder cache eviction.

Verifies that encoder cache entries are not freed until after MTP draft
proposal completes, preventing cache misses in _gather_mm_embeddings.
...
class TestNewModelRunnerDeferredFree:
    ...
    def test_free_states_defers_instead_of_freeing(self):
        """free_states() should save hashes, not free them."""
        ...
        scheduler_output.free_encoder_mm_hashes = ["img_a"]
        ...
        # Hashes should be deferred.
        assert runner._deferred_encoder_free_hashes == ["img_a"]
