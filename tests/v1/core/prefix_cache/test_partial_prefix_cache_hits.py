...
def test_dcp_full_report_events_use_effective_and_partial_spans():
    block_size = 4
    manager = make_full_mamba_manager(
        dcp_world_size=2,
        hash_block_size=block_size,
    ...
    prefix = list(range(12))
    req0 = make_request("event-owner", prefix, block_size, sha256)
    computed_blocks, num_computed, _ = manager.get_computed_blocks(req0)
    assert manager.allocate_slots(req0, 12, num_computed, computed_blocks) is not None
    manager.free(req0)
    manager.new_step_starts()
    manager.block_pool.take_events()

    req1 = make_request(
        "event-replay", prefix + list(range(12, 16)), block_size, sha256
    )
    ...
    full_events = [event for event in events if event.group_idx == 0]
    mamba_events = [event for event in events if event.group_idx == 1]
    assert [(event.block_size, event.token_ids) for event in full_events] == [
        (8, prefix[:8]),
        (4, prefix[8:12]),
    ]
    assert [(event.block_size, event.token_ids) for event in mamba_events] == [
        (4, prefix[8:12])
    ]
