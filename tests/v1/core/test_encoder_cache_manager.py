...
def test_basic_allocate_and_reuse():
    ...
    assert cache.can_allocate(req, 0, int(1e9), accumulate_encoder_token)
...
def test_freeing_decreases_refcount_and_moves_to_freeable():
    ...
    accumulate_encoder_token = 0
    assert manager.can_allocate(req, 0, int(1e9), accumulate_encoder_token)
...
def test_free_request_frees_all_inputs():
    ...
    assert manager.can_allocate(req, 1, int(1e9), accumulate_encoder_token)
...
def test_eviction_when_cache_is_full():
    ...
    accumulate_encoder_token = 0
    assert manager.can_allocate(req1, 0, int(1e9), accumulate_encoder_token)
    manager.allocate(req1, 0)
    manager.free_encoder_input(req1, 0)

    assert manager.can_allocate(req2, 0, int(1e9), accumulate_encoder_token)
...
def test_get_cached_input_ids():
    ...
    assert manager.can_allocate(req, 2, int(1e9), accumulate_encoder_token)
...
def test_schedule_request_multi_images():
    ...
    accumulate_encoder_token = 0
    assert manager.can_allocate(req, 0, int(1e9), accumulate_encoder_token)
    accumulate_encoder_token += req._token_counts[0]

    assert not manager.can_allocate(req, 1, int(1e9), accumulate_encoder_token)
