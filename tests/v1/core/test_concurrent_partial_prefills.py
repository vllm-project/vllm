...
def test_max_long_partial_prefills_limits_long_subset():
    ...
    long_req = create_requests(
        num_requests=1, num_tokens=200, max_tokens=4, req_ids=["long"]
    )
    short_req = create_requests(
        num_requests=1, num_tokens=20, max_tokens=4, req_ids=["short"]
    )
...
def test_short_requests_not_blocked_by_partial_prefill_limit():
    ...
    short_reqs = create_requests(
        num_requests=4, num_tokens=10, max_tokens=4, req_ids=["s0", "s1", "s2", "s3"]
    )
    for r in short_reqs:
        scheduler.add_request(r)
