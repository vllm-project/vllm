...
from vllm.config import SchedulerConfig, ModelConfig, CacheConfig
...
def _create_test_request(request_id: str, 
                         max_tokens: int,
...
def test_multiple_stop_tokens(scheduler):
    ...
    # EOS token is generated in the beginnig of the output tokens
    request = _create_test_request("test1", 100, stop_token_ids=[42, 43, 44])
    scheduler.requests[request.request_id] = request
    ...
    max_tokens = 2
    request = _create_test_request("test2", max_tokens, stop_token_ids=[42, 43, 44])
    scheduler.requests[request.request_id] = request
