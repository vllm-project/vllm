...
from vllm.config import KVCacheConfig, SchedulerConfig, VllmConfig
...
def create_mock_request(
    ...
    mock_req._all_token_ids = list(
        range(num_prompt_tokens)
    )  # Internal list for all_token_ids
    # Public view
    mock_req.all_token_ids = list(range(num_prompt_tokens))  

    # This property needs to be dynamic based 
    # on _output_token_ids and spec_token_ids
    type(mock_req).num_tokens_with_spec = PropertyMock(
        return_value=num_prompt_tokens
        + len(mock_req.output_token_ids)
        + len(getattr(mock_req, "spec_token_ids", []))
    )
    ...
    mock_req.sampling_params.max_tokens = (
    ...
    def record_event_side_effect(event_type, timestamp=None):
        if not hasattr(mock_req, "events"):
            mock_req.events = []
        mock_req.events.append(
            MagicMock(type=event_type, timestamp=timestamp or time.monotonic())
        )
    ...
    def append_output_token_ids_side_effect(token_ids):
        ...
        # Update property mocks if necessary, though for these tests,
        # fixed values might be okay
        # For simplicity, we assume num_tokens_with_spec and num_tokens 
        # don't change drastically after creation
        # in a way that affects basic scheduling decisions for these tests.

    mock_req.append_output_token_ids = MagicMock(
        side_effect=append_output_token_ids_side_effect
    )
    ...
    mock_req.get_finished_reason = MagicMock(
        side_effect=get_finished_reason_side_effect
    )
...
class TestSchedulerPriority(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_scheduler_config.long_prefill_token_threshold = (
            0  # Disable for these tests
        )
        ...
        self.mock_kv_cache_config.num_gpu_blocks = (
            128  # Enough blocks for tests
        )
        self.mock_kv_cache_config.num_cpu_blocks = 128
        self.mock_kv_cache_config.kv_cache_dtype = "auto"
        self.mock_kv_cache_config.kv_cache_groups = [
            MagicMock()
        ]  # Simplistic mock for kv_cache_groups
        self.mock_vllm_config.cache_config = (
            self.mock_kv_cache_config
        )  # Assign to vllm_config

        # Other configs needed by Scheduler's KVCacheManager
        self.mock_vllm_config.model_config = MagicMock()
        self.mock_vllm_config.model_config.max_model_len = (
            self.mock_scheduler_config.max_model_len
        )
        self.mock_vllm_config.model_config.get_vocab_size = MagicMock(
            return_value=32000
        )
        self.mock_vllm_config.speculative_config = None
        self.mock_vllm_config.lora_config = None
        self.mock_vllm_config.kv_events_config = None
        self.mock_vllm_config.kv_transfer_config = (
            None  # Important for disabling KVConnector
        )
        ...
        self.mock_kv_cache_manager.get_block_ids = MagicMock(
            return_value=[[1, 2, 3]]
        )  # Dummy block IDs
        self.mock_kv_cache_manager.get_computed_blocks = MagicMock(
            return_value=(MagicMock(), 0)
        )
        self.mock_kv_cache_manager.take_events = MagicMock(return_value=[])

        # StructuredOutputManager Mock
        self.mock_structured_output_manager = MagicMock(
            spec=StructuredOutputManager
        )
        self.mock_structured_output_manager.grammar_bitmask = MagicMock(
            return_value={}
        )

        # EncoderCacheManager Mock (needed by Scheduler)
        self.mock_encoder_cache_manager = MagicMock()
        self.mock_encoder_cache_manager.get_freed_ids = MagicMock(
            return_value=[]
        )
    ...
    def _create_scheduler(self, policy: str) -> Scheduler:
        ...
        # Re-create KVCacheManager instance for each scheduler if 
        # its behavior depends on config 
        # For these tests, the shared mock_kv_cache_manager is okay 
        # as its methods are mocked.

        scheduler = Scheduler(
            vllm_config=self.mock_vllm_config,
            # Pass the KVCacheConfig object
    ...
    def test_basic_priority_scheduling(self):
        ...
        self.mock_kv_cache_manager.allocate_slots.return_value = (
            MagicMock()
        ...
        req1 = create_mock_request(
            "req1",
            priority=1,
            arrival_time=time.monotonic() + 0.01,
            num_prompt_tokens=10,
        )
        req2 = create_mock_request(
            "req2",
            priority=0,
            arrival_time=time.monotonic() + 0.02,
            num_prompt_tokens=10,
        )  # Higher priority
        req3 = create_mock_request(
            "req3",
            priority=1,
            arrival_time=time.monotonic() + 0.03,
            num_prompt_tokens=10,
        )
        req4 = create_mock_request(
            "req4",
            priority=0,
            arrival_time=time.monotonic() + 0.04,
            num_prompt_tokens=10,
        )  # Higher priority, later arrival
        ...
        self.assertEqual(
            scheduled_ids1, ["req2", "req4"]
        )  # req2 and req4 have priority 0
        ...
        self.assertEqual(
            scheduled_ids2, ["req1", "req3"]
        )  # req1 and req3 have priority 1
    ...
    def test_fcfs_for_same_priority(self):
        ...
        req1_p0_t1 = create_mock_request(
            "req1_p0_t1", priority=0, arrival_time=time.monotonic() + 0.01
        )
        req2_p0_t2 = create_mock_request(
            "req2_p0_t2", priority=0, arrival_time=time.monotonic() + 0.02
        )
        req3_p1_t0 = create_mock_request(
            "req3_p1_t0", priority=1, arrival_time=time.monotonic() + 0.00
        )  # Lower priority, earliest arrival
        ...
        self.assertEqual(
            scheduled_ids, ["req1_p0_t1", "req2_p0_t2"]
        )  # FCFS among priority 0
    ...
    def test_preemption_lowest_priority(self):
        ...
        running_req1_p1_t1 = create_mock_request(
            "running_req1_p1_t1",
            priority=1,
            arrival_time=1.0,
            status=RequestStatus.RUNNING,
        )
        running_req2_p2_t2 = create_mock_request(
            "running_req2_p2_t2",
            priority=2,
            arrival_time=2.0,
            status=RequestStatus.RUNNING,
        )  # Lowest priority among running

        scheduler.running = [running_req1_p1_t1, running_req2_p2_t2]
        scheduler.requests = {r.request_id: r for r in scheduler.running}

        # Simulate KVCacheManager being full for new requests initially, 
        # then becoming available after preemption
        # For the new high-priority request, allocate_slots will initially 
        # fail, triggering preemption.
        # Then, for the same request in the next schedule call 
        # (after it's added to waiting and picked again), it should succeed.
        ...
        def allocate_slots_side_effect(req, num_new_tokens, *args, **kwargs):
            ...
            if (
                req.request_id == "waiting_req0_p0_t0"
            ):  # The new high-priority request
                allocation_attempts += 1
                if (
                    allocation_attempts == 1
                ):  # First attempt for the new request
                    # Ensure this is called when trying to schedule the 
                    # new high-prio request
                    self.assertEqual(
                        len(scheduler.running), 2
                    )  # Max capacity before preemption
            ...
            # Allocate for other requests if any 
            # (e.g. if running ones are scheduled again)
            return (
                MagicMock()
            )  

        self.mock_kv_cache_manager.allocate_slots.side_effect = (
            allocate_slots_side_effect
        )

        # New high-priority request
        waiting_req0_p0_t0 = create_mock_request(
            "waiting_req0_p0_t0", priority=0, arrival_time=0.5
        )
        ...
        self.assertEqual(output1.scheduled_new_reqs[0].req_id, \
                         "waiting_req0_p0_t0")

        # Check running queue: running_req1 should still be there, 
        # waiting_req0 should have been scheduled
        ...
        preempted_priority, _, preempted_req_obj = scheduler.waiting[
            0
        ]  # It's a heap
        ...
        # Check that allocate_slots was called twice for the new request 
        # (once failed, once succeeded)
        # This check is a bit tricky due to how many times schedule might be 
        # called internally or how allocate_slots is used.
        # The important part is that preemption happened and the correct 
        # request was preempted.
        # A simpler check might be to ensure the preempted request 
    ...
    def test_fcfs_policy_scheduling(self):
        ...
        req1_p1_t1 = create_mock_request(
            "req1_p1_t1", priority=1, arrival_time=time.monotonic() + 0.01
        )
        req2_p0_t2 = create_mock_request(
            "req2_p0_t2", priority=0, arrival_time=time.monotonic() + 0.02
        )  # Priority ignored
        req3_p2_t0 = create_mock_request(
            "req3_p2_t0", priority=2, arrival_time=time.monotonic() + 0.00
        )  # Priority ignored, earliest arrival

        scheduler.add_request(
            req1_p1_t1
        )  # Added first by this call, but t0 is earlier
        scheduler.add_request(req2_p0_t2)
        scheduler.add_request(
            req3_p2_t0
        )  # Added last by this call, but t0 is earliest

        # Order of addition to scheduler's internal deque matters for FCFS 
        # if arrival_time were identical
        # But here, arrival_time should dominate if the scheduler sorts
        # by it or processes in order of add_request
        # vLLM's FCFS is based on the order requests are added to the 
        # self.waiting deque.

        # Expected order if add_request appends and schedule processes 
        # from left of deque:
    ...
    def test_fcfs_policy_preemption(self):
        ...
        running_req1 = create_mock_request(
            "running_req1",
            priority=0,
            arrival_time=1.0,
            status=RequestStatus.RUNNING,
        ...
        self.mock_kv_cache_manager.allocate_slots.return_value = (
            None  # No space, trigger preemption
        )

        waiting_req2 = create_mock_request(
            "waiting_req2", priority=1, arrival_time=2.0
        )  # Priority ignored
        scheduler.add_request(waiting_req2)

        output = scheduler.schedule()
        self.assertEqual(len(output.scheduled_new_reqs), 1)
        self.assertEqual(output.scheduled_new_reqs[0].req_id, "waiting_req2")

        # running_req1 should be preempted
        self.assertEqual(
            len(scheduler.running), 1
        )  # waiting_req2 should now be running
