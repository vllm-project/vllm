...
def create_mock_request(
    ...
    mock_req._all_token_ids = list(
        range(num_prompt_tokens))  # Internal list for all_token_ids
    ...
    type(mock_req).num_tokens = PropertyMock(return_value=num_prompt_tokens +
    ...
    def record_event_side_effect(event_type, timestamp=None):
        ...
        mock_req.events.append(
            MagicMock(type=event_type, timestamp=timestamp
                      or time.monotonic()))
...
class TestSchedulerPriority(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_kv_cache_manager.get_block_ids = MagicMock(
            return_value=[[1, 2, 3]])  # Dummy block IDs
    ...
    def _create_scheduler(self, policy: str) -> Scheduler:
        ...
        scheduler = Scheduler(
            ...
            kv_cache_config=self.
            mock_kv_cache_config,  # Pass the KVCacheConfig object
    ...
    def test_basic_priority_scheduling(self):
        ...
        req2 = create_mock_request("req2",
                                   ...
                                   num_prompt_tokens=10)  # Higher priority
        ...
        req4 = create_mock_request(
            ...
            num_prompt_tokens=10)  # Higher priority, later arrival
        ...
        self.assertEqual(scheduled_ids1,
                         ["req2", "req4"])  # req2 and req4 have priority 0
        ...
        self.assertEqual(scheduled_ids2,
                         ["req1", "req3"])  # req1 and req3 have priority 1
    ...
    def test_fcfs_for_same_priority(self):
        ...
        req1_p0_t1 = create_mock_request("req1_p0_t1",
                                         priority=0,
                                         arrival_time=time.monotonic() + 0.01)
        req2_p0_t2 = create_mock_request("req2_p0_t2",
                                         priority=0,
                                         arrival_time=time.monotonic() + 0.02)
        req3_p1_t0 = create_mock_request(
            "req3_p1_t0", priority=1, arrival_time=time.monotonic() +
            0.00)  # Lower priority, earliest arrival
        ...
        self.assertEqual(scheduled_ids,
                         ["req1_p0_t1", "req2_p0_t2"])  # FCFS among priority 0
    ...
    def test_preemption_lowest_priority(self):
        ...
        running_req1_p1_t1 = create_mock_request("running_req1_p1_t1",
                                                 priority=1,
                                                 arrival_time=1.0,
        ...
        running_req2_p2_t2 = create_mock_request(
            ...
            priority=2,
            arrival_time=2.0,
            status=RequestStatus.RUNNING)  # Lowest priority among running
        ...
        def allocate_slots_side_effect(req, num_new_tokens, *args, **kwargs):
            ...
            if req.request_id == "waiting_req0_p0_t0":  # The new high-priority request
                ...
                if allocation_attempts == 1:  # First attempt for the new request
                    ...
                    self.assertEqual(len(scheduler.running),
                                     2)  # Max capacity before preemption
            ...
            return MagicMock(
            )  # Allocate for other requests if any (e.g. if running ones are scheduled again)

        self.mock_kv_cache_manager.allocate_slots.side_effect = allocate_slots_side_effect

        # New high-priority request
        waiting_req0_p0_t0 = create_mock_request("waiting_req0_p0_t0",
                                                 priority=0,
                                                 arrival_time=0.5)
    ...
    def test_fcfs_policy_scheduling(self):
        ...
        req1_p1_t1 = create_mock_request("req1_p1_t1",
                                         priority=1,
                                         arrival_time=time.monotonic() + 0.01)
        req2_p0_t2 = create_mock_request("req2_p0_t2",
                                         priority=0,
                                         arrival_time=time.monotonic() +
                                         0.02)  # Priority ignored
        req3_p2_t0 = create_mock_request(
            "req3_p2_t0", priority=2, arrival_time=time.monotonic() +
            0.00)  # Priority ignored, earliest arrival

        scheduler.add_request(
            req1_p1_t1)  # Added first by this call, but t0 is earlier
        scheduler.add_request(req2_p0_t2)
        scheduler.add_request(
            req3_p2_t0)  # Added last by this call, but t0 is earliest
    ...
    def test_fcfs_policy_preemption(self):
        ...
        running_req1 = create_mock_request("running_req1",
                                           priority=0,
                                           arrival_time=1.0,
                                           status=RequestStatus.RUNNING)
        ...
        waiting_req2 = create_mock_request(
            "waiting_req2", priority=1, arrival_time=2.0)  # Priority ignored
        ...
        self.assertEqual(len(scheduler.running),
                         1)  # waiting_req2 should now be running
