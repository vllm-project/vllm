...
@pytest.mark.parametrize("block_drop", [False, True])
def test_get_kv_cache_config_glm5_with_dflash_draft_group(dtype, with_tail, block_drop):
    """Draft pages have independent storage and retain their native geometry."""
    model_config = ModelConfig(max_model_len=8192)
    vllm_config = VllmConfig(model_config=model_config)
    vllm_config.cache_config.kv_cache_layout = "LBNHC"
    ...
    draft_groups = [
        ...
        and all(
            type(s) is SlidingWindowSpec
            for s in g.kv_cache_spec.kv_cache_specs.values()
        )
    ]
    assert len(draft_groups) == 1
    assert draft_groups[0].is_eagle_group is block_drop
    assert draft_groups[0].kv_cache_spec.block_size == 16
    ...
    assert len(groups) == 6 + int(with_tail)
