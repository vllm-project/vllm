...
def test_dsv4_flash_0731_kv_capacity(monkeypatch):
    ...
    # The DSv4 mixed-page packing needs a block-outermost layout; the engine
    # core resolves this once, tests set it explicitly.
    ...
    kv_cache_config = get_kv_cache_config_from_groups(vllm_config, groups, available)
    ...
    # The startup max_model_len fit-check must gate on the steady footprint
    # only. Charging each SWA-class group's in-flight pages at pool-stride
    # cost inflated the per-request estimate to ~100 KiB/token at short
    # contexts and blocked 1M-context startup entirely (RTX PRO 5000
    # datapoint in the #51041 PR discussion).
    startup_gate = _max_memory_usage_bytes_from_groups(vllm_config, groups)
    gate_per_token = startup_gate / max_model_len
    assert gate_per_token < 10 * 1024, gate_per_token
    # The gate is a steady bound: it cannot exceed the full per-request peak.
    assert startup_gate <= sum(
