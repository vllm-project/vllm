...
class TestFindBestGroupSize:
    ...
    KV cache group sizes while preferring larger groups (fewer tensors).

    Key behaviors:
    - Prefers LARGER group sizes (fewer tensors = better memory patterns)
    - Enforces group_size >= 3 unless overhead exceeds 10%
    - For homogeneous layers, group_size == num_layers (single group)
