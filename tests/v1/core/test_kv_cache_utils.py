...
def test_mamba_groups_never_flagged_when_draft_matches_target_layout():
    # The draft marker keeps the draft layer in a separate group even when its
    # other cache properties match the target layers. What must never happen is
    # a Mamba group being flagged: that widens its lookup window to two
    # consecutive chunks, which align-mode checkpointing never produces,
    # zeroing every lookup.
    groups = get_kv_cache_groups(
