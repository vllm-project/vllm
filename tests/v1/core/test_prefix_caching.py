...
def make_request(request_id,
    ...
    req.block_hashes = hash_request_tokens(hash_fn, block_size, req)
...
def append_output_token_id(request: Request,
                           token_id: int,
                           block_size: int = 16,
                           hash_fn=hash):
    request.append_output_token_ids(token_id)
    # check if new token completed a new block
    if request.num_tokens % block_size == 0:
        # calculate hash for the newly completed block
        end_token_idx = request.num_tokens
        start_token_idx = end_token_idx - block_size

        extra_keys, _ = generate_block_hash_extra_keys(request,
                                                       start_token_idx,
                                                       end_token_idx, -1)
