...
def test_prefill(hash_algo):
    ...
    assert blocks.to_block_ids() == [1, 2, 3, 4]
    ...
    assert computed_blocks.to_block_ids() == [1, 2, 3]
    assert num_computed_tokens == 3 * 16
    num_new_tokens = 53 - 3 * 16
    blocks = manager.allocate_slots(req1, num_new_tokens, computed_blocks)
    assert blocks.to_block_ids() == [5]
    ...
    assert blocks.to_block_ids() == [7, 8, 9, 10, 4, 5, 6, 3, 2, 1]
...
def test_evict():
    ...
    computed_blocks, num_computed_tokens = manager.get_computed_blocks(req2)
    assert computed_blocks.to_block_ids() == [1, 2]
    assert num_computed_tokens == 2 * 16
    blocks = manager.allocate_slots(req2, 3, computed_blocks)
    assert blocks.to_block_ids() == [10]
