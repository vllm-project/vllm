...
def test_has_pending_kv_transfers_no_connector():
    """Without a KV connector, should never have pending KV transfers."""
    scheduler = create_scheduler()
    requests = create_requests(num_requests=3)
    for request in requests:
        scheduler.add_request(request)

    # no connector = never has pending transfers
    assert scheduler.has_pending_kv_transfers() is False

    scheduler.finish_requests(requests[0].request_id, RequestStatus.FINISHED_STOPPED)
    assert scheduler.has_pending_kv_transfers() is False


def test_has_pending_kv_transfers_with_connector():
    ...
    assert scheduler.has_pending_kv_transfers() is True
    ...
    assert scheduler.has_pending_kv_transfers() is True  # still has second request
