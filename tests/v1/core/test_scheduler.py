...
def test_kv_cache_manager_drop_wiring_for_drafter_method(
    ...
    """The scheduler must wire `use_eagle_block_drop()` — whose default is
    drafter-method aware, not `use_eagle`, which stays True for all of these
    drafters and keeps driving the prefill lookahead — into the KV cache
    manager's last-block drop."""
    ...
    assert scheduler.use_eagle_block_drop is expected_drop
