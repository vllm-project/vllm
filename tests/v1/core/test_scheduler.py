...
def _prepare_running_spec_decode_request(
    scheduler: Scheduler,
    request: Request,
    ...
    scheduler.update_draft_token_ids(
        DraftTokenIds([request.request_id], [draft_token_ids])
    )


def test_kv_transfer_bootstrap_isolation_disabled_allows_mixed_batch():
    """Flag disabled: bootstrap and running spec-decode mix in one batch."""
    scheduler = create_scheduler(
    ...
    running_req = create_requests(
        num_requests=1, num_tokens=1, req_ids=["running"]
    )[0]
    _prepare_running_spec_decode_request(scheduler, running_req, [1, 2, 3])

    bootstrap_req = create_requests(
        num_requests=1, num_tokens=5, req_ids=["bootstrap"]
    )[0]
    bootstrap_req.kv_transfer_params = {
        "do_remote_decode": True,
        "remote_num_tokens": 4,
    }
    scheduler.add_request(bootstrap_req)

    output = scheduler.schedule()

    assert output.num_scheduled_tokens == {
        running_req.request_id: 4,
        bootstrap_req.request_id: 1,
    }


def test_kv_transfer_bootstrap_isolation_defers_sync_bootstrap():
    """Flag enabled, sync connector, first scheduling: bootstrap request with
    num_computed_tokens==0 is deferred to the next step via the elif branch
    when the running batch is uniform spec-decode."""
    scheduler = create_scheduler(
        max_num_batched_tokens=8,
        num_speculative_tokens=3,
        use_kv_connector=mock_kv(matched_tokens=4, is_async=False),
        enable_kv_transfer_bootstrap_isolation=True,
    )
    running_req = create_requests(
        num_requests=1, num_tokens=1, req_ids=["running"]
    )[0]
    ...
    # Step 1: running decode proceeds, bootstrap is deferred.
    output1 = scheduler.schedule()
    assert running_req.request_id in output1.num_scheduled_tokens
    assert bootstrap_req.request_id not in output1.num_scheduled_tokens

    # Feed output so the running request advances.
    model_runner_output = ModelRunnerOutput(
    ...
    scheduler.update_from_output(output1, model_runner_output)
    scheduler.update_draft_token_ids(
        DraftTokenIds([running_req.request_id], [[4, 5, 6]])
    )

    # Step 2: _force_kv_transfer_bootstrap_next_step was set, so bootstrap
    # is scheduled alone.
    output2 = scheduler.schedule()
    assert output2.num_scheduled_tokens == {bootstrap_req.request_id: 1}
    assert running_req.request_id not in output2.num_scheduled_tokens


def test_kv_transfer_bootstrap_isolation_schedules_post_transfer_alone():
    """Flag enabled, post-transfer (num_computed_tokens>0): bootstrap request
    is scheduled alone; running spec-decode is skipped. This is the NIXL /
    async-connector scenario after KV transfer completes."""
    scheduler = create_scheduler(
        max_num_batched_tokens=8,
        num_speculative_tokens=3,
    ...
    # Simulate a post-transfer request: external KV already loaded,
    # only 1 residual token remains to be computed.
    bootstrap_req = create_requests(
        num_requests=1, num_tokens=5, req_ids=["bootstrap"]
    )[0]
    bootstrap_req.kv_transfer_params = {
    ...
    # Mark KV as already computed (post async-transfer state).
    bootstrap_req.num_computed_tokens = 4
    scheduler.add_request(bootstrap_req)

    output = scheduler.schedule()

    assert output.num_scheduled_tokens == {bootstrap_req.request_id: 1}
