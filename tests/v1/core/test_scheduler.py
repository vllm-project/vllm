...
def test_priority_scheduling_preempted_request_readmission_priority():
    """Test that when choosing between an unstarted request and a preempted request
    of the SAME priority, the preempted request is prioritised (fair re-admission)
    even if the unstarted request arrived earlier.
    """
    scheduler = create_scheduler_with_priority(
...
def test_priority_scheduling_preemption_victim_anti_thrashing():
    ...
    # Simulate 1 decode step: both requests now need a 3rd block
    # (6 blocks needed, only 5 free)
    model_output = ModelRunnerOutput(
        req_ids=["req_preempted_once", "req_never_preempted"],
    ...
    # Anti-thrashing check: req_never_preempted (0 preemptions) should be preempted
    # first, leaving req_preempted_once (1 preemption) running!
    output = scheduler.schedule()
