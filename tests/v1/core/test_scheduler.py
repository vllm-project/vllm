...
def _create_kv_events_scheduler(tier_scoped_clear_events: bool = True) -> Scheduler:
    """Prefix caching + KV events on a null publisher (tests swap in a Mock)."""
    return create_scheduler(
        enable_prefix_caching=True,
        kv_events_config=KVEventsConfig(
            enable_kv_cache_events=True,
            publisher="null",
            tier_scoped_clear_events=tier_scoped_clear_events,
        ),
    )
...
def test_idle_reset_publishes_scoped_clear_without_scheduler_step():
    ...
    scheduler = _create_kv_events_scheduler()
...
def test_scoped_clear_is_widened_to_legacy_all_clear_by_default():
    """``tier_scoped_clear_events`` defaults off: subscribers that predate
    ``TierBlocksCleared`` keep receiving ``AllBlocksCleared``, batched with the
    surrounding events exactly as before, until they are upgraded."""
    scheduler = _create_kv_events_scheduler(tier_scoped_clear_events=False)
    scheduler.kv_event_publisher = Mock()
    removed = BlockRemoved(block_hashes=[b"\xab" * 32], medium="GPU")
    scheduler.kv_cache_manager.take_events = Mock(
        return_value=[removed, TierBlocksCleared(medium="GPU")]
    )
    ...
    scheduler.kv_event_publisher.publish.assert_called_once()
    batch = scheduler.kv_event_publisher.publish.call_args.args[0]
    assert batch.events == [removed, AllBlocksCleared()]
