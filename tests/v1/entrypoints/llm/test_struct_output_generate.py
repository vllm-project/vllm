...
def test_guided_json_completion(
    monkeypatch, sample_json_schema, guided_decoding_backend: str
):
...
def test_guided_json_unsupported_schema(
    monkeypatch, unsupported_json_schema, guided_decoding_backend: str
):
    monkeypatch.setenv("VLLM_USE_V1", "1")
    llm = LLM(model=MODEL_NAME, max_model_len=1024)
    sampling_params = SamplingParams(
...
def test_guided_grammar_ebnf(
    monkeypatch, sample_sql_ebnf, guided_decoding_backend: str
):
    monkeypatch.setenv("VLLM_USE_V1", "1")
    llm = LLM(model=MODEL_NAME, max_model_len=1024)
    sampling_params = SamplingParams(
...
def test_guided_grammar_lark(
    monkeypatch, sample_sql_lark, guided_decoding_backend: str
):
    monkeypatch.setenv("VLLM_USE_V1", "1")
    llm = LLM(model=MODEL_NAME, max_model_len=1024)
    sampling_params = SamplingParams(
...
@pytest.mark.skip_global_cleanup
@pytest.mark.parametrize("guided_decoding_backend", GUIDED_DECODING_BACKENDS_V1)
def test_guided_grammar_ebnf_invalid(monkeypatch, guided_decoding_backend: str):
    monkeypatch.setenv("VLLM_USE_V1", "1")
    llm = LLM(model=MODEL_NAME, max_model_len=1024)
    sampling_params = SamplingParams(
        temperature=0.8,
...
def test_guided_regex(monkeypatch, sample_regex, guided_decoding_backend: str):
    ...
    with pytest.raises(
        ValueError, match="Regex guided decoding is not supported."
    ):
        llm.generate(
            prompts=[
...
def test_guided_choice_completion(
    monkeypatch, sample_guided_choice, guided_decoding_backend: str
):
    monkeypatch.setenv("VLLM_USE_V1", "1")
    llm = LLM(model=MODEL_NAME, max_model_len=1024)
    sampling_params = SamplingParams(
