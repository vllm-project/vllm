...
def test_topp_basic():
    with torch.device(DEVICE):
        ...
        result = apply_top_k_top_p(logits=logits.clone(), k=torch.tensor([3, 3]), p=torch.tensor([0.79, 0.79]))
...
def test_topp_select_all():
    with torch.device(DEVICE):
        ...
        result = apply_top_k_top_p(logits=logits.clone(), k=torch.tensor([3, 3]), p=torch.tensor([1.0, 1.0]))
...
def test_topp_with_ties():
    with torch.device(DEVICE):
        ...
        result = apply_top_k_top_p(logits=logits.clone(), k=torch.tensor([4]), p=torch.tensor([0.2]))
...
def test_both_topk_topp():
    with torch.device(DEVICE):
        ...
        result = apply_top_k_top_p(logits=logits.clone(), k=torch.tensor([1, 3]), p=torch.tensor([0.79, 0.79]))

        # Since for the first batch k=1, expect only the largest element gets selected.
        expected_result = logits.clone()
        expected_result[0,0] = float("-inf")
