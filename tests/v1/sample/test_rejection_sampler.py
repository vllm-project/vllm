...
def test_placeholder_draft_token_rejected_random(rejection_sampler):
    """A placeholder/invalid draft id (PLACEHOLDER_TOKEN_ID == -1) must always
    be rejected in non-greedy sampling, without indexing the probability
    tensors by the invalid id.
    ...
    Reproduces the bug where ``rejection_random_sample_kernel`` gathered
    ``target_probs[..., -1]`` (out of bounds / wrong row), which could
    spuriously "accept" the placeholder and emit ``-1`` as a real token.
    """
    vocab_size = 100
    # Drafts: two valid tokens (forced-accepted) followed by a placeholder.
    # The two valid drafts have target prob ~1, so they are accepted and we
    # reach the placeholder position. The second valid draft id is the last
    # vocab id, which is exactly the (wrong) cell the buggy kernel reads for
    # the placeholder position -- so the bug deterministically accepts it.
    spec_tokens = [[1, vocab_size - 1, PLACEHOLDER_TOKEN_ID]]
    # Target argmax per draft position: 1, vocab-1, then 7 (recovered token).
    output_tokens = [[1, vocab_size - 1, 7, 9]]  # 9 is the bonus token
    ...
    # The two valid drafts are accepted.
    assert sampled[0, 0].item() == 1
    assert sampled[0, 1].item() == vocab_size - 1
    # The placeholder draft must be rejected and replaced with a valid recovered
    # token -- never emitted as -1, and never out of range.
    recovered = sampled[0, 2].item()
    assert 0 <= recovered < vocab_size, (
        f"placeholder draft was not rejected: got {recovered}"
    )
