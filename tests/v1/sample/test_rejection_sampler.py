...
def create_logits_tensor(token_ids: List[int], vocab_size: int = 100) -> torch.Tensor:
    """Helper function to create logits tensor that will produce desired token ids on argmax"""
...
def test_multiple_sequences(sampler):
    ...
    spec_tokens = [[1, 2], [3, 4]]
    output_tokens = [1, 2, 5, 3, 4, 6]  # Two sequences with bonus tokens 5 and 6
...
@pytest.mark.parametrize("spec_tokens,output_tokens,expected", [
