...
def _create_allowed_token_ids(
    ...
    for i in range(batch_size):
        ...
        end = min(i + num_allowed_token_ids, vocab_size)
        mask[i, start:end] = True
        has_mask[i] = True
    return (has_mask, mask)
...
def _create_default_sampling_metadata(
    ...
    fake_sampling_metadata = SamplingMetadata(
        ...
        has_allowed_token_ids=[False] * batch_size,
        allowed_token_ids_mask=torch.zeros(batch_size,
                                           vocab_size,
                                           dtype=torch.bool,
                                           device=device),
    )
...
def test_sampler_allowed_token_ids(device: str, batch_size: int,
    ...
    sampling_metadata = _create_default_sampling_metadata(
        NUM_OUTPUT_TOKENS, batch_size, VOCAB_SIZE, torch.device(device))
    has_mask, mask = _create_allowed_token_ids(
        batch_size=batch_size,
        vocab_size=VOCAB_SIZE,
        num_allowed_token_ids=num_allowed_token_ids,
        device=torch.device(device),
    )
    sampling_metadata.has_allowed_token_ids = has_mask
