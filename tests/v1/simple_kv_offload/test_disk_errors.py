...
def submit(worker, is_store, event_idx=7, load_req_ids=()):
    # Two blocks exercise an I/O failure after part of a transfer has succeeded.
    metadata = (
        SimpleCPUOffloadMetadata(
            store_event=event_idx, store_gpu_blocks=[0, 1], store_cpu_blocks=[0, 1]
        )
        if is_store
            ...
            load_gpu_blocks=[0, 1],
            load_cpu_blocks=[0, 1],
            load_req_ids=list(load_req_ids),
            load_event_to_reqs={event_idx: ["request-0"]},
        )
    )
...
def test_disk_failure_reaches_model_runner_output(
    monkeypatch, disk_worker, runner, is_store, request_level
):
    ...
    submit(worker, is_store, load_req_ids=["request-0"] * 2 if request_level else [])
    events = worker._store_events if is_store else worker._load_events
    assert events.ready.wait(timeout=5)
    ...
    assert (output.finished_recving or set()) == (set() if is_store else {"request-0"})
    assert output.invalid_block_ids == (
        {0, 1} if not is_store and not request_level else set()
    )
    assert output.failed_recving == (
        {"request-0"} if not is_store and request_level else set()
    )
    if is_store:
...
@pytest.mark.parametrize("num_groups,failed_group", [(1, 0), (2, 0), (2, 1)])
...
def test_disk_read_failure_isolates_request_in_shared_batch(
    monkeypatch, disk_worker, num_groups, failed_group, policy, failure
):
    ...
    failed_gpu_block = transfer.gpu_block_ids[2 * failed_group + 1]
    failed_disk_slot = transfer.cpu_block_ids[2 * failed_group + 1]
    cpu_blocks = {
    ...
    received = results.finished_recving
    assert received == {request.request_id for request in requests}
    failed = worker.get_block_ids_with_load_errors()
    assert failed == ({failed_gpu_block} if num_groups == 1 else set())
    assert results.failed_recving == (
        {failed_request.request_id} if num_groups > 1 else set()
    )
...
def test_disk_failure_uses_transfer_position_when_block_ids_overlap(
    monkeypatch, disk_worker
):
    """Multi-group failure identities survive idle steps and repeated block IDs."""
    worker, backend = disk_worker
    monkeypatch.setattr(
        disk_module.os,
        "preadv",
        MagicMock(side_effect=[4096, OSError(errno.EIO, "read failed")]),
        raising=False,
    )
    worker.bind_connector_metadata(
        SimpleCPUOffloadMetadata(
            ...
            load_gpu_blocks=[0, 0],
            load_cpu_blocks=[0, 1],
            load_req_ids=["healthy", "failed"],
            load_event_to_reqs={7: ["healthy", "failed"]},
        )
    )
    worker.start_load_kv()
    assert worker._load_events.ready.wait(timeout=5)
    worker.bind_connector_metadata(
        SimpleCPUOffloadMetadata(load_event_to_reqs={7: ["healthy", "failed"]})
    )
    result = worker.get_transfer_results(set())
    assert result.finished_recving == {"healthy", "failed"}
    assert result.failed_recving == {"failed"}
    assert worker.get_block_ids_with_load_errors() == set()
    assert not worker._load_event_blocks
    assert not worker.get_transfer_results(set()).failed_recving
    assert backend._load_thread.is_alive()
