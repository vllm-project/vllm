...
def test_failed_load_evicts_disk_cache_before_all_ranks_finish(reset, request_level):
    ...
    sched.update_connector_output(
        KVConnectorOutput(
            failed_recving={req.request_id} if request_level else set(),
            invalid_block_ids=set() if request_level else set(load.load_gpu_blocks),
        )
    )
    assert sched.get_num_new_matched_tokens(req, 0) == (0, False)
