...
def test_bypass_external_lookup_releases_pending_cpu_hits() -> None:
    """Bypass releases a previous hit's pins even without a later allocation."""
    fix = make_scheduler(num_cpu_blocks=8, num_gpu_blocks=16, lazy=False)
    sched = fix.scheduler
    ...
    pending_hit = sched._pending_cpu_hits[cached_request.request_id]
    pinned_blocks = [
        block for group in pending_hit[0] for block in group if not block.is_null
    ]
    assert pinned_blocks
    ...
    for _ in range(2):
        ...
        assert cached_request.request_id not in sched._pending_cpu_hits
        assert all(block.ref_cnt == 0 for block in pinned_blocks)
