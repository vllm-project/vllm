...
def test_already_stored_block_not_evicted_during_prepare_store(eviction_policy):
    ...
    assert manager.lookup(to_hash(2)) is True
...
def test_cpu_manager():
    ...
    assert cpu_manager.lookup(to_hash(1)) is False
    assert cpu_manager.lookup(to_hash(2)) is False
    ...
    # lookup [1, 2]
    assert cpu_manager.lookup(to_hash(1)) is True
    assert cpu_manager.lookup(to_hash(2)) is True
    assert cpu_manager.lookup(to_hash(3)) is False
    ...
    assert cpu_manager.lookup(to_hash(1)) is False
    assert cpu_manager.lookup(to_hash(2)) is True
    assert cpu_manager.lookup(to_hash(3)) is True
    assert cpu_manager.lookup(to_hash(4)) is True
    assert cpu_manager.lookup(to_hash(5)) is True
    assert cpu_manager.lookup(to_hash(0)) is False
    ...
    assert cpu_manager.lookup(to_hash(7)) is True
    assert cpu_manager.lookup(to_hash(9)) is False
...
def test_arc_manager_basic():
    ...
    # lookup [1, 2] -> not ready
    assert arc_manager.lookup(to_hash(1)) is False
    assert arc_manager.lookup(to_hash(2)) is False
    ...
    # lookup [1, 2]
    assert arc_manager.lookup(to_hash(1)) is True
    assert arc_manager.lookup(to_hash(2)) is True
    assert arc_manager.lookup(to_hash(3)) is False
...
def test_arc_manager_t1_to_t2_promotion():
    ...
    assert to_hash(1) in arc_policy.t1
    assert to_hash(1) not in arc_policy.t2

    # touch block 1 (simulate second access)
    arc_manager.touch(to_hashes([1]))

    # block 1 should now be in T2 (frequent)
    assert to_hash(1) not in arc_policy.t1
    assert to_hash(1) in arc_policy.t2
...
def test_arc_manager_adaptive_target():
    ...
    assert to_hash(1) in arc_policy.b1
...
def test_arc_manager_t1_t2_eviction_policy():
    ...
    assert to_hash(5) in arc_policy.t1
...
def test_arc_manager_failed_store():
    ...
    assert arc_manager.lookup(to_hash(5)) is False
    # block 5 should not be in T1 or T2
    assert to_hash(5) not in arc_policy.t1
    assert to_hash(5) not in arc_policy.t2
...
def test_arc_manager_full_scenario():
    ...
    assert arc_manager.lookup(to_hash(2)) is True
    assert arc_manager.lookup(to_hash(3)) is True
...
def test_filter_reused_manager():
    ...
    assert manager.lookup(to_hash(1)) is False
    assert manager.lookup(to_hash(2)) is False
