...
def test_transfer(
    ...
    handler = CpuGpuOffloadingHandler(attn_backend=attn_backend,
                                      gpu_block_size=gpu_block_size,
                                      cpu_block_size=cpu_block_size,
                                      num_cpu_blocks=num_cpu_blocks,
                                      gpu_caches=list(gpu_caches.values()))
    ...
    if gpu_to_cpu:
        src_kv_caches = handler.gpu_caches
        dst_kv_caches = handler.cpu_caches
        ...
        src_kv_caches = handler.cpu_caches
        dst_kv_caches = handler.gpu_caches
    ...
    assert handler.transfer_async(1, (src_specs, dst_specs))
