...
These tests exercise:
  ...
  - The multimodal warmup can be launched in a background thread to overlap
    engine-core init (thread lifecycle, join by warmup/reset/shutdown,
    no double-run)
...
def _make_renderer_mock(mm_limits: dict[str, int]) -> MagicMock:
    ...
    renderer._mm_warmup_thread = None
...
class TestBackgroundMmWarmup:
    """The multimodal warmup can be launched in a background thread to overlap
    engine-core initialization; warmup()/reset_mm_cache/shutdown join it."""
    ...
    def test_start_creates_daemon_thread(self):
        ...
        with patch("vllm.multimodal.processing.TimingContext", autospec=True):
            renderer.start_mm_warmup_in_background()
            assert isinstance(renderer._mm_warmup_thread, Thread)
            assert renderer._mm_warmup_thread.daemon
            renderer._join_mm_warmup()

        assert renderer._mm_warmup_thread is None
        renderer.mm_processor.apply.assert_called_once()
    ...
    def test_start_is_run_at_most_once(self):
        ...
        with patch("vllm.multimodal.processing.TimingContext", autospec=True):
            renderer.start_mm_warmup_in_background()
            first = renderer._mm_warmup_thread
            renderer.start_mm_warmup_in_background()  # must not spawn a second
            assert renderer._mm_warmup_thread is first
            renderer._join_mm_warmup()
    ...
    def test_warmup_joins_background_and_does_not_rerun_mm(self):
        ...
        # MM apply called exactly once (by the background warmup, not re-run).
        renderer.mm_processor.apply.assert_called_once()
        # Chat warmup ran exactly once.
        renderer.render_chat.assert_called_once()
        # Background thread has been joined and cleared.
        assert renderer._mm_warmup_thread is None
    ...
    def test_warmup_does_not_rerun_mm_after_reset_joins_background(self):
        ...
        warmup() runs (it clears _mm_warmup_thread). warmup() must still not
        ...
        with patch("vllm.multimodal.processing.TimingContext", autospec=True):
            ...
            # (clears _mm_warmup_thread, sets _mm_warmup_done via the join).
            renderer._join_mm_warmup()
            assert renderer._mm_warmup_thread is None  # joined & cleared
            BaseRenderer.warmup(renderer, ChatParams())

        # MM apply called exactly once (by the background warmup); warmup()
        # did not re-run warmup_mm despite the thread pointer being cleared.
        renderer.mm_processor.apply.assert_called_once()
        renderer.render_chat.assert_called_once()
    ...
    def test_shutdown_joins_background_warmup(self):
        ...
        with patch("vllm.multimodal.processing.TimingContext", autospec=True):
            renderer.start_mm_warmup_in_background()
            thread = renderer._mm_warmup_thread
            renderer.shutdown()

        assert renderer._mm_warmup_thread is None
        # The background warmup was allowed to complete (apply ran).
        renderer.mm_processor.apply.assert_called_once()
        # Sanity: the thread object itself is no longer alive after shutdown.
