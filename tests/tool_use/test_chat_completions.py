...
# response_format silently suppressed tool calls under tool_choice: auto
@pytest.mark.asyncio
@pytest.mark.timeout(120)
async def test_response_format_with_tool_choice_auto(
    ...
    """
    Test that response_format does not prevent tool calls under the default
    tool_choice: auto.

    Before the fix the schema took the single structured-outputs slot, so the
    grammar masked out every tool-call token and the model answered with
    fabricated JSON instead of calling the tool.
    """
    models = await client.models.list()
    model_name: str = models.data[0].id
