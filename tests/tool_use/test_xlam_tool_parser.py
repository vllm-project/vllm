...
def test_extract_tool_calls_streaming(
    ...
    """Test incremental streaming behavior for cases that work correctly"""
...
def test_streaming_with_tool_call_xml_tags(xlam_tool_parser):
    ...
    current_text = """<tool_call>[{"name": "get_current_weather", "arguments": {"city": "Seattle", "state": "WA", "unit": "celsius"}}]</tool_call>"""  # noqa: E501
    ...
    # Manually set up the state for sending the tool
    xlam_tool_parser.current_tools_sent = [False]
...
def test_extract_tool_calls_streaming_incremental(xlam_tool_parser,
                                                  xlam_tokenizer):
    """Test that streaming is truly incremental for plain JSON tool calls"""
    model_output = """[{"name": "get_current_weather", "arguments": {"city": "Dallas", "state": "TX"}}]"""
