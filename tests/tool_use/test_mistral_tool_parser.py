...
@pytest.fixture
def mistral_v11_tool_parser(mistral_v11_tokenizer):
    return MistralToolParser(mistral_v11_tokenizer)
...
@pytest.mark.parametrize(
    ids=[
        "single_tool_add", "single_tool_weather",
    ],
    argnames=["model_output", "expected_tool_calls", "expected_content"],
    argvalues=[
        (
            '''[TOOL_CALLS]add{"a": 3.5, "b": 4}''',  # noqa: E501
            [
                ToolCall(function=FunctionCall(name="add",
                                               arguments=json.dumps({
                                                   "a": 3.5,
...
def test_extract_tool_calls_v11_tokenizer(mistral_v11_tool_parser, model_output,
                            expected_tool_calls, expected_content):
    extracted_tool_calls = mistral_v11_tool_parser.extract_tool_calls(
        model_output, request=None)  # type: ignore[arg-type]
    assert extracted_tool_calls.tools_called
...
def _test_extract_tool_calls_streaming(tool_parser, tokenizer,
    ...
    for delta_message in stream_delta_message_generator(
            tool_parser, tokenizer, model_output):
