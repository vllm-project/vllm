...
from vllm.entrypoints.openai.tool_parsers import (TngR1T2ToolParser,
                                                  ToolParserManager)
...
@pytest.fixture
def tng_tool_parser(tng_tokenizer: AnyTokenizer) -> TngR1T2ToolParser:
    return TngR1T2ToolParser(tng_tokenizer)
...
def test_streaming_and_non_streaming(model_output: str,
    ...
    tool_parser = TngR1T2ToolParser(tokenizer=MagicMock())
    if chunk_length:
...
async def test_chat_completion_serving(full_model_output: str,
    ...
    ToolParserManager.register_module("tng_r1t2", module=TngR1T2ToolParser)
