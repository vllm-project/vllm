...
from .utils import (MESSAGES_ASKING_FOR_PRODUCT_INFO,
                    MESSAGES_ASKING_FOR_SEARCH_WITH_OBJECT,
                    MESSAGES_ASKING_FOR_TOOLS, MESSAGES_WITH_TOOL_RESPONSE,
                    PRODUCT_TOOL, SEARCH_TOOL,
                    SEARCH_TOOL_WITH_OBJECT_ARGUMENT, WEATHER_TOOL)
...
# with integer and boolean arguments
@pytest.mark.asyncio
...
# test: request a chat completion with a tool that has object type arguments
# This test specifically checks the SEARCH_TOOL_WITH_OBJECT_ARGUMENT which
# has a nested object structure in its parameters.
@pytest.mark.asyncio
async def test_object_type_tool_call(client: openai.AsyncOpenAI):
    models = await client.models.list()
    model_name: str = models.data[0].id

    # Non-streaming test
    chat_completion = await client.chat.completions.create(
        messages=MESSAGES_ASKING_FOR_SEARCH_WITH_OBJECT,
        temperature=0,
        max_completion_tokens=100,
        model=model_name,
        tools=[SEARCH_TOOL_WITH_OBJECT_ARGUMENT],
        logprobs=False)
    ...
    # make sure the search tool was called with correct arguments
    assert tool_calls[0].function.name == SEARCH_TOOL_WITH_OBJECT_ARGUMENT[
        "function"]["name"]
    assert tool_calls[0].function.arguments is not None
    assert isinstance(tool_calls[0].function.arguments, str)
    ...
    # Validate the nested object structure
    assert "search_request" in parsed_arguments
    search_request = parsed_arguments["search_request"]
    assert isinstance(search_request, dict)

    # Validate the fields within the search_request object
    assert "query" in search_request
    assert isinstance(search_request["query"], str)
    assert search_request["query"] == "latest transformers papers"

    assert "retrieval_method" in search_request
    assert isinstance(search_request["retrieval_method"], str)
    assert search_request["retrieval_method"] == "rrf"
    ...
    # Validate the nested object structure in streamed response
    assert "search_request" in streamed_args
    streamed_search_request = streamed_args["search_request"]
    assert isinstance(streamed_search_request, dict)

    # Validate the fields within the search_request object in streamed response
    assert "query" in streamed_search_request
    assert isinstance(streamed_search_request["query"], str)
    assert streamed_search_request["query"] == "latest transformers papers"

    assert "retrieval_method" in streamed_search_request
    assert isinstance(streamed_search_request["retrieval_method"], str)
    assert streamed_search_request["retrieval_method"] == "rrf"
