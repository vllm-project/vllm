...
class TestGlm4AdjustRequestForcedToolChoice:
    """Tests for ``Glm4MoeModelToolParser.adjust_request`` on forced
    tool_choice values.

    GLM-4 family parsers (4.5 / 4.6 / 4.7) emit XML ``<tool_call>`` markers,
    so the grandparent ``ToolParser.adjust_request`` (which injects a Hermes
    JSON schema for ``tool_choice="required"`` and named-function choices)
    would force JSON output and break extraction. The parent must short-
    ...
    See vllm-project/vllm#41631 (Responses API named-function leak). The
    fix lives on the parent so it covers every GLM-4x subclass uniformly.
    """
    ...
    def test_required_skips_schema_injection_responses(self, glm4_moe_tool_parser):
        """``tool_choice="required"`` on /v1/responses must not get a JSON
        schema injected via ``request.text.format``."""
        ...
        # tool_choice itself stays as the user requested; the parser fixes
    ...
    def test_named_function_skips_schema_injection_responses(
        self, glm4_moe_tool_parser
    ):
        """Named-function ``tool_choice`` on /v1/responses arrives as
        ``ToolChoiceFunction`` (Pydantic-parsed). The parent must catch it
        too, not only the Chat Completions
    ...
    def test_required_skips_schema_injection_chat_completions(
        self, glm4_moe_tool_parser
    ):
        """Chat Completions ``tool_choice="required"`` is the parent's
        original primary case — confirm it still works."""
        request = ChatCompletionRequest(
            model=MODEL,
            messages=[],
            tools=[
    ...
    def test_auto_delegates_to_super_responses(self, glm4_moe_tool_parser):
        """``tool_choice="auto"`` should fall through to the grandparent
        path: no schema injected, but ``skip_special_tokens`` still flipped
        to ``False`` so XML markers survive decoding."""
    ...
    def test_no_tools_is_noop(self, glm4_moe_tool_parser):
        """No tools -> the parser delegates to ``super().adjust_request``
        which returns the request untouched."""
        from vllm.entrypoints.openai.responses.protocol import ResponsesRequest
