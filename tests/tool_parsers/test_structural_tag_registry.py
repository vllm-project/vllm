...
@pytest.mark.parametrize("model", ["llama", "kimi"])
def test_non_strict_rejected_for_xgrammar_builtin_models(
    model: str,
    sample_tools_strict: list[ChatCompletionToolsParam],
):
    with pytest.raises(ValueError, match="does not support non-strict"):
        get_model_structural_tag(
            model=model,
            tools=sample_tools_strict,
            tool_choice="required",
...
@pytest.mark.parametrize("model", sorted(VLLM_BUILTIN_STRUCTURAL_TAG_MODELS))
def test_vllm_builtin_structural_tags_accept_non_strict(
    model: str,
    sample_tools: list[ChatCompletionToolsParam],
):
    tag = get_model_structural_tag(
