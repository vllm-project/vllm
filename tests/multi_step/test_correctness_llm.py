...
def test_multi_step_llm_chunked_prefill_prefix_cache(
    ...
    * Generated logprobs are all very close
    ...
    example_prompts[0] = ('vLLM is a high-throughput and memory-efficient '
    ...
    if len(example_prompts) < num_prompts:
        example_prompts = (example_prompts *
                           ((num_prompts // len(example_prompts)) + 1))
    example_prompts = example_prompts[:num_prompts]
    assert len(example_prompts) == num_prompts
    ...
    with vllm_runner(
        ...
        outputs_baseline = (vllm_model.generate_greedy(
            example_prompts, max_tokens) if num_logprobs is None else
                            vllm_model.generate_greedy_logprobs(
