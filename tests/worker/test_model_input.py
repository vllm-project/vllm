...
def test_model_runner_input():
    ...
    assert isinstance(
        received_model_input, ModelInputForGPUWithSamplingMetadata
    )
    ...
    assert (
        received_model_input.input_positions == model_input.input_positions
    ).all()
    assert received_model_input.multi_modal_kwargs is None
    assert (
        received_model_input.multi_modal_kwargs
    ...
    for field in dataclasses.fields(AttentionMetadata):
        assert getattr(
            received_model_input.attn_metadata, field.name, None
...
def test_multi_step_model_runner_input():
    ...
    assert (
        receieved_frozen_input.input_tokens == frozen_model_input.input_tokens
    ).all()
    assert receieved_frozen_input.input_positions is not None
    assert (
        receieved_frozen_input.input_positions
    ...
    assert (
        receieved_frozen_input.lora_requests == frozen_model_input.lora_requests
    )
    ...
    for field in dataclasses.fields(AttentionMetadata):
        assert getattr(
            receieved_frozen_input.attn_metadata, field.name, None
        ) == getattr(attn_metadata, field.name, None)
    # For sampling metadata, only selected_token_indices is copied.
    assert (
        receieved_frozen_input.sampling_metadata.selected_token_indices
        == sampling_metadata.selected_token_indices
    )
    ...
    assert (
        received_model_input.is_first_multi_step
