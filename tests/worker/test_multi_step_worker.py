...
def test_same_output_for_single_step():
    ...
    multi_step_worker.model = worker.model
    ...
    multi_step_execute_model_data = create_execute_model_data(
        ...
        num_preallocated_slots=num_steps - 1)
    ...
    actual_output = multi_step_worker.execute_model(
    ...
    set_random_seed(seed)
    expected_output, = worker.execute_model(single_step_execute_model_data)

    actual_token_ids = [
        output.samples[0].output_token for output in actual_output.outputs
    ]
    actual_logprobs = [
        output.samples[0].logprobs for output in actual_output.outputs
    ]

    expected_token_ids = [
        output.samples[0].output_token for output in expected_output.outputs
    ]
    expected_logprobs = [
...
def test_same_output_for_multi_step():
    ...
    zero_kv_cache(multi_step_worker.cache_engine)
    set_random_seed(seed)
    multi_step_output = multi_step_worker.execute_model(execute_model_data)
    ...
    for _ in multi_step_output:
        ...
        single_step_output.append(worker.execute_model(execute_model_data)[0])
    ...
    for i, (multi_step_tokens, single_step_tokens) in enumerate(
        ...
        print(f'{multi_step_tokens=}')
        print(f'{single_step_tokens=}')
        print(f'equal {multi_step_tokens == single_step_tokens}')
...
def test_handles_empty_batch():
    """Verify an empty input batch (but with finished_request_ids populated)
    ...
    # A dummy output should be returned, with no actual contents.
    assert len(multi_step_output) == 1
    sampler_output = multi_step_output[0]
    assert not sampler_output.probs
    assert not sampler_output.sampled_tokens
    assert not sampler_output.outputs
