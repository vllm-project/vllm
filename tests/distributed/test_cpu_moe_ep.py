...
from vllm.model_executor.layers.fused_moe.router.cpu_router import (  # noqa: E402
    CPURouter,
)
...
def _moe_ep_worker(rank, world_size, tp_size, dp_size, port, dp_port, params, err_q):
    try:
        ...
        with _make_forward_context(dp_rank, dp_size, M_per_dp):
            ...
            expert_out = torch.empty_like(a_gathered)
            ops.fused_experts_cpu_local_skip(
                expert_out,
                a_gathered.clone(),
                pw1,
                pw2,
...
def _make_monolithic_ep_step(
    ...
    def step(hidden_states, router_logits):
        ...
        expert_out = torch.empty_like(hidden_states)
        ops.fused_experts_cpu_local_skip(
            expert_out,
            hidden_states,
            packed_w1,
