...
def nccl_symm_mem_allreduce_worker(local_rank: int, world_size: int):
    ...
    with monkeypatch.context() as m:
        ...
        if get_nccl_mem_pool() is None:
            pytest.skip(
                "NCCL allocator compilation failed ",
...
def test_nccl_symm_mem_allreduce(
    monkeypatch: pytest.MonkeyPatch, world_size
):
    ...
    mp.spawn(
        nccl_symm_mem_allreduce_worker, args=(world_size,), nprocs=world_size
    )
