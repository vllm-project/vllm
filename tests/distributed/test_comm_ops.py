...
def test_isend_tensor_dict_includes_metadata_handle(
    ...
    handles = g.isend_tensor_dict(td, dst=1, is_async=True)
...
def test_isend_tensor_dict_sync_default_does_not_self_retain(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    posted: list[torch.Tensor] = []
    ...
    def fake_send_object(*args: Any, **kwargs: Any) -> None:
        # sync metadata send completes inline; nothing to record here.
        return None

    monkeypatch.setattr(torch.distributed, "isend", fake_isend)
    monkeypatch.setattr(GroupCoordinator, "send_object", fake_send_object)

    g = _make_group_for_unit_test(rank_in_group=0, world_size=2)
    td = {"a": torch.arange(4, dtype=torch.float32, device="cpu")}

    # default (``is_async=False``) takes the sync path: only the tensor is
    # posted with ``isend``; the metadata send is synchronous and no FIFO
    # retention entry is appended.
...
def test_isend_tensor_dict_self_retains_for_fire_and_forget(
    ...
    g.isend_tensor_dict(td, dst=1, is_async=True)
    ...
    g.isend_tensor_dict({"meta": "no-tensors"}, dst=1, is_async=True)
