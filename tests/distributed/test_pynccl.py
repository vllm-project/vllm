...
def all_gather_worker_fn():
    ...
    num_elems = 1000
    tensor = (
        torch.arange(num_elems, dtype=torch.float32, device=device)
        + rank * num_elems
    )
    result = torch.zeros(
        num_elems * world_size, dtype=torch.float32, device=device
    )
...
def send_recv_worker_fn():
    ...
    if pynccl_comm.rank == 0:
        ...
        tensor = torch.empty(16, 1024, 1024, dtype=torch.float32).cuda(
            pynccl_comm.rank
        )

    if pynccl_comm.rank == 0:
        pynccl_comm.send(
            tensor, dst=(pynccl_comm.rank + 1) % pynccl_comm.world_size
        )
    else:
        pynccl_comm.recv(
            tensor, src=(pynccl_comm.rank - 1) % pynccl_comm.world_size
        )
    torch.cuda.synchronize()
...
def broadcast_worker_fn():
    ...
    recv_tensors = [
        torch.empty(
            16, 1024, 1024, dtype=torch.float32, device=pynccl_comm.device
        )
