...
def enable_ray_v2_backend():
    ...
    # When multiprocessing is enabled (default), the LLM engine runs in a
    # forked subprocess and `llm.llm_engine.model_executor` is not directly
    # accessible for test cleanup (`executor.shutdown()`). Disabling it
    # keeps the engine in-process so tests can call shutdown explicitly.
