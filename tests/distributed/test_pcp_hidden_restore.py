...
def _multicast_restore_worker(rank: int, world_size: int, port: int) -> None:
    os.environ.update(
    ...
    try:
        ...
        for dtype in (torch.bfloat16, torch.float16):
            ...
            first_restore_idx = torch.tensor(first_restore, device=device)
            first_reference = _collective_restore(
                first_hidden,
                first_restore_idx,
                nccl_group=nccl_group,
            )
            first_multicast = restorer.restore(
                first_hidden,
                first_restore_idx,
                num_global_tokens=len(first_restore),
            )
            assert torch.equal(first_multicast, first_reference)
            ...
            selected_restore_idx = torch.tensor([0, 6, 7, 2], device=device)
            selected_reference = first_reference[
                torch.tensor([14, 3, 11, 5], device=device)
            ]
            selected_multicast = restorer.restore_selected(
                first_hidden,
                torch.tensor(selected_local_rows[rank], device=device),
                selected_restore_idx,
                num_selected_rows=4,
            )
            assert torch.equal(selected_multicast, selected_reference)

            if rank == 3:
                torch.cuda._sleep(20_000_000)
            saved_first = first_multicast.clone()
            ...
            second_restore_idx = torch.tensor(second_restore, device=device)
            second_reference = _collective_restore(
                second_hidden,
                second_restore_idx,
                nccl_group=nccl_group,
            )
            second_multicast = restorer.restore(
                second_hidden,
                second_restore_idx,
                num_global_tokens=len(second_restore),
            )
            assert torch.equal(second_multicast, second_reference)
            ...
            third_multicast = restorer.restore(
                third_hidden,
                first_restore_idx,
                num_global_tokens=len(first_restore),
            )
            assert torch.equal(third_multicast, third_reference)
...
def test_multicast_hidden_restore_matches_collective_on_four_gpus() -> None:
    from torch._C._autograd import DeviceType
    from torch._C._distributed_c10d import _SymmetricMemory

    world_size = 4
    if not _peer_access_available(world_size):
        pytest.skip("PCP multicast restore requires four peer-accessible GPUs")
    if not _SymmetricMemory.has_multicast_support(DeviceType.CUDA, 0):
        pytest.skip("PCP multicast restore requires CUDA multicast support")
    mp.spawn(
        _multicast_restore_worker,
