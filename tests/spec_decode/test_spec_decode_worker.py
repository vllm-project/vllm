...
@pytest.mark.parametrize("acceptance_sampler_method",
                         ["rejection_sampler", "typical_acceptance_sampler"])
@torch.inference_mode()
def test_batch_expansion_correctly_calls_target_model(k: int, batch_size: int,
                                                      acceptance_sampler_method: str):
    ...
    for prompt, prev_generated, draft_tokens in zip(prompts, prev_output_tokens,
                                                    proposal_token_ids.tolist()):
...
def test_correctly_formats_output(k: int, batch_size: int,
    ...
    for i in range(batch_size):
        ...
        spec_decode_sampler_output[i][-random.randint(minimum_accepted_tokens, k +
                                                      1):] = -1
...
def test_determine_num_available_blocks(available_gpu_blocks: int,
    ...
    target_worker.determine_num_available_blocks.return_value = (available_gpu_blocks,
...
def test_populate_seq_ids_with_bonus_tokens():
    ...
    seq_indexes_with_bonus_tokens = random.sample(range(batch_size),
    ...
    worker._create_output_sampler_list(seq_group_metadata_list=seq_group_metadata_list,
