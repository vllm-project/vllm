...
def test_batch_expansion_correctly_calls_target_model(
    ...
    for prompt, prev_generated, draft_tokens in zip(
        ...
        for i in range(len(draft_tokens) + 1):
            expected_seen_contexts.append(
                prompt + prev_generated + draft_tokens[:i]
            )
    ...
    for seq_id in all_seen_seq_ids:
        ...
        for i in range(k + 1):
            if i >= len(actual_by_step):
                ...
                continue
            assert (
                actual_by_step[i].output_token
                == expected_by_step[i].output_token
            )
...
def test_k_equals_zero(k: int, batch_size: int, acceptance_sampler_method: str):
    ...
    assert out[0].sampled_token_ids is None, (
        "expect gpu tensor references to be None"
    )
...
@pytest.mark.parametrize(
    "available_gpu_blocks", list(range(20)) + [1024, 1024**2]
)
@pytest.mark.parametrize(
    "target_cache_block_size_bytes", [2 * 2 * 4096, 2 * 2 * 8192]
)
@pytest.mark.parametrize("draft_kv_size_bytes", [0, 2 * 2 * 768, 2 * 2 * 4096])
@pytest.mark.skip_global_cleanup
...
def test_populate_seq_ids_with_bonus_tokens():
    ...
    assert (
        worker._request_id_seq_id_mapping == expected_request_id_seq_ids_mapping
    )
...
@pytest.mark.parametrize(
    "batch_composition", ["prefill_only", "decode_only", "mixed"]
)
@torch.inference_mode()
