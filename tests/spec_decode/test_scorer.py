...
def create_proposal(batch_size: int, 
    ...
    proposal_probs = torch.rand((batch_size, propose_len, vocab_size), device=device)
    proposal_token_ids = torch.argmax(proposal_probs, dim=-1)
    proposal_lens = torch.tensor([propose_len] * batch_size, device=device)
    return SpeculativeProposals(proposal_token_ids, proposal_probs, proposal_lens)
...
def test_scoroer(model_name: str, batch_size: int, propose_len: int, device: str) -> None:
    """
    ...
    scorer_worker = create_worker(Worker, 
                                  model_name, 
                                  block_size, 
                                  num_gpu_blocks,
                                  seed)
    ...
    requests = ExecuteModelRequest(seq_group_metadatalist, num_lookahead_slots=propose_len)
