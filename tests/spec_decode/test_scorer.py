...
def create_proposal(
    ...
    return SpeculativeProposals(
        proposal_token_ids, proposal_probs, propose_lens
    )


def assert_score_equal(
    score1: SpeculativeScores, score2: SpeculativeScores
) -> None:
...
def test_scorer(
    ...
    scorer_worker = create_worker(
        Worker, model_name, block_size, num_gpu_blocks, seed
    )
    ...
    if (
        ...
        target_group_metadatalist = [
            seq_group_metadatalist[i]
            for i, p in enumerate(propose_lens)
            if p > 0
        ]
    ...
    batch_expansion_scorer = BatchExpansionTop1Scorer(
        scorer_worker, device, vocab_size
    )
