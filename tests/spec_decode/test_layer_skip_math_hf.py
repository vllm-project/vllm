...
def test_hf_postnorm_at_k_is_deterministic():
    ...
    out1 = hf_postnorm_at_k(TINY_LLAMA_HF, input_ids, k=6, dtype=torch.float32, device="cpu")
    out2 = hf_postnorm_at_k(TINY_LLAMA_HF, input_ids, k=6, dtype=torch.float32, device="cpu")
    assert torch.allclose(out1, out2, rtol=1e-6, atol=1e-7)


def test_hf_postnorm_changes_with_k():
    input_ids = torch.randint(5, 50, (1, 8), dtype=torch.long)
    early = hf_postnorm_at_k(TINY_LLAMA_HF, input_ids, k=3)
    late = hf_postnorm_at_k(TINY_LLAMA_HF, input_ids, k=8)
