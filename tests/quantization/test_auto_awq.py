...
@pytest.mark.skipif(
    ...
    reason="The fused AWQ BI GEMM path only dispatches on SM89 or newer.",
)
@pytest.mark.parametrize("model_id", AWQ_MODELS)
def test_auto_awq_batch_invariant_uses_fused_gemm(
    model_id: str, monkeypatch, dist_init, workspace_init
):
    """Under VLLM_BATCH_INVARIANT on SM89 or newer, AWQ linears should
    dispatch to the fused GEMM (not the legacy dequantize-then-matmul
    fallback)."""
    monkeypatch.setenv("VLLM_ALLOW_INSECURE_SERIALIZATION", "1")
