...
def test_dot_delimited_pattern_matching_prevents_substring_collision(
    quark_sample_config,
):
    """Verify layers.1 does not collide with or match layers.18."""
    cfg_1 = quark_sample_config.get_layer_quant_config_from_name(
        "layers.1.mlp.gate_proj"
    )
    cfg_18 = quark_sample_config.get_layer_quant_config_from_name(
    ...
    cfg_19 = quark_sample_config.get_layer_quant_config_from_name(
...
def test_prefix_stripping_model_dot_symmetry(quark_sample_config):
    """Verify lookups succeed symmetrically whether model. prefix is present
    in layer or pattern."""
    # Pattern was defined without model. prefix ("layers.1.mlp.gate_proj")
    cfg_with_model = quark_sample_config.get_layer_quant_config_from_name(
    ...
    cfg_without_model = quark_sample_config.get_layer_quant_config_from_name(
...
def test_wildcard_pattern_with_prefix_stripping(quark_sample_config):
    ...
    cfg_wildcard = quark_sample_config.get_layer_quant_config_from_name(
    ...
    cfg_wildcard_neg = quark_sample_config.get_layer_quant_config_from_name(
...
def test_should_ignore_layer_quark_utils():
    """Verify dot-delimited layer ignore checks in quark utils for
    MoE parent modules."""
    ignored = ["layers.1.mlp.experts.*.down_proj"]

    # Parent layer 1 experts module must be ignored
    assert should_ignore_layer(
        "model.layers.1.mlp.experts", ignore=ignored, check_children=True
    )
    assert should_ignore_layer(
        "layers.1.mlp.experts", ignore=ignored, check_children=True
    )

    # Parent layer 18 experts module must NOT be ignored
    assert not should_ignore_layer(
        "model.layers.18.mlp.experts", ignore=ignored, check_children=True
    )
    assert not should_ignore_layer(
        "layers.18.mlp.experts", ignore=ignored, check_children=True
    )
