...
def test_load_4bit_bnb_model(
    ...
    validate_generated_texts(
        hf_runner, vllm_runner, example_prompts[:1], model_name, hf_model_kwargs
    )
...
@pytest.mark.parametrize(
    "model_name, description", models_pre_qaunt_4bit_to_test
)
@fork_new_process_for_each_test
def test_load_pre_quant_4bit_bnb_model(
    ...
    validate_generated_texts(
        hf_runner, vllm_runner, example_prompts[:1], model_name
    )
...
@pytest.mark.parametrize(
    "model_name, description", models_pre_quant_8bit_to_test
)
@fork_new_process_for_each_test
...
@pytest.mark.skipif(
    torch.cuda.device_count() < 2, reason="Test requires at least 2 GPUs."
)
@pytest.mark.skipif(
    not is_quant_method_supported("bitsandbytes"),
    reason="bitsandbytes is not supported on this GPU type.",
