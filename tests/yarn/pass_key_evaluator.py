...
class PassKeyEvaluator:
    system = (
        "There is an important pass key hidden inside a lot of irrelevant text. Find this key and memorize it. "
    ...
    def format_prompt(self, garbage_count: int,
                      key_position: int) -> Tuple[str, str, int]:
        """Generates a text file and inserts an execute line at a random position."""
        assert (
            0 <= key_position <= garbage_count
        ), f"key_position={key_position}, garbage_count={garbage_count}"

        garbage_prefix = "".join(self.garbage for _ in range(key_position))
        garbage_suffix = "".join(self.garbage
                                 for _ in range(garbage_count - key_position))
        ...
        return (
            "".join(fragments),
            pass_key,
            self.model.count_tokens(garbage_prefix),
        )
    ...
    def evaluate(self,
        ...
        for position in range(resolution):
            key_position = int(
                round(garbage_count * position / (resolution - 1)))
            prompt, pass_key, prefix_token_count = self.format_prompt(
                garbage_count, key_position)
            outputs = self.model.generate(
                prompt,
                n=n,
                max_new_tokens=self.model.count_tokens(pass_key) + 1,
            )
