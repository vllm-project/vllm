...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ("`--data-parallel-hybrid-lb` requires `--data-parallel-size-local`"),
        ),
        (
            {"nnodes": 2, "data_parallel_backend": "ray"},
            "`--nnodes 2` requires `--data-parallel-backend mp`",
        ),
        (
            {"nnodes": 2, "data_parallel_backend": "mp"},
            "must evenly divide the total world size",
        ),
        (
            {
                "nnodes": 2,
        ...
        (
            ...
            "`--data-parallel-external-lb` requires a data-parallel rank",
        ),
        (
            {
                "data_parallel_size": 2,
            ...
            ("external data-parallel rank requires `--data-parallel-size-local 1`"),
        ),
    ],
)
def test_invalid_data_parallel_launch_options(monkeypatch, kwargs, match):
    monkeypatch.setattr(ModelConfig, "is_moe", property(lambda _: True))

    with pytest.raises(ValueError, match=match):
        EngineArgs(model="facebook/opt-125m", **kwargs).create_engine_config()


def test_valid_multinode_data_parallel_launch_options():
    config = EngineArgs(
    ...
    assert config.parallel_config.data_parallel_rank == 1
    assert config.parallel_config.data_parallel_size_local == 1
