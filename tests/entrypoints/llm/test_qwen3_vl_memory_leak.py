...
WARMUP_ROUNDS = 1
MEASURED_ROUNDS = 4
GPU_GROWTH_THRESHOLD_MIB = 256
CPU_PEAK_GROWTH_THRESHOLD_MIB = 128
MIB = 1024 * 1024
...
def _build_request_batch(image_urls: list[str]) -> list[list[ChatCompletionMessageParam]]:
    return [
...
def test_qwen3_vl_no_memory_leak(image_urls: list[str]) -> None:
    ...
    try:
        ...
        gpu_threshold = GPU_GROWTH_THRESHOLD_MIB * MIB
        ...
        if warmup_cpu_peak is not None and post_warmup_cpu_peak_samples:
            ...
            cpu_threshold = CPU_PEAK_GROWTH_THRESHOLD_MIB * MIB
            assert cpu_peak_growth <= cpu_threshold, (
                ...
                f"post_warmup_ru_maxrss={[_format_mib(x) for x in post_warmup_cpu_peak_samples]}, "
