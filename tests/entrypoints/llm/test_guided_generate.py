...
def test_guided_json_completion(
    sample_json_schema, llm, guided_decoding_backend: str
):
    sampling_params = SamplingParams(
        temperature=1.0,
        max_tokens=1000,
...
def test_guided_complex_json_completion(
    ...
    sampling_params = SamplingParams(
        ...
        max_tokens=1000,
        guided_decoding=GuidedDecodingParams(
            json=sample_complex_json_schema, backend=guided_decoding_backend
        ),
    )
    outputs = llm.generate(
        prompts=[
    ...
    for output in outputs:
        ...
        output_json = json.loads(generated_text)
        jsonschema.validate(
            instance=output_json, schema=sample_complex_json_schema
        )
...
def test_guided_definition_json_completion(
    ...
    sampling_params = SamplingParams(
        ...
        max_tokens=1000,
        guided_decoding=GuidedDecodingParams(
            json=sample_definition_json_schema, backend=guided_decoding_backend
        ),
    )
    outputs = llm.generate(
        prompts=[
    ...
    for output in outputs:
        ...
        output_json = json.loads(generated_text)
        jsonschema.validate(
            instance=output_json, schema=sample_definition_json_schema
        )
...
def test_guided_enum_json_completion(
    ...
    sampling_params = SamplingParams(
        ...
        max_tokens=1000,
        guided_decoding=GuidedDecodingParams(
            json=sample_enum_json_schema, backend=guided_decoding_backend
        ),
    )
    outputs = llm.generate(
        prompts=[
    ...
    for output in outputs:
        ...
        output_json = json.loads(generated_text)
        jsonschema.validate(
            instance=output_json, schema=sample_enum_json_schema
        )
...
def test_guided_choice_completion(
    sample_guided_choice, llm, guided_decoding_backend: str
):
    sampling_params = SamplingParams(
        temperature=0.8,
        top_p=0.95,
...
def test_guided_grammar(
    sample_sql_statements, llm, guided_decoding_backend: str
):
    sampling_params = SamplingParams(
        temperature=0.8,
        top_p=0.95,
        max_tokens=1000,
        guided_decoding=GuidedDecodingParams(
            grammar=sample_sql_statements, backend=guided_decoding_backend
        ),
    )
    outputs = llm.generate(
        prompts=(
...
def test_disable_guided_decoding_fallback(sample_regex, llm):
    sampling_params = SamplingParams(
        ...
        top_p=0.95,
        guided_decoding=GuidedDecodingParams(
            regex=sample_regex, backend="xgrammar:no-fallback"
        ),
    )

    with pytest.raises(
        ValueError, match="xgrammar does not support regex guided decoding"
    ):
        llm.generate(
            prompts="This should fail",
            sampling_params=sampling_params,
...
def test_json_with_any_whitespace_disabled(llm):
    ...
    guided_params = GuidedDecodingParams(
        json=schema, backend=guided_decoding_backend
    )
    sampling_params = SamplingParams(
        max_tokens=2000,
        frequency_penalty=0,
