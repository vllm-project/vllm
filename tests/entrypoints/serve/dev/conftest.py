...
RFC: https://github.com/vllm-project/vllm/issues/52689
...
def _is_expert_parallel(extra_args):
    return any(
        a == "--enable-expert-parallel"
        or a.startswith("--enable-expert-parallel=")
...
def sleep(url, level=1, mode="abort"):
    return requests.post(
        f"{url}/sleep", params={"level": level, "mode": mode}, timeout=120
    ).status_code


def wake(url, tags=None):
    params = {"tags": tags} if tags else {}
    return requests.post(f"{url}/wake_up", params=params, timeout=120).status_code
...
def is_sleeping(url) -> bool:
    return requests.get(f"{url}/is_sleeping", timeout=10).json()["is_sleeping"]
