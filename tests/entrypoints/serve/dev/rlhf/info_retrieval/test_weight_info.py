...
def test_initial_weight_info_schema(server_state):
    _, initial_weight_info = server_state
    assert initial_weight_info == {"weight_version": "default"}


def test_update_weight_version_is_visible(server_url):
...
def test_weight_version_can_be_overwritten(server_url):
...
def test_invalid_version_update_preserves_state(server_url, payload):
...
def test_weight_info_does_not_affect_generation(server_url):
    before = gen(server_url)
    assert ok(before)

    update_weight_version_response(server_url, "generation-check").raise_for_status()
    assert weight_info_response(server_url).json() == {
