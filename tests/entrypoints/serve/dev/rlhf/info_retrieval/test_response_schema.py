...
def test_sleep_response_schema(server_url, level):
...
def test_full_wake_response_schema(server_url):
...
def test_partial_wake_response_tracks_engine_state(server_url):
    sleep_response(server_url, level=1).raise_for_status()
...
def test_is_sleeping_response_schema(server_url):
...
def test_invalid_sleep_parameters_preserve_state(server_url, params, invalid_param):
    assert not is_sleeping(server_url)
