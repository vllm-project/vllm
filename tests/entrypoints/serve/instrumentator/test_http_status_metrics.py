...
from vllm.entrypoints.openai.server_utils import exception_handler
...
async def _validation_exception_handler(
    ...
    return JSONResponse(
        {"error": str(exc)}, status_code=HTTPStatus.BAD_REQUEST
    )
...
async def test_http_requests_total_records_correct_status(
    ...
    assert response.status_code == expected_http_code, (
        f"Expected HTTP {expected_http_code} for {endpoint}, "
