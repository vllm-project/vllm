...
This is the GPU version of test_websocket_real.py (CPU version).

Run:
    pytest tests/entrypoints/openai/responses/test_websocket_gpu.py -v -s
...
# GPU model - Qwen2-0.5B from HuggingFace (has chat template)
MODEL_NAME = "Qwen/Qwen2-0.5B-Instruct"
SERVED_MODEL_NAME = "test-model-gpu"
...
def server():
    """Start vLLM server with GPU model."""
    from .conftest import BASE_TEST_ENV

    # GPU-specific configuration
    args = [
        # Use bfloat16 for faster inference and less memory
        "--dtype",
        "bfloat16",
        # Adequate context length for testing
        "--max-model-len",
        "2048",
        # Enforce eager mode for testing (easier debugging)
        "--enforce-eager",
        # Name served model for WebSocket endpoint
        "--served-model-name",
        SERVED_MODEL_NAME,
    ]
...
async def test_basic_streaming(server):
    """Basic streaming works over WebSocket with GPU: connect, send
    response.create, receive streaming events ending with response.completed."""
...
async def test_streaming_text_consistency(server):
    ...
    ws_url = _get_ws_url(server)
    async with websockets.connect(ws_url) as ws:
        await _send(ws, _make_create_event(max_output_tokens=30))
        events = await _collect_until_done(ws)
