...
def zephyr_lora_added_tokens_files(zephyr_lora_files):
    ...
    added = tokenizer.add_tokens(
        ["vllm1", "vllm2", "vllm3"], special_tokens=True
    )
    assert added == 3
...
@pytest.fixture(
    scope="module", params=["", "--disable-frontend-multiprocessing"]
)
...
async def test_logits_bias(client: openai.AsyncOpenAI):
    ...
    response_tokens = tokenizer(
        completion.choices[0].text, add_special_tokens=False
    )["input_ids"]
...
async def test_guided_regex_completion(
    ...
    for i in range(3):
        assert (
            re.fullmatch(sample_regex, completion.choices[i].text) is not None
        )
...
@pytest.mark.asyncio
async def test_guided_grammar(
    client: openai.AsyncOpenAI, sample_sql_statements
):
    completion = await client.completions.create(
        model=MODEL_NAME,
        prompt=(
...
async def test_guided_decoding_type_error(
    ...
    with pytest.raises(openai.BadRequestError):
        _ = await client.completions.create(
            ...
            extra_body=dict(
                guided_regex=sample_regex, guided_json=sample_json_schema
            ),
        )
