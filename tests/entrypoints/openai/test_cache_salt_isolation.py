...
def test_empty_salt_does_not_match_unsalted_prefix(pooling_server):
    ...
    ``generate_block_hash_extra_keys`` tests the salt for truthiness rather
    than for ``None``, so an empty string would produce no extra key and land
    in the unsalted namespace rather than being rejected — which a client
    emitting ``""`` by mistake would do silently.

    Two independent guards now prevent that: ``min_length=1`` on the pooling
    field (#54353) and the ``check_cache_salt_support`` validator added in
    #51444, which routes the value through ``validate_cache_salt`` and raises
    on an empty string. This arm pins that rejection so neither guard can be
    removed without a test failing.

    Unlike the arms above, this one does not depend on prefix caching: the
    rejection is a validation error, so it is asserted directly rather than
    paired with a reuse control. The reuse check that follows confirms the
    request is refused before it can reach the cache at all.
    """
