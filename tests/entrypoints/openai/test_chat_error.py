...
@pytest.mark.asyncio
async def test_chat_error_response_format_json_schema_no_schema() -> None:
    """Test that missing schema in response_format.json_schema
    returns 400 BAD_REQUEST
    """
    mock_engine = MagicMock(spec=AsyncLLM)
    mock_engine.errored = False
    ...
    assert (
        response.error.message
        == "response_format.json_schema object must contain a 'schema' field when "
