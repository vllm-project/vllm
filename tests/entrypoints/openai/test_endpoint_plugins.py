...
Includes a worked in repo example plugin (`_DummyAdminEndpointPlugin`)
exercising both `EndpointPlugin` hooks against a fake `EngineClient`, unit
tests for the `load_endpoint_plugins` gating matrix and an e2e test
that drives a real HTTP request through the plugin's route.
"""
...
class _DummyAdminEndpointPlugin:
    """Worked example: reports scheduler config via `collective_rpc`.

    Demonstrates the full contract: `attach_router` registers the route at
    Phase A (`build_app`) and `init_state` stashes the `EngineClient` the route
    handler needs at Phase B (`init_app_state`).
    """
...
def test_dummy_plugin_satisfies_protocol():
    assert isinstance(_DummyAdminEndpointPlugin(), EndpointPlugin)
...
def test_no_plugins_loaded_when_allowlist_unset(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        ...
        _fake_loader({"dummy_admin_endpoint_plugin": _DummyAdminEndpointPlugin}),
    )
...
def test_plugin_loaded_when_allowlisted_and_task_matches(
    ...
    assert isinstance(plugins[0], _DummyAdminEndpointPlugin)


def test_plugin_skipped_when_required_tasks_miss(monkeypatch: pytest.MonkeyPatch):
    class _GenerateOnlyPlugin(_DummyAdminEndpointPlugin):
        ...
        _fake_loader(
            {
                ...
                "dummy_admin_endpoint_plugin": _DummyAdminEndpointPlugin,
            }
        ),
    )
