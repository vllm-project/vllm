...
@pytest.mark.asyncio
async def test_chat_stop_token_ids_out_of_vocab(client):
    """Test that out-of-vocab stop_token_ids are rejected with a 400."""
    vocab_size = get_vocab_size(MODEL_NAME)
    ...
    with pytest.raises(openai.BadRequestError) as excinfo:
        await client.chat.completions.create(
            model=MODEL_NAME,
            messages=[{"role": "user", "content": "Testing stop token ids"}],
            max_tokens=5,
...
@pytest.mark.asyncio
async def test_chat_stop_token_ids_negative(client):
    """Test that negative stop_token_ids are rejected with a 400."""
    with pytest.raises(openai.BadRequestError) as excinfo:
        await client.chat.completions.create(
            model=MODEL_NAME,
            messages=[{"role": "user", "content": "Testing stop token ids"}],
            max_tokens=5,
            extra_body={"stop_token_ids": [-1]},
        )
