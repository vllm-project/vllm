...
@pytest.mark.parametrize(
    "tool_choice",
    ["required", {"type": "function", "function": {"name": "get_weather_report"}}],
)
async def test_tool_schema_with_defs_and_refs(
    client: openai.AsyncOpenAI, tool_choice: str | dict
):
    """A ``$ref``-ed nested argument must decode as an object, not a string.

    Tool schemas generated by Pydantic v2 put nested models behind
    ``$defs``/``$ref``. Both the structured output schema derived from the
    tools and the parsed tool call have to keep those definitions intact,
    otherwise ``json.loads(tool_call.function.arguments)`` hands the caller a
    JSON string where an object is expected. See issue #46924.
    """
    chat_completion = await client.chat.completions.create(
    ...
    assert isinstance(arguments["options"], dict)
    assert arguments["options"]["unit"] in ("celsius", "fahrenheit")
    assert isinstance(arguments["options"]["include_forecast"], bool)
