...
@pytest.mark.asyncio
async def test_chat_completion_n_with_different_seeds(
    n_parameter_client,
):
    """Test that n parameter with seed produces deterministic outputs."""
    messages = [
    ...
    # First call
    chat_completion1 = await n_parameter_client.chat.completions.create(**common_params)

    # Second call with the same seed
    chat_completion2 = await n_parameter_client.chat.completions.create(**common_params)

    assert len(chat_completion1.choices) == 2
    assert len(chat_completion2.choices) == 2

    # Verify that both runs with the same seed produce identical results
    content1 = [c.message.content for c in chat_completion1.choices]
    content2 = [c.message.content for c in chat_completion2.choices]
    assert content1 == content2, (
        "Completions with the same seed should be deterministic"
    )

    # Verify that the two choices within a single run are different.
    # This is highly likely with a non-zero temperature.
    assert content1[0] is not None
    assert content1[1] is not None
    assert content1[0] != content1[1], "The two choices should be different"
