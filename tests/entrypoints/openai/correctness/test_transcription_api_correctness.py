...
async def transcribe_audio(client, tokenizer, y, sr, language):
...
async def bound_transcribe(model_name, sem, client, audio, reference, language):
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    # Use semaphore to limit concurrent requests.
    async with sem:
        result = await transcribe_audio(client, tokenizer, *audio, language)
...
async def process_dataset(model, client, data, concurrent_request, language):
    sem = asyncio.Semaphore(concurrent_request)

    # Warmup call as the first `librosa.load` server-side is quite slow.
    audio, sr = data[0]["audio"]["array"], data[0]["audio"]["sampling_rate"]
    _ = await bound_transcribe(model, sem, client, (audio, sr), "", language)

    tasks: List[asyncio.Task] = []
    for sample in data:
        audio, sr = sample["audio"]["array"], sample["audio"]["sampling_rate"]
        task = asyncio.create_task(
            bound_transcribe(model, sem, client, (audio, sr), sample["text"], language)
        )
        tasks.append(task)
...
def run_evaluation(
    ...
    results = asyncio.run(
        process_dataset(model, client, dataset, max_concurrent_reqs, language=language)
    )
...
def test_wer_correctness(
    ...
    with RemoteOpenAIServer(model_name, ["--enforce-eager"]) as remote_server:
        ...
        wer = run_evaluation(
            model_name, client, dataset, max_concurrent_request, n_examples=n_examples
        )
        if expected_wer:
...
def test_wer_correctness_with_lang_detection(
    model_name,
    dataset_repo,
    expected_wer,
    language,
    ...
    with RemoteOpenAIServer(model_name, ["--enforce-eager"]) as remote_server:
        ...
        if expected_wer:
            # increase tolerance for language detection
            torch.testing.assert_close(wer, expected_wer, atol=0.3, rtol=0.05)
