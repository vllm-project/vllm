...
@pytest.mark.asyncio
async def test_client_data_methods_use_timeouts(monkeypatch, dummy_server, client):
    with monkeypatch.context() as m:
...
@pytest.mark.asyncio
async def test_client_data_methods_reraise_exceptions(monkeypatch, dummy_server, client):
    with monkeypatch.context() as m:
...
@pytest.mark.asyncio
async def test_client_health_check_times_out(monkeypatch, dummy_server, client: AsyncEngineRPCClient):
    with monkeypatch.context() as m:
        ...
        client_task = asyncio.get_running_loop().create_task(client.check_health())
        done, pending = await asyncio.wait([client_task], timeout=0.05)
        assert len(done) > 0
        with pytest.raises(TimeoutError):
            list(done)[0].result()