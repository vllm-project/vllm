...
def client(app, mock_async_llm):
    ...
    with patch.object(api_router, "_require_async_llm", return_value=mock_async_llm):
...
class TestPauseStepEndpoint:
    ...
    def test_pause_step_default_with_barrier(self, client, mock_engine_core):
        ...
        mock_engine_core._call_utility_async.assert_any_call(
            "run_until_target_step_count", 43, engine=b"\x00\x00"
        )
    ...
    def test_pause_step_no_barrier(self, client, mock_engine_core):
        ...
        for call in mock_engine_core._call_utility_async.call_args_list:
    ...
    def test_pause_step_custom_barrier(self, client, mock_engine_core):
        ...
        mock_engine_core._call_utility_async.assert_any_call(
            "run_until_target_step_count", 100, engine=b"\x00\x00"
        )
    ...
    def test_pause_step_barrier_overrides_default(self, client, mock_engine_core):
        ...
        mock_engine_core._call_utility_async.assert_any_call(
            "run_until_target_step_count", 50, engine=b"\x00\x00"
        )

    def test_pause_step_multi_engine_returns_first_step(self, client, mock_engine_core):
        """
        When there are multiple engines, /pause/step uses the first engine's step.
        """
        mock_engine_core.core_engines = [b"\x00\x00", b"\x01\x00"]

        async def _call_utility_async(method, *args, engine=None, **kwargs):
            if method == "pause":
                return 7 if engine == b"\x00\x00" else 9
            if method == "get_step_counter":
                return 100
            if method == "run_until_target_step_count":
                return None
        ...
        mock_engine_core._call_utility_async = AsyncMock(
            side_effect=_call_utility_async
        )
