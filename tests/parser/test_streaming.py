...
def stream_text(parser, tokenizer, text, request):
    token_ids = tokenizer.encode(text, add_special_tokens=False)
    results: list[DeltaMessage | None] = []
    for tid in token_ids:
        delta_text = tokenizer.decode([tid])
        result = parser.parse_delta(delta_text, [tid], request)
...
def test_parse_delta_neither_parser(tokenizer, request_obj):
    ...
    results = stream_text(parser, tokenizer, MODEL_OUTPUT, request_obj)
