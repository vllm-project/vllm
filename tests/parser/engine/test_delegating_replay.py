...
def _discover_pairings() -> list[_PairingInfo]:
    ...
    for engine_cls, adapters in engines.items():
        ...
        if cfg.name in ("inkling", "llama_json"):
            ...
            # terminal matching; llama_json has no TOOL_END terminal because
            # calls close when the envelope JSON balances. Combined-parser
            # replay lives in test_inkling.py / test_llama_json.py.
            continue
