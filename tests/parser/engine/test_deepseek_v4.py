...
class TestMissingToolCallsWrapper:
    ...
    def declare_terminal(self, mock_request):
        mock_request.tools = [_make_tool("terminal", {"command": {"type": "string"}})]
...
class TestMalformedWrapperRecovery:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                _recovery_invoke(name="not_declared") + DSML_TOOL_END,
                [_recovery_tool()],
                "auto",
            ),
            (_recovery_invoke() + DSML_TOOL_END, [], "auto"),
            (_recovery_invoke() + DSML_TOOL_END, [_recovery_tool()], "none"),
            (
                "Docs quote " + DSML_INVOKE_PREFIX + "get_wea",
                [_recovery_tool()],
                "auto",
            ),
            (
    ...
    def test_invalid_or_truncated_orphan_invoke_stays_content(
        self, mock_tokenizer, mock_request, text, tools, tool_choice
    ):
        mock_request.tools = tools
