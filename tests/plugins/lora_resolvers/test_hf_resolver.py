...
REPO_DOWNLOAD_DIR = os.path.join(HF_HUB_CACHE,
...
async def test_hf_resolver_with_direct_path():
    ...
    lora_request = await hf_resolver.resolve_lora(LORA_REPO_MODEL_NAME,
                                                  LORA_REPO)
...
async def test_hf_resolver_with_nested_paths():
    ...
    lora_request = await hf_resolver.resolve_lora(LORA_LIB_MODEL_NAME,
                                                  LORA_NAME)
...
async def test_missing_adapter():
    ...
    missing_lora_request = await hf_resolver.resolve_lora(
        LORA_LIB_MODEL_NAME, "foobar")
...
async def test_nonlora_adapter():
    ...
    readme_request = await hf_resolver.resolve_lora(LORA_LIB_MODEL_NAME,
                                                    NON_LORA_SUBPATH)
