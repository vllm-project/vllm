...
from vllm.sequence import SampleLogprob, Sequence, SequenceGroup, SequenceStatus
...
def test_append_slot_single_seq():
    ...
    for i in range(block_size):
        token_id = i + 5
        prompt.append_token_id(token_id, {token_id: SampleLogprob(0.0)})
...
def test_append_multiple_slot_single_seq(prompt_len: int,
    ...
    prompt.append_token_ids(
        new_token_ids,
        [{token_id: SampleLogprob(logprob=0.0)
...
def test_append_slot_cow():
    ...
    child = prompt.fork(2)
    token_id = 4
    child.append_token_id(token_id, {token_id: SampleLogprob(0.0)})
