...
def test_allocate_naive(block_size: int, sequence_len: int):
    ...
    num_blocks_per_alloc = len(list(token_ids.chunks(block_size)))
...
def test_allocate_prefix_caching(block_size: int, sequence_len: int):
    ...
    token_ids = TokenIds(range(sequence_len))
    chunked_tokens = list(token_ids.chunks(block_size))
...
def test_append_token_ids_allocation(block_size: int, sequence_len: int,
    ...
    num_expected_blocks_before_append = len(list(token_ids.chunks(block_size)))
    num_expected_appended_blocks = len(
        list((token_ids + token_ids_to_append
              ).chunks(block_size))) - num_expected_blocks_before_append
...
def test_append_token_ids_correct_content(block_size: int, sequence_len: int,
    ...
    for append in token_ids_to_append.chunks(append_size):
