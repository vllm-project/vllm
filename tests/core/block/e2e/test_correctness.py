...
@pytest.mark.parametrize(
    "test_llm_kwargs",
    [{"preemption_mode": "swap"}, {"preemption_mode": "recompute"}],
)
...
def test_block_manager_with_preemption(
    ...
    for expected_token_ids, actual_token_ids in zip(
        baseline_token_ids, test_token_ids
    ):
...
@pytest.mark.parametrize("per_test_common_llm_kwargs", [{}])
@pytest.mark.parametrize(
    "baseline_llm_kwargs", [{"enable_prefix_caching": False}]
)
@pytest.mark.parametrize(
    "test_llm_kwargs",
    [
...
@pytest.mark.parametrize("seed", [1])
def test_auto_prefix_caching_after_evition_start(
