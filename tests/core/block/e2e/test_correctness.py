...
def test_lookahead_greedy_equality(baseline_llm_generator, test_llm_generator,
                                   batch_size):
...
@pytest.mark.parametrize("seed", [1])
def test_block_manager_prefix_caching_enabled(baseline_llm_generator,
                                              test_llm_generator, batch_size):
    """Verify block manager produces same outputs with prefix caching enabled.
...
@pytest.mark.parametrize("batch_size", [10])
@pytest.mark.parametrize("seed", [1])
def test_auto_prefix_caching(baseline_llm_generator, test_llm_generator,
                             batch_size):
    """Verify block manager v2 with auto prefix caching enabled produces same
    outputs as auto prefix caching disabled.
