...
class TestNaiveBlockAllocator:
    ...
    def test_allocate_ooms(
        allocate_type: str, num_blocks: int, block_size: int
    ):
        allocator = NaiveBlockAllocator(
    ...
    def test_naive_block_get_num_full_blocks_touched(num_blocks, block_size):
        ...
        assert (
            allocator_dst.get_num_full_blocks_touched(src_blocks)
            == num_blocks - 1
        )
        ...
        assert (
            allocator_dst.get_num_full_blocks_touched(src_blocks) == num_blocks
        )
