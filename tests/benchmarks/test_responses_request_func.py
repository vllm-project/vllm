...
def test_latency_accounting_matches_the_other_endpoints():
    """`latency - ttft` must equal `sum(itl)`.

    `serve.py` derives TPOT as `(latency - ttft) / (output_len - 1)`, which only
    means average inter-token latency when that identity holds. #55508 fixed the
    same accounting on the chat and audio backends; the trailing events after
    the last token must not extend the request.
    """
    output = run(
        [
            PREAMBLE,
    ...
    # The identity is exact, so no wall-clock margin is involved. The .done
    # family and the GAP_FINALIZE wait before response.completed sit outside
    # the request, exactly as the trailing usage chunk does on chat.
    assert output.latency - output.ttft == pytest.approx(sum(output.itl), abs=1e-9)
