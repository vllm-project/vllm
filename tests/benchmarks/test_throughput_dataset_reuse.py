...
"""Tests for routing ``throughput.get_requests`` through the shared
``datasets.get_samples`` so bench throughput and bench serve share one dataset
dispatch (see issue #50838).
"""
...
def _hf_mm_args(backend: str) -> SimpleNamespace:
    ...
    # (MMVU itself is multimodal in content but does not set IS_MULTIMODAL, so
    # it does not exercise this gate.)
