...
class MockModelConfig:
    ...
    def __getattr__(self, name):
        ...
        if isinstance(attr, types.FunctionType):
            # Bind the function to 'self' so it acts like a method of this instance.
            # This creates a bound method where 'self' is automatically passed as the first arg.
...
def test_base_attention_config_parser_without_gqa():
    """Test BaseAttentionConfigParser defaults to MHA when num_key_value_heads not specified."""
    hf_config = Qwen3Config(
        hidden_size=4096,
        num_attention_heads=32,
...
def test_base_ffn_config_parser_dense():
    ...
    assert result.num_experts_per_tok is None
...
def test_moe_layer_freq_parser():
    ...
    expected_moe_layers = len([
        ...
        if layer >= 6 and layer % 3 == 0
    ])
    assert expected_moe_layers == 8
    assert result.num_moe_layers == expected_moe_layers


# Tests for component metrics using inherent logic properties
...
def test_moe_expert_activation_proportional_scaling():
    ...
    # The difference between double and base should equal one additional expert's contribution
    one_expert_diff = double_flops - base_flops

    # The difference between triple and base should equal two additional experts' contribution
...
def test_quantization_config_parser_mxfp4():
    ...
    class MockQuantConfig:
        def get_name(self): return "mxfp4"
...
def test_moe_per_gpu_with_expert_parallelism():
    ...
    if 'routed_up_gate_weights' in per_gpu_read_breakdown.model_dump():
...
def test_moe_per_gpu_expert_activation_accounting():
    ...
    if 'routed_up_gate_weights' in small_read.model_dump():
...
def test_model_metrics_per_gpu_aggregation():
    ...
    # With TP=2 and PP=2, the ratio depends on which parallelism applies to which component
    # But we can verify that global is reasonably larger
