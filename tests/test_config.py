...
@pytest.mark.parametrize("mode", ["none", "align", "all"])
...
@pytest.mark.parametrize("use_replayssm", [False, True])
@pytest.mark.parametrize(
    "backend", [MambaBackendEnum.TRITON, MambaBackendEnum.FLASHINFER]
)
def test_v2_all_cache_requires_flashinfer_replayssm(use_replayssm, backend):
    config = SimpleNamespace(
        model_config=None,
        ...
        cache_config=SimpleNamespace(
            mamba_cache_mode="all", use_replayssm=use_replayssm
        ),
        mamba_config=SimpleNamespace(backend=backend),
    )
    unsupported = VllmConfig._get_v2_model_runner_unsupported_features(config)
    supported = use_replayssm and backend == MambaBackendEnum.FLASHINFER
    assert ("mamba cache mode 'all'" not in unsupported) == supported
