...
def test_traces(
    ...
    with monkeypatch.context() as m:
        ...
        if not trace_service.evt.wait(timeout):
            raise TimeoutError(f"The fake trace service didn't receive a trace within "
        ...
        assert attributes.get(
            SpanAttributes.GEN_AI_REQUEST_TEMPERATURE) == sampling_params.temperature
        assert attributes.get(
        ...
        assert attributes.get(
            SpanAttributes.GEN_AI_LATENCY_TIME_IN_SCHEDULER) == metrics.scheduler_time
