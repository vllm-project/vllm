    ...
    env.update(
        {
            ...
            "FAKE_METADATA": str(tmp_path / "metadata"),
            "FAKE_SMOKE_REF": "rocm/vllm-ci@sha256:" + "a" * 64,
...
def test_rocm_smoke_marker_still_requires_exact_image_validation(
    tmp_path: Path,
) -> None:
    ...
    assert "verified inside BuildKit" not in result.stdout
    assert env["FAKE_SMOKE_REF"] in docker_called.read_text().splitlines()
    assert Path(env["FAKE_METADATA"]).read_text().splitlines() == [
        f"rocm-ci-image-smoked-ref={env['FAKE_SMOKE_REF']}",
        "rocm-ci-image-smoked=1",
    ]
    assert marker.exists()


def test_rocm_smoke_rejects_image_outside_this_build_handoff(tmp_path: Path) -> None:
    env, marker, _, docker_called = prepare_rocm_smoke_test(
        tmp_path,
        marker_id="previous-build",
        build_id="current-build",
    )
    env["VLLM_CI_SMOKE_IMAGE"] = "rocm/vllm-ci@sha256:" + "b" * 64
    ...
    assert "does not match its build-scoped handoff" in result.stderr
    ...
    env.update(
        {
            ...
            "VLLM_CI_SMOKE_IMAGE": "rocm/vllm-ci@sha256:" + "b" * 64,
        }
    )
