...
@pytest.mark.parametrize(
    "architecture",
    ["KimiK3ForConditionalGeneration", "KimiK3MTPModel", "KimiLinearForCausalLM"],
)
def test_kimi_only_accepts_flashinfer_nvfp4_backend(architecture):
    validate_flashinfer_moe_ep_model("flashinfer_moe_ep_mega_cutedsl", [architecture])
    with pytest.raises(ValueError, match="only supported for models"):
        validate_flashinfer_moe_ep_model(
            "flashinfer_moe_ep_mega_deep_gemm", [architecture]
        )
