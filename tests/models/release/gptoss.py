...
async def test_online_serving_v1_chat_completions(
    ...
    for i, messages in enumerate(test_messages_list, 1):
        ...
        assert len(response_text) > 0, "Message should have content or reasoning"
        assert choice.finish_reason is not None, "Finish reason should be set"
    ...
    first_response_text = (
        assistant_message.content or getattr(assistant_message, "reasoning", None) or ""
    )
    assert len(first_response_text) > 0, (
        "First response should have content or reasoning"
    )
    ...
    assert len(combined) > 0, "Streamed content or reasoning should not be empty"
