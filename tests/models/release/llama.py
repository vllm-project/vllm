...
async def test_online_serving_v1_completions(
    ...
    """
    Test online serving via /v1/completions endpoint for Llama 3.3 70B Instruct.
...
async def test_online_serving_v1_chat_completions(
    ...
    """
    Test online serving via /v1/chat/completions endpoint for Llama 3.3 70B Instruct.
    ...
    for i, messages in enumerate(test_messages_list, 1):
        ...
        assert len(response_text) > 0, "Message should have content"
        assert choice.finish_reason is not None, "Finish reason should be set"
    ...
    first_response_text = assistant_message.content or ""
    assert len(first_response_text) > 0, "First response should have content"

    # Add assistant response and continue conversation
    conversation_messages.append({"role": "assistant", "content": first_response_text})
    ...
    second_response_text = second_response.choices[0].message.content or ""
    assert len(second_response_text) > 0, "Second response should have content"
    ...
    assert finish_reason_count == 1, "Finish reason should appear exactly once"
    streamed_text = "".join(chunks)
    assert len(streamed_text) > 0, "Streamed content should not be empty"

    print(f"Streamed text: {streamed_text[:100]}...")
    print(f"Received {len(chunks)} content chunks")
...
async def test_online_serving_concurrent_requests(
    ...
    for result in all_results:
        text = result["text"] or ""
        assert len(text) > 0, f"Request {result['id']} should have non-empty text"
...
def test_accuracy_gsm8k(server: RemoteOpenAIServer, model_name: str) -> None:
    """
    Measure accuracy via GSM8K evaluation against Llama 3.3 70B Instruct.
