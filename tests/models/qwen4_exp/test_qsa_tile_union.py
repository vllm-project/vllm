...
class Case:
    ...
    def __init__(
        ...
        # Random, not constant: sigmoid of a constant gate is a uniform scale
        # both kernels would apply identically, so a constant gate cannot tell
        # a correct gate indexing from a transposed or broadcast one.
        self.output_gate = torch.randn(
            num_rows, NUM_QUERY_HEADS, HEAD_DIM, device=device, generator=g
        ).to(torch.bfloat16)
