...
MODELS = ["facebook/opt-125m", "/mnt/local_storage/llama-7b"]
...
def test_models(
    ...
    vllm_tracer = LLMExecutionTracer(
        vllm_model.model.llm_engine.workers[0].model, is_hf=False)
    ...
    for request_id in range(len(example_prompts)):
        ...
        for iteration in range(max_tokens):
            for layer in range(len(vllm_captured)):
                hf_input, hf_output = hf_captured[request_id][layer][iteration]
                vllm_input, vllm_output_padded = vllm_captured[request_id][
                    layer][iteration]
