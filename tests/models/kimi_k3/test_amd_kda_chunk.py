...
def _on_fused_kda_chunk_arch() -> bool:
...
def test_empty_sequence_preserves_initial_state() -> None:
    """T == 0 must not commit uninitialized fp32 into the SSM cache.

    The last-group block is the only writer of ``ht``. An empty sequence has
    ``nt_all == 0``, so every group takes the early return and would otherwise
    leave ``torch.empty`` untouched. The host path seeds ``final_state`` from
    ``initial_state`` (and the kernel copies ``h0`` as a backstop).
    """
    _requires_kernel()

    seqlens = [64, 0, 128]
    inp = _inputs(seqlens, seed=3)
    o_fused, state_fused = _run(inp, use_fused=True)

    assert state_fused is not None
    torch.testing.assert_close(state_fused[1], inp["h0"][1], rtol=0, atol=0)

    o_ref, state_ref = _run(inp, use_fused=False)
    torch.testing.assert_close(o_fused.float(), o_ref.float(), rtol=2e-2, atol=2e-2)
    torch.testing.assert_close(state_fused[0], state_ref[0], rtol=1e-3, atol=1e-3)
    torch.testing.assert_close(state_fused[2], state_ref[2], rtol=1e-3, atol=1e-3)


def test_explicit_fused_errors_on_precondition_failure() -> None:
    """``--kda-prefill-backend=hipkda`` must not silently fall back to Triton."""
    from vllm.models.kimi_k3.amd.ops.kda_prefill import chunk_kda_prefill

    inp = _inputs([64], seed=1)
    with pytest.raises(RuntimeError, match="Fused KDA chunk prefill was requested"):
        chunk_kda_prefill(
            q=inp["q"],
            k=inp["k"],
            v=inp["v"],
            ...
            require_fused_chunk=True,
            lower_bound=None,
        )


def test_chunk_groups_independent_of_batch_size(monkeypatch) -> None:
    """G is a function of sequence length and head count, not num_seqs.

    A batch-dependent split would make prefill numerics vary with scheduling,
    which breaks MLPerf / agentic-CI reproducibility.
    """
    import vllm.models.kimi_k3.amd.ops.kda_chunk as kda_chunk

    monkeypatch.setattr(kda_chunk, "_num_cus", lambda *_: 256)
    kda_chunk._chunk_groups.cache_clear()
    g = kda_chunk._chunk_groups(64, 12)
    assert g == kda_chunk._chunk_groups(64, 12)
    assert kda_chunk._chunk_groups(1, 12) == 1
    assert g >= 1
