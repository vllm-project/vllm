        ...
        for idx, (output_id_0,
            ...
            if output_id_0 != output_id_1:
                ...
                assert output_id_0 in logprobs_1[idx], (
                    f"{name_0} token '{output_id_0}' not in "
                    f"[{list(logprobs_1[idx].keys())}]"
                    f"\nprompt index {prompt_idx}, token index {idx}:"
                    f"\n{name_0}:\t{output_str_0!r}"
                    f"\n{name_1}:\t{output_str_1!r}")
                assert output_id_1 in logprobs_0[idx], (
                    f"{name_1} token '{output_id_1}' not in "
                    f"[{list(logprobs_0[idx].keys())}]"
                    f"\nprompt index {prompt_idx}, token index {idx}:"
                    f"\n{name_0}:\t{output_str_0!r}"
                    f"\n{name_1}:\t{output_str_1!r}")
