...
from vllm.pooling_metadata import PoolingMetadata
...
class TestBasicFunctionality:
    ...
    def test_text_only_embeddings(self, model):
        ...
        prompts = [
            ...
            TextPrompt(prompt="Passage: Machine learning is a subset of artificial intelligence.")
        ]
    ...
    def test_image_embeddings(self, model):
        ...
        # Check dimension
        assert embedding.shape[0] == model.llm_engine.model_config.hf_config.hidden_size
    ...
    def test_concurrent_requests(self, model):
        ...
        def process_request(thread_id):
            ...
            for i in range(requests_per_thread):
                prompt = TextPrompt(prompt=f"Query from thread {thread_id}, request {i}")
                outputs = model.encode([prompt])
                embedding = extract_embeddings(outputs[0])
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [executor.submit(process_request, i) for i in range(num_threads)]
...
class TestMemoryManagement:
    ...
    def test_memory_cleanup(self, model):
        ...
        for _ in range(5):
            prompts = [
                TextPrompt(prompt=f"Query: test {i}")
                for i in range(10)
            ]
            outputs = model.encode(prompts)
            del outputs
...
class TestPerformance:
    ...
    def test_pooling_performance(self, model):
        ...
        # Create test prompts
        text_prompts = [TextPrompt(prompt=f"Query: test {i}") for i in range(10)]
