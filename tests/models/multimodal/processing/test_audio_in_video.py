...
def test_audio_in_video_cache_correctness(model_id: str, num_videos: int) -> None:
    ...
    baseline_processor = MULTIMODAL_REGISTRY.create_processor(ctx.model_config)
    ...
    cached_processor = MULTIMODAL_REGISTRY.create_processor(ctx.model_config)
    ...
    baseline_ids = run(baseline_processor, cache=None)
    ...
    first_ids = run(cached_processor, cache=sender_cache)
    ...
    second_ids = run(cached_processor, cache=sender_cache)
