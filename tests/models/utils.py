...
def check_logprobs_close(
    *,
    ...
    cross_scorer: (
        Callable[[int, int, int, int, int], tuple[float, float] | None] | None
    ) = None,
    cross_logprob_tol: float = 0.0,
) -> None:
    ...
    * `always_check_logprobs == False`: highest-logprob token ids are
    ...
    Args:
      ...
      cross_scorer: Optional callable used to rescue a divergence whose top-k
                    sets are disjoint. Called as (prompt_idx, idx_0, idx_1,
                    token_id_0, token_id_1); returns
                    (log P_m1(token_id_0), log P_m0(token_id_1)), each
                    conditioned on the prefix the two sequences share up to the
                    divergence, or None if unscorable. When None (the default),
                    the top-k membership checks are unconditional, i.e.
                    behaviour is unchanged.
      cross_logprob_tol: Maximum gap, in nats, between the two cross-scored
                         conditional logprobs for which a divergence is
                         excused. Only consulted when `cross_scorer` is
                         supplied. 0.0 excuses only exact ties. Assumes the two
                         token streams are aligned, i.e.
                         num_outputs_0_skip_tokens == 0.
    """
    assert len(outputs_0_lst) == len(outputs_1_lst)
    ...
    for prompt_idx, (outputs_0, outputs_1) in enumerate(
        ...
        for idx, (output_id_0, output_id_1) in enumerate(
            ...
            if is_tok_mismatch or always_check_logprobs:
                ...
                # Ask each model how likely the *other* model's token was, on
                # the prefix they share. If the two answers agree, neither
                # model prefers its own pick and the divergence is a tie.
                rescued = False
                if fails_inclusion_test and cross_scorer is not None:
                    ...
                    if scores is not None:
                        tok_0_in_1, tok_1_in_0 = scores
                        gap = abs(tok_0_in_1 - tok_1_in_0)
                        rescued = gap <= cross_logprob_tol
                        fail_msg += (
                            f"\ncross-scored:"
                            f"\tlogP_{name_1}({name_0} tok)={tok_0_in_1:.6f}"
                            f"\tlogP_{name_0}({name_1} tok)={tok_1_in_0:.6f}"
                            f"\tgap={gap:.6f}"
                            f"\ttol={cross_logprob_tol}"
                        )
