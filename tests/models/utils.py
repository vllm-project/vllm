...
# (prompt_idx, idx_0, idx_1, token_id_0, token_id_1) ->
# (log P_m1(seq_0[:idx_0 + 1]), log P_m0(seq_1[:idx_1 + 1])), or None if unscorable.
CrossScorer = Callable[[int, int, int, int, int], tuple[float, float] | None]
    ...
    * `always_check_logprobs == False`: highest-logprob token ids are
    ...
    Args:
      ...
      cross_scorer: Optional callable used to rescue a divergence whose top-k
                    sets are disjoint. See `CrossScorer`. When None (the
                    default), the top-k membership checks are unconditional,
                    i.e. behaviour is unchanged.
      cross_logprob_tol: Maximum per-token gap, in nats, between the two
                         cross-scored sequence logprobs for which a divergence
                         is excused. Only consulted when `cross_scorer` is
            ...
            if is_tok_mismatch or always_check_logprobs:
                ...
                if is_top_k_mismatch and cross_scorer is not None:
                    ...
                    if scores is not None:
                        seq_0_in_1, seq_1_in_0 = scores
                        # The gap accumulates with length, so normalize to a
                        # per-token rate or a late divergence is judged harder.
                        num_tokens = idx + 1
                        gap = abs(seq_0_in_1 - seq_1_in_0)
                        spread = gap / num_tokens
                        rescued = spread <= cross_logprob_tol
                        fail_msg += (
                            f"\ncross-scored:"
                            f"\tlogP_{name_1}({name_0} seq)={seq_0_in_1:.6f}"
                            f"\tlogP_{name_0}({name_1} seq)={seq_1_in_0:.6f}"
                            f"\tgap={gap:.6f}\ttokens={num_tokens}"
                            f"\tper_token={spread:.6f}"
