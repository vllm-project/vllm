...
def check_tokenstexts_match_or_in_top_logprobs(
    *,
    ...
    """Compare two sequences, allowing for small differences
    resulting from rounding error.

    The sequence can be represented by token lists or text, coupled
    with logprobs.
    
    The primary test for a match is whether the text/tokens are identical.
    However, if the text/tokens mismatch at a particular sequence offset,
    the fallback is to check where the sequence 0 token is in the set
    of top logprobs associated with sequence 1, and visa-versa.
