...
def _logprob_value(logprob: float | Logprob) -> float:
    return logprob.logprob if isinstance(logprob, Logprob) else logprob


def _in_topk_or_near_tie(
    token: Any,
    token_topk: Mapping[Any, float | Logprob],
    other_topk: Mapping[Any, float | Logprob],
    tol: float,
) -> bool:
    """Whether `token` may be missing from `other_topk` due to a near-tie.

    True if `token` is in `other_topk`, or if its logprob on its own side
    is within `tol` of the lowest logprob included in `other_topk`, i.e.
    the token plausibly fell just outside the other side's top-k because
    near-tied candidates were reordered by numerical drift.
    """
    if token in other_topk:
        return True
    if token not in token_topk or not other_topk:
        return False
    min_included = min(_logprob_value(lp) for lp in other_topk.values())
    return _logprob_value(token_topk[token]) >= min_included - tol


def _topk_token_sets_match(
    topk_0: Mapping[Any, float | Logprob],
    topk_1: Mapping[Any, float | Logprob],
    tol: float,
) -> bool:
    if tol <= 0:
        return set(topk_0) == set(topk_1)
    return all(
        _in_topk_or_near_tie(tok, topk_0, topk_1, tol) for tok in topk_0
    ) and all(_in_topk_or_near_tie(tok, topk_1, topk_0, tol) for tok in topk_1)
...
def check_logprobs_close(
    ...
    * `always_check_logprobs == False`: highest-logprob token ids are
    ...
    Args:
      ...
      near_tie_tol: If > 0, tolerate near-ties at the top-k boundary: a
                    token missing from one side's top-k logprobs is accepted
                    when its logprob on the other side is within
                    near_tie_tol of that side's lowest included logprob.
                    This absorbs top-k reordering from bf16 drift between
                    backends (e.g. HF vs vLLM) without relaxing comparisons
                    of tokens that are clearly outside the tie group.
                    The default of 0.0 keeps the comparison strict.
    ...
    for prompt_idx, (outputs_0, outputs_1) in enumerate(
            ...
            if prompt_logprobs_0 is not None and prompt_logprobs_1 is not None:
                ...
                for idx, (logprobs_elem_0, logprobs_elem_1) in enumerate(
                    ...
                    if logprobs_elem_0 is None:
                        ...
                        assert _topk_token_sets_match(
                            logprobs_elem_0, logprobs_elem_1, near_tie_tol
                        ), fail_msg
            else:
        ...
        for idx, (output_id_0, output_id_1) in enumerate(
            ...
            if is_tok_mismatch or always_check_logprobs:
                ...
                assert logprobs_elem_1 is not None, fail_msg
                if near_tie_tol > 0:
                    assert _in_topk_or_near_tie(
                        output_id_0, logprobs_elem_0, logprobs_elem_1, near_tie_tol
                    ), fail_msg
                    assert _in_topk_or_near_tie(
                        output_id_1, logprobs_elem_1, logprobs_elem_0, near_tie_tol
                    ), fail_msg
                else:
