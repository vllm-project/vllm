...
def test_prefill_checkpoint_resumes_suffix(monkeypatch, dim_first, num_spec, offset):
    """Restoring both cached states must reproduce an uninterrupted GLM prefill."""
    ...
    # Include padding between pages and use the selected physical conv layout.
    ...
    recurrent_storage = torch.zeros(num_slots, heads * D * D + 8, device=device)
