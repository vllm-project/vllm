    ...
    return [
        {
            ...
            "content": [
                ...
                *(
                    {"type": "image", "image": download_image(url)}
                    for url in urls
                ),
            ],
        }
...
def test_model_engine(vllm_runner, model: str, dtype: str) -> None:
    ...
    while True:
        ...
        if count == 2:
            engine.add_request(
                uuid.uuid4().hex, ENGINE_INPUTS[2], SAMPLING_PARAMS
            )
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [{"offset": 10, "length": 494}],
        ),
        (
            _create_engine_inputs_hf(IMG_URLS[1:4]),
            [
                {"offset": 10, "length": 266},
                {"offset": 276, "length": 1056},
                {"offset": 1332, "length": 418},
            ],
        ),
    ],
)
def test_multi_modal_placeholders(
    vllm_runner, prompt, expected_ranges: list[PlaceholderRange]
) -> None:
    with vllm_runner(
        ...
        assert hasattr(output, "multi_modal_placeholders"), (
            f"{output.__dict__=}"
        )
