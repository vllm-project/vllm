...
def replace_test_placeholder(
    prompt: str, img_idx_to_prompt: Callable[[int], str], test_placeholder: str
) -> str:
...
def build_embedding_inputs_from_test_info(
    ...
    vllm_embeddings = build_single_image_inputs(
        embeds, model_prompts, size_wrapper
    )
...
def build_video_inputs_from_test_info(
    ...
    video_scaler = (
        resize_video
        if size_wrapper.type == SizeType.FIXED_SIZE
