...
from transformers import AutoConfig, AutoTokenizer, BatchEncoding,GenerationConfig
...
def mlomo_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    def _generate_greedy_logprobs_limit(
        ...
        **kwargs: Any,
        ...
        if len(all_inputs):
            ...
            batch_inputs['image_input_idx']=torch.cat(images_input_idx_lst,dim=0)
            batch_inputs['image_masks']=torch.cat(imges_masks_lst,dim=0)
        
            outputs = self.model.generate_from_batch(
                batch=self.wrap_device(batch_inputs, device=self.model.device.type),
        ...
        for index in range(len(all_inputs)):
            (
                ...
                outputs.hidden_states, num_logprobs
            )
    ...
    hf_model.generate_greedy_logprobs_limit = types.MethodType(
        _generate_greedy_logprobs_limit, hf_model
    )
    return hf_model