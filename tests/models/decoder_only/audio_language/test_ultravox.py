...
def run_test(
    ...
    **kwargs,
    ...
    with vllm_runner(
        model, dtype=dtype, enforce_eager=True, **kwargs
    ) as vllm_model:
    ...
    def process(hf_inputs: BatchEncoding, **kwargs):
        hf_inputs["audio_values"] = hf_inputs["audio_values"].to(torch_dtype)  # type: ignore
        return hf_inputs

    with hf_runner(
        model, dtype=dtype, postprocess_inputs=process, auto_cls=AutoModel
    ) as hf_model:
        hf_outputs_per_audio = [
            hf_model.generate_greedy_logprobs_limit(
                ...
                audios=[
                    (
                        resample_audio(
                            audio[0], orig_sr=audio[1], target_sr=16000
                        ),
                        16000,
                    )
                ],
    ...
    for hf_outputs, vllm_outputs in zip(
        hf_outputs_per_audio, vllm_outputs_per_audio
    ):
