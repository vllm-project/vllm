...
def test_causal_mixin_compute_logits_forwards_lm_head_bias():
    """``CausalMixin.compute_logits`` must apply a learned ``lm_head.bias``.

    ``LogitsProcessor`` only adds the ``embedding_bias`` it is *passed* (its
    GEMM uses ``lm_head.weight`` plus the passed bias and never reads
    ``lm_head.bias``), so ``compute_logits`` must forward ``self.lm_head.bias``.
    Without it, a tied-embedding model that ships a learned output bias has the
    bias silently dropped, flipping the greedy argmax. GPU-free regression.
    """
    ...
    class _SpyLogitsProcessor:
        def __call__(self, lm_head, hidden_states, embedding_bias=None):
            captured["lm_head"] = lm_head
            captured["embedding_bias"] = embedding_bias
            return "LOGITS"

    bias = object()
    model = SimpleNamespace(
        logits_processor=_SpyLogitsProcessor(),
        lm_head=SimpleNamespace(bias=bias),
    )

    result = CausalMixin.compute_logits(model, "HIDDEN_STATES")

    assert result == "LOGITS"
    assert captured["lm_head"] is model.lm_head
    assert captured["embedding_bias"] is bias


def test_causal_mixin_has_lm_head_bias_detection():
    """``_has_lm_head_bias`` detects a learned output bias from a config flag
    (any known alias) or, failing that, the model's ``lm_head`` module."""
    from types import SimpleNamespace

    from vllm.model_executor.models.transformers.causal import CausalMixin

    # any known config flag alias -> True
    for attr in ("lm_head_bias", "use_lm_head_bias", "output_bias"):
        model = SimpleNamespace(
            text_config=SimpleNamespace(**{attr: True}), model=SimpleNamespace()
        )
        assert CausalMixin._has_lm_head_bias(model) is True

    # no flag, but the lm_head module carries a bias tensor -> True
    model = SimpleNamespace(
        text_config=SimpleNamespace(),
        model=SimpleNamespace(lm_head=SimpleNamespace(bias=object())),
    )
    assert CausalMixin._has_lm_head_bias(model) is True

    # no flag and no bias anywhere -> False
    model = SimpleNamespace(
        text_config=SimpleNamespace(),
        model=SimpleNamespace(lm_head=SimpleNamespace(bias=None)),
    )
    assert CausalMixin._has_lm_head_bias(model) is False
