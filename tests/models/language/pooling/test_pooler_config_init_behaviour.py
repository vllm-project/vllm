...
def test_reward_models_using_activation(
    ...
    for wo, w in zip(wo_activation, w_activation):
        ...
        assert not torch.allclose(
            wo, w, atol=1e-2), "pooler_config activation is not working"
        assert torch.allclose(
            softmax(wo), w, atol=1e-2
...
def test_multi_vector_retrieval_models_using_normalize(
    ...
    for wo, w in zip(wo_normalize, w_normalize):
        assert not torch.allclose(
            wo, w, atol=1e-2), "pooler_config normalize is not working"
        assert torch.allclose(
            F.normalize(wo, p=2, dim=-1), w,
            atol=1e-2), "w_normal should be close to normal(wo_normal)."
