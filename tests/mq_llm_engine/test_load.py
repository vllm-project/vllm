...
async def run_to_completion(client: MQLLMEngineClient,
                            request_id: str):
...
async def test_generation(tmp_socket):
    with RemoteMQLLMEngine(engine_args=ENGINE_ARGS,
        ...
        for request_id in request_ids:
            tasks.append(asyncio.create_task(run_to_completion(client, 
                                                               request_id)))
        ...
        for task in tasks:
            ...
            if (num_generated_tokens != NUM_EXPECTED_TOKENS and 
                failed_request_id is None):
