...
@pytest.mark.parametrize(
    "tokenizer_group_type", [None, "ray", CustomTokenizerGroup]
)
...
async def test_tokenizer_group_ray_pool_fault_tolerance(tokenizer_group_type):
    ...
    class FailingTokenizerGroup(TokenizerGroup):
        def __init__(
            self, *args, fail_at: Optional[list[int]] = None, **kwargs
        ):
            super().__init__(*args, **kwargs)
            self.i = 0
    ...
    await tokenizer_group_pool.encode_async(
        request_id="1", prompt="prompt", lora_request=None
    )
    await tokenizer_group_pool.encode_async(
        request_id="1", prompt="prompt", lora_request=None
    )
    ...
    with pytest.raises(ValueError):
        await tokenizer_group_pool.encode_async(
            request_id="1", prompt="prompt" * 100, lora_request=None
        )
    await tokenizer_group_pool.encode_async(
        request_id="1", prompt="prompt", lora_request=None
    )
