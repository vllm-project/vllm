...
def create_complete_sequence_token_ids(
    complete_sequence: str, tokenizer
) -> list[int]:
...
def test_decode_prompt_logprobs(
    complete_sequence_token_ids: list[int], detokenizer: Detokenizer
):
    """Verify Detokenizer decodes prompt logprobs correctly."""
    sampling_params = SamplingParams(
        skip_special_tokens=True, prompt_logprobs=1
    )
    ...
    assert text == "".join(
        [
            ...
            for token_id, logprobs in zip(
                token_ids[1:], decoded_prompt_logprobs
            )
        ]
    )
    assert text != "".join(
