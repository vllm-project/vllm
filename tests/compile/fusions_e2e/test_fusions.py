...
def test_fusion_correctness():
    ...
    # Compare the outputs to the huggingface baseline
    huggingface_model = AutoModelForCausalLM.from_pretrained(model_name)
    huggingface_output = huggingface_model(input_ids, attention_mask=attention_mask)
    assert torch.allclose(baseline_output.logits, huggingface_output.logits)

def test_fusion_correctness_with_hf_overrides():
    # Load model and tokenizer
    model_name = "vllm-base"
    model = AutoModelForCausalLM.from_pretrained(model_name)
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    ...
    # Run the fusion configuration with hf overrides
