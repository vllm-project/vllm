...
def test_unwrapped_native_dp_ep_disables_combo_kernel_benchmark(monkeypatch):
    ...
    try:
        ...
        assert should_use_aot_compile(cfg) is False
        assert (
            is_compile_cache_enabled(cfg.compilation_config.inductor_compile_config)
            is False
        )
    finally:
        if hasattr(envs.__getattr__, "cache_clear"):
...
def test_unwrapped_non_dp_ep_keeps_combo_kernel_benchmark(monkeypatch):
    monkeypatch.setenv("VLLM_FUSED_MOE_WRAP_MODE", "unwrapped")
    monkeypatch.setenv("VLLM_USE_AOT_COMPILE", "1")
    ...
    try:
        ...
        assert (
            cfg.compilation_config.inductor_compile_config.get(
                "force_disable_caches", False
            )
            is False
        )
        assert should_use_aot_compile(cfg) is True
        assert (
            is_compile_cache_enabled(cfg.compilation_config.inductor_compile_config)
            is True
        )
