    ...
    with monkeypatch.context() as m:
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            with use_vllm_config(vllm_config):
                ...
                ret = cached_mod(*args)
            assert cached_mod.aot_compile_loaded_from_cache, (
                "Expected aot_compile_loaded_from_cache to be True"
            )
...
def test_cache_load_returns_tuple_consistency(monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            assert cached_mod.aot_compile_loaded_from_cache, (
                "Expected aot_compile_loaded_from_cache to be True after "
                "loading from cache"
            )
    ...
    with monkeypatch.context() as m:
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            with use_vllm_config(vllm_config):
                compiled_mod = CompiledMod(vllm_config=vllm_config)
                compiled_mod(*args)
                assert compiled_mod.aot_compile_loaded_from_cache, (
                    "Expected aot_compile_loaded_from_cache to be True"
                )
                artifacts = compiled_mod.aot_compiled_fn._artifacts
                guards_string = artifacts.compiled_fn.shape_env.format_guards()
