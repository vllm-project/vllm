...
def test_nested_slice_partitions_are_canonicalized_dependency_first():
    """Cover slices whose source is itself another slice."""
    graph = torch.fx.Graph()
    ...
    assert outer_split.args[2] == -1
    assert inner_split.args[2] == -1

    inner_source = inner_split.args[0]
    assert inner_source.op == "call_function"
    assert inner_source.target == operator.getitem
    assert inner_source.args == (outer_split, 0)
