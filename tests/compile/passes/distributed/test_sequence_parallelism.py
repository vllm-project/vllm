...
class TestRocmAiterAllReduceRMSNormModel(torch.nn.Module):
    """Model using AITER RMSNorm ops for ROCm SP pattern matching tests."""
...
def test_rocm_aiter_sequence_parallelism_pass(
    batch_size: int,
    seq_len: int,
    hidden_size: int,
    dtype: torch.dtype,
    ...
    run_torch_spawn(_rocm_aiter_sp_worker_fn, num_processes)


def _rocm_aiter_sp_worker_fn(
    local_rank: int,
    world_size: int,
    batch_size: int,
    seq_len: int,
    ...
    with set_current_vllm_config(vllm_config):
        ...
        passes: list[VllmInductorPass] = [noop_pass, sp_pass, cleanup_pass]
        backend = TestBackend(*passes)

        model = TestRocmAiterAllReduceRMSNormModel(hidden_size)
        hidden_states = torch.randn((batch_size * seq_len, hidden_size), dtype=dtype)
        compiled_model = torch.compile(model, backend=backend)
        compiled_model(hidden_states)
