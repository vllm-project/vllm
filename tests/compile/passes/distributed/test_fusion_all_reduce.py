...
# ---------------------------------------------------------------------------
# ROCm AITER allreduce + RMSNorm fusion tests
# ---------------------------------------------------------------------------


class TestRocmAiterAllReduceRMSNormModel(torch.nn.Module):
    """
    ROCm AITER test model with 4 allreduce+rmsnorm pairs that should each
...
@multi_gpu_test(num_gpus=2)
@pytest.mark.parametrize("test_model", [TestRocmAiterAllReduceRMSNormModel])
@pytest.mark.parametrize("batch_size", [8])
@pytest.mark.parametrize("seq_len", [8])
@pytest.mark.parametrize("hidden_size", [64])
@pytest.mark.parametrize("dtype", [torch.bfloat16])
...
def test_rocm_aiter_allreduce_fusion_pass(
    ...
    run_torch_spawn(_rocm_aiter_worker_fn, num_processes)


def _rocm_aiter_worker_fn(
    local_rank: int,
    world_size: int,
    test_model_cls: torch.nn.Module,
    batch_size: int,
