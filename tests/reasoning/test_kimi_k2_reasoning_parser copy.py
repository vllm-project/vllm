...
def test_streaming_end_token_id_buffered(mock_kimi_k2_tokenizer):
    ...
    yet in delta_text (still buffered). The parser must emit visible reasoning
    text and defer the state transition rather than returning None and silently
    dropping the reasoning text.
    """
    ...
    # Visible reasoning text is emitted; the transition is deferred.
    assert isinstance(result, DeltaMessage)
    assert result.reasoning == "extra"
    assert result.content is None
    assert parser._pending_transition is True
    assert parser._emit_transition is False


def test_streaming_end_token_id_buffered_then_released(mock_kimi_k2_tokenizer):
    """Two-delta sequence: </think> buffered, then text released in next delta.

    Delta N:  token arrives but text is held back → emit reasoning, defer transition.
    Delta N+1: text is released → emit content marker, fire transition.
    is_reasoning_end_streaming must return False in delta N and True in delta N+1.
    """
    parser = KimiK2ReasoningParser(mock_kimi_k2_tokenizer)
    think_id = parser._start_token_id
    end_think_id = parser._end_token_id
    ...
    # Delta N: buffered
    result_n = parser.extract_reasoning_streaming(
        previous_text="some reasoning",
        current_text="some reasoning extra",
        delta_text="extra",
        previous_token_ids=[think_id],
    ...
    assert isinstance(result_n, DeltaMessage)
    assert result_n.reasoning == "extra"
    # is_reasoning_end_streaming defers while pending
    assert (
        parser.is_reasoning_end_streaming(
            [think_id, end_think_id, 999], [end_think_id, 999]
        )
        is False
    )
    assert parser._pending_transition is True

    # Delta N+1: text released
    result_n1 = parser.extract_reasoning_streaming(
        previous_text="some reasoning extra",
        current_text="some reasoning extra</think>",
        delta_text="</think>",
        previous_token_ids=[think_id, end_think_id, 999],
        current_token_ids=[think_id, end_think_id, 999],
        delta_token_ids=[],
    )
    assert isinstance(result_n1, DeltaMessage)
    assert result_n1.reasoning is None
    assert result_n1.content is None  # nothing after </think>
    assert parser._emit_transition is True
    # is_reasoning_end_streaming fires the deferred transition
    assert parser.is_reasoning_end_streaming([think_id, end_think_id, 999], []) is True
    assert parser._emit_transition is False


def test_streaming_tool_section_id_buffered_then_released(mock_kimi_k2_tokenizer):
    """Two-delta sequence for <|tool_calls_section_begin|> buffering.

    Delta N:  token arrives but text is held back → emit reasoning, defer transition.
    Delta N+1: text is released → emit content with marker, fire transition.
    is_reasoning_end_streaming must return False in delta N and True in delta N+1.
    """
    parser = KimiK2ReasoningParser(mock_kimi_k2_tokenizer)
    think_id = parser._start_token_id
    ...
    assert isinstance(result_n, DeltaMessage)
    assert result_n.reasoning == "extra"
    # is_reasoning_end_streaming must defer
    assert (
        parser.is_reasoning_end_streaming(
            [think_id, tool_begin_id, 999], [tool_begin_id, 999]
        )
        is False
    )
    assert parser._pending_transition is True

    # Delta N+1: text released (includes the marker and some tool call content)
    result_n1 = parser.extract_reasoning_streaming(
        previous_text="some reasoning extra",
        current_text="some reasoning extra<|tool_calls_section_begin|>",
        delta_text="<|tool_calls_section_begin|>",
        previous_token_ids=[think_id, tool_begin_id, 999],
        current_token_ids=[think_id, tool_begin_id, 999],
        delta_token_ids=[],
    )
    assert isinstance(result_n1, DeltaMessage)
    assert result_n1.content == "<|tool_calls_section_begin|>"
    assert parser._emit_transition is True
    # is_reasoning_end_streaming fires the deferred transition
    assert parser.is_reasoning_end_streaming([think_id, tool_begin_id, 999], []) is True
    assert parser._emit_transition is False
...
def test_streaming_tool_section_id_buffered(mock_kimi_k2_tokenizer):
    ...
    The parser must emit visible reasoning text and defer the state transition
    rather than returning None (which would silently drop reasoning text and
    cause the tool parser to never see <|tool_calls_section_begin|>).
    """
    parser = KimiK2ReasoningParser(mock_kimi_k2_tokenizer)
    think_id = parser._start_token_id
    tool_begin_id = parser._tool_section_start_token_id
