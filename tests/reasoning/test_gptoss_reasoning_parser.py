...
def test_gptoss_reasoning_end_delta_index(gpt_oss_tokenizer):
    parser: ReasoningParser = GptOssReasoningParser(gpt_oss_tokenizer)

    previous_output = gpt_oss_tokenizer.tokenize(
        REASONING_SECTION_START + "This is reasoning"
    )
    delta_output = gpt_oss_tokenizer.tokenize(
        ASSISTANT_CONTENT_START + "This is the rest"
    )

    previous_ids = gpt_oss_tokenizer.convert_tokens_to_ids(previous_output)
    delta_ids = gpt_oss_tokenizer.convert_tokens_to_ids(delta_output)

    actual_end_idx = parser.reasoning_end_delta_index(previous_ids, delta_ids)
    expected_end_idx = (
        len(
            gpt_oss_tokenizer.convert_tokens_to_ids(
                gpt_oss_tokenizer.tokenize(ASSISTANT_CONTENT_START)
            )
        )
        - 1
    )

    assert actual_end_idx == expected_end_idx
