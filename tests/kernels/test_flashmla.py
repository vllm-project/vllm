...
@pytest.mark.skipif(
    not is_flashmla_supported()[0], reason=FLASH_MLA_UNSUPPORTED_REASON
)
...
def test_flash_mla(
    b, s_q, mean_sk, h_q, h_kv, d, dv, block_size, causal, varlen
):
    ...
    for i in range(b):
        blocked_k.view(b, max_seqlen_pad, h_kv, d)[
