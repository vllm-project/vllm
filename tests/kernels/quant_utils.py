...
def ref_dynamic_per_token_quant(
    ...
    qtype_traits_max = (
        ROCM_FP8_MAX if current_platform.is_rocm() else qtype_traits.max
    )
    qtype_traits_min = (
        -ROCM_FP8_MAX if current_platform.is_rocm() else qtype_traits.min
    )
    ...
    if quant_dtype == torch.int8:
        ...
        torch_out = torch_out.clamp(qtype_traits_min, qtype_traits_max).to(
            quant_dtype
        )
    else:
        assert quant_dtype == FP8_DTYPE
...
def ref_dynamic_per_tensor_fp8_quant(
    ...
    fp8_traits_max = (
        ROCM_FP8_MAX if current_platform.is_rocm() else fp8_traits.max
    )
    fp8_traits_min = (
        -ROCM_FP8_MAX if current_platform.is_rocm() else fp8_traits.min
    )
