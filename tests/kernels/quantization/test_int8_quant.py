...
def test_dynamic_scaled_int8_azp_quant(num_tokens: int, hidden_size: int,
    ...
    torch_out = ((x / scales).round() + azps).clamp(int8_traits.min,
...
@torch.inference_mode()
def test_static_scaled_int8_quant(num_tokens: int, hidden_size: int, dtype: torch.dtype,
...
def test_static_scaled_int8_azp_quant(num_tokens: int, hidden_size: int,
                                      dtype: torch.dtype, seed: int, scale: float,
                                      azp: int) -> None:
    current_platform.seed_everything(seed)
    int8_traits = torch.iinfo(torch.int8)
