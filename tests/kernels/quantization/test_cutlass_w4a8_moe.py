        ...
        for K in Ks:
            for N in Ns:
                ...
                per_chan_scales = torch.randn(
                    (num_experts, N, 1), dtype=torch.float32
                ).cuda()
                ...
                for _ in range(num_experts):
                    ...
                    w_ref, w_q, w_s, _ = cutlass_quantize(
                        atype, b.to(torch.float16), wtype, stype, GROUP_SIZE, False
                    )
                    w_refs.append(w_ref)
                    w_qs.append(w_q)
                    w_ss.append(w_s)
                ...
                problem_sizes = torch.tensor(
                    [[N, M, K] for M in Ms], dtype=torch.int32
                ).cuda()
                ...
                group_scale_strides = torch.zeros(
                    (num_experts, 2), dtype=torch.int64
                ).cuda()
                ...
                for i in range(num_experts):
                    ...
                    torch.testing.assert_close(
                        out[start:end], out_ref, rtol=1e-2, atol=1e-2
                    )
    print("tests passed!")
