...
@pytest.mark.parametrize(
    ("hidden_size", "expected_k"),
    [(1280, 5120), (1792, 7168), (1920, 7680), (4096, 16384), (7168, 28672)],
)
def test_hc_prenorm_gemm_warmup_keys_come_from_model(
    monkeypatch: pytest.MonkeyPatch,
    hidden_size: int,
    expected_k: int,
) -> None:
    from vllm.model_executor.kernels.mhc import tilelang_kernels
    from vllm.model_executor.kernels.mhc.cutedsl import HCPrenormGemmKernel

    split_by_grid = {1: 8, 2: 4, 3: 4, 4: 1}
    monkeypatch.setattr(
        tilelang_kernels,
        "compute_num_split",
        lambda block_k, k, grid_size: split_by_grid[grid_size],
    )

    kernel = HCPrenormGemmKernel()
    assert kernel.get_warmup_keys(_mhc_warmup_config(hidden_size=hidden_size)) == [
        kernel.CompileKey(k=expected_k, num_splits=8),
        kernel.CompileKey(k=expected_k, num_splits=4),
        kernel.CompileKey(k=expected_k, num_splits=1),
    ]


@requires_cutedsl_mhc
def test_hc_prenorm_gemm_warmup_is_model_type_independent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from vllm.model_executor.kernels.mhc import tilelang_kernels
    from vllm.model_executor.kernels.mhc.cutedsl import HCPrenormGemmKernel
    ...
    monkeypatch.setattr(
        tilelang_kernels,
        "compute_num_split",
        lambda block_k, k, grid_size: 4,
    )
    kernel = HCPrenormGemmKernel()

    assert kernel.get_warmup_keys(
        _mhc_warmup_config(model_type="future_mhc_model")
    ) == [kernel.CompileKey(k=16384, num_splits=4)]


@requires_cutedsl_mhc
@pytest.mark.parametrize(
    ("hc_mult", "hidden_size", "max_num_batched_tokens"),
    [
        (2, 4096, 256),
        ("4", 4096, 256),
        (4, 1024, 256),
...
def test_hc_prenorm_gemm_warmup_skips_inapplicable_models(
    hc_mult: object,
    hidden_size: int,
    ...
    config = _mhc_warmup_config(
        hc_mult=hc_mult,
        hidden_size=hidden_size,
        max_num_batched_tokens=max_num_batched_tokens,
    )
    assert HCPrenormGemmKernel().get_warmup_keys(config) == []


@requires_cutedsl_mhc
def test_hc_prenorm_gemm_warmup_skips_missing_hc_mult() -> None:
    from vllm.model_executor.kernels.mhc.cutedsl import HCPrenormGemmKernel

    config = _mhc_warmup_config()
    del config.model_config.hf_config.hc_mult

    assert HCPrenormGemmKernel().get_warmup_keys(config) == []


@requires_cutedsl_mhc
def test_hc_prenorm_gemm_runtime_uses_warmup_compile_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from vllm.model_executor.kernels.mhc import tilelang_kernels
    from vllm.model_executor.kernels.mhc.cutedsl import HCPrenormGemmKernel
    ...
    kernel = HCPrenormGemmKernel()
    compile_key = kernel.CompileKey(k=16384, num_splits=4)
    calls: list[tuple[object, ...]] = []
    kernel.compiled_kernels[compile_key] = lambda *args: calls.append(args)
    a = SimpleNamespace(shape=(1, 16384))
    b, d, sqr_sum = object(), object(), object()

    kernel(a, b, d, sqr_sum, num_splits=4)

    assert calls == [(a, b, d, sqr_sum)]
    assert kernel.get_warmup_keys(_mhc_warmup_config()) == [compile_key]
