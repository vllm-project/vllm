...
def ref_grouped_matmul(
    ...
    output = torch.zeros(fused_input.shape[0], fused_group_b.shape[2], device=fused_input.device, dtype=fused_input.dtype)
...
def test_moe_grouped_matmul(
    ...
    cum_group_range = torch.tensor([0] + list(itertools.accumulate(groups)), dtype=torch.int32, device="cuda")
    fused_group_b = torch.randn(group_size, k, n, dtype=dtype, device="cuda")

    ref_output = ref_grouped_matmul(fused_input, cum_group_range, fused_group_b, activation)
    output = grouped_matmul(fused_input, cum_group_range, fused_group_b, activation)
