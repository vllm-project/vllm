...
def test_dequant_add_residual_rms_norm_quant(
    ...
    x = torch.randint(-1000, 1000, (num_tokens, hidden_size), dtype=torch.int32, device="cuda")
    ...
    out2 = torch.empty_like(x, dtype=torch.int8)
    layernorm_ops.invoke_dequant_add_residual_rms_norm_quant(out2, x, residual, ref.weight.data, ref.variance_epsilon, scale)
