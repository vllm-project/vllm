...
def test_rms_norm(
    ...
    ref_out = ref(x)
    # FIXME: A10G has slight larger difference than A100.
    if torch.cuda.get_device_name().find("NVIDIA A100") >= 0:
        assert torch.allclose(out, ref_out, atol=1e-2, rtol=1e-5)
    else:
        assert torch.allclose(out, ref_out, atol=1e-1, rtol=1e-4)
