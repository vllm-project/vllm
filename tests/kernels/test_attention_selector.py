...
from vllm.utils import (STR_FLASH_ATTN_VAL, STR_FLASH_ATTN_VLLM_V1_VAL,
                        STR_INVALID_VAL)
...
@pytest.mark.parametrize("use_v1", [True, False])
def test_flash_attn(use_v1: bool, monkeypatch):
    """Test FlashAttn validation."""
    # TODO: When testing for v1, pipe in `use_v1` as an argument to
    # get_attn_backend

    monkeypatch.setenv("VLLM_USE_V1", "1" if use_v1 else "0")
    STR_FLASH_ATTN_BACKEND = (STR_FLASH_ATTN_VLLM_V1_VAL
                              if use_v1 else STR_FLASH_ATTN_VAL)

    override_backend_env_variable(monkeypatch, STR_FLASH_ATTN_BACKEND)

    # Unsupported CUDA arch
    with patch("torch.cuda.get_device_capability", return_value=(7, 5)):
        backend = get_attn_backend(16, torch.float16, None, 16, False)
        assert backend.get_name() != STR_FLASH_ATTN_BACKEND
...
def test_invalid_env(use_v1: bool, monkeypatch):
    ...
    with patch("vllm.attention.selector.current_platform", CudaPlatform()):
        ...
        if use_v1:
            backend = get_attn_backend(16, torch.float16, None, 16, False)
            assert backend.get_name() == "XFORMERS_VLLM_V1"
        else:
            backend = get_attn_backend(16, torch.float16, None, 16, False)
