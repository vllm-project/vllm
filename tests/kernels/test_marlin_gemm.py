...
@pytest.mark.parametrize(
    "quant_type", query_marlin_supported_quant_types(False)
)
...
def test_awq_marlin_repack(
    k_chunk, n_chunk, quant_type, group_size, mnk_factors
):
    m_factor, n_factor, k_factor = mnk_factors
...
def test_gptq_marlin_24_gemm(
    ...
    (w_24_ref, marlin_24_q_w_comp, marlin_24_meta, marlin_24_s) = (
        marlin_24_quantize(b_weight, quant_type, group_size)
    )
...
def test_hqq_marlin_gemm(
    ...
    b_weight = torch.randint(
        0, 10, (size_n, size_k), dtype=torch.uint8, device=dev
    )
    ...
    marlin_w_q = ops.gptq_marlin_repack(
        gptq_w_q, sort_indices, size_k, size_n, 4
    ).to(dev)
    ...
    output_ref = torch.matmul(
        a_input, dequant.reshape(b_weight.shape).transpose(1, 0)
    )
