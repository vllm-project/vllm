...
def test_merge_kernel(
    ...
    prefix_output = torch.randn(
        num_tokens, num_query_heads, head_size, dtype=dtype
    )
...
def test_cascade(
    ...
    query = torch.randn(
        total_num_query_tokens, num_query_heads, head_size, dtype=dtype
    )
    ...
    cu_prefix_query_lens = torch.tensor(
        [0, total_num_query_tokens], dtype=torch.int32
    )
