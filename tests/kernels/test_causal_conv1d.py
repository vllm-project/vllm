...
def test_causal_conv1d(
    ...
    if has_initial_state:
        initial_states = torch.randn(
            batch, dim, width - 1, device=device, dtype=itype
        )
        has_initial_state_tensor = torch.ones(
...
def test_causal_conv1d_update_with_batch_gather(
    ...
    unused_states_bool = torch.ones(
        total_entries, dtype=torch.bool, device=device
    )
...
def test_causal_conv1d_varlen(
    ...
    state_indices = torch.randperm(
        total_entries, dtype=torch.int32, device=x.device
    )[:batch_size]
    padded_state_indices = torch.concat(
        [
            state_indices,
