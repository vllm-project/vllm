...
def _decoder_attn_setup(
    ...
    * max_block_idx: max physical address in decoder self-attention block-table
    ...
    causal_mask = make_causal_mask(max_q_seq_len, max_kv_seq_len).to(
        CUDA_DEVICE
    )
    ...
    (
        ...
        slot_mapping_list, qkv.q_seq_lens, device=CUDA_DEVICE
    )
...
@pytest.mark.skipif(
    current_platform.is_rocm(), reason=STR_NOT_IMPL_ENC_DEC_ROCM_HIP
)
