...
def _gemm(a, b):
    ...
    return LL_BF16_GEMM_KERNEL(a, b)
...
def test_gate_linear_uses_ll_bf16_for_bf16_fast_path(monkeypatch):
    ...
    monkeypatch.setattr(
        "vllm.model_executor.kernels.linear.cute_dsl.ll_bf16.LL_BF16_GEMM_KERNEL",
        fake_ll_bf16_gemm,
    )
    out, bias = gate(x)
    assert bias is None
...
def test_invalid_output_dtype():
    ...
    with pytest.raises(ValueError, match="output_dtype=torch.float32"):
        ...
        LL_BF16_GEMM_KERNEL(a, b, output_dtype=torch.bfloat16)


def test_cache_miss_raises_for_unwarmed_dotprod():
    ...
    kernel = LLBf16Gemm()
    with pytest.raises(RuntimeError, match="unwarmed JIT compile key"):
        kernel(a, b)


def test_warmup_keys_cover_router_compile_keys():
    from vllm.model_executor.kernels.linear.cute_dsl.ll_bf16 import LLBf16Gemm

    kernel = LLBf16Gemm()
    expected = [
        kernel.CompileKey(backend="dotprod", m=1, k=7168, bs=128),
        kernel.CompileKey(backend="dotprod", m=2, k=7168, bs=128),
