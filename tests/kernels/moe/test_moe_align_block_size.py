...
@pytest.mark.parametrize("max_tokens_per_batch", [16, 32, 256, 512])
...
def test_batched_moe_align_block_size(max_tokens_per_batch: int,
    ...
    def ref_outputs(
        ...
        Msum = max(max_tokens_per_batch, block_size) * E
        Msum = round_up(Msum, block_size)
        ...
        return (ref_sorted_ids.to("cuda"), ref_expert_ids.to("cuda"),
                ref_num_tokens_post_pad.to("cuda"))

    # Compute expert_num_tokens
    expert_num_tokens = torch.randint(low=0,
