...
from vllm.model_executor.layers.fused_moe.cutlass_moe import (
    CutlassExpertsFp8, CutlassExpertsBlockedFp8)
...
def pplx_cutlass_blocked_moe(
    ...
    experts = CutlassExpertsBlockedFp8((num_experts + world_size - 1) // world_size,
                                out_dtype, per_act_token)
...
def _pplx_blocked_moe(
    ...
    with set_current_vllm_config(vllm_config):
        torch_output = torch_moe2(a_full, w1_full, w2_full, topk_weights,
                                  topk_ids)
        pplx_output = pplx_cutlass_blocked_moe(pgi, dp_size, a, w1, w2, w1_scale,
                                       w2_scale, topk_weights, topk_ids,
                                       a1_scale, out_dtype, per_act_token)
...
@pytest.mark.skipif(
    ...
    reason="Grouped gemm is not supported on this GPU type.")
@requires_pplx
def test_blocked_cutlass_moe_pplx(
    m: int,
    n: int,
    k: int,
    e: int,
    ...
    with set_current_vllm_config(vllm_config):
        ...
        w11_scales = 2 * n // block_size[0]
        w12_scales = k // block_size[1]
        w21_scales = k // block_size[0]
        w22_scales = n // block_size[1]

        w1_q = torch.empty((e, 2 * n, k),
                           device="cuda",
                           dtype=torch.float8_e4m3fn)
        w2_q = torch.empty((e, k, n), device="cuda", dtype=torch.float8_e4m3fn)
        w1_scale = torch.empty((e, w11_scales, w12_scales),
                               device="cuda",
                               dtype=torch.float32)
        w2_scale = torch.empty((e, w21_scales, w22_scales),
                               device="cuda",
                               dtype=torch.float32)

        for expert in range(e):
            w1_q[expert], w1_scale[expert] = per_block_cast_to_fp8(w1[expert])
            w2_q[expert], w2_scale[expert] = per_block_cast_to_fp8(w2[expert])
        ...
        if per_act_token:
            a_scale1 = torch.randn(
                (m, k // block_size[1]), device="cuda",
                dtype=torch.float32)
        else:
            a_scale1 = torch.randn((1, ), device="cuda", dtype=torch.float32)
            a_scale1 = a_scale1.repeat(world_size, 1)

        parallel_launch(world_size, _pplx_blocked_moe, dp_size, a, w1_q, w2_q,
                        w1_scale, w2_scale, topk_weights, topk_ids, a_scale1,
                        dtype, a, w1_d, w2_d, per_act_token)
