...
def test_w8a16_block_fp8_cpu_fused_moe_expert_parallel(
    ...
    The router produces global expert ids; each rank explicitly remaps and masks
    them to provide an independent reference for the production compact-and-skip
    path. Because the kernel applies topk_weights internally and combine is SUM,
    the sum of per-rank outputs must equal the full single-rank mixture.
    """
    if num_ranks > E:
    ...
    for rank in range(num_ranks):
        ...
        # Remap + mask provides a reference for the local-skip path.
...
def test_w8a16_block_fp8_cpu_fused_moe_expert_parallel_apply(
    ...
    local-expert compaction through ``CPUExpertsFp8.apply()`` (the production
    entry point) instead of hand-rolling the masked reference, so the
    ``expert_map is not None`` branch is exercised.
    """
    if num_ranks > E:
        pytest.skip(f"E={E} < num_ranks={num_ranks}, skipping degenerate case")
...
def test_fp8_cpu_fused_moe_skip_padding_expert_parallel(
    ...
    out_unpadded = torch.empty_like(a)
    fused_experts_cpu_local_skip(
        out_unpadded,
        a.clone(),
        *common_args[:2],
