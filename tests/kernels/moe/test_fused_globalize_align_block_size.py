...
# These have effective_work 1504 (single block) and 6068 (cooperative).
@pytest.mark.parametrize("n", [1, 64])
def test_fused_cuda_graph_replay(n):
    """Both launch paths remain capturable and consume replay-time inputs."""
    topk = 16
    local_e = 28
    block_size = 64
    ep_size = 4
    ...
    graph_idx = torch.empty_like(local_idx)
    graph_psum = torch.empty_like(psum)

    # Initialize lazy CUDA state before capture.
    fused_globalize_align_block_size(
        local_idx.clone(), psum, reo, gne, local_e, block_size
    )
    ...
    with torch.cuda.graph(graph):
        graph_idx.copy_(local_idx)
        graph_psum.copy_(psum)
        graph_outputs = fused_globalize_align_block_size(
            graph_idx, graph_psum, reo, gne, local_e, block_size
        )

    for frac_recv in (0.75, 0.5):
        psum[-1] = int(round(n * frac_recv))
        ref_idx = _globalize_recv_topk_idx(local_idx.clone(), psum, reo, gne)
        ref_sorted, ref_expert_ids, ref_num = moe_align_block_size(
            topk_ids=ref_idx,
            block_size=block_size,
            num_experts=gne,
        ...
        graph.replay()
        torch.accelerator.synchronize()
        fused_idx, fused_sorted, fused_expert_ids, fused_num = graph_outputs

        assert torch.equal(fused_idx, ref_idx)
        assert torch.equal(fused_num, ref_num)
        valid = int(fused_num.item())
        num_blocks = valid // block_size
        assert torch.equal(fused_expert_ids[:num_blocks], ref_expert_ids[:num_blocks])
        fused_groups = _group_by_expert(
            fused_sorted, fused_expert_ids, block_size, valid, n * topk
        )
        ref_groups = _group_by_expert(
            ref_sorted,
            ref_expert_ids,
        ...
        assert set(fused_groups) == set(ref_groups)
        for eid in ref_groups:
            assert sorted(fused_groups[eid]) == sorted(ref_groups[eid])
