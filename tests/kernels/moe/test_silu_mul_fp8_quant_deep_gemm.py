...
def pack_scales(x: torch.Tensor, tokens_per_expert: torch.Tensor) -> torch.Tensor:
    ...
    ref_s_i8 = torch.empty((E, T, G + i32_padding), dtype=torch.uint8, device=x.device)
    for e in range(E):
...
def ref_with_scale_fmt(
    ...
    ref_q = torch.empty((E, T, H), dtype=fp8_dtype, device=gate.device)
    ref_s_f32 = torch.empty(
        (E, T, cdiv(H, group_size)), dtype=torch.float32, device=gate.device
    )
