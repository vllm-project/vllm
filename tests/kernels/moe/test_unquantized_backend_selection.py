...
@pytest.mark.parametrize(
    "quant_backend",
    ["humming", "marlin", "flashinfer_b12x", "deep_gemm", "flashinfer_cutedsl"],
)
def test_quantization_only_backend_fallback_to_auto(quant_backend):
    """Quantization-only backends (like 'humming', 'marlin', 'flashinfer_b12x', 'deep_gemm')
    should fall through to 'auto' backend selection for unquantized layers (e.g. MTP draft MoE
    layers or modules excluded via modules_to_not_convert) instead of raising ValueError.
    """
    with (
        patch.object(current_platform, "is_cuda", return_value=False),
        patch.object(current_platform, "is_rocm", return_value=False),
        ...
        patch.object(current_platform, "is_out_of_tree", return_value=False),
        patch(
            "vllm.model_executor.layers.fused_moe.oracle.unquantized.logger.info_once"
        ) as mock_log,
    ):
