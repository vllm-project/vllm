...
class MarlinMoEWeightData:
    ...
    sort_indices: Optional[torch.Tensor]
    marlin_bias: Optional[torch.Tensor]

    @staticmethod
    def make(w: torch.Tensor,
             quant_type: ScalarType,
             group_size: int,
             act_order: Optional[bool] = None,
             bias: Optional[torch.Tensor] = None) -> "MarlinMoEWeightData":
        ...
        for i in range(w.shape[0]):
            if quant_type == scalar_types.float4_e2m1f:
                if group_size == 16:
                    ...
                    w_ref, qweight, scales = \
                        rand_marlin_weight_mxfp4_like(w[i], group_size)
                    global_scale = None
                ...
                w_ref, qweight, scales, g_idx, sort_indices, _ = \
                    marlin_quantize(w[i].transpose(1, 0), quant_type,
        ...
        global_scale = stack_and_dev(
            global_scale_l) if global_scale_l else None
        ...
        sort_indices = stack_and_dev(
            sort_indices_l) if sort_indices_l else None
...
def test_fused_marlin_moe(
    ...
    w1_data = MarlinMoEWeightData.make(w=w1,
                                       quant_type=quant_type,
                                       group_size=group_size,
                                       act_order=act_order)

    w2_data = MarlinMoEWeightData.make(w=w2,
                                       quant_type=quant_type,
                                       group_size=group_size,
                                       act_order=act_order)
...
def test_fused_marlin_moe_with_bias(m):
    ...
    with set_current_vllm_config(vllm_config):
        torch_output = torch_moe(a, w1_data.w_ref, w2_data.w_ref, score, topk,
                                 b_bias1, b_bias2)
...
def test_batched_moe_align_block_size_opcheck():
    ...
    opcheck(torch.ops._moe_C.batched_moe_align_block_size,
            (max_tokens_per_batch, block_size, expert_num_tokens, sorted_ids,
...
@pytest.mark.skipif(current_platform.is_rocm(), reason="Skip for rocm")
def test_batched_fused_marlin_moe(m: int, n: int, k: int, e: int, topk: int,
                                  max_tokens_per_batch: int):

    print(f"testing m={m}, n={n}, k={k}, e={e}, "
          f"topk={topk}, "
    ...
    class BatchedRun:
        ...
        def _make_expert_num_tokens_cpu(
                e: int,  # num_experts
                topk_ids_cpu: torch.Tensor) -> torch.Tensor:
            expert_num_tokens_cpu = torch.zeros((e, ),
                                                dtype=torch.int32,
                                                device="cpu")
        ...
        def __init__(self, max_tokens_per_batch: int, num_experts: int,
                     _topk_ids: torch.Tensor, _topk_weights: torch.Tensor):
        ...
        def is_valid(self):
            ...
            """
            return torch.all(
                self.expert_num_tokens_cpu <= self.max_tokens_per_batch)
        ...
        def _gather(self, batched_outputs: torch.Tensor,
                    gather_outputs: torch.Tensor) -> torch.Tensor:
            ...
            for t_idx in range(md):
                token = None
                for topk_id, topk_weight in zip(self.topk_ids_cpu[t_idx],
                                                self.topk_weights_cpu[t_idx]):
                    pos_in_batch = counter_cpu[topk_id]
                    t = batched_outputs_cpu[topk_id,
                                            pos_in_batch] * topk_weight
                    if token is None:
                        token = t
                    else:
                        token += t
        ...
        def run(self, hidden_states: torch.Tensor,
            ...
            batched_outputs = torch.ops.vllm.batched_fused_marlin_moe(**kwargs)
    ...
    ref_marlin_output = torch.ops.vllm.fused_marlin_moe(
        **fused_marlin_moe_kwargs)
    ...
    torch.testing.assert_close(marlin_output,
                               ref_marlin_output,
                               atol=1e-3,
                               rtol=0)
