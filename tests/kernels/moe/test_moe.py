...
def test_triton_moe_launcher_rejects_fp8_weight_only_below_sm89(monkeypatch) -> None:
    """A w8a16 (fp8 weight, unquantized activation) caller passes
    use_fp8_w8a8=False, but B is still float8_e4m3fn and hits the same
    unsupported Triton JIT path below SM89 -- the guard must key off the
    tensor dtypes, not the use_fp8_w8a8 flag."""
    launched = False
    ...
    with pytest.raises(ValueError, match="SM89"):
        fused_moe_module.invoke_fused_moe_triton_kernel(
            A=torch.ones((1, 1), dtype=torch.float16, device=DEVICE_TYPE),
            B=torch.ones((1, 1, 1), dtype=torch.float8_e4m3fn, device=DEVICE_TYPE),
            C=torch.empty((1, 1, 1), device=DEVICE_TYPE),
            A_scale=None,
            B_scale=torch.ones(1, device=DEVICE_TYPE),
