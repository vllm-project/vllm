...
def is_valid_config(config: MoETestConfig) -> tuple[bool, str | None]:
    ...
    if config.quantization == "modelopt_fp4" and not (
        current_platform.is_rocm() or current_platform.has_device_capability(90)
    ):
...
def test_moe_layer(
    ...
    if backend == "mori" and os.environ.get("VLLM_TEST_ENABLE_MORI_MOE_LAYER") == "1":
