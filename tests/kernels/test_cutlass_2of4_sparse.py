...
def prune_to_2_4(tensor):
    ...
    mask.scatter_(
        dim=1, index=indices, src=torch.ones_like(indices, dtype=mask.dtype)
    )
...
def test_cutlass_sparse_subset():
    ...
    # Create tensors
    b_comp, e, whole_a, b = make_rand_sparse_tensors(
        torch.float8_e4m3fn, big_m, n, k
    )
    a = whole_a[0:m, 0:k]
...
def test_cutlass_sparse_fp8_gemm(m: int, n: int, k: int, use_bias: bool):
    ...
    bias = (
        torch.rand((n,), device="cuda", dtype=out_dtype) * 10
        if use_bias
        else None
    )
