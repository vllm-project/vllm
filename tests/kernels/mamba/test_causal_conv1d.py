...
def causal_conv1d_update_ref(x,
    ...
    if cache_seqlens is None:
        x_new = torch.cat([conv_state, x],
                          dim=-1).to(weight.dtype)  # (batch, dim, state_len + seqlen)
        conv_state.copy_(x_new[:, :, -state_len:])
    else:
        width_idx = torch.arange(
            -(width - 1), 0, dtype=torch.long,
            device=x.device).unsqueeze(0) + cache_seqlens.unsqueeze(1)
        width_idx = torch.remainder(width_idx,
...
def causal_conv1d_opcheck_fn(x: torch.Tensor,
    ...
    opcheck(torch.ops._C.causal_conv1d_fwd,
            (x, weight, bias, conv_states, cu_seq_len, cache_indices, has_initial_state,
             activation in ["silu", "swish"], pad_slot_id))
...
@pytest.mark.parametrize('seqlen',
                         [1, 8, 16, 32, 64, 128, 256, 512, 784, 1024, 1025, 2048, 4096])
...
def test_causal_conv1d(batch, dim, seqlen, width, has_bias, silu_activation,
    ...
    out_ref, final_states_ref = causal_conv1d_ref(x_ref,
                                                  weight_ref,
                                                  bias_ref,
...
def test_causal_conv1d_update_with_batch_gather(with_padding, dim, width, seqlen,
...
def test_causal_conv1d_varlen(with_padding, dim, seqlen, width, has_bias,
    ...
    seqlens.append(
        torch.diff(torch.cat([torch.tensor([-1]), eos_pos,
    ...
    has_initial_states = torch.randint(0,
                                       ...
                                       dtype=torch.bool,
                                       device=x.device)
    state_indices = torch.randperm(total_entries, dtype=torch.int32,
    ...
    out = causal_conv1d_fn(x.squeeze(0), weight, bias, cumsum.cuda(),
                           padded_state_indices, has_initial_states, final_states,
