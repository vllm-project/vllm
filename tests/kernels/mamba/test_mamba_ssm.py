...
def test_selective_state_update_with_num_accepted_tokens(
    ...
    cu_seqlens = torch.tensor(
        [0] + list(torch.cumsum(torch.tensor(tokens_per_seq), dim=0)),
        dtype=torch.int32,
        device=device,
    )

    total_state_slots = 20
    ...
    for seq_idx in range(batch_size):
        token_pos = max(num_accepted_tokens[seq_idx].item() - 1, 0)
        state_slot = state_batch_indices[seq_idx, token_pos].item()
        initial_state = state[state_slot].clone()
        state_ref_list.append(initial_state)

    state_ref = torch.stack(state_ref_list, dim=0)
    ...
    for seq_idx in range(batch_size):
        ...
        state_seq = state_ref[seq_idx : seq_idx + 1]
        for token_idx in range(num_tokens):
            idx = seq_start + token_idx
    ...
    print(f"Tokens in current iteration: {tokens_per_seq}")
    print(f"Tokens accepted in previous iteration: {num_accepted_tokens}")
    print(f"Initial states read from slots: {initial_state_slots}")

    assert torch.allclose(out, out_ref, rtol=rtol, atol=atol)

    for seq_idx in range(batch_size):
        state_slot = initial_state_slots[seq_idx]
        assert torch.allclose(
            state[state_slot], state_ref[seq_idx], rtol=rtol, atol=atol
        )
...
@pytest.mark.parametrize("dim", [2048, 4096])
@pytest.mark.parametrize("max_seq_len", [2, 4])
def test_selective_state_update_with_inplace_final_state(
    dim, dstate, has_z, itype, max_seq_len
):
    device = "cuda"
    rtol, atol = (3e-4, 1e-3) if itype == torch.float32 else (5e-3, 1e-2)
    ...
    total_state_slots = 30
    state = torch.randn(total_state_slots, dim, dstate, dtype=itype, device=device)

    initial_state_slots = torch.randint(
        0, 10, (batch_size,), device=device, dtype=torch.int32
    )
    ...
    for seq_idx in range(batch_size):
        state_batch_indices[seq_idx, 0] = initial_state_slots[seq_idx]
    ...
    for seq_idx in range(batch_size):
        ...
        initial_slot = state_batch_indices[seq_idx, 0].item()
    ...
    for seq_idx in range(batch_size):
        ...
        for token_idx in range(num_tokens):
            dst_slot = dst_state_batch_indices[seq_idx, token_idx].item()
