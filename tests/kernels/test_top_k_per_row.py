...
def create_random_logits(row_starts: torch.Tensor, row_ends: torch.Tensor, vocab_size: int, dtype: torch.dtype, seed: int) -> torch.Tensor:
...
def compare_top_k_results(cuda_indices: torch.Tensor, cuda_values: torch.Tensor,
                         torch_indices: torch.Tensor, torch_values: torch.Tensor,
                         row_starts: torch.Tensor, row_ends: torch.Tensor,
                         top_k: int, tolerance: float = 1e-5) -> bool:
    """
    ...
    for row_idx in range(num_rows):
        ...
        if len(cuda_values) != len(torch_values):
            print(f"Row {row_idx} has {len(cuda_values)} values in CUDA and {len(torch_values)} values in Torch")
            return False
        if not torch.allclose(torch.tensor(cuda_values), torch.tensor(torch_values), rtol=tolerance, atol=tolerance):
            print(f"CUDA values: {cuda_values} don't match Torch values: {torch_values}")
            return False
...
def test_top_k_per_row(
    ...
    torch.ops._C.top_k_per_row(
        ...
        num_rows, top_k, logits.stride(0), logits.stride(1)
    )
