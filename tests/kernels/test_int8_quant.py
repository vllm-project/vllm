...
def opcheck_int8_quant_dynamic(output, input, symmetric=True):
    ...
    if symmetric:
        opcheck(
            torch.ops._C.dynamic_scaled_int8_quant, (output, input, scale, None)
        )
    else:
        azp = torch.empty(
            (input.numel() // input.shape[-1], 1),
        ...
        opcheck(
            torch.ops._C.dynamic_scaled_int8_quant, (output, input, scale, azp)
        )
...
def test_dynamic_scaled_int8_azp_quant(
    ...
    x = (
        torch.rand(num_tokens, hidden_size, dtype=dtype, device="cuda") * 1000
        - 300
    )
...
def test_static_scaled_int8_azp_quant(
    ...
    out2, scale2, azp2 = scaled_int8_quant(
...
def test_static_scaled_int8_azp_quant_saturating_cast(is_max: bool) -> None:
    ...
    x_vals = [
        [nextafter(val, inf), val + 1, val, val - 1, nextafter(val, -inf)]
    ]
