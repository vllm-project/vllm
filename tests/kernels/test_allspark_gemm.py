...
from vllm.model_executor.layers.quantization.utils.allspark_utils import (
    ALLSPARK_AMPERE_K_ALIGN, ALLSPARK_AMPERE_N_ALIGN)
...
def test_gptq_allspark_gemm_ampere(mnk_factors, group_size, has_zp, dtype):
    ...
    opcheck(torch.ops._C.allspark_a16w8_gemm,
            (input, qw_reorder, s_reorder, zp_reorder, n, group_size, sm_count,
             sm_version, 1024, has_zp, True, weight_name_pattern),
            test_utils=DEFAULT_OPCHECK_TEST_UTILS)
    output = ops.allspark_a16w8_gemm(input, qw_reorder, s_reorder, zp_reorder,
                                     n, group_size, sm_count, sm_version, 1024,
