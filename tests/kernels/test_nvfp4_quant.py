...
def ref_nvfp4_quant(x, global_sf):
    ...
    scale = global_sf * (vec_max * get_reciprocal(FLOAT4_E2M1_MAX))
    scale = scale.to(torch.float8_e4m3fn).to(torch.float32)
    output_scale = get_reciprocal(scale * get_reciprocal(global_sf))
...
def recover_swizzled_scales(sf, m, n):
    assert sf.dtype == torch.int32
    rounded_m = ((m + 128 - 1) // 128) * 128
    scale_n = n // BLOCK_SIZE
    rounded_n = ((scale_n + 4 - 1) // 4) * 4
    # Recover the swizzled scaling factor to linear layout
    tmp = sf.view(torch.float8_e4m3fn)
    tmp = torch.reshape(tmp, (1, rounded_m // 128, rounded_n // 4, 32, 4, 4))
