...
def maybe_make_int_tensor(
    ...
    * None otherwise
    ...
    return (
        None
        if _list is None
        else torch.tensor(_list, dtype=torch.int, device=device)
    )
...
def maybe_make_long_tensor(
    ...
    * None otherwise
    ...
    return (
        None
        if _list is None
        else torch.tensor(_list, dtype=torch.long, device=device)
    )
...
def make_qkv(
    ...
    * Decode QKVInputs structure (containing all only the last sequence offset)
    ...
    query = torch.rand((batch_size, max_q_seq_len, num_heads, head_size)).to(
        device
    )
    key = torch.rand((batch_size, max_kv_seq_len, num_heads, head_size)).to(
        device
    )
    value = torch.rand((batch_size, max_kv_seq_len, num_heads, head_size)).to(
        device
    )

    prefill_query = torch.zeros(
        (batch_size, max_q_seq_len, num_heads, head_size)
    ).to(device)
    prefill_key = torch.zeros(
        (batch_size, max_kv_seq_len, num_heads, head_size)
    ).to(device)
    ...
    for bdx, (q_seq_len, kv_seq_len) in enumerate(zip(q_seq_lens, kv_seq_lens)):
        ...
        decode_query[bdx, :, :, :] = query[
            bdx, (q_seq_len - 1) : q_seq_len, :, :
        ]
        decode_key[bdx, :, :, :] = key[bdx, (kv_seq_len - 1) : kv_seq_len, :, :]
        decode_value[bdx, :, :, :] = value[
            bdx, (kv_seq_len - 1) : kv_seq_len, :, :
        ]
...
def pack_qkv(
    qkv: QKVInputs, device: Union[torch.device, str]
) -> PackedQKVInputs:
    """
    ...
    * Packed (number_of_tokens x num_heads x head_size) QKV inputs
    ...
    packed_key, kv_start_loc_list = pack_tensor(
        qkv.key, qkv.kv_seq_lens, device=device
    )
...
def make_test_metadata(
    ...
    * AttentionMetadata structure
    ...
    kv_mmap = (
        None if decoder_test_params is None else decoder_test_params.kv_mmap
    )
    ...
    num_prefill_or_decode_tokens = (
        None
        if seq_lens is None
        else (sum(seq_lens) if is_prompt else len(seq_lens))
    )
...
def fp8_allclose(
    ...
    """
    torch._refs._check_close_args(
        name="torch.allclose", a=a, b=b, rtol=rtol, atol=atol
    )
        ...
        for i, s in enumerate(shape):
            if t.shape[i] != s and t.shape[i] != 1:
                ...
                t = (
                    t.unsqueeze(i + 1)
                    .expand(
                        *t.shape[: i + 1], s // t.shape[i], *t.shape[i + 1 :]
                    )
                    .flatten(i, i + 1)
                )
        return t
