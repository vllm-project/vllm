...
# N and K correspond to the size of the weight matrix and likely to be multiples of a large power of two.
# In any case, the kernel will have a naive fallback when N and K are not divisible by 16
# But M is the number of tokens and the kernel must handle any M thrown at it.
@pytest.mark.parametrize("per_act_token", [True, False])
@pytest.mark.parametrize("per_out_ch", [True, False])
@pytest.mark.skipif(capability < 89, reason="FP8 is not supported on this GPU type.")
