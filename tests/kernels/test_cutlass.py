...
@pytest.mark.parametrize("out_dtype", [torch.bfloat16, torch.float16])
@pytest.mark.skip
def test_cutlass_int8_azp_bias_fold(
...
def test_cutlass_int8_azp(
    ...
    azp_a = (
        torch.rand((m_azp, 1), device="cuda", dtype=torch.float32) * 10 + 1.5
    )
    azp_aq_i8 = (azp_a / scale_a).to(dtype=torch.int8)
    azp_a = azp_aq_i8.to(dtype=torch.float32) * scale_a  # correct for rounding
    ...
    torch.testing.assert_close(
        a_dq, scale_a * aq_f32 - azp_a, rtol=1e-4, atol=1e-3
    )
...
def test_cutlass_cuda_graph(per_act_token: bool, per_out_ch: bool):
    ...
    scale_a = (
        torch.randn((m_a_scales, 1), device="cuda", dtype=torch.float32) / 10
    )
    scale_b = (
        torch.randn((1, n_b_scales), device="cuda", dtype=torch.float32) / 10
    )
