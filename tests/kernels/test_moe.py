...
def test_fused_moe(
    ...
    if ep_size > 1:
        local_e = e // ep_size
        e_ids = torch.randint(
            0, e, (local_e,), device="cuda", dtype=torch.int32
        )
    ...
    torch.testing.assert_close(
        iterative_output, torch_output, atol=2e-2, rtol=0
    )
...
def test_fused_moe_wn16(
    ...
    w1_scales = torch.empty(
        (e, 2 * n, k // group_size), device="cuda", dtype=dtype
    )
...
def test_fused_marlin_moe(
    ...
    quant_type = (
        scalar_types.uint4b8 if num_bits == 4 else scalar_types.uint8b128
    )
    dtype = torch.float16
    a = torch.randn((m, k), device="cuda", dtype=dtype) / 10
    w1 = torch.randn((e, 2 * n, k), device="cuda", dtype=dtype) / 10
    ...
    if ops.supports_moe_ops:
        token_expert_indicies = torch.empty(
            m, topk, dtype=torch.int32, device=a.device
        )
        ...
        zp = torch.empty(
            (0, 0), dtype=dtype, device="cuda", requires_grad=False
        )
        opcheck(
...
def test_moe_align_block_size_opcheck():
    ...
    topk_ids = torch.randint(
        0, num_experts, (3, 4), dtype=torch.int32, device="cuda"
    )
