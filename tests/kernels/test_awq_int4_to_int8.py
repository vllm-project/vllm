...
def make_awq_checkpoint_data(K, N, group_size, seed=42):
    ...
    return (packed_qweight, packed_qzeros, scales,
            float_ref, weight_int4_orig, zeros_int4_orig)
...
class TestConvertWeightPackedScaleZp:
    ...
    def test_packing_output_shapes(self, K, N, group_size):
        ...
        (packed_qweight, packed_qzeros, scales,
         _, _, _) = make_awq_checkpoint_data(K, N, group_size)
        ...
        print(f"  [PASS] packing shapes K={K}, N={N}, gs={group_size}: "
...
class TestInt4ScaledMmCpu:
    ...
    def test_gemm_vs_float_reference(self, M, K, N, group_size):
        ...
        (packed_qweight, packed_qzeros, scales,
         float_ref, _, _) = make_awq_checkpoint_data(K, N, group_size)
        ...
        out = torch.ops._C.int4_scaled_mm_cpu(
            x, blocked_w, blocked_zp, blocked_s, None)
    ...
    def test_gemm_fp16_input(self):
        ...
        blocked_w, blocked_zp, blocked_s = (
            torch.ops._C.convert_weight_packed_scale_zp(packed_qweight, packed_qzeros, scales))
