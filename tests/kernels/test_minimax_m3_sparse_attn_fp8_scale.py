...
def _make_kv_cache(num_blocks: int, seed: int):
    ...
    kv_fp8[:, 0] = (kv_ref[:, 0].float() / K_SCALE).to(fp8_dtype)
    kv_fp8[:, 1] = (kv_ref[:, 1].float() / V_SCALE).to(fp8_dtype)

    kv_dequant = torch.empty_like(kv_ref)
    kv_dequant[:, 0] = (kv_fp8[:, 0].float() * K_SCALE).to(DTYPE)
    kv_dequant[:, 1] = (kv_fp8[:, 1].float() * V_SCALE).to(DTYPE)
