...
def test_selective_scan(
    ...
    state = torch.randn(
        state_shape, device=u.device, dtype=itype, requires_grad=False
    )
    ...
    for c in range(scan_chunks):
        ...
        out = selective_scan_fn(
            ...
            has_initial_state=torch.ones(
                batch_size, device=u.device, dtype=torch.bool
            )
            if c > 0
            else None,
        )
...
def test_selective_scan_varlen(
    ...
    state_indices = torch.randperm(
        total_entries, dtype=torch.int32, device=u.device
    )[:batch_size]
...
def test_selective_state_update_with_batch_indices(
    ...
    print(
        "Output state diff mean", (state[state_indices, :] - state_ref).mean()
    )
    # test padded entries stay the same
    if with_padding:
        assert torch.equal(
            state_before[unused_states_bool], state[unused_states_bool]
        )
