...
@pytest.mark.parametrize("num_tokens", [17, 4096])
def test_sparse_prefill_reuses_padded_workspace(monkeypatch, num_tokens):
    """Padding and kernel output must not allocate beyond the two FP32 stats."""
    ...
    num_heads = 32
    padded_heads = 128 if current_platform.is_device_capability_family(100) else 64
    ...
    impl.prefill_padding = padded_heads
    impl.prefill_buffer_index = 0
    impl.softmax_scale = 576**-0.5
    impl.workspace_specs = [
        ((4096, padded_heads, 576), torch.bfloat16),
        ((4096, padded_heads, 512), torch.bfloat16),
    ]
    _, output_buffer = manager.get_simultaneous(*impl.workspace_specs)
    manager.lock()

    q = torch.randn(num_tokens, num_heads, 576, dtype=torch.bfloat16, device=device)
    kv = torch.randn(64, 1, 576, dtype=torch.bfloat16, device=device)
    indices = torch.arange(64, dtype=torch.int32, device=device)
    indices = indices.expand(num_tokens, 64).contiguous()
    reference_q = torch.zeros(
        num_tokens, padded_heads, 576, dtype=q.dtype, device=device
    )
    reference_q[:, :num_heads].copy_(q)
    reference, _, reference_lse = fm.flash_mla_sparse_fwd(
        reference_q, kv, indices.unsqueeze(1), impl.softmax_scale
    )

    # Warm up before checking steady-state allocations and synchronization.
    actual, lse = impl._bf16_flash_mla_kernel(
        q, kv, indices, out=output_buffer[:num_tokens]
    )
    del actual, lse
    ...
    stats_bytes = 2 * num_tokens * padded_heads * torch.float32.itemsize
    assert peak_delta <= stats_bytes + 1024
    assert actual.data_ptr() == output_buffer.data_ptr()
    torch.testing.assert_close(actual, reference[:, :num_heads], rtol=0, atol=0)
    torch.testing.assert_close(lse, reference_lse[:, :num_heads], rtol=0, atol=0)

    # The flat model consumes this head-sliced view through a strided BMM.
    weight = torch.randn(num_heads, 512, 16, dtype=q.dtype, device=device)
    projected = torch.bmm(actual.transpose(0, 1), weight)
    expected = torch.bmm(reference[:, :num_heads].transpose(0, 1), weight)
