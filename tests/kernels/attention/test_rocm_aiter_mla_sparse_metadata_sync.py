...
def _make_builder():
    ...
    builder.qo_indptr = torch.arange(
        max_num_batched_tokens + 1, dtype=torch.int32
    )
    builder.paged_kv_last_page_len = torch.ones(
        max_num_batched_tokens, dtype=torch.int32
    )
    builder.paged_kv_indices = torch.zeros(
        max_num_batched_tokens * topk_tokens, dtype=torch.int32
    )
    builder.paged_kv_indptr = torch.zeros(
        max_num_batched_tokens + 1, dtype=torch.int32
    )
    builder._num_attention_heads = 16
    builder._mla_work_meta_data = torch.empty(1, dtype=torch.int32)
    builder._mla_work_indptr = torch.empty(1, dtype=torch.int32)
    builder._mla_work_info_set = torch.empty(1, dtype=torch.int32)
    builder._mla_reduce_indptr = torch.empty(1, dtype=torch.int32)
    builder._mla_reduce_final_map = torch.empty(1, dtype=torch.int32)
    builder._mla_reduce_partial_map = torch.empty(1, dtype=torch.int32)
...
def test_sparse_persistent_metadata_syncs_only_after_recompute(monkeypatch):
    ...
    fake_aiter.get_mla_metadata_v1 = Mock(side_effect=fake_get_mla_metadata_v1)
    ...
    assert fake_aiter.get_mla_metadata_v1.call_count == 1
