...
def kernel_unified_attention_2d(
    ...
    for j in range(tile_start, tile_end):
        ...
        if use_native_fp4 >= 3: # PV quant to fp4 will significantly increase error
           p_zoom_scale: tl.constexpr = 1.0 #200.0
           if use_native_fp4 == 3:
              P1 = tl.full(shape=P.shape, value=1, dtype=tl.int32)
              Pz = P*p_zoom_scale
              p_fp4, scale_p = _compute_quant_and_scale(Pz, P1, tl.uint8,2) if (MXOP_SEL == 1) else \
                            _mxfp4_quant_op(Pz, HEAD_SIZE_PADDED, BLOCK_M, 32)

           Vt = tl.trans(V)
           V1 = tl.full(shape=Vt.shape, value=1, dtype=tl.int32)
           v_fp4, scale_v = _compute_quant_and_scale(Vt, V1, tl.uint8,2) if (MXOP_SEL == 1) else \
                            _mxfp4_quant_op(Vt, TILE_SIZE, HEAD_SIZE_PADDED, 32)
           ...
           # K_RHS, N = rhs.type.shape[-2:]
           if use_native_fp4 == 3:
              acc_fp4 = tl.dot_scaled(p_fp4, scale_p, "e2m1", v_fp4_t, scale_v, "e2m1", acc*p_zoom_scale, lhs_k_pack=PACK_ALONG_K,
                                   rhs_k_pack=PACK_ALONG_K, out_dtype=tl.float32) / p_zoom_scale
           else:
              P_fp8 = (P*p_zoom_scale).to(tl.float8e4nv)
              V_fp8 = Vt.to(tl.float8e4nv)
              if V_fp8 is None:
                 acc_fp4 = tl.dot_scaled(p_fp4, scale_p, "e2m1", v_fp4_t, scale_v, "e2m1", acc*p_zoom_scale, lhs_k_pack=PACK_ALONG_K,
                                  rhs_k_pack=PACK_ALONG_K, out_dtype=tl.float32) / p_zoom_scale
              else:
                 acc_fp4 = tl.dot_scaled(P_fp8, None, "e4m3", V_fp8, None, "e4m3", acc*p_zoom_scale, lhs_k_pack=PACK_ALONG_K,
                                   rhs_k_pack=PACK_ALONG_K, out_dtype=tl.float32) / p_zoom_scale
           if pid0 < -1 and pid1 < 1 and j == 0 and test_id == 0:
...
def unified_attention(
    ...
    use_native_fp4 = int(os.getenv('MXFP4_OPTION', '1'))
    if total_print == 0:
       print(f" ******* Value of use_native_fp4: {use_native_fp4} ****** ")
    ...
    if max_seqlen_q > 1 or total_num_q_blocks * num_kv_heads > 128:
        ...
        if total_print == 2:
            ...
            print(f"Elapsed time: {elapsed_time*1000:.6f} milli-seconds")
    else:
