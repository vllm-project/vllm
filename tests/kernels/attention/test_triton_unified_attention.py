...
# Regression: the decode q-block grid must cover every sequence. BLOCK_Q > 1
# (any GQA model) exposed a grid that dropped the last sequences.
@pytest.mark.parametrize("num_seqs", [8, 64, 256])
@pytest.mark.parametrize("num_heads", [(8, 2), (5, 1), (8, 8)])
@pytest.mark.parametrize("seq_threshold_3D", [0, 512])  # 0 -> 2D, 512 -> 3D
def test_triton_unified_attn_high_batch_decode(
    num_seqs: int,
    num_heads: tuple[int, int],
    seq_threshold_3D: int,
) -> None:
    torch.set_default_device(DEVICE_TYPE)
    set_random_seed(0)
    head_size, block_size, kv_len = 128, 16, 1024
    ...
    query = torch.randn(num_seqs, nq, head_size, dtype=dtype)
    key_cache = torch.randn(num_blocks, block_size, nkv, head_size, dtype=dtype)
    value_cache = torch.randn_like(key_cache)
    cu_q = torch.arange(num_seqs + 1, dtype=torch.int32)
    kv_lens_t = torch.tensor(kv_lens, dtype=torch.int32)
    max_blk = (kv_len + block_size - 1) // block_size
    block_tables = torch.randint(0, num_blocks, (num_seqs, max_blk), dtype=torch.int32)

    # poison the output so any unwritten row is a loud failure, not a lucky zero
    output = torch.full_like(query, float("nan"))
    npar = 16
    hsp = next_power_of_2(head_size)
    thr = seq_threshold_3D
    segm_o = torch.empty((max(thr, 1), nq, npar, hsp), dtype=torch.float32)
    segm_m = torch.empty((max(thr, 1), nq, npar), dtype=torch.float32)
    segm_e = torch.empty((max(thr, 1), nq, npar), dtype=torch.float32)
    ...
    unified_attention(
        ...
        num_par_softmax_segments=npar,
        softmax_segm_output=segm_o,
        softmax_segm_max=segm_m,
        softmax_segm_expsum=segm_e,
    )
    ...
    assert not torch.isnan(output).any(), "some decode sequences were never written"
