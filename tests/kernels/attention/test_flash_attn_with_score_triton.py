...
def reference_varlen_key_importance(q, k, cu_seqlens, softmax_lse, scale=None):
    ...
    Args:
        ...
        softmax_lse: [heads, total_tokens] - Packed format (matches flash_attn_varlen_func output)
        scale: softmax scaling factor
    ...
    # Initialize output
    key_importance = torch.zeros(total_tokens, heads, dtype=torch.float32, device=q.device)
...
class TestTokenImportanceTriton:
    ...
    def test_compute_token_importance_basic(
        ...
        from vllm.model_executor.layers.attention.compute_flash_attn_score_triton import (
            compute_token_importance_triton,
        )
        ...
        computed_importance = compute_token_importance_triton(
        ...
        torch.testing.assert_close(
            computed_importance, ref_importance,
            rtol=1e-3, atol=1e-3
        )
    ...
    def test_compute_token_importance_transposed_format(self, seq_len, nheads):
        """Test token importance with transposed input format [batch, seq, heads, dim]."""
        from vllm.model_executor.layers.attention.compute_flash_attn_score_triton import (
            compute_token_importance_triton,
        )
        ...
        q = torch.randn(
            batch_size, seq_len, nheads, head_dim,
            device=device, dtype=torch.float16
        )
        k = torch.randn(
            batch_size, seq_len, nheads, head_dim,
        ...
        # Compute logsumexp
        qk = torch.einsum('bhqd,bhkd->bhqk', q_transposed.float(), k_transposed.float()) * scale
        softmax_lse = torch.logsumexp(qk, dim=-1)  # [batch, heads, seq_q]
        ...
        # If input was in [batch, seq, heads, dim] format, output may be [batch, seq_k, heads]
        # Transpose to match reference format [batch, heads, seq_k]
        # if computed_importance.shape[1] != softmax_lse.shape[1]:  # heads dimension mismatch
        #     computed_importance = computed_importance.transpose(1, 2)
        
        # Compute reference
        ref_importance = reference_key_importance(q_transposed, k_transposed, softmax_lse, scale)
    ...
    def test_compute_token_importance_different_qk_lengths(self):
        ...
        q = torch.randn(
            batch_size, nheads, q_len, head_dim,
            device=device, dtype=torch.float16
        )
        k = torch.randn(
            batch_size, nheads, k_len, head_dim,
            device=device, dtype=torch.float16
        )
...
class TestVarlenImportanceTriton:
    ...
    def test_compute_varlen_importance_basic(self, batch_size):
        ...
        softmax_lse = torch.zeros(nheads, total_tokens, dtype=torch.float32, device=device)
        for b in range(batch_size):
            start_idx = cu_seqlens[b].item()
            end_idx = cu_seqlens[b + 1].item()
            seq_len = end_idx - start_idx
        ...
        # Compute token importance using Triton
        computed_importance = compute_varlen_importance(
        ...
        # Verify values
        torch.testing.assert_close(
            computed_importance, ref_importance_mean,
            rtol=1e-3, atol=1e-3
        )
    ...
    def test_compute_varlen_importance_single_sequence(self):
        ...
        qk = torch.einsum('hqd,hkd->hqk', q_transposed.float(), k_transposed.float()) * scale
