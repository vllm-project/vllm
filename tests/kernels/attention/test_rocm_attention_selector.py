...
def test_selector(monkeypatch: pytest.MonkeyPatch):
    ...
    vllm_config = VllmConfig(
        attention_config=attention_config, cache_config=CacheConfig(block_size=16)
    )
    ...
    with set_current_vllm_config(vllm_config):
        backend = get_attn_backend(576, torch.bfloat16, "auto", use_mla=True)
    ...
    vllm_config = VllmConfig(
        attention_config=attention_config, cache_config=CacheConfig(block_size=1)
    )
