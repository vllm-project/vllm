...
def test_fast_plan_decode_non_tensor_core_gqa(
    ...
    # Ground truth from the reference implementation
    gt_output = ref_paged_attn(
        query=query,
        key_cache=key_cache,
    ...
    # CUDAGraph wrapper with use_tensor_cores=False (fa2 GQA backend)
    kv_indices_buf = kv_indices.clone()
    workspace_cg = torch.empty(128 * 1024 * 1024, dtype=torch.int8)
    cg_wrapper = _make_cg_decode_wrapper(
        num_seqs, kv_indices_buf, workspace_cg, use_tensor_cores=False
    )
    ...
    # Warmup call (self.plan path)
    fast_plan_decode(cg_wrapper, **plan_kwargs)
    warmup_output = cg_wrapper.run(query, key_value_cache)
    torch.testing.assert_close(warmup_output, gt_output, atol=1e-2, rtol=1e-2)

    # Subsequent call (fast_decode_plan path) with fa2 backend
    fast_plan_decode(cg_wrapper, **plan_kwargs)
    fast_output = cg_wrapper.run(query, key_value_cache)
    torch.testing.assert_close(fast_output, gt_output, atol=1e-2, rtol=1e-2)
