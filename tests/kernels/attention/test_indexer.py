...
def deepgemm_mqa_indexer(seq_len, query_seq_len, q, kv, weights, block_size, topk, is_kv_batched=True):
    B = seq_len.shape[0]
    concat_q = []
    concat_kv = []
    concat_weights = []
...
def test_prefill_indexer():
    ...
    deepgemm_logits = deepgemm_mqa_indexer(seq_len, seq_len, q, kv, weights, block_size, topk)
...
def test_decode_paged_indexer():
    ...
    flatten_kv = torch.empty(
        [seq_len.sum(), D], device="cuda", dtype=torch.bfloat16
    )
    ...
    ref_logits = deepgemm_mqa_indexer(seq_len, query_seq_len, q, flatten_kv, weights, block_size, topk, is_kv_batched=False)
    ...
    logits = fp8_paged_mqa_logits(
        q_fp8, kv_cache_fp8, weights, seq_len.int(), block_tables,
        schedule_metadata, 4096)
