...
@torch.inference_mode()
def test_fa4_fp8_per_head_descales() -> None:
    """FA4 must apply non-unity per-KV-head FP8 Q/K/V descales."""
    torch.set_default_device("cuda")
    if not is_fa_version_supported(4):
        pytest.skip(
            f"Flash attention version 4 is not supported due "
            f'to: "{fa_version_unsupported_reason(4)}"'
        )
    ...
    query = torch.randn(query_len, num_query_heads, head_size)
    key = torch.randn(kv_len, num_kv_heads, head_size)
    value = torch.randn_like(key)
    q_descale = torch.tensor([[0.125, 2.0]], dtype=torch.float32)
    ...
    output = flash_attn_varlen_func(
        ...
        cu_seqlens_k=torch.tensor([0, kv_len], dtype=torch.int32),
    ...
    query_head_to_kv = torch.arange(num_query_heads) // (
        num_query_heads // num_kv_heads
    )
    key_broadcast = key[:, query_head_to_kv]
    value_broadcast = value[:, query_head_to_kv]
    scores = torch.einsum("qhd,khd->hqk", query, key_broadcast) * scale
