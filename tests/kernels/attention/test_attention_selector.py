                ...
                if use_mla:
                    ...
                    if name == "TRITON_MLA" and block_size == 1:
                        ...
                        with pytest.raises(ValueError) as exc_info:
                            get_attn_backend(16, torch.float16, None, use_mla=use_mla)
                        assert f"The selected backend, {name}" in str(exc_info.value)
                    else:
                        # Valid backend-block_size combination
                        backend = get_attn_backend(
                            16, torch.float16, None, use_mla=use_mla
                        )
                        expected = name
                        assert backend.get_name() == expected
                else:
                    backend = get_attn_backend(16, torch.float16, None, use_mla=use_mla)
                    expected = "ROCM_ATTN"
                    assert backend.get_name() == expected
