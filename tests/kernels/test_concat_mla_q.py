...
def test_concat_mla_q_contiguous(num_tokens, num_heads, nope_dim, rope_dim,
                                  dtype):
    """Test with contiguous inputs (standard layout)."""
    torch.manual_seed(42)
    ql_nope = torch.randn(num_tokens, num_heads, nope_dim, dtype=dtype,
                          device="cuda")
...
@pytest.mark.parametrize("rope_dim", ROPE_DIM)
@pytest.mark.parametrize("dtype", DTYPES)
def test_concat_mla_q_transposed_nope(num_tokens, num_heads, nope_dim,
                                       rope_dim, dtype):
    ...
    """
    torch.manual_seed(42)
    nope_raw = torch.randn(num_heads, num_tokens, nope_dim, dtype=dtype,
                           device="cuda")
...
def test_concat_mla_q_split_rope(num_tokens, num_heads, dtype):
    ...
    q_orig = torch.randn(num_tokens, num_heads, orig_dim, dtype=dtype,
                         device="cuda")
...
def test_concat_mla_q_values_preserved(num_tokens):
    ...
    ql_nope_bits = torch.arange(num_tokens * 128 * nope_dim, dtype=torch.int16,
                                device="cuda").view(num_tokens, 128, nope_dim)
    q_pe_bits = torch.arange(num_tokens * 128 * rope_dim, dtype=torch.int16,
                             device="cuda").view(num_tokens, 128, rope_dim)

    ql_nope = ql_nope_bits.view(torch.bfloat16)
    q_pe = q_pe_bits.view(torch.bfloat16)

    q_out = torch.empty(num_tokens, 128, nope_dim + rope_dim,
