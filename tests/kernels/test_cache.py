...
def test_copy_blocks(
    ...
    for value_cache, cloned_value_cache in zip(
        value_caches, cloned_value_caches
    ):
...
def test_reshape_and_cache_flash(
    ...
    if kv_cache_dtype == "fp8":
        cloned_key_cache = torch.empty_like(key_cache, dtype=torch.float16)
        ops.convert_fp8(
            cloned_key_cache, key_cache, k_scale.item(), kv_cache_dtype
        )
        cloned_value_cache = torch.empty_like(value_cache, dtype=torch.float16)
        ops.convert_fp8(
            cloned_value_cache, value_cache, v_scale.item(), kv_cache_dtype
        )
    else:
        cloned_key_cache = key_cache.clone()
        cloned_value_cache = value_cache.clone()
...
def test_gather_cache_mla(
    ...
    seq_len_tensor = torch.randint(
        0, max_seq_len + 1, (batch_size,), device=device
    )
