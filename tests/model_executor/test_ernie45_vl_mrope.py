...
from vllm.model_executor.models.ernie45_vl import Ernie4_5_VLMoeForConditionalGeneration
...
def make_mm_feature(
    *, modality: str, offset: int, length: int, grid_thw: tuple[int, int, int]
) -> MultiModalFeatureSpec:
...
def test_get_mrope_input_positions_interleaved_image_and_video():
    ...
    mm_features = [
        make_mm_feature(modality="image", offset=1, length=4, grid_thw=(1, 4, 4)),
        make_mm_feature(modality="video", offset=7, length=2, grid_thw=(2, 4, 2)),
    ]
