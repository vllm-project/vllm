...
class NeverSelectsCannotImplementMixin:
    ...
    @classmethod
    def can_implement(cls, config):
        return False, "cannot implement this config"
...
def _fp_config():
    qk = QuantKey(
        ...
        scale=ScaleDesc(
            dtype=torch.float32, static=True, group_shape=GroupShape.PER_TENSOR
        ),
        symmetric=True,
    )
    return w8a8.FpKernelConfig(
        weight_quant_key=qk, activation_quant_key=qk, out_dtype=torch.float16
    )


def _int_config():
    return w8a8.IntKernelConfig(
        is_channelwise=True, is_static_input_scheme=True, input_symmetric=True
    )
...
def test_raises_when_no_kernel_available(ctx):
    ...
    with pytest.raises(ValueError, match="Failed to find a kernel"):
        choose_w8a8_linear_kernel(
            config, [unsupported, cannot_implement], compute_capability=HIGH_CAP
        )
...
def test_forced_kernel_is_selected_when_available(ctx):
    config, always_a, always_b, *_ = ctx
    assert (
        choose_w8a8_linear_kernel(config, [always_a, always_b], force_kernel=always_b)
        == always_b
    )
...
def test_forced_kernel_fallback_when_unavailable(ctx):
    config, always_a, _, unsupported, *_ = ctx
    assert (
        choose_w8a8_linear_kernel(config, [always_a], force_kernel=unsupported)
        == always_a
    )
...
def test_compute_capability_filtering(ctx):
    ...
    assert (
        choose_w8a8_linear_kernel(
            config, [requires_high_cap, always_a], compute_capability=LOW_CAP
        )
        == always_a
    )
    assert (
        choose_w8a8_linear_kernel(
            config, [requires_high_cap, always_a], compute_capability=HIGH_CAP
        )
        == requires_high_cap
    )
