...
def test_get_layer_params_supports_heterogeneous_config() -> None:
    config = SimpleNamespace(
        ...
        per_layer_config=[
            SimpleNamespace(head_dim=64, num_key_value_heads=4),
            SimpleNamespace(head_dim=128, num_key_value_heads=2),
            SimpleNamespace(head_dim=64, num_key_value_heads=4),
        ],
        attention_k_eq_v=True,
        intermediate_size=1024,
    ...
    assert len(params) == config.num_hidden_layers
    assert [p["head_dim"] for p in params] == [64, 128, 64]
    assert [p["num_kv_heads"] for p in params] == [4, 2, 4]
    assert [p["has_v_proj"] for p in params] == [True, False, True]
