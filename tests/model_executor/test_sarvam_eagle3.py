...
def test_sarvam_mla_advertises_eagle3_support():
    """Allow the runner to detect the EAGLE3 interface."""
    assert supports_eagle3(SarvamMLAForCausalLM)


def test_sarvam_mla_configures_aux_hidden_state_layers():
    """Propagate auxiliary capture configuration to the inner model."""
...
def test_sarvam_mla_forward_captures_aux_hidden_states(monkeypatch):
    """Capture embeddings and complete layer outputs for the drafter."""
...
def test_sarvam_mla_forward_returns_bare_tensor_without_eagle3(monkeypatch):
    """Preserve the tensor return contract when auxiliary capture is disabled."""
...
def test_sarvam_mla_forward_uses_absolute_layer_indices(monkeypatch):
    """Use global capture indices within one simulated pipeline stage.

    This checks local indexing, not cross-stage EAGLE3 state transport.
    """
...
def test_sarvam_mla_pipeline_forward_without_eagle3(monkeypatch):
    """Preserve hidden states and residual across two stages without EAGLE3."""
    inputs_embeds = torch.tensor([[1.0, 2.0]])
    positions = torch.tensor([0])
    hidden_states = torch.tensor([[3.0, 4.0]])
    residual = torch.tensor([[5.0, 6.0]])
    first = _make_model(layers=[_layer(hidden_states, residual)])
    final_layer = Mock(side_effect=lambda h, p, r: (h * 2, r + 1))
    last = _make_model(start_layer=1, end_layer=2, layers=[None, final_layer])
    _patch_pp_group(monkeypatch, is_last_rank=False)

    intermediate = first.forward(
        input_ids=None,
        positions=positions,
        intermediate_tensors=None,
        inputs_embeds=inputs_embeds,
    ...
    torch.testing.assert_close(intermediate["hidden_states"], hidden_states)
    torch.testing.assert_close(intermediate["residual"], residual)
    _patch_pp_group(monkeypatch, is_first_rank=False)
    ...
    assert isinstance(output, torch.Tensor)
    torch.testing.assert_close(output, hidden_states * 2 + residual + 1)
