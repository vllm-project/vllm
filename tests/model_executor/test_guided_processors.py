...
def test_guided_logits_processors(
    zephyr_7B_tokenzer, sample_regex, sample_json_schema
):
    """Basic unit test for RegexLogitsProcessor and JSONLogitsProcessor."""
    regex_LP = RegexLogitsProcessor(
        sample_regex, zephyr_7B_tokenzer, reasoner=None
    )
...
async def test_guided_logits_processor_black_box(
    ...
    json_request = GuidedDecodingParams(
        json=sample_json_schema, backend=backend
    )
...
@pytest.mark.parametrize(
    "backend", GUIDED_DECODING_BACKENDS_WITH_REASONING_SUPPORT
)
@pytest.mark.parametrize("is_local", [True, False])
...
def test_guided_decoding_backend_options():
    ...
    params = GuidedDecodingParams(backend="xgrammar:option-1,option-2,option-3")
