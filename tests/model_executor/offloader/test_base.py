...
def test_set_offloader_none_resets_to_noop(monkeypatch):
    """`set_offloader(None)` resets the singleton back to a NoopOffloader
    without warning: a reset is not a lost configuration."""
    cfg = OffloadConfig(uva=UVAOffloadConfig(cpu_offload_gb=8.0))
    set_offloader(create_offloader(cfg))
    calls = _capture_log_calls(monkeypatch)

    set_offloader(None)

    assert isinstance(get_offloader(), NoopOffloader)
    assert calls["warning_once"] == []
    assert len(calls["debug_once"]) == 1
