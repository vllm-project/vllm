...
"""
End-to-end test for routed experts capture with hybrid models.
...
Usage:
    python tests/model_executor/test_routed_experts_e2e.py \
    ...
    Or with pytest (requires GPU):
    pytest tests/model_executor/test_routed_experts_e2e.py -v -s
"""
...
def test_routed_experts_basic():
    """Basic test: check that routed_experts is returned and valid."""
    ...
    validate_shapes(result.experts_list)
    validate_expert_ids(result.experts_list, num_experts=result.num_experts)


def test_routed_experts_deterministic():
    """Test that two runs with temperature=0 produce the same routing."""
    if not torch.cuda.is_available():
        pytest.skip("CUDA not available")
    ...
    compare_token_ids(baseline.token_ids_list, reference.token_ids_list)
    compare_routed_experts(
        baseline.experts_list, reference.experts_list, threshold=0.05
    )
