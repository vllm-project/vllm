...
def test_qwen38_extended_live_set_is_90kib_per_token():
    est = _qwen38_estimate(8192)
    assert est.peak_live_bytes == 720 * _MIB
    assert est.peak_live_bytes / 8192 == 90 * 1024
    assert est.h_bytes == 192 * _MIB
    assert est.v_new_bytes == est.w_bytes == est.u_bytes == est.a_bytes == 96 * _MIB
    # Output (v_new-sized) and the bf16 copy of the fp32 A stay live at peak.
    assert est.o_bytes == est.v_new_bytes
    assert est.a_cast_bytes == est.a_bytes // 2
