...
"""Unit tests for MergedColumnParallelLinear TP rank synchronization and scale sharding."""
...
@pytest.mark.parametrize("disable_tp, expected_rank, expected_size", [
    (True, 0, 1),
    (False, 3, 4),
])
def test_merged_column_parallel_linear_tp_status(
    ...
    """Verify disable_tp status synchronizes layer and child parameter TP rank and size."""
...
def test_merged_column_parallel_linear_scale_shard_adjustment():
    """Verify GroupQuantScaleParameter uses block-aware shard adjustments during weight_loader_v2."""
    with patch("vllm.model_executor.layers.linear.get_tensor_model_parallel_rank", return_value=0), \
        ...
        layer = MergedColumnParallelLinear(input_size=128, output_sizes=[64, 64], bias=False)
