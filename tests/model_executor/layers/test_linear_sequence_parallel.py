...
def test_column_parallel_linear_gathers_sequence_shards(monkeypatch):
    ...
    layer.quant_method.apply.assert_called_once_with(
        layer, gathered_input, None
    )
...
def test_sequence_parallel_reduce_scatter_does_not_pad(monkeypatch):
    ...
    custom_collective.assert_called_once_with(
        "custom_reduce_scatter", output_parallel
    )
