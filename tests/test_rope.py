...
def test_rope():
    def ref_rope(dim, max_position_embeddings):
        ...
        return torch.cat((emb.cos_cached.squeeze()[:, :dim//2], emb.sin_cached.squeeze()[:, :dim//2]), dim=-1)
    ...
    assert torch.allclose(ref_rope(dim, max_position_embeddings), vllm_rope(dim, max_position_embeddings))

def test_linear_rope():
    def ref_rope(dim, max_position_embeddings, scaling_factor):
        emb = RefLlamaLinearScalingRotaryEmbedding(dim, max_position_embeddings, scaling_factor=scaling_factor)
        return torch.cat((emb.cos_cached.squeeze()[:, :dim//2], emb.sin_cached.squeeze()[:, :dim//2]), dim=-1)

    def vllm_rope(dim, max_position_embeddings, scaling_factor):
        emb = LlamaLinearScalingRotaryEmbedding(dim, max_position_embeddings, scaling_factor=scaling_factor)
        return emb.cos_sin_cache
    
    dim, max_position_embeddings, scaling_factor = 128, 2048, 8.0
    assert torch.allclose(ref_rope(dim, max_position_embeddings, scaling_factor), vllm_rope(dim, max_position_embeddings, scaling_factor))


def test_ntk_rope():
    def ref_rope(dim, max_position_embeddings, scaling_factor):
        emb = RefLlamaDynamicNTKScalingRotaryEmbedding(dim, max_position_embeddings, scaling_factor=scaling_factor)
        return torch.cat((emb.cos_cached.squeeze()[:, :dim//2], emb.sin_cached.squeeze()[:, :dim//2]), dim=-1)

    def vllm_rope(dim, max_position_embeddings, scaling_factor):
        emb = LlamaDynamicNTKScalingRotaryEmbedding(dim, max_position_embeddings, scaling_factor=scaling_factor)
        return emb.cos_sin_cache
