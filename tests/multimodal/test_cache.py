...
from vllm.multimodal.cache import (
    BaseMultiModalProcessorCache, BaseMultiModalReceiverCache, MultiModalCache,
    MultiModalProcessorCacheInItem, MultiModalProcessorCacheItem,
    MultiModalProcessorCacheItemMetadata, MultiModalProcessorSenderCache,
    MultiModalReceiverCache, ShmObjectStoreReceiverCache,
    ShmObjectStoreSenderCache, engine_receiver_cache_from_config,
...
def test_cache_eviction_lru_cache():
    ...
    _run_test_cache_eviction_lru(sender_cache,
                                 receiver_cache,
                                 base_item_size=1)
...
def _run_test_cache_eviction_shm(
    ...
    request2_items = {
        h:
        MultiModalKwargsItem.dummy(h,
                                   nbytes=(5 if h in request1_hashes else 2) *
                                   base_item_size)
        for h in request2_hashes
    }
...
def test_cache_eviction_shm_cache():
    ...
    _run_test_cache_eviction_shm(sender_cache,
                                 receiver_cache,
                                 base_item_size=MiB_bytes)
