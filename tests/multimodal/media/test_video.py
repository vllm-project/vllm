...
# ---------------------------------------------------------------------------
# Lazy video decode tests
# ---------------------------------------------------------------------------


@VIDEO_LOADER_REGISTRY.register("recording_lazy_backend")
class RecordingLazyVideoLoader(VideoLoader):
    """Test loader that records every load_bytes invocation."""
...
def test_load_bytes_lazy_defers_decode(monkeypatch: pytest.MonkeyPatch):
    ...
    processor's parallel decode phase.
    """
    RecordingLazyVideoLoader.calls.clear()
    with monkeypatch.context() as m:
...
def test_load_bytes_lazy_matches_load_bytes(tmp_path, codec_backend: str):
    ...
    video_path = f"{tmp_path}/test_lazy_video.mp4"
    create_video_from_image(str(image_path), video_path, num_frames=4)
...
def test_load_base64_lazy_jpeg_defers_decode(monkeypatch: pytest.MonkeyPatch):
    ...
    num_test_frames = 3
    data = ",".join(_make_jpeg_b64_frames(num_test_frames))

    videoio = VideoMediaIO(ImageMediaIO(), num_frames=num_test_frames)

    calls: list[str] = []
    original_load_base64 = ImageMediaIO.load_base64

    def counting_load_base64(self, media_type, frame_data):
        calls.append(frame_data)
        return original_load_base64(self, media_type, frame_data)
...
@pytest.mark.parametrize(
    ("kwargs", "match"),
    [
        ({"num_frames": 0}, "num_frames must be greater than 0 or -1"),
        ({"num_frames": 3, "frames_indices": [0, 1]}, "frames_indices length"),
        ({"num_frames": 3, "frames_indices": "0,1"}, "list of integers"),
        ({"num_frames": 3, "total_num_frames": 2}, "must be >="),
...
def test_load_base64_lazy_jpeg_enforces_num_frames_limit():
    ...
    assert metadata["total_num_frames"] == 4
    assert metadata["frames_indices"] == list(range(4))


def test_lazy_video_metadata_check_fires_at_decode():
    """Parsing stores metadata=None for lazy video items; the
    video_needs_metadata check fires when the item is decoded, where the
    jpeg_sequence decoder supplies metadata."""
...
def test_lazy_video_without_metadata_raises_at_decode():
