...
def decode_hf(model_name, prompts, beam_width=1, max_new_tokens=16):
    ...
    for prompt in prompts:
        ...
        token_ids.append(generate_ids.tolist())
    return token_ids
...
def test_beam_search_single_input(model_name, max_new_tokens, beam_width, prompt):
    hf_token_ids = decode_hf(model_name, [prompt], beam_width, max_new_tokens)
    vllm_token_ids = decode_vllm(model_name, prompt, beam_width, max_new_tokens)
