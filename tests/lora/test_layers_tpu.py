...
TPU_DEVICES = [
    f"xla:{i}" for i in range(1 if torch.cuda.device_count() == 1 else 2)
]
...
def get_random_id_to_index(num_loras: int,
    ...
    random_slot_selections = (torch.randperm(num_slots, device="cpu")[:num_loras]).tolist()
...
@pytest.mark.parametrize("device", TPU_DEVICES)
...
def test_lm_head_logits_processor(dist_init, num_loras, device, vocab_size,
    ...
    for i in range(10):
        ...
        input_ = torch.rand(20, 1024, dtype=torch.float16)
