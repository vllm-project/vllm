...
def create_lora(
    ...
    for name in sub_modules:
        ...
        if use_dora:
            magnitude_param = torch.abs(torch.rand([w.shape[0]], device=device))
    ...
    for replaced_module_name in replaced_module_names:
        ...
        if use_dora:
            magnitude_param = torch.abs(
                torch.rand(
                    [w.shape[0] // len(replaced_module_names)], device=device
                )
            )
...
@pytest.mark.parametrize("device", DEVICES)
def test_worker_adapter_manager_with_dora(
    llama_2_7b_model_extra_embeddings, dora_files, device
):
...
def test_dora_model_manager(dist_init, dummy_model, device):
    ...
    dora_lora2 = create_lora(
        2, model, ["dense1", "dense2", "lm_head"], device=device, use_dora=True
    )

    # Create a regular LoRA adapter for comparison
    std_lora3 = create_lora(
        3, model, ["dense1", "dense2", "lm_head"], device=device, use_dora=False
    )
    ...
    for module_name, lora_module in dora_lora1.loras.items():
        ...
        assert (
            lora_module.magnitude_param.shape[0] == lora_module.lora_b.shape[1]
        )
...
def test_packed_dora_loras(dist_init, dummy_model_gate_up, device):
    ...
    assert packed_dora_lora and isinstance(
        packed_dora_lora, PackedLoRALayerWeights
    )
    ...
    assert packed_std_lora and isinstance(
        packed_std_lora, PackedLoRALayerWeights
    )
