...
def test_lru_cache():
    ...
    try:
        ...
        assert str(e) == 'Cannot pin key: 10 not in cache.'

    # If all items are pinned,
    # should raise an RuntimeError
    cache.pin(3)
    cache.pin(4)
    cache.pin(5)
    assert set(cache._Cache__data) == {3, 4, 5}
    try:
        ...
        assert str(
            e) == 'All items are pinned, cannot remove oldest from the cache.'
    assert set(cache._Cache__data) == {3, 4, 5}
    ...
    # If the size of the item is greater than the maxsize,
