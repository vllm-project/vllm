...
# @pytest.mark.parametrize("device", CUDA_DEVICES)
@pytest.mark.parametrize("num_loras", [1])
@pytest.mark.parametrize("orientation", ["col"])
...
def test_linear_parallel(dist_init, num_loras, orientation, fully_shard,
    ...
    for i in range(1):
        ...
        # print(f'given {lora_result[:10,:10]}')
        # print(f'correct {expected_result[:10,:10]}')
        # assert False
        # assert torch.allclose(lora_result,
        #                       expected_result,
        #                       rtol=rtol,
...
@pytest.mark.parametrize("fully_shard", [False])
@pytest.mark.parametrize("device", ['cuda:0'])
def test_linear_parallel_(dist_init, num_loras, orientation, fully_shard,
                         device) -> None:
...
# expand throwing an illegal access error, most likely getting an invalid request
# CUDA_LAUNCH_BLOCKING=1 TORCH_USE_CUDA_DSA=1 CUDA_VISIBLE_DEVICES=0 pytest test_layers.py