    ...
    for i in range(10):
        ...
        for input_, lora_id in zip(inputs, prompt_mapping):
            ...
            for i, sublora in enumerate(subloras):
                result[
                    :,
                    sublora.lora_b.shape[1] * i : sublora.lora_b.shape[1]
                    * (i + 1),
...
@pytest.mark.parametrize(
    "scaling_factors", [(1.0,), (4.0,), (4.0, 8.0), (6.0, 1.0)]
)
...
def test_rotary_embedding_long_context(
    dist_init,
    num_loras,
    device,
    scaling_factors,
    ...
    linear_rope = get_rope(
        head_size,
        rotary_dim,
        max_position,
        base,
        is_neox_style,
        {"rope_type": "linear", "factor": scaling_factors},
    )
    ...
    long_lora_context = LongContextLoRAContext(
        list(scaling_factors), rotary_dim
    )
...
def test_vocab_parallel_embedding_indices(tp_size, seed):
    ...
    for tp_rank in range(tp_size):
        ...
        assert (
            shard_indices.added_vocab_start_index == last_added_vocab_end_index
        )
