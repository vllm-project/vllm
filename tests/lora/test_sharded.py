...
class Worker:
    ...
    def create_column_parallel_packed_layer(self, repeats, dtype, device, linear_method=None, fully_sharded=True):
        if repeats == 2:
            linear = MergedColumnParallelLinear(4096, [4096] * repeats, bias=False, linear_method=linear_method)
            linear.weight.data = torch.rand_like(linear.weight.data, dtype=dtype, device=device) * HIGH + LOW
            lora_linear = MergedColumnParallelLinearWithShardedLoRA(linear) if fully_sharded else MergedColumnParallelLinearWithLoRA(linear)
        else:
            linear = QKVParallelLinear(4096, 64, 32, bias=False, linear_method=linear_method)
            linear.weight.data = torch.rand_like(linear.weight.data, dtype=dtype, device=device) * HIGH + LOW
            lora_linear = QKVParallelLinearWithShardedLora(linear) if fully_sharded else QKVParallelLinearWithLora(linear)
    ...
    @torch.inference_mode()
    def test_column_parallel_packed_lora(
        self,
        num_loras,
        repeats,
        dtype
        ...
        id_to_index = get_random_id_to_index(num_loras, self.lora_config.max_loras)
        linear, lora_linear = self.create_column_parallel_packed_layer(repeats, dtype, device)
        ...
        for i in range(repeats):
            lora_a = torch.rand((4096, self.lora_config.max_lora_rank), dtype=dtype, device=device) * HIGH + LOW
            if repeats == 2:
                lora_b = torch.rand((self.lora_config.max_lora_rank, 4096), dtype=dtype, device=device) * HIGH + LOW
            else:
                ...
                lora_b = torch.rand((self.lora_config.max_lora_rank, out_size), dtype=dtype, device=device) * HIGH + LOW
        ...
        for lora_idx in range(self.lora_config.max_loras):
            lora_linear.set_lora(lora_idx, [lora[0] for lora in loras], [lora[1] for lora in loras], None)
        ...
        for input_, lora_idx in zip(inputs, mapping_info[0]):
            ...
            for r in range(repeats):
                ...
                result = (input_.to(dtype=torch.float32) @ lora_a.to(dtype=torch.float32)) @ lora_b.to(dtype=torch.float32)
    ...
    @torch.inference_mode()
    def create_random_linear_parallel_layer(self, orientation, dtype, device, linear_method=None, fully_sharded=True):
        if orientation == "row":
            linear = RowParallelLinear(4096, 4096, input_is_parallel=False, reduce_results=True, bias=False, linear_method=linear_method)
            linear.weight.data = torch.rand_like(linear.weight.data, dtype=dtype, device=device) * HIGH + LOW
            lora_linear = RowParallelLinearWithShardedLoRA(linear) if fully_sharded else RowParallelLinearWithLoRA(linear)
        else:
            linear = ColumnParallelLinear(4096, 4096, gather_output=True, bias=False, linear_method=linear_method)
            linear.weight.data = torch.rand_like(linear.weight.data, dtype=dtype, device=device) * HIGH + LOW
            lora_linear = ColumnParallelLinearWithShardedLoRA(linear) if fully_sharded else ColumnParallelLinearWithLoRA(linear)
        linear.weight.data = broadcast(linear.weight.data, src=0)
        lora_linear.create_lora_weights(self.lora_config.max_loras, self.lora_config)
    ...
    def test_lora(
        ...
        device = torch.device(f'cuda:{torch.distributed.get_rank()}')
        id_to_index = get_random_id_to_index(num_loras, self.lora_config.max_loras)
        linear, lora_linear = self.create_random_linear_parallel_layer(orientation, dtype, device)
        ...
        for lora_idx in range(self.lora_config.max_loras):
            ...
            start, end = shard * get_tensor_model_parallel_rank(), shard*(get_tensor_model_parallel_rank()+1)
            if orientation == 'col':
                lora_linear.set_lora(lora_idx, lora_a, lora_b[:, start:end], embeddings_tensor=None)
            else:
                lora_linear.set_lora(lora_idx, lora_a, lora_b, embeddings_tensor=None)
            loras.append((lora_a, lora_b))
        ...
        for input_, lora_idx in zip(inputs, mapping_info[0]):
            ...
            # on my gpu's, this computation in fp16 leads to diff results on each. Off by exactly 0.0625 or 0.125. fp32 removes those differences. 
            # Inherently less comparable to punica, but, better than having this issue
            result = (input_.to(dtype=torch.float32) @ lora[0].to(dtype=torch.float32)) @ lora[1].to(dtype=torch.float32)
    ...
    def speed(self, num_loras, inputs_per_lora):
        ...
        linear, lora_linear = self.create_random_linear_parallel_layer('col', torch.float16, device, fully_sharded=True)
        lora_linear.set_mapping(*mapping_info, )
        fs_times = self.profile_linear(lora_linear, inputs, steps)

        linear, lora_linear = self.create_random_linear_parallel_layer('col', torch.float16, device, fully_sharded=False)
        lora_linear.set_mapping(*mapping_info, )
        times = self.profile_linear(lora_linear, inputs, steps)
        if get_tensor_model_parallel_rank() == 0:
            print(f'num loras, inputs per lora, rank [{num_loras}, {inputs_per_lora}, {self.lora_config.max_lora_rank}], \
                    fully sharded col: {sum(fs_times)/steps} ms | partially sharded col: {sum(times)/steps} ms')

        ############     TEST FULLY SHARDED ROW     ############
        linear, lora_linear = self.create_random_linear_parallel_layer('row', torch.float16, device, fully_sharded=True)
        lora_linear.set_mapping(*mapping_info, )
        fs_times = self.profile_linear(lora_linear, inputs, steps)

        linear, lora_linear = self.create_random_linear_parallel_layer('row', torch.float16, device, fully_sharded=False)
        lora_linear.set_mapping(*mapping_info, )
        times = self.profile_linear(lora_linear, inputs, steps)
        if get_tensor_model_parallel_rank() == 0:
            print(f'num loras, inputs per lora, rank [{num_loras}, {inputs_per_lora}, {self.lora_config.max_lora_rank}], \
        ...
        linear, lora_linear = self.create_column_parallel_packed_layer(2, torch.float16, device, fully_sharded=True)
        linear.gather_output = True
        lora_linear.set_mapping(*mapping_info, )
        fs_times = self.profile_linear(lora_linear, inputs, steps)

        linear, lora_linear = self.create_column_parallel_packed_layer(2, torch.float16, device, fully_sharded=False)
        linear.gather_output = True
        lora_linear.set_mapping(*mapping_info, )
        times = self.profile_linear(lora_linear, inputs, steps)
        ...
        linear, lora_linear = self.create_column_parallel_packed_layer(3, torch.float16, device, fully_sharded=True)
        linear.gather_output = True
        lora_linear.set_mapping(*mapping_info, )
        fs_times = self.profile_linear(lora_linear, inputs, steps)

        linear, lora_linear = self.create_column_parallel_packed_layer(3, torch.float16, device, fully_sharded=False)
        linear.gather_output = True
        lora_linear.set_mapping(*mapping_info, )
        times = self.profile_linear(lora_linear, inputs, steps)
        if get_tensor_model_parallel_rank() == 0:
...
def test_sharded_layers(rank, tp_size, max_loras, orientation='col', repeats=None, speed=False):
    engine_args = EngineArgs(model='mistralai/Mistral-7B-Instruct-v0.2', # "meta-llama/Llama-2-7b-hf",
                             enable_lora=True,
    ...
    driver_ip = get_ip()
    for bundle_id, bundle in enumerate(parallel_config.placement_group.bundle_specs):
    ...
    if speed:
        run_workers(driver_worker, workers, 'speed', num_loras=max_loras, inputs_per_lora=32)
    elif not repeats:
        run_workers(driver_worker, workers, 'test_lora', num_loras=max_loras, orientation=orientation, dtype=torch.float16)
    else:
        run_workers(driver_worker, workers, 'test_column_parallel_packed_lora', num_loras=max_loras, repeats=repeats, dtype=torch.float16)
