...
def test_load_dora_checkpoint_from_huggingface(dora_files):
    ...
    assert len(magnitude_vectors) > 0, (
        "No magnitude vectors found in DoRA adapter"
    )

    # Load the adapter
    peft_helper = PEFTHelper.from_local_dir(
        lora_path, max_position_embeddings=4096
    )
    assert peft_helper.use_dora is True, (
        "DoRA flag not set correctly in PEFTHelper"
    )
    ...
    for module_name, lora_weights in lora_model.loras.items():
        ...
        assert hasattr(lora_weights, "magnitude_param"), (
            f"No magnitude parameter for {module_name}"
        )
        assert lora_weights.magnitude_param is not None, (
            f"Magnitude parameter is None for {module_name}"
        )
...
def test_dora_weight_structure_in_huggingface(dora_files):
    ...
    for magnitude_key in magnitude_vector_keys:
        ...
        assert is_magnitude, (
            f"Failed to identify {magnitude_key} as a magnitude vector"
        )
        ...
        for key in all_keys:
            if key.startswith(
                magnitude_key.split(".lora_magnitude_vector")[0]
            ) and key.endswith("lora_A.weight"):
                lora_a_pattern = key
            elif key.startswith(
                magnitude_key.split(".lora_magnitude_vector")[0]
            ) and key.endswith("lora_B.weight"):
                lora_b_pattern = key

        # Assert that both LoRA A and B weights exist
        assert lora_a_pattern is not None, (
            f"Missing LoRA A weight for module {module_name}"
        )
        assert lora_b_pattern is not None, (
            f"Missing LoRA B weight for module {module_name}"
        )
        ...
        assert magnitude_tensor.dim() == 1, (
            f"Magnitude should be 1D for {module_name}"
        )

        # LoRA matrices should be 2D
        assert lora_a_tensor.dim() == 2, (
            f"LoRA A should be 2D for {module_name}"
        )
        assert lora_b_tensor.dim() == 2, (
            f"LoRA B should be 2D for {module_name}"
        )
...
def test_dora_vector_values_in_huggingface(dora_files):
    ...
    for magnitude_key in magnitude_keys:
        ...
        assert torch.all(magnitude_tensor >= 0), (
            f"Found negative magnitude values in {magnitude_key}"
        )
