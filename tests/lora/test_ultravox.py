...
def mk_llama3_1_8b_ultravox_chess_lora(source_repo, target_path):
    ...
    shutil.copyfile(
        path.join(source_repo, config_file), path.join(target_path, config_file)
    )
    return target_path
...
def test_ultravox_lora(vllm_runner):
    ...
    with TemporaryDirectory() as temp_ultravox_lora_dir:
        ...
        with vllm_runner(
            ...
            ultravox_outputs: list[tuple[list[int], str]] = (
                vllm_model.generate_greedy(
                    [
                        _get_prompt(
                            0, PROMPT, VLLM_PLACEHOLDER, ULTRAVOX_MODEL_NAME
                        )
                    ],
                    256,
                    lora_request=LoRARequest(
                        str(1), 1, llama3_1_8b_ultravox_chess_lora
                    ),
                )
            )
