...
"""
Integration tests for SnowflakeGteNewModel (GteModel) with LoRA adapters.

Tests verify that Snowflake/snowflake-arctic-embed-m-v2.0 can load and use
LoRA adapters for embedding tasks, producing distinct outputs per adapter.
"""
...
SNOWFLAKE_MODEL = "Snowflake/snowflake-arctic-embed-m-v2.0"

# SnowflakeGteNewModel config: hidden_size=768, num_heads=12, num_layers=12
...
def _create_random_snowflake_lora(save_dir: str, seed: int = 0):
    """Generate a synthetic PEFT LoRA adapter for SnowflakeGteNewModel."""
    ...
    adapter_config = {
        "peft_type": "LORA",
        "auto_mapping": None,
        "base_model_name_or_path": SNOWFLAKE_MODEL,
        "revision": None,
        "task_type": None,
        "inference_mode": True,
        "r": LORA_RANK,
...
def snowflake_lora_adapter_1(tmp_path_factory):
    """Generate a random LoRA adapter (seed=42)."""
    adapter_dir = str(tmp_path_factory.mktemp("snowflake_lora_1"))
    _create_random_snowflake_lora(adapter_dir, seed=42)
    return adapter_dir


@pytest.fixture(scope="session")
def snowflake_lora_adapter_2(tmp_path_factory):
    """Generate a different random LoRA adapter (seed=123)."""
    adapter_dir = str(tmp_path_factory.mktemp("snowflake_lora_2"))
    _create_random_snowflake_lora(adapter_dir, seed=123)
    return adapter_dir


def create_snowflake_llm(enable_lora: bool = True, max_loras: int = 4):
    """Create a SnowflakeGteNewModel LLM instance with optional LoRA support."""
    return vllm.LLM(
...
def test_snowflake_lora_basic_inference(snowflake_lora_adapter_1):
    """Test that SnowflakeGteNewModel can load and run with a LoRA adapter."""
    llm = create_snowflake_llm(enable_lora=True)

    lora_request = LoRARequest(
        lora_name="adapter-1",
        lora_int_id=1,
        lora_path=snowflake_lora_adapter_1,
    )
...
@create_new_process_for_each_test()
def test_snowflake_lora_changes_output(snowflake_lora_adapter_1):
...
@create_new_process_for_each_test()
def test_snowflake_multi_lora(snowflake_lora_adapter_1, snowflake_lora_adapter_2):
    """Test that different LoRA adapters produce different embeddings."""
    llm = create_snowflake_llm(enable_lora=True, max_loras=4)
    ...
    lora_request_2 = LoRARequest(
        lora_name="adapter-2",
        lora_int_id=2,
        lora_path=snowflake_lora_adapter_2,
    )
...
@create_new_process_for_each_test()
def test_snowflake_multi_lora_mixed_batch(
    snowflake_lora_adapter_1, snowflake_lora_adapter_2
):
