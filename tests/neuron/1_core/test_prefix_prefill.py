...
def ref_softmax(x: torch.Tensor,
    ...
    if mixed_precision:
        sum_value = torch.sum(exp.astype(torch.float32), dim=dim,
...
def sample_inputs(
    ...
    b_start_loc = torch.cumsum(torch.tensor([0] + query_lens[:-1], dtype=torch.long),
                               dim=0)
    # copy kv to cache
...
def test_contexted_kv_attention(
    ...
    with monkeypatch.context() as m:
        ...
        B_P_SIZE = 128
        assert (large_tile_size
                >= B_P_SIZE), f"Expect {large_tile_size=} to be larger than {B_P_SIZE=}"
