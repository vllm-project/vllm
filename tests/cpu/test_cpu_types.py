...
import pytest
import torch
from cputypes import (bf16vec8, bf16vec16, bf16vec32, fp32vec4, fp32vec8,
                      fp32vec16, int8vec16, int32vec16)
...
def ctor_f(meta, arr_s):
    return {
        'in': np.arange(arr_s, arr_s + meta['size'], 1, dtype=meta['type']),
...
def ctor_bf16_f(meta, arr_s):
    ...
    return {
        'in': tensor_bf16_uint16.numpy(),
        'in2': arr_fp32,
        'gt': tensor_bf16.to(torch.float32).numpy().astype(np.float32),
        'out': np.zeros((meta['size'], ), dtype=meta['type']),
        'msg': f'{meta["vect"]} ctor '
    }
...
class TestBF16Vec8:
    ...
    def test_ctor_vect(self, ctor_bf16_f, arr_s, vect2, size2):
        ...
        assert np.allclose(ctor_bf16_f['in'], ctor_bf16_f['out'],
                           atol=1e0), f"{ctor_bf16_f['msg']}FP32Vec8 {arr_s}"
...
class TestBF16Vec16:
    ...
    @pytest.mark.parametrize(
        'arr_s', [1, 100, 2000.5, 3000.5, 4000.5, 5000.5, 6000.5, 7000.5])
    @pytest.mark.parametrize('meta', [meta])
    def test_ctor_vect(self, ctor_bf16_f, arr_s, vect2, size2):
        vec2 = vect2(ctor_bf16_f['in2'][:size2])
        vec = self.meta['vect'](vec2)
        ...
        # no more than 1 in uint16 format difference from torch due to rounding
        assert np.allclose(ctor_bf16_f['in'], ctor_bf16_f['out'],
                           atol=1e0), f"{ctor_bf16_f['msg']}FP32Vec16 {arr_s}"
    ...
    def test_save_n(self, ctor_f, arr_s, n):
        ...
        gt = np.concatenate((np.arange(arr_s,
                                       arr_s + n,
                                       1,
                                       dtype=self.meta['type']),
                             np.zeros((self.meta['size'] - n, ),
                                      dtype=self.meta['type'])))
...
class TestBF16Vec32:
    ...
    def test_ctor_vect(self, ctor_f, arr_s, vect2, size2):
        ...
        vec.save(ctor_f['out'])
        gt = np.tile(
            np.arange(arr_s, arr_s + size2, 1, dtype=self.meta['type']),
...
class TestFP32Vec8:
    ...
    @pytest.mark.parametrize(
        'arr_s', [1, 100.8, 200.5, -2000.5, 3000.5, -3000.5, -5000.5])
    @pytest.mark.parametrize('meta', [meta])
    def test_ctor_bf16vec8(self, ctor_bf16_f, arr_s):
        ...
        assert np.array_equal(
            ctor_bf16_f['out'],
            ctor_bf16_f['gt']), f"{ctor_bf16_f['msg']}BF16Vec8 {arr_s}"
    ...
    def test_func(self, ctor_f, f_name, f_vec, f_gt):
        ...
        if not isinstance(vec2, float):
            ...
            assert np.array_equal(f_gt(ctor_f['in']),
                                  ctor_f['out']), ctor_f['msg'] + f_name
        else:
            assert np.array_equal(f_gt(ctor_f['in']),
                                  vec2), ctor_f['msg'] + f_name
    ...
    def test_ctor_vects(self, ctor_f, arr_s, vect2, size2):
        vec2 = vect2(ctor_f['in'][:size2])
        vecs = [vec2 for _ in range(int(self.meta['size'] / size2))]
        vec = self.meta['vect'](*vecs)
        vec.save(ctor_f['out'])
        gt = np.tile(
            np.arange(arr_s, arr_s + size2, 1, dtype=self.meta['type']),
            int(self.meta['size'] / size2))
...
class TestINT8Vec16:
    ...
    @pytest.mark.parametrize('arr_s',
                             [1, 111, 121, 131, -11, -128, -138, -158])
