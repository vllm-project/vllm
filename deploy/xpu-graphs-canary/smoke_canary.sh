#!/usr/bin/env bash
# XPU graphs canary smoke (TRITON_ATTN + PIECEWISE).
# Run on an XPU host with vllm installed. Does not touch Ornith production.
set -euo pipefail
...
RESULTS_DIR="${RESULTS_DIR:-${ROOT}/deploy/xpu-graphs-canary/results}"
mkdir -p "${RESULTS_DIR}"
...
export VLLM_XPU_GRAPH_FORCE_PIECEWISE="${VLLM_XPU_GRAPH_FORCE_PIECEWISE:-1}"
export ZE_AFFINITY_MASK="${ZE_AFFINITY_MASK:-0}"

VLLM_BIN="${VLLM_BIN:-$(command -v vllm)}"
PYTHON_BIN="${PYTHON_BIN:-$(command -v python3)}"

echo "=== canary smoke MODEL=${MODEL} PORT=${PORT} ===" | tee "${SMOKE_LOG}"
...
print("xpu_available", torch.xpu.is_available() if hasattr(torch, "xpu") else False)
print("supports_xpu_graph", supports_xpu_graph())
if not supports_xpu_graph():
    raise SystemExit("FAIL: supports_xpu_graph() is False (need torch >= 2.11.dev)")
PY

# Kill any prior canary on this port
...
grep -E "Triton backend|TRITON_ATTN|XPU Graph|PIECEWISE|FORCE_PIECEWISE|Flash Attention" \
