...
def _make_sliding_window_causal_mask(
    ...
    if past_key_values_length > 0:
        # kv_mask = torch.ones(tgt_len, past_key_values_length, dtype=dtype, device=device)
        # kv_mask = torch.triu(kv_mask, diagonal=past_key_values_length - sliding_window)
        # kv_mask = torch.log(kv_mask).to(dtype)
        kv_mask = torch.zeros(tgt_len, past_key_values_length, dtype=dtype, device=device)
...
class MistralAttention(nn.Module):
    ...
    def forward(
        ...
        #     for ki in range(kv_seq_len):
        #         if kv_seq_len - ki < qi:
        #             attn_weights[:, :, qi, ki] += torch.finfo(attn_we)
...
class MistralModel(MistralPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(
        ...
        if attention_mask is not None:
            ...
            # print('expanded_attn_mask.shape:', expanded_attn_mask.shape)
