...
def save_autotuned_configs(
    results: dict[str, AutotuneResult],
    platform: str,
    config_manager: ConfigManager,
) -> dict[str, AutotuneResult]:
    """Save autotuned configs and return updated results with save errors."""
    ...
    for kernel_name, result in results.items():
        if not result.configs:
            logger.debug("No configs to save for kernel '%s'", kernel_name)
            updated_results[kernel_name] = result
            continue

        logger.debug(
            "Saving configs for '%s': %s", kernel_name, list(result.configs.keys())
        )
        try:
            config_manager.save_configs(kernel_name, platform, result.configs)
            logger.info(
                "Saved %d configs for kernel '%s' on platform '%s'",
                len(result.configs),
                kernel_name,
                platform,
            )
            ...
            logger.error(
                "Failed to save configs for kernel '%s': %s",
                kernel_name,
                e,
            )
            updated_results[kernel_name] = replace(
                result,
                status="error",
                message=f"Failed to save configs: {e}",
            )
...
def main():
    ...
    results = save_autotuned_configs(results, platform, config_manager)
