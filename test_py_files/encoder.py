...
class XCodeEncConfig(PretrainedConfig):
    model_type = "xcodeenc"
    ...
    def __init__(
        ...
        use_final_norm: bool = False,  # Add option to control final norm
        **kwargs,
    ):
        ...
        # Override num_hidden_layers to match the actual number of encoder layers
        # Remove it from kwargs if it exists to avoid conflicts
        kwargs.pop('num_hidden_layers', None)
        other_config_dict['num_hidden_layers'] = enc_num_layers
...
class XCodeEncForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "enc"):
        super().__init__()
        config = vllm_config.model_config.hf_config
        quant_config = vllm_config.quant_config
        ...
        self.quant_config = quant_config
        self.enc = XCodeModel(vllm_config=vllm_config,
        ...
        self.make_empty_intermediate_tensors = (
            self.enc.make_empty_intermediate_tensors)
    ...
    def forward(
        ...
        data = self.enc(input_ids, positions, intermediate_tensors,
                                   inputs_embeds)
        return data
    ...
    ) -> Optional[Union[torch.Tensor, IntermediateTensors]]:
        return hidden_states
    def load_weights(self, weights: Iterable[tuple[str,
                                                   torch.Tensor]]) -> set[str]:
