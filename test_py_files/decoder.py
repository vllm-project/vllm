...
class XCodeDecForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.quant_config = quant_config
        self.model = XCodeModel(vllm_config=vllm_config,
    ...
    def load_weights(self, weights: Iterable[tuple[str,
                                                   torch.Tensor]]) -> set[str]:
        # Create a mapping to strip the 'dec.' prefix from checkpoint parameter names
        weights_with_stripped_prefix = []
        for name, weight in weights:
            if name.startswith("dec."):
                name = name[4:]  # Remove 'dec.' prefix
            weights_with_stripped_prefix.append((name, weight))
        ...
        return loader.load_weights(weights_with_stripped_prefix)