...
def test_accuracy(logits, k, p):
    ...
    original_logits = apply_top_k_top_p(input_logits_torch, k, p)
    triton_logits = apply_top_k_top_p_triton(input_logits_triton, k, p)

    torch.cuda.synchronize()
    is_correct = torch.allclose(triton_logits, original_logits)

    if not is_correct:
        print_to_log(r_str("Error: logits are not close"), log_file)
        error_mask = torch.abs(triton_logits - original_logits) > 1e-5
        ...
        logits_to_show = torch.sort(triton_logits[error_rows],
...
def test_time(logits, k, p, num_runs=30, num_warmup=5):
    ...
    input_logits_torch = [logits.clone().detach() for _ in range(num_runs)]
    ...
    torch.cuda.synchronize()
    torch_time_taken = start.elapsed_time(end) / num_runs

    input_logits_triton = [logits.clone().detach() for _ in range(num_runs)]
    ...
    torch.cuda.synchronize()
    triton_time_taken = start.elapsed_time(end) / num_runs
...
if __name__ == "__main__":
    ...
    vocab_size_list = [4096, 16384, 65536, 262144, 102400, 128256]
