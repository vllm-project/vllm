...
from vllm.v1.sample.ops.topk_topp_sampler import apply_top_k_top_p, apply_top_k_top_p_triton
...
def test_accuracy(logits, k, p):
    ...
    if not is_correct:
        ...
        print_to_log(f"logits: {torch.sort(logits[error_rows], descending=True).values[:row_to_show, :50]}", log_file)
        print_to_log(f"original_logits: {torch.sort(original_logits[error_rows], descending=True).values[:row_to_show, :50]}", log_file)

    return is_correct

def test_time(logits, k, p, num_runs=256):
    # We must clone the logits for each run to avoid modifying the original logits
    ...
    start_time = time.time()
    for _ in range(num_runs):
        input_logits_triton[_] = apply_top_k_top_p_triton(input_logits_triton[_], k, p)
    torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    k_list = [None, "RAND"] + [i for i in range(1, 10)] + [i for i in range(20, 210, 30)]
    log_file = f"triton_topk_topp_test_{date_str}.log"
    csv_file = f"triton_topk_topp_test_{date_str}.csv"

    print_to_log(y_str(f"Testing TopKTopPSampler with Triton"), log_file)
    print_to_log(y_str(f"batch_size_list:") + f"{batch_size_list}", log_file)
    print_to_log(y_str(f"vocab_size_list:") + f"{vocab_size_list}", log_file)
    print_to_log(y_str(f"p_list:") + f"{p_list}", log_file)
    print_to_log(y_str(f"k_list:") + f"{k_list}", log_file)
    print_to_log(y_str(f"log_file:") + f"{log_file}", log_file)
    print_to_log(y_str(f"csv_file:") + f"{csv_file}", log_file)

    with open(csv_file, "w") as f:
        f.write("dist_generator,batch_size,vocab_size,p,k,is_correct,torch_time_taken,triton_time_taken,speedup\n")

    for batch_size, vocab_size, p, k in product(batch_size_list, vocab_size_list, p_list, k_list):
        ...
        for dist_generator, logits in logits_list:
            print_to_log(y_str(f"--------------------------------"), log_file)
            print_to_log(g_str(f"Testing ") + f"{dist_generator}" 
                         + y_str(f" with batch_size: ") + f"{batch_size}" 
                         + y_str(f" vocab_size: ") + f"{vocab_size}" 
                         + y_str(f" p: ") + f"{p}" 
                         + y_str(f" k: ") + f"{k}", log_file)
            is_correct = test_accuracy(logits, k_tensor, p_tensor)
            if not is_correct:
                print_to_log(f"Error: logits are not close for batch_size: {batch_size}, vocab_size: {vocab_size}, dist_generator: {dist_generator}, p: {p}, k: {k}", log_file)
            torch_time_taken, triton_time_taken = test_time(logits, k_tensor, p_tensor)
            print_to_log(b_str(f"torch_time_taken: ") + f"{torch_time_taken}", log_file)
            print_to_log(b_str(f"triton_time_taken: ") + f"{triton_time_taken}", log_file)
            print_to_log(g_str(f"Triton Speedup over Torch: ") + f"{torch_time_taken / triton_time_taken:.8f}x", log_file)
            with open(csv_file, "a") as f:
                f.write(f"{dist_generator},{batch_size},{vocab_size},{p},{k},{is_correct},{torch_time_taken},{triton_time_taken},{torch_time_taken / triton_time_taken:.8f}\n")
