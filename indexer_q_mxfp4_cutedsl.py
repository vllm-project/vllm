...
def _compile_indexer_q_mxfp4(
    ...
    cos_div = math.gcd(128 // cos_sin_dtype.width, rope_dim)
    cos_sin_cache = make_fake_tensor(
        cos_sin_dtype, (max_pos, rope_dim), divisibility=cos_div
    )
...
def fused_indexer_q_rope_quant(
    ...
    if not use_fp4:
        raise NotImplementedError(
            "indexer_q_mxfp4_cutedsl only implements use_fp4=True"
        )

    num_tokens, num_heads, head_dim = q.shape
    rope_dim = cos_sin_cache.shape[-1]
    q_fp4 = q.new_empty((num_tokens, num_heads, head_dim // 2), dtype=torch.uint8)
    q_scale = q.new_empty(
        (num_tokens, num_heads, head_dim // MXFP4_BLOCK_SIZE), dtype=torch.uint8
    )
    ...
    scale = float(softmax_scale * head_scale)
    compiled(
        positions,
        q,
        cos_sin_cache,
