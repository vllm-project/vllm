...
def test_harmony_utils():
    ...
    try:
        ...
        print(f"✓ Encoded reasoning step: {encoded[:50]}..." if len(encoded) > 50 else f"✓ Encoded reasoning step: {encoded}")
...
def test_reasoning_parser():
    ...
    try:
        from vllm.transformers_utils.openai_reasoning_parser import (
            OpenAIReasoningParser, extract_reasoning_and_final
        )
        ...
        print(f"✓ Parsed reasoning: {reasoning[:50]}..." if reasoning and len(reasoning) > 50 else f"✓ Parsed reasoning: {reasoning}")
