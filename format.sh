...
void single_query_cached_kv_attention_launcher(
  ...
  int head_size = query.size(2);
	@@ -388,20 +421,19 @@ void single_query_cached_kv_attention_launcher(
  ...
  constexpr int NUM_WARPS = NUM_THREADS / WARP_SIZE;
  int padded_max_context_len = ((4096 + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE;
  int logits_size = padded_max_context_len * sizeof(float);
  int outputs_size = (NUM_WARPS / 2) * head_size * sizeof(float);
  // Python-side check in vllm.worker.worker._check_if_can_support_max_seq_len
	@@ -411,110 +443,120 @@ void single_query_cached_kv_attention_launcher(
