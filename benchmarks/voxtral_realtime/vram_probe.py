...
def main():
    ...
    print("elapsed_s,vram_mib,enginecore_rss_mib,kv_usage,running,host_avail_mib",
          flush=True)
    while True:
        ...
        if el > duration:
            break
        kv, run = metrics(port)
        print(f"{el:.0f},{nvidia_vram_mib()},{enginecore_rss_mib()},{kv:.4f},"
              f"{run:.0f},{host_avail_mib()}", flush=True)
        time.sleep(interval)
