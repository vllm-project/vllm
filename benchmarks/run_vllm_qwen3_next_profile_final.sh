#!/usr/bin/env bash
# Final A/B benchmark of the BlockScale SplitK Zero-Init fusion pass on
# Qwen3-Next-80B-A3B-Instruct-FP8, on top of the full kernel set:
#   - CK kernels       : 1046 rows, 88 with splitK > 0
#   - cktile kernels   : 436 rows  (prefill / wider-M shapes)
#   - FSE              : ON (shared expert folded into routed-experts FusedMoE)
#
...
#   <results_dir>/<mode>.bench.log           bench stdout
#   <results_dir>/<mode>.server.log          vllm serve stdout (incl. fusion
#                                            pass attribution lines)
#   <results_dir>/<mode>/dp0_*.pt.trace.json torch profiler trace
#   <results_dir>/<mode>/profiler_out_0.txt  cuda_time_total table
#
# Usage:
#   bash benchmarks/run_vllm_qwen3_next_profile_final.sh [<results_dir>]
set -euo pipefail

VLLM_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
AITER_ROOT="${AITER_ROOT:-${HOME}/dev/aiter}"
RESULTS_DIR="${1:-${VLLM_ROOT}/benchmarks/zero_init_demo_results/profile_qwen3_next_final_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${RESULTS_DIR}"

# Production tune with both CK (1046) and cktile (436) rows. Filtered to the
# kernels currently present in aiter's JIT-built .so set.
...
# vllm is now an editable install in the venv, so VLLM_ROOT no longer needs
# to be on PYTHONPATH; aiter still does because it isn't pip-installed.
...
# IMPORTANT: FSE on. Eliminates the standalone shared-expert linear path so
# the only residual blockscale GEMMs in the trace come from the post-grad FX
# graph that the fusion pass actually rewrites.
