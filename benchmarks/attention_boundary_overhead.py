#!/usr/bin/env python3
"""Benchmark attention boundary computation scalar overhead.

This script compares the current vectorized helper against local legacy-style
baselines that intentionally use Python scalar extraction in the hot path. It
is meant as a micro-benchmark for the boundary/routing computation only, not a
full model throughput benchmark.
"""
...
def _load_boundary_helper(repo: str) -> BoundaryFn:
    if repo == "vllm":
    ...
    return getattr(module, "_split_decode_prefill_boundary")


def _load_extend_helper() -> BoundaryFn:
    module = importlib.import_module("vllm.v1.attention.backends.utils")
    return getattr(module, "_split_decode_extend_prefill_boundary")


def _load_chunk_helper() -> Callable[[torch.Tensor, int], list[tuple[int, int]]]:
    module = importlib.import_module("vllm.v1.attention.backends.utils")
    return getattr(module, "split_prefill_chunks")
...
def _legacy_split_decode_extend_prefill_boundary(
    ...
    num_decode_tokens = (
        int(query_start_loc[first_extend].item())
    ...
    num_prefill_tokens = (
        num_tokens - int(query_start_loc[first_prefill].item())
        if first_prefill < num_reqs
        else 0
    )
...
def run(args: argparse.Namespace) -> None:
    ...
    boundary_helper = _load_boundary_helper(args.repo)
    extend_helper = _load_extend_helper() if args.repo == "vllm" else None
    chunk_helper = _load_chunk_helper() if args.repo == "vllm" else None
    ...
    for case in _boundary_cases(dtype):
        ...
        expected = _legacy_split_decode_prefill_boundary(
        ...
        legacy_us = _time_call(
            lambda: _legacy_split_decode_prefill_boundary(
                *legacy_args, **legacy_kwargs
            ),
            iters=args.iters,
            warmup=args.warmup,
        ...
        current_us = _time_call(
            lambda: boundary_helper(*legacy_args, **legacy_kwargs),
            iters=args.iters,
            warmup=args.warmup,
        )
    ...
    if extend_helper is not None:
        for case in _extend_cases(dtype):
            ...
            legacy_us = _time_call(
                lambda: _legacy_split_decode_extend_prefill_boundary(*call_args),
                iters=args.iters,
                warmup=args.warmup,
            )
            current_us = _time_call(
                lambda: extend_helper(*call_args),
                iters=args.iters,
                warmup=args.warmup,
            )
            _print_result(case.name, legacy_us, current_us)
    ...
    if chunk_helper is not None:
        ...
        for case in _chunk_cases():
            expected = _legacy_split_prefill_chunks(
                case.seq_lens, case.workspace_size
            )
            ...
            legacy_us = _time_call(
                lambda: _legacy_split_prefill_chunks(
                    case.seq_lens, case.workspace_size
                ),
                iters=chunk_iters,
                warmup=chunk_warmup,
            ...
            current_us = _time_call(
                lambda: chunk_helper(case.seq_lens, case.workspace_size),
                iters=chunk_iters,
                warmup=chunk_warmup,
            )
            _print_result(case.name, legacy_us, current_us)
