...
class ServerResponse(NamedTuple):
    ...
    num_chunks: int
    # RFC: Add usage info from API for accurate token counting
    prompt_tokens: int | None = None  # Actual tokens used by API
    completion_tokens: int | None = None  # Actual tokens generated by API
...
async def send_request(
    ...
    if stream:
        payload["stream"] = True
        # RFC: Enable usage info in streaming response for accurate token counting
    ...
    first_chunk = ""
    generated_text = ""
    # RFC: Collect usage info from API
        ...
        if http_status == HTTPStatus.OK:
            async for chunk_bytes in response.content:
                ...
                if chunk == "[DONE]":
                    ...
                    # RFC: Collect usage info from non-streaming response
                    if "usage" in data:
                    ...
                    # RFC: Collect usage info from streaming response
                    # Usage info is sent in the final chunk
    ...
    sr = ServerResponse(
        ...
        # RFC: Include usage info from API
...
def get_messages_token_count(
    ...
    try:
        token_ids = tokenizer.apply_chat_template(
            messages,
            tokenize=True,
            add_generation_prompt=True,  # Match API behavior
        )
        return len(token_ids)
    except Exception as e:
        # Fallback to old method if chat template is not available
        logger.warning(
            f"Failed to apply chat template for token counting: {e}. "
            "Falling back to simple content tokenization."
        )
        token_count = 0
        for m in messages:
...
async def send_turn(
    ...
    # RFC: Allow additional keys like "output_tokens" in messages
    ...
    if len(conversation_messages) > messages_to_use:
        ...
        if (
            ...
            # Get number of tokens from the dataset
            assistant_answer = conversation_messages[messages_to_use]
            assert assistant_answer["role"] == "assistant"

            # RFC: Use output_tokens from dataset if available (for reproducibility)
            # Otherwise, tokenize the content
            if "output_tokens" in assistant_answer:
                answer_num_tokens = assistant_answer["output_tokens"]
            else:
                answer_num_tokens = get_token_count(
                    tokenizer, assistant_answer["content"]
                )
    ...
    if response.prompt_tokens is not None:
        ...
        # Fallback: use chat template for accurate input token counting
        input_num_tokens = get_messages_token_count(tokenizer, messages)

    if response.completion_tokens is not None:
        output_num_tokens = response.completion_tokens
    else:
        # Fallback: tokenize output content
        output_num_tokens = get_token_count(tokenizer, response.content)
    ...
    if len(conversation_messages) > answer_index:
        ...
        # RFC: Record output_tokens for reproducibility
...
async def main() -> None:
    ...
    if args.output_file is not None:
        ...
        # RFC: output_tokens is now included in assistant messages for reproducibility
