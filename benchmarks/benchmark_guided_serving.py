...
async def get_request(
    ...
    theta = 1.0 / (request_rate *
                   burstiness) if request_rate != float("inf") else 0
...
def sample_requests(tokenizer: PreTrainedTokenizerBase,
    ...
    if args.dataset == 'xgrammar_bench':
        ...
        with open(args.json_schema_path) as f:
            schema = json.load(f)
        prompt = ("Generate an example of a user profile given the following "
...
def calculate_metrics(
    ...
    total_output_tokens = sum(
        len(tokenizer(r.generated_text, add_special_tokens=False).input_ids)
    ...
    for r in results:
        if r.success and r.itl:
            itls.extend([tl * 1000 for tl in r.itl])
    
    print(f"Total Output Tokens: {total_output_tokens}")
    print(f"Total ITLs: {len(itls)}")
...
async def run_benchmark(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    semaphore = asyncio.Semaphore(
        args.max_concurrency) if args.max_concurrency else None
    ...
    tasks = []
    async for request in get_request(requests, args.request_rate,
                                     args.burstiness):
...
def main():
    ...
    parser.add_argument("--burstiness",
                        ...
                        help=("Burstiness factor for request generation "
                              "(default: 1.0 for Poisson process)"))
    ...
    print(f"Completed requests: {metrics['completed_requests']}/"
          f"{metrics['total_requests']} ({metrics['completion_rate']:.2f}%)")
    print(f"Total Input Tokens: {metrics['total_input_tokens']}")
    print(f"Total Output Tokens: {metrics['total_output_tokens']}")
    print(
        f"Request throughput: {metrics['request_throughput']:.2f} requests/s")
    print(
        f"Total Token throughput: {metrics['token_throughput']:.2f} tokens/s")
    print(
        f"Output Token throughput: {metrics['output_throughput']:.2f} tokens/s"
    )
