...
from cpu_binding import BindingPolicy, CPU_Binding
...
def generate_yaml_file(
    cpuset_csv, num_alloc, idle_cpuset_csv, num_idle_cpus, args_cpuservice, args_output
):
    yaml = YAML()
...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "Generate override docker-compose YAML (x-sets) for single 'vllm-server'."
        )
    )
    ...
    ap.add_argument(
        ...
        help=(
            "CSV with columns: model_id,input length,output length,world_size,"
            "num_allocated_cpu"
        ),
    )
    ap.add_argument(
        ...
        help="Output compose YAML path",
    )
    ap.add_argument(
        "--cpuservice", help="name of the docker service binding on idle CPUs"
    )
    args = ap.parse_args()
    ...
    print(
        f"Wrote {args.output} for MODEL={model} "
        f"(num_allocated_cpu={cpu_binder.num_allocated_cpu})"
    )
