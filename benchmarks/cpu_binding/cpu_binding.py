...
class CPU_Binding:
    ...
    def __init__(self, csv_path: str = "cpu_binding_gnr.csv", use_hyperthread: bool = False):
        ...
        if self.libnuma_found and self.psutil_found:
            ...
            # Idle CPUs: initialize from per-NUMA CPU list; drop HT or non-HT according to use_hyperthread
            # Note: we keep idle pool aligned with node_to_cpus (not ranked) because we remove allocated CPUs anyway.
            ...
            if self.cards is not None:
                ...
                for card in self.cards[:self.world_size]:
                    if card['numa_node'] not in self.card_numa_list:
                        ...
                        print(f"NOT Append Card {card['card_id']} ({card['model']}) {card['numa_node']}:")
    ...
    def _load_pct_preferred_cpus_from_file(self) -> set[int]:
        ...
        default_pct = Path(__file__).resolve().parent / "priority_core_turbo" / "results" / "clos0_cpulist.txt"
        pct_path = os.environ.get("PCT_CPULIST_FILE", str(default_pct))

        p = Path(pct_path)
        if not p.exists():
            print(f"Warning: PCT cpulist file not found: {pct_path}. Falling back to non-PCT ordering.")
            return set()

        try:
            content = p.read_text(encoding="utf-8")
        except Exception as e:
            print(f"Warning: failed to read PCT cpulist file '{pct_path}': {e}. Falling back to non-PCT ordering.")
            return set()
    ...
    def pick_row_by_parameters(self, rows: list[dict], model: str, input_tok: str, output_tok: str,
        ...
        if not matches:
            ...
            matches = [r for r in rows if r.get('model_id', '').strip() == (model or "").strip()]
    ...
    def get_cpus_id_binding_based_on_numa_nodes(self, rank: int) -> str:
        ...
        if self.binding_policy is BindingPolicy.Evenly_on_NUMAs or self.cards is None:
            ...
            self.allocated_cpu_per_numa = self.num_allocated_cpu // len(self.card_numa_list)
