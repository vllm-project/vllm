...
def run(args, MKNs: Iterable[Tuple[int, int, int]], tp_size=None) -> Iterable[TMeasurement]:
    results = []
    dtype = args.dtype
    ...
    for m, k, n in MKNs:
        ...
        if tp_size is not None:
            with open(f"chunk_bench-{m}_{k}_{n}-{args.dtype}_{tp_size}.pkl", "wb") as f:
                pkl.dump(timers, f)
        else:
...
def run_range_bench(args):
    ...
    dim_sizes = [1, 16, 32, 64, 128, 256, 512, 1024]
...
Benchmark Cutlass GEMM.
    ...
    To run square GEMMs:
        python3 ./benchmarks/cutlass_benchmarks/dense_mm/mm_benchmarks.py --dtype fp8 square_bench --dim-start 128 --dim-end 512 --dim-increment 64
    
    To run constant N and K and sweep M:
        python3 ./benchmarks/cutlass_benchmarks/dense_mm/mm_benchmarks.py --dtype fp8 range_bench --dim-start 128 --dim-end 512 --dim-increment 64 --n-constant 16384 --k-constant 16384
    
    To run dimensions from a model:
        python3 ./benchmarks/cutlass_benchmarks/dense_mm/mm_benchmarks.py --dtype fp8 model_bench --models meta-llama/Llama-2-7b-hf --batch-sizes 16 --tp-sizes 1
