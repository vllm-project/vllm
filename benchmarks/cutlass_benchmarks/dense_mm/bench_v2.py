...
from utils import make_n_rand_tensors, to_fp8
...
def get_autogen_functions(dtype_str):
    import importlib
    ...
    # pattern = re.compile(r'vllm\._nm_cutlass_\d+_C')
    # matching_modules = [name for name in sys.modules.keys() if pattern.match(name)]
    
    # # Import or reimport all matching modules
    ...
    m_idx = 0
    m_name = f'vllm._nm_cutlass_{dtype_str}_{m_idx}_C'
...
def run_single_benchmark_process(kernel_config: Dict, gpu_id: int, queue: Queue, dtype_str: str):
    """
    ...
    try:
        ...
        if kernel_type == 'pytorch_mm':
            ...
            kernel_name, autogen_fn = get_autogen_functions(dtype_str)[kernel_num]
...
def benchmark_gpu_worker(gpu_id: int, task_queue: Queue, result_queue: Queue, dtype_str: str):
        ...
        while True:
            try:
                ...
                p = Process(target=run_single_benchmark_process, 
                          args=(kernel_config, gpu_id, process_queue, dtype_str))
                p.start()
...
def run_kernels_on_gpus(configs: List[Dict], dtype_str: str) \
    ...
    if MULTI_GPU_MULTI_PROCESS:
        ...
        for gpu_id in gpus_list:
            p = Process(target=benchmark_gpu_worker, args=(gpu_id, task_queue, result_queue, dtype_str))
            p.start()
            workers.append(p)
        ...
        while completed < total_tasks:
            ...
            if config['kernel_type'] == 'autogen_kernel':
                ...
                kernel_name = get_autogen_functions(dtype_str)[kernel_num][0]
...
def bench_dtype(dtype: torch.dtype, with_cuda_graph: Optional[int],
    ...
    timers = []
    gpus_list = [0]  # Using the same GPU list as original code
    ...
    dtype_str = 'fp8' if dtype == torch.float8_e4m3fn else \
                'bf16' if dtype == torch.bfloat16 else \
                'fp16' if dtype == torch.float16 else 'int8'
    ...
    results = run_kernels_on_gpus(all_configs, dtype_str)
...
def bench_v2(dtype: torch.dtype, with_cuda_graph: Optional[int],
             with_arg_pool: Optional[int], m: int, k: int, n: int, label: str,
             sub_label: str) -> Iterable[TMeasurement]:
    if dtype == torch.float8_e4m3fn or dtype == torch.int8:
        return bench_dtype(dtype, with_cuda_graph, with_arg_pool, m, k, n, label, sub_label)
