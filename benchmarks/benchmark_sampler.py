...
"""Sampler benchmark - testing core performance, strategies, penalties, FlashInfer."""
...
def benchmark_sampling_strategies(
    ...
    strategies = {
        "greedy": {"temp": 0.0},
        "random_temp_1.0": {"temp": 1.0},
        "top_k_50": {"temp": 1.0, "k": 50},
        "top_p_0.9": {"temp": 1.0, "p": 0.9}
    }
    ...
    for strategy_name, params in strategies.items():
        ...
        for batch_size in batch_sizes:
            ...
            for vocab_size in vocab_sizes:
                ...
                results["strategies"][strategy_name][batch_size][vocab_size] = {
...
def apply_penalties_and_sample(
    logits: torch.Tensor, 
...
def benchmark_penalties(
    ...
    penalties = {
        "no_penalty": {},
        "repetition_penalty_1.1": {"repetition_penalty": 1.1},
        "frequency_penalty_0.1": {"frequency_penalty": 0.1},
        "presence_penalty_0.1": {"presence_penalty": 0.1}
    }
    ...
    for penalty_name, penalty_params in penalties.items():
        ...
        for batch_size in batch_sizes:
            ...
            for vocab_size in vocab_sizes:
                ...
                prev_tokens = torch.randint(
                    0, vocab_size, (batch_size, 10), device=device
                )
                ...
                for _ in range(5):
                    logits = create_synthetic_logits(batch_size, vocab_size, device)
                    with torch.no_grad():
                        _ = apply_penalties_and_sample(
                            logits, prev_tokens, penalty_params
                        )
...
def benchmark_pytorch_native(
    ...
    for batch_size in batch_sizes:
        ...
        for vocab_size in vocab_sizes:
            print(f"  PyTorch native: batch_size={batch_size}, vocab_size={vocab_size}")
...
def benchmark_flashinfer_impl(
    ...
    for batch_size in batch_sizes:
        ...
        for vocab_size in vocab_sizes:
            print(f"  FlashInfer: batch_size={batch_size}, vocab_size={vocab_size}")
        ...
        for batch_size in results["batch_sizes"]:
            for vocab_size in results["vocab_sizes"]:
                ...
                for strategy_name, strategy_data in results["strategies"].items():
                    if (batch_size in strategy_data and 
                        vocab_size in strategy_data[batch_size]):
                        data = strategy_data[batch_size][vocab_size]
                        print(f"{strategy_name:<20} {data['avg_latency_ms']:<18.2f} "
        ...
        for batch_size in results["batch_sizes"]:
            for vocab_size in results["vocab_sizes"]:
                ...
                for penalty_name, penalty_data in results["penalties"].items():
                    if (batch_size in penalty_data and 
                        vocab_size in penalty_data[batch_size]):
                        data = penalty_data[batch_size][vocab_size]
                        print(f"{penalty_name:<25} {data['avg_latency_ms']:<18.2f} "
                              f"{data['p99_latency_ms']:<18.2f} "
                              f"{data['throughput_tokens_per_sec']:<20.2f}")
    ...
    if "flashinfer" in all_results:
        ...
        if "pytorch_native" in results and "flashinfer" in results:
            for batch_size in results["batch_sizes"]:
                for vocab_size in results["vocab_sizes"]:
                    print(f"\nBatch Size: {batch_size}, Vocab Size: {vocab_size}")
                    print(f"{'Implementation':<20} {'Avg Latency (ms)':<18} "
                          f"{'P99 Latency (ms)':<18} {'Throughput (tok/s)':<20}")
                    print("-" * 80)
                    
                    pytorch_data = results["pytorch_native"][batch_size][vocab_size]
                    flashinfer_data = results["flashinfer"][batch_size][vocab_size]
                    
                    print(f"{'PyTorch Native':<20} "
                          f"{pytorch_data['avg_latency_ms']:<18.2f} "
                          f"{pytorch_data['p99_latency_ms']:<18.2f} "
                          f"{pytorch_data['throughput_tokens_per_sec']:<20.2f}")
                    print(f"{'FlashInfer':<20} "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--benchmarks", 
                       type=str, 
                       nargs="+", 
                       default=["core", "strategies", "penalties", "flashinfer"],
                       choices=["core", "strategies", "penalties", "flashinfer"],
