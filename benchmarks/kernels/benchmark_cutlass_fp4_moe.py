...
def bench_run(results: list[benchmark.Measurement], model: str,
    ...
    w1_gs = torch.empty((num_experts,),
                           device=device,
                           dtype=torch.float32)
    w2_gs = torch.empty((num_experts,),
                           device=device,
                           dtype=torch.float32)
    a1_gs = torch.ones((num_experts,),
                           device=device,
                           dtype=torch.float32)
    a2_gs = torch.ones((num_experts,),
                           device=device,
                           dtype=torch.float32)
    ...
    def run_cutlass_moe_fp4(a: torch.Tensor, 
                            w1_fp4: torch.Tensor,
                            w2_fp4: torch.Tensor,
    ...
    def run_cutlass_from_graph(a: torch.Tensor,
    ...
    def run_triton_from_graph(a: torch.Tensor, w1: torch.Tensor,
                         ...
                         w2_scale: torch.Tensor, a_fp8_scale: torch.Tensor):
        with set_current_vllm_config(
                VllmConfig(parallel_config=ParallelConfig(
                    pipeline_parallel_size=1))):
    ...
    with torch.cuda.graph(triton_graph, stream=triton_stream):
        run_triton_from_graph(a, w1_fp8q_notransp, w2_fp8q_notransp,
                             topk_weights, topk_ids, w1_fp8scale,
                             w2_fp8scale, a_fp8_scale)
    torch.cuda.synchronize()
