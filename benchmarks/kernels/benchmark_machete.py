...
def marlin_create_bench_fn(bt: BenchmarkTensors) -> Callable:
    ...
    if bt.a.dtype.is_floating_point:
        ...
        if bt.w_ch_s is not None:
            ...
            s_ch = torch.ones(
                bt.w_ref.shape[1], dtype=torch.float32, device=device
            )
...
def bench_fns(
    label: str, sub_label: str, description: str, fns: list[Callable]
):
...
def bench(
    ...
    name_type_string = (
        f"W{types.weight_type}" + f"-A{terse_type_name(types.act_type)}"
    )
    if types.group_scale_type is not None:
    ...
    if sweep_schedules:
        ...
        for schedule in reversed(schedules):
            ...
            _SWEEP_SCHEDULES_RESULTS.loc[len(_SWEEP_SCHEDULES_RESULTS)] = (
                results_row
            )
