    ...
    if "torch-bf16" in provider:
        ...
        if "noquant" in provider:
            ...
            if "tensor-w-token-a" in provider:
                ...
                def run_quant():
                    return vllm_scaled_mm(a_fp8, b_fp8, scale_a_fp8,
                                          scale_b_fp8, dtype)
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: run_quant(),
                                                     quantiles=[0.5, 0.2, 0.8])

    # Calculate GB/s
    gbps = lambda ms: (2 * M * N * K + M * N) * a.element_size() * 1e-9 / (
        ms * 1e-3)
