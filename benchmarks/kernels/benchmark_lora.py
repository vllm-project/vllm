...
def make_rand_tensors(
    ...
    # LoRA weights column major
    Bs = [
        torch.rand(b_shape, dtype=b_dtype).to(device) for _ in range(num_slices)
    ]
...
def make_prompt_lora_mapping(
    ...
    if not sort_by_lora_id:
        return torch.randint(
            0, num_active_loras, (num_prompts,), dtype=torch.long
        )
...
class BenchmarkTensors:
    ...
    def convert_to_sgmv_benchmark_tensors(self):
        ...
        _, seq_lens = torch.unique_consecutive(
            self.token_lora_mapping, return_counts=True
        )
        ...
        prompt_lora_mapping = torch.unique_consecutive(self.prompt_lora_mapping)
    ...
    def as_v1_shrink_kwargs(self) -> dict[str, Any]:
        ...
        return {
            ...
            "token_indices_sorted_by_lora_ids": self.v1_kernel_meta.token_indices_sorted_by_lora_ids,
...
def bench_optype(
    ...
    describe_args = (
        ...
        if expand_fn_add_inputs is not None
        else ""
    )
    description = (
...
def print_timers(
    timers: list[TMeasurement], args: Optional[argparse.Namespace] = None
):
...
def run(args: argparse.Namespace, bench_ctxs: list[BenchmarkContext]):
    if args.cuda_graph_nops is not None:
        ...
        print(
            f"Benchmarking {args.cuda_graph_nops} invocations inside a CUDA "
            "Graph"
        )
    else:
...
if __name__ == "__main__":
    ...
    def add_common_command_args(p: argparse.ArgumentParser):
        ...
        p.add_argument(
            "--num-loras", nargs="+", type=int, default=DEFAULT_NUM_LORAS
        )
        p.add_argument(
            "--num-active-loras",
            type=int,
...
Benchmark LoRA kernels:
    ...
    range_parser.add_argument(
        "--hidden-sizes-increment", type=int, required=True
    )
